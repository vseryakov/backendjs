...
int CRegExp::RegFind(const TCHAR *str)
{
    ...
    if (regmust != NULL && _tcsstr(string, regmust) == NULL) return (-1);
...
int CRegExp::regmatch(TCHAR *prog)
{
    ...
    for (scan = prog; scan != NULL; scan = next) {
        ...
        case EXACTLY: {
            ...
            if (len > 1 && _tcsncmp(opnd, reginput, len) != 0) return (0);
        ...
        case ANYOF:
            ...
            if (*reginput == _T('\0') || _tcschr(OPERAND(scan), *reginput) == NULL) return (0);
            reginput++;
            break;
        ...
        case OPEN + 9: {
            ...
            if (regmatch(next)) {
                ...
                // already has.
                if (startp[no] == NULL) startp[no] = input;
                return (1);
            } else
                return (0);
            break;
...
TCHAR* CRegExp::GetReplaceString(const TCHAR* sReplaceExp)
{
    ...
    while ((c = *src++) != _T('\0')) {
        ...
        if (no < 0) {
            ...
            // Ordinary character.
            if (c == _T('\\') && (*src == _T('\\') || *src == _T('&'))) c = *src++;
            replacelen++;
        }

        else if (startp[no] != NULL && endp[no] != NULL && endp[no] > startp[no]) {
