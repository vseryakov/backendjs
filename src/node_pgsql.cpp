...
{
    PgSQLDatabase *db = (PgSQLDatabase *)req->data;
    LogDev("%p: fd=%d", db->handle, db->fd);
    Baton* baton = static_cast<Baton*>(db->poll.data);
    db->Close();
    if (baton) baton->Call("connection timeout");
}
