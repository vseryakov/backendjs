...
it('Issuer (correct - string)', async () => {
    ...
    const rc = await lib.jwt.verify(tok, secret, { alg: "HS256", iss: 'correct-issuer' })
...
it('HS512 sign & verify & decode', async () => {
    ...
    const rc = await lib.jwt.verify(tok.token, secret)
