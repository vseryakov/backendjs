...
`;
    ...
    it("test parameters", async () => {
        ...
        assert.partialDeepStrictEqual(db._config.sqlite.config, { arg1: 1, arg2: 2 });

        assert.partialDeepStrictEqual(db._config.sqlite1, { url: "a", max: 10, config: { test: "test" } })

        assert.partialDeepStrictEqual(db._config.sqlite, { config: { discoveryInterval: 30000, 'map.test': "test" } });
