...
describe("lib.validate checks", function () {
  ...
  it("string max/min and trunc", function () {
    const q = { a: "abcd", b: "ab", c: "abcd", o: { 1: 1 } };
    const s = {
      a: { max: 3, trunc: 1 },
      b: { min: 3, errmsg: "too short" },
      c: { max: 3, errmsg: "too long" },
      o: { type: "object", max: 10, errmsg: "too large" },
    };
    ...
    assert.strictEqual(lib.validate(q, { o: s.o })?.err, undefined);
    assert.strictEqual(lib.validate({ o: q }, { o: s.o })?.err?.message, "too large");
