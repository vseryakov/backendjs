...
const aws = {
    ...
    args: [
        ...
        { name: "config-secrets", type: "list", descr: "AWS Secrets Manager filters to load and parse as config before initializing the database pools, supports @..@ app.instance placeholders in filters", example: "production,production-@tag@,production-@role@" },
 ...
 *
 * When AWS environment is detected the `app.instance.type` is set to **aws**.
 *
 * @example
 *
 * # aws login
...
aws.configureServer = function(options, callback)
{
    ...
    var opts = lib.clone(options, { retryCount: options.retryCount || 3, retryOnError: 1 });
    lib.everyParallel([
        function(next) {
