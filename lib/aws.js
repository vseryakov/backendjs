...
aws.configure = function(options, callback)
{
    ...
    if (os.platform() != "linux" || options.noConfigure || this.roles.indexOf(core.role) == -1) {
        if (!this.key || this.sdkProfile) return this.readCredentials(this.sdkProfile, callback);
        return callback();
    }
    ...
    fs.readFile(process.env.HOME + "/.aws/credentials", function(err, data) {
        if (data && data.length) {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (state == 1) {
                    if (x[0][0] == '[') break;
                    if (x[0].trim() == "aws_access_key_id" && x[1]) aws.key = x[1].trim();
                    if (x[0].trim() == "aws_secret_access_key" && x[1]) aws.secret = x[1].trim();
                    if (x[0].trim() == "region" && x[1]) aws.region = x[1].trim();
                }
            }
