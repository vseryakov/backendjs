...
aws.querySign = function(region, service, host, method, path, body, headers, now)
{
    now = util.isDate(now) ? now : new Date();
    ...
    var credString = [ date, region, service, 'aws4_request' ].join('/');
    ...
    var strToSign = [ 'AWS4-HMAC-SHA256', isodate, credString, lib.hash(canonString, "sha256", "hex") ].join('\n');

    var sigKey = lib.sign(this.secret, this.key + "," + credString, "sha256", "hex");
