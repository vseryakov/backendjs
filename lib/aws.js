...
var aws = {
    ...
    roles: ["shell","web","master","server","worker"],
};
    ...
    this.getInstanceMeta("/latest/meta-data/iam/security-credentials/" + this.amiProfile, function(err, data) {
        if (!err && data) {
            ...
            if (obj.Code === 'Success') {
                ...
                logger.info("getInstanceCredentials:", aws.key, lib.strftime(aws.tokenExpiration), "interval:", lib.toDuration(aws.tokenExpiration - Date.now()));
            }
        }
...
aws.getInstanceInfo = function(callback)
{
    lib.series([
        ...
        logger.debug('getInstanceInfo:', aws.name, core.instance, 'profile:', aws.amiProfile, 'expire:', aws.tokenExpiration, err || "");
        if (typeof callback == "function") callback();
    });
}
...
aws.parseXMLResponse = function(err, params, options, callback)
{
    ...
    if (params.status != 200) {
        ...
        logger.logger((options && options.logger_error) || "error", 'queryAWS:', params.href, params.search, lib.objDescr(params.Action), err, params.toJSON(), "token:", aws.key, aws.tokenExpiration, params.headers.Authorization);
...
aws.queryAWS = function(region, endpoint, proto, host, path, obj, options, callback)
{
    ...
    opts.signer = function() {
        aws.querySign(this.region, this.endpoint, this.hostname, "POST", this.pathname, this.postdata, this.headers);
    }
...
aws.queryRoute53 = function(method, path, data, options, callback)
{
    ...
    core.httpGet(uri, this.queryOptions(method, data, headers, options), function(err, params) {
        aws.parseXMLResponse(err, params, options, callback);
    });
}
    ...
    aws.queryRoute53("GET", "/hostedzone", "", function(err, rc) {
        ...
        lib.forEachSeries(names, function(host, next) {
            ...
            var req = '<?xml version="1.0" encoding="UTF-8"?>' +
                    ...
                    '<ResourceRecordSet>' +
                    '<Name>' + host.name + '</Name>' +
                    '<Type>' + (host.type || (options && options.type) || "A") + '</Type>' +
