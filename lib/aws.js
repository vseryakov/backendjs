...
var aws = {
    ...
    args: [ { name: "key", descr: "AWS access key" },
            ...
            { name: "parameters", type: "list", descr: "AWS Config Parameters Store to set on start, supports @..@ core.instance placeholders: format is: path:value,...." },
    ],
        ...
        function(next) {
            ...
            aws.ssmGetParametersByPath("/", opts, (err, params) => {
                lib.forEachSeries(aws.parameters, (param, next2) => {
                    param = param.split(":");
                    var val = lib.toTemplate(param[1], [core.instance, core]);
                    if (params.some((x) => (x.Name == param[0] && x.Value == val))) return next2();
                    aws.querySSM("PutParameter", { Name: param[0], Type: "String", Value: val, Overwrite: true }, opts, () => { next2() });
                }, () => { next() });
            });
        },
    ], callback);
