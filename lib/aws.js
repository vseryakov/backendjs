...
aws.parseError = function(params, options)
{
    ...
    if (params.action) err.action = params.action;
    return err;
}
...
aws.parseXMLResponse = function(err, params, options, callback)
{
    if (!err && params.data) {
        ...
        if (params.status != 200) {
            ...
            if (err) logger.errorWithOptions(err, options, 'queryAWS:', lib.objDescr(params.Action), err, params.toJSON(options && options.debug_error));
        } else {
