...
aws.configureMaster = function(options, callback)
{
    ...
    lib.everyParallel([
        function(next) {
            ...
            if (app.instance.tag && !/^([a-z]+)-(a-z)-([0-9.]+)$/i.test(app.instance.tag)) return next();
            aws.ec2CreateTags(app.instance.id, app.runMode + "-" + app.appName + "-" + app.appVersion, opts, next);
        },
        function(next) {
            if (!lib.isArray(aws.hostName) || !app.ipaddr) return next();
            logger.info("configureMaster:", aws.hostName, app.ipaddr, app.instance);
            lib.forEverySeries(aws.hostName, (host, next2) => {
                aws.route53Change(lib.toTemplate(host, [app.instance, core]), next2);
            }, next, true);
        },
        function(next) {
        ...
        function(next) {
            ...
            this.queryELB2("DescribeTargetGroups", { "TargetGroupArns.member.1": aws.targetGroup }, (err, rc) => {
                ...
                logger.info("configureMaster:", aws.targetGroup, group);
        ...
        function(next) {
            ...
            var params = aws.setParameters.reduce((x, y) => {
                y = y.split(":");
                y[1] = lib.toTemplate(y[1], [app.instance, core]);
                if (y[1]) x[y[0]] = y[1];
                return x;
            }, {});
