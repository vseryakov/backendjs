...
var aws = {
    ...
    args: [ { name: "key", descr: "AWS access key" },
            ...
            { name: "meta", type: "bool", descr: "Retrieve instance metadata, 0 to disable, 2 to use IMDSv2" },
            { name: "meta-expiration", type: "int", min: 1, max: 21600, descr: "Session timeout for IMDSv2 in seconds" },
...
aws.configureMaster = function(options, callback)
{
    ...
    lib.everyParallel([
        ...
        function(next) {
            ...
            aws.ssmGetParameters({ Names: Object.keys(params) }, opts, (err, rc) => {
...
aws.getInstanceMeta = function(path, callback)
{
    ...
    if (!lib.rxUrl.test(path)) path = "http://169.254.169.254" + path;
    if (this.metaToken) opts.headers = { "X-aws-ec2-metadata-token": this.metaToken };

    core.httpGet(path, opts, (err, params) => {
        logger[[200, 404, 529].indexOf(params.status) == -1 ? "error": "debug"]('getInstanceMeta:', path, params.status, params.data, err || "");
...
aws.getInstanceMetaToken = function(callback)
{
    var opts = {
        method: "PUT",
        headers: { "X-aws-ec2-metadata-token-ttl-seconds": this.metaExpiration },
        noparse: 1,
        httpTimeout: 100,
        quiet: true,
        retryCount: 5,
    ...
    core.httpGet(`http://169.254.169.254/latest/api/token`, opts, (err, params) => {
        logger[[200, 404, 529].indexOf(params.status) == -1 ? "error": "debug"]('getInstanceToken:', path, params.status, params.data, err || "");

        if (this.metaExpiration > 1) {
            if (!err && params.data) this.metaToken = params.data;
            var timeout = err || !params.data ? 1000 : this.metaExpiration*1000-500;
            setTimeout(this.getInstanceMetaToken.bind(this), timeout);
        }
        if (typeof callback == "function") callback(err, params.status == 200 ? params.data : "");
    });
}
    ...
    lib.series([
        ...
        function(next) {
            ...
            aws.getInstanceMeta(path, (err, data) => {
                ...
                setTimeout(aws.getInstanceCredentials.bind(aws, path), timeout);
                next(err);
            });
        },
    ], callback, true);
...
aws.getInstanceInfo = function(options, callback)
{
    if (process.env.AWS_EXECUTION_ENV == "AWS_ECS_FARGATE") {
        ...
        return aws.getInstanceCredentials("http://169.254.170.2" + process.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI, callback);
    }

    lib.series([
        function(next) {
            if (aws.meta != 2) return next();
