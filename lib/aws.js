...
var aws = {
    ...
    args: [ { name: "key", descr: "AWS access key" },
            ...
            { name: "ddb-shard-records-count", type: "int", min: 10, descr: "Default DynamoDB Streams processing records count in a batch" },
            { name: "ddb-shard-records-retries", type: "int", descr: "Default DynamoDB Streams processing records retry count before exiting on empty records, <=0 to poll indefinitly" },
            { name: "ddb-shard-records-interval", type: "int", min: 50, descr: "Default DynamoDB Streams processing records interval between retries on empty records, multiplied with every retry" },
            { name: "ddb-shard-records-timeout", type: "int", min: 50, descr: "Default DynamoDB Streams processing records max timeout on empty records start exponential backoff again from 0" },
...
aws.configure = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            if (!aws.configParameters) return next();
            var opts = lib.objClone(options, "retryCount", options.retryCount || 3, "retryOnError", 1);
            aws.ssmGetParametersByPath(aws.configParameters, opts, (err, params) => {
                var argv = [];
...
aws.parseXMLResponse = function(err, params, options, callback)
{
    if (!err && params.data) {
        ...
        if (params.status != 200) {
            ...
            if (err) logger.errorWithOptions(err, options, 'queryAWS:', lib.objDescr(params.Action), err, params.toJSON());
        } else {
