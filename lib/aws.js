    ...
    this.getInstanceMeta("/latest/meta-data/iam/security-credentials/" + this.amiProfile, function(err, data) {
        if (!err && data) {
            ...
            if (obj.Code === 'Success') {
                ...
                logger.info("getInstanceCredentials:", core.role, aws.key, lib.strftime(aws.tokenExpiration), "interval:", lib.toDuration(aws.tokenExpiration - Date.now()));
            }
        }
...
aws.parseXMLResponse = function(err, params, options, callback)
{
    if (!err && params.data) {
        ...
        if (params.status != 200) {
            ...
            logger.logger((options && options.logger_error) || "error", 'queryAWS:', params.href, params.search, lib.objDescr(params.Action), err, params.toJSON());
        } else {
