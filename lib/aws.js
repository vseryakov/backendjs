...
var aws = {
    ...
    args: [ { name: "key", descr: "AWS access key" },
            ...
            { name: "release", type: "regexp", descr: "OS release must match this regexp in order to read instance metadata" },
...
aws.configure = function(options, callback)
{
    ...
    if (options.noConfigure || core.platform != "linux" || !this.release.test(core.release) || this.roles.indexOf(core.role) == -1) {
...
aws.configureServer = function(options, callback)
{
    ...
    lib.parallel([
       function(next) {
           ...
           aws.elbRegisterInstances(aws.elbName, core.instance.id, function() { next() });
       },
       function(next) {
           if (!aws.targetGroup) return next();
           aws.elb2RegisterInstances(aws.targetGroup, core.instance.id, function() { next() });
       },
    ], callback);
}
    ...
    lib.parallel([
        ...
        function(next) {
            ...
            aws.ssmGetParameters({ Names: Object.keys(params) }, opts, (err, rc) => {
                ...
                lib.forEachSeries(Object.keys(params), (name, next2) => {
...
aws.queryRekognition = function(action, obj, options, callback)
{
    ...
    this.httpGet("https://rekognition." + opts.region + ".amazonaws.com/", opts, function(err, params) {
...
aws.querySSM = function(action, obj, options, callback)
{
    ...
    opts.action = action;
    logger.debug(opts.action, opts);
    this.httpGet("https://ssm." + opts.region + ".amazonaws.com/", opts, function(err, params) {
        if (params.status != 200) err = aws.parseError(params, options);
        if (typeof callback == "function") callback(err, params.obj);
    });
}
...
aws.queryACM = function(action, obj, options, callback)
{
    ...
    opts.action = action;
    logger.debug(opts.action, opts);
    this.httpGet("https://acm." + opts.region + ".amazonaws.com/", opts, function(err, params) {
        if (params.status != 200) err = aws.parseError(params, options);
        if (typeof callback == "function") callback(err, params.obj);
    });
}
...
{
    var headers = { 'content-type': 'application/x-amz-json-1.1', 'x-amz-target': "Comprehend_20171127." + action };
    var opts = this.queryOptions("POST", lib.stringify(obj), headers, options);
    opts.region = options && options.region || this.region || 'us-east-1';
    opts.endpoint = "comprehend";
    opts.signer = this.querySigner;
    opts.action = action;
    logger.debug(opts.action, opts);
    this.httpGet("https://comprehend." + opts.region + ".amazonaws.com/", opts, function(err, params) {
        if (params.status != 200) err = aws.parseError(params, options);
        if (typeof callback == "function") callback(err, params.obj);
    });
}
