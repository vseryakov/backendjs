...
const aws = {
    ...
    args: [
        ...
        { name: "config-secrets", type: "list", descr: "Secrets to load and parse as config before initializing the database pools, supports @..@ app.instance placeholders", example: "production,production-@role@" },
        { name: "conf-file", descr: "S3 url for config file to download on start, may include @placeholders@ to refer properties from app.instance" },
...
aws.configure = function(options, callback)
{
    ...
    if (options.noConfigure || !this.meta || app.platform != "linux" || !lib.isFlag(this.roles, app.role)) {
        ...
        return this.readCredentials(this.sdkProfile, (err, creds) => {
    ...
    lib.everySeries([
        ...
        function(next) {
            ...
            aws.batchGetSecretValue({ filters }, (err, params) => {
                var argv = [];
                for (const p of params) {
                    argv.push("-" + p.Name, p.SecretString);
                }
                app.parseArgs(argv, 0, "aws-secrets");
                next();
            });
        },
    ], callback, true);
