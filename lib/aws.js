...
const aws = {
    ...
    args: [
        ...
        { name: "config-secrets", type: "list", descr: "Secrets to load and parse as config before initializing the database pools", example: "/bkjs/config/" },
...
aws.configure = function(options, callback)
{
    ...
    lib.everySeries([
        ...
        function(next) {
            if (!aws.key || !aws.configSecrets) return next();
            aws.batchGetSecretValue({ filters: aws.configSecrets }, (err, params) => {
                var argv = [];
                for (const i in params) {
                    argv.push("-" + params[i].Name.split("/").pop(), params[i].SecretString);
                }
