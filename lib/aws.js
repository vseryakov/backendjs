...
const aws = {
    ...
    args: [
        ...
        { name: "config-set-parameters", type: "list", descr: "AWS Config Parameters Store to set on start, supports @..@ app.instance placeholders: format is: path:value,...." },
        { name: "config-secrets", type: "list", descr: "AWS Secrets manager filters to load and parse as config before initializing the database pools, supports @..@ app.instance placeholders in filters", example: "production,production-@role@" },
        { name: "config-s3-file", descr: "S3 url for config file to download on start, may include @placeholders@ to refer properties from app.instance" },
...
aws.configure = function(options, callback)
{
    lib.everySeries([
        function(next) {
            if (!this.meta || app.platform != "linux" || !lib.isFlag(this.roles, app.role)) return next();
        ...
        function(next) {
            ...
            aws.readCredentials(aws.sdkProfile, (err, creds) => {
        ...
        function(next) {
            modules.ipc.on('config:init', aws.readConfigS3.bind(aws));
            aws.readConfigS3(next);
        },
        ...
        function(next) {
            ...
            const filters = aws.configSecrets.map(x => lib.toTemplate(x, [app.instance, app]));
...
aws.configureServer = function(options, callback)
{
    ...
    lib.everyParallel([
        ...
        function(next) {
            if (!lib.isArray(aws.configSetParameters)) return next();
            logger.info("configureServer:", aws.configSetParameters, opts);
            var params = aws.configSetParameters.reduce((x, y) => {
                y = y.split(":");
...
{
    this.queryService("events", "AWSEvents", action, obj, options, callback);
}
...
{
    this.queryService("secretsmanager", "secretsmanager", action, obj, options, callback);
}
