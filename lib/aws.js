...
aws.configureMaster = function(options, callback)
{
    ...
    lib.parallel([
       ...
       function(next) {
           ...
           var host = lib.toTemplate(aws.hostName, core.instance);
           logger.info("configureMaster:", aws.hostName, host, core.ipaddr, core.instance);
           aws.route53Change(host, function() { next() });
       },
    ], callback);
}
...
aws.parseError = function(params, options)
{
    ...
    if (params.obj) {
        ...
        if (params.obj.__type) {
            err = lib.newError({ message: params.obj.Message, code: params.obj.__type, status: params.status });
        }
    }
...
aws.parseXMLResponse = function(err, params, options, callback)
{
    if (!err && params.data) {
        ...
        if (params.status != 200) {
            ...
            logger.logger(options && options.logger_error, 'queryAWS:', lib.objDescr(params.Action), err, params.toJSON());
        } else {
...
aws.queryEndpoint = function(endpoint, version, action, obj, options, callback)
{
    ...
    // Limit to the suppported regiosn per endpoint
    ...
    var e = options.endpoint ? url.parse(String(options.endpoint)) : this.endpoints[endpoint] ? url.parse(this.endpoints[endpoint]) : lib.empty;
    var proto = options.endpoint_protocol || e.protocol || 'https';
    var host = options.endpoint_host || (e.host || e.hostname) || (endpoint + '.' + region + '.amazonaws.com');
