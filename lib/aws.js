...
aws.getInstanceMeta = function(path, callback)
{
    ...
    core.httpGet(path, opts, function(err, params) {
...
aws.getInstanceCredentials = function(options, callback)
{
    var url = options.credentialsUrl || (this.amiProfile ? "/latest/meta-data/iam/security-credentials/" + this.amiProfile : "");
    if (!url) return typeof callback == "function" && callback();

    this.getInstanceMeta(url, function(err, data) {
        if (!err && data) {
        ...
        setTimeout(aws.getInstanceCredentials.bind(aws, options), timeout);
...
aws.getInstanceInfo = function(options, callback)
{
    if (process.env.AWS_EXECUTION_ENV == "AWS_ECS_FARGATE") {
        ...
        return aws.getInstanceCredentials({ credentialsUrl: "http://169.254.170.2" + process.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI }, callback);
    }
    ...
    lib.series([
        ...
        function(next) {
            ...
            if (aws.key) return next();
            aws.getInstanceCredentials(options, next);
        },
        function(next) {
