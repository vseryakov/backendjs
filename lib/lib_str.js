...
// using index 0 retutns the whole matched string.
...
lib.matchRegexp = function(str, rx, index)
{
    ...
    var d = str.match(rx);
    return d ? /g/.test(rx.flags) ? d : d[typeof index == "number" ? index : 1] : null;
}
...
// - `options.keepempty` - will preserve empty items, by default empty strings are ignored
// - `options.notrim` - will skip trimming strings, trim is the default
//
...
lib.strSplit = function(str, sep, options)
{
    ...
    for (let i = 0; i < len; ++i) {
        v = list[i];
        if (options.datatype) v = lib.toValue(v, options.datatype);
        if (typeof v != "string") {
            rc.push(v);
            continue;
        }
