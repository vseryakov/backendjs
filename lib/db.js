...
/**
 * Supported database column types usable in {@link module:db.DbTableColumn#type}
 * @readonly
 * @enum {string}
 */
const DbColumnType = {
    ...
    /** Date stored in database, usually as text in SQL */
    date: "date",
    ...
    /** timestamp in milliseconds, numeric */
    mtime: "mtime",

    /** stored as JSON text but converted to/from native Javascript objects */
    json: "json",

    /** native JSON object type */
    obj: "obj",

    /** native JSON object type */
    object: "object",

    /** native JSON array type */
    array: "array",

    /** store a list of primitive types, strings and/or numbers */
    list: "list",

    /** a unique set of string or numbers if supported natively, otherwise same as list */
    set: "set",

    /** generates a random number in {@link module:db.add}/{@link module:db.put} methods, uses optional .max/.min properties, numeric */
    random: "random",

    /** autogenerate the column value with UUID, optional _prefix_ property will be prepended, _{ type: "uuid", prefix: "u_" }__ */
    uuid: "uuid",

    /** autogenerate the column value with UUID */
    suuid: "suuid",

    /** autogenerate the column value with UUID */
    sfuuid: "sfuuid",

    /** defines a column to be automatically filled with the current timestamp in milliseconds, numeric */
    now: "now",

    /** defines a column that will be automatically incremented by the {@link module:db.incr} command, on creation it is set with 0 in SQL, numeric */
    counter: "counter",
};
 ...
 * @property {DbColumnType} type - column type, supported types: {@link DbColumnType}
 ...
 * @property {boolean} unique - true for UNIQUE index column (SQL)
 *
 ...
 *
 * On return the second arg to the callback is a list of records with errors as the input record with
 * added property `errstatus` and `errmsg`, for the `get` op the result will be the retrieved record if exists.
 *
 ...
 * @param {DbResultCallback} [callback] (err:object, data:object[])
 *
 * @example
 * var ops = [
 *   { table: "bk_counter", query: { id: 1, like: 1 } },
 *   { op: "put", table: "users", query: { login: "test", id: 1, name:"test" },
 *   { op: "get", table: "users", query: { login: "admin" } }
 * ];
 *
    ...
    lib.forEach(Object.keys(tables), (table, next) => {
        ...
        lib.forEachLimit(tables[table], options?.concurrency, (obj, next2) => {
            db[obj.op](obj.table, obj.query, obj.options, (err, rc) => {
                if (err) {
                    data.push(Object.assign(obj, { errstatus: err.code || err.status, errmsg: err.message }));
 ...
 * ];
 *
 * const { err, data } = await db.abatch(ops);
 *
...
{
    return new Promise((resolve, _reject) => {
        db.batch(list, options, (err, data) => {
            resolve({ err, data });
        });
    });
}
