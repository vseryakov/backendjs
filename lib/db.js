 ...
 *    between, begins_with, not_begins_with, ends_with, not_ends_with,
 *    like, like%, not like%, ilike%, not like%,
 ...
 * - Postgres specific:
 * ```&&, not &&, @>, not @>, <@", not <@, ~* any, ! ~* any,
 *    similar to, not similar to, regexp, iregexp,
 *    ~, ~*, !~", !~,
 * ```
 *
 ...
 * { count_$incr: 3 }
 *
 * { tag_$expr: "tag=COALESCE(name,'none')||count" }
 *
 * { tag_$expr: ["tag=COALESCE(name,?)||count", "none"] }
 * ```
 ...
 * @property {object} typesMap - type mapping between supplied and actual column types, an object, use * for default type
