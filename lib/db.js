...
db.getConfig = function(options, callback)
{
    ...
    var tags = lib.strSplitUnique(options && options.tag || core.instance.tag);
    var roles = [options && options.role || core.role].concat(this.configRoles);
    ...
    var mods = [ options && options.network || core.network, options && options.region || core.instance.region];
    ...
    var opts = { ops: { type: "in", mtime: "gt" }, pool: options && options.pool || this.config, count: this.configCount };
