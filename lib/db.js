...
db.init = function(options, callback)
{
    ...
    var pools = db.none ? [] : Object.keys(db._config).filter(x => !lib.isFlag(this.skip?.pools, x));
...
function queryResult(req, rows, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            if (lib.isArray(db.cache.pools) && !lib.isFlag(db.cache.pools, req.pool)) return next();
            // For sync cache flushes wait before returning
            var cbnext = lib.isFlag(db.cache.sync, req.table) ? next : undefined;
            switch (req.op) {
            case "bulk":
                let skipped = 0;
...
db.queryProcessSync = function(pool, req, _row)
{
    ...
    case "del":
        ...
        const table = req.op === "bulk" ? req.query[0]?.table : req.table;
        if (!lib.isFlag(pool.config.tables, table)) return;
