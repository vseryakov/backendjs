...
// On return the second arg to the callback is a list of records with errors, same input record with added property `errstatus` and `errmsg`
//
// NOTE: DynamoDB only supports add/put/del only.
//
// Example
//
//          var ops = [ { op: "add", table: "bk_counter", obj: { id:1, like:1 } },
...
// Same as the `bulk` but in transaction mode, all operations must succeed or fail. Not every driver can support it,
// in DynamoDB case only 25 operations can be done at the same time, if the list is larger then it will be sequentially run with batches of 25 records.
//
    ...
    case "bulk":
        ...
        for (const i in req.obj) {
            ...
            if (obj.error) {
                ...
                lib.objDel(obj, "error", "columns", "options", "orig");
            }
            list.push(obj);
        }
        req.obj = list;
