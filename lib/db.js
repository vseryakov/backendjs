...
{
    if (typeof options == "function") callback = options, options = null;
    if (!lib.isObject(req)) return lib.tryCall(callback, lib.newError("invalid request"));
    if (req.error) return lib.tryCall(callback, req.error);
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (var p in req.columns) {
        ...
        if (col.failempty && lib.isEmpty(req.obj[p])) {
            req.error = { status: 400, message: p + " cannot be empty" };
        }
    }
}
