...
const db = {
    ...
    args: [
        ...
        { name: "([a-z0-9]+)-pool-options$", obj: '_config.$1.configOptions', type: "map", maptype: "auto", merge: 1, onupdate: "applyPoolOptions", descr: "General options for a DB pool" },
        { name: "([a-z0-9]+)-pool-options-([a-zA-Z0-9_.-]+)$", obj: '_config.$1.configOptions', camel: '-', autotype: 1, make: "$2", onupdate: "applyPoolOptions", descr: "General options for a DB pool" },
        { name: "([a-z0-9]+)-pool-table-map", obj: '_config.$1.configOptions.tableMap', type: "map", maptype: "auto", merge: 1, onupdate: "applyPoolOptions", descr: "Table mapping, aliases" },
    ...
    tables: {
        ...
        bk_config: {
            ...
            stime: { type: "mtime", nozero: 0 },    // start time when in use
            etime: { type: "mtime", nozero: 0 },    // end time when not in use
 ...
 * ```js
 * const { core, db } = require("backendjs");
 * app.init((err) => {
 *   db.add(...
 *   ...
 * });
 ...
 * ```
 *
 * @property {string[]} roles - additional roles to be used for DB config
 */
 ...
 * @param {boolean} [options.localTables] - if true only enable local, default and config pools
 ...
 * @property {string} pool - a DB pool used for this operation
 * @property {string} table - a DB table name
 * @property {string} [text] - native DB query, SQL or etc...
 * @property {object} obj - a query object
 * @property {object} options - an object with optional properties for the operation
 ...
 * @param {string} options.pool - name of the database pool where to execute this query.
 ...
 * @param {function} options.filterrows - function to filter rows not to be included in the result, returns a new result set, args are: function(req, rows)
 * @param {function} options.processrows - function to process rows in the result, returns a new result, args are: function(req, rows), this result will be put in cache
 *      if requested so this may be used for preparing cached results, it must return an array
 * @param {function} options.processasync - function to process result rows via async callback, return a new result in the callback, the function is: function(req, rows, callback),
 *      the callback is function(err, rows)
 * @param {boolean} options.syncMode - skip columns preprocessing and dynamic values for pool sync and backup restore
 * @param {boolean} options.quiet - report errors in debug level
 * @param {string} options.logger_db - log results at the end with this level or debug by default
 * @param {string|object} options.logger_error - a log level to report about the errors, default is 'error', if an object it can specify different log levels by err.code, * is default level for not matched codes
 * @param {boolean|string[]} options.ignore_error - clear errors occurred as it never happen, do not report in the log, if an array then only matched codes will be cleared
 * @param {boolean} options.noprocessrows - if true then skip post processing result rows, return the data as is, this will result in returning combined columns as it is
 * @param {boolean} options.noconvertrows - if true skip converting the data from the database format into Javascript data types, it uses column definitions
 * @param {boolean} options.nopreparerow - if true skip row preparation and columns processing, the req.obj is passed as is, useful for syncing between pools
 *      for the table to convert values returned from the db into the the format defined by the column
 * @param {boolean} options.total - if true then it is supposed to return only one record with property `count`, skip all post processing and convertion
 * @param {boolean} options.info_obj - to return the record just processed in the info object as `obj` property, it will include all generated and updated columns
 * @param {boolean} options.result_obj - to return the query record as result including all post processing and new generated columns, this is not what `returning` property does, it only
 *      returns the query record with new columns from memory
 * @param {boolean} options.keep_req - on return do not clear out the request object, by default all properties are deleted to free up memory
 * @param {boolean} options.keep_obj - only preserve op, obj, table properties in the reqest after return
 * @param {boolean} options.cached - if true then run getCached version directly
 * @param {boolean} options.nocache - disable caching even if configured for the table
 * @param {object} options.ops - operators to use for comparison for properties, an object with column name and operator. The following operators are available:
 *       `>, gt, <, lt, =, !=, <>, >=, ge, <=, le, in, all_in, between, regexp, iregexp, begins_with, not_begins_with, like%, ilike%, contains, not_contains`
 * @param {object} options.opsMap - operator mapping between supplied operators and actual operators supported by the db
 * @param {object} options.typesMap - type mapping between supplied and actual column types, an object
 * @param {string[]} options.select - a list of columns or expressions to return or all columns if not specified, only existing columns will be returned
 * @param {string[]} options.select_all - a list of columns or expressions to return, passed as is to the underlying driver
 * @param {string|object} options.start - start records with this primary key, this is the next_token passed by the previous query
 * @param {int} options.count - how many records to return
 * @param {boolean} options.first - a convenient option to return the first record from the result or null (similar to `db.get` method)
 * @param {boolean} options.last - similar to first but return last record
 * @param {string} options.join - how to join condition expressions, default is AND
 * @param {object} options.joinOps - operators to use to combine several expressions in case when an array of values is given, supports `and|or|AND|OR``
 * @param {string} options.sort - sort by this column. if null then no sorting must be done at all, records will be returned in the order they are kept in the DB.
 ...
 * @param {int} options.sort_timeout - for pagination how long to keep internal state in millisecons, depends on the DB, for example for Elasticsearch it corresponds
 *       to the scroll param and defaults to 60000 (1m)
 * @param {boolean} options.desc - if sorting, do in descending order
 * @param {int} options.page - starting page number for pagination, uses count to find actual record to start, for SQL databases mostly
 * @param {boolean} options.unique - specified the column name to be used in determining unique records, if for some reasons there are multiple records in the location
 *       table for the same id only one instance will be returned
 * @param {string} options.cacheKey - exlicit key for caching, return from the cache or from the DB and then cache it with this key, works the same as `get`
 * @param {string} options.cacheKeyName - a name of one of the cache keys to use, it must be defined by a `db-cache-keys-table-name` parameter
 * @param {object} options.aliases - an object with mapping between artificial name to real column name, useful in or_$/and_$ conditions with same column but different values,
 *       alternative is to append aliases with _$, like name_$ or name_$$, the name must be a valid column name
 * @param {string[]} options.custom_columns - an array of pairs to define global or artificial columns, the format is: [ RegExp, type, ...], useful with aliases
 */
 ...
 * @param {DBRequest} req - an object with request required data
 * @param {DBRequestOptions} [options] - optional properties customizations
...
db.query = function(req, options, callback)
{
    ...
    const pool = db.getPool(req.options);
    ...
    if (req.op && lib.isFlag(pool.configOptions.metricsTables, table)) {
    ...
    if (req.error) {
        return queryEnd(req.error, req, null, callback);
    }
    pool.acquire((err, client) => {
        if (err) return queryEnd(err, req, null, callback);
        try {
            queryRun(pool, client, req, callback);
        } catch (e) {
 ...
 * @returns {Promise}
 * @example
 * await db.aquery({ text: "SELECT ....." }, { pool: "sqlite" });
 * @memberof module:db
 * @method aquery
 */
db.aquery = util.promisify(db.query.bind(db));

function queryRun(pool, client, req, callback)
{
    req.client = client;
...
function queryEnd(err, req, rows, callback)
{
    ...
    if (err) {
        err = db.convertError(pool, req.table, req.op || "", err, req.options);
    ...
    if (req.options.info_obj) {
        ...
        if (req.options.info_obj.convertrows) {
            ...
            db.convertRows(req.pool, req, [info.obj], req.options.info_obj);
        }
    }
    ...
    lib.series([
        function(next) {
            ...
            case "bulk":
                ...
                lib.forEachLimit(req.obj, db.concurrency, (row, next2) => {
...
/**
 * Async version of db.get, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.select, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.search, throws exception on error
 * @memberOf module:db
 */
 ...
 *   @param {} options.no_columns - do not check for actual columns defined in the pool tables and add all properties from the obj, only will work for NoSQL dbs,
 *        by default all properties in the obj not described in the table definition for the given table will be ignored.
 *   @param {} options.skip_columns - ignore properties by name listed in the this array, the most use case is to skip autogeneratd columns like "now"
 *
...
/**
 * Async version of db.add, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.incr, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.put, throws exception on error
 * @memberOf module:db
 */
 ...
 *  @param {} options.expected - an object with the condition for the update, it is used in addition to the primary keys condition from the `obj`,
 *        a property named or_$/and_$ will be treated as a sub-expression if it is an object. For multiple OR/AND use
 *        or_$$, or_$$$,...
 *  @param {} options.expectedJoin - how to join expected expressions: OR, AND, default is AND
...
/**
 * Async version of db.update, throws exception on error
 * @memberOf module:db
 */
 ...
 * @param {} options.updateOptions - options to be passed to the db.update if needed, this is useful so select and update options will not be mixed up
 * @param {} options.updateCollect - if true return all updated rows in the callback otherwise just the number of updated rows
 * @param {} options.factorCapacity - write capacity factor for update operations, default is 0.25
 * @param {} options.op - by default it uses db.update but the `op` can be set to `put` or `add`
 * @param {} options.updateProcess - a function callback that will be called for each row before updating it, this is for some transformations of the record properties
 ...
 * @param {} options.updateFilter - a function that must return something to the callback in order to skip the current record. `options.updateFilter(row, options, (skip) => {})`
 *
...
/**
 * Async version of db.updateAll, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.del, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.delAll, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.list, throws exception on error
 * @memberOf module:db
 */
 ...
 * - `list` an array of objects to get/put/delete from the database in the format:
 * @param {} options.op - is one of get, add, incr, put, update, del
 * @param {} options.table - which table to use
 * @param {} options.obj - an object with data
 ...
 *
 * One advantage using this with op: get over db.list is caching, db.get may use both local and remote caches but db.list always hits the database.
 *
 *
 *  Example:
 *
...
db.batch = function(list, options, callback)
{
    ...
    lib.forEach(Object.keys(tables), (table, next) => {
        ...
        lib.forEachLimit(tables[table], options?.concurrency || 1, (obj, next2) => {
            db[obj.op](obj.table, obj.obj, obj.options, (err, rc) => {
                if (err) {
...
/**
 * Async version of db.batch, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.bulk, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.transaction, throws exception on error
 * @memberOf module:db
 */
 ...
 *              // Copy all userс from one db into another
...
/**
 * Async version of db.scan, throws exception on error
 * @memberOf module:db
 */
                ...
                if (qopts.batch) {
                    lib.series([
                        function(next3) {
                            ...
                            db.join(table, row, { pool: qopts.pool }, (err, rc) => {
                                if (!err) row = rc;
                                next3();
                            });
                        },
...
/**
 * Async version of db.copy, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.join, throws exception on error
 * @memberOf module:db
 */
 ...
 * - `internal` - if set then this property can only be updated by admin/root or with `isInternal`` property
 ...
 * - `increment` - for numeric columns add this value before saving
...
/**
 * Async version of db.create, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.upgrade, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.drop, throws exception on error
 * @memberOf module:db
 */
...
/**
 * Async version of db.sql, throws exception on error
 * @memberOf module:db
 */
