 ...
 * - ``` data: { type: "obj", _$elasticsearch: { enabled: false } }```
 *
 */
 ...
 *
 * const { data } = await db.aselect("users", { type_$in: ["admin", "manager"] });
 *
 * ```
 *
...
function queryEnd(err, req, rows, callback)
{
    ...
    if (err) {
        ...
        logger.logger(req.options.logger_db || req.failed && "warn" || "debug", "queryEnd:", req, req.info?.affected_rows || rows.length, 'rows');
    }
    if (err) {
