 ...
 *
 * All queries and update operations ignore properties that starts with undersapp.
 *
 ...
 *
 * Additionally collest stats individually for a list of tables
 *
 ...
 *
 *   deep property path can be used as well, like `prop.subprop`,
 ...
 *        });
 *
 * Example: scan users with custom filter, not by primary key: by exact zipcode
 *
 *        db.select("bk_user", { zipcode: '20000' }, (err, rows) => {
 *
 *        });
 *
 * Example: select users by type for the last day
 *
 *        db.select("bk_user", { type: 'admin', mtime: Date.now()-86400000 }, { ops: { type: "contains", mtime: "gt" } }, (err, rows) => {
 *
 *        });
 *
 * Example: select users by JSON sub property using Elasticsearch
 *
 ...
 *   - op - is one of get, add, incr, put, update, del
 ...
 *   - sourcePool - original pool, default if not provided
 *   - pool - other DB pool
 ...
 *   - queryOptions - pass options to scan operations
