...
db.queryResult = function(err, req, rows)
{
    ...
    // Flush all other caches for the given record, this is for select or custom cache keys mostly
    if (lib.isArray(options.cacheKeys) && ['add','put','update','incr','del'].indexOf(req.op) > -1) {
        for (var i in options.cacheKeys) {
            this.delCache(req.table, req.obj, { cacheKey: options.cacheKeys[i] });
        }
    }
