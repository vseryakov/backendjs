...
var db = {
    ...
    args: [{ name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
           ...
           { name: "([a-z]+)-pool-options-([a-zA-Z0-9\.-]+)-?([0-9]+)?$", obj: 'poolParams.$1$3.configOptions', camel: '-', autotype: 1, make: "$2", onparse: function(v,o) {this.parsePoolOptions(v,o)}, descr: "General options for a DB pool" },
...
db.init = function(options, callback)
{
    ...
    lib.forEachSeries(Object.keys(this.poolParams), function(name, next) {
        ...
        if (self.pools[name] && (!options || !options.force) && (!params.url || params.url == self.pools[name].url)) {
        ...
        if (cluster.isMaster && (self._createTables || pool.configOptions.createTables)) return self.createTables(name, function() { next() });
...
//  - the application name and major/minor version specified in the package.json: `-1.0`
...
db.initConfig = function(options, callback)
{
    ...
    var items = [ core.runMode,
                  ...
                  core.appName + "-" + ver[0],
                  options && options.role || core.role,
                  options && options.network || core.network,
                  options && options.region || core.instance.region,
                  options && options.zone || core.instance.zone,
                  options && options.tag || core.instance.tag ];
    ...
    var query = { type: types, mtime: options && options.delta ? this._configMtime : 0 };
    ...
    this.select(table, query, opts, function(err, rows) {
        ...
        rows.forEach(function(x) {
            ...
            if (x.ttl > 0) {
                self.configRefresh[x.type+"|"+x.name] = x.ttl*1000 + Date.now();
            } else {
                delete self.configRefresh[x.type+"|"+x.name];
            }
        });
        core.parseArgs(argv);
...
db.refreshConfig = function(options, callback)
{
    ...
    var list = Object.keys(this.configRefresh).reduce(function(l, x) {
        if (self.configRefresh[x] > 0 && self.configRefresh[x] <= now) {
            x = x.split("|");
    ...
    this.list(table, list, { pool: pool }, function(err, rows) {
        var argv = rows.reduce(function(l, x) {
            ...
            if (x.ttl > 0) {
                self.configRefresh[x.type+"|"+x.name] = x.ttl*1000 + now;
            } else {
                delete self.configRefresh[x.type+"|"+x.name];
            }
            return l;
...
//     - expected_join - how to join expected expressions, default is AND
...
//   - updateCollect - if true return all updated rows in the callback
...
//  - delCollect - if true return all deleted rows in the callback
...
db.join = function(table, rows, options, callback)
{
    ...
    rows.forEach(function(x) {
        var key = self.getQueryForKeys(mkeys, x, { keysMap: rkeys, noempty: 1 });
...
db.getLocations = function(table, query, options, callback)
{
    ...
    if (!options.geohash) {
        ...
        options.minDistance = this.getColumn(table, options.geokey).minDistance || options.minDistance || 1;
        options.distance = lib.toNumber(options.distance || options.minDistance, { float: 0, min: 0, max: 999 });
        options.start = null;
        // Have to maintain sorting order for pagination
        if (!options.sort && keys.length > 1) options.sort = keys[1];
        var geo = lib.geoHash(options.latitude, options.longitude, { distance: options.distance, minDistance: options.minDistance });
...
// - `now` - means on every add/put/update set this column with current time as Date.now(), obsolete, use type "now"
...
// - `joinOps` - an array with operations for which perform columns join only, if not specified it applies for all operations, allowed values: add, put, incr, update, del, get, select
//
...
// all or some indexes it can be specified as `projections`, can be a number/boolean or an array with index numbers:
//
...
db.refreshColumns = function(options, callback)
{
    ...
    lib.forEachLimit(pools, pools.length, function(pool, next) {
        if (!self.pools[pool.name].configOptions.cacheColumns) return next();
        self.cacheColumns(pool.name, next);
    }, callback);
}
...
{
    if (!req.options || !lib.searchObj(req.options.updateOps, { hasvalue: "incr", count: 1 })) {
...
db.prepareForUpdate = function(pool, req, orig)
{
    ...
    for (var p in req.obj) {
        ...
        if (col) {
            ...
            if (col.notempty && lib.isEmpty(v)) {
                if (req.op == "update" || req.op == "incr") continue;
    ...
    for (var p in req.columns) {
        ...
        if (req.op == "add" || req.op == "put") {
            ...
            if (col.type == "uuid" && !lib.isUuid(req.obj[p])) req.obj[p] = lib.uuid(col.prefix);
        }
        ...
        if (col.type == "now") req.obj[p] = Date.now();
        ...
        this.joinColumn(req, req.obj, p, col, orig);
    }
}
...
db.prepareForDelete = function(pool, req, orig)
{
    ...
    req.obj = o;
    for (var p in req.columns) {
        this.joinColumn(req, req.obj, p, req.columns[p], orig);
    }
}
...
db.prepareForSelect = function(pool, req, orig)
{
    ...
    for (var p in req.obj) {
        col = req.columns[p];
        if (col && col.hidden) continue;
        if ((p == "$or" || p == "$and") || !this.skipColumn(p, req.obj[p], req.options, req.columns)) o[p] = req.obj[p];
    }
    req.obj = o;
...
db.prepareForList = function(pool, req, orig)
{
    ...
    for (var i = 0; i < req.obj.length; i++) {
        for (var p in req.columns) {
            ...
            // Joined values for queries, if nothing joined or only one field is present keep the original value
            this.joinColumn(req, req.obj[i], p, col, orig);
...
db.getSortingColumn = function(table, options)
{
    ...
    if (options.sort[0] == "_") return options.sort;
    var cols = this.getColumns(table, options);
...
db.joinColumn = function(req, obj, name, col, orig)
{
    ...
    if (Array.isArray(col.joinOps) && col.joinOps.indexOf(op) == -1) return;
    var separator = col.separator || this.separator;
    if (typeof obj[name] == "string" && obj[name].indexOf(separator) > -1) return;
    var c, d, v = "", n = 0;
    var ops = req.options && req.options.ops, strict_join = req.options && req.options.strict_join;
    for (var i = 0; i < col.join.length; i++) {
        ...
        if (d) {
            ...
            if (col.strict_join || strict_join) return;
            switch (ops && ops[name]) {
            case "lt":
            case "le":
            case "gt":
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (typeof row[name] == "string" && row[name].indexOf(separator) > -1) {
                ...
                if (v.length >= unjoin.length) {
                    for (var j = 0; j < unjoin.length; j++) row[unjoin[j]] = v[j];
                }
            }
        }
    }
...
db.skipColumn = function(name, val, options, columns)
{
    ...
    if (options) {
        ...
        if (Array.isArray(options.allow_pools) && options.allow_pools.indexOf(options.pool || this.pool) == -1) return true;
...
{
    var pool = this.getPool(table, options);
    return options && options.cache2Ttl || this.cache2[pool.name + "-" + table] || this.cache2[table];
}
