    ...
    for (var p in req.columns) {
        ...
        case "between":
            if (!Array.isArray(req.obj[p])) {
                if (req.obj[p]) {
                    req.obj[p] = lib.strSplitUnique(req.obj[p], null, { datatype: col.type });
                } else {
                    delete req.obj[p];
                }
            }
...
//      db.getFilteredColumns("bk_account", { type: "now" })
//
db.getFilteredColumns = function(table, filter)
{
    ...
    for (var name in filter) {
        ...
        for (var p in cols) {
            ...
            if ((!reverse && typeof col[name] != "undefined") || (reverse && col[name] == "undefined")) {
                if (v === null || v == col[name]) obj[p] = col;
            }
        }
    }
    return obj;
