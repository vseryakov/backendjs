 ...
 * @property {boolean|function} tryCatch - call the callback via {@link module:lib.tryCatch},
 * in case of exception if tryCatch is a function it will be called with the raised error and same arguments, useful in case some continueation is required or
 * returning an error from deep context back to the user.
 * ```js
 ...
 * @property {regexp} ignore_error - clear errors occurred as it never happen, do not report in the log, if an array then only matched codes will be cleared
 ...
 * @property {boolean} info_query - to return the record just processed in the info object as __query__ property, it will include all generated and updated columns like uuid...
 * @property {boolean} result_query - to return the query record as result including all post processing and new generated columns,
 * this is not what __returning__ property does, it only returns the query record with new columns from memory like autogenerated uuid...
 * @property {string} returning - return record values after operation:
 *  - `* or new` - return new record, update works for all databases, add only works for SQL, DynamoDB/ElasticSearch will
 * return the input record, making it similar to `result_query`
 ...
 * @property {object} ops - operators to use for for properties, an object with column name and operator, for query methods like get, select
 * ops is used for filter condition, for update methods like add/put/update/incr the ops define how a column is modified,
 * see {@link module:db} for description about all comparison operators.
 ...
 * @property {string|object} start - start position by primary keys for NoSQL, this is the __next_token__ passed by the previous query from {@link module.db.select} or
 *  for SQL it is OFFSET position.
 * @property {int} page - starting page number for pagination, uses count to find actual record to start, for SQL databases mostly
 * @property {int} count - how many records to retrieve in a single batch or the size of a page for SQL
 ...
 * @property {string|string[]|object[]} sort - sort by column(s). if not provided then no sorting must be done at all,
 * records will be returned in the order they are kept in the DB.
 ...
 *  - Elasticsearch: by default page based pagination is used if no __fullscan__ is provided with or without the __sort__ option, can be an object
 * in native ES format: { name: { order: "desc" }}, special name `_random` to return in random order using script
 * @property {boolean} desc - if sorting, do in descending order for all columns
 ...
 * @property {object} query - an object with the conditions for the update/incr, in SQL this is added to WHERE,
 *    it is used instead of or in addition to the primary keys in the __query__,
 *    a property named $OR/$AND/$NOT will be treated as a sub-expression if it is an object. For multiple OR/AND use $$OR, $$$OR,...
 ...
 * properties starting with _ or $ are ignored and used for other specific purposes.
 * @typedef {object} DbTable
 * @property {Array<DbTableColumn>} ... - column definitions
 *
 * @property {object} _$db - Common options to apply during table creation
 * @property {object} _$sqlite - Sqlite statements to append to create table statement, e.g. ```_$sqlite: { sql_extra: "STRICT" } ```
 * @property {object} _$pg - PostgreSQL statements to apply during table creation
 * @property {object} _$dynamodb - DynamoDB options to apply during table creation
 * @property {object} _$elasticsearch - Elasticsearch options to apply during table creation
 */

/**
 * @enum {string} DbColumnType
 * @description Supported database column types usable in {@link module:db.DbTableColumn#type}
 * @property {string} int - numeric type
 * @property {string} bigint - numeric type
 * @property {string} log - numeric type
 * @property {string} real - numeric type
 * @property {string} float - numeric type
 * @property {string} number - numeric type
 * @property {string} bool - stored as boolean type
 * @property {string} boolean - stored as boolean type
 * @property {string} text - text type
 * @property {string} varchar - text type
 * @property {string} str - text type
 * @property {string} string - text type
 * @property {string} keyword - text type
 * @property {string} date - Date stored in database, usually as text in SQL
 * @property {string} time - Date stored in database, usually as text in SQL
 * @property {string} timestamp - Date stored in database, usually as text in SQL
 * @property {string} mtime - timestamp in milliseconds
 * @property {string} json - stored as JSON text but converted to/from native Javascript objects
 * @property {string} obj - native JSON object type
 * @property {string} object - native JSON object type
 * @property {string} array - native JSON array type
 * @property {string} list - store a list of primitive types, strings and/or numbers
 * @property {string} set - a unique set of string or numbers if supported natively, otherwise same as list
 * @property {string} random - generates a random number in {@link module:db.add}/{@link module:db.put} methods, uses optional .max/.min properties
 * @property {string} uuid - autogenerate the column value with UUID, optional _prefix_ property will be prepended, _{ type: "uuid", prefix: "u_" }__
 * @property {string} suuid - autogenerate the column value with UUID
 * @property {string} sfuuid - autogenerate the column value with UUID
 * @property {string} now - defines a column to be automatically filled with the current timestamp in milliseconds
 * @property {string} counter - defines a column that will be automatically incremented by the {@link module:db.incr} command, on creation it is set with 0
 */

/**
 * @summary Database column definition, all properties are optional
 * @typedef {object} DbTableColumn
 * @property {DbColumnType|String} type - column type, supported types: {@link module:db.DbColumnType}
 * @property {int} primary - column is part of the primary key, for composite keys the number defines the place starting with 1
 * @property {int} index - column is part of an index, the value is a number for the column position in the index, this creates a regular index
 * @property {int} indexN - additonal indexes where N is a number, e.g. index1, index2, ...
 ...
 * @property {boolean} keyword - this column must be used as is, no parsing/stemming (Elasticsearch),
 * same as type: "keyword" in case the type is specified already
 * @property {boolean} read_only - only add/put operations will use the value, incr/update will ignore it, this happens in the app not database
 ...
 * it will use the given record to produce a new property, all properties must exist even if empty, any undefined value will
 * skip the whole join
 * @property {string} separator - to be used as a separator for joins or lists converted into string
 *
 ...
 * @property {string} foreign.on_delete - action on delete, cascade, ...
 * @property {string} foreign.custom - additional SQL statements
 *
 * @property {object} [split] - split returned value, options for {@link module:lib.split}, this is also used implicitly by all
 * list types like `list, set`.
 *
 * @property {object} [convert] - convert the value on save, all operations will be done in the order of definition
 ...
 * @property {int|string} convert.increment -  for numeric columns add this value before saving,
 *    if it is a string `now` then add Date.now()
 ...
 * @property {function} convert.format - a function (val, req) => {} that must return new value for the given column,
 * for custom formatting, always runs last
 *
 * @property {object} validate - validation checks before save, in-app checks, supports a subset of {@link module:lib.validate}
 * properties, all other properties are ignored
 * @property {int} validate.max - return an error if a __text__, __json__ or __obj__ value is greater than specified limit, unless __trunc__ is provided
 * @property {int} validate.max_list - max number of items in the __list__, __set__ or __array__ column types
 * @property {boolean} validate.trunc - do not error but truncate the column string or a list, the value will be truncated before saving
 * into the DB, uses the __max__ or __max_list__ as the threshold
 * @property {boolean} validate.skip_empty - ignore the column if the value is empty, i.e. null or empty string, i.e. no storing empty data
 * @property {boolean} validate.not_empty - return an error if there is no value for the column, this is checked during record
 * preparing in the code, not by a database
 *
 * @property {boolean|object} cleanup - cleanup the row according to {@link module:db.cleanupResult} logic, if not defined this
 * column will be removed from the result, `false` means do not cleanup explicitly.
 * @property {string[]} [cleanup.roles] - keep the field if a user has any role
 * @property {string[]} [cleanup.no_roles] - remove the field if a user has no any role

 * @property {string} _$db - Common pool specific instructions to apply to this column when creating a table
 * @property {string} _$sqlite - Sqlite specific instructions to apply to this column when creating a table, e.g.
 * - ``` details: { type: "text", _$sqlite: { sql_extra: "HIDDEN" } }```
 * - ``` data: { type: "obj", _$sqlite: { sql: "@name@ UNINDEXED" } } ```
 * @property {string} _$pg - PostgreSQL specific instructions to apply to this column when creating a table
 * @property {object} _$dynamodb - DynamoDB specific instructions to apply to this column when creating a table, e.g.
 * - ``` id: { type: "text", index: 1, _$dynamodb: { projection1: ["name"], global: 1 } }```
 * @property {object} _$elasticsearch - Elasticsearch properties to apply to this column when creating a table, e.g.
 ...
 */
  ...
  * @property {boolean} typesMap - type mapping, convert lowercase type into other type supported by any specific database, e.g. "long": "integer"
  * @property {boolean} opsMap - ops mapping, convert lowercase op into other op supported by any specific database, e.g. "gt": ">="
  * @property {object} features
  * @property {boolean} features.multi - database supports multiple SQL commands separated by ;
  * @property {boolean} ifexists - supports IF EXISTS on table or indexes
  * @property {boolean} auto - supports for auto increment columns
  * @property {boolean} not_null - supports NOT NULL constraint
  */
 ...
 * ## Concepts
 *
 * **The goal is not to create another ORM to support all databases but to make
 * the API usable for simple/common use cases and easily fallback to native access for each database.**
 *
 * By simple access it means:
 * - read a row by primary key
 * - update a row by primary key
 * - delete a row by primary key
 * - select rows by primary key and/or other condition
 *
 ...
 *
 * Conversion between JavaScript types and database types is unified to some degree meaning JavaScript data types
 * will be converted into the corresponding data types supported by any particular database and vice versa.
 *
 ...
 * {@link module:db.sql} API call, all high level operations like add/put/del perform SQL generation for single table on the fly.
 *
 * All queries and update operations ignore properties that starts with underscore.
 *
 * ## Configuration
 *
 * All database methods can use the default db pool or any other available db pool by using __pool: name__ in the options.
 * If not specified then default db pool is used, __none__ is default if no -db-pool config parameter specified in the command line or the config file.
 *
 ...
 *
 * ```js
 * db.get("users", { id: "123" }, { pool: "pg" }, (err, row) => {
 *    if (row) {
 *       ......
 ...
 * });
 *
 * const { data } = await db.aget("users", { id: "123" });
 *
 * const { err, data } = await db.aselect("users", { type_$in: ["admin", "manager"] });
 *
 * const { err } = await db.aupdate("users", { id: 123, type: ["admin", "staff"] });
 *
 * const { err } = await db.adel("users", { id: 123 });
 * ```
 *
 * Most database pools can be configured with options __min__ and __max__ for number of connections to be maintained,
 * so no overload will happen and keep warm connection for faster responses.
 *
 * ```
 * db-pg-pool-max = 100
 ...
 * ```
 * and use in Javascript by alias using callback or await:
 *
 * ```js
 ...
 *
 * These are used in query condutions and filters:
 *
 * ### Generic
 *  - `>, gt, <, lt, =, eq, !=, <>, ne, >=, ge, <=, le, between` - simple operators
 *  - `in, not in` - compare a column value against a list, as in SQL `val IN (...)`
 *  - `begins_with, not_begins_with, like%, not like%` - column value starts with given string
 *  - `ends_with, not_ends_with, %like, not %like` - column value ends with given string
 *  - `like, not like` - simple SQL LIKE
 *  - `ilike%, not ilike%` - case insensitive SQL LIKE
 *  - `includes, not includes, %like%, not %like%` - column string value contains given substring
 *  - `all_in, not all_in` - column list/set contains all given list values
 *  - `contains, not_contains` - columns list/set contains any of the given list values
 *  - `expr` - SQL expression as is
 *  - `none` - ignore this column
 *
 * ### NULL check
 *
 * - `null` - explicit IS NULL, value is ignored
 * - `not_null` - explicit IS NOT NULL, value is ignored
 * - `eq`, no op - if value is `null` then it corresponds to "IS NULL"
 * - `ne` - if value is `null` then it corresponds to "IS NOT NULL"
 *
 * ### Sqlite FTS
 *
 ...
 * ### Postgres
 *
 *  - `&&, not &&` - corresponds to generic `in` for text[] columns, used by postgres pool for `in` automatically
 *  - `@>, not @>` - corresponds to generic `contains` for text[] columns, used by postgres pool for `contains` automatically
 *  - `<@, not <@` - Postgres operators used as is, refer to PG docs
 *  - `similar to, not similar to` - Postgres operators used as is
 *  - `regexp, iregexp` - Postgres operators used as is
 *  - `~* any, ! ~* any` - Postgres operators used as is
 *  - `~, ~*, !~", !~` - Postgres operators used as is
 *
 * Operators can be used inside column names, in the format **name_$op** or via `ops` the object in the options:
 *
 * ```js
 * await db.aselect("test", { flags_$contains: ["flag1", "flag2"] });
 *
 * await db.aselect("test", { flags: ["flag1", "flag2"] }, { ops: { flags: "contains" } });
 *
 * await db.aselect("test", { status_$in: ["ok", "done"] });
 *
 * await db.aselect("test", { status: ["ok", "done"] }, { ops: { status: "in" } });
 *
 * await db.aselect("test", { name_$begins_with: "A" });
 *
 * await db.aselect("test", { name: "A" }, { ops: { name: "begins_with" } });
 *
 * await db.aselect("test", { date_$gt: Date.now() - 86400000 });
 *
 * await db.aselect("test", { date: Date.now() - 86400000 }, { ops: { date: "gt" } });
 *
 * await db.aselect("test", { status_$null: "" });
 * await db.aselect("test", { status_$not_null: "" });
 *
 * await db.aselect("test", { status: null });
 * await db.aselect("test", { status_$ne: null });
 *
 * ```
 *
 * ## Update operators
 *
 * These are used in incr/update/del
 *
 *  - `none` - ignore this column, join columns will not be built
 *  - `add` - add to list
 *  - `del` - remove from list
 *  - `incr` - increment a number
 *  - `unset` - remove column or set to NULL
 *  - `remove` - same as unset
 ...
 *  - `expr` - full set SQL expression as is or an array of values with element 0 being the expression
 *    and the rest as values for placeholders:  **["name=COALESCE(name,?)||?", 'N/A', ' jr']**
 *
 * Operators can be used inside column names, in the format **name_$op** or via `ops` the object in the options:
 *
 * ```js
 * await db.aupdate("test", { flags_$add: "flag" });
 *
 * await db.aupdate("test", { flags: "flag" } ,{ ops: { flags: "add" } });
 *
 * await db.aupdate("test", { flags_$del: "flag" });
 *
 * await db.aupdate("test", { flags: "flag" } ,{ ops: { flags: "del" } });
 *
 * await db.aupdate("test", { flags_$none: "" });
 *
 * await db.aupdate("test", { flags: "flag" }, { ops: { flags: "none" } });
 *
 * await db.aupdate("test", { count_$incr: 3 } });
 *
 * await db.aupdate("test", { count: 3 } , { ops: { count: "incr" } });
 *
 * await db.aupdate("test", { tag_$expr: "tag=COALESCE(name,'none')||count" });
 *
 * await db.aupdate("test", { tag: "tag=COALESCE(name,'none')||count" }, { ops: { tag: "expr" }});
 *
 * await db.aupdate("test", { tag_$expr: ["tag=COALESCE(name,?)||count", "none"] });
 * ```

 * ## Caching
 *
 * In-app caching is supported, it can use any available cache that implements `get/put` operations like `Redis, local`.
 *
 * The caching is best effort, for example when getting a record by primary key and a table is in the cche list the
 * module first asks the cache for it and returs if found, otherwise the DB is asked for the actual data and
 * cached immediately or after returning. This method is simple but may return stale data between cache get and put but does not
 * require any additional infrustructure.
 *
 * TTL can be used to expire record, global or per table.
 *
 * To enable caching for a table:
 *
 * ```
 * db-cache-tables = table1, table2, ...
 * db-cache-ttl-* = 3600000
 * ```
 *
 * The caching system has second in-process level, data is kept in local LRU cache. It is checked before regular cache
 * and require tables to be listed in `db-cache-tables`.
 * This is very useful for frequently requested data which is not updated too often.
 * This cache level can only expire, eviction is not supported.
 *
 * ```
 * db-cache2 = table1:300000, table2:30000
 * ```
 */
...
module.exports = {
     ...
     */
    args: [
        { name: "cap-(.+)", type: "number", strip: "cap-", same_type: 1, descr: "Capability parameters" },
        ...
        { name: "aliases-(.+)", obj: "aliases", no_camel: 1, reverse: 1, onparse(v,o) { o.name=this.table(o.name); return this.table(v) }, descr: "Table aliases to be used instead of the requested table name, only high level db operations will use it, low level utilities use the real table names" },
        ...
        { name: "describe-column-([a-z0-9_]+)-([a-zA-Z0-9_.]+)", obj: "columns.$1", make: "$2", type: "map", no_camel: 1, dot: 1, descr: "Describe a table column properties, can be a new or existing column, overrides existing property", example: "db-describe-column-users-name.validate = max:255", },
        { name: "cache-tables", obj: "cache", array: 1, type: "list", descr: "List of tables that can be cached: users, bk_counter. This list defines which DB calls will cache data with currently configured cache. This is global for all db pools." },
        { name: "cache-pools", obj: "cache", array: 1, type: "list", descr: "List of pools which trigger cache flushes on update." },
        { name: "cache-sync", obj: "cache", array: 1, type: "list", descr: "List of tables that perform synchronized cache updates before returning from a DB call, by default cache updates are done in the background" },
        { name: "cache-keys-([a-z0-9_]+)-(.+)", obj: "cache.keys.$1", make: "$2", no_camel: 1, type: "list", descr: "List of columns to be used for the table cache, all update operations will flush the cache if the cache key can be created from the record columns. This is for ad-hoc and caches to be used for custom selects which specified the cache key." },
        { name: "cache-ttl-(.+)", type: "int", obj: "cache.ttl", no_camel: 1, descr: "TTL in milliseconds for each individual table being cached, use * as default for all tables", },
        { name: "cache-name-(.+)", obj: "cache.name", no_camel: 1, make: "$1", descr: "Cache client name to use for cache reading and writing for each table instead of the default in order to split cache usage for different tables, it can be just a table name or `pool.table`, use `*` to set default cache for all tables", },
        { name: "cache-update-(.+)", obj: "cache.update", no_camel: 1, make: "$1", descr: "Cache client name to use for updating only for each table instead of the default in order to split cache usage for different tables, it can be just a table name or `pool.table` or `*`. This cache takes precedence for updating cache over `cache-name` parameter", },
        { name: "cache2-max", type: "int", min: 1, obj: "lru", make: "max", descr: "Max number of items to keep in the LRU Level 2 cache" },
        { name: "cache2", obj: "cache2", type: "map", map_type: "int", merge: 1, descr: "Tables with TTL for level2 cache, i.e. in the local process LRU memory. It works before the primary cache and keeps records in the local LRU cache for the given amount of time, the TTL is in ms and must be greater than zero for level 2 cache to work" },
        { name: "skip-tables", obj: "skip", array: 1, type: "list", descr: "List of tables that will not be created or modified, this is global for all pools" },
        { name: "skip-pools", obj: "skip", array: 1, type: "list", descr: "List of pools to be skipped during initialization" },
        { name: "skip-drop", obj: "skip", type: "regexp", descr: "A pattern of table names which will skip in db.drop operations to prevent accidental table deletion" },
        { name: "custom-column-([a-zA-Z0-9_]+)-(.+)", obj: "customColumn.$1", make: "$2", no_camel: 1, descr: "A column that is allowed to be used in any table, the name is a column name regexp with the value to be a type", example: "db-custom-column-users-^stats = counter", },
        { name: "cleanup-(.+)", obj: "cleanup.$1", type: "map", map_type: "bool", merge: 1, no_camel: 1, descr: "Rules for the db.cleanupResult per table, only allows on/off condition for columns", example: "db-cleanup-bk_user = email:0,phone:1" },
        { name: "([a-z0-9]+)-pool", obj: '_config.$1', make: "url", dflt: "default", descr: "A database pool name, depending on the driver it can be an URL, name or pathname, url format: `protocol://[user:password@]hostname[:port]/dbname`", example: "db-pg-pool = default" },
        ...
        { name: "([a-z0-9]+)-pool-(tables)", obj: '_config.$1.config', make: "$2", array: 1, type: "list", onupdate: "applyPoolOptions", descr: "Tables to be created only in this pool, to prevent creating all tables in every pool" },
        { name: "([a-z0-9]+)-pool-connect", obj: '_config.$1.connect', type: "map", merge: 1, logger: "warn", descr: "Connect options for a DB pool driver for new connection, driver specific" },
        { name: "([a-z0-9]+)-pool-options", obj: '_config.$1.config', type: "map", merge: 1, onupdate: "applyPoolOptions", descr: "General options for a DB pool, a simple map case", example: "db-pg-pool-options = max:25" },
        { name: "([a-z0-9]+)-pool-options-([a-zA-Z0-9_.-]+)$", obj: '_config.$1.config', camel: '-', auto_type: 1, make: "$2", onupdate: "applyPoolOptions", descr: "General options for a DB pool by name with specific type" },
        { name: "([a-z0-9]+)-pool-table-map", obj: '_config.$1.config.tableMap', type: "map", merge: 1, onupdate: "applyPoolOptions", descr: "Table mapping, aliases" },
        { name: "([a-z0-9]+)-pool-(skip-tables)", obj: '_config.$1.config', make: "$2", array: 1, type: "list", descr: "Tables not to be created in this pool" },
        { name: "([a-z0-9]+)-pool-(metrics-tables)", obj: '_config.$1.config', make: "$2", array: 1, type: "list", descr: "Tables to collect metrics in this pool" },
        { name: "([a-z0-9]+)-pool-cache2-(.+)", obj: 'cache2', no_camel: 1, strip: /pool-cache2-/, type: "int", descr: "Level 2 cache TTL for the specified pool and table, data is JSON strings in the LRU cache" },
    ...
    modules: {
        none: { defaults: Object.create(null), Pool: dbPool },
    },
    ...
    _config: Object.create(null, { none: { value: Object.create(null) } }),
    ...
    /** @var {string} - Database pool for config */
    ...
    /** @var {number} - How many simultaneous tasks to run at the same time inside one process */
    concurrency: 3,

    /** @var {number} - run db callbacks via lib.tryCatch
     * @default
     */
    tryCatch: 1,

    processRows: Object.create(null),
    processColumns: [],
    customColumn: Object.create(null),
    columns: Object.create(null),
    aliases: Object.create(null),
    poolAliases: Object.create(null),
    cleanup: Object.create(null),

    /** @var {string} - Separator to combined columns
     * @default
     */
    separator: "|",

    /** @var {string[]} - list of DDL operations
     * @default
     */
    ddlOps: ["incr","update","bulk","put","add","del","delall","updateall"],

    /** @var {regexp} - Table name valid characters, used by {@link module:db.table}
     * @default
     */
    rxTable: /^\s*([$#.a-z0-9_]+)/i,
    ...
    joins: Object.create(null),
};

db.configure = function(_options, callback)
{
    if (this.config) {
        db.initConfigTable();
    }
...
db.shutdown = function(options, callback)
{
    ...
    this._configTimer = undefined;

    const pools = Object.keys(this.pools).filter(x => x !== "none");
    lib.forEach(pools, (name, next) => {
...
db.init = function(options, callback)
{
    ...
    logger.debug("init:", "db", "start", app.role, options, Object.keys(this._config), Object.keys(this.pools));
    ...
    const pools = db.none ? [] : Object.keys(db._config).filter(x => !lib.includes(this.skip?.pools, x));
    ...
    lib.forEachLimit(pools, options?.concurrency || db.concurrency, (name, next) => {
        ...
        logger.debug("init:", "db", "check", app.role, name, options, params);

        if (params.disabled) return next();

        let pool;
        const old = db.pools[name];
        ...
        if (old && !options?.force && (!params.url || !old.url || params.url === old.url)) {
            ...
            try {
                let mod = db.modules[params.type];
                if (!mod) {
                    mod = db.modules[params.type] = require(__dirname + "/db/" + params.type);
                }
                pool = db.pools[name] = new mod.Pool(params);
            } catch (e) {
                logger.error("init:", "db", app.role, params, e.stack);
                return next();
            }
            if (old) old.shutdown();

            logger.debug("init:", "db", "done", app.role, name, options, params);
        }
        next();
    }, callback, true);
}
...
db.query = function(req, callback)
{
    ...
    if (req.op && lib.includes(req.config.metricsTables, table)) {
    ...
    req.pool.use((err, client) => {
        if (err) {
            return queryEnd(err, req, null, callback);
        }
        try {
...
function queryRun(client, req, callback)
{
    ...
    req.pool.query(client, req, (err, rows, info) => {
        rows = rows || [];
        if (!err) {
            req.info = info || Object.create(null);
...
{
    req.pool.metrics.running--;

    req.elapsed = req._timer?.end();
    req._timer = undefined;

    if (req.client) {
        req.pool.release(req.client);
        req.client = undefined;
    }
    if (!Array.isArray(rows)) rows = [];
    if (err) {
        req.pool.metrics.err_count++;
        logger.logger(req.options.logger_error || "error", "queryEnd:", "db", req, err);
    } else {
        if (req.info) req.info.count = rows.length;
        logger.logger(req.options.logger_db && "warn" || "debug", "queryEnd:", "db", req, req.info?.affected_rows || rows.length, 'rows');
    }
    if (err) {
        err = db.convertError(req, err);
        if (lib.isFunc(req.options.ignore_error?.test) && req.options.ignore_error.test(err.code)) err = null;
    }
    if (req.info?.retry_count > 0) {
        req.pool.metrics.retry_count += req.info.retry_count;
    }
    let info = req.info;
    if (req.options.info_query) {
        if (!info) info = Object.create(null);
    ...
    callback = lib.isFunc(callback || req.callback);
    if (!callback) return;

    rows = req.options.first ? rows[0] : req.options.last ? rows.at(-1) : rows;

    if (db.tryCatch || req.options.tryCatch) {
        const e = lib.tryCatch(callback, err, rows, info);
        if (e && typeof req.options.tryCatch === "function") {
            req.options.tryCatch(e, rows, info);
        }
    } else {
...
function queryProcessCache(req, rows, callback)
{
    // With total we only have one property 'count'
    if (req.op === "select" && req.options.total) {
        return callback();
    }

    // Automatic support for getCached and global caches
    if (lib.isArray(db.cache.pools) && !lib.includes(db.cache.pools, req.pool)) {
        return callback();
    }

    // For sync cache flushes wait before returning
    const cbnext = lib.includes(db.cache.sync, req.table) ? callback : undefined;
    switch (req.op) {
    case "bulk":
        let skipped = 0;
        lib.forEachLimit(req.query, db.concurrency, (row, next) => {
            db.delCacheKeys(row, rows, req.options, (e) => {
                if (Number.isNaN(e)) skipped++; else skipped = 0;
                if (skipped < lib.maxStackDepth) return next();
                setImmediate(next);
                skipped = 0;
            });
        }, cbnext, true);
...
function queryProcessResult(req, rows)
{
    ...
    if (req.options.result_query) {
        rows = [ Object.assign(Object.create(null), req.query) ];
    }
    ...
    return db.runProcessRows("post", "*", req, rows.length ? rows : {});
}
 ...
 */
db.queryProcessSync = function(pool, req, _row)
{
    ...
    case "del":
        ...
        const table = req.op === "bulk" ? req.query[0]?.table : req.table;
        if (!lib.includes(pool.config.tables, table)) return;
 ...
 * @param {object|string|any[]} query - query by primary key(s)
 * @param {DbRequestOptions} [options]
 * @param {DbResultCallback} callback
 *
 * NOTE: On return the `info.cached` will be set if caching is enabled, it uses {@link module:db.getCached}:
...
db.get = function(table, query, options, callback)
{
    ...
    case "array":
        query = this.getKeys(table, options).
                     reduce((a, b, i) => { a[b] = query[i]; return a }, {});
        break;
    }

    if (lib.isEmpty(query)) return callback(null, []);

    if (this.checkCached("get", table, query, options, callback) === true) return;

    const req = this.prepare({ op: "get", table, query, options });
 ...
 * @param {DbRequestOptions} [options]
 * @param {DbResultCallback} callback as (err, rows, info)
 *
...
db.select = function(table, query, options, callback)
{
    ...
    if (options?.cacheKey || options?.cacheKeyName) {
        if (this.checkCached("select", table, query, options, callback) === true) return;
    }
 ...
 *
 * Query in general is an object or text string with the format that is supported by the underlying driver,
 * the db module *DOES NOT PARSE* the query at all.
 *
 * Options make take the same properties as in the `select` method.
 *
 * A special query property `q` may be used for generic search in all fields (Elasticsearch)
 *
 ...
 *
 * @example
 * db.incr("bk_counter", { id: '123', likes: 1, invites: 1 }, (err, rows, info) => {
 * });
 *
 * @memberOf module:db
 * @method incr
 ...
 * @async
 * @example
 * const { err, data, info } = await db.aincr("bk_counter", { id: '123', likes: 1, invites: 1 })
 * @method aincr
 */
 ...
 * const opts = {
 *     query: { first_name: "Carl" }
 * }
 *
 * db.update("users", { login: 'test', first_name: 'John' }, opts, (err, rows, info) => {
 *    console.log('updated:', info.affected_rows);
 * });
 *
 ...
 *     pool: "elasticsearch"
 * }
 * db.update("users", { first_name: 'John' }, opts, (err, rows, info) => {
 *    console.log('updated:', info.affected_rows);
 * });
 *
 ...
 * @method updateAll
 */
db.updateAll = function(table, query, update, options, callback)
{
    if (typeof options === "function") callback = options, options = null;
...
db.aupdateAll = function(table, query, update, options)
{
    return new Promise((resolve, _reject) => {
        db.updateAll(table, query, update, options, (err, data, info) => {
            resolve({ err, data, info });
        });
    });
}
 ...
 * @param {string} table - table to use
 * @param {string|string[]|object[]} query - list of primary keys
 * @param {DbRequestOptions} [options]
 * @param {DbResultCallback} callback
 * @example
...
db.list = function(table, query, options, callback)
{
    ...
    case "array":
        ...
        if (typeof query[0] === "string" || typeof query[0] === "number") {
            const keys = this.getKeys(table, options);
 ...
 * - DynamoDB: only supports `add/put/del` and 25 at a time, if more specified it will send multiple
 *   batches sequentially, **NOTE:** empty values or NULLs are not supported
 ...
 *    - rqlite uses native bulk writes
 *
 * @param {object[]} list - an array of objects to get/put/update/delete from the database in the format:
 * @param {DbRequestOptions} [options]
...
db.bulk = function(list, options, callback)
{
    ...
    if (!req.query.length) return callback(null, []);
    this.query(req);
}
...
db.abulk = function(list, options)
{
    return new Promise((resolve, _reject) => {
        db.bulk(list, options, (err, data, info) => {
            resolve({ err, data, info });
        });
    });
}
 ...
 *
 *  - DynamoDB: only 25 operations can be done at the same time, if the list is larger then it will be
 *    sequentially run with batches of 25 records.
...
db.scan = function(table, query, options, rowCallback, endCallback)
{
    ...
    // Old DynamoDB days without on-demand capacity, still can be useful for provisioned mode
    if (pool.config.features?.capacity || options.useCapacity || options.factorCapacity) {
...
db.copy = function(table, query, options, callback)
{
    ...
    const cap = db.getCapacity(table, { pool: options.pool, factorCapacity: options.factorCapacity || 0.99, minCapacity: options.minCapacity });

    let copied = 0, errors = 0, elapsed = Date.now();
    options = lib.clone(options);

    const qopts = Object.assign({
    ...
    const uopts = Object.assign({}, options.updateOptions, {
    ...
    const file = options.file === 1 || options.file === true ? table + ".json" : options.file;
    logger.logger(options.logger || "debug", "copy:", "db", table, query, options, "started:", cap.rateCapacity);
    ...
    lib.series([
        function(next) {
            ...
            const keys = db.getIndexes(table)[options.sort] || db.getKeys(table);
            const sopts = Object.assign(uopts, { desc: 1, sort: options.sort, count: 1, first: 1 });
            db.select(table, query, sopts, (_err, row) => {
                if (row) {
        ...
        function(next) {
            ...
            db.scan(table, query, qopts, (row, next2) => {
                if (options.progress && Date.now() - elapsed > options.progress) {
                    elapsed = Date.now();
                    logger.logger(options.logger || "debug", "copy:", "db", table, query, options, "progress:", copied, "records", errors, "errors", lib.toDuration(started, { age: 1 }));
                }
                if (qopts.batch) {
                    lib.series([
                        function(next3) {
                        ...
                        function(next3) {
                            ...
                            for (const i in row) {
                                ...
                                rows.push({ op: "put", table, obj: row[i], options: uopts });
                            }
                            if (file) {
                    ...
                    lib.series([
                        function(next3) {
                            if (!options.reget) return next3();
                            db.get(table, row, { pool: qopts.pool }, (_err, r) => {
                                if (r) row = r;
                                next3();
                            });
                        },
        ...
        logger.logger(options.logger || "debug", "copy:", "db", table, query, options, "done:", rc, err);
 ...
 * or if the first property contains `_$dynamodb.global` property then it is a global index.
 *
 * DynamoDB projections are defined by the `_$dynamodb.projection[N]` property in the first index property
 ...
 *
 * _$db
 *  - `sql:` - full create statement with placeholders:
 *    - @table@ - table name
 *    - @columns@ - column names
 *    - @sql_columns@ - built SQL with column definitions
 *  - `sql_extra:` - only extra text to be added after the columns section to CREATE TABLE
 *  - `index[N]` - object custom index properties, similar
 *    - sql - full SQL for the index with placeholders: @table, @index@, @columns@
 ...
 *
 * @example <caption>Generic standard table</caption>
 *
 * db.create("table2", {
 ...
 * });
 *
 * @example <caption>DynamoDB global index `id_mtime` will include `flags` property in the projection </caption>
 *
 * db.create("table1", {
 *     id: {
 *         type: "int",
 *         primary: 1,
 *         index: 1,
 *         // DynamoDB specific projection instructions
 *         _$dynamodb: { projection1: ["flags"] }
 *     },
 *     name: { primary: 2 },
 *     flags: { type: "list" },
 ...
 *
 * @example <caption>PostgreSQL GIN index or special search column</caption>
 *
 * db.create("table2", {
 *      id: { type: "int" },
 *      name: { },
 ...
 *
 * @example <caption>SQLite FTS5 virtual table using _$db object</caption>
 *
 * db.create("table2", {
 *      id: { type: "int" },
 *      name: {},
...
{
    this.query(this.prepare({ op: "drop", table, options, callback }));
}
...
db.sql = function(text, values, options, callback)
{
    ...
    if (typeof options === "function") callback = options,options = null;
    const req = this.prepare({ op: "sql", options, callback });
    req.text = text;
    req.values = values;
    this.query(req);
}
