...
db.queryEnd = function(err, req, rows, callback)
{
    ...
    var info = req.info || lib.empty;
    if (req.options && req.options.info_obj) info.obj = req.obj;
...
db.prepareForUpdate = function(pool, req, orig)
{
    ...
    for (var p in req.columns) {
        ...
        if (req.op == "add" || req.op == "put") {
            ...
            if (col.type == "uuid" && (!req.obj[p] || req.obj[p].length < 32)) req.obj[p] = lib.uuid(col.prefix);
        }
        if (col.type == "uid" && req.options.account) req.obj[p] = req.options.account.id;
        if (col.type == "uname" && req.options.account) req.obj[p] = req.options.account.name;
...
db.joinColumn = function(op, obj, name, col, options, orig)
{
    ...
    if (Array.isArray(col.join) &&
        ...
        if (typeof obj[name] != "string" || obj[name].indexOf(separator) == -1) {
            var c, d, v = "", n = 0;
            var ops = options && options.ops, strict_join = options && options.strict_join;
