...
db.init = function(options, callback)
{
    ...
    lib.forEachSeries(Object.keys(this.poolParams), function(name, next) {
        ...
        logger.log('dbinit:', core.role, params);
...
db.queryEnd = function(err, req, rows, callback)
{
    ...
    if (err) {
        ...
        logger[options.quiet ? "debug" : "error"]("queryError:", req.pool, err, 'REQ:', req.op, req.table, req.obj, req.text, req.values, 'OPTS:', req.options, lib.traceError(err));
    } else {
...
db.delAll = function(table, query, options, callback)
{
    ...
    var pool = this.getPool(table, options);
    var delOptions = options && options.delOptions;
    var process = options && typeof options.process == "function" ? options.process : null;
    ...
    this.select(table, query, options, function(err, rows) {
        ...
        lib.forEachLimit(rows, options && options.concurrency || 1, function(row, next) {
            if (process) process(row, options);
            self.del(table, row, delOptions, function(err) {
                if (err && !ignore_error) return next(err);
                db.checkCapacity(cap, next);
            });
        }, function(err) {
        ...
        if (col) {
            ...
            if (v !== null) {
                if (pool.configOptions.strictTypes) {
                    ...
                    if (pool.configOptions.noJson && col.type == "json") v = lib.stringify(v);
                }
            }
