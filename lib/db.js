...
db.queryEnd = function(err, req, rows, callback)
{
    ...
    if (err && !options.quiet) {
        ...
        logger.error("query:", req.pool, err, 'REQ:', req.op, req.table, req.obj, req.text, req.values, 'OPTS:', req.options, lib.traceError(err));
    } else {
...
db.prepareForUpdate = function(pool, req, orig)
{
    ...
    for (var p in req.columns) {
        ...
        if (col.type == "now" || col.now) req.obj[p] = Date.now();
...
db.joinColumn = function(req, obj, name, col, orig)
{
    ...
    default:
        ...
        if (!Array.isArray(col.join)) break;
        if (req.options && req.options.noJoinColumns) break;
        if (req.options && Array.isArray(req.options.skip_join) && req.options.skip_join.indexOf(name) > -1) break;
        if (Array.isArray(col.joinOps) && col.joinOps.indexOf(op) == -1) break;
        var separator = col.separator || this.separator;
        if (typeof obj[name] == "string" && obj[name].indexOf(separator) > -1) break;
        var c, d, v = "", n = 0;
