 ...
 *
 * ```js
 * await db.select("test", { flags_$contains: ["flag1", "flag2"] });
 *
 * await db.select("test", { flags: ["flag1", "flag2"] }, { ops: { flags: "contains" } });
 *
 * await db.select("test", { status_$in: ["ok", "done"] });
 *
 * await db.select("test", { status: ["ok", "done"] }, { ops: { status: "in" } });
 *
 * await db.select("test", { name_$begins_with: "A" });
 *
 * await db.select("test", { name: "A" }, { ops: { name: "begins_with" } });
 *
 * await db.select("test", { date_$gt: Date.now() - 86400000 });
 *
 * await db.select("test", { date: Date.now() - 86400000 }, { ops: { date: "gt" } });
 *
 * ```
 *
 * ## Update operators
 ...
 *
 * ```js
 * await db.update("test", { flags_$add: "flag" });
 *
 * await db.update("test", { flags: "flag" } ,{ ops: { flags: "add" } });
 *
 * await db.update("test", { flags_$del: "flag" });
 *
 * await db.update("test", { flags: "flag" } ,{ ops: { flags: "del" } });
 *
 * await db.update("test", { flags_$none: "" });
 *
 * await db.update("test", { flags: "flag" }, { ops: { flags: "none" } });
 *
 * await db.update("test", { count_$incr: 3 } });
 *
 * await db.update("test", { count: 3 } , { ops: { count: "incr" } });
 *
 * await db.update("test", { tag_$expr: "tag=COALESCE(name,'none')||count" });
 *
 * await db.update("test", { tag: "tag=COALESCE(name,'none')||count" }, { ops: { tag: "expr" }});
 *
 * await db.update("test", { tag_$expr: ["tag=COALESCE(name,?)||count", "none"] });
 * ```
