 ...
 *    so by default only a few columns are marked as public in the __bk_user__ table**
 ...
 *
 * The common convention is to pass the options object with flags that are common for all drivers or specific,
...
module.exports = {
    ...
    args: [
        ...
        { name: "describe-column-([a-z0-9_]+)-([a-zA-Z0-9_.]+)", obj: "columns.$1", make: "$2", type: "map", nocamel: 1, dot: 1, descr: "Describe a table column properties, can be a new or existing column, overrides existing property", example: "-db-describe-column-bk_user-name.check max:255", },
 ...
 *
 * // same as above but also if created within 24hrs
 *
 * db.select("bk_user", { roles: 'admin', $or: { mtime_$gt: Date.now()-86400000 }, ctime_$gt: Date.now()-86400000 }, (err, rows) => { });
 *
