...
// Return a names global cche key for a table
...
db.delCacheKeys = function(table, obj)
{
    var keys = this.cacheKeys[table];
    for (var i in keys) {
        var key = keys[i].map(function(x) { return obj[x] }).join(":");
        if (!key) continue;
        logger.debug("delCacheKeys:", table, key)
        this.delCache(table, obj, { cacheKey: table + ":" + key });
    }
}
