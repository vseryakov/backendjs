...
db.copy = function(table, query, options, callback)
{
    ...
    var uopts = lib.objMerge(options.updateOptions, { region: options.region, pool: options.pool, endpoint: options.endpoint, upsert: true });
...
db.prepareForList = function(pool, req)
{
    ...
    for (var i = 0; i < req.obj.length; i++) {
        ...
        for (const p in row) {
            if (!cols[p] || lib.isEmpty(row[p])) delete row[p];
        }
