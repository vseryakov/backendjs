...
db.queryResult = function(err, req, rows)
{
    ...
    if (!options.noprocessrows) {
        rows = this.runProcessRows("post", req.table, req, rows.length ? rows : lib.empty);
    }
    // Always run global hooks
    rows = this.runProcessRows("post", "*", req, rows.length ? rows : lib.empty);
...
db.updateAll = function(table, query, obj, options, callback)
{
    ...
    if (!options.select && !(options.updateCollect || process || filter)) {
        options = lib.objClone(options, "select", db.getKeys(table));
    }
    this.scan(table, query, options, function(row, next) {
        ...
        logger.debug("updateAll:", table, query, nupdated, "records");
...
db.delAll = function(table, query, options, callback)
{
    ...
    this.scan(table, query, options, function(row, next) {
        ...
        logger.debug("delAll:", table, query, ndeleted, "records");
...
db.copy = function(table, query, options, callback)
{
    ...
    logger.debug("copy:", table, query, options, "started:", cap.rateCapacity);
    ...
    lib.series([
        ...
        logger.debug("copy:", table, query, options, "done:", count, "records", lib.toAge(started), err);
