 ...
 * @property  {boolean} options.keep_query - only preserve op, query, table properties in the reqest after return
...
db.modules.push({ name: "none", createPool: function(opts) { return new dbPool(opts) } });
 ...
 * @method aquery
 */
db.aquery = function(req, options)
{
    return new Promise((resolve, reject) => {
        db.query(req, options, (err, data, info) => {
...
{
    req.client = client;
    pool.query(client, req, (err, rows, info) => {
        req.info = info || {};
        rows = rows || [];
        if (!err) {
...
function queryEnd(err, req, rows, callback)
{
    ...
    if (err) {
        ...
        logger.logger(req.options.logger_error || "error", "queryEnd:", req.pool, err, 'REQ:', req.op, req.table, req.query, req.text, req.values, 'OPTS:', req.options, "I:", req.info, "F:", req.failed);
    } else {
        if (req.info) req.info.count = rows.length;
        logger.logger(req.options.logger_db || req.failed && "warn" || "debug", "queryEnd:", req.pool, req.elapsed, 'ms', rows.length, 'rows', 'REQ:', req.op, req.table, req.query, req.values, 'OPTS:', req.options, "F:", req.failed);
    }
    if (err) {
    ...
    if (req.options.info_query) {
        ...
        if (req.options.info_obj.convertrows) {
            ...
            db.convertRows(req.pool, req, [info.query], req.options.info_query);
        }
    }
    if (info) info.elapsed = req.elapsed;
    var first = req.options.first, last = req.options.last, keep_query = req.options.keep_query;
...
db.queryProcessSync = function(pool, req, row)
{
    ...
    case "del":
        ...
        req = { op: req.op, table: table, obj: req.query, options: req.options };
        switch (req.op) {
        case "add":
            req.op = "put";
...
{
    if (typeof options == "function") callback = options,options = null;
    var req = this.prepare("add", table, query, options);
    this.query(req, req.options, callback);
}
...
{
    if (typeof options == "function") callback = options,options = null;
    var req = this.prepare("incr", table, query, options);
    this.query(req, req.options, callback);
}
...
db.put = function(table, query, options, callback)
{
    ...
    if (pool.put) return pool.put(table, query, options, callback);

    var req = this.prepare("put", table, query, options);
    this.query(req, req.options, callback);
}
 ...
 *  @param {} options.expected - an object with the condition for the update,
 ...
 * @callback {DBResultCallback} callback
 *
 * Note: not all database drivers support atomic update with conditions, all drivers for SQL, DynamoDB fully atomic, but other drivers
 ...
 *  db.update("bk_user", { first_name: 'John' }, { expected: { or$: { first_name: "Carl", first_name_$: null } }, (err, rows, info) => {
 *     console.log('updated:', info.affected_rows);
 *  });
 * @memberOf module:db
 * @method update
...
{
    if (typeof options == "function") callback = options,options = null;
    var req = this.prepare("update", table, query, options);
    this.query(req, req.options, callback);
}
 ...
 * - SQL: runs UPDATE via `updateAll` method
 * - NoSQL: one by one, the input is the same as for `db.scan` and every record
 *    returned will be updated using `db.update` call by the primary key, so make sure `options.select`
 *    include the primary key for every row found by the select.
 *
 ...
 * @param {string[]} [options.select] - select columns, if not specified only the primary keys will be returned or collected
 ...
 *
 * @example
 *  // update birthday format if not null
 *
...
db.updateAll = function(table, query, obj, options, callback)
{
    ...
    if (typeof pool.updateAll == "function" && !process) {
        return pool.updateAll(table, query, obj, options, callback);
    }
...
db.aupdateAll = function(table, query, obj, options)
{
    return new Promise((resolve, reject) => {
        db.updateAll(table, query, options, (err, data, info) => {
            resolve({ err, data, info });
        });
    });
}
...
{
    if (typeof options == "function") callback = options,options = null;
    var req = this.prepare("del", table, query, options);
    this.query(req, req.options, callback);
}
...
/**
 * Delete all records that match given condition
 * - SQL: runs DELETE via `delAll` method
 * - NOSQL: one by one, the input is the same as for `db.select` and every record
...
db.delAll = function(table, query, options, callback)
{
    ...
    options = lib.objClone(options, { table });
    var pool = this.getPool(options);
    var process = typeof options.delProcess == "function" ? options.delProcess : null;

    if (typeof pool.delAll == "function" && !process && !options.delScan) {
        return pool.delAll(table, query, options, callback);
    }
...
db.atransaction = function(table, query, options)
{
    return new Promise((resolve, reject) => {
        db.transaction(table, query, options, (err, data, info) => {
            resolve({ err, data, info });
        });
    });
}
...
/**
 * Async version of db.join
 * @memberOf module:db
 * @method ajoin
 */

db.ajoin = function(table, rows, options)
{
    return new Promise((resolve, reject) => {
        db.join(table, rows, options, (err, data, info) => {
            resolve({ err, data, info });
        });
    });
}
