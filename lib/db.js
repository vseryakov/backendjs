...
db.init = function(options, callback)
{
    ...
    lib.forEachSeries(Object.keys(this.poolParams), function(name, next) {
        ...
        if (cluster.isMaster && (db._createTables || pool.configOptions.createTables)) {
            return setTimeout(db.createTables.bind(db, name, next), 500);
        }
        if (pool.configOptions.cacheColumns) {
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (lib.isEmpty(v) && pool.configOptions.skipEmpty) {
