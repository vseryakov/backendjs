...
var db = {
    ...
    args: [{ name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
           ...
           { name: "([a-z0-9]+)-pool-(connect)$", obj: 'poolParams.$1', make: "$2", type: "json", logger: "warn", descr: "Connect options for a DB pool driver for new connection, driver specific" },
...
db.scan = function(table, query, options, rowCallback, endCallback)
{
    ...
    if (!options.emptyscan && !lib.isEmpty(query) && !Object.keys(this.getColumns(table)).some((x) => (typeof query[x] != "undefined"))) {
        return lib.tryCall(endCallback, { status: 400, message: "No valid columns and empty scan is not enabled" });
    }
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in columns) {
        col = columns[p];
        // Restrictions
        if (this.skipColumn(p, "", req.options, columns) ||
                col.hidden ||
...
db.prepareForSelect = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (/^\$(or|and)/.test(p) ||
            (col && col.allow) ||
            !this.skipColumn(p, req.obj[p], req.options, columns)) {
            o[p] = req.obj[p];
        }
    }
    req.obj = o;
...
{
    var col, columns = this.getColumns(req.table, req.options);
