...
db.join = function(table, rows, options, callback)
{
    ...
    var keys = options.keys || db.getKeys(table, options);
    var mkeys = options.keysMap ? keys.map(function(x) { return options.keysMap[x] || x }) : [].concat(keys);
    for (var p in cols) {
        if (cols[p].primary && Array.isArray(cols[p].join)) {
            for (var i in cols[p].join) if (mkeys.indexOf(cols[p].join[i]) == -1) mkeys.push(cols[p].join[i])
        }
    }
    ...
    db.list(table, ids, options, function(err, list, info) {
        ...
        list.forEach(function(x) {
            var key = db.getQueryForKeys(mkeys, x);
            var k = Object.keys(key).map(function(y) { return key[y] }).join(db.separator);
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (var p in req.columns) {
        ...
        if (col.type == "now") {
            ...
            if (col.random > 0) req.obj[p] += lib.randomInt(0, col.random);
        }
