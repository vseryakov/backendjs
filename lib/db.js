...
db.queryEnd = function(err, req, rows, callback)
{
    ...
    if (err) {
        ...
        var log = req.options.quiet ? "debug" : req.options.logger_error;
        ...
        logger.logger(req.options.logger_db || "debug", "queryEnd:", req.pool, req.elapsed, 'ms', rows.length, 'rows', 'REQ:', req.op, req.table, req.obj, req.values, 'OPTS:', req.options, err);
    }
    if (err) {
        ...
        function(next) {
            ...
            case "bulk":
                lib.forEachLimit(req.obj, core.concurrency, (row, next2) => {
                    db.delCachKeys(row, rows, req.options, next2);
                }, next);
                break;
            default:
                db.delCacheKeys(req, rows, req.options, next);
            }
        },
        function(next) {
...
db.prepare = function(op, table, obj, options)
{
    ...
    var req = { op: op, table: table, text: "", obj: obj, options: lib.objClone(options, "__bk", 1) };
...
db.delCacheKeys = function(req, result, options, callback)
{
    ...
    var opts = this.getCacheOptions(req.table, options);
    ...
    logger.debug("delCacheKeys:", keys);
