...
var db = {
    ...
    args: [{ name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
           ...
           { name: "([a-z]+)-pool-cache2-(.+)", obj: 'cache2', nocamel: 1, strip: /pool-cache2-/, type: "int", descr: "Level 2 cache TTL for the specified pool and table, data is JSON strings in the LRU cache" },
           { name: "([a-z]+)-pool-jscache2-(.+)", obj: 'jscache2', nocamel: 1, strip: /pool-jscache2-/, type: "int", descr: "Level 2 cache TTL for the specified pool and table, data is Js objects in the heap" },
    ],
...
db.bulk = function(list, options, callback)
{
    if (typeof options == "function") callback = options,options = null;
    for (var i in list) {
        list[i] = this.prepare(list[i].op, list[i].table, list[i].obj, list[i].options);
    }
    this.getPool("", options).bulk(list, options, callback);
}
...
db.getCache = function(table, query, options, callback)
{
    ...
    ipc.get(key, opts, function(err, val) {
        ...
        if (ttl2 < 0) {
            ...
            self.jsCache2[key] = [Date.now() - ttl2, lib.objClone(val)];
        }
...
// Bulk operations
db.Pool.prototype.bulk = function(list, options, callback)
{
    if (typeof callback == "function") callback();
}
