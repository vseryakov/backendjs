...
var db = {
    ...
    args: [{ name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
           ...
           { name: "skip-drop", type: "regexp", descr: "A pattern of table names which will skipped in db.drop operations to prevent accidental table deletion" },
...
{
    if (typeof options == "function") callback = options, options = null;
    if (this.skipDrop && this.skipDrop.test(table) && lib.isEmpty(query)) return lib.tryCall(callback, "skip-drop");
...
// On return the second arg to the callback is a list of records with errors, same input record with added property `errstatus` and `errmsg`
//
// NOTE: DynamoDB only supports add, put, del
//
// Example
//
//          var ops = [ { op: "add", table: "bk_counter", obj: { id:1, like:1 } },
...
db.copy = function(table, query, options, callback)
{
    ...
    lib.series([
        function(next) {
            if (!uopts.region && db.getPool(table, qopts) == db.getPool(table, uopts)) return next({ status: 400, message: "no copy in the same pool" });
...
// - `autoexpire` - for supported databases, mark the column to be auto expired, the value must be time in the future as Epoch seconds
...
{
    if (typeof options == "function") callback = options,options = null;
    if (this.skipDrop && this.skipDrop.test(table)) return lib.tryCall(callback, "skip-drop");
...
db.prepareRow = function(pool, req)
{
    ...
    case "bulk":
        for (const i in req.obj) {
            ...
            if (req.obj[i].error) {
                req.obj[i].errstatus = req.obj[i].error.code || req.obj[i].error.status;
                req.obj[i].errmsg = req.obj[i].error.message;
                lib.objDel(req.obj[i], "error", "columns", "options", "orig");
            }
        }
        break;
    }
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (col) {
            ...
            // Ignore if smaller then expected but only in new records
            if (col.minlength && insert && (typeof v == "string" || Array.isArray(v)) && !v.length < col.minlength) continue;
            // Skip empty values by type
            if (lib.isEmpty(v) &&
                pool.configOptions.skipEmpty &&
                pool.configOptions.skipEmpty[req.op] &&
                pool.configOptions.skipEmpty[req.op].test(col.type)) continue;
        }
        // Handle NULL constraints by type
        if ((v == null || v === "") && !(col && col.notempty) &&
            pool.configOptions.skipNull &&
            pool.configOptions.skipEmpty[req.op] &&
            pool.configOptions.skipEmpty[req.op].test(col.type)) continue;
        o[p] = v;
    }
...
db.getSelectedColumns = function(table, options)
{
    ...
    if (options && options.select && options.select.length) {
        ...
        var select = list.filter(function(x) {
...
// Returns type for a global cusgtom column if exists otherwise null, all resolved
// columns will be saved in `req.allow` for further reference as name: type.
// For request specific custom columns pass `options.custom_columns` object in the format: [ RegExp, type, ...]
