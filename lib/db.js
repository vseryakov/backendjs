...
const db = {
    ...
    args: [{ name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
           ...
           { name: "no-unknown-tables", type: "bool", descr: "Only allow to operate with known described tables, ignore existing but not known tables" },
           ...
           { name: "([a-z0-9]+)-pool-exclusive-tables$", obj: 'poolTables', make: "$1", type: "regexp", array: 1, descr: "A DB pool tables, pattern of tables that belong to this pool only" },
...
db.query = function(req, options, callback)
{
    ...
    const pool = this.getPool(req.table, req.options);
    ...
    // Prevent from using non-app tables
    if (!req.error && (this.noUnknownTables || pool.configOptions.noUnknownTables) && !this.tables[req.table]) {
        req.error = lib.newError("invalid table");
    }
    if (req.error) {
