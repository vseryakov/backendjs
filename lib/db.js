 ...
 *      returns the query record with new columns from memory like autogenerated uuid...
...
function queryResult(req, rows, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            if (lib.isArray(db.cache.pools) && !lib.includes(db.cache.pools, req.pool)) return next();

            // For sync cache flushes wait before returning
            const cbnext = lib.includes(db.cache.sync, req.table) ? next : undefined;
            switch (req.op) {
            case "bulk":
                let skipped = 0;
