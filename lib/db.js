...
var db = {
    ...
    args: [{ name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
           ...
           { name: "([a-z]+)-pool-options-([a-zA-Z0-9_\.-]+)-?([0-9]+)?$", obj: 'poolParams.$1$3.configOptions', camel: '-', autotype: 1, make: "$2", onparse: function(v,o) {this.parsePoolOptions(v,o)}, descr: "General options for a DB pool" },
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (var p in req.columns) {
        ...
        if (col.type == "now") {
            ...
            if (insert && col.random) req.obj[p] += lib.randomShort();
        }
