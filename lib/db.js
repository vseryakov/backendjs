...
//        });
//
// Example: allow all user icons to be visible
//
...
//        });
//
// Example: scan userс with custom filter, not by primary key: by exact zipcode
//
//        db.select("bk_user", { zipcode: '20000' }, (err, rows) => {
//
//        });
//
// Example: select userс by type for the last day
//
//        db.select("bk_user", { type: 'admin', mtime: Date.now()-86400000 }, { ops: { type: "contains", mtime: "gt" } }, (err, rows) => {
//
//        });
//
// Example: select userс by JSON sub property using Elasticsearch
//
...
db.list = function(table, query, options, callback)
{
    ...
    case "string":
    case "array":
        query = lib.strSplitUnique(query).filter(x => x);
...
// - `auth` - this property will be set in `req.options.user` for access permissions checks when only options are available
