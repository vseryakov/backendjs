...
const db = {
    ...
    // Config parameters
    args: [
        { name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
        ...
        { name: "custom-column-([a-z0-9_]+)-(.+)", obj: "customColumn.$1", make: "$2", nocamel: 1, descr: "A column that is allowed to be used in any table, the name is a column name regexp with the value to be a type, Ex: -db-custom-column-bk_user-^stats=counter", },
        { name: "describe-column-([a-z0-9_]+)-([a-z0-9_]+)", obj: "columns.$1", make: "$2", type: "map", maptype: "auto", nocamel: 1, descr: "Describe a table column properties, can be a new or existing column, overrides existing property, ex: -db-describe-column-bk_user-name max:255", },
        ...
        { name: "config-count", type: "number", min: 0, descr: "Max number of records to read fron the config table" },
        ...
        { name: "([a-z0-9]+)-pool$", obj: 'poolParams.$1', make: "url", novalue: "default", descr: "A database pool name, depending on the driver it can be an URL, name or pathname, examples of db pools: ```-db-pg-pool, -db-dynamodb-pool```, url format: ```protocol://[user:password@]hostname[:port]/dbname``` or ```default```" },
        ...
        { name: "([a-z0-9]+)-pool-(tables)$", obj: 'poolParams.$1.configOptions', make: "$2", array: 1, type: "list", onupdate: function(v,o) {this.applyPoolOptions(v,o)}, descr: "Tables to be created only in this pool, to prevent creating all tables in every pool" },
        ...
        { name: "([a-z0-9]+)-pool-options$", obj: 'poolParams.$1.configOptions', type: "map", maptype: "auto", merge: 1, onupdate: function(v,o) {this.applyPoolOptions(v,o)}, descr: "General options for a DB pool" },
        { name: "([a-z0-9]+)-pool-options-([a-zA-Z0-9_.-]+)$", obj: 'poolParams.$1.configOptions', camel: '-', autotype: 1, make: "$2", onupdate: function(v,o) {this.applyPoolOptions(v,o)}, descr: "General options for a DB pool" },
...
db.delAll = function(table, query, options, callback)
{
    ...
    this.scan(table, query, options, function(row, next, info) {
