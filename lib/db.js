...
//    - filterrows - function to filter rows not to be included in the result, returns a new result set, args are: function(req, rows, options)
//    - processrows - function to process rows in the result, returns a new result, args are: function(req, rows, options), this result will be put in cache
//      if requested so this may be used for preparing cached results, it must return an array
//    - processasync - function to process result rows via async callback, return a new result in the callback, the function is: function(req, rows, options, callback),
...
db.queryEnd = function(err, req, rows, callback)
{
    ...
    if (err) {
        ...
        logger.logger(log, "queryEnd:", req.pool, e, 'REQ:', req.op, req.table, req.obj, req.text, req.values, 'OPTS:', req.options, req.info, lib.traceError(e));
    } else {
...
db.queryResult = function(req, rows, callback)
{
    ...
    if (req.op == "select" && options.total) return callback(rows);

    lib.series([
        function(next) {
            // Automatic support for getCached and global caches
            if (!lib.isFlag(this.cachePools, req.pool)) return next();
            switch (req.op) {
            case "bulk":
                for (var i in req.obj) db.queryCacheUpdate(req.obj[i], rows, options);
                break;
            default:
                db.queryCacheUpdate(req, rows, options);
            }
            next();
        },
        function(next) {
            ...
            if (rows.length && !options.noconvertrows) {
                db.convertRows(req.pool, req, rows, options);
            }
            ...
            if (typeof options.filterrows == "function" && rows.length) {
                rows = options.filterrows(req, rows, options);
            }

            // Always run explicit post processing callback
            if (typeof options.processrows == "function") {
                rows = options.processrows(req, rows, options);
            }
            next();
        },
        function(next) {
            if (typeof options.processasync != "function") return next();
            options.processasync(req, rows, options, (err, rc) => {
                if (!err && rc) rows = rc;
                next();
            });
        },
...
//
// On return the second arg to the callback is a list with errors in the format, [[errcode, row],...]
//
// Example
//
//          var ops = [ { op: "add", table: "bk_counter", obj: { id:1, like:1 } },
...
db.copy = function(table, query, options, callback)
{
    ...
    lib.series([
        function(next) {
            if (db.getPool(table, qopts) == db.getPool(table, uopts)) return next({ status: 400, message: "no copy in the same pool" });
                    ...
                    lib.series([
                        ...
                        function(next3) {
                            ...
                            for (var i in row) {
                                ...
                                rows.push({ op: "put", table: table, obj: row[i] });
                            }
...
db.prepareForUpdate = function(pool, req)
{
        ...
        if (col) {
            ...
            if (v !== null) {
                ...
                default:
                    ...
                    if (col.primary || col.index || col.type || pool.configOptions.defaultType) {
    ...
    for (var p in req.columns) {
        ...
        case "tuuid":
            ...
            break;
        case "suuid":
            if (insert && !lib.isPrefix(req.obj[p], col.prefix)) req.obj[p] = lib.suuid(col.prefix, col);
            break;
        case "uid":
        ...
        if (insert && col.fail_ifempty && lib.isEmpty(req.obj[p])) {
            req.error = lib.newError(col.errmsg_ifempty || ((col.label || p) + " cannot be empty"));
        }
    }
}
...
db.prepareForDelete = function(pool, req)
{
    ...
    for (var p in req.obj) {
        ...
        if (pool.configOptions.strictTypes && typeof v != "undefined") {
            if ((col && (col.primary || col.type)) || pool.configOptions.defaultType) {
...
//
// The `options.join_hash` can be used to store a hash of the joined column to reduce the sspace and make the result value easier to use
//
...
// the pool by given name. This call always return valid pool object, in case no requested pool found, it returns
// the default pool. A special pool `none` always return empty result and no errors.
