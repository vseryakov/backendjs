 ...
 *       item 0 being expression with the rest values: ["COALESCE(name,$1)||$2", ...]
 *
 * As alternative, operators also can be used with column names, the format is **name_$op**:
 * ```js
 * { flags_$del: "flag" }
 * { count_$incr: 3 }
 ...
 * @property {object} typesOps - an object that defines ops by column type, for example `typesOps: { list: "add" }` will
 * @property {object} typesMap - type mapping between supplied and actual column types, an object
 *   make sure all lists will have options.ops set to `add` if not specified explicitly
 ...
 *    it is used instead or in addition to the primary keys in the `query`,
 ...
 *  - `now` -  defines a column to be automatically filled with the current timestamp
 ...
 * });
 *
 * // remove "admin" role from the roles column
 * db.update("bk_user", { login: "test", roles_$del: "admin" }, (err, rows, info) => {
 *    console.log('updated:', info.affected_rows);
 * });
 * @memberOf module:db
 * @method update
