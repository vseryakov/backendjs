 ...
 *    - expr - full set SQL expression as is or an array of values with
 *       element 0 being the expression and the rest as values for placeholders:
 *       `["name=COALESCE(name,?)||?", 'N/A', ' jr']`
 *
 ...
 * @property {DbTableColumn} name1 - column definition
 * @property {DbTableColumn} name2 - column definition
 ...
 * @property {boolean} check.fail_ifempty - returtn an error if there is no  value for the column, this is checked during record
...
module.exports = {
    ...
    args: [
        ...
        { name: "skip-drop", obj: "skip", type: "regexpobj", descr: "A pattern of table names which will skipp in db.drop operations to prevent accidental table deletion" },
...
function queryEnd(err, req, rows, callback)
{
    ...
    if (err) {
        ...
        logger.logger(req.options.logger_db || req.failed && "warn" || "debug", "queryEnd:", "db", req, req.info?.affected_rows || rows.length, 'rows');
    }
    if (err) {
...
function queryResult(req, rows, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            var cbnext = lib.includes(db.cache.sync, req.table) ? next : undefined;
            switch (req.op) {
            case "bulk":
                let skipped = 0;
...
db.copy = function(table, query, options, callback)
{
    ...
    var uopts = Object.assign({}, options.updateOptions, {
    ...
    lib.series([
        function(next) {
            ...
            var sopts = Object.assign(uopts, { desc: 1, sort: options.sort, count: 1, first: 1 });
...
{
    if (typeof options === "function") callback = options,options = null;
    if (lib.testRegexpObj(this.skip?.drop, table)) return lib.tryCall(callback, "skip-drop");
