...
var db = {
    ...
    args: [{ name: "pool", dns: 1, descr: "Default pool to be used for db access without explicit pool specified" },
           ...
           { name: "allow-column-([a-z0-9_]+)-(.+)", obj: "allowColumn.$1", make: "$2", nocamel: 1, descr: "A column that is allowed to be used in any table, the name is a regexp with the value to be a type", },
           ...
           { name: "config-roles", type: "list", descr: "Roles to assume when puling config parameters" },
           ...
           { name: "([a-z0-9]+)-pool-tables$", obj: 'poolTables', strip: /PoolTables/, type: "list", flatten: 1, descr: "A DB pool tables, list of tables that belong to this pool only" },
...
db.getConfig = function(options, callback)
{
    ...
    var tag = options && options.tag || core.instance.tag;
    if (tag) attrs.push(core.runMode + "-" + tag, core.runMode + "-" + core.appName + "-" + tag);
    var roles = lib.isArray(this.configRoles) || [options && options.role || core.role];
    for (var i in roles) {
        if (roles[i]) attrs.push(core.runMode + "-" + roles[i], core.runMode + "-" + core.appName + "-" + roles[i]);
    }
...
db.createTables = function(options, callback)
{
    ...
    lib.series([
      ...
      function(next) {
          ...
          logger.info('createTables:', core.role, pool.name, 'changed:', copts.tables);
          if (pool.configOptions.cacheColumns) return db.cacheColumns(copts, next);
          next();
      },
    ], callback);
}
...
//    - postfilter - function to filter rows not to be included in the result, return false to skip row, args are: function(req, row, options)
//    - postprocess - function to process rows in the result, return a new result, args are: function(req, row, options), this result will be put in cache
...
//    - logger_error - a log level to report about errors, default is 'error'
//    - ignore_error - clear errors occured as it never happen, do not report in the log, if an array then only matched codes will be cleared
...
db.queryEnd = function(err, req, rows, callback)
{
    ...
    if (err) {
        ...
        var log = options.quiet ? "debug" : options.logger_error || "error";
        logger.logger(log, "queryError:", req.pool, err, 'REQ:', req.op, req.table, req.obj, req.text, req.values, 'OPTS:', req.options, req.info, lib.traceError(err));
    } else {
        logger.logger(options.logger_db || "debug", "queryEnd:", req.pool, req.elapsed, 'ms', rows.length, 'rows', 'REQ:', req.op, req.table, req.obj, req.values, 'OPTS:', req.options, err);
    }
    if (err) {
        err = this.convertError(pool, req.table, req.op || "", err, req.options);
        if (options.ignore_error && (!lib.isArray(options.ignore_error) || lib.isFlag(options.ignore_error, err.code))) err = null;
    }
    ...
    if (options.info_obj) {
        if (!info) info = {};
        info.obj = req.obj;
        if (options.info_obj.convertrows) {
            info.obj = lib.objClone(req.obj);
            this.convertRows(req.pool, req, [info.obj], options.info_obj);
        }
    }
    for (var p in req) {
        switch (p) {
...
db.queryResult = function(err, req, rows)
{
    ...
    if (!this.cachePools.length || lib.isFlag(this.cachePools, req.pool)) {
        switch (req.op) {
        case "bulk":
            for (var i in req.obj) this.queryCacheUpdate(req.obj[i], rows, options);
            break;
        default:
            this.queryCacheUpdate(req, rows, options);
        }
    }
    ...
    if (!options.noprocessrows) {
        rows = this.runProcessRows("post", req.table, req, rows.length ? rows : {});
    }
    // Always run global hooks
    rows = this.runProcessRows("post", "*", req, rows.length ? rows : {});

    // Custom filters to return the final result set
    if (typeof options.postfilter == "function" && rows.length) {
        rows = rows.filter(function(row) { return options.postfilter(req, row, options) });
    }
    if (typeof options.postprocess == "function") {
        rows = options.postprocess(req, rows, options);
    }
    return rows;
}

db.queryCacheUpdate = function(req, rows, options)
{
    var cached = req.table && req.obj && (options.cached || this.cacheTables.indexOf(req.table) > -1);
    switch (req.op) {
    case "add":
        this.delCacheKeys(req.table, req.obj);
        break;
    case "put":
    case "update":
    case "incr":
        if (cached) this.delCache(req.table, req.obj, options);
        this.delCacheKeys(req.table, req.obj);
        if (options.returning == "*" && rows.length) this.delCacheKeys(req.table, rows[0]);
        break;
    case "del":
        if (cached) this.delCache(req.table, req.obj, options);
        this.delCacheKeys(req.table, req.obj);
...
db.select = function(table, query, options, callback)
{
    if (typeof options == "function") callback = options,options = null;
    if (options) {
        if (!options.cacheKey && options.cacheKeyName) options.cacheKey = this.getCacheKeys(table, options.cacheKeyName, query);
    ...
    this.query(req, req.options, function(err, rows, info) {
        lib.tryCall(callback, err, options && options.first ? rows && rows[0] : rows, info);
    });
}
...
//   - updateFilter - a function that must return true in order to skip the current record. `options.updateFilter(row, options)`
//
...
db.updateAll = function(table, query, obj, options, callback)
{
    ...
    var pool = this.getPool(table, options);
    var process = typeof options.process == "function" ? options.process : null;
    if (typeof pool.updateAll == "function" && !process) return pool.updateAll(table, query, obj, options, callback);

    var rows = [], nupdated = 0;
    var filter = typeof options.updateFilter == "function" ? options.updateFilter : null;
    ...
    if (!options.select && !(options.updateCollect || process || filter)) {
        options = lib.objMerge(options, { select: db.getKeys(table), noconvertrows: 1, noprocessrows: 1 });
    }
    this.scan(table, query, options, function(row, next) {
        ...
        if (filter && filter(row, options) === true) return next();
...
//    in case of complex columns that may contain concatenated values as in the case of using DynamoDB. The callback will be called
//    as `options.process(row, options)`
//   - delFilter - a function that must return true in order to skip the current record. `options.delFilter(row, options)`
//
db.delAll = function(table, query, options, callback)
{
    ...
    var rows = [], ndeleted = 0;
    var filter = typeof options.delFilter == "function" ? options.delFilter : null;
    var cap = db.getCapacity(table, { useCapacity: "write", factorCapacity: options.factorCapacity || 0.35 });

    // No need to pull all columns, just the primary key
    if (!options.select && !(options.delCollect || process || filter)) {
        options = lib.objMerge(options, { select: db.getKeys(table), noconvertrows: 1, noprocessrows: 1 });
    }
...
//   - pool - other DB pool
//   - filter - a function(table, row, options) to filter records, must return true to be skipped
//   - posprocess - a function(table, row, options, next) to call after a record copied, for recursive or joined cases
...
db.copy = function(table, query, options, callback)
{
    ...
    options = lib.objClone(options);
    var count = 0, started = Date.now();
    var cap = db.getCapacity(table, { factorCapacity: options.factorCapacity || 0.99, minCapacity: options.minCapacity });
    ...
    var uopts = { region: options.region, pool: options.pool, endpoint: options.endpoint, upsert: true };
    logger.logger(options.logger || "debug", "copy:", table, query, options, "started:", cap.rateCapacity);

    lib.series([
        function(next) {
            if (!options.incremental) return next();
        ...
        function(next) {
            db.scan(table, query, qopts, function(row, next2) {
                lib.series([
                    ...
                    function(next3) {
                        if (typeof options.filter == "function" && options.filter(table, row, options)) return next3();
        ...
        logger.logger(options.logger || "debug", "copy:", table, query, options, "done:", count, "records", lib.toAge(started), err);
...
db.prepare = function(op, table, obj, options)
{
    ...
    pool.prepare(req);
    logger.debug("prepare:", table, req);
    return req;
}
...
db.prepareRow = function(pool, req)
{
    ...
    if (type == "object") {
        for (var p in req.obj) {
            ...
            if (!pool.configOptions.noAllowColumns && !req.columns[p]) {
                this.checkAllowColumn(req, p);
            }
        }
    }
...
{
    var o = {}, v, col, updateOps = req.options && req.options.updateOps || lib.empty;
    var insert = req.op == "add" || req.op == "put";
    for (var p in req.obj) {
        v = req.obj[p];
        col = req.columns[p];
        if (!col && req.allow) col = req.allow[p];
        if (!(col && col.allow) && this.skipColumn(p, v, req.options, req.columns)) continue;
        if (col) {
            ...
            if (v !== null) {
                ...
                default:
                    if (pool.configOptions.strictTypes) {
                        if (col.primary || col.index || col.type) v = lib.toValue(v, col.type, col);
                    }
                }
            }
            ...
            if (col.maxlength && typeof v == "string" && lib.isTextType(col.type) && v.length > col.maxlength) v = v.substr(0, col.maxlength);
    ...
    for (var p in req.columns) {
        ...
        case "tuuid":
            ...
            break;
        case "uid":
            if (req.options && req.options.account) req.obj[p] = req.options.account.id;
            break;
        case "uname":
            if (req.options && req.options.account) req.obj[p] = req.options.account.name;
            break;
        case "now":
        ...
        if (insert && col.fail_ifempty && lib.isEmpty(req.obj[p])) {
            req.error = lib.newError(p + " cannot be empty");
        }
    }
}
...
db.prepareForSelect = function(pool, req)
{
    ...
    for (var p in req.obj) {
        ...
        if (/^\$(or|and)/.test(p) ||
            (col && col.allow) ||
            !this.skipColumn(p, req.obj[p], req.options, req.columns)) {
            o[p] = req.obj[p];
        }
    }
    req.obj = o;
    ...
    for (var p in req.columns) {
        ...
        ops = req.options && req.options.ops || lib.empty;
...
db.joinColumn = function(req, obj, name, col, orig)
{
    ...
    case "geohash":
        if (obj[name]) break;
        var lat = obj[col.lat || "latitude"] || orig[col.lat || "latitude"];
        var lon = obj[col.lon || "longitude"] || orig[col.lon || "longitude"];
        if (lat && lon) obj[name] = lib.geoHash(lat, lon, { minDistance: col.minDistance }).geohash;
        break;
...
db.convertRows = function(pool, req, rows, options)
{
    ...
    var col, cols = req.columns || this.getColumns(req.table, req.options || options);
    ...
    for (var p in cols) {
        ...
        if (cols[p].dflt && !opts.noconvertrows_dflt) {
            for (var i = 0; i < rows.length; i++) {
                if (!rows[i][p]) rows[i][p] = lib.objClone(cols[p].dflt);
            }
        }
...
db.getCapacity = function(table, options)
{
    ...
    cap.maxCapacity = Math.max(0, typeof use == "number" ? use : use == "read" ? cap.readCapacity : cap.writeCapacity, options.minCapacity || 0);
...
// Returns type for a global column if exists otherwise null, if cache is an object all resolved
// columns will be saved in it for further reference as name: type
db.checkAllowColumn = function(req, name)
{
    var cols = this.allowColumn[req.table];
    for (var p in cols) {
        var col = cols[p];
        if (typeof col == "string") {
            col = [ lib.toRegexp(p), col];
            this.allowColumn[req.table][p] = col;
        }
...
{
    var ttl = (options && options.cacheTtl) || this.cacheTtl[table] || this.cacheTtl.default;
    var cacheName = (options && options.cacheName) || (options && options.pool ? this.cacheName[options.pool + "." + table] : "") || this.cacheName[table];
...
// Return a named global cache key for a table
db.getCacheKeys = function(table, name, obj)
{
    if (!table || !name || !obj) return "";
    var keys = this.cacheKeys[table];
    if (keys) keys = keys[name];
    if (keys) keys = keys.map(function(x) { return obj[x] }).join(":");
    if (keys) return table + ":" + name + ":" + keys;
    return "";
}

// Flush all global caches for a table
db.delCacheKeys = function(table, obj)
{
    var keys = this.cacheKeys[table];
    for (var p in keys) {
        var key = keys[p].map(function(x) { return obj[x] }).join(":");
        if (!key) continue;
        logger.debug("delCacheKeys:", table, p, key)
        this.delCache(table, obj, { cacheKey: table + ":" + p + ":" + key });
    }
}
