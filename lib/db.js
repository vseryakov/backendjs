 ...
 * same as type: "keyword" in case the type is taken
 ...
 *
 * @property {object} check - validation checks on save
 * @property {int} check.max - return an error if a __text__, __json__ or __obj__ value is greater than specified limit, unless __trunc__ is provided
 * @property {int} check.maxlist - max number of items in the __list__, __set__ or __array__ column types
 * @property {boolean} check.trunc - do not error but truncate the column string or a list, the value will be truncated before saving
 * into the DB, uses the __max__ or __maxlist__ as the threshold
 * @property {boolean} check.skip_zero - true if should be skipped if 0 value
 * @property {boolean} check.skip_empty - ignore the column if the value is empty, i.e. null or empty string, i.e. no storing empty data
 * @property {boolean} check.not_empty - return an error if there is no  value for the column, this is checked during record
 ...
 */
  ...
  * @property {boolean} no_ifexists - do not support IF EXISTS on table or indexes
  * @property {boolean} no_auto - no support for auto increment columns
  */
...
module.exports = {
     ...
     */
    args: [
        { name: "cap-(.+)", type: "number", strip: "cap-", sametype: 1, descr: "Capability parameters" },
        ...
        { name: "aliases-(.+)", obj: "aliases", nocamel: 1, reverse: 1, onparse(v,o) { o.name=this.table(o.name); return this.table(v) }, descr: "Table aliases to be used instead of the requested table name, only high level db operations will use it, low level utilities use the real table names" },
        ...
        { name: "cache-keys-([a-z0-9_]+)-(.+)", obj: "cache.keys.$1", make: "$2", nocamel: 1, type: "list", descr: "List of columns to be used for the table cache, all update operations will flush the cache if the cache key can be created from the record columns. This is for ad-hoc and caches to be used for custom selects which specified the cache key." },
        { name: "cache-ttl-(.+)", type: "int", obj: "cache.ttl", nocamel: 1, descr: "TTL in milliseconds for each individual table being cached, use * as default for all tables", },
        { name: "cache-name-(.+)", obj: "cache.name", nocamel: 1, make: "$1", descr: "Cache client name to use for cache reading and writing for each table instead of the default in order to split cache usage for different tables, it can be just a table name or `pool.table`, use `*` to set default cache for all tables", },
        { name: "cache-update-(.+)", obj: "cache.update", nocamel: 1, make: "$1", descr: "Cache client name to use for updating only for each table instead of the default in order to split cache usage for different tables, it can be just a table name or `pool.table` or `*`. This cache takes precedence for updating cache over `cache-name` parameter", },
        ...
        { name: "cleanup-(.+)", obj: "cleanup.$1", type: "map", maptype: "bool", merge: 1, nocamel: 1, descr: "Rules for the db.cleanupResult per table, only allows on/off condition for columns", example: "db-cleanup-bk_user = email:0,phone:1" },
        ...
        { name: "([a-z0-9]+)-pool-options-([a-zA-Z0-9_.-]+)$", obj: '_config.$1.config', camel: '-', autotype: 1, make: "$2", onupdate: "applyPoolOptions", descr: "General options for a DB pool by name with specific type" },
