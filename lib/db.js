 ...
 * @property {string} logger_db - log results at the end with this level or `debug` by default
 * @property {string} logger_error - log errors with this level instead  of `error`
 ...
 * @property {boolean} total - if true then it is supposed to return only one record with property `count`, skip all post processing and convertion
 * @property {boolean} info_query - to return the record just processed in the info object as `query` property, it will include all generated and updated columns
 * @property {boolean} result_query - to return the query record as result including all post processing and new generated columns, this is not what `returning` property does, it only
 ...
 * @property {object} ops - operators to use for comparison for properties, an object with column name and operator.
 ...
 * @property {string|object} start - start records with this primary key, this is the `next_token` passed by the previous query from {@link module.db.select}
 * @property {int} count - how many records to retrieve in a single batch
 * @property {boolean} first - a convenient option to return the first record from the result or null (similar to `db.get` method)
 ...
 * @property {object} joinOps - operators to use to combine several expressions in case when an array of values is given, supports `and|or|AND|OR``
 ...
 *  - Elasticsearch: by default page based pagination is used if no `fullscan` is provided with or without the `sort` option
 ...
 * @property {string} cacheKey - exlicit key for caching, return from the cache or from the DB and then cache it with this key, works the same as `get`
 * @property {string} cacheKeyName - a name of one of the cache keys to use, it must be defined by a `db-cache-keys-table-name` parameter
 ...
 *    it is used instead in addition to the primary keys in the `query`,
 ...
 * @property {object} updateOps - an object with column names and operations to be performed on the named column, for update/incr ops to
 *  customize how a column should be updated, the following comon actionbs are supported:
 *  - add - add to list
 ...
 * @property {object} typesOps - an object that defines updateOps operation by column type, for example `typesOps: { list: "add" }` will
 *   make sure all lists will have updateOps set to `add` if not specified explicitly
 * @property {string} useCapacity - triggers to use specific capacity, default is `read`
 * @property {number} factorCapacity - a factor to apply for the read or write capacity limit and trigger the capacity check, default is `0.9`
 * @property {string} tableCapacity - use a different table for capacity throttling instead of the `table`, useful for cases when the row callback performs
 ...
 *   not every row individually, batch size is defined by the `count` property, used in {@link module:db.scan}
 * @property {boolean} sync - as batch mode but the rowCallback is called synchronously as `rowCallback(row, info)`, used in {@link module:db.scan}
 ...
 *  - `text, string, keyword` -  text types
 ...
 *  - `random` -  generates a random number in {@link module:db.add}/{@link module:db.put} methods, uses optional .max/.min properties
 ...
 * @property {any} dflt - default value to assign if not present in the record
 ...
 * @property {boolean} convert.list -  on return splits the column value into an array, uses the `separator` property
 ...
 * @property {int} check.max -  ignore the column if a `text`, `json` or `obj` value is greater than specified limit, unless `trunc` is provided
 * @property {boolean} check.trunc -  truncate the column value, the value will be truncated before saving into the DB, uses the `max` as the limit
 * @property {int} check.maxlist -  max number of items in the `list`, `set` or `array` column types
 ...
 * @property {boolean} api.admin -  a generic read permission requires `req.options.isAdmin` when used with {@link module:api.cleanupResult}
 * @property {boolean} api.staff -  a generic read permission requires `req.options.isStaff` when used with {@link module:api.cleanupResult}
 * @property {boolean} api.internal -  if set then this property can only be updated by admin/root or with `req.options.isInternal`` property
...
const db = {
    ...
    args: [
        ...
        { name: "describe-column-([a-z0-9_]+)-([a-zA-Z0-9_]+)", obj: "columns.$1", make: "$2", type: "map", nocamel: 1, descr: "Describe a table column properties, can be a new or existing column, overrides existing property", example: "-db-describe-column-bk_user-name max:255", },
 ...
 * if it does not exist yet.* To disable upsert pass `noupsert` in the options.
 *
 * @param {string} table - table to use
 * @param {object} query - an actual record to be updated, primary key properties must be specified
 * @param {DBRequestOptions} [options]
 ...
 * @param {DBResultCallback} [callback]
 * @example
 * db.delAll("bk_user", { type: "tester" }, lib.log)
 * @memberOf module:db
 * @method delAll
 */
 ...
 * db.transaction([
 *   { op: "del", table: "bk_user", query: { login: "test1" } },
 *   { op: "add", table: "bk_user", query: { login: "test2", name: "tester 2" } },
 * ], lib.log);
 * @memberOf module:db
 * @method transaction
 */
 ...
 * db.sql("SELECT * FROM bk_user", lib.log);
 * @memberOf module:db
 * @method sql
 */
