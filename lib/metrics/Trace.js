...
'use strict';
    ...
    constructor(options, parent)
    {
        if (parent instanceof Trace) {
            ...
            this.trace_id = `1-${Math.round(Date.now() / 1000).toString(16)}-${lib.randomBytes(12)}`;
        }
    ...
    stop(options)
    {
        ...
        if (options?.statusCode && options.url) {
            this.http = {
                request: {
    ...
    {
        if (dgram === undefined) {
            dgram = lib.tryRequire("node:dgram") || null;
        }
        if (!dgram) return;

        if (!Trace.#sock) {
            Trace.#sock = dgram.createSocket('udp4').unref();
        }
