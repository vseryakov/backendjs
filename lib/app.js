...
const app = {
    ...
    args: [
        ...
        { name: "(import|preload)-packages", type: "list", array: 1, push: 1, descr: "NPM packages to load on startup, the modules, viewes, web subfolders from the package will be added automatically to the system paths, modules will be loaded if present, the config file in etc subfolder will be parsed if present", pass: 1 },
 ...
 * @params {function} [callback] - called at the end with possible err
 *
 */
app.init = function(options, callback)
{
    ...
    lib.series([
        ...
        logger.debug("init:", app.role, app.runMode, app.instance, options, err || "");
        if (!err) app._initialized = true;
