...
module.exports = {
    ...
    args: [
        ...
        { name: "log", type: "callback", callback(v) { logger.setLevel(v) }, descr: "Set debugging level to any of " + Object.keys(logger.levels), pass: 2 },
        { name: "log-options", type: "map", empty: 1, ephemeral: 1, onupdate(v) { logger.setOptions(v) }, descr: "Update logger options, the format is a map: name:val,...", pass: 1, env: "BKJS_LOG_OPTIONS" },
        { name: "log-file", type: "callback", callback(v) { if (v) this.logFile=v;logger.setFile(this.logFile, this) }, descr: "Log to a file, if not specified used default logfile, disables syslog", pass: 1, env: "BKJS_LOG_FILE" },
        { name: "log-ignore", type: "regexp", obj: "logInspect", strip: /log-/, no_camel: 1, descr: "Regexp with property names which must not be exposed in the log when using custom logger inspector" },
        { name: "log-inspect", type: "callback", callback: "setLogInspect", descr: "Install custom logger inspector instead of util.inspect: 'json' - JSON output, 1|true - lib.inspect, 0 - util.inspect", pass: 1, env: "BKJS_LOG_INSPECT" },
        { name: "log-inspect-options", type: "map", obj: "log-inspect", merge: 1, vreplace: { "%20": " ", "%2c": ",", "%3a": ":", "%3b": ";" }, descr: "Options for the logger using lib.inspect" },
        { name: "log-filter", type: "callback", callback(v) { if (v) logger.setDebugFilter(v) }, descr: "Enable debug filters, format is: label,... to enable, and !label,... to disable. First 2 arguments are used by debug filter in logger.debug", pass: 1, example: "//To log all debugging api and queries: as first/second args\napp-log-filter = api,query:" },
        { name: "log-clear", type: "bool", ephemeral: 1, onupdate(v) { if (v) logger.filters={} }, descr: "Clear all log filters", pass: 1 },
        { name: "syslog", type: "callback", callback(v) { logger.setSyslog(v || 1) }, descr: "Log messages to syslog, pass 0 to disable, 1 or url (tcp|udp|unix):[//host:port][/path]?[facility=F][&tag=T][&retryCount=N][&bsd=1][&rfc5424=1][&rfc3164=1]...", pass: 1, env: "BKJS_SYSLOG" },
        { name: "console", type: "callback", callback() { logger.setFile(null) }, descr: "All logging goes to the console resetting all previous log related settings, this is used in the development mode mostly", pass: 1 },
        ...
        { name: "salt", type: "callback", callback(v) { lib.salt=v; }, descr: "Set random or specific salt value to be used for consistent suuid generation", pass: 1 },
        { name: "version", descr: "Set app name/version explicitely and skip reading it from the package.json", pass: 1 },
        { name: "roles", type: "list", obj: "env", descr: "Additional config roles, a shortcut for app-env-roles", example: "app-roles = redis, dynamodb", pass: 1 },
        { name: "env-([a-z0-9_-]+)", obj: 'env', make: "$1", camel: "-", auto_type: { roles: "list" }, descr: "Set runtime env properties explicitly: tag, region, zone, roles, ...", pass: 1, example: "app-env-roles = dev,web\napp-env-tag = api" },
        ...
        { name: "no", type: "callback", callback(v) { lib.arrayUpdate("add", this.none, lib.split(v)) }, descr: "List of subsystems to disable instead of using many inidividual -no-NNN parameters", pass: 1 },
        { name: "no-([a-z]+)", type: "callback", callback(_v,o) { lib.arrayUpdate("add", this.none, o.name) }, strip: "no-", no_camel: 1, descr: "Do not start or disable a service, server, web, jobs, ipc, db, dbconf, watch, modules, packages, configure", pass: 1 },
        { name: "ok-(.+)", type: "callback", callback(_v,o) { lib.arrayUpdate("del", this.none, o.name) }, strip: "ok-", no_camel: 1, descr: "Enable disabled service, opposite of -no", pass: 1 },
        ...
        { name: "modules-methods-(.+)", obj: "modules", make: "$1", type: "regexp", no_camel: 1, descr: "Modules that allowed to run methods by name, useful to restrict configure methods", example: "-app-modules-methods-configureMiddleware myapp", pass: 1 },
        ...
        { name: "exit-timeout", type: "int", descr: "Duration in ms to delay process exit in app.exit" },
    ...
    /**
     * @var {object} - Current ruintime env properties and instance/container attributes gathered by other modules
     * @property {string} roles - Additional roles to use for configuration purposes, set via BKJS_ROLES
     * @property {string} tag - Instance/container tag set manually, via BKJS_TAG or derived from AWS tags
     * @property {string} id - hostname or EC2 instance id
     * @property {string} type - **aws** if running in AWS
     * @property {string} worker_id - set from cluster.worker_id
     * @property {string} image - EC2 image id
     * @property {string} instance_type - EC2 instance type
     * @property {string} container - ECS container name
     * @property {string} container_id - ECS container id
     * @property {string} task_id - ECS container task id
     * @property {string} ip - EC2/ECS private IP address
     * @property {string} region - AWS region
     * @property {string} accountId - AWS account id
     * @property {string} zone - AWS availability zone
     */
     ...
     * @property {string} bind - listen address
     * @property {string} file - file with history
     * @property {string} size - how much of history to load
     * @property {string} serverPort - server listen port
     * @property {string} webPort - web worker listen port
     * @property {string} workerPort - job worker listen port
     */
    repl: {
        bind: '127.0.0.1',
        file: '.bkjs_history',
    ...
    /** @var {object} - options for inspect when used by the logger, supports options for {@link module:lib.inspect} */
    logInspect: {
        depth: 7,
        count: 200,
        keys: 100,
...
app.init = function(options, callback)
{
    ...
    if (options.roles) {
        this.env.roles = lib.split(options.roles)
    }
    ...
    http.globalAgent.maxSockets = http.Agent.defaultMaxSockets = Number.POSITIVE_INFINITY;
    ...
    lib.series([
        function(next) {
            ...
            if (app.isOk("modules", options)) {
                const local = path.resolve(__dirname, "../modules");
        ...
        logger.logger(err ? "error": "debug", "init:", app.role, app.isPrimary, app.env, options, err);
