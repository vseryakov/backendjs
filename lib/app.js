...
module.exports = {
    ...
    args: [
        ...
        { name: "instance-([a-z0-9_-]+)", obj: 'instance', make: "$1", camel: "-", autotype: 1, descr: "Set instance properties explicitly: tag, region, zone, roles", pass: 1 },
        ...
        { name: "no-([a-z]+)", type: "callback", callback: function(v,o) { lib.toFlags("add", this.none, o.name) }, strip: "no-", nocamel: 1, descr: "Do not start or disbale a service, server, web, jobs, ipc, db, dbconf, watch, modules, packages, configure", pass: 1 },
        ...
        { name: "allow-methods-(.+)", obj: "allow-methods", type: "regexp", nocamel: 1, descr: "Modules that allowed to run methods by name, useful to restrict configure methods", example: "-allow-methods-configureWeb app", pass: 1 },
    ...
    /** @var {string} - Environment mode of the process or the application */
...
app.init = function(options, callback)
{
    ...
    if (options.roles) {
        this.instance.roles = lib.strSplit(options.roles)
    }
    ...
    lib.series([
        function(next) {
            ...
            if (app.isOk("packages", options)) {
                extra = "\n" + app.loadPackages(options.importPackages || app.importPackages);
            }
