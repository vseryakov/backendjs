...
const cache = {
    ...
    args: [
        ...
        { name: "([a-z0-9]+)-options$", obj: "_config.$1", type: "map", merge: 1, onupdate: "applyOptions", descr: "Additional parameters for clients, specific to each implementation, ex: `-cache-redis-options count:10,interval:100`" },
 ...
 *
 * All public `cache.get|put|incr|del|lock|unlock|limiter` operations have corresponding promisifed methods
 * starting with `a`, like `cache.get` -> `cache.aget`.
 *
...
cache.astats = promisify(cache.stats.bind(cache));
...
cache.aclear = promisify(cache.clear.bind(cache));
...
cache.aget = promisify(cache.get.bind(cache));
...
cache.adel = promisify(cache.del.bind(cache));
...
cache.aput = promisify(cache.put.bind(cache));
...
cache.aincr = promisify(cache.incr.bind(cache));
...
cache.alimiter = promisify(cache.limiter.bind(cache));
...
cache.acheckLimiter = promisify(cache.checkLimiter.bind(cache));
...
cache.alock = promisify(cache.lock.bind(cache));
...
cache.aunlock = promisify(cache.unlock.bind(cache));
