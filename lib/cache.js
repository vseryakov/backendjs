 ...
 *
 * All methods use **options.cacheName** property for non-default cache.
...
cache.applyOptions = function(val, options)
{
    ...
    const d = lib.split(options.obj, ".");
    const client = d[0] === "_config" && this.getClient(d[1]);
 ...
 * @method shutdown
 */
cache.shutdown = function(_options, callback)
{
 ...
 * @param {string} [options.data_type] specifies that the returned value must be converted into the specified type using lib.toValue
 * @param {function} callback
 *
 * @example
...
{
    if (typeof options === "function") callback = options, options = null;
    logger.dev("get:", cache.name, key, options);
    try {
        const client = this.getClient(options);
        const _timer = client.metrics.start();
        client.get(key, options || {}, (err, val) => {
            if (!err && options?.data_type) {
                val = Array.isArray(val) ? val.map((x) => (lib.toValue(x, options.data_type))) : lib.toValue(val, options.data_type);
            }
            _timer.end();
            if (typeof callback === "function") callback(err, val);
        });
    } catch (e) {
        logger.error('get:', cache.name, key, e.stack);
        if (typeof callback === "function") callback(e);
    }
}
...
{
    if (typeof options === "function") callback = options, options = null;
    logger.dev("put:", cache.name, key, val, options);
    try {
        const client = this.getClient(options);
        const _timer = client.metrics.start();
        ...
        logger.error('put:', cache.name, key, e.stack);
        if (typeof callback === "function") callback(e);
    }
}
...
{
    if (typeof options === "function") callback = options, options = null;
    logger.dev("incr:", cache.name, key, val, options);
    try {
        const client = this.getClient(options);
        const _timer = client.metrics.start();
        ...
        logger.error('incr:', cache.name, key, e.stack);
        if (typeof callback === "function") callback(e);
    }
}
...
{
    logger.dev("limiter:", cache.name, options);
    ...
    try {
        ...
        logger.error('limiter:', cache.name, e.stack);
        callback(options.interval, {});
    }
}

/**
 * Async verson of {@link module:cache.limiter}
 ...
 * @async
 */
cache.alimiter = function(options)
{
    return new Promise((resolve, _reject) => {
...
cache.checkLimiter = function(options, callback)
{
    ...
    this.limiter(options, (delay, info) => {
        logger.debug("checkLimiter:", cache.name, delay, options, info);
...
/**
 * Async verson of {@link module:cache.checkLimiter}
 * @param {object} options - same as in {@link module:cache.limiter}
 * @returns {object} { delay, info }
 * @memberof module:cache
 * @method acheckLimiter
 * @async
 */
cache.acheckLimiter = function(options)
{
    return new Promise((resolve, _reject) => {
...
cache.localLimiter = function(msg)
{
    ...
    logger.debug("localLimiter:", cache.name, msg, token);
    return msg;
}
...
{
    if (typeof options === "function") callback = options, options = null;
    logger.dev("lock:", cache.name, name, options);
    let locked = false, delay = 0, timeout = 0;
    const started = Date.now();
    options = options || {};
    lib.doWhilst(
        (next) => {
            try {
                this.getClient(options).lock(name, options, (err, val) => {
        ...
        (err) => {
            if (err) logger.error('lock:', cache.name, err.stack);
...
/**
 * Implements lock logic in the current process
 * @param {string} name
 * @param {object} [options]
 * @param {numeric} [options.ttl=1h] - TTL for lock, default is 1h
 * @returns {boolean}
 */
cache.localLock = function(name, options)
{
    logger.debug("localLock:", cache.name, name, options, "L:", this.lock[name]);

    const now = Date.now();
    if (this.lock[name] > 1 && this.lock[name] <= now) {
        delete this.lock[name];
    }

    if (options?.set || !this.lock[name]) {
        this.lock[name] = now + lib.toNumber(options?.ttl, { dflt: 3600000 });
        return true;
    }
    return false;
}
...
/**
 * Unlocks local lock by name unconditionally
 * @param {string} name
 */
cache.localUnlock = function(name, _options)
{
    delete this.lock[name];
}
