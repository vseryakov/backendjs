...
mod.bkCollectStats = function(options)
{
    for (let q in this.clients) {
        ...
        const m = metrics.toJSON(cl.metrics, { reset: 1 });
        q = cl.queueName;
        if (m.req?.meter?.count) {
            options.stats["cache_" + q + "_req_count"] = m.req.meter.count;
            options.stats["cache_" + q + "_req_rate"] = m.req.meter.rate;
            options.stats["cache_" + q + "_res_time"] = m.req.histogram.med;
        }
        if (m.que?.meter?.count) {
            options.stats["queue_" + q + "_req_count"] = m.que.meter.count;
            options.stats["queue_" + q + "_req_rate"] = m.que.meter.rate;
            options.stats["queue_" + q + "_res_time"] = m.que.histogram.med;
        }
    }
}
