...
function IpcNatsClient(url, options)
{
    ...
    // Workaround for nats.js 2.5.0 options parser
    delete this.port;
    this._subs = {};
    this._pending = [];
...
IpcNatsClient.prototype.unsubscribe = function(channel, options, callback)
{
    ...
    if (!callback) this._unsubscribe(channel);
}
...
IpcNatsClient.prototype.subscribeQueue = function(options, callback)
{
    ...
    if (this._subs[chan]) {
        logger.debug("subscribeQueue:", mod.name, chan, "reusing:", options);
    ...
    if (options.pushSubscribe) {
        ...
        if (options.pullSubscribe) {
            ...
            opts.manualAck();
            opts.queue(options.groupName || chan);
            opts.durable(options.groupName || chan);
    ...
    p.then((sub) => {
        ...
        logger.debug("subscribeQueue:", mod.name, chan, opts);
    }).catch((err) => {
        logger.error("subscribeQueue:", mod.name, chan, opts, err);
    });
}
...
IpcNatsClient.prototype.unsubscribeQueue = function(options, callback)
{
    ...
    if (!callback) this._unsubscribe(this.channel(options));
}
...
{
    var chan = this.channel(options);
    if (err && err.code != 404 && err.code != 408) return logger.error("pollQueue:", this.name, chan, this._running.size, err, jsmsg);
    if (!jsmsg) return;

    var vtimer, done;
    var mtimer = this.metrics.Timer('timer').start(1);
    ...
    logger.debug("pollQueue:", this.name, chan, running.size, "MSG:", msg, "ITEM:", jsmsg);
    ...
    try {
        ...
        logger.error("pollQueue:", this.name, chan, running.size, e, msg);
    }
}
