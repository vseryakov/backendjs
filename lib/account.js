...
const mod = {
    ...
    args: [
        ...
        { name: "passkey-path", obj: "passkey", descr: "Endpoint path for passkey challenge cookies" },
        { name: "passkey-secret", obj: "passkey", descr: "Secret for challenges" },
        { name: "passkey-queue", obj: "passkey", descr: "Limiter queue for challenges" },
        { name: "passkey-(.+)", obj: "passkey", type: "int", descr: "Passkey numeric parameters: max, ttl, disabled" },
    ],
    table: "bk_user",
    sigversion: -1,
    hash: "bcrypt",
    ...
    errInvalidPasskeys: "Too many passkeys",
};
module.exports = mod;
...
mod.configure = function(options, callback)
{
    this.tables = {
        [this.table]: {
            login: { primary: 1, max: mod.maxLength },                   // User login/username
            id: {                                                        // Autogenerated ID
                type: "uuid",
                pub: 1,
                prefix: "u_",
    ...
    this.loadFile((err) => {
        if (err) return;
        fs.watch(this.file, () => {
            core.setTimeout(this.usersFile, () => { this.loadFile() }, lib.randomInt(1000, 5000));
        });
    });
