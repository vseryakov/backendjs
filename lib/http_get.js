...
//   - stopOnAbort - if true do not retry on network errors or connection errors
...
function httpGet(uri, params, callback)
{
    ...
    if (params.hardTimeout) {
        params._timer = setTimeout(() => { req.destroy(new Error("timeout")) }, params.hardTimeout);
    }
    if (params.postdata) {
...
HttpRequest.prototype.prepareCookies = function(callback)
{
    ...
    if (typeof this.cookies == "boolean" && this.hostname) {
        core.cookieGet(this.hostname, (err, cookies) => {
...
HttpRequest.prototype.onError = function(err, callback)
{
    ...
    if (!this.quiet) logger[this.retryCount ? "debug" : "error"]("httpError:", err, this.toJSON({ postdata: 256, data: 256 }));

    if (!this.stopOnAbort && this.retryCount-- > 0) {
        this.lastError = err;
        if (typeof this.retryPrepare == "function") this.retryPrepare.call(this);
        setTimeout(httpGet.bind(null, this._uri, lib.objClone(this, "retryTimeout", this.retryTimeout * this.retryMultiplier), callback), this.retryTimeout);
    } else {
...
HttpRequest.prototype.onEnd = function(res, callback)
{
    ...
    if (Array.isArray(this.cookies)) {
        core.cookieSave(this.cookies, headers["set-cookie"], this.hostname);
    }
    ...
    if ((this.status < 200 || this.status >= 400) &&
        ...
        if (!this.quiet) logger.debug("httpRetry:", this.toJSON({ postdata: 256, data: 256 }));
        if (typeof this.retryPrepare == "function") this.retryPrepare.call(this);
        setTimeout(httpGet.bind(null, this._uri, lib.objClone(this, "retryTimeout", this.retryTimeout * this.retryMultiplier), callback), this.retryTimeout);
        this._done = this.status || 1;
        return;
    }
    if (this.checkRedirect(callback)) {
        this._done = this.status;
        return;
    }
...
HttpRequest.prototype.checkRedirect = function(callback)
{
    ...
    case 308:
        ...
        httpGet(uri, lib.objClone(this), callback);
        return true;
    }
}
