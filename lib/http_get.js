...
HttpRequest.prototype.init = function(uri)
{
    ...
    this.retryCount = this.retryTotal = lib.toNumber(this.retryCount, { min: 0 });
...
HttpRequest.prototype.open = function(callback)
{
    ...
    logger.dev("httpOpen:", core.role, this.method, this.uri, this.postsize);
...
{
    if (!this.quiet) logger[this.retryCount ? "debug" : "error"]("httpGet:", core.role, "onerror:", err, this.toJSON(), typeof this.postdata == "string" ? this.postdata.substr(0, 512) : "");

    if (this.retryCount-- > 0) {
        setTimeout(httpGet.bind(null, this.uri, this, callback), this.retryTimeout *= 2);
    } else {
...
HttpRequest.prototype.onEnd = function(res, callback)
{
    ...
    if ((this.status < 200 || this.status >= 400) &&
        ...
        this.retryCount-- > 0) {
        logger.dev("httpEnd:", core.role, "retrying", this.toJSON());
        setTimeout(httpGet.bind(null, this.uri, this, callback), this.retryTimeout *= 2);
        return;
    }
    ...
    logger.debug("httpGet:", core.role, "done", this.toJSON());
