...
//   - httpTimeout - timeout in milliseconds afte which the request is aborted if no data received
...
function httpGet(uri, params, callback)
{
    ...
    if (params.httpTimeout) {
        req.setTimeout(params.httpTimeout, () => { req.destroy() });
    }
    if (params.postdata) {
...
HttpRequest.prototype.onData = function(res, chunk)
{
    ...
    logger.dev("httpData:", core.role, this.toJSON({ postdata: 256 }));
}
...
{
    logger.dev("httpEnd:", core.role, this.toJSON({ postdata: 256 }));
    ...
    if ((this.status < 200 || this.status >= 400) &&
        ...
        this.retryCount-- > 0) {
        if (!this.quiet) logger.debug("httpRetry:", core.role, this.toJSON({ postdata: 256, data: 256 }));
        if (typeof this.retryPrepare == "function") this.retryPrepare.call(this);
        setTimeout(httpGet.bind(null, this._uri, this, callback), lib.objMult(this, "retryTimeout", this.retryMultiplier, "old"));
        return;
    }
