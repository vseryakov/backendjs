...
function IpcSQSClient(host, options)
{
    ...
    this.options.retryCount = lib.toNumber(this.options.retryCount, { dflt: 2 });
...
IpcSQSClient.prototype.poller = function()
{
    ...
    aws.sqsReceiveMessage(this.host, this.options, function(err, rows) {
        if (err) return setTimeout(self.poller.bind(self), self.options.interval);
        ...
        lib.forEach(rows || [], function(item, next) {
            ...
            if (self.options.visibilityTimeout) {
                timer = setInterval(function() {
                    aws.querySQS("ChangeMessageVisibility", { QueueUrl: self.host, ReceiptHandle: item.ReceiptHandle, VisibilityTimeout: self.options.visibilityTimeout }, function(err) {
