...
function IpcSQSClient(host, options)
{
    ...
    this.options.retryCount = lib.toNumber(this.options.retryCount, { dflt: 4, min: 0 });
...
IpcSQSClient.prototype.poller = function()
{
    ...
    this._working++;
    aws.sqsReceiveMessage(this.host, this.options, function(err, items) {
        if (err) {
            self._working--;
        ...
        lib.forEach(items || lib.emptylist, function(item, next) {
            ...
            if (self.options.visibilityTimeout) {
                timer = setInterval(function() {
                    ...
                    var req = { QueueUrl: self.host, ReceiptHandle: item.ReceiptHandle, VisibilityTimeout: self.options.visibilityTimeout };
