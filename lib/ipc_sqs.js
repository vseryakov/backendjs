    ...
    aws.sqsReceiveMessage(this.url, this.options, (err, items) => {
        ...
        lib.forEach(items, (item, next) => {
            ...
            if (msg.endTime > 0 && msg.endTime < now) {
                ...
                aws.querySQS("DeleteMessage", { QueueUrl: self.url, ReceiptHandle: item.ReceiptHandle }, self.options, () => { next() });
                return;
            }
            ...
            if (msg.visibilityTimeout > 0 || self.options.visibilityTimeout) {
                timer = setInterval(() => {
                    ...
                    var timeout = Math.round((msg.visibilityTimeout || self.options.visibilityTimeout)*1.1/1000);
                    var req = { QueueUrl: self.url, ReceiptHandle: item.ReceiptHandle, VisibilityTimeout: timeout };
                    aws.querySQS("ChangeMessageVisibility", req, opts, (err) => {
                        logger.info("sqs.keepAlive:", name, msg.channel, msg.data, err);
            ...
            if (!self.emit("message", msg, (err) => {
                logger.debug("sqs.finished:", name, err, msg);
                clearInterval(timer);
                msg.done = 1;
