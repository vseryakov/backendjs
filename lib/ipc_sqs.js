...
IpcSQSClient.prototype.processQueue = function(callback)
{
    ...
    aws.sqsReceiveMessage(this.host, this.options, function(err, rows) {
        ...
        (rows || []).forEach(function(item) {
            ...
            var cb = self.callbacks[msg.key || self.options.key || ""];
            // Keep the message in the queue if there is no callback for the given key, this is persistent queue by explicit keys
            if (!cb) return;
...
IpcSQSClient.prototype.publish = function(key, data, options, callback)
{
    aws.sqsSendMessage(this.host, JSON.stringify({ key: key, data: data }), options, callback);
}
