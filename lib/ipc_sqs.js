...
function IpcSQSClient(host, options)
{
    ...
    if (this.host.match(/^sqs:/)) {
        this.host = "";
        aws.querySQS("GetQueueUrl", this.hostname, function(err, data) {
            if (err) return;
        ...
        lib.forEach(rows || [], function(item, next) {
            ...
            if (self.options.visibilityTimeout) {
                timer = setInterval(function() {
                    aws.querySQS("ChangeMessageVisibility", self.host, { ReceiptHandle: item.ReceiptHandle, VisibilityTimeout: self.options.visibilityTimeout }, function(err) {
