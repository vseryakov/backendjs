        ...
        lib.forEach(items || lib.emptylist, function(item, next) {
            ...
            if (self.options.visibilityTimeout) {
                timer = setInterval(function() {
                    ...
                    var req = { QueueUrl: self.url, ReceiptHandle: item.ReceiptHandle, VisibilityTimeout: Math.round(self.options.visibilityTimeout/1000) };
...
IpcSQSClient.prototype.publish = function(channel, msg, options, callback)
{
    ...
    logger.debug("sqs.publish:", channel, options, lib.objDescr(obj));
    aws.sqsSendMessage(this.url, JSON.stringify(obj), this.options, callback);
}
