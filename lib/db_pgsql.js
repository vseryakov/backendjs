...
db.pgsqlInitPool = function(options)
{
    ...
    pool.open = function(opts, callback) {
        ...
        if (opts.url == "default") opts.url = "postgresql://postgres@127.0.0.1/" + self.dbName;
        if (!client) client = new utils.PgSQLDatabase(opts.url);
        client.connect(function(err) {
            if (err) {
                logger.error('pgsql: connect:', err);
                callback(err);
            } else {
                if (opts.connect.blocking) client.setNonblocking(0);
...
db.pgsqlBindValue = function(val, info)
{
    ...
    switch ((info && info.data_type) || info.type || "") {
    case "json":
