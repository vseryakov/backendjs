...
//
// The `direct` argument controls how the final callback is called, if true it is called directly otherwisde via setImmediate
//
...
lib.forEach = function(list, iterator, callback, direct)
{
    ...
    for (var i = 0; i < list.length; i++) {
        iterator(list[i], function itForEach(err) {
            if (err) {
                _call(direct, callback, Array.prototype.slice.apply(arguments))
                callback = lib.noop;
                i = list.length + 1;
            } else
            if (--count == 0) {
...
lib.forEvery = function(list, iterator, callback, direct)
{
    ...
    var count = list.length;
    for (var i = 0; i < list.length; i++) {
        iterator(list[i], function itForEvery(err) {
            if (--count == 0) {
                _call(direct, callback, Array.prototype.slice.apply(arguments));
                callback = lib.noop;
            }
...
lib.forEachSeries = function(list, iterator, callback, direct)
{
    ...
    function iterate(i) {
        if (i >= list.length) {
            return _call(direct, callback, [null, ...Array.prototype.slice.apply(arguments).slice(1)]);
        }
        iterator.apply(null, [list[i], function itForEachSeries(err) {
            if (err) {
                _call(direct, callback, Array.prototype.slice.apply(arguments));
                callback = lib.noop;
            } else {
                iterate.apply(null, [++i, ...Array.prototype.slice.apply(arguments).slice(1)]);
            }
...
lib.forEverySeries = function(list, iterator, callback, direct)
{
    ...
    function iterate(i) {
        if (i >= list.length) {
            return _call(direct, callback, Array.prototype.slice.apply(arguments).slice(1));
        }
        iterator.apply(null, [list[i], function itForEverySeries() {
            iterate.apply(null, [++i, ...Array.prototype.slice.apply(arguments)]);
        }, ...Array.prototype.slice.apply(arguments).slice(1)]);
    }
    iterate(0);
}
...
lib.parallel = function(tasks, callback, direct)
{
    this.forEach(tasks, function itEach(task, next) {
        task(function itTask() {
            _call(direct, next, Array.prototype.slice.apply(arguments));
        });
    }, callback, direct);
}
...
lib.everyParallel = function(tasks, callback, direct)
{
    this.forEvery(tasks, function itEach(task, next) {
        task(function itTask() {
            _call(direct, next, [null, ...Array.prototype.slice.apply(arguments).slice(1)]);
        });
    }, callback, direct);
}
...
{
    this.forEachSeries(tasks, function itEach(task, next) {
        task.apply(null, [function itTask() {
            _call(direct, next, Array.prototype.slice.apply(arguments));
...
{
    this.forEverySeries(tasks, function itEach(task, next) {
        task.apply(null, [function itTask() {
            _call(direct, next, Array.prototype.slice.apply(arguments));
        }, null, ...Array.prototype.slice.apply(arguments).slice(2)]);
    }, callback, direct);
}
