 ...
 * @property {string} id - sorting number, auto created or explicit
 * @property {string} method - HTTP method, empty for all methods
 * @property {string} path - configured path to match
 * @property {string[]} paths - parts of the path
 * @property {string[]} params - list of params to extract from path
 * @property {any|function} handler - middleware function(context, next)
 * @property {object} [handler.boundThis] - context the handler is bound to
 */
 ...
 * `next` function.
 *
 * `context.route` is set to the currerntly processed route.
 *
 ...
 *
 * Method parameter may contain explicit priorty number in the form `#NUM`, it will be used instead of auto generated sequence number.
 *
 * All matched middleware handlers are executed in the order of this priority number.
 *
 * **Examples**
 *
...
class Router {
     ...
     * @param {string} [method=''] - empty for all, or a valid HTTP method. Optional sorting ID can be appended as `#ID`, it will be used
     * as is instead of default sequential id. This is to allow manual sorting of routes and not depend on order of definition.
     * @param {string} [path=*]
    ...
    use(method, path, ...handlers) {
        ...
        for (let handler of handlers) {
            if (lib.isFunc(handler.handle)) {
                ...
                Object.defineProperty(handler, "boundThis", { value: bound, writable: true });
            }
     ...
     * @param {string} method to handle, optional sorting ID can be set as `#ID` where ID is a number,
     * # without a number is treated as empty, i.e. 0 must be explicit
     * @param {string} path to handle
     * @param {any} handler associated data
     * @example
     * router.add("GET", "/api/info", middleware1)
     * router.add("GET#2", "/api/info", middleware2)
            ...
            if (!child) {
                if (!node.children) {
                    node.children = Object.create(null, {
                        toJSON: { value() { return Object.keys(this).join(", ") } }
                    });
                }
    ...
    del(method, path, handler) {
        return this.#find(method, path, (result, method, _paths, node) => {
            const rc = [];
            for (const route of node.handlers) {
                if ((!handler || route.handler === handler) &&
                    ...
                    result.push(route);
                    if (route.handler.boundThis) {
                        route.handler.boundThis = undefined;
                    }
                } else {
                    rc.push(route);
                }
    ...
    dump(_node) {
        this.walk(node => {
            const text = util.inspect(node, { showHidden: true, depth: null, compact: 3, breakLength: Number.POSITIVE_INFINITY }).split("\n");
            console.log(text.map(x => (" ".repeat(node.index || 0) + x)).join("\n"));
        });
    }
