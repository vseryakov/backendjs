 ...
 * ```js
 *
 *  api.app.get("/challenge", (context) => { context.json(api.passkey.createChallenge(context)) }).
 *          post("/login", api.passkey.login).
 *          post("/register", api.passkey.register));
 *
 * ```
 */
...
{
    const { err, query } = api.validate(context, {
        username: { required: 1 },
    ...
    lib.series([
        ...
        function(next, challenge) {
            mod.verifyRegistration(context, { query, challenge }, next);
        },
    ...
    lib.series([
        ...
        function(next, challenge) {
            mod.read(query.userHandle, query.credentialId, (err, user, passkey) => {
                ...
                mod.verifyAuthentication(context, { query, challenge, passkey }, (err) => {
...
mod.getChallenge = function(context, callback)
{
...
mod.verifyChallenge = function(context, callback)
{
    ...
    cache.incr("PSK:" + challenge, 1, { ttl: mod.ttl, cacheName: mod.cache }, (err, rc) => {
