...
mod.setup = function(req, callback)
{
    ...
    options.session = req.user?.login && req.user?.secret && req.headers ? true : false;
    var hooks = api.findHook('sig', req.method, req.path);
    logger.debug("setup:", mod.name, hooks.length, "hooks", options);

    if (!hooks.length) {
        if (options.session) this.create(req, options);
