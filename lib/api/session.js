 ...
 * @param {RequestContext} context
 * @return {Object|null} session object if valid and not expired, { id, exp, sig }
 * @memberof module:api/session
 * @method parse
 */
mod.parse = function(context)
{
    if (!context.session) {
        ...
        context.session = Object.create(null, {
            id: { value: session[0] },
            exp: { value: exp },
            sig: { value: session[2], enumerable: false },
        });
 ...
 * if `api.session.secret` is defined the signed token is also encrypted and stored and base64
 * @param {RequestContext} context
...
/**
 * Expire and clear session cookie for the context
 * @param {RequestContext} context
 * @memberof module:api/session
 * @method clear
 */
