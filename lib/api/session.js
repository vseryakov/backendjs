...
api.handleSessionSignature = function(req, callback)
{
    ...
    options.session = options.session && req.user?.login && req.user?.secret && req.headers ? true : false;
...
{
    var sig = this.createSignature(req.user.login, req.user.secret, "", req.headers.host, "", { version: 2, expires: options?.sessionAge || this.sessionAge });
...
api.checkSessionSignature = function(sig, callback)
{
    ...
    cache.get(`SIG:${sig.login}:${sig.signature}`, { cacheName: this.sessionCache }, (err, val) => {
...
api.saveSessionSignature = function(sig, val, callback)
{
    ...
    cache.put(`SIG:${sig.login}:${sig.signature}`, val || Date.now(), { cacheName: this.sessionCache, ttl: this.sessionAge }, callback);
}
