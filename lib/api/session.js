...
api.checkSessionSignature = function(sig, callback)
{
    ...
    cache.get(`SIG:${sig.login}:${sig.signature}`, { queueName: this.sessionCache }, (err, val) => {
...
api.saveSessionSignature = function(sig, val, callback)
{
    ...
    cache.put(`SIG:${sig.login}:${sig.signature}`, val || Date.now(), { queueName: this.sessionCache, ttl: this.sessionAge }, callback);
}
