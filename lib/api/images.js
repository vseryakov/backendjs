...
mod.path = function(id, options)
{
    ...
    return api.normalize(core.path.images, options?.prefix || "user", num.substr(-2), num.substr(-4, 2), name);
}
...
{
    var icon = this.path(id, options);
    logger.debug('sendImage:', icon, id, options);

    if (options?.imagesS3 || this.s3) {
        var opts = {};
        var params = url.parse(core.modules.aws.signS3("GET", options?.imagesS3 || this.s3, icon, "", opts));
...
mod.save = function(file, id, options, callback)
{
    if (typeof options == "function") callback = options, options = null;
    if (!options) options = {};
    var outfile = this.path(id, options);
...
api.del = function(id, options, callback)
{
    ...
    if (!options.localIcon && (this.s3 || options.imagesS3)) {
        core.modules.aws.queryS3(options.imagesS3 || this.s3, icon, { method: "DELETE" }, (err, params) => {
            lib.tryCall(callback, err);
        });
    } else {
        fs.unlink(icon, (err) => {
