 ...
 * @method toPadding
 */
mod.toPadding = function(options)
{
    return {
    ...
    if (border) {
        const borderImage = await mod.sharp({
            create: {
                ...
                height: ch,
                channels: 4,
                background: options.color || { r: 255, g: 255, b: 255, alpha: 0.5 }
            }
        }).
        composite([{
 ...
 * @param {int} options.color] - text color
...
mod.createText = async function(options)
{
    ...
    var text = lib.isString(options.text).replaceAll("\\n", "\n");

    var attrs = [];
    if (options.color) attrs.push(`color="${options.color}"`);
    if (options.bold) attrs.push(`weight="bold"`);
    if (options.italic) attrs.push(`style="italic"`);
    ...
    return mod.sharp({
        create: {
            width: meta.width + padding.left + padding.right,
            height: meta.height + padding.top + padding.bottom,
            channels: 4,
            background: { r: 0, g: 0, b: 0, alpha: 0 }
        }
    }).
