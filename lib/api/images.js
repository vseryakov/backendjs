 ...
 * @param {string} [options.bcolor=FFFFFFF0] - border color
 * @param {number} [options.bradius] - make border round, this is divider of the size
 ...
 * @param {string} [options.stretch] - One of ‘ultracondensed’, ‘extracondensed’, ‘condensed’, ‘semicondensed’, ‘normal’,
...
{
    logger.debug("createImage:", mod.namer, options);

    const border = lib.toNumber(options.border);
    const radius = lib.toNumber(options.radius);
    let width = lib.toNumber(options.width);
    let height = lib.toNumber(options.height);
    ...
    if (options.file) {
        innerImage = await mod.sharp(options.file).
            resize(width, height, {
    ...
    if (options.text) {
        ...
        const attrs = [];
        for (const p of ["color", "size", "weight", "style", "stretch", "strikethrough", "underline",
                         "line_height", "segment", "letter_spacing", "allow_breaks",
        ...
        logger.debug("createImage:", mod.name, attrs);
    ...
    if (radius) {
        innerImage.
            composite([{
                ...
                blend: options.rblend || options.blend || 'dest-in'
            }]);
    }
    ...
    if (border) {
        const borderImage = await mod.sharp({
            create: {
                ...
                height: ch,
                channels: 4,
                background: options.bcolor || { r: 255, g: 255, b: 255, alpha: 0.5 }
            }
        });

        const bradius = options.bradius || radius;
        if (bradius) {
            borderImage.composite([{
                input: Buffer.from(`<svg><rect x="0" y="0" width="${cw}" height="${ch}" rx="${cw/bradius}" ry="${ch/bradius}"/></svg>`),
                blend: options.bblend || options.blend || 'dest-in'
            }]);
        }
