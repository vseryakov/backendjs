...
/**
 * Create an avatar image from a photo
 * @param {object} options
 * @param {string} options.file - input file
 * @param {int} [options.size=300] - avatar size
 * @param {int} [options.border=10] - border size in px
 * @param {int} [options.padding] - transparent padding around avatar
 * @param {int} [options.radius=2] - how circle the avatar is, divider of the size
 * @returns {Promise} - a sharp object containing an image
 * @memberof module:api/images
 * @method createAvatar
 * @async
 */
mod.createAvatar = async function(options)
{
    if (!mod.sharp) mod.sharp = lib.tryRequire('sharp');

    const width = lib.toNumber(options.width) || 300;
    const height = lib.toNumber(options.height) || width;
    const border = lib.toNumber(options.border) || 10;
    const padding = lib.toNumber(options.padding);
    const radius = lib.toNumber(options.radius) || 2;
    const cw = width + border * 2
    const ch = height + border * 2
    const images = [];

    const innerImage = await mod.sharp(options.file).
        resize(width, height, { fit: options.fit || 'cover' }).
        composite([{
            input: Buffer.from(`<svg><rect x="0" y="0" width="${width}" height="${height}" rx="${width/radius}" ry="${height/radius}"/></svg>`),
            blend: options.blend || 'dest-in'
        }]).
        png().
        toBuffer();
    ...
    if (border) {
        const borderImage = await mod.sharp({
            create: {
                ...
                channels: 4,
                background: options.color || { r: 255, g: 255, b: 255, alpha: 0.6 }
            }
        }).
        composite([{
            input: Buffer.from(`<svg><rect x="0" y="0" width="${cw}" height="${ch}" rx="${cw/radius}" ry="${ch/radius}"/></svg>`),
            blend: options.blend || 'dest-in'
        }]).
        png().
        toBuffer();

        images.push({ input: borderImage })
    }
 ...
 * @param {int} options.color=#FFF] - text color
 * @param {int} options.font] - font family
 * @param {int} options.fontfile] - font file
 * @param {int} [options.dpi] - text size in DPI
 * @param {int} [options.padding] - transparent padding around text
  * @returns {Promise} - a sharp object containing an image
 * @memberof module:api/images
 * @method createText
 * @async
 */
mod.createText = async function(options)
{
    if (!mod.sharp) mod.sharp = lib.tryRequire('sharp');

    const text = lib.isString(options.text).replaceAll("\\n", "\n");

    const image = await mod.sharp({
        text: {
            rgba: true,
            text: `<span foreground="${options.color || "#FFF"}">${text}</span>`,
            font: options.font || undefined,
            fontfile: options.fontfile || undefined,
            dpi: lib.toNumber(options.dpi) || undefined,
            height: lib.toNumber(options.height) || undefined,
            width: lib.toNumber(options.width) || undefined,
    ...
    const xpad = lib.toNumber(options.xpadding || options.padding);
    const ypad = lib.toNumber(options.ypadding || options.padding);

    return mod.sharp({
        create: {
            width: meta.width + xpad * 2,
            height: meta.height + ypad * 2,
            channels: 4,
            background: { r: 0, g: 0, b: 0, alpha: 0 }
        }
    }).
