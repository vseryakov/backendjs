...
/**
 * Return the dominant color for the given region
 ...
 */
mod.getDominantColor = async function(image, gravity)
{
    if (!mod.sharp) mod.sharp = lib.tryRequire('sharp');
    const buffer = Buffer.isBuffer(image) ? image : await image.toBuffer();
    const meta = await mod.sharp(buffer).metadata();
    const region = mod.getGravityRegion(gravity, meta.width, meta.height);
    const stats = await mod.sharp(buffer).extract(region).stats();
    return `#${hex(255 - stats.dominant.r)}${hex(255 - stats.dominant.g)}${hex(255 - stats.dominant.b)}`;
}
...
mod.createImage = async function(options)
{
    ...
    if (options.file) {
        ...
        if (width && height) {
            innerImage.resize(width, height, {
    ...
    if (border) {
        ...
        const bradius = options.border_radius || radius;
        if (bradius) {
