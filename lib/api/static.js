 ...
 *   - View templating engine (e.g., EJS, Pug)
...
const mod = {
    ...
    args: [
        ...
        { name: "views-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: 'views', descr: "Locations to be rendered as views, use ! in front of regexp to remove particular redirect from the list", example: "-api-static-views-^/user/get show-user" },
...
mod.checkViews = function(req, res)
{
    ...
    for (const p in mod.views) {
        if (lib.testRegexpObj(req.options.path, this.views[p]) || lib.testRegexpObj(location, mod.views[p])) {
            ...
            res.locals.options = req.options;
