...
mod.checkViews = function(req, res)
{
    ...
    for (const p in mod.views) {
        if (lib.testRegexpObj(req.options.path, mod.views[p]) || lib.testRegexpObj(req.options.location, this.views[p])) {
            logger.debug("checkViews:", mod.name, req.options.location, "render by:", p);
...
mod.checkRouting = function(req)
{
    ...
    for (const p in mod.compressed) {
        if (lib.testRegexp(req.options.path, mod.compressed[p])) {
            ...
            req.res.setHeader("Content-Type", app.getMimeType(req.options.opath));
