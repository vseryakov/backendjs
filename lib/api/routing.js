...
mod.check = function(req, name)
{
    ...
    for (const p in rules) {
        if (lib.testRegexpObj(req.options.path, rules[p]) || lib.testRegexpObj(req.options.location, rules[p])) {
            req.signatureUrl = req.url;
            api.replacePath(req, api.checkRequestPlaceholders(req, p));
            logger.debug("check:", this.name, name, req.options.location, "switch to:", p, req.url);
            return true;
        }
    }
}
