 ...
 *
 * All secured requests to the API server must be signed with user login/secret pair.
...
/**
 * Parse text return all pieces wrapped in an object, this object will be used by `verifySignature` function.
 *
 * @param {object} req - HTTP incoming request
 * @param {string} signature - encoded signature text
 * @returns {object|undefined} - parsed signature object
 * @memberOf module:api/signature
 * @method parse
 */
mod.parse = function(req, signature)
{
    ...
    return this.fromRequest(req, {
        version: lib.toNumber(d[1]),
        tag: d[2]?.trim().substr(0, mod.maxLength * 2) || "",
        login: d[3]?.trim().substr(0, mod.maxLength) || "",
        signature: d[4] || "",
        expires: lib.toNumber(d[5]),
        checksum: d[6] || "",
    });
}
...
mod.verify = function(req, sig, user, callback)
{
    ...
    var query = (sig.query || "").split("&").sort().join("&");
