...
api.getSignature = function(req)
{
    ...
    if (signature) {
        ...
        if (d) {
            ...
            if (d[2]) sig.tag = d[2].trim().substr(0, accounts.maxLength * 2);
            if (d[3]) sig.login = d[3].trim().substr(0, accounts.maxLength);
...
api.verifySignature = function(req, sig, account, callback)
{
    ...
    if (sig.version < 0) {
        accounts.checkSecret(account, sig.secret, (err) => {
    ...
    default:
        ...
        if (hooks.length) {
            lib.forEachSeries(hooks, (hook, next) => {
                hook.callback(req, account, sig, next);
            }, (rc) => {
