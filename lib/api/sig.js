...
api.getSignature = function(req)
{
    ...
    if (signature) {
        ...
        if (d) {
            ...
            if (d[2]) sig.tag = d[2].trim().substr(0, users.maxLength * 2);
            if (d[3]) sig.login = d[3].trim().substr(0, users.maxLength);
...
api.verifySignature = function(req, sig, user, callback)
{
    ...
    if (sig.version < 0) {
        users.checkSecret(user, sig.secret, (err) => {
            if (err) logger.debug("verifySignature:", 'failed', err, sig, user);
    ...
    default:
        ...
        if (hooks.length) {
            lib.forEachSeries(hooks, (hook, next) => {
                ...
                if (!rc) logger.debug("verifySignature:", 'failed', sig, user);
                callback(rc);
            }, true);
            return;
        }
    ...
    if (!lib.timingSafeEqual(sig.signature, sig.hash)) {
        logger.debug('verifySignature:', 'failed', sig, user);
        sig = null;
    }
    callback(sig);
}
