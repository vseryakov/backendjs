...
api.checkRequestPlaceholders = function(req, pathname)
{
    return pathname.replace(/@(HOST|IP|DOMAIN|PATH([1-9])?|URL|BASE|FILE|DIR|SUBDIR|EXT|QUERY)@/g, function(_, m) {
        ...
        case "DO": return lib.domainName(req.options.hostname);
        case "PA":
            if (m[4] > 0) {
                req.options.apath ??= req.options.path.split("/");
                return req.options.apath.slice(m[4]).join("/");
            }
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    const mapping = this.rlimitsMap;
    if (lib.isEmpty(mapping)) return callback();
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        switch (type) {
        case "ip":
            name = options.ip || req.options?.ip;
            break;
        ...
        case "path":
            ...
            name = key + "/" + (options.ip || req.options?.ip);
            break;
        }
        ...
        var cacheName = options.cache || map?.cache || this.rlimits.cache;
...
api.sendJSON = function(req, err, data)
{
    ...
    lib.forEachSeries(hooks, (hook, next) => {
        try {
            ...
            logger.error('sendJSON:', hook.path, req.options, e.stack);
        }
        logger.debug('sendJSON:', hook.path, req.options, 'sent:', sent || req.res.headersSent);
...
api.sendStatus = function(res, options)
{
    ...
    var delay = req.options?.sendDelay ||
                (options.code && api.delays?.[`${status}:${options.code}`]) ||
                api.delays?.[status];
    ...
    try {
        ...
        default:
            ...
            lib.forEachSeries(hooks, (hook, next) => {
                try {
                    ...
                    logger.error('sendStatus:', hook.path, options, "OPTS:", req.options, e.stack);
                }
                logger.debug('sendStatus:', hook.path, req.options, 'sent:', sent || res.headersSent, delay);
        ...
        logger.error('sendStatus:', options, "OPTS:", req.options, e.stack);
        if (!res.headersSent) {
            res.status(500).send(res.__(this.errInternalError));
        }
    }
}
...
api.sendReply = function(res, status, text)
{
    if (util.types.isNativeError(status)) {
        ...
        if (status.message && !this.errlog.ignore?.rx.test(status.message)) {
            if (!this.errlog.token || this.errlog.token.consume(1)) {
                logger.error("sendReply:", res.req.options, status.message, lib.traceError(status), res.req.body);
            }
        }
        status = status.status >= 400 ? status.status : 500;
        text = lib.testRegexpObj(err.code || status, this.errlog.codes) ? res.__(status.message) :  res.__(this.errInternalError);
        return this.sendStatus(res, { status, message: lib.isString(text) });
    }
    ...
    if (status >= 400) logger.debug("sendReply:", res.req.options, status, text);
    this.sendStatus(res, { status: status || 200, message: lib.isString(text) });
}

/**
 * Send file back to the client or return 404 status
 * @param {object} req - Express Request
 * @param {string} file - file path, if no options.root is provided the file will be normalized with {@link module:lib/normalize}
 * which strips all non valid characters, i.e. no relative paths
 * @param {object} [options] - properties for Express `res.sendFile`
 * @param {string} [options.root] - root for relative filenames
 * @param {boolean} [options.redirect] - redirect url in case of error instead of returning 404
 * @memberof module:api
 * @method sendFile
 */
api.sendFile = function(req, file, options)
{
    if (!options?.root) {
        file = lib.sanitizePath(file);
    }

    if (!options?.redirect && req.method != "HEAD") {
        return req.res.sendFile(file, options);
    }

    fs.stat(file, (err, st) => {
        logger.debug("sendFile:", req.options, file, st);
        if (req.method == 'HEAD') {
            return req.res.set("Content-Length", err ? 0 : st.size).
                           set("Content-Type", app.getMimeType(file)).
                           status(!err ? 200 : 404).send();
        }
        if (!err) {
            return req.res.sendFile(file, options);
        }
        if (options?.redirect) {
            return req.res.redirect(options.redirect);
        }
...
/**
 * Parse body/query parameters according to the `schema` by using `lib.validate`,
 * uses the req.body if present or req.query.
 * @param {object} req - Express request
 * @param {module:lib.ParamsOptions} schema - schema object
 * @param {object} [options]
 * @param {object} [options.defaults] - merged with global `body.defaults`
 ...
 * @method validate
 */
api.validate = function(req, schema, options)
{
    var opts = lib.extend({}, options, {
        dprefix: req.options?.path + "-",
        defaults: lib.extend({}, options?.defaults, this.body.defaults)
    });
    logger.debug("validate:", "api", req.options, "S:", schema, "O:", opts);
 ...
 * @method getCookie
 */
api.getCookie = function(req, name)
{
 ...
 * new columns or columns created on the fly are returned to the client. `api.cleanup.strict=1` can be configured globally.
 *
...
api.cleanupResult = function(req, table, data)
{
    ...
    const strict = options.cleanup_strict || this.cleanup.strict;
    ...
    logger.debug("cleanupResult:", "api", req.options, table, rows.length, all, pos, cols, options, _rules);
    return data;
}
...
api.replacePath = function(req, path)
{
    ...
    req.url = req.options.url = req.options.path + req.url.substr(req.options.opath.length);
}
...
/**
 * Set response headers by location from the config
 * empty values in the config
 * @param {object} req
 * @param {object} res
 * @memberof module:api
 * @method setResponseHeaders
 */
api.setResponseHeaders = function(req, res)
{
    for (const p in this.headers) {
        const rule = this.headers[p];
        if (rule.value && lib.testRegexpObj(req.options.path, rule) || lib.testRegexpObj(req.options.location, rule)) {
            for (const h in rule.value) {
                if (rule.value[h]) {
                    res.setHeader(h, rule.value[h]);
                } else {
                    res.removeHeader(h);
                }
                logger.debug('setResponseHeaders:', "api", req.options, "HDR:", h, rule.value[h]);
            }
        }
    }
}
 ...
 */
api.getAuthorization = function(req)
{
    const rc = {};
