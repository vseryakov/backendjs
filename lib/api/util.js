 ...
 *
 * This method uses `{@link module:api~defaults} to always limit incoming data and adjust if needed,
 * text fields are limited to 512 chars, object to 32k, json to 4k, arrays to 512 items.
 *
 ...
 * @param {boolean} [options.dprefix] - defaults prefix, if not given `context.path-`
 * is used to check defaults by path
 * @param {boolean} [options.query] - use `context.query` not body
 * @param {boolean} [options.params] - use `context.params` not body
 ...
 *
 * @example <caption>Overriding defaults, allow longer text fields</caption>
 * const opts = {
 *     defaults: {
 *         "*.string": { max: 1024 },
 *     }
...
api.validate = function(context, schema, options)
{
    const opts = Object.create(null, {
        dprefix: { value: `${context?.path}-`, enumerable: true },
        defaults: { value: api.defaults, enumerable: true },
    });

    let body = context.body || context.query;

    if (options) {
        lib.extend(opts, options);
        body = options.query ? context.query :
               options.params ? context.params : body;
    }

    logger.debug("validate:", "api", context, "schema:", schema, "opts:", opts, "body:", body);
    return lib.validate(body, schema, opts);
}
 ...
 *
 * api.limiter(context.ip, { rate: 100, interval: 60000, ttl: 900000 }, lib.log);
 *
 ...
 *
 * @example <caption>Login validation and limiter middleware, rate limit by login</caption>
 * async function(context) {
 *     const { err, data } = api.validate(context, {
 *         login: { type: "email", max: 512, required: true },
 *         secret: { max: 128, required: true },
 *     });
 *     if (err) return context.reply(err);
 *
 *     const key = context.path + "/" + data.login;
 *     const { limit }  = await api.alimiter(key, { rate: 1, interval: 1000, ttl: 3600000 });
 *     if (limit.err) return context.reply(limit.err)
 *
 *     const rc = await api.users.login(context);
 *
 *     context.reply(rc.err, api.users.cleanup(rc.user));
 * }
 * @memberof module:api
 * @method limiter
 */
 ...
 * @async
 */
api.alimiter = function(name, options)
{
