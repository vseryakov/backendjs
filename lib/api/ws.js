 ...
 *    ... processing logic
 *    ....
 *    api.ws.notify({ user_id: req.query.uid }, { op: "/message/new", msg: context.body.msg });
 * });
 * ```
 */
module.exports = {
...
mod.configureMiddleware = function(options, callback)
{
    ...
    api.app.all(this.path, (context, next) => {
        app.runMethods("configureWebsocketRequest", { wsid: req.wsid, context }, () => {
            context.send(200, "");
        });
    });
...
mod.handleWebSocketUpgrade = function(req, socket, head)
{
    ...
    // Prepare request/response for signature verification, have to similate middleware flow
    Object.setPrototypeOf(req, api.app.request);

    api.prepareContext(req);
...
mod.handleWebSocketConnect = function(ws, req)
{
    ...
    ws.user = Object.assign({}, req.context?.user);
    ws.query = lib.clone(req.query);
    ws.alive = true;
    ws.secure = req.secure;
    ws.hostname = req.hostname;

    logger.debug("handleWebSocketConnect:", req.socket.remoteAddress, req.url, ws.user?.id);
...
mod.handleWebSocketRequest = function(ws, data)
{
    ...
    if (data[0] == "/") {
        req.url = data;
        var q = data.indexOf("?");
        req.query = qs.parse(q > 0 ? data.substr(q + 1) : "");
    } else
    if (data[0] == "{" || data[0] == "[") {
        req.query = lib.jsonParse(data, { datatype: "obj", logger: "error" });
    } else {
        req.query = { data };
    }
...
mod.set = function(type, req, value)
{
    ...
    case "query":
    case "user":
        if (lib.isObject(value)) ws[type] = lib.clone(value);
        break;
    }
}
    ...
    for (const ws of mod.server.clients) {
        if ((!options.wsid || options.wsid === ws.wsid || lib.isFlag(options.wsid, ws.wsid)) &&
            ...
            if (options.cleanup) {
                ...
                for (const p in ws.user) if (p[0] == "i" && p[1] == "s" && p[2] >= 'A' && p[2] <= 'Z') req.context[p] = ws.user[p];
                for (const p in options) if (optsRx.test(p)) req.context[p] = options[p];
