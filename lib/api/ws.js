...
mod.configureWebServer = function(options, callback)
{
    ...
    if (!server) {
        ...
        server = app.createWebServer(opts, (req, res) => { res.status(200).send("OK") });
    }
    if (server) {
        ...
        mod.server.on("close", () => {
            mod.server.connections = {};
            clearInterval(mod.server._pingInterval);
            for (const ws of mod.server.clients) ws.terminate();
        });
        mod.server._pingInterval = setInterval(() => {
    ...
    logger.log("configureWebServer:", mod.name, app.role, "created", server?.name);
...
mod.shutdownServer = function(options, callback)
{
    if (mod.server) {
        ...
        try {
            ...
            mod.server.closeIdleConnections();
        } catch (e) {
            err = e
        }
        delete mod.server;
