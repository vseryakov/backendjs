 ...
 *
 * To enable websockets configure __api-ws-port__ to the same value as the HTTP port or other number, eg `api-ws-port = 8000`
 *
 * ## In the browser
 *
 * Open a socket first
...
mod.handleConnect = function(req, socket, head)
{
    mod.server.handleUpgrade(req, socket, head, (ws) => {
        ...
        ws.context = new RequestContext(req, null, api.contextOptions);
