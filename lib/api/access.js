 ...
 *  4. CSRF check
 *  5. run {@link module:api/access.authenticate}, if status is not 200 return an error
 *  6. run {@link module:api/access.authorize}, at least one ACL must match, on error return
...
mod.allow = function(req, callback)
{
    ...
    api.hooks.run("access", req, status, (err) => {
        logger.debug("allow:", mod.name, req.method, req.options.path, status, "ERR:", err);
        callback(err || status);
    }, true);
}
 ...
 * Also sets `user` in `req.options` for tracking with limited details.
 * @param {Request} req
 * @param {object} user
 * @memberof module:api/access
 * @method setUser
...
mod.setUser = function(req, user)
{
    ...
    if (user?.id) {
        ...
        req.options.user = { id: user.id, name: user.name, roles: user.roles };
    }
    logger.debug("setUser:", mod.name, req.options, "USER:", req.user)
}
...
mod.authenticate = function(req, callback)
{
    ...
    lib.everySeries([
        ...
        function(next, err, user) {
            ...
            api.signature.verify(req, sig, user, (ok) => {
                if (!ok) {
                    api.session.clear(req);
        ...
        function(next, err) {
            ...
            api.hooks.run("auth", req, err, (e) => {
                logger.debug('authenticate:', mod.name, req.options, "SIG:", req.signature, e || err);
                next(e || err);
            });
        },
    ], callback, true);
