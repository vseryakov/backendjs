...
class RequestContext {
    ...
    /** @var {{id:string, exp:number}} - parsed or created session from cookies or token */
    session
    ...
    */
    ...
    /**
     * Options object, set only on creation
     * @type {object}
     * @readOnly
     */
    get options() {
    ...
    /**
     * Assign a value to a context option
     * @param {string} name
     * @param {any} value
     */
    ...
    set params(data) {
        this.#params = data && typeof data === "object" ? data : _empty;
    }
    ...
    /**
     * Currently processing route, assigned by the router
     * @type {Route}
     * */
    get route() {
        return this.#route;
    ...
    */
     ...
     * @param {function} callback (context, data), this can be async as well
     * @returns {this}
     * Supported hooks:
     *  - destroy - called when context is destroyed
     *  - error - fatal error occured
     *  - body - body has been parsed
    ...
    /**
     * Emit hooks by name, uses `runHooks` and resolves all promises based on the callback parameter:
     * - callback is not given, resolve all promises silently via allSettled
     * - callback is given, run allSettled and call the callback, errors are passed to the callback as well
     ...
     * 'done!'
     *
     * @example <caption>using body callback in middleware, next middleware will be executed after all body hooks are done</caption>
     * context.on("body", async (ctx, data) => {
     *
     * })
     *
     * function handle(context, next) {
     *    context.emit("body", null, next);
     * }
     */
    emit(hook, data, callback) {
        const pending = this.runHooks(hook, data);
        if (!pending) return
        if (lib.isFunc(callback)) {
            Promise.allSettled(pending).then(() => callback()).catch(callback);
        } else {
            Promise.allSettled(pending).then(lib.none).catch(logger.trace);
        }
    }

    /**
     * Executes hook handlers and returns a list of Promises which must be resolved or awaited, non-promise handlers
     * are just run synchronously
     * @param {string} hook
     * @param {any} [data]
     * @returns {undefined|Promise[]} undefined if no async handlers executed or a list of pending Promises
     * @example
     *
     * context.on("event", async (ctx, data) => {
     *     await db.update(......)
     ...
     * })
     *
     * const pending = context.runHooks("event", data);
     * if (pending) {
    ...
    /**
     * Send either an error or the data as JSON, data is never sent if err is present.
     * @param {undefined|object|Error} err - error to return
     * @param {number} [err.status=500] - HTTP status or 500 if not given
     * @param {string|object|array} data - body to send if no error
     * @returns {this}
     */
    reply(err, data) {
        if (this.closed) return this;

        return err ? this.send(err.status || 500, err, JSON_TYPE) : this.json(data);
    }
