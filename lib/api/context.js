...
class RequestContext {
    ...
    */
    ...
    /**
     * Currrent authenticated user, on set  property `userId` is created with user.id, it is not cleared
     * on reseting the user to be logged
     * @type {object}
     */
    get user() {
        return this.#user;
    ...
    /**
    * Parse and return Authorization header as an object, { type, token, user, password }
    * @type {object}
    */
    get auth() {
    ...
    */
    ...
    /**
     * Emit hooks by name, promises are resolved awaited, all listeners are executed in the order of registration
     * @param {string} hook
     * @param {any} [data]
     * @async
     * @example <caption>Wait for DB op to finish before returning</caption>
     *
        ...
        this.stat(file, options, (err, stat) => {
            ...
            try {
                ...
                if (options?.noCache) {
                    ...
                    if (unmodifiedSince && stat.mtimeMs > unmodifiedSince) {
