...
class RequestContext {
    ...
    */
    setUrl(url) {
        ...
        if (this.#req && this.url != this.#req.url) {
    ...
    */
     ...
     */
    get closed() {
        const status = !this.#res || this.#req.closed || this.#res.headersSent || this.#res.writableEnded;
        if (status) logger.debug("send:", "context", this, "closed");
        return status;
    }
     ...
     * @param {boolean} [options.lastModified] - support If-Last-Modified condition
    ...
    sendFile(file, options, next) {
        ...
        fs.stat(file, (err, stat) => {
            logger.dev("sendFile:", "context", this, file, "STAT:", stat, "ERR:", err);
            ...
            try {
                ...
                stream.pipeline(fs.createReadStream(file, opts), this.#res, (err) => {
                    logger.error("sendFile:", "context", this, file, "ERR:", err);
                });
            } catch (e) {
                this.error(e);
            }
