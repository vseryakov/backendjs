...
const mod = {
    ...
    args: [
        ...
        { name: "path", descr: "Path to store files" },
    ],
};
...
mod.getUrl = function(file, options)
{
    if (file) {
        ...
        if (s3url && s3opts) {
            s3opts.url = true;
            file = modules.aws.signS3("GET", s3url, lib.normalize(file), "", s3opts);
        } else {
            var filesUrl = options?.filesUrl || this.url;
            if (filesUrl) {
                file = filesUrl + lib.normalize(file);
            }
        }
    }
    return file || "";
...
mod.send = function(req, file, options)
{
    ...
    if (options?.filesS3 || this.s3) {
        var rc = modules.aws.s3ParseUrl("s3://" + lib.normalize(options?.filesS3 || this.s3, file));
        ...
        file = lib.normalize(options?.filesPath || this.path, file);
...
mod.copy = function(source, dest, options, callback)
{
    ...
    if (options?.filesS3 || this.s3) {
        ...
        if (/^s3:/.test(source) && !/^s3:/.test(dest)) {
            dest = lib.normalize(options?.filesPath || this.path, dest);
    ...
    // both local files
    source = lib.normalize(options?.filesPath || this.path, source);
    dest = lib.normalize(options?.filesPath || this.path, dest);
...
mod.list = function(options, callback)
{
    ...
    if (options?.filesS3 || this.s3) {
        ...
        lib.findFile(`${options?.filesPath || this.path}/${options?.path}`, { depth: 0, include: options?.filter, details: options?.details }, (err, rows) => {
            for (const i in rows) {
...
mod.store = function(tmpfile, outfile, options, callback)
{
    ...
    if (options?.filesS3 || this.s3) {
        ...
        outfile = path.normalize(path.join(options?.filesPath || this.path, outfile));
        logger.debug("storeFile:", tmpfile, outfile, options);
...
mod.del = function(file, options, callback)
{
    ...
    if (options?.filesS3 || this.s3) {
        ...
        fs.unlink(path.join(options?.filesPath || this.path, file), (err) => {
