    ...
    lib.series([
        function(next) {
            api.checkSessionSignature(sig, (err, rc) => {
                if (rc < 0) return next({ status: 401, message: "This session has expired", code: "INVALID" });

                // Pre-authenticated request (WS)
                if (req.account.login == sig.login && req.account.id) {
                    api.setCurrentAccount(req, req.account);
                    return next({ status: api.authStatus, message: api.authMessage });
                }
                next();
            });
        },
        function(next) {
            ...
            auth.get({ login: sig.login }, (err, account, info) => {
                if (err) return next({ status: 500, message: String(err) });
                if (!account) return next({ status: 401, message: "Authentication failed", code: "NOLOGIN" });
                ...
                if (account.expires && account.expires < Date.now()) {
                    return next({ status: 412, message: "This account has expired", code: "EXPIRED" });
                }
                next();
            });
        },
        function(next) {
            ...
            api.verifySignature(req, sig, req.__account, (sig) => {
                if (!sig) {
                    api.clearSessionSignature(req);
                    return next({ status: 401, message: "Authentication failed", code: "NOLOGIN" });
                }
                ...
                next({ status: api.authStatus, message: api.authMessage });
            });
        },
    ], callback, true);
}
...
api.checkAuthorization = function(req, status, callback)
{
    ...
    if (!status?.status) status = { status: this.authStatus, message: this.authMessage };
    ...
    if (!this.checkAccountType(req.account, auth.adminRoles)) {
        ...
        if (this.checkAcl(this.allowAdmin, this.alowAclAdmin, req.options)) {
            ...
            return this.checkPreHooks(req, { status: 401, message: this.checkErrmsg(req, this.allowAclAdmin, "Restricted access"), code: "ADMIN" }, callback);
        }
        ...
        if (req.account.id && this.checkAcl(this.allowAuthenticated, this.allowAclAuthenticated, req.options)) {
            ...
            if (this.authStatus < 400 && rc.matched.length && !rc.allow) {
                ...
                return this.checkPreHooks(req, { status: 401, message: this.checkErrmsg(req, rc.matched, "Access is not allowed"), code: "ALLOW" }, callback);
            }
