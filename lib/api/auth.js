    ...
    lib.everySeries([
        function(next) {
            api.checkAccess(req, (status) => {
                if (!status || !status.status) return next(status);
        ...
        function(next, status) {
            ...
            api.checkRequestSignature(req, (status) => {
                ...
                // Run authentication hooks for alternative credentials
...
api.checkAuthorization = function(req, status, callback)
{
    ...
    if (req.signature && req.signature.source == "s" && this.checkAcl(this.disableSession, this.disableSessionAcl, req.options)) {
        ...
        return this.checkPreHooks(req, { status: 401, message: this.checkErrmsg(req, this.disableSessionAcl, "Not authorized"), code: "DISABLED" }, callback);
    }
...
api.checkPreHooks = function(req, status, callback)
{
    ...
    }, callback.bind(null, status), true);
}
