...
// Example:
//
//          api.registerAccessCheck('', 'account', (req, cb) => { cb({ status: 500, message: "access disabled"}) }))
//
//          api.registerAccessCheck('POST', '/account/add', (req, cb) => {
...
// the API route method is called. The `req.account` object will always exist at this point.
//
...
// Example:
//
//           api.registerPreProcess('GET', '/account/get', (req, status, cb) => {
...
// Example:
//
//           api.registerSignature('', '/', (req, account, sig, cb) => {
//                if (sig) {
//                    if (invalid) sig = null;
//                } else {
