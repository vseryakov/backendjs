...
{
    if (!type || !path || typeof callback != "function") return false;
    var bucket = type;
...
//  - `_count, _page, _tm, _sort, _select, _ext, _start, _token, _format, _total, _encoding, _ops`
//
...
//
// To continue to next hoook return nothing in the `cb`,
...
//  - callback is a function function(req, cb) {}
//
// Example:
//
//          api.registerAccessCheck('', 'user', (req, cb) => { cb({ status: 500, message: "access disabled"}) }))
//
//          api.registerAccessCheck('POST', '/user/add', (req, cb) => {
...
// Example:
//
//           api.registerPreProcess('GET', '/user/get', (req, status, cb) => {
...
//              if (req.user.type != "admin") return cb({ status: 401, message: "access denied, admins only" });
//              cb();
//          });
//
...
//
//          api.registerPostProcess('', '/data/', (req, res, rows) => {
//              rows.forEach((row) => { ...});
//          });
//
// Example, add data to the rows and return result after it
//
//          api.registerPostProcess('', '/data/', (req, res, row) => {
...
//
//          api.registerCleanup('', '/data/', (req, next) => {
...
// Example, do custom logging of all requests
//
//          api.registerSendStatus('', '/data/', (req, res, data) => {
...
// Example:
//
//           api.registerSignature('', '/', (req, user, sig, cb) => {
//                if (sig) {
//                    if (invalid) sig = null;
//                } else {
