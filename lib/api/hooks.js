...
{
    if (api.app) {
        var req = api.metrics.req.toJSON({ reset: 1 });
        var que = api.metrics.que.toJSON({ reset: 1 });
        options.stats.api_req_count = req.meter.count;
        options.stats.api_req_rate = req.meter.rate;
        options.stats.api_res_time = req.histogram.med;
        options.stats.api_que_size = que.med;
        for (const p in api.metrics) {
            if (typeof api.metrics[p] == "number" && p.endsWith("_count")) {
                options.stats["api_" + p] = metrics.take(api.metrics, p);
            }
        }
    }
}
