...
api.getCsrfToken = function(req)
{
    if (req && !req.csrfToken) {
        req._csrfToken = `,${lib.randomInt()},${Date.now() + this.csrfAge},${!req._csrfPub && req.user?.id || ""}`;
...
api.verifyCsrfToken = function(req)
{
    ...
    if (h[0] === "h" && lib.toNumber(h[2]) > Date.now() && (!h[3] || h[3] === req.user?.id)) {
...
api.clearCsrfToken = function(req)
{
    ...
    req.res.cookie(api.csrfHeaderName, "", opts);
