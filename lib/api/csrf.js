...
mod.check = function(req, options)
{
    ...
    for (const p in mod.origin) {
        if (lib.testRegexpObj(req.options.path, mod.origin[p]) || lib.testRegexpObj(location, mod.origin[p])) {
            if (!origin || origin != p) {
    ...
    for (const p in mod.secFetch) {
        if (lib.testRegexpObj(req.options.path, mod.secFetch[p]) || lib.testRegexpObj(location, mod.secFetch[p])) {
