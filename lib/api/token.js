 ...
 *
 * if context options contains `secretNoHash: true` then session secret will not be hashed when comparing with user secret, for cases when
 * storing raw secret is acceptable.
 *
...
mod.verify = function(context, secret)
{
    ...
    const hash = context.options.secretNoHash ? session.secret : lib.hash(session.secret);
 ...
 * @param {boolean} [options.nohash] - do not hash the secret
...
mod.create = function(context, options)
{
    ...
    context.session = Object.create(null, {
        ...
        secret: { value: options?.nohash ? secret : lib.hash(secret) },
