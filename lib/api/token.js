...
module.exports = {
    ...
    /**
     * Token age in milliseconds, default is 1 year
     * @var {number}
     * @default
     */
    age: 365 * 86400 * 1000,
...
mod.verify = function(context, secret)
{
    ...
    if (session.exp > 0 && session.exp < Date.now()) return false;
    return lib.timingSafeEqual(secret, lib.hash(session.secret));
}
