...
api.cleanupResult = function(table, data, options)
{
    ...
    const roles = lib.strSplit(options.account && options.account.type);
    const tables = lib.strSplit(table);
    const rules = options.cleanup_rules || lib.empty;
    var row, nrows, nrow;

    for (let i = 0; i < tables.length; ++i) {
        const dbcols = db.getColumns(tables[i], options);
        const dbrules = this.cleanupRules[tables[i]] || lib.empty;
        for (const p in dbcols) {
            col = dbcols[p] || lib.empty;
            r = cols[p] = typeof rules[p] == "number" ? rules[p] ? 1 : 0 :
                          typeof dbrules[p] == "number" ? dbrules[p] ? 1 : 0 :
                          typeof rules[p] == "function" ? rules[p](tables[i], p, col, options) ? 2 : 0 :
                          col.priv ? 0 :
                          col.pub ? 1 :
            ...
            if (r && !options.isInternal) {
                ...
                if (r && col.params) {
                    ...
                    for (const k in params) {
                        ...
                        r = col.priv ? 0 :
                            col.pub_staff ? options.isStaff || options.isInternal ? 1 : 0 :
                            col.pub_admin ? options.isAdmin || options.isInternal ? 1 : 0 : 1;
                        all++;
                        pos += r;
                        if (!r) hidden.push(k);
                    }
    ...
    const dbrules = this.cleanupRules["*"] || lib.empty;
    for (let i = 0; i < rows.length; ++i) {
        row = rows[i];
        nrow = null;
        for (const p in row) {
            col = cols[p];
            r = col === 0 || Array.isArray(col) || (strict && col === undefined && !rules[p] && !dbrules[p]);
            if (r) {
                // Lazy copy on modify
