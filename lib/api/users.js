 ...
 * - check the `user.expires` column, must be empty or in the future
...
mod.verifyToken = function(context, callback)
{
    ...
    mod.get({ login: auth.token.substr(0, pos) }, (err, user) => {
        ...
        if (user.expires > 0 && user.expires < Date.now()) {
            logger.debug("verifyToken:", mod.name, "expired:", context, user.id, user.name, user.expires);
            return callback({ status: 401, message: mod.errInvalidSession });
        }
 ...
 */
mod.prepareToken = function(query, options)
{
    query.login ??= lib.uuid(options?.prefix);

    const secret = lib.uuid();
    query.secret = lib.hash(secret);

    return `${query.login}${secret}`;
}
