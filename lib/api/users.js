 ...
 * api-acl-add-authenticated = ^/(auth|logout)
 *
 * ```
 *
 * ## API endpoints
...
module.exports = {
    ...
    args: [
        ...
        { name: "enabled", type: "bool", descr: "Enable users middleware" },
    ],
...
mod.configureMiddleware = function(options, callback)
{
    ...
    api.app.use(mod.access.bind(mod));
...
mod.auth = function(req, res)
{
    if (!req.context.user?.id) {
        return api.sendReply(req, { status: 417, message: mod.errInvalidLogin, code: "NOLOGIN" });
    }
    req.context.cleanup = mod.table;
    req.context.cleanup_strict = 1;
    api.sendJSON(req, null, req.context.user);
}
...
mod.login = function(req, res)
{
    ...
    mod.get(query.login, (err, user) => {
        if (!user) {
            return api.sendReply(req, { status: 401, message: mod.errInvalidLogin, code: "NOLOGIN" });
        }
        ...
        lib.checkSecret(user.secret, query.secret, (err, ok) => {
            ...
            const session = api.session.create(req, user.id, user.secret);
...
mod.logout = function(req, res)
{
    ...
    if (!req.context.user?.id || !req.context.session?.id) {
        return api.sendJSON(req);
    }

    // Delete all expired sessions including the current one
    const sessions = lib.isBool(req.context.query.force) ? null :
                     lib.split(req.context.user.sessions, { datatype: "int" }).
                         filter(x => x !== req.context.session.exp && x > Date.now());

    db.update(mod.table, { login: req.context.user.login, sessions }, () => {
        api.sendJSON(req);
    });
}
 ...
 * Steps:
 *  1. check if access is allowed by public path or via "access" hooks
 *  2. if status given and not 200 return the error
 *  3. if not matched continue otherwise
 *  4. CSRF check
 *  5. run {@link module:api/users.authenticate}, if status is not 200 return an error
 *  6. run {@link module:api/acl.authorize}, at least one ACL must match, on error return
 *  7. check for routing {@link module:api/routing.check}
 * @param {Request} req
 * @param {Response} res
 * @param {function} callback
 * @memberof module:api/users
 * @method access
 */
mod.access = function(req, res, callback)
{
    if (!this.enabled) return callback();

    const trace = req.context.trace.start("access");

    logger.debug("access:", mod.name, req.context);

    lib.everySeries([
        function(next) {
            if (api.acl.isPublic(req.context)) {
                return callback();
            }

            api.hooks.run("access", req, null, (err) => {
                if (!err) {
                    return next();
                }
        ...
        function(next) {
            ...
            mod.authenticate(req, (err) => {
                ...
                if (err?.status == 401) {
                    if (api.acl.isAnonymous(req.context)) {
                        return next();
                    }
                }

                if (err?.status >= 401 && err.status < 500) {
                    var loc = api.redirect.check(req, "loginPath", "loginLocation");
                    if (loc) {
                        return req.context.redirect(loc.url, loc.status);
                    }
                }
        ...
        function(next) {
            var status = api.acl.authorize(req.context);

            api.hooks.run("pre", req, status, (err) => {
...
{
    logger.debug("authenticate:", mod.name, req.context);

    const session = api.session.parse(req);
    if (!session) {
        logger.debug("authenticate:", mod.name, "nosession:", req.context);
        return callback({ status: 401, mess: mod.errInvalidSession });
    }

    api.users.get(session.id, (err, user, info) => {
        if (!err) {
            if (!user) {
                logger.debug("authenticate:", mod.name, "nouser:", req.context, session.id);
                err = { status: 401, message: mod.errInvalidSession };
            } else

            if (user.expires > 0 && user.expires < Date.now()) {
                logger.debug("authenticate:", mod.name, "expired:", req.context, user.id);
                err = { status: 401, message: mod.errInvalidSession };
            } else

            if (!lib.isFlag(user.sessions, session.exp) ||
                !api.session.verify(req, user.id, user.secret)) {
                logger.debug("authenticate:", mod.name, "nosession:", req.context, user.id, session);
                err = { status: 401, message: mod.errInvalidSession }
            }
            if (!err) {
                req.context.user = user;
