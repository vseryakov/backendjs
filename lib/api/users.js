 ...
 *
 * Supports API authentication as well for records with auto-generated login, real users can have more than one API token by
 * saving api tokens with the same user id.
 *
...
/**
 * Verify API token from Authorization header and sets user in the context on success.
 *
...
mod.verifyToken = function(context, callback)
{
    ...
    mod.get({ login: auth.token.substr(0, pos) }, (err, user) => {
        ...
        if (!lib.timingSafeEqual(lib.hash(auth.token.substr(pos)), user.secret)) {
