...
const logwatcher = {
    ...
    args: [
        ...
        { name: "local", type: "map", obj: "local", maptype: "auto", merge: 1, descr: "Save matched lines in local file, ex. file:filename, size:maxsize, ext:ext" },
    ...
    lib.forEvery(lib.objKeys(errors), (type, next) => {
        ...
        case "db://":
            db.add(d[2], {
               ...
               instance_tag: app.instance.tag,
               instance_region: app.instance.region,
               run_mode: app.runMode,
               data: subject + "\n" + text
            }, (err) => {
               if (err) logger.info("send:", this.name, err);
               next()
            });
            break;
        ...
        default:
            sendmail.send({ from: this.from, to: d[2], subject: subject, text: text }, (err) => {
                if (err) logger.info("notify:", this.name, err);
                next()
            });
        }
