...
shell.help.push(
    ...
    "-stats [-age S] [-interval S] [-groups tag] [-fields host_cpu_util] [-raw] [-count N] [-hdr] - show aggregated stats from bk_stats table",
);
...
{
    var pool = shell.getArg("-pool", options, "elasticsearch");
    var groups = lib.strSplit(shell.getArg("-groups", options, "tag"));
    var tags = lib.strSplit(shell.getArg("-tags", options));
    var nterms = shell.getArgInt("-nterms", options, 50);
    var age = shell.getArgInt("-age", options, 300) * 1000;
    var interval = shell.getArgInt("-interval", options, 180) * 1000;
    var fields = lib.strSplit(shell.getArg("-fields", options, "host_cpu_util"));
    var count = shell.getArgInt("-count", options);
    var hdr = shell.isArg("-hdr", options);
    ...
    db.search("bk_stats", q, { pool: pool, count: count }, (err, rows, info) => {
        if (!err) {
            ...
            if (!shell.isArg("-raw")) {
                ...
                for (const t of info.aggregations.stats.buckets) {
                    tags[t.key] = {};
                    for (const f of fields) {
                        tags[t.key][f] = t.fields.buckets.sort((a, b) => (b.key - a.key)).map((x) => (x[f] && Math.round(x[f].value) || 0)).join(", ");
            ...
            if (shell.isArg("-flatten")) {
                info = lib.objFlatten(info);
            }
            shell.jsonFormat(header, info, rows);
        }
        shell.exit(err);
    });
}
