...
shell.cmdStats = function(options, callback)
{
    var opts = {
        ...
        columns: lib.jsonParse(shell.getArg("-columns", options), { logger: "error" }),
    }
    ...
    core.modules.stats.queryElasticsearch(opts, (err, rc) => {
        if (!err) {
            if (shell.isArg("-raw")) {
                ...
                for (const k in rc.values) {
                    for (const p in rc.values[k]) {
                        rc.values[k][p] = rc.values[k][p].map((x) => (Math.round(x))).sort((a, b) => (b - a)).join(", ");
                    }
                }
                if (shell.isArg("-flatten")) {
                    rc.values = lib.objFlatten(rc.values);
                }
                var header = "# key, fields, " + rc.timestamps.map((x) => (lib.toAge(x, { short: 1, round: 2 }))).join(", ");
                shell.jsonFormat(header, rc.values, rc.rows);
            }
        }
        shell.exit(err);
    });
