...
shell.help.push(
    ...
    "-send-request -url URL [-method GET|POST] [-user ID|LOGIN] [-hdr name=value] param value param value ... - send API request to the server specified in the url as user specified by login or account id, resolving the user is done directly from the current db pool, param values should not be url-encoded",
...
shell.cmdSendRequest = function(options)
{
    ...
    lib.series([
      ...
      function(next, user) {
        core.sendRequest({ url: url, method: method, login: user?.login, secret: user?.secret, postdata: method && query, query: !method && query, headers: headers }, (err, params) => {
