...
shell.cmdExit = function()
{
    shell.exit();
}

shell.cmdHelp = function()
{
    for (const i in this.help) console.log("  ", this.help[i]);
    shell.exit();
}

shell.cmdHelpConfig = function()
{
    Object.keys(modules).forEach((n) => {
        if (!modules[n].args) return;

        console.log("## **{@link module:" + modules[n].name + "}**");
        ...
        modules[n].args.forEach((y) => {
            ...
            var dflt = lib.objGet(modules[n], `${y.obj ? y.obj + "." : ""}${name}`);
...
shell.cmdSendRequest = function(options)
{
    ...
    lib.series([
        ...
        function(next, user) {
            if (!csrf) return next();
            app.fetch({ url: csrf, method: method, cookies: {} }, (err, rc) => {
