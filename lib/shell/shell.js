...
shell.cmdStats = function(options, callback)
{
    var opts = {
        ...
        nterms: shell.getArgInt("-nterms", options, 50),
        age: shell.getArgInt("-age", options, 300) * 1000,
        interval: shell.getArgInt("-interval", options, 180) * 1000,
        ...
        if (!err) {
            if (shell.isArg("-raw")) {
                ...
                for (const k in rc.groups) {
                    for (const p in rc.groups[k]) {
                        rc.groups[k][p] = rc.groups[k][p].map((x) => (Math.round(x))).sort((a, b) => (b - a)).join(", ");
                    }
                }
                if (shell.isArg("-flatten")) {
                    rc.groups = lib.objFlatten(rc.groups);
                }
                var header = "# key, fields, " + rc.timeline.map((x) => (lib.toAge(x, { short: 1, round: 2 }))).join(", ");
                shell.jsonFormat(header, rc.groups, rc.rows);
            }
        }
        shell.exit(err);
    });
