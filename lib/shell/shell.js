...
const { app, aws, lib, api, jobs, events, cache, queue, shell, stats } = modules;
...
shell.help.push(
    ...
    "-run-method MOD.METHOD - run a method from a module",
    ...
    "-fetch -url URL [-method GET|POST] [-raw] [-user ID|LOGIN] [-csrf URL] [-hdr name=value] [-cookie name=value] [-aws service] param value param value ... - send API request to the server specified in the url as user specified by login or id, resolving the user is done directly from the current db pool, param values should not be url-encoded",
...
{
    var query = {};
    var index = typeof options?.index === "number" ? options.index : shell.cmdIndex;
    for (let i = index + 2; i < process.argv.length; i++) {
        const a = process.argv[i - 1], b = process.argv[i];
        if (a[0] === '-' && b[0] !== '-') i++; else
...
{
    var query = [];
    for (let i = process.argv.length - 1; i > shell.cmdIndex; i--) {
...
shell.getArgs = function(options)
{
    var query = {};
    var index = typeof options?.index === "number" ? options.index : shell.cmdIndex;
    for (let i = process.argv.length - 1; i > index + 1; i--) {
        const a = process.argv[i - 1], b = process.argv[i];
...
shell.commands.helpConfig = function()
{
    ...
    Object.keys(modules).sort().forEach((n) => {
        if (!modules[n].args) return;

        console.log(`## ${modules[n].name}`);
        console.log("See {@link module:" + modules[n].name.replace(".", "/") + "}");
        ...
        modules[n].args.forEach((y) => {
            ...
            if (!y.no_camel) name = lib.toCamel(name, y.camel || "-");
...
// App version
shell.commands.showInfo = function(_options)
{
    var info = {
        home: app.home,
        host: app.host,
...
shell.commands.showArgs = function(_options)
{
 ...
 *   bksh -run-file a.js
 *
 * // In the shell now it has new methods be executed
 *
 * > shell.newMethod()
 *
...
shell.commands.runFile = function(options)
{
    ...
    const mod = shell.loadFile(file);
    // Exported functions are set in the shell module
    for (const p in mod) if (typeof mod[p] === "function") shell[p] = mod[p];
...
shell.commands.runConfig = function(options)
{
    ...
    const args = lib.readFileSync(file, { cfg: 1 });
    app.parseArgs(args, 0, file);
    app.parseArgs(process.argv, 0, "cmdline");
    return "continue";
}

shell.commands.runMethod = function(options)
{
    const name = shell.getArg("-run-method", options);
    if (!name) return shell.exit("-run-method argument is required");

    const func = lib.objGet(modules, name);
    if (typeof func !== "function") shell.exit("invalid method");

    const res = func(options, lib.log);
    if (util.types.isPromise(res)) {
        res.then(null, lib.log);
    }
    return "continue";
}

// Initialize ipc/cache clients
shell.commands.runIpc = function(_options)
{
    queue.checkConfig();
    cache.checkConfig();
    return "continue";
...
shell.commands.runApi = function(_options)
{
    if (app.isPrimary) api.init();
    return "continue";
}

// Dump API routes inside the shell
shell.commands.dumpApi = function(_options)
{
    api.app.dump();
    return "continue";
}
...
shell.commands.runJobs = function(_options)
{
    if (app.isPrimary) {
        jobs.initServer();
    } else {
...
// Run jobs workers inside the shell
shell.commands.runWorker = function(_options)
{
    if (app.isPrimary) jobs.initWorker();
    return "continue";
}

// Run event workers inside the shell
shell.commands.runEvents = function(_options)
{
    events.initWorker();
    return "continue";
}
...
shell.commands.logWatch = function(_options)
{
    const mod = require(__dirname + '/../../dist/logwatcher');
    mod.run(shell.exit);
}
 ...
 * Shell command: `fetch`
 *
 * Sends an HTTP request using {@link lib.fetch} with optional BackendJS-style API signing,
 * CSRF token bootstrap, custom headers/cookies, and response shaping/formatting.
 *
 * Request inputs:
 * - Request data is taken from {@link shell.getQuery}:
 ...
 * - `-aws service` sign as AWS service
 *
 ...
 *
 * Signing behavior:
 * - When a user is loaded (`-user`) and has `login/secret`, a `signer` hook is provided to `lib.fetch` to create a session cookie
 *
 * Control flow:
 * - Runs as a 3-step async series:
 *   1) Optional user lookup
 *   3) Main request fetch + output formatting
 * - Terminates via {@link shell.exit} on completion or error.
 *
 * @function fetch
 * @memberof module:shell
 * @param {string[]|Object} options CLI options/args for the command.
 */
shell.commands.fetch = function(options)
{
    ...
    var select = lib.split(shell.getArg("-select", options));
    var json = shell.isArg("-json", options);
    var raw = shell.isArg("-raw", options);
    var service = shell.isArg("-aws", options);
    ...
    lib.series([
        ...
        function(_next, user) {
            var opts = {
                ...
                signer() {
                    if (user) {
                        const ctx = new api.RequestContext(null, new http.ServerResponse({}));
                        api.session.create(ctx, user);
                        this.headers[api.session.header] = ctx.res.getHeader("Set-Cookie");
                    } else

                    if (service) {
                        this.service = service;
                        aws.signer.call(this);
                    }
                }
            };
            lib.fetch(url, opts, (err, rc) => {
                ...
                if (flatten) rc.obj = lib.flatten(rc.obj);
...
shell.commands.submitJob = function(options)
{
    ...
    var query = shell.getQuery();
    opts.job = undefined;
    jobs.submit({ job: { [job]: query } }, opts, shell.exit)
}

/**
 * Shell command: `stats`
 *
 * Queries Elasticsearch-backed metrics via {@link stats.queryElasticsearch} and prints results.
 *
 * Options parsing:
 ...
 * - prints a header line with relative time labels derived from `rc.timestamps`
 *   using `lib.toDuration(..., { age: 1, short: 1, round: 2 })`.
 * - `-info`: includes `rc.info` in the formatted output.
 *
 * Exits the shell process via `shell.exit(err)` when done.
 *
 * @param {string[]|Object} options Command-line args/options passed to the shell command.
 * @param {Function} callback Unused (kept for command signature compatibility).
 * @memberOf module:shell
 * @method stats
 */
shell.commands.stats = function(options, _callback)
{
    var opts = {
        ...
        if (!err) {
            if (shell.isArg("-raw")) {
                ...
                if (shell.isArg("-flatten")) {
                    rc.data = lib.flatten(rc.data);
                }
                const header = "# key, fields, " + rc.timestamps.map((x) => (lib.toDuration(x, { age: 1, short: 1, round: 2 }))).join(", ");
