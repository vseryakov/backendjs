...
shell.help.push(
    ...
    "-stats [-age S] [-interval S] [-groups tag] [-fields host_cpu_util] - show aggregated stats from bk_stats table",
);
...
shell.cmdStats = function(options, callback)
{
    ...
    db.search("bk_stats", q, { pool: pool, count: 0, track_total_hits: 1 }, (err, rows, info) => {
        if (!err) {
            if (!shell.isArg("-raw")) {
                var tags = {};
