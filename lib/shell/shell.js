...
const { app, lib, api, jobs, cache, queue, shell, stats } = modules;
...
{
    var query = [];
    for (var i = process.argv.length - 1; i > shell.cmdIndex; i--) {
 ...
 * - `-csrf <url|token>`: If provided, performs a preliminary fetch to obtain CSRF token and attaches it:
 *   - If response contains `api.csrfHeaderName` in headers, copies it into outgoing request headers
 *   - If response sets a cookie named `api.csrfHeaderName`, copies it into outgoing request cookies
...
shell.commands.sendRequest = function(options)
{
    ...
    lib.series([
        ...
        function(next, user) {
            if (!csrf) return next();
            lib.fetch(url, { csrf, method: method, cookies: {} }, (err, rc) => {
 ...
 * - `-flatten`: flattens nested `rc.data` using `lib.objFlatten` before printing.
    ...
    stats.queryElasticsearch(opts, (err, rc) => {
        if (!err) {
            if (shell.isArg("-raw")) {
                ...
                var header = "# key, fields, " + rc.timestamps.map((x) => (lib.toDuration(x, { age: 1, short: 1, round: 2 }))).join(", ");
