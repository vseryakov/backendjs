...
shell.cmdAwsSetElbRoute53 = function(options)
{
    ...
    if (!domain) shell.exit("ERROR: -domain must be specified")
    var elb = this.getArg("-elb", options);
    if (!elb) shell.exit("ERROR: -elb is required");
    ...
    lib.series([
        ...
        function(next) {
            aws.queryELB2("DescribeLoadBalancers", {}, (err, rc) => {
                var b = rc?.DescribeLoadBalancersResponse?.DescribeLoadBalancersResult?.LoadBalancers?.member?.filter((x) => (x.LoadBalancerName == elb)).pop();
                dnsname = b?.DNSName;
