...
shell.help.push("-aws-set-elb-route53 -domain DOMAIN -elb NAME - assign a domain to ELB by name, creates a new domain if needed");
...
shell.help.push("-aws-create-cert -domain NAME [-star] [-email] [-arn ARN] - create a public SSL certificate from ACM, if -arn is given just update the Route53");
...
shell.cmdAwsSetRoute53 = function(options)
{
    ...
    if (!name) return shell.exit("ERROR: -name must be specified and must be a full host name")
    var filter = this.getArg("-filter", options);
...
{
    aws.route53Get({ zone: this.getArg("-zone", options), name: this.getArg("-name", options) }, (err, rc) => {
        var zone = rc?.GetHostedZoneResponse?.HostedZone?.Id;
        if (zone) {
            console.log(rc.GetHostedZoneResponse.HostedZone.Id,
                        rc.GetHostedZoneResponse.HostedZone.Name,
                        rc.GetHostedZoneResponse.HostedZone.ResourceRecordSetCount,
                        String(rc.GetHostedZoneResponse.DelegationSet?.NameServers?.NameServer));
        }
...
// Assign an ELB alias to a hosted zone, create a new domain if does not exist
shell.cmdAwsSetElbRoute53 = function(options)
{
    var domain = this.getArg("-domain", options);
    if (!domain) return shell.exit("ERROR: -domain must be specified")
    ...
    lib.series([
        function(next) {
            aws.route53Get({ name: domain }, (err, rc) => {
                hostedzone = rc?.GetHostedZoneResponse?.HostedZone?.Id;
                if (rc) console.log(util.inspect(rc.GetHostedZoneResponse, { depth: null }));
                next(err);
            });
        },
        function(next) {
            ...
            if (shell.isArg("-dry-run", options)) return next();
            aws.route53Create({ name: domain }, (err, rc) => {
                hostedzone = rc?.CreateHostedZoneResponse?.HostedZone?.Id;
                if (!err) console.log(util.inspect(rc?.CreateHostedZoneResponse, { depth: null }));
                next(err);
            });
        },
        function(next) {
        ...
        function(next) {
            logger.log("setRoute53:", domain, hostedzone, elb, dnsname, zoneId);
            if (shell.isArg("-dry-run", options)) return next();
            aws.route53Change({ hostedzone: hostedzone, name: domain, alias: dnsname, zoneId: zoneId }, next);
        },
    ], shell.exit);
}
    ...
    lib.series([
        ...
        function(next, rc) {
            ...
            db.get("bk_config", { type: type, name: `jobs-worker-queue` }, (err, row) => {
                ...
                logger.info("CreateQueue:", type, queue);
...
shell.cmdAwsManageCert = function(options, callback)
{
    ...
    lib.series([
        function(next) {
            aws.queryELB2("DescribeLoadBalancers", { "Names.member.1": elb }, (err, rc) => {
                var b = rc?.DescribeLoadBalancersResponse?.DescribeLoadBalancersResult?.LoadBalancers?.member;
                elb = b?.LoadBalancerArn;
                next(err);
            });
        },
        function(next) {
            aws.queryELB2("DescribeListeners", { LoadBalancerArn: elb }, (err, rc) => {
...
shell.cmdAwsCreateCert = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            ...
            aws.queryACM("DescribeCertificate", req, (err, rc) => {
                cert = rc?.Certificate;
                if (!err && cert) {
        ...
        function(next) {
            ...
            logger.info("DNS Validation:", cname);
