...
shell.cmdAwsRunInstances = function(options)
{
    this.awsRunInstances(options, shell.exit);
}
...
shell.cmdAwsSetupSsh = function(options)
{
    ...
    lib.series([
       ...
       function(next) {
           ...
           app.httpGet("http://checkip.amazonaws.com", function(err, params) {
...
shell.cmdAwsCreateQueue = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            ipc.broadcast(":master", "config:init");
            setTimeout(next, 3000);
        },
        function(next) {
            ipc.broadcast(":master", "queue:check");
        ...
        function(next, rc) {
            ...
            db.select("bk_config", { type, name }, (err, rows) => {
                ...
                db.setConfig({ ctime: row?.ctime, type, name, value }, (err) => {
                   if (!err) ipc.broadcast(":master", "config:init");
                   next(err);
                });
            });
        },
