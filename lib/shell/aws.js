...
shell.help.push("-aws-show-amazon-images [-filter PATTERN] [-arch ARCH] [-rootdev ebs|instance-store|*] [-devtype gp3|gp2|io1|standard|*] - show Amazon Linux AMIs (use %2A instead of *)");
...
shell.awsSearchImage = function(filter, appName, callback)
{
    ...
    this.awsGetSelfImages(filter, (err, rc) => {
        if (err) return callback(err, []);
...
// Return Amazon AMIs for the current region
shell.awsGetAmazonImages = function(options, callback)
{
    var query = {
        'Filter.1.Name': 'name',
        'Filter.1.Value': options.filter,
    };
    if (options.owner) query['Owner.0'] = options.owner;
...
shell.awsGetUserData = function(options)
{
    ...
    if (!userData || userData.match(/^#cloud-config/)) {
        ...
        if (bkjsCmd.length) cloudInit += "runcmd:\n" + bkjsCmd.map((x) => (" - /home/" + user + "/bin/bkjs " + x)).join("\n") + "\n";
...
shell.awsLaunchInstances = function(options, callback)
{
    ...
    var req = {
        ...
        device: {
            name: this.getArg("-dev-name", options, "/dev/xvda"),
            size: this.getArg("-dev-size", options, 32),
            type: this.getArg("-dev-type", options, "gp3"),
            iops: this.getArgInt("-dev-iops", options),
            keep: this.isArg("-dev-keep", options),
            virtual: this.getArg("-dev-virtual", options),
        }
    };
    ...
    lib.series([
       function(next) {
           ...
           shell.awsSearchImage(imageName, appName, (err, ami) => {
       ...
       function(next) {
           ...
           aws.queryELB("DescribeLoadBalancers", {}, (err, rc) => {
               ...
               if (req.elbName) {
                   if (!list.filter((x) => (x.LoadBalancerName == req.elbName)).length) return next("ERROR: Invalid load balancer " + aws.elbName);
               } else {
                   req.elbName = list.filter((x) => (x.LoadBalancerName.match("^" + appName))).map((x) => (x.LoadBalancerName)).pop();
               }
               next();
           });
       },
       ...
       function(next) {
           ...
           lib.forEachLimit(subnets, subnets.length, (subnet, next2) => {
               req.subnetId = subnet;
               logger.debug("awsLaunchInstances:", req);
               ...
               aws.ec2RunInstances(req, (err, rc) => {
                   if (!err) instances.push(...lib.objGet(rc, "RunInstancesResponse.instancesSet.item", { list: 1 }));
                   next2(err);
               });
           }, next);
       },
       function(next) {
           if (instances.length != 1 || !shell.isArg("-wait", options)) {
               for (const x of instances) console.log("EC2-Instance:", x.instanceId, x.privateIpAddress || "");
               return next();
           }
           var opts = {
           ...
           aws.ec2WaitForInstance(instances[0].instanceId, "running", opts, (err, rc) => {
               if (!err) console.log("EC2-Instance:", rc.instanceId, rc.privateIpAddress || "-", rc.publicIpAddress || "-", rc.architecture || "-", rc.name || "-");
               next(err, rc);
           });
       },
    ], callback);
...
shell.cmdAwsShowImages = function(options)
{
    ...
    this.awsGetSelfImages(filter || "*", function(err, images) {
        ...
        images.forEach((x) => {
            console.log(x.imageId, x.name, x.imageState, x.architecture, x.creationDate, x.description);
        });
        shell.exit();
    });
}
...
{
    options.owner = this.getArg("-owner", options);
...
shell.cmdAwsCreateImage = function(options)
{
    ...
    lib.series([
        ...
        function(next) {
            ...
            var running = 1;
            var interval = this.getArgInt("-interval", options, 5000);
...
shell.cmdAwsSetupInstance = function(options)
{
    ...
    if (!file && !cmd) return shell.exit("-file or -cmd is required");
...
shell.cmdAwsShowLogs = function(options, callback)
{
    ...
    var q = {
        ...
        prefix: this.getArg("-prefix", options),
        timeout: this.getArgInt("-timeout", options),
        limit: this.getArgInt("-limit", options, 100),
        names: lib.strSplit(this.getArg("-streams", options)),
    };
...
{
    var arn = this.getArg("-arn", options);
...
{
    var status = lib.strSplit(this.getArg("-status", options, "ISSUED"));
    var elb = this.getArg("-elb", options), list = [];
...
shell.cmdAwsManageCert = function(options, callback)
{
    ...
    var elb = this.getArg("-elb", options);
...
{
    var name = this.getArg("-domain", options);
    ...
    var filter = this.getArg("-filter", options);
    var values = lib.strSplit(this.getArg("-value", options));
...
{
    aws.route53Get({ zone: this.getArg("-zone", options), name: this.getArg("-domain", options) }, (err, rc) => {
