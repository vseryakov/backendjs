...
const { shell } = require(__dirname + '/../modules');
...
shell.help.push("-aws-show-queues [-prefix PATTERN] - show SQS queues stats");
...
shell.help.push("-aws-create-cert -domain NAME [-domains D,D...] [-star] [-email] [-arn ARN] [-wait N] - create a public SSL certificate from ACM, if -arn is given just update the Route53");
...
shell.awsCheckTags = function(obj, _name)
{
    var tags = lib.objGet(obj, "tagSet.item", { list: 1 });
    if (!tags.length) return false;
    for (let i = 1; i < arguments.length; i++) {
        if (!arguments[i]) continue;
        const rx = new RegExp(String(arguments[i]), "i");
...
shell.awsSearchImages = function(options, callback)
{
    ...
    if (options.owner) {
        const owner = lib.split(options.owner);
...
shell.awsRunInstances = function(options, callback)
{
    var appVersion = shell.getArg("-app-version", options, app.version).replace("/", "-");

    var req = {
        name: shell.getArg("-name", options, appVersion),
        ...
        publicIp: shell.isArg("-public-ip", options),
        ...
        terminate: shell.isArg("-terminate", options),
        launchTemplate: shell.getArg("-launch-template", options),
        metadata: shell.isArg("-metadata", options),
        alarms: [],
        ...
        device: {
            ...
            keep: shell.isArg("-dev-keep", options),
            virtual: shell.getArg("-dev-virtual", options),
        }
    };

    logger.debug("awsRunInstances:", req);

    lib.series([
        function(next) {
            if (req.imageId || req.launchTemplate) return next();
            var opts = {
        ...
        function(next) {
            if (req.groupId || req.launchTemplate) return next();
            var filter = shell.getArg("-group-name", options, appVersion + "|^default$");
            aws.ec2DescribeSecurityGroups({ filter }, (err, rc) => {
        ...
        function(next) {
            ...
            if (shell.isArg("-dry-run", options)) {
                logger.dump("awsRunInstances:", req);
                return next();
            }

            logger.debug("awsRunInstances:", req);

            aws.ec2RunInstances(req, (err, _rc, info) => {
...
shell.commands.awsRunInstances = function(options)
{
    shell.awsRunInstances(options, shell.exit);
}

shell.commands.awsShowImages = function(options)
{
    ...
    shell.awsSearchImages(options, (err, images) => {
        if (!err) {
            images.forEach((x) => {
                console.log(x.imageId, x.name, x.architecture, x.imageState, x.creationDate, x.description);
            });
        }
        shell.exit(err);
    });
...
shell.commands.awsShowGroups = function(options)
{
    options.filter = shell.getArg("-filter", options);
...
shell.commands.awsShowSubnets = function(options)
{
    options.filter = shell.getArg("-filter", options);
    options.name = shell.getArg("-name", options);
...
// Delete an AMI with the snapshot
shell.commands.awsDeleteImage = function(options)
{
...
shell.commands.awsCreateImage = function(options)
{
    options.name = shell.getArg("-name", options);
    ...
    options.noreboot = shell.isArg("-no-reboot", options);
    options.reboot = shell.isArg("-reboot", options);
    options.quiet = !shell.isArg("-verbose", options);
    if (shell.isArg("-dry-run", options)) return shell.exit(null, options);
    var imgId, imgState;
        ...
        function(next) {
            aws.ec2CreateImage(options, (err, rc) => {
                ...
                if (err?.code === "InvalidAMIName.Duplicate" && shell.isArg("-force", options)) {
                    const d = err.message.match(/in use by AMI (ami-[0-9a-z]+)/);
...
shell.commands.awsCopyImage = function(options)
{
    ...
    lib.series([
      ...
      // Deregister existing image with the same name in the destination region
      function(next) {
          aws.queryEC2("DescribeImages", { 'ImageId.1': imageId }, { region }, function(err, rc) {
              if (err) return next(err);
              var images = lib.objGet(rc, "DescribeImagesResponse.imagesSet.item", { list: 1 });
              ...
              if (shell.isArg("-dry-run", options)) return next();
              aws.ec2DeregisterImage(images[0].imageId, { snapshots: 1, region }, next);
          });
      },
      function(next) {
          ...
          if (shell.isArg("-dry-run", options)) return next();
          aws.queryEC2("CopyImage", req, { region }, function(err, rc) {
              if (err) return next(err);
...
shell.commands.awsRebootInstances = function(options)
{
    var instances = [];
    var filter = shell.getArg("-filter", options);
...
shell.commands.awsTerminateInstances = function(options)
{
    var instances = [];
    var filter = shell.getArg("-filter", options);
    if (!filter) return shell.exit("-filter is required");
...
shell.commands.awsShowInstances = function(options)
{
    var instances = [];
    var filter = shell.getArg("-filter", options);
    var col = shell.getArg("-col", options);
    var cols = lib.split(shell.getArg("-cols", options, "id,az,priv,ip,arch,type,name"));
    ...
    lib.series([
        ...
        function(next) {
            ...
            if (col) {
                console.log(instances.map((x) => (lib.inspect(lib.objGet(x, map[col] || col)))).join(" "));
            } else {
                instances.forEach((x) => {
                    console.log(cols.map((col) => (lib.inspect(lib.objGet(x, map[col] || col)))).join("\t"));
                });
            }
            next();
        },
...
shell.commands.awsShowTasks = function(options)
{
    lib.series([
        function(next) {
            var req = {
        ...
        function(next, rc) {
            logger.debug("showTasks:", rc);
            if (rc?.failures?.length) console.log("failures:", lib.inspect(rc.failures))
            if (!rc?.tasks?.length) return next();

            var col = shell.getArg("-col", options);
            var cols = lib.split(shell.getArg("-cols", options, "id,priv,az,cpu,memory,arch,name,family"));

            var map = { priv: "privateIpAddress", az: "availabilityZone", disk: "ephemeralStorage.sizeInGiB" }
            if (col) {
                console.log(rc.tasks.map((x) => (lib.inspect(lib.objGet(x, map[col] || col)))).join(" "));
            } else {
                rc.tasks.forEach((x) => {
                    console.log(cols.map((col) => (lib.inspect(lib.objGet(x, map[col] || col)))).join("\t"));
                });
...
// Show SQS queue stats
shell.commands.awsShowQueues = function(options)
{
    var all = shell.isArg("-all", options);
    var prefix = shell.getArg("-prefix", options);

    lib.series([
        function(next) {
            aws.querySQS("ListQueues", { MaxResults: 1000, QueueNamePrefix: prefix }, options, next);
        },
        function(next, rc) {
            logger.debug("awsShowQueues:", rc);

            lib.forEachSeries(rc?.QueueUrls?.sort(), (q, next2) => {
                aws.querySQS("GetQueueAttributes", { QueueUrl: q, AttributeNames: ["All"] }, options, (err, rc) => {
                    if (!err) {
                        if (all) {
                            console.log(q.split("/").pop(), lib.inspect(rc?.Attributes));
                        } else {
                            console.log(q.split("/").pop(),
                                "timeout:", lib.toNumber(rc?.Attributes?.VisibilityTimeout),
                                "total:", lib.toNumber(rc?.Attributes?.ApproximateNumberOfMessages),
                                "inflight:", lib.toNumber(rc?.Attributes?.ApproximateNumberOfMessagesNotVisible),
                            );
                        }
                    }
                    next2(err);
...
shell.commands.awsSetupSsh = function(options)
{
    var ip = "", groupId;
    ...
    lib.series([
       ...
       function(next) {
           ...
           lib.fetch("http://checkip.amazonaws.com", (err, params) => {
...
// Get file
shell.commands.awsS3Get = function(options)
{
...
// Put file
shell.commands.awsS3Put = function(options)
{
    var query = shell.getQuery();
...
// List folder
shell.commands.awsS3List = function(options)
{
    var query = shell.getQuery();
...
shell.commands.awsCheckCfn = function(options)
{
    var file = shell.getArg("-file", options);
...
shell.commands.awsCreateCfn = function(options)
{
    ...
    aws.queryCFN("CreateStack", req, (err, rc) => {
        ...
        shell.commands.awsWaitCfn(options, (err) => {
            if (err) return shell.commands.awsShowCfnEvents(options);
            shell.exit();
        });
    });
}

shell.commands.awsWaitCfn = function(options, callback)
{
    var name = shell.getArg("-name", options);
    if (!name) return shell.exit("ERROR: -name is required");
...
shell.commands.awsShowCfnEvents = function(options)
{
    var name = shell.getArg("-name", options);
    if (!name) return shell.exit("ERROR: -name is required");
...
shell.commands.awsCreateLaunchTemplateVersion = function(options, callback)
{
    var appVersion = shell.getArg("-app-version", options, app.version).replace("/", "-");
    var name = shell.getArg("-name", options);
    ...
    lib.series([
        ...
        function(next) {
            var opts = {
                LaunchTemplateName: name,
                VersionDescription: image ? image.name : appVersion,
            };
            ...
            var groups = lib.split(shell.getArg("-group-id", options)).sort();
            ...
            if (dsize && (!dev?.ebs || dev.ebs.volumeSize !== dsize || dev.ebs.volumeType !== dtype || (iops && dev.ebs.iops !== iops))) {
...
shell.commands.awsShowLogs = function(options, _callback)
{
    ...
    var q = {
        ...
        names: lib.split(shell.getArg("-streams", options)),
    };
...
shell.commands.awsCreateQueue = function(options, _callback)
{
    var name = shell.getArg("-name", options);
    if (!name) return shell.exit("ERROR: -name is required");
    ...
    lib.series([
        function(next) {
            ...
            for (const a of ["VisibilityTimeout", "DelaySeconds", "FifoQueue", "ContentBasedDeduplication"]) {
                const v = shell.getArg("-" + lib.toUncamel(a), options);
                if (!v) continue;
        ...
        function(next, rc) {
            ...
            db.setConfig({ type, name: `cache-${name.replace(/[^a-z0-9]/ig, "")}`, value: url }, next);
        },
        function(next) {
            ipc.broadcast(":server", "config:init");
            setTimeout(next, 3000);
        },
        function(next) {
            ipc.broadcast(":server", "queue:check");
        ...
        function(next, _rc) {
            ...
            if (!type) return next();
            var name = "jobs-worker-queue";
            db.select("bk_config", { type, name }, (_err, rows) => {
                var row = rows?.at(-1);
                var value = lib.arrayUpdate("add", lib.split(row?.value), name);
                console.log("CreateQueue:", type, value);
                db.setConfig({ ctime: row?.ctime, type, name, value }, (err) => {
                   if (!err) ipc.broadcast(":server", "config:init");
                   next(err);
                });
            });
        },
...
shell.commands.awsShowCert = function(options, _callback)
{
...
shell.commands.awsListCert = function(options, _callback)
{
    var status = lib.split(shell.getArg("-status", options, "ISSUED"));
    var elb = shell.getArg("-elb", options), list = [];

    lib.series([
        function(next) {
            aws.listCertificates({ status }, (err, rc) => {
                if (rc?.length) {
                    list = rc;
                    if (!elb) {
                        list.sort((a, b) => (a.DomainName.localeCompare(b.DomainName)));
                        for (const i in rc) console.log(rc[i].DomainName, lib.inspect(convCert(rc[i])));
                    }
                }
                next(err);
            });
        ...
        function(next) {
            if (!elb) return next();
            shell.awsGetCertListener({ elb }, next);
        },
        function(next, listener) {
            ...
            aws.queryELB2("DescribeListenerCertificates", { ListenerArn: listener }, (err, rc) => {
                if (!err) {
                    const l = rc?.DescribeListenerCertificatesResponse?.DescribeListenerCertificatesResult?.Certificates?.member;
                    if (l?.length) {
                        for (const c of list) {
                            for (const m of l) {
...
shell.commands.awsManageCert = function(options, _callback)
{
    var arn = shell.getArg("-arn", options);
    if (!arn) return shell.exit("ERROR: -arn is required");
    ...
    shell.awsManageCert({ arn, elb }, shell.exit);
}
...
shell.awsManageCert = function(options, callback)
{
    lib.series([
        ...
        function(next, listener) {
            ...
            var i = 1;
            for (let a of lib.split(options.arn)) {
...
shell.commands.awsCreateCert = function(options, _callback)
{
    ...
    var domains = lib.split(shell.getArg("-domains", options))
    ...
    lib.series([
        function(next) {
            ...
            var req = {
                ...
                IdempotencyToken: shell.getArg("-it", options, app.id),
            };
            if (domains.length) {
                req.SubjectAlternativeNames = domains;
            }
            ...
            lib.doWhilst(
                function(next2) {
                    aws.queryACM("DescribeCertificate", { CertificateArn: arn }, (err, rc) => {
                        cert = rc?.Certificate;
                        if (!err && cert) {
                            if (cert?.status === "FAILED") return next(cert)
        ...
        function(next) {
            ...
            shell.awsManageCert({ arn: cert.CertificateArn, elb }, next);
        },
    ], (err) => {
        shell.exit(err, cert);
    });
...
shell.commands.awsDelCert = function(options, _callback)
{
    var arn = shell.getArg("-arn", options);
    if (!arn) return shell.exit("ERROR: -arn is required");
    if (!arn.includes(":")) arn = `arn:aws:acm:${aws.region}:${aws.accountId}:certificate/${arn}`;
...
shell.commands.awsSetRoute53 = function(options)
{
    ...
    var current = shell.isArg("-current", options);
    var filter = shell.getArg("-filter", options);
    var values = lib.split(shell.getArg("-value", options));
    var type = shell.getArg("-type", options, "A");

    lib.series([
        function(next) {
            if (current || !filter) return next();
            var pub = shell.isArg("-public", options);
            var req = { stateName: "running", tagName: filter };
            aws.ec2DescribeInstances(req, (err, list) => {
                if (err) return next(err);''
                values = list.map((x) => {
                    switch (type) {
                    case "A":
                        return pub ? x.ipAddress || x.publicIpAddress : x.privateIpAddress;
                    case "CNAME":
                        return pub ? x.publicDnsName : x.privateDnsName;
                    }
                    return 0;
                }).filter((x) => (x));
                next();
        ...
        function(next) {
            var host = lib.toTemplate(name, app.env);
            if (current) {
                console.log("CHANGE:", name, host, values, app.ipaddr);
                if (shell.isArg("-dry-run", options)) return next();
            ...
            var rr = { op, name: host, type, ttl, value: values, alias, zoneId }
...
shell.commands.awsListRoute53 = function(_options)
{
...
shell.commands.awsGetRoute53 = function(options)
{
    aws.route53Get({ zone: shell.getArg("-zone", options), name: shell.getArg("-domain", options) }, (err, rc) => {
        ...
        if (!zone || !shell.isArg("-rrset")) return shell.exit(err);
...
shell.commands.awsCreateRoute53 = function(options)
{
    var name = shell.getArg("-domain", options);
    ...
    lib.series([
        ...
        function(next) {
            ...
            if (shell.isArg("-dry-run", options)) return next();
            aws.route53Change({ hostedzone, name, alias: dnsname, zoneId }, next);
        },
    ], shell.exit);
}
