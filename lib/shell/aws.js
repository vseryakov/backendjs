...
shell.help.push("-aws-set-route53 -domain HOSTNAME [-current] [-filter PATTERN] [-type A|CNAME] [-ttl N] [-public] [-dry-run] - create or update a Route53 record of specified type with IP/hostnames of all instances that satisfy the given filter, -public makes it use public IP/hostnames");
...
shell.cmdAwsCreateCert = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            if (!ready || !elb) {
                if (cname) shell.exit(`TIMEOUT: rerun again as:`, `bksh -aws-create-cert -domain ${domain} -arn ${arn.split("/").pop()} -elb ${elb}`);
                return next();
            }
...
shell.cmdAwsSetRoute53 = function(options)
{
    ...
    lib.series([
       ...
       function(next) {
           ...
           console.log("CHANGE:", name, host, type, values, core.ipaddr);
           if (shell.isArg("-dry-run", options)) return next();
