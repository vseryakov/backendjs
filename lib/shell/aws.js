...
shell.awsLaunchInstances = function(options, callback)
{
    ...
    var req = {
        ...
        vpcId: shell.getArg("-vpc-id", options, aws.vpcId),
        instanceType: shell.getArg("-instance-type", options, aws.instanceType),
        imageId: shell.getArg("-image-id", options, aws.imageId),
        subnetId: shell.getArg("-subnet-id", options, aws.subnetId),
        keyName: shell.getArg("-key-name", options, aws.keyName) || appName,
        elbName: shell.getArg("-elb-name", options, aws.elbName),
        elasticIp: shell.getArg("-elastic-ip", options),
        publicIp: this.isArg("-public-ip", options),
        groupId: shell.getArg("-group-id", options, aws.groupId),
        iamProfile: shell.getArg("-iam-profile", options, aws.iamProfile) || appName,
...
shell.cmdAwsCreateLaunchTemplateVersion = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            ...
            shell.awsSearchImage(filter, appName, (err, rc) => {
                if (!err) image = rc;
                next(err);
            });
        },
...
shell.cmdAwsShowLogs = function(options, callback)
{
    var name = shell.getArg("-name", options);
    if (!name) return shell.exit("ERROR: -name is required");
    var stime = shell.getArgInt("-start", options) * 3600000;
