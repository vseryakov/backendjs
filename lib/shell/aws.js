...
{
    var name = this.getArg("-name", options);
    if (!name) return shell.exit("ERROR: -name must be specified")
    ...
    lib.series([
        ...
        function(next) {
            logger.log("createRoute53:", name, hostedzone, elb, dnsname, zoneId);
            if (shell.isArg("-dry-run", options)) return next();
...
shell.cmdAwsListCert = function(options, callback)
{
    ...
    var elb = this.getArg("-elb", options), listener, list;

    lib.series([
        function(next) {
            aws.listCertificates({ status: status }, (err, rc) => {
                list = rc;
                if (!elb) for (const i in rc) console.log(lib.objDescr(convCert(rc[i])));
                next(err);
            });
        },
        function(next) {
        ...
        function(next) {
            ...
            aws.queryELB2("DescribeListenerCertificates", { ListenerArn: listener }, (err, rc) => {
                if (!err) {
                    ...
                    for (const i in l) {
                        var c = list.filter((x) => (x.CertificateArn == l[i].CertificateArn)).pop();
                        if (c) console.log(c.DomainName, c.CertificateArn); else console.log(l[i]);
                    }
