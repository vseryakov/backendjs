...
shell.help.push("-aws-create-queue -name NAME [-type production] [-visibility-timeout 30] - create a SQS queue and refresh the config");
    ...
    lib.series([
        function(next) {
            ...
            for (const a of ["VisibilityTimeout", "DelaySeconds", "FifoQueue", "ContentBasedDeduplication"]) {
                var v = this.getArg("-" + lib.toUncamel(a), options);
                if (!v) continue;
                q["Attribute." + n + ".Name"] = a;
                q["Attribute." + n + ".Value"] = v;
                n++;
            }
            this.querySQS("CreateQueue", q, next);
        },
        function(next, rc) {
            var type = this.getArg("-type", options, "production");
            var url = lib.objGet(rc, "CreateQueueResponse.CreateQueueResult.QueueUrl");
            db.put("bk_config", { type: type, name: `ipc-queue-${name}`, value: url }, next);
        },
        function(next) {
            ["master","server","web","worker"].forEach((x) => {
                ipc.broadcast(core.name + ":" + x, "config:init", { queueName: ipc.systemQueue });
            });
            setTimeout(next, 3000);
        },
        function(next) {
            ["master","server","web","worker"].forEach((x) => {
                ipc.broadcast(core.name + ":" + x, "queue:check", { queueName: ipc.systemQueue });
            });
        },
    ], callback);
}
