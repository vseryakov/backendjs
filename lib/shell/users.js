...
{
    api.users.add(shell.getQuery(), lib.objExtend(shell.getArgs(), { isInternal: 1 }), shell.exit);
}

// Update a user login
shell.commands.userUpdate = function(options)
{
    api.users.update(shell.getQuery(), lib.objExtend(shell.getArgs(), { isInternal: 1 }), shell.exit);
}
