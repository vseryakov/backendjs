...
shell.commands.userSendMfa = function(_options)
{
...
shell.commands.userSecret = function(_options)
{
...
// Add a user login
shell.commands.userAdd = async function(_options)
{
...
shell.commands.userAddToken = function(_options)
{
    const query = shell.getQuery();
    const opts = shell.getArgs();

    const context = new api.RequestContext();
    const session = api.token.create(context, opts);
    query.login = session.id;
    query.secret = session.secret;

    api.users.add(query, Object.assign(opts, { isInternal: 1 }), (err, _row) => {
       shell.exit(err, !err ? session.token : "");
    });
}


// Update a user login
shell.commands.userUpdate = async function(_options)
{
    const query = shell.getQuery();
...
// Delete a user login
shell.commands.userDel = function(_options)
{
