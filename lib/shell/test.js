    ...
    setTimeout(() => {
        ...
        lib.whilst(
            ...
            function(err) {
                ...
                if (err) {
                    logger.error("TEST FAILED:", test.cmd);
                    shell.die(err, test);
                }
                setInterval(() => {
...
// Generic access checker to be use din tests, accepts an array in .config with urls to check
...
// - postprocess - function(conf, cb) to be called after the request
        ...
        lib.everySeries([
            ...
            function(next2) {
                core.sendRequest(q, (err, rc) => {
                    ...
                    if (conf.delay) {
                        return setTimeout(next2, conf.delay);
                    }
                    next2();
                });
            },
