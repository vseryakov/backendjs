...
tests.expect = function(ok, ...args)
{
    ...
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.run, this.test.filename, args[0]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.run, this.test.file);
...
tests.assert = function(...args)
{
    if (!args[0]) return;
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.run, this.test.filename, args[1]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.run, this.test.file);
