...
tests.expect = function(ok, ...args)
{
    ...
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.filename, this.descr && JSON.stringify(tests.descr) || "", args[0]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.file, this.descr && JSON.stringify(tests.descr) || "");
...
tests.assert = function(...args)
{
    if (!args[0]) return;
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.filename, this.descr && JSON.stringify(tests.descr) || "", args[1]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.file, this.descr && JSON.stringify(tests.descr) || "");
...
// Set the title and description of the next test, the title will be printed at the beginning of the global test object,
// this is a convenience utility to better document tests
tests.describe = function(...args)
{
    tests.descr = args.join(" ");
}
        ...
        lib.everySeries([
            ...
            function(next2) {
                core.sendRequest(q, (err, rc) => {
                    tests.expect(rc.status == q._rc, `${conf.user?.login || "pub"}: ${q.url}: expect ${q._rc} but got ${rc.status}`, rc.data, "config:", conf);
...
shell.cmdTestRun = function(options)
{
    ...
    for (const cmd of cmds) {
        if (!tests['test_' + cmd]) {
            console.error(logger.prefix("TEST"), "INVALID:", cluster.isMaster ? "master" : "worker", test.run, test.filename, "TEST:", cmd, "CMDS:", tlist.join(", "), "ARGS:", process.argv, "TEST:", test);
            process.exit(1);
        }
    }
    ...
    setTimeout(() => {
        ...
        lib.forEachLimit(cmds, test.concurrency, (cmd, next) => {
            ...
            setTimeout(() => {
                console.log(logger.prefix("TEST"), errcode ? tests.failed : tests.success, cluster.isMaster ? "master" : "worker", test.run, test.filename, tests.descr && JSON.stringify(tests.descr) || "");
...
shell.cmdTestConfig = function(options)
{
    ...
    for (const conf of test.config) {
        ...
        for (let i = 0; i < args.length - 1; i++) {
            if (/^-test-/.test(args[i]) && typeof tests.test[args[i].substr(6)] == "undefined") {
                tests.test[args[i].substr(6)] = args[i + 1][0] != "-" ? args[i + 1] : true;
            }
        }
    }
    return "continue";
