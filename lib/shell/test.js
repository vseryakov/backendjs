...
tests.expect = function(ok, ...args)
{
    ...
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), "FAILED:", cluster.isMaster ? "master" : "worker", this.test.run, this.test.file, args[0]);
        process.exit(1);
    }
    console.error("\n");
...
tests.assert = function(...args)
{
    if (!args[0]) return;
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), "FAILED:", cluster.isMaster ? "master" : "worker", this.test.run, this.test.file, args[1]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), "FAILED:", cluster.isMaster ? "master" : "worker", this.test.run, this.test.file);
...
shell.cmdTestRun = function(options)
{
    ...
    for (const cmd of cmds) {
        if (!tests['test_' + cmd]) {
            console.error(logger.prefix("TEST"), "INVALID:", cmd, cluster.isMaster ? "master" : "worker", test.run, test.file, "CMDS:", tlist.join(", "), "ARGS:", process.argv, "TEST:", test);
            process.exit(1);
        }
    }
    ...
    setTimeout(() => {
        ...
        lib.forEachLimit(cmds, test.concurrency, (cmd, next) => {
            ...
            setTimeout(() => {
                console.log(logger.prefix("TEST"), errcode ? "FAILED:" : "FINISHED:", cluster.isMaster ? "master" : "worker", test.run, test.file);
