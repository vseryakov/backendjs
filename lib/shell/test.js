...
tests.expect = function(ok, ...args)
{
    ...
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker",
                      this.test.cmd || this.test.run, this.test.filename,
                      tests.descr.length && JSON.stringify(tests.descr) || "", args[0]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker",
                  this.test.cmd || this.test.run, this.test.file,
                  tests.descr.length && JSON.stringify(tests.descr) || "");
...
tests.assert = function(...args)
{
    ...
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker",
                      this.test.cmd || this.test.run, this.test.filename,
                      tests.descr.length && JSON.stringify(tests.descr) || "", args[1]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker",
...
{
    console.log(typeof tests.descr)
    tests.descr.push.apply(tests.descr, args);
}
...
shell.cmdTestRun = function(options)
{
    ...
    setTimeout(() => {
        ...
        lib.forEachLimit(cmds, test.concurrency, (cmd, next) => {
            ...
            setTimeout(() => {
                console.log(logger.prefix("TEST"), errcode ? tests.failed : tests.success, cluster.isMaster ? "master" : "worker",
