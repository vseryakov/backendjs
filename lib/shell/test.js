...
shell.cmdTestRun = function(options)
{
    ...
    test.cluster = cmds.length > 1 ? lib.toBool(test.cluster) : test.cluster != 2;
...
shell.testJob = function(options, callback)
{
    ...
    if (options.cancel) {
        interval = setInterval(() => {
            if (core.modules.jobs.isCancelled(options.cancel)) {
                ...
                if (options.file) {
                    fs.writeFileSync(options.file, `${core.modules.ipc.role} ${process.pid} cancelled`);
                }
    ...
    if (options.file) {
        fs.writeFileSync(options.file, `${core.modules.ipc.role} ${process.pid} ${options.data}`);
    }
