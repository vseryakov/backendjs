...
shell.expect = function(ok, ...args)
{
    if (ok) return;
    console.error("\n");
    if (this.test) console.error(logger.prefix("TEST"), "FAILED:", cluster.isMaster ? "master" : "worker", this.test.run, this.test.file);
...
// - -test-config - a config file to load and parse, all test- params will be added to the test object
...
shell.cmdTestRun = function(options)
{
    ...
    if (cluster.isWorker) {
        test.run = process.env.BKJS_TEST_NAME || "none";
    }

    test.run = this.getArg("-test-run", options);

    var tlist = this.listTests();
    var cmds = test.run ? lib.strSplit(test.run) : tlist;
    for (const cmd of cmds) {
        if (!this['test_' + cmd]) {
            console.log(logger.prefix("TEST"), "INVALID:", cluster.isMaster ? "master" : "worker", test.run, test.file, "CMDS:", tlist.join(", "), "ARGS:", process.argv, "TEST:", test);
            process.exit(1);
        }
    }

    setTimeout(() => {
        console.log(logger.prefix("TEST"), "STARTED:", cluster.isMaster ? "master" : "worker", test.run, test.file);
...
{
    var test = this.initTests();
    test.config = this.getArg("-test-config", options);
    if (!test.config) shell.exit("-test-config argument is required");

    var args = lib.readFileSync(test.config, { cfg: 1 });
