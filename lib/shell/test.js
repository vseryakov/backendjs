...
// assert(next, err, ....)
//
//  - callback - is a callback to be called to continue to the next test or fail with an error
...
// Example
//
//          function testGetUser(callback) {
//              db.get("bk_user", { login: "123" }, function(err, row) {
//                  tests.assert(callback, err || !row || row.id != "123", "Record not found or invalid", row)
//              });
//          }
shell.assert = function(callback, err, ...args)
{
    if (typeof callback != "function") throw new Error("invalid assertion");
    if (this.test.forever) return callback();

    if (err) {
        if (!util.isError(err)) Error.captureStackTrace(err, shell.assert);
        err.details = lib.objDescr(args);
    }
    setTimeout(callback.bind(null, err), this.test.delay || this.test.timeout || 0);
    this.test.delay = 0;
}
...
//             db.get("bk_user", { login: "123" }, function(err, row) {
//                 tests.assert(next, err || !row || row.id != "123", "Record not found or invalid", row)
//             });
//          }
//
