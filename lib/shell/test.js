...
{
    if (ok) return;
    this.die("Expectation failed:", args);
}

shell.assert = function(...args)
{
    if (!args[0]) return;
    this.die("Assertion occured:", args);
}

shell.die = function(text, ...args)
{
    console.error("\n");
    if (this.test) console.error(logger.prefix("TEST"), "FAILED:", this.test.cmd, this.test.file);
    console.error(text);
    for (const a of args) {
...
shell.cmdTestRun = function(options)
{
    ...
    setTimeout(() => {
        console.log(logger.prefix("TEST"), "STARTED:", cluster.isMaster ? "master" : "worker", test.cmd, test.file);
        ...
        lib.whilst(
            ...
            function(err) {
                ...
                if (err) {
                    console.error(logger.prefix("TEST"), "FAILED:", test.cmd, test.file);
                    shell.die(err, test);
                }
                setInterval(() => {
                    if (test.running > 0) return;
                    console.log(logger.prefix("TEST"), "FINISHED:", test.cmd, test.file);
                    process.exit(0);
                }, 100);
        });
    }, test.delay, true);
