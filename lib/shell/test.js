...
const tests = {
    ...
    init: function() {
        ...
        Object.defineProperty(global, "modules", { value: core.modules, enumerable: true });
            ...
            function(next2) {
                core.sendRequest(q, (err, rc) => {
                    ...
                    if (typeof conf.match == "function") {
                        tests.expect(conf.match(rc, conf), "match failed", rc.obj, "config:", conf);
                    } else
                    if (conf.match) {
...
shell.cmdTestRun = function(options)
{
    ...
    setTimeout(() => {
        console.log(logger.prefix("TEST"), tests.started, cluster.isMaster ? "master" : "worker", cmds.join(","), test.filename);
        ...
        lib.forEachLimit(cmds, test.concurrency, (cmd, next) => {
            if (cluster.isMaster && cmd.startsWith("master_")) {
                ...
                core.modules.ipc.on("shell:ready", (msg, w) => {
                    try {
                        test.cmd = cmd;
                        tests["test_" + cmd]((err) => {
                            console.log(logger.prefix("TEST"), err ? tests.failure : tests.success, "master", cmd, test.filename, err || "");
                            worker.kill();
                            next(err);
                        }, test);
                    } catch (e) {
                        console.log(logger.prefix("TEST"), tests.failure, "master", cmd, test.filename, e);
                        worker.kill();
                        next(e);
                    }
                });
            ...
            if (cluster.isMaster && test.cluster > 0) {
            ...
            test.cmd = cmd;
            if (cluster.isMaster) {
                console.log(logger.prefix("TEST"), tests.started, "master", cmd, test.filename);
            }
            ...
            try {
                tests["test_" + cmd]((err) => {
                    ...
                    setTimeout(next.bind(null, err), test.interval)
                }, test);

            } catch (e) {
                console.log(logger.prefix("TEST"), tests.failure, cluster.isMaster ? "master" : "worker", cmd, test.filename, e);
                setTimeout(next.bind(null, e), test.interval)
            }
        }, (err) => {
