...
shell.cmdTestRun = function(options)
{
    ...
    // Add all arguments starting with test-
    ...
    for (const cmd of cmds) {
        if (!this['test_' + cmd]) {
            console.log(logger.prefix("TEST"), "INVALID:", cluster.isMaster ? "master" : "worker", test.run, test.file, "ALL:", tlist.join(", "), "ARGS:", process.argv, "TEST:", test);
            process.exit(1);
        }
    }
