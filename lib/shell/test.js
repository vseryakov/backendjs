...
tests.expect = function(ok, ...args)
{
    ...
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.filename, args[0]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.file);
    args.push("", "TEST CONTEXT:", this.test);
    console.error("Expectation failed:");
    if (this.descr) args.unshift(this.descr);
    shell.die(...args);
}
...
tests.assert = function(...args)
{
    if (!args[0]) return;
    if (!this.test.verbose) {
        console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.filename, args[1]);
        process.exit(1);
    }
    console.error("\n");
    console.error(logger.prefix("TEST"), tests.failed, cluster.isMaster ? "master" : "worker", this.test.cmd || this.test.run, this.test.file);
...
{
    this.descr = args.length == 1 ? args[0] : args;
}
...
shell.cmdTestRun = function(options)
{
    ...
    setTimeout(() => {
        ...
        lib.forEachLimit(cmds, test.concurrency, (cmd, next) => {
            ...
            setTimeout(() => {
                console.log(logger.prefix("TEST"), errcode ? tests.failed : tests.success, cluster.isMaster ? "master" : "worker", test.run, test.filename);
