...
shell.help.push("-db-create-tables - create or upgrade tables and exit");
shell.help.push("-db-get-config [-separator =] [-format text] [name VALUE ...] - show all config parameters retrieved from the remote database bk_config table for the current environment, to simulate another environment pass the following arguments as name value pairs: role, network, region, zone, tag");
...
shell.commands.dbCreateTables = function(options)
{
    db.createTables(options, (err) => {
        shell.exit(err)
    });
    return "continue";
}
...
shell.commands.dbGetConfig = function(options)
{
    ...
    db.getConfig(opts, (err, data) => {
        if (fmt === "text") {
            for (let i = 0; i < data.length; i++) console.log(data[i].name + (sep) + (data[i].value || "").replace(/[\r\n]/g, ""));
        } else {
...
shell.commands.dbGet = function(options)
{
    ...
    var sep = shell.getArg("-separator", options, "!");
    var fmt = shell.getArg("-format", options);
    var cols = lib.split(shell.getArg("-cols", options));
...
shell.commands.dbBackup = function(options)
{
    ...
    var tables = lib.split(shell.getArg("-tables", options));
    var skip = lib.split(shell.getArg("-skip", options));
    ...
    var parallel = lib.split(shell.getArg("-parallel-tables", options));
    ...
    lib.forEachLimit(tables, concurrency, (table, next) => {
        ...
        lib.forEach(opts2.parts, (n, next2) => {
            var opts3 = typeof n === "number" ? lib.clone(opts2, { Segment: n, TotalSegments: jobs }) : opts2;
...
shell.commands.dbRestore = function(options)
{
    ...
    var root = shell.getArg("-path", options);
    var filter = shell.getArg("-filter", options);
    var mapping = lib.split(shell.getArg("-mapping", options));
    var table = shell.getArg("-table", options);
    var tables = lib.split(shell.getArg("-tables", options));
    ...
    var progress = shell.getArgInt("-progress", options);
    var concurrency = lib.split(shell.getArg("-concurrency", options, 1));
            ...
            function(next) {
                ...
                lib.forEachLine(file, opts, function(lines, next2) {
                    lines = lines.map((l) => {
                        ...
                        return { op: "put", table, obj: row, options: opts };
                    }).filter((x) => (x));
                    if (progress && opts.nlines % progress === 0) logger.info("cmdDbRestore:", table, opts.nlines, "records");
...
shell.commands.dbSync = function(options)
{
    ...
    lib.forEachLimit(lib.split(opts.tables), opts.concurrency || 1, (table, next) => {
        db.copy(table, {}, opts, (err, _rc) => {
            next(err);
        });
    }, shell.exit);
}
