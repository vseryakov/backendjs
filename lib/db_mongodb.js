...
db.mongodbInitPool = function(options)
{
    ...
    pool.query = function(client, req, opts, callback) {
        ...
        case "upgrade":
            var keys = [];
            var cols = corelib.searchObj(obj, { name: 'primary', sort: 1, flag: 1 });
            var kopts = corelib.mergeObj(opts, { unique: true, background: true });
            ...
            ["", "1", "2", "3", "4", "5"].forEach(function(n) {
                var cols = corelib.searchObj(obj, { name: "unique" + n, sort: 1, flag: 1 });
                var uopts = corelib.mergeObj(opts, { name: Object.keys(cols).join('_'), unique: true, background: true });
                ...
                cols = corelib.searchObj(obj, { name: "index" + n, sort: 1, flag: 1 });
                var iopts = corelib.mergeObj(opts, { name: Object.keys(cols).join('_'), background: true });
            ...
            client.createCollection(table, opts, function(err, item) {
                ...
                corelib.forEachSeries(keys, function(idx, next) {
        ...
        case "search":
            ...
            for (var p in obj) {
                ...
                case "between":
                    var val = corelib.strSplit(obj[p]);
                    if (val.length == 2) {
