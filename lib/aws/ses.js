/*
 *  Author: Vlad Seryakov vseryakov@gmail.com
 *  backendjs 2018
 */
'use strict';

const app = require(__dirname + '/../app');
const lib = require(__dirname + '/../lib');
const aws = require(__dirname + '/../aws');

/**
 * AWS SES (Simple Email Service) API request.
 * @memberof module:aws
 * @method querySES
 * @param {string} action - SES API action, e.g. `SendEmail`, `SendRawEmail`
 * @param {object} obj - API-specific request parameters
 * @param {object} [options] - request options passed to {@link module:aws.queryEndpoint}
 * @param {function} callback - `(err, data, request)`
 */
aws.querySES = function(action, obj, options, callback)
{
    this.queryEndpoint("email", '2010-12-01', action, obj, options, callback);
}

/**
 * Send an email via SES.
 * @memberof module:aws
 * @method sesSendEmail
 * @param {string|string[]} to - recipient(s) for the To: header
 * @param {string} subject - email subject
 * @param {string} body - email body (text or HTML)
 * @param {object} [options]
 * @param {string} [options.from] - From: address, defaults to `app.emailFrom` or `admin@<domain>`
 * @param {string|string[]} [options.cc] - Cc: recipients
 * @param {string|string[]} [options.bcc] - Bcc: recipients
 * @param {string|string[]} [options.replyTo] - Reply-To: addresses
 * @param {string} [options.returnPath] - address to send bounces to
 * @param {string} [options.charset=UTF-8] - charset for subject and body
 * @param {boolean} [options.html] - send the body as MIME HTML
 * @param {string} [options.config] - configuration set name
 * @param {function} callback
 */
aws.sesSendEmail = function(to, subject, body, options, callback)
{
    if (typeof options === "function") callback = options, options = null;
    if (!options) options = lib.empty;

    var params = { "Message.Subject.Data": subject, "Message.Subject.Charset": options.charset || "UTF-8" };
    params["Message.Body." + (options.html ? "Html" : "Text") + ".Data"] = body;
    params["Message.Body." + (options.html ? "Html" : "Text") + ".Charset"] = options.charset || "UTF-8";
    params.Source = options.from || app.emailFrom || ("admin@" + app.domain);
    lib.split(to).forEach((x, i) => { params["Destination.ToAddresses.member." + (i + 1)] = x; })
    if (options.cc) lib.split(options.cc).forEach((x, i) => { params["Destination.CcAddresses.member." + (i + 1)] = x; })
    if (options.bcc) lib.split(options.bcc).forEach((x, i) => { params["Destination.BccAddresses.member." + (i + 1)] = x; })
    if (options.replyTo) lib.split(options.replyTo).forEach((x, i) => { params["ReplyToAddresses.member." + (i + 1)] = x; })
    if (options.returnPath) params.ReturnPath = options.returnPath;
    if (options.config) params.ConfigurationSetName = options.config;
    this.querySES("SendEmail", params, options, callback);
}

/**
 * SES V1 send raw email
 * @param {string} body - base64 encoded raw mail envelope
 * @param {object} [options]
 * @param {string} [options.to] - list of email addresses to use in RCPT TO
 * @param {string} [options.from] - an email to use in from header
 * @param {string} [options.config] - configuration set name
 * @memberof module:aws
 * @method sesSendRawEmail
 */
aws.sesSendRawEmail = function(body, options, callback)
{
    if (typeof options === "function") callback = options, options = null;

    var params = { "RawMessage.Data": body };
    if (options) {
        if (options.from) params.Source = options.from;
        if (options.to) lib.split(options.to).forEach((x, i) => { params["Destinations.member." + (i + 1)] = x; })
        if (options.config) params.ConfigurationSetName = options.config;
    }
    this.querySES("SendRawEmail", params, options, callback);
}

/**
 * SES V2 version
 * Send raw email
 * @param {string} body - base64 encoded raw mail envelope
 * @param {object} [options]
 * @param {string} [options.to] - list of email addresses to use in RCPT TO
 * @param {string} [options.from] - an email to use in from header
 * @param {string} [options.config] - configuration set name
 * @memberof module:aws
 * @method sesSendRawEmail2
 */
aws.sesSendRawEmail2 = function(body, options, callback)
{
    if (typeof options === "function") callback = options, options = null;

    var obj = {
        Content: { Raw: { Data: body } }
    };
    if (options) {
        if (options.from) obj.FromEmailAddress = options.from;
        if (options.to) obj.Destination = { ToAddresses: lib.split(options.to) }
        if (options.config) obj.ConfigurationSetName = options.config;
    }
    this.queryService({ endpoint: "email", service: "ses", path: "/v2/email/outbound-emails" }, obj, options, callback);
}
