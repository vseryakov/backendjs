...
aws.ddbTableMap = {};
aws.ddbOpsMap = { "!=": "<>", "": "=", eq: "=", ne: "<>", lt: "<", le: "<=", gt: ">", ge: ">=" };
...
aws.queryDDB = function(action, obj, options, callback)
{
    ...
    if (/[a-z][a-z]-[a-z]+-[1-9]/.test(options.endpoint)) region = options.endpoint;
        ...
        if (params.status != 200) {
            if (!err) {
                err = lib.newError(params.obj.message || params.obj.Message || ("Error " + params.status + " " + params.data),
                                   params.status,
                                   lib.strSplit(params.obj.__type || params.obj.code, "#").pop());
            }
            err.action = action;
...
aws.queryExpression = function(params, query, options, join)
{
    var req = modules.db.prepareRequest({
        table: params.TableName,
        query,
        options,
        config: {
            opsMap: this.ddbOpsMap,
        }
    });
    var expr = [];
...
{
    var n = 0, list = [];
    lib.strSplit(names).forEach((name) => {
        if (name.includes(".")) {
            if (!params.ExpressionAttributeNames) params.ExpressionAttributeNames = {};
            name = name.split(".").map((x) => {
                for (const n in params.ExpressionAttributeNames) {
 ...
 *    - expected - an object with column names to be used in ConditionExpression clause and value as null to set condition to { Exists: false } or
...
aws.ddbPutItem = function(name, item, options, callback)
{
    ...
    if (options.expected) {
        this.buildExpression(params, "ConditionExpression", options.expected, options);
    }
    if (options.expr) {
...
aws.ddbUpdateItem = function(name, keys, item, options, callback)
{
    ...
    if (typeof item == "object") {
        var c = 0, d = 0, names = {}, values = {}, updateOps = options.updateOps || lib.empty;
...
aws.ddbTransactWriteItems = function(items, options, callback)
{
    ...
    lib.isArray(items, []).forEach(function(x) {
        ...
        case "get":
        case "check":
            obj = { TableName: aws.ddbTable(x.table), Key: {} };
            for (const p in x.obj) {
                obj.Key[p] = aws.toDynamoDB(x.obj[p]);
            }
            if (opts.expected) {
                aws.buildExpression(obj, "ConditionExpression", opts.expected, opts);
            }
            if (opts.expr) {
