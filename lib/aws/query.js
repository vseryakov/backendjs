 ...
 * @param {object} req - service request descriptor
 * @param {string} req.region - must be set for correct resolution
 * @param {string} [req.path] - wil be used if not empty instead of any other endpoint paths
 * @param {object} options
 * @param {string} [options.endpoint] - full endpoint url to use instead of defaults AWS endpoint
 * @param {string} [options.endpoint_protocol] - endpoint protocol, takes precedence over full url proto
 * @param {string} [options.endpoint_host] - endpoint host, takes precedence over full url host
 * @param {string} [options.endpoint_path] - endpoint path, takes precedence over full url path
 * @memberof module:aws
 * @method getServiceEndpoint
 */
aws.getServiceEndpoint = function(req, options)
{
    let url = lib.isString(options?.endpoint) ||
              lib.isString(this.endpoints[req.endpoint + "-" + req.region]) ||
              lib.isString(this.endpoints[req.endpoint]);

    if (url) url = URL.parse(url);

    const protocol = options?.endpoint_protocol || url?.protocol || 'https';
    const host = options?.endpoint_host || url?.host || (req.endpoint + '.' + req.region + '.amazonaws.com');
    const pathname = req.path || options?.endpoint_path || url?.pathanme || '/';
 ...
 */
aws.getServiceOptions = function(req, options)
{
    return {
        method: req?.method || options?.method || "POST",
        query: req?.query || options?.query,
        postdata: req?.postdata,
        headers: req?.headers || Object.create(null),
        quiet: options?.quiet,
        logger_error: req?.logger_error || options?.logger_error,
        ignore_error: req?.ignore_error || options?.ignore_error,
        retryCount: req?.retryCount || options?.retryCount,
        retryTimeout: req?.retryTimeout || options?.retryTimeout,
        retryOnError: req?.retryOnError || options?.retryOnError,
        httpTimeout: req?.httpTimeout || options?.httpTimeout,
        credentials: req?.credentials || options?.credentials,
        signer: req?.signer || this.signer,
        action: req?.action,
        service: req?.service,
...
aws.queryEndpoint = function(endpoint, version, action, obj, options, callback)
{
    ...
    const region = aws.getServiceRegion(endpoint, options?.region || this.region || 'us-east-1');

    obj = Object.assign({ Action: action, Version: version }, obj);
    ...
    for (const p in obj) {
        if (obj[p] !== undefined && obj[p] !== null && obj[p] !== "") {
            body.push(p + "=" + lib.encodeURIComponent(obj[p]));
        }
    }

    const req = { region, action, endpoint, postdata: body.sort().join("&") };
    const uri = url.format(aws.getServiceEndpoint(req, options));
    const opts = aws.getServiceOptions(req, options);

    aws.fetch(uri, opts, (err, rc) => {
 ...
 * @param {Object} [options] - Optional configuration options, (retryTimeout, retryCount, etc., see {@link module:lib.fetch})
 * @param {string} [options.region] - AWS region, overrides library/default region
 * @param {string} [options.endpoint] - custom endpoint for local env or alternatives
...
aws.queryService = function(req, obj, options, callback)
{
    ...
    const region = aws.getServiceRegion(req.endpoint, options?.region || this.region || 'us-east-1');
    ...
    req = Object.assign({ region, headers, postdata: lib.stringify(obj) }, req);
    const uri = url.format(aws.getServiceEndpoint(req, options));
    const opts = aws.getServiceOptions(req, options);
 ...
 * @param {function} callback - sync method to wrap inside a Promise, it runs in aws module context
 * @param {string} action - API specific action
 * @param {object} obj - API-specific request parameters
 * @param {object} [options] - request options passed to {@link module:aws.queryEndpoint} or
 * {@link module:aws.queryService} (region, retryCount, retryTimeout, endpoint...)
 * @returns {Promise(object)} - { err, data, request }
 * @async
 * @memberof module:aws
 * @method aqueryService
 ...
 */
aws.aqueryMethod = function(callback, action, obj, options)
{
    return new Promise((resolve, _reject) => {
        callback.call(aws, action, obj, options, (err, data, request) => {
            resolve({ err, data, request })
        })
    })
}
 ...
 * @memberof module:aws
 * @method queryRdsData
 * @param {string} action - RDS Data API action, e.g. `Execute`
 * @param {object} postdata - API-specific request parameters
 * @param {object} [options] - request options passed to {@link module:aws.queryService}
 * @param {function} callback - `(err, data, request)`
 */
aws.queryRdsData = function(action, postdata, options, callback)
{
    if (typeof options === "function") callback = options, options = null;
    ...
    this.fetch(url, opts, (err, rc) => {
        if (rc.status < 200 || rc.status >= 399) err = aws.parseError(rc);
        rc.logger(err ? rc.logger_error || "error" : "debug", "queryRdsData:", err, "postdata:", rc.postdata, "data:", rc.data);
        if (typeof callback === "function") callback(err, rc.obj, rc);
    });
}
