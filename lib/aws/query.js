...
aws.parseError = function(req)
{
    var err;
    if (req.obj) {
        var error = req.obj.Response?.Errors?.Error;
        if (error?.Message) {
            err = lib.newError({ message: error.Message, code: error.Code, status: req.status });
        } else
 ...
 */
aws.aqueryEndpoint = async function(endpoint, version, action, obj, options)
{
 ...
 */
aws.aqueryService = async function(req, obj, options)
{
    return new Promise((resolve, reject) => {
