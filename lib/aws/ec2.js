...
aws.ec2AfterRunInstances = function(options, callback)
{
    lib.series([
        ...
        function(next) {
            ...
            aws.elbRegisterInstances(options.elbName, options.instances.map((x) => (x.instanceId)), options, next);
        },
        function(next) {
            // Add to the ELB
...
{
    if (obj) {
        obj.name = lib.objGet(obj, "tagSet.item", { list: 1 }).filter((t) => (t.key == "Name")).map((t) => (t.value)).pop();
