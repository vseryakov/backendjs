...
//  - launchTemplate - launch template name to use, latest version
...
aws.ec2RunInstances = function(options, callback)
{
    ...
    var req = {
        ...
        InstanceType: options.instanceType || this.instanceType || "t4g.micro",
    ...
    const subnetId = lib.strSplit(options.subnetId || this.subnetId)[0];
    ...
    if (subnetId) {
        ...
        if (!req["NetworkInterface.0.SubnetId"]) {
    ...
    if (groups.length && req["NetworkInterface.0.DeviceIndex"] === undefined) {
        groups.forEach((x, i) => { req["SecurityGroupId." + i] = x; });
    }

    if (!subnetId && options.availabilityZone) {
