    ...
    if (process.env.AWS_EXECUTION_ENV == "AWS_ECS_FARGATE") {
        lib.series([
            function(next) {
                ...
                aws.getInstanceCredentials(process.env.AWS_CONTAINER_CREDENTIALS_FULL_URI || process.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI, next);
            },
            function(next) {
                aws.getInstanceMeta(`${process.env.ECS_CONTAINER_METADATA_URI_V4}/task`, { noparse: 0 }, (err, rc) => {
                    if (!err && rc) {
                        aws.task = rc;
                        core.instance.zone = rc.AvailabilityZone;
                        core.instance.id = lib.strSplit(rc.TaskARN, "/").pop();
                        core.instance.task = rc.Family;
                        core.instance.service = rc.ServiceName;
                        core.instance.netdev = "eth1";
                            ...
                            if (c.KnownStatus == "RUNNING") {
                                for (const j in c.Networks) {
                                    if (c.Networks[j].NetworkMode == "awsvpc") {
                                        core.instance.image = c.Image;
                                        core.instance.ip = String(c.Networks[j].IPv4Addresses);
                                        break;
                                    }
                                }
                                break;
            ...
            logger.debug('getInstanceInfo:', aws.name, core.instance, err);
...
aws.bkCollectStats = function(options, callback)
{
    ...
    aws.getInstanceMeta(`${process.env.ECS_CONTAINER_METADATA_URI_V4}/stats`, { noparse: 0 }, (err, stats) => {
        logger.debug("awsCollectStats:", err, stats, "ENV:", process.env);
        if (err || !stats?.cpu_stats) return callback(err);

        if (stats.cpu_stats?.cpu_usage && stats.precpu_stats?.cpu_usage) {
            const t = stats.cpu_stats.cpu_usage.total_usage - stats.precpu_stats.cpu_usage.total_usage;
            const s = stats.cpu_stats.system_cpu_usage - stats.precpu_stats.system_cpu_usage;
            const n = stats.cpu_stats.online_cpus;
            if (s > 0 && t > 0 && n > 0) {
                if (s > t) {
                    options.stats.task_cpu_util = lib.toNumber((t / s) * n * 100, { digits: 2 });
                } else {
                    const d = (lib.toMtime(stats.read) - lib.toMtime(stats.preread)) * (10**6);
                    options.stats.task_cpu_util = lib.toNumber((t / d) * 100, { digits: 2 });
                }
            }
        }
        if (stats.memory_stats?.usage && stats.memory_stats.limit) {
            options.stats.task_mem_used = stats.memory_stats?.usage;
            options.stats.task_mem_util = lib.toNumber(stats.memory_stats.usage / stats.memory_stats.limit * 100.0, { digits: 2 })
        }
        if (stats.memory_stats?.stats?.total_rss) {
            options.stats.task_mem_rss = stats.memory_stats.stats.total_rss;
        }
        if (stats.networks && options.eth) {
            var eth = stats.networks[options.eth];
            if (eth) {
                for (const p of ["rx_bytes","rx_packets","rx_errors","rx_dropped","tx_bytes","tx_packets","tx_errors","tx_dropped"]) {
                    options.stats["net_" + p] = eth[p] || 0;
                }
                options.stats.net_rx_rate = stats.network_rate_stats?.rx_bytes_per_sec || 0
                options.stats.net_tx_rate = stats.network_rate_stats?.tx_bytes_per_sec || 0
            }
        }
