    ...
    core.httpGet(`${process.env.ECS_CONTAINER_METADATA_URI_V4}/stats`, {}, (err, rc) => {
        ...
        if (rc?.cpu_stats?.cpu_usage && rc?.precpu_stats?.cpu_usage) {
            ...
            if (sd > 0 && d > 0) {
                ...
                options.stats.host_cpu_util = lib.toNumber(p, { digits: 2 });
            }
        }
        if (rc?.memory_stats?.usage && rc.memory_stats.limit) {
            options.stats.host_mem_used = rc.memory_stats?.usage;
            options.stats.host_mem_util = lib.toNumber(rc.memory_stats.usage / rc.memory_stats.limit * 100.0, { digits: 2 })
        }
        if (rc?.networks?.eth0) {
            for (const p of ["rx_bytes","rx_packets","rx_errors","rx_dropped","tx_bytes","tx_packets","tx_errors","tx_dropped"]) {
                options.stats["net_" + p] = rc.networks.eth0[p] || 0;
            }
            options.stats.net_rx_rate = rc?.network_rate_stats?.rx_bytes_per_sec || 0
            options.stats.net_tx_rate = rc?.network_rate_stats?.tx_bytes_per_sec || 0
        }
