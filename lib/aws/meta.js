...
aws.getInstanceInfo = function(options, callback)
{
    ...
    if (process.env.AWS_EXECUTION_ENV == "AWS_ECS_FARGATE") {
        lib.series([
            ...
            function(next) {
                aws.getInstanceMeta(`${process.env.ECS_CONTAINER_METADATA_URI_V4}/taskWithTags`, { noparse: 0 }, (err, rc) => {
                    if (!err && rc) {
                        aws.task = rc;
...
aws.bkCollectStats = function(options, callback)
{
    ...
    aws.getInstanceMeta(`${process.env.ECS_CONTAINER_METADATA_URI_V4}/task/stats`, { noparse: 0 }, (err, rc) => {
        logger.debug("awsCollectStats:", err, rc?.obj, "ENV:", process.env);
        var stats;
        for (const p in rc?.obj) {
            stats = rc.obj[p];
            options.stats.id = p;
        }
        ...
        if (stats.cpu_stats?.cpu_usage && stats.precpu_stats?.cpu_usage) {
            ...
            options.stats.host_cpu_util = lib.toNumber((t / s) * n * 100, { digits: 2 });
        }
        if (stats.memory_stats?.usage && stats.memory_stats.limit) {
            options.stats.host_mem_used = stats.memory_stats?.usage;
            options.stats.host_mem_util = lib.toNumber(stats.memory_stats.usage / stats.memory_stats.limit * 100.0, { digits: 2 })
        }
        if (stats.networks?.eth0) {
            for (const p of ["rx_bytes","rx_packets","rx_errors","rx_dropped","tx_bytes","tx_packets","tx_errors","tx_dropped"]) {
                options.stats["net_" + p] = stats.networks.eth0[p] || 0;
            }
