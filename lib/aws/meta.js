        ...
        function(next) {
            aws.readCredentialsProfile(`${process.env.HOME || process.env.BKJS_HOME}/.aws/config`, profile, (data) => {
                ...
                lib.readFile(file, { json: 1 }, (err, data) => {
                    ...
                    creds.tokenExpiration = lib.toDate(data?.accessToken?.expiresAt).getTime();
                    logger.debug('readCredentials:', creds.key, creds.accountId);
                    next(err, creds);
                });
            });
        },
...
aws.readConfig = function(callback)
{
    ...
    const file = lib.toTemplate(this.confFile, [app.instance, app]);
    aws.s3GetFile(file, { httpTimeout: 1000 }, (err, rc) => {
        logger.debug("readConfig:", file, "status:", rc.status, "length:", rc.size);
        if (rc.status == 200) {
