...
aws.readCredentialsProfile = function(file, profile, callback)
{
    lib.readFile(file, { list: "\n" }, (_err, lines) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (state === 0) {
                if (key?.[0] === '[' && key.at(-1) === "]" &&
                    (profile === key.slice(1, -1) || profile === key.slice(9, -1))) {
                    state = 1;
                }
            } else
...
/**
 * Read key and secret from the AWS SDK credentials or config file, if no profile is given in the config or command line only the default peofile
 * will be loaded. Supports login sessions created via `aws login`.
 * @param {string} profile
 * @param {function} callback
 * @memberof module:aws
    ...
    lib.parallel([
        function(next) {
            aws.readCredentialsProfile(`${process.env.HOME || process.env.BKJS_HOME}/.aws/credentials`, profile, (data) => {
                ...
                creds.token = data.aws_session_token;
            ...
            aws.readCredentialsProfile(`${process.env.HOME || process.env.BKJS_HOME}/.aws/config`, profile, (data) => {
                ...
                lib.readFile(file, { json: 1 }, (err, cache) => {
                    ...
                    if (expiresAt > Date.now()) {
                        ...
                        logger.debug('readCredentials:', creds.key, creds.accountId, cache?.accessToken?.expiresAt);
                    }
                    next(err, creds);
                });
            });
...
/**
 * Read and apply configuration from an S3 bucket, AWS Secrets Manager and AWS Systems Manager
 * Parameter Store, based on `aws.configS3File`, `aws.configSecrets` and `aws.configParameters`.
 * When `aws.configS3Interval` is set the config is reloaded periodically.
 * @memberof module:aws
 * @method readConfig
 * @param {function} [callback]
 ...
 *
 * @example <caption>To load config periodically setup the interval in the config, readConfig wil be called overriding
 * existing config from all existingh sources
 * </caption>
 *
...
{
    const interval = this.configS3Interval > 0 ? this.configS3Interval * 60000 + lib.randomShort() : 0;
    if (interval > 0) {
        setInterval(this.readConfig.bind(this), interval);
    }

    lib.everyParallel([
        function(next) {
            if (!aws.key || !/^s3:\/\//.test(aws.configS3File)) return next();
            const file = lib.toTemplate(aws.configS3File, [app.env, app]);
            aws.s3GetFile(file, { httpTimeout: 1000 }, (_err, rc) => {
        ...
        function(next) {
            ...
            aws.ssmGetParametersByPath(aws.configParameters, (_err, params) => {
                var argv = [];
        ...
        function(next) {
            ...
            aws.batchGetSecretValue({ filters }, (_err, list) => {
 ...
 * @param {string} path - metadata path, e.g. `/latest/meta-data/instance-id`, or a full URL
 * @param {object} [options] - options merged into the fetch request (httpTimeout, retryCount, noparse...)
 * @param {function} callback - `(err, data)` where data is the metadata value or `""` if not found
 */
aws.getInstanceMeta = function(path, options, callback)
{
    ...
    var opts = Object.assign({
        ...
        retryTimeout: 100,
        errorCount: 0,
        retryOnError() { return this.status >= 400 && this.status !== 404 && this.status !== 529 },
    }, options);
...
aws.getInstanceMetaToken = function(callback)
{
    ...
    lib.fetch(`http://${aws.metaHost}/latest/api/token`, opts, (err, params) => {
        ...
        if (params.status === 200 || params.status >= 500) {
            const timeout = params.status === 200 ? 21000000 : 1000 * aws.metaRetries;
...
/**
 * Retrieve temporary credentials from the EC2/ECS instance profile, store them on `aws` (key/secret/token)
 * and schedule automatic refresh before expiration.
 * @memberof module:aws
 * @method getInstanceCredentials
 * @param {string} [path] - explicit credentials metadata path/URL, otherwise derived from the IAM profile
 * @param {function} [callback]
 */
    ...
    lib.series([
        ...
        function(next) {
            ...
            aws.getInstanceMeta(path, (err, data) => {
                if (!err && data) {
                    const obj = lib.jsonParse(data, { data_type: "obj", logger: "info" });
...
/**
 * Detect the AWS environment and populate {@link module:app.env} and `aws` with instance/task identity,
 * region, credentials, key pair and tags from EC2/ECS metadata.
 * @memberof module:aws
    ...
    lib.series([
        ...
        function(next) {
            aws.getInstanceMeta("/latest/dynamic/instance-identity/document", (err, data) => {
                if (!err && data) {
                    data = lib.jsonParse(data, { data_type: "obj", logger: "error" });
                    app.env.type = "aws";
...
/**
 * Get the current instance details via {@link module:aws.ec2DescribeInstances}, caching them in `aws.instance`.
 * @memberof module:aws
...
/**
 * If running inside ECS, pull the task metadata and populate `aws.task` and {@link module:app.env}
 * (task, service, container, ip...). No-op otherwise.
 * @memberof module:aws
...
aws.getTaskDetails = function(options, callback)
{
    ...
    aws.getInstanceMeta(`${uri}/task`, { noparse: 0 }, (err, rc) => {
        if (!err && rc) {
            aws.task = rc;
            const arn = lib.split(rc.TaskARN, /[:/]/);
            aws.accountId = arn[4];
