...
aws.getInstanceInfo = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            ...
            aws.queryEC2("DescribeInstances", { 'Filter.1.Name': 'instance-id', 'Filter.1.Value.1': core.instance.id }, opts, (err, rc) => {
                if (!err) {
                    aws.instance = lib.objGet(rc, "DescribeInstancesResponse.reservationSet.item.instancesSet.item", { obj: 1 });
                    aws.tags = lib.objGet(aws.instance, "tagSet.item", { list: 1 });
                    core.instance.tag = aws.tags.filter((x) => (x.key == "Name")).map((x) => (x.value)).join(",");
                }
                next();
            });
        },
