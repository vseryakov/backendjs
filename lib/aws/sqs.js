 ...
 * @param {object} messages.id - message id
 * @param {object} messages.body - body contents
 ...
 *  all other types will be converted into strings
 * @param {function} [callback] (err, rc) where rc is `{ Failed: object[], Successful: object[] }`
 * @memberOf module:aws
...
aws.sqsSendMessageBatch = function(url, messages, options, callback)
{
    ...
    for (const msg of lib.isArray(messages, [])) {
        if (!msg?.body || !msg?.id) continue;
        params.Entries.push(_setParams({ Id: msg.id, MessageBody: msg.body }, msg));
    }
