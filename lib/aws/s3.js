...
aws.queryS3 = function(bucket, path, options, callback)
{
    ...
    core.httpGet(uri, options, (err, params) => {
...
aws.s3PutFile = function(path, file, options, callback)
{
    ...
    if (!options.headers['content-type']) options.headers['content-type'] = core.mime.getType(path);
...
aws.s3Proxy = function(res, bucket, file, options, callback)
{
    ...
    var s3req = mod.request(params, (s3res) => {
        if (options.attachment) s3res.headers["Content-Disposition"] = "attachment; filename=" + path.basename(file);
