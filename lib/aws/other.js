 ...
 *
 */
aws.rdsDataExecute = function(options, callback)
{
    const postdata = {
    ...
    this.queryRdsData("Execute", postdata, options, (err, obj, rc) => {
        if (!err && options.convertResult && obj?.records?.length) {
            const rows = [];
            for (const row of obj.records) {
                rows.push(obj.columnMetadata.reduce((a, b, i) => {
