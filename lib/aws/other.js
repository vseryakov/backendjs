...
aws.batchGetSecretValue = function(options, callback)
{
    ...
    var Filters = options.Filters || options.filters?.map(x => ({ Key: x.key || "all", Values: x.values || [x] }));
    ...
    lib.doWhilst(
        function(next) {
            aws.querySecrets("BatchGetSecretValue", { Filters, SecretIdList, NextToken: token }, (err, rc) => {
                if (err) return next(err);
                token = rc.NextToken;
                for (const i in rc.SecretValues) {
                    const v = rc.SecretValues[i];
                    v.value = v.SecretString || v.SecretBinary;
                    list.push(v);
                }
                next();
            });
