 ...
 * real Lambda handler. The function context `this` must point to real Lambda handler.
...
aws.lambdaEventsProxySQS = async function(event, callback)
{
    ...
    try {
        const rc = await handler(sqsEvent, mockContext);
...
aws.lambdaEventsProxyAPIGateway = async function(event, callback)
{
    const handler = lib.isFunc(this, lib.isFunc(this?.handler));
    if (!handler) return callback({ status: 500, message: "no handler" });
