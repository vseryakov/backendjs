...
mod.create = async function(options)
{
    ...
    if (options.file || options.data) {
        ...
        if (width || height) {
            ...
            innerImage = await mod.sharp(await innerImage.toBuffer());
        }
    ...
    if (!images.length && !border && !(padding.top + padding.left + padding.right + padding.bottom)) {
        logger.debug("create:", mod.name, "size:", options.id, width, height, "CW:", cw, ch, "B:", border, "I:", images.length, "P:", padding);
    ...
    return outterImage.composite(images).png();
}
