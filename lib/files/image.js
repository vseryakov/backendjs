...
mod.rgb2hex = function(rgb, gg, bb)
{
    const [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];
    return "#" + ((r << 16) | (g << 8) | b).toString(16).padStart(6, '0').toUpperCase();
}

mod.rgb2lum = function(rgb, gg, bb)
{
    const [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];
...
mod.rgb2rlum = function(rgb, gg, bb)
{
    const [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];
...
mod.rgb2hsl = function(rgb, gg, bb)
{
    let [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];
...
mod.hsl2rgb = function(hsl, ss, ll)
{
    var [h, s, l] = Array.isArray(hsl) ? hsl : hsl?.h ? [hsl.h, hsl.s, hsl.l] : [hsl, ss, ll];
...
mod.rgb2hsv = function(rgb, gg, bb)
{
    let [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];
...
mod.hsv2rgb = function(hsv, ss, vv)
{
    let [h, s, v] = Array.isArray(hsv) ? hsv : hsv?.h ? [hsv.h, hsv.s, hsv.v] : [hsv, ss, vv];
...
mod.rgb2compl = function(rgb, gg, bb)
{
    const { h, s, l } = mod.rgb2hsl(rgb, gg, bb);
    return mod.hsl2rgb([(h + 180) % 360, s, l]);
}

mod.rgb2yiq = function(rgb, gg, bb)
{
    const [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];
    return {
...
mod.rgb2lighter = function(rgb, gg, bb)
{
    const [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];

}

mod.rgb2darker = function(rgb, gg, bb)
{
    const [r, g, b] = Array.isArray(rgb) ? rgb : rgb?.r ? [rgb.r, rgb.g, rgb.b] : [rgb, gg, bb];

}

mod.padding = function(options)
{
    return {
 ...
 * @method stats
 */
mod.stats = async function(image, region)
{
    ...
    logger.debug("stats:", mod.name, region, "stats:", stats);
    return stats;
}
...
mod.create = async function(options)
{
    ...
    const padding = mod.padding(options);
 ...
 * @method mergeOptions
 */
mod.mergeOptions = function(options, defaults)
{
 ...
 * @async
 */
mod.detect = async function(options, bgimage)
{
    const image = await mod.create(options);
    const meta = await image.metadata();

    const bgmeta = await bgimage.metadata();
    const region = mod.gravityRegion(options.gravity, bgmeta.width, bgmeta.height);
    ...
    const stats = await mod.stats(bgimage, region);
    let c = stats.dominant;

    switch (options.text_auto) {
    case "complement":
        stats.color = mod.rgb2hex(mod.rgb2compl({ r: 255 - c.r, g: 255 - c.g, b: 255 - c.b }));
        break;

    case "yiq":
        stats.color = mod.rgb2yiq(c).l > 128 ? '#333' : '#FAFAFA';
        break;

    case "wcag":
        stats.color = mod.rgb2rlum(c) > 0.179 ? '#333' : '#FAFAFA';
        break;

    case "mean":
        c = stats.mean;
    default:
        stats.color = mod.rgb2hex({ r: 255 - c.r, g: 255 - c.g, b: 255 - c.b });
    }

    logger.debug("detect:", options, stats);
 ...
 * @async
 */
mod.composite = async function(items, defaults)
{
    logger.debug("composite:", mod.name, items, defaults);
    ...
    await Promise.all(items.
        map(async (item) => {
            mod.mergeOptions(item, defaults);

            // Autodetect text color for texts
            if (item.text && !item.color) {
                const stats = await mod.detect(item, bgimage);
