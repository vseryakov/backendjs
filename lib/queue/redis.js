...
class RedisQueueClient extends QueueClient {
    ...
    _monitor(options) {
        ...
        var chan = this.channel(options);
        this.client.eval(scripts.monitor, 2, chan, Date.now(), (err, count) => {
    ...
    poll(options) {
        ...
        this.client.eval(script, 2, chan, Date.now(), (err, rc) => {
            ...
            const vtimeout = msg.visibilityTimeout > 0 ? msg.visibilityTimeout : visibilityTimeout;
            if (vtimeout && !msg.noWait) {
                Object.defineProperty(msg, "__msgid", { enumerable: false, value: data });
                if (msg.visibilityTimeout > 0) {
                    this.client.zadd(chan + "#", Date.now() + vtimeout, data);
                }
                vtimer = setInterval(() => {
                    if (done || !this.client) {
                    ...
                    this.client.zadd(chan + "#", Date.now() + vtimeout, data, (err) => {
                        if (err) logger.error("poll:", this.name, chan, lib.traceError(err));
            ...
            if (!this.emit(chan, msg, (err, next) => {
                ...
                if (!msg.noVisibility && (err && err.status >= 500 || msg.noWait)) {
                    const rtimeout = lib.toNumber(msg.retryVisibilityTimeout?.[err.status]);
                    if (rtimeout > 0) {
                        this.client.zadd(chan + "#", Date.now() + rtimeout, data, (err) => {
                            if (err) logger.error("poll:", this.name, chan, lib.traceError(err));
    ...
    drop(msg, options, callback) {
        ...
        var chan = this.channel(options);
        this.client.zrem(chan + "#", msg.__queueMessageId, callback);
    }

    purge(options, callback) {
        var chan = this.channel(options);
        this.client.del(chan);
        this.client.del(chan + "#");
    }
