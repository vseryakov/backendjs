...
class RedisClient extends QueueClient {
    ...
    stats(options, callback) {
        ...
        this.client.eval(redisClient.scripts.stats, 1, chan, (err, count) => {
            if (!err) {
    ...
    _monitor(options) {
        ...
        this.client.eval(redisClient.scripts.monitor, 2, chan, now, (err, count) => {
    ...
    poll(options) {
        ...
        var script = visibilityTimeout ? redisClient.scripts.vpoller : redisClient.scripts.poller;
        var chan = this.channel(options);
