...
const redis = require(__dirname + "/../../dist/redis");
...
class RedisQueueClient extends QueueClient {
    ...
    constructor(options) {
        ...
        this.options.retry_strategy = (options) => {
            logger.logger(options.attempt === 2 ? "error": "dev", this.name, "connect:", options);
    ...
    close() {
        ...
        this.options.retry_strategy = undefined;
    }
    ...
    connect(hostname, port) {
        ...
        logger.debug("connect:", this.name, host, port, this.options);
        return client;
    }
    ...
    _poll_update(options, item, visibilityTimeout, callback) {
        const subject = this.subject(options);
        logger.dev("_poll_update:", this.name, subject, visibilityTimeout, item)
        this.client.zadd(subject, Date.now() + visibilityTimeout, item.data, callback);
    }

    _poll_del(options, item, callback) {
        const subject = this.subject(options);
        logger.dev("_poll_del:", this.name, subject, item)
