...
class QueueClient extends EventEmitter {
    ...
    unsubscribe(subject, _options, callback) {
    ...
    // Publish an event
    publish(_subject, _msg, _options, callback) {
        lib.tryCall(callback);
    }
    ...
    schedule(options) {
        ...
        if (this.options.maxPollRate > 0) {
            if (!this._tokenBucket?.equal(this.options.maxPollRate)) {
     ...
     */
    _poll_get(_options, _callback) {}

    /**
     * Update visibilityTimeout for the item in ms
     */
    _poll_update(_options, _item, _visibilityTimeout, _callback) {}


    /**
     * Delete an item from the queue by id
     */
    _poll_del(_options, _item, _callback) {}
    ...
    _poll_emit(subject, msg, callback) {
        try {
            ...
            logger.error("_poll_emit:", this.name, subject, e, msg);
            callback(e);
        }
    }
    ...
    _poll_run_item(options, item, callback) {
        ...
        if (lib.isString(item.data)) {
            ...
            msg = lib.jsonParse(item.data, { data_type: "obj", logger: "error", id: item.id, sub });
        } else {
            msg = lib.isObject(item.data);
        }

        logger.debug("_poll_run_item:", this.name, sub, "MSG:", msg, "ITEM:", item);
        if (!msg) {
        ...
        if (msg.endTime > 0 && msg.endTime < Date.now()) {
            logger.info("_poll_run_item:", this.name, sub, "expired:", item);
            return this._poll_del(options, item, callback)
        }
        ...
        if (msg.startTime > 0 && msg.startTime - Date.now() > this.options.pollInterval) {
            ...
            logger.info("_poll_run_item:", this.name, sub, timeout, "scheduled:", item);
            return this._poll_update(options, item, timeout, callback)
        }

        // Delete immediately or after delay, this is a one-off message not to be handled or repeated
        if (msg.noWait) {
            if (msg.noWait > 1) {
                setTimeout(() => {
        ...
        if (!msg.noVisibilityTimeout) {
            const visibilityTimeout = msg.visibilityTimeout > 0 ? msg.visibilityTimeout : this.options.visibilityTimeout;
            ...
            if (visibilityTimeout > 0) {
                timer = setInterval(() => {
                    ...
                    this._poll_update(options, item, visibilityTimeout, (err) => {
                        logger.debug("_poll_run_item:", this.name, sub, "keepalive:", item);
                        if (err) clearInterval(timer);
                    });
                }, visibilityTimeout * 0.8);
            }
        }
        ...
        if (!this._poll_emit(sub, msg, (err) => {
            ...
            logger.debug("_poll_run_item:", this.name, sub, err, item);

            if (msg.noWait) return callback();
            ...
            if (!msg.noRetryVisibilityTimeout && err?.status >= 500) {
            ...
            logger.debug("_poll_run_item:", this.name, sub, "skipped:", item);
            done = 1;
            clearInterval(timer);
            callback();
        }
