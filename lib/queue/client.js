...
class QueueClient extends EventEmitter {
    ...
    applyOptions(options) {
        ...
        this.options.count = lib.toNumber(this.options.count, { dflt: 0, min: 1 });
     ...
     * maintainence work the requires. Automatically enabled if `_monitor` method is defined. Interval is defined by
     * `monitorInterval` or `visibilityTimeout`.
     */
    monitor(options) {
        if (typeof this._monitor != "function") return;
        var timeout = lib.validPositive(options.monitorInterval, this.options.monitorInterval,
                                       options.visibilityTimeout, this.options.visibilityTimeout);
        if (!this._monitorTimer && timeout) {
            this._monitorTimer = setInterval(this._monitor.bind(this, options), timeout);
    ...
    _poll_run(options) {
        ...
        this._poll_get(options, (err, items) => {
            ...
            lib.forEvery(items, (item, next) => {
                ...
                Object.defineProperty(msg, "__itemId", { enumerable: false, value: item.id });
                processed++;
