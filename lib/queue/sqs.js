...
class SQSQueueClient extends QueueClient {
    ...
    stats(_options, callback) {
        aws.querySQS("GetQueueAttributes", { QueueUrl: this.url, AttributeNames: ["All"] }, this.options, (err, rc) => {
            if (!err) {
                rc = Object.assign(Object.create(null), rc?.Attributes);
    ...
    submit(job, options, callback) {
        ...
        if (/\.fifo$/.test(this.url)) {
            ...
            opts.groupId = this.group(options) || options.uniqueKey || this.queueName;
            opts.deduplicationId = options.deduplicationId;
        }
        if (typeof job !== "string") {
            job = lib.stringify(job);
        }
        if (/[^\x09\x0A\x0D\x20-\uD7FF\uE000-\uFFFD\u{10000}-\u{10FFFF}]/u.test(job)) {
            job = Buffer.from(job).toString("base64");
        }
        aws.sqsSendMessage(this.url, job, opts, callback);
    }
    ...
    _poll_update(_options, item, visibilityTimeout, callback) {
        logger.dev("_poll_update:", this.name, visibilityTimeout, item)
        const req = {
            QueueUrl: this.url,
    ...
    _poll_del(_options, item, callback) {
        logger.dev("_poll_del:", this.name, item)
        const req = {
            QueueUrl: this.url,
            ReceiptHandle: item.id
        };
