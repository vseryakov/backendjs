...
class DBQueueClient extends QueueClient {
    ...
    async poll(options) {
        ...
        lib.forEvery(data, async (row, next) => {
            ...
            const vopts = { pool, query: { status: "new" }, logger_error: "debug" };
            const { err, info } = await db.aupdate("bk_queue", { id: row.id, status: "running", vtime }, vopts);
            ...
            if (visibilityTimeout) {
                ...
                if (msg.endTime > 0 && msg.endTime < now) {
                    logger.info("poll:", this.name, name, "expired", row);
            ...
            if (!this.emit(name, msg, async (err) => {
                ...
                logger.debug("poll:", this.name, name, err, row);
                ...
                if (!msg.noVisibility && (err?.status >= 500 || msg.noWait)) {
                    vtime = lib.toNumber(msg.retryVisibilityTimeout?.[err?.status]);
                    if (err && vtime > 0) {
                        return db.update("bk_queue", { id: row.id, status: "new", vtime }, { pool }, next);
                    }
                    return next();
