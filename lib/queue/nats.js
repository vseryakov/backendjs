...
class NatsClient extends Client {
    ...
    applyOptions(options) {
        ...
        this.options.name = this.options.name && lib.toTemplate(this.options.name, [this.options, core]) || `${app.instance.id}-${app.instance.pid}`;
    ...
    listen(options, callback) {
        ...
        if (this._subs[sub]) {
            logger.debug("listen:", mod.name, sub, "reusing:", options);
        ...
        p.then((s) => {
            ...
            logger.debug("listen:", mod.name, sub, opts);
        }).catch((err) => {
            logger.error("listen:", mod.name, sub, opts, err);
        });
    }
