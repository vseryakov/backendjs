...
const nats = require(__dirname + "/../../dist/nats");
...
class NatsQueueClient extends QueueClient {
    ...
    subscribe(subject, options, callback) {
        ...
        const opts = {
            ...
            callback: (err, msg) => {
                ...
                if (options.raw) {
                    data = { subject: msg.subject, data, sid: msg.sid, headers: msg.headers };
                }
                if (msg.reply) {
    ...
    _poll_update(_options, item, _visibilityTimeout, callback) {
        logger.dev("_poll_update:", this.name, _visibilityTimeout, item)
