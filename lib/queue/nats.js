...
class NatsQueueClient extends QueueClient {
    ...
    poll(options) {
        ...
        if (this._running.size < count) {
            this._subs[sub].pull({
                batch: count - this._running.size,
        ...
        if (!raw) {
            ...
            if (!ackNone) {
                ...
                if (msg.noWaitTimeout > 0) {
                    ...
                    const vtimeout = msg.visibilityTimeout > 0 ? msg.visibilityTimeout :
                                           lib.validPositive(options.visibilityTimeout, this.options.visibilityTimeout);
                    if (vtimeout) {
