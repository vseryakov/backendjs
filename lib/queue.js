 ...
 *
 * The class {@link module:queue.QueueClient} defines the methods that a driver can implement.
 *
 ...
 *
 * The url query parameters that start with **bk-** will be extracted from the url and placed in the class's **options* object,
 * this is a way to pass special properties without using **queue-options**, the rest of the url parameters will be passed to the driver.
 *
 * Job queues are designed by using SQS visibility timeout logic.
 *
 * @param {object} options
 * @param {int} [options.queueCount] config property specifies how many messages to process at the same time, default is 1.
 * Not all drivers support multiple messages.
 *
 * @param {int} [options.pollInterval] config property defines in ms how often to check for new messages,
 * i.e. after messages processed it can poll immediately or after this amount of time, default is 1000 milliseconds.
 *
 ...
 *  The scheduling is implemented using `visibilityTimeout` feature, keep scheduled messages hidden until the actual time.
 *
 * @param {int} [options.maxVisibilityTimeout] which defines in milliseconds the max time a messsage can stay invisible while waiting
 ...
 * @method shutdown
 */
queue.shutdown = function(_options, callback)
{
...
queue.createClient = function(options)
{
    var client = null;
    try {
        const type = lib.split(options?.url, ":")[0];
        if (!type) return;
 ...
 * use queueName to specify a specific driver. Ignores subject (@) and group(#)  if present.
 ...
 * @memberof module:queue
 */
queue.astats = function(options)
{
    return new Promise((resolve, _reject) => {
        queue.stats(options, (err, data) => {
            resolve({ err, data });
        });
    });
}
 ...
 * @method subscribe
 */
queue.subscribe = function(subject, options, callback)
{
    if (typeof options === "function") callback = options, options = null;
    logger.dev("queue.subscribe:", subject, options);
    try {
        this.getClient(options).subscribe(subject, options || {}, typeof callback === "function" ? callback : undefined);
    } catch (e) {
        logger.error('queue.subscribe:', subject, options, e.stack);
    }
}
 ...
 * @method unsubscribe
 */
queue.unsubscribe = function(subject, options, callback)
{
    if (typeof options === "function") callback = options, options = null;
    logger.dev("queue.unsubscribe:", subject, options);
    try {
        this.getClient(options).unsubscribe(subject, options || {}, typeof callback === "function" ? callback : undefined);
    } catch (e) {
        logger.error('queue.unsubscribe:', subject, e.stack);
    }
}

/**
 * Publish an event with the subject to be delivered to all subscribers. If the msg is not a string it will be stringified.
 ...
 */
queue.publish = function(subject, msg, options, callback)
{
    if (typeof options === "function") callback = options, options = null;
    logger.dev("queue.publish:", subject, options);
    try {
        if (typeof msg !== "string") msg = lib.stringify(msg);
        const client = this.getClient(options);
        const _timer = client.metrics.start();
        client.publish(subject, msg, options || {}, (err, val) => {
            _timer.end();
            if (typeof callback === "function") callback(err, val);
        });
    } catch (e) {
        logger.error('queue.publish:', subject, e.stack);
        if (typeof callback === "function") callback(e);
    }
}
 ...
 * @async
 */
queue.apublish = function(subject, msg, options)
{
    return new Promise((resolve, _reject) => {
        queue.publish(subject, msg, options, (err, data) => {
            resolve({ err, data });
        });
    });
}
 ...
 * - For job processing at least one delivery is expected, the {@link module:jobs} uses this.
 ...
 * @method purge
 */
queue.purge = function(options, callback)
{
    logger.dev("queue.purge:", options);
    try {
        this.getClient(options).purge(options || {}, callback);
    } catch (e) {
        logger.error('queue.purge:', e.stack);
    }
}
 ...
 * @async
 */
queue.apurge = function(options)
{
    return new Promise((resolve, _reject) => {
        queue.purge(options, (err, data) => {
            resolve({ err, data });
        });
    });
}
