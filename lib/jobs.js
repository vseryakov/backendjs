        ...
        ipc.listen({ queueName: name }, function(msg, next) {
            ...
            core.runMethods("configureJob", opts, function() {
                jobs.runJob(opts.message, { queueName: name }, function(err) {
                    if (!err) logger.debug("endJob:", name, opts.message);
                    if (err) logger.logger(err.status == 600 ? err.logger || "warn" : err.status != 200 ? "error" : "info", "endJob:", name, lib.traceError(err), lib.objDescr(opts.message));
...
jobs.isJob = function(jobspec)
{
    ...
    if (lib.isObject(jobspec.job)) {
        if (!Object.keys(jobspec.job).every(function(y) { return jobs.jobRx.test(y) })) return lib.newError('invalid job: ' + lib.objDescr(jobspec));
    } else
...
jobs.submitJob = function(jobspec, options, callback)
{
    ...
    if (this.globalQueue) options = lib.objClone(options, "queueName", this.globalQueue);
...
jobs._finishTask = function(err, name, jobspec, options, callback)
{
    if (err && err.status != 200) {
        logger.error('endTask:', options.queueName, name, lib.traceError(err), lib.objDescr(jobspec, { length: 512, count: 10 }));
    } else {
