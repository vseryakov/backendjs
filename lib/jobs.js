...
{
    return this.jobs.reduce((m, x) => (Math.max(m, x.maxRuntime)), 0);
}
...
jobs.checkTimes = function()
{
    ...
    if (this.running.length) {
        ...
        var jobTime = this.jobs.reduce((m, x) => (Math.min(m, x.jobTime)), 0);
        var runTime = Math.max(now - jobTime, now - this.runTime);
