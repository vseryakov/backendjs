...
mod.initWorker = function(options, callback)
{
    ...
    ipc.on('jobs:cancel', function(msg, worker) {
        ...
        mod.cancelled[mod._taskName(msg.name, msg.tag)] = 1;
...
mod.isCancelled = function(name, tag)
{
    return this.cancelled[mod._taskName(name, tag)] || this.cancelled[name];
}
...
mod.cancelTask = function(name, options)
{
    ...
    for (var i in workers) {
        ...
        if (cluster.workers[workers[i]]) {
            cluster.workers[workers[i]].send(ipc.newMsg("jobs:cancel", { name: name, tag: options?.tag || "" }));
        }
    }
}
...
// Send a cancellation request to the given `job` with optional `tag`
mod.cancelJob = function(options, callback)
{
    var msg = ipc.newMsg("jobs:cancel", { name: options.job, tag: options.tag });
