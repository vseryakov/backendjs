            ...
            ipc.listen({ queueName: name }, function(msg, next) {
                ...
                core.runMethods("configureJob", opts, function() {
                    self.runJob(opts.message, { queueName: name }, function(err) {
                        logger[err && err.status == 600 ? "warn" : err ? "error" : "info"]("runJob:", "finished", name, lib.traceError(err), lib.objDescr(msg));
...
//
// if 'jobspec.unique` is greater than zero it defines number of milliseconds for this job to stay in the queue or run,
// it creates a global lock using the job object as the hash key, no other job can be run until the ttl expires or the job finished.
//
...
jobs.runJob = function(jobspec, options, callback)
{
    ...
    lib.series([
      function(next) {
          ...
          var ttl = lib.toNumber(jobspec.unique);
          if (ttl <= 0) return next();
          jobspec.uniqueKey = "JOB:" + lib.hash(lib.stringify(jobspec.job));
          ipc.lock(jobspec.uniqueKey, { ttl: ttl, queueName: self.uniqueQueue }, function(err, locked) {
              if (!err && !locked) err = { status: 600, message: "another job is still running" };
              next(err);
          });
      },
      function(next) {
          self._runJob(jobspec, options, function(err) {
