...
mod.initServer = function(options, callback)
{
    ...
    for (const name of this.workerQueue) {
        queue.monitor(lib.objExtend(mod.workerOptions[name], { queueName: name }));
    }
...
mod.runJob = function(jobspec, options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            cache.lock("JOB:" + key, { ttl: ttl, cacheName: mod.uniqueCache }, (err, locked) => {
                // If the queue service is down keep all messages in the queue until it is up again
                if (!locked) {
                    if (!err && jobspec.uniqueDrop) {
                        logger.logger(jobspec.uniqueLogger || mod.uniqueLogger || "info", "runJob:", "dropped", q, jobspec);
                ...
                if (!err && !jobspec.uniqueOnce) {
                    ...
                    timer = setInterval(function() {
                        cache.lock("JOB:" + key, { ttl: ttl, cacheName: mod.uniqueCache, set: 1 });
                    }, Math.max(ttl * 0.7, 1000));
                }
                logger.debug("runJob:", q, cache.getCache(mod.uniqueCache).name, "locked:", locked, "ttl:", ttl, "key:", key, "JOB:", jobspec)
                next(err);
            });
        },
        function(next) {
            ...
            if (cluster.isWorker) process.title = `${app.id}: worker ${mod.getRunningJobs()}`;
            ...
            lib.forEvery(Object.keys(jobspec.job), (task, next2) => {
                ...
                if (ttl && key && !jobspec.uniqueKeep) {
                    cache.unlock("JOB:" + key, { cacheName: mod.uniqueCache });
                }
...
function _runTask(name, jobspec, options, callback)
{
    ...
    if (mod.taskIgnore && mod.taskIgnore.test(name)) {
        logger.error("runTask:", options?.queueName, name, "task ignored", job, "RX:", mod.taskIgnore);
...
function _finishTask(err, name, jobspec, options, callback)
{
    ...
    mod.metrics.que.update(Date.now() - jobspec.jobTime);
    mod.metrics.running--;
    mod.runTime = Date.now();
...
// Jobs run time stats
mod.configureCollectStats = function(options)
{
