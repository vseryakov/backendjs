...
const mod = {
    ...
    args: [
        ...
        { name: "exit-on-empty", type: "bool", descr: "Exit the jobs server process after last worker terminated in no restart mode" },
...
{
    logger.log("shutdownWorker:", mod.name, "queue:", this.workerQueue, "max-runtime:", this.maxRuntime, "max-lifetime:", this.maxLifetime, options);
    ...
    var timer = setInterval(() => {
        if (mod.runningTasks.length && Date.now() - mod.runTime < mod.shutdownTimeout * 0.9) return;
        clearInterval(timer);
        callback();
    }, mod.shutdownTimeout/10);
}
...
mod.initServer = function(options, callback)
{
    ...
    cluster.on("exit", function(worker, code, signal) {
        ...
        if (!core.exiting && !mod.noRestart) {
            lib.checkRespawn(() => { cluster.fork() });
        }
        if (mod.noRestart && mod.exitOnEmpty && !Object.keys(cluster.workers).length) {
...
mod.initWorker = function(options, callback)
{
    ...
    setTimeout(function() {
        mod.subscribeWorker();
        logger.log("initWorker:", mod.name, "started", core.role, cluster.isWorker ? cluster.worker.id : process.pid, "queue:", mod.subscribed, "maxRuntime:", mod.maxRuntime, "maxLifetime:", mod.maxLifetime);
