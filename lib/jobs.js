...
//
// `jobspec.uniqueOnce` defines than no need to keep alive the visibility timeout
//
            ...
            ipc.lock("JOB:" + jobspec.uniqueKey, { ttl: ttl, queueName: jobs.uniqueQueue }, function(err, locked) {
                ...
                if (!locked) {
                    if (!err && jobspec.uniqueDrop) {
                        logger.logger(jobspec.uniqueLogger || "info", "runJob:", "dropped", options.queueName, jobs.descrJob(jobspec));
