...
var jobs = {
    ...
    args: [{ name: "workers", type: "number", min: -1, max: 32, descr: "How many worker processes to launch to process the job queue, -1 disables jobs, 0 means launch as many as the CPUs available" },
           ...
           { name: "unique-unlock", type: "int", descr: "Timeout in ms when to clear unique job lock after finished" },
    ],
...
jobs.subscribeWorker = function()
{
    ...
    this.workerQueue.forEach(function(name) {
        ...
        if (q.queueName != name) return;
        if (jobs.subscribed.indexOf(q.queueName) > -1) return;
        jobs.subscribed.push(q.queueName);

        logger.info("subscribeWorker:", name, q.name, q.hostname, q.pathname);
...
{
    var idx = this.subscribed.indexOf(name);
    if (idx == -1) return;
    this.subscribed.splice(idx, 1);
    ipc.unlisten({ queueName: name });
    const q = ipc.getQueue(name);
    logger.info("unsubscribeQueue:", name, q.name, q.hostname, q.pathname);
}
...
{
    return this.jobs.reduce((m, x) => (Math.max(m, x.maxRuntime || 0)), this.maxRuntime);
}
...
jobs.checkTimes = function()
{
    ...
    if (this.running.length) {
        ...
        var jobTime = this.jobs.reduce((m, x) => (Math.min(m, x.jobTime || now)), now) - now;
        var runTime = Math.max(now - this.runTime, jobTime);
        if (runTime > maxRuntime * 1000) {
            logger.warn('checkLifetime:', 'jobs: exceeded max run time', maxRuntime, runTime, this.jobs);
...
{
    if (typeof jobspec == "string" && this.jobRx.test(jobspec)) jobspec = { job: { [jobspec]: null } };
    if (!lib.isObject(jobspec)) return this._badJob(jobspec);

    if (typeof jobspec.job == "string") jobspec.job = { [jobspec.job]: null };
...
//
// `jobspec.uniqueUnlock` timeout in ms when to clear the lock after finished to avoid race conditions
//
...
//
// `jobspec.visibilityTimeout` custom tieout for how long to keep this job invisible, overrides default timeout
//
...
jobs.submitJob = function(jobspec, options, callback)
{
    ...
    var q = ipc.getQueue(this.globalQueue || options && options.queueName);
...
jobs.runJob = function(jobspec, options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            if (!ttl) return next();
            key = jobspec.uniqueKey = jobspec.uniqueKey || lib.hash(lib.stringify(jobspec.job));
        ...
        function(next) {
            ...
            jobs._runJob(jobspec, options, function(err) {
                ...
                if (ttl && key && !jobspec.uniqueKeep) {
                    // Delay the unlock to avoid race conditions with visibility timeout
                    setTimeout(function() {
                        ipc.unlock("JOB:" + key, { queueName: jobs.uniqueQueue })
                    }, jobspec.uniqueUnlock || jobs.uniqueUnlock);
                }
...
jobs.runTask = function(name, jobspec, options, callback)
{
    ...
    d.run(function() {
        ...
        ipc.emitMsg("jobs:task:started", { name: name, job: jobspec });
...
jobs._finishTask = function(err, name, jobspec, options, callback)
{
    ...
    ipc.emitMsg("jobs:task:stopped", { name: name, job: jobspec });
