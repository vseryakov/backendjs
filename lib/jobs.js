...
const mod = {
    ...
    args: [
        ...
        { name: "worker-options-(.+)", obj: "workerOptions", make: "$1", type: "json", descr: "Custom parameters by queue name, passed to `ipc.subscribeQueue` on worker start, useful with channels, ex: `-jobs-worker-options-nats#events {\"count\":10}`" },
        { name: "no-restart", type: "bool", descr: "Do not restart workers" },
        { name: "exit-on-empty", type: "int", descr: "Duration in ms to exit the jobs server process after last worker terminated" },
...
{
    if (options.noJobs) return callback();
...
{
    if (options.noJobs || this.workers < 0) return callback();
...
mod.shutdownWorker = function(options, callback)
{
    ...
    for (const q of this.workerQueue) ipc.unsubscribeQueue({ queueName: q });

    // Give some time for a running job to exit by itself
    var timer = setInterval(() => {
        if (this.runningTasks.length && Date.now() - this.runTime < this.shutdownTimeout * 0.9) return;
        clearInterval(timer);
        callback();
...
mod.exitWorker = function(options)
{
    ...
    setTimeout(() => { process.exit(99) }, this.shutdownTimeout * 2);
    core.runMethods("shutdownWorker", options, () => { process.exit(0) });
}
...
mod.initServer = function(options, callback)
{
    ...
    for (const name of this.workerQueue) {
        ipc.monitorQueue(lib.objExtend(mod.workerOptions[name], { queueName: name }));
    }

    // Restart if any worker dies, keep the worker pool alive
    cluster.on("exit", (worker, code, signal) => {
        logger.log('initServer:', this.name, core.role, 'worker terminated:', worker.id, 'pid:', worker.process.pid || "", "code:", code || "", 'signal:', signal || "");
        if (core.exiting) return;
        if (!this.noRestart) {
            lib.checkRespawn(() => { cluster.fork() });
        } else
        if (this.exitOnEmpty && !Object.keys(cluster.workers).length) {
            logger.log('initServer:', this.name, "exiting", core.role, 'no more workers', this.exitOnEmpty, "ms");
            setTimeout(() => { process.kill(process.pid) }, this.exitOnEmpty);
        }
    });

    ipc.on('jobs:cancel', function(msg, worker) {
        mod.cancelTask(msg.name, msg);
    });
    ...
    logger.log("initServer:", this.name, "started", core.role, cluster.isWorker ? cluster.worker.id : process.pid, "workers:", workers, "cron:", this.cron);
...
mod.initWorker = function(options, callback)
{
    ...
    ipc.on('jobs:cancel', function(msg, worker) {
        if (!msg.name || !mod.runningTasks.some((x) => (x.substr(0, msg.name.length) == msg.name))) return;
        mod.cancelled[mod._taskName(msg.name, msg.tag)] = msg.value !== undefined ? msg.value : 1;
    ...
    setTimeout(() => {
        ...
        logger.log("initWorker:", this.name, "started", core.role, cluster.isWorker ? cluster.worker.id : process.pid, "queue:", this.subscribed, "maxRuntime:", this.maxRuntime, "maxLifetime:", this.maxLifetime);
...
mod.subscribeWorker = function()
{
    ...
    for (const name of this.workerQueue) {
        ...
        ipc.subscribeQueue(qopts, this.processJobMessage.bind(this, name));
...
// if `value` is given it will return true only if it exactly equals to the set value in the task cancel state.
// The cancel state is cleared only if tag is given, if only name is matched the cancel state remains for other tasks
mod.isCancelled = function(job, tag, value)
{
    if (!tag) return this.cancelled[job];
    var task = this._taskName(job, tag);
    var rc = this.cancelled[task];
    if (rc) {
        if (value !== undefined && value !== rc) return;
        delete this.cancelled[task];
        this.cancelJob({ job, tag, value });
        return rc;
    }
}
...
mod.cancelTask = function(name, options)
{
    if (!cluster.isMaster || !name) return;
    ...
    for (var i in workers) {
        ...
        if (cluster.workers[workers[i]]) {
            cluster.workers[workers[i]].send(ipc.newMsg("jobs:cancel", { name: name, tag: options?.tag, value: options?.value }));
        }
    }
}
...
mod.isJob = function(jobspec)
{
    ...
    if (Array.isArray(jobspec.job)) {
        var job = jobspec.job.filter((x) => {
            if (typeof x == "string" && mod.jobRx.test(x)) return true;
            if (lib.isObject(x) && Object.keys(x).every((y) => (mod.jobRx.test(y)))) return true;
            return false;
...
mod.submitJob = function(jobspec, options, callback)
{
    ...
    if (util.types.isNativeError(jobspec)) return lib.tryCall(callback, jobspec);
    ...
    jobspec.origin = core.role + ":" + process.pid + ":" + core.ipaddr + ":" + Date.now();
    ...
    if (q.options.selfQueue && core.role == "worker") {
        setTimeout(() => {
            mod.runJob(jobspec, options, callback)
        }, options?.delay || 0);
    } else {
...
mod.runJob = function(jobspec, options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            ipc.lock("JOB:" + key, { ttl: ttl, queueName: mod.uniqueQueue }, function(err, locked) {
                // If the queue service is down keep all messages in the queue until it is up again
                if (!locked) {
                    if (!err && jobspec.uniqueDrop) {
                        logger.logger(jobspec.uniqueLogger || mod.uniqueLogger || "info", "runJob:", "dropped", options.queueName, jobspec);
                ...
                if (!jobspec.uniqueOnce) {
                    ...
                    timer = setInterval(function() {
                        ipc.lock("JOB:" + key, { ttl: ttl, queueName: mod.uniqueQueue, set: 1 });
                    }, Math.max(ttl * 0.7, 1000));
                }
                logger.debug("runJob:", options.queueName, ipc.getQueue(mod.uniqueQueue).name, "locked:", locked, "ttl:", ttl, "key:", key, "JOB:", jobspec)
                next(err);
            });
        },
        function(next) {
            ipc.emitMsg("jobs:started", { job: jobspec, queueName: options.queueName });
            mod._runJob(jobspec, options, function(err) {
                clearInterval(timer);
                if (ttl && key && !jobspec.uniqueKeep) {
                    ipc.unlock("JOB:" + key, { queueName: mod.uniqueQueue });
                }
...
// Send a cancellation request to the given `job` with optional `tag` and `value`. The value must mtach exactly.
mod.cancelJob = function(options, callback)
{
    var msg = ipc.newMsg("jobs:cancel", { name: options.job, tag: options.tag, value: options.value });
    ipc.broadcast(core.name + ":master", msg, { queueName: ipc.systemQueue }, callback);
}

// Sequentially execute all tasks in the list, run all subtasks in parallel
mod._runJob = function(jobspec, options, callback)
{
    ...
    lib.forEachSeries(tasks, (task, next) => {
        ...
        lib.forEvery(Object.keys(task), function(name, next2) {
            mod.runTask(name, task[name], options, (err) => {
...
mod.runTask = function(name, jobspec, options, callback)
{
    if (typeof options == "function") callback = options, options = null;
    if (!options) options = lib.empty;
    ...
    d.run(function() {
        ...
        mod.runningTasks.push(mod._taskName(name, jobspec.uniqueTag));
        mod.runTime = Date.now();
        if (cluster.isWorker) process.title = core.name + ': worker ' + mod.runningTasks.join(',');
...
mod._taskName = function(name, tag)
{
    if (tag) name += ":" + tag;
    return name;
}
...
{
    logger.logger(err && err.status >= 400 || util.types.isNativeError(err) ? "error" : err && err.status != 200 ? "info" : "debug", 'endTask:', options.queueName, name, lib.traceError(err), jobspec);
    this.runTime = Date.now();
    // Update process title with current job list
    var taskname = mod._taskName(name, jobspec.uniqueTag);
...
mod.scheduleCronjob = function(jobspec)
{
    ...
    try {
        ...
        var cj = this.croner.Cron(jobspec.cron, {}, (self) => {
            mod.submitJob(self.jobspec, { queueName: self.jobspec.queueName || mod.cronQueue });
        });
        cj.jobspec = jobspec;
...
mod.scheduleCronjobs = function(type, list)
{
    ...
    this.crontab = this.crontab.filter((x) => {
