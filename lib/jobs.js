...
var jobs = {
    ...
    args: [{ name: "workers", type: "number", min: -1, max: 32, descr: "How many worker processes to launch to process the job queue, -1 disables jobs, 0 means launch as many as the CPUs available" },
           ...
           { name: "unique-unlock-timeout", type: "int", descr: "Timeout in ms when to clear unique job lock after finished" },
    ],
    ...
    properties: [
        ...
        "noVisibility", "visibilityTimeout",
        "stopOnError",
        "startTime", "endTime",
        "uniqueTtl", "uniqueUnlockTimeout", "uniqueKey", "uniqueKeep",
...
jobs.initWorker = function(options, callback)
{
    ...
    ipc.on('jobs:cancel', function(msg, worker) {
        if (!msg.name || !jobs.runningTasks.some(function(x) { return x.substr(0, msg.name.length) == msg.name })) return;
        ...
        if (maxRuntime > 0) {
            ...
            if (badJobs.length) {
                if (this.runningJobs.length == badJobs.length) {
                    ...
                    for (const job of badJobs) ipc.drop(job, { queueName: job.jobQueue });
...
//
// `jobspec.uniqueUnlockTimeout` timeout in ms when to clear the lock after finished to avoid race conditions
//
...
jobs.runJob = function(jobspec, options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            ttl = jobspec.uniqueTtl = lib.toNumber(jobspec.uniqueTtl, { min: 0 });
            if (ttl && lib.testRegexp(jobspec.uniqueKey, jobs.uniqueIgnore)) {
                ttl = jobspec.uniqueTtl = 0;
            }
            if (!ttl) return next();
        ...
        function(next) {
            ...
            jobs._runJob(jobspec, options, function(err) {
                ...
                if (ttl && key && !jobspec.uniqueKeep) {
                    ...
                    }, jobspec.uniqueUnlockTimeout || jobs.uniqueUnlockTimeout);
                }
...
jobs.scheduleCronjob = function(jobspec)
{
    ...
    try {
        var cj = new cron.CronJob(jobspec.cron, () => {
