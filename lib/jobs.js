...
jobs.processJobMessage = function(name, msg, next)
{
    ...
    app.runMethods("configureJob", opts, { stopOnError: true, direct: true }, (err) => {
        ...
        jobs.runJob(opts.message, { queueName: name }, (err) => {
            ...
            app.runMethods("finishJob", opts, { stopOnError: true, parallel: true, direct: true, }, next);
...
jobs.runJob = function(jobspec, options, callback)
{
    ...
    logger.debug("runJob:", jobs.name, q, jobspec);
            ...
            cache.lock("JOB:" + key, { ttl, cacheName: jobs.uniqueCache }, (err, locked) => {
                ...
                if (!locked) {
                    if (!err && jobspec.uniqueDrop) {
                        logger.info("runJob:", jobs.name, "dropped", q, jobspec);
                ...
                logger.debug("runJob:", jobs.name, q, cache.getCache(jobs.uniqueCache).name, "locked:", locked, "ttl:", ttl, "key:", key, "JOB:", jobspec)
                next(err);
            });
        },
        function(next) {
