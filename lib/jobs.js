...
mod.isJob = function(jobspec)
{
    ...
    if (lib.isObject(jobspec.job)) {
        if (!Object.keys(jobspec.job).every(function(y) { return mod.jobRx.test(y) })) {
    ...
    if (Array.isArray(jobspec.job)) {
        var job = jobspec.job.filter(function(x) {
            ...
            if (lib.isObject(x) && Object.keys(x).every(function(y) { return mod.jobRx.test(y) })) return true;
            return false;
        }).map(function(x) {
...
mod.scheduleCronjob = function(jobspec)
{
    ...
    try {
        ...
        var cj = new cron.CronJob(jobspec.cron, function() {
            mod.submitJob(this.jobspec, { queueName: jobspec.queueName || mod.cronQueue });
        }, null, true);
        cj.jobspec = jobspec;
...
mod.loadCronjobs = function()
{
    ...
    this.cronWatcher = fs.watch(core.path.etc, function (event, filename) {
        if (filename && filename.match(/crontab/)) core.setTimeout(filename, function() { mod.loadCronjobs(); }, 5000);
    });
}
