...
const jobs = {
    ...
    args: [
        ...
        { name: "schedule", type: "json", onupdate: function() { if (app.role == "server" && this.cron) this.scheduleCronjobs("config", this.schedule) }, logger: "error", descr: "Cron jobs to be scheduled, the JSON must be in the same format as crontab file, cron format by https://croner.56k.guru" },
 ...
 * Job queue processor
 *
 * When launched with `jobs-workers` parameter equal or greater than 0, the server spawns a number of workers which subscribe to
