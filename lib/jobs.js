...
// but jobs sent to worker queue will be run locally if the current process is not master.
//
//
const mod = {
    name: "jobs",
...
mod.runJob = function(jobspec, options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            ipc.lock("JOB:" + key, { ttl: ttl, queueName: mod.uniqueQueue }, (err, locked) => {
                ...
                logger.debug("runJob:", queue, ipc.getQueue(mod.uniqueQueue).name, "locked:", locked, "ttl:", ttl, "key:", key, "JOB:", jobspec)
                next(err);
            });
        },
        function(next) {
            ...
            lib.forEvery(Object.keys(jobspec.job), (task, next2) => {
                mod._runTask(task, jobspec.job[task], options, (err) => {
...
mod.scheduleCronjob = function(jobspec)
{
    ...
    try {
        ...
        var cj = this.croner.Cron(jobspec.cron, jobspec.croner || {}, (job) => {
            mod.submitJob(job.jobspec, { queueName: job.jobspec.queueName || mod.cronQueue });
        });
        cj.jobspec = jobspec;
