...
jobs.runJob = function(jobspec, options, callback)
{
    ...
    lib.series([
      function(next) {
          ...
          ipc.lock(jobspec.uniqueKey, { ttl: jobspec.uniqueTtl, queueName: self.uniqueQueue }, function(err, locked) {
              if (!locked) {
                  // If the queue service is down keep all messages in the queue
                  if (!err && jobspec.uniqueDrop) return lib.tryCall(callback);
                  err = { status: 600, message: err || self.uniqueError || "non-unique condition" };
              }
              next(err);
          });
      },
