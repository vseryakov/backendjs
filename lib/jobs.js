...
{
    logger[err ? "error" : "info"]('finishTask:', options.queueName, name, lib.traceError(err), lib.objDescr(jobspec));
...
jobs.scheduleCronjob = function(jobspec)
{
    ...
    try {
        var cj = new cron.CronJob(jobspec.cron, function() {
            ...
            if (this.jobspec.singleTask) {
                var names = [];
                if (typeof this.jobspec.singleTask == "string") {
                    names.push(this.jobspec.singleTask);
                } else {
                ...
                for (var p in cluster.workers) {
                    if (names.indexOf(cluster.workers[p].taskName) > -1) {
                        ...
                        if (this.jobspec.singleTimeout > 0 && Date.now() - cluster.workers[p].taskTime > this.jobspec.singleTimeout) continue;
                        return;
                    }
                }
            }
...
// - singleTask - if set only one cron job can be run at a time with the same name, this works only within single master and is checked
...
// - singleTimeout - how long to wait for running job before submitting another one, for cases when unique job stuck or other condition
