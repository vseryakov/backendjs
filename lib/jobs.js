...
mod.initWorker = function(options, callback)
{
    ...
    setTimeout(function() {
        mod.subscribeWorker();
        logger.log("initWorker:", mod.name, "started", cluster.isWorker ? cluster.worker.id : process.pid, "queue:", mod.subscribed, "maxRuntime:", mod.maxRuntime, "maxLifetime:", mod.maxLifetime);
...
mod.subscribeWorker = function()
{
    ...
    for (const name of this.workerQueue) {
        ...
        if (mod.subscribed.indexOf(q.canonical(name)) > -1) continue;
        mod.subscribed.push(q.canonical(name));

        ipc.subscribeQueue(lib.objExtend(mod.workerOptions[name], "queueName", name), mod.processJobMessage.bind(mod, name));
        logger.info("subscribeWorker:", mod.name, q.name, name);

        q.on("closed", () => {
            mod.unsubscribeQueue(name);
            if (mod._subscribeTimer) clearTimeout(mod._subscribeTimer);
            mod._subscribeTimer = setTimeout(mod.subscribeWorker.bind(mod), 1000);
        });
    }
}
...
{
    return this.runningmod.reduce((m, x) => (Math.max(m, x.maxRuntime || 0)), this.maxRuntime) * 1000;
}
        ...
        function(next) {
            ...
            ipc.lock("JOB:" + key, { ttl: ttl, queueName: mod.uniqueQueue }, function(err, locked) {
                ...
                if (!locked) {
                    ...
                    err = { status: 600, message: err || mod.uniqueError, logger: jobspec.uniqueLogger || "info" };
