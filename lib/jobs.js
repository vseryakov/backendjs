...
jobs.initWorker = function(options, callback)
{
    ...
    setTimeout(function() {
        ...
        logger.log("initWorker:", "started", cluster.isWorker ? cluster.worker.id : process.pid, "queue:", jobs.workerQueue, "maxRuntime:", jobs.maxRuntime, "maxLifetime:", jobs.maxLifetime);
...
jobs.subscribeWorker = function()
{
    ...
    this.workerQueue.forEach(function(name) {
        ...
        if (jobs.subscribed.indexOf(q.name) > -1) return;
        jobs.subscribed.push(q.name);
