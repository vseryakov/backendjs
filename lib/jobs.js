        ...
        ipc.listen({ queueName: name }, function(msg, next) {
            ...
            core.runMethods("configureJob", opts, function() {
                jobs.runJob(opts.message, { queueName: name }, function(err) {
                    if (!err) logger.debug("runJob:", "finished", name, opts.message);
                    if (err) logger[err.status == 600 ? "warn" : err.status != 200 ? "error" : "info"]("runJob:", "finished", name, lib.traceError(err), lib.objDescr(opts.message));
...
jobs.runJob = function(jobspec, options, callback)
{
    ...
    logger.debug("runJob:", "started", options.queueName, jobspec);
...
jobs._finishTask = function(err, name, jobspec, options, callback)
{
    if (err && err.status != 200) {
        logger.error('finishTask:', options.queueName, name, lib.traceError(err), lib.objDescr(jobspec, { length: 512, count: 10 }));
    } else {
        logger.debug('finishTask:', options.queueName, name, err, jobspec);
    }
