...
var jobs = {
    ...
    args: [{ name: "workers", type: "number", min: -1, max: 32, descr: "How many worker processes to launch to process the job queue, -1 disables jobs, 0 means launch as many as the CPUs available" },
           ...
           { name: "schedule", type: "json", trigger: function(v) { if((core.role == "master" || ipc.role == "server") && this.cron) this.scheduleCronjobs("config", this.schedule) }, logger: "error", descr: "Cron jobs to be scheduled, the JSON must be in the same format as crontab file" },
    ],
