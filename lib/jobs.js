...
mod._finishTask = function(err, name, jobspec, options, callback)
{
    if (err && !(err.status >= 200 && err.status < 300)) {
        logger.logger(err.status >= 400 || util.types.isNativeError(err) ? "error" : "info", 'endTask:', options.queueName, name, lib.traceError(err), jobspec);
    } else {
