...
jobs.subscribeWorker = function()
{
    ...
    this.workerQueue.forEach(function(name) {
        ...
        ipc.listen({ queueName: name }, function(msg, next) {
            var opts = { queue: name, message: msg, stopOnError: 1 };
            core.runMethods("configureJob", opts, function(err) {
                if (err) {
                    logger.debug("configureJob:", err, opts.message);
                    return lib.tryCall(next, err);
                }
                jobs.runJob(opts.message, { queueName: name }, function(err) {
                    if (!err) logger.logger(opts.message.logger || "debug", "endJob:", name, opts.message);
                    if (err) logger.logger(err.status == 600 ? err.logger || "warn" : err.status != 200 ? "error" : "info", "endJob:", name, lib.traceError(err), jobs.descrJob(opts.message));
...
jobs.isJob = function(jobspec)
{
    ...
    if (!lib.isObject(jobspec)) return lib.newError("invalid job:" + jobs.descrJob(jobspec));
    ...
    if (lib.isObject(jobspec.job)) {
        if (!Object.keys(jobspec.job).every(function(y) { return jobs.jobRx.test(y) })) {
            return lib.newError('invalid job: ' + jobs.descrJob(jobspec));
        }
    } else
    ...
    if (Array.isArray(jobspec.job)) {
        ...
        if (!job.length) return lib.newError('invalid job: ' + jobs.descrJob(jobspec));
        jobspec.job = job;
    } else {
        return lib.newError('invalid job: ' + jobs.descrJob(jobspec));
    }
...
// Returns safe description of the job
jobs.descrJob = function(jobspec, options)
{
    return lib.objDescr(jobspec, lib.objExtend({ hide: logger.ignore }, options));
}
    ...
    lib.series([
          ...
          ipc.lock("JOB:" + jobspec.uniqueKey, { ttl: ttl, queueName: jobs.uniqueQueue }, function(err, locked) {
              ...
              if (!locked) {
                  if (!err && jobspec.uniqueDrop) {
                      logger.info("runJob:", "dropped", options.queueName, jobs.descrJob(jobspec));
                      ipc.emitMsg("jobs:dropped", { job: jobspec });
                      return lib.tryCall(callback, { status: 200, message: "dropped" });
                  }
                  err = { status: 600, message: err || jobs.uniqueError, logger: "info" };
      ...
      function(next) {
          ...
          jobs._runJob(jobspec, options, function(err) {
              clearInterval(timer);
              if (jobspec.uniqueKey) ipc.unlock("JOB:" + jobspec.uniqueKey, { queueName: jobs.uniqueQueue });
...
jobs.runTask = function(name, jobspec, options, callback)
{
    ...
    if (!module || typeof module[method[1]] != "function") {
        logger.error("runTask:", options.queueName, name, "unknown method", jobs.descrJob(jobspec));
...
jobs._finishTask = function(err, name, jobspec, options, callback)
{
    if (err && err.status != 200) {
        logger.error('endTask:', options.queueName, name, lib.traceError(err), jobs.descrJob(jobspec, { length: 256, count: 10 }));
    } else {
    ...
    var idx = this.running.indexOf(taskname);
...
jobs.scheduleCronjob = function(jobspec)
{
    ...
    if (util.isError(jobspec)) {
        logger.error("scheduleCronjob:", "invalid", jobs.descrJob(jobspec));
        return false;
    }
    ...
    try {
        ...
        logger.error("scheduleCronjob:", e, jobs.descrJob(jobspec));
        return false;
    }
}
