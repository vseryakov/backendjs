 ...
 * "module.method"
 * { job: { "module.method": { ... } } }
 * { job: { "module.method": { ... }, "module2.method2": { ... } } }
 * ```
 *
 ...
 * ## Example:
 *
 * ### The common queue is shared via config gfile or DB between all processes
 * ```
 * queue-users=sqs://users
 * ```
 *
 * ### Execute jobs in a module defined and run somewhere in a worker process
 *
 * ```js
 ...
 *
 * This will require to install peer dependency: `npm install croner`
 *
 ...
 *   ```
 *
 * 2. Start the server with cron parameters (in cmdline or config file)
 *
 *   ```shell
