 ...
 *
 * Once configured, all calls to {@link module:jobs.submitJob} will push jobs to be executed to the provided or default queue.
 *
 ...
 *
 *             jobs.submitJob({ job: { "mymod.processUsers": { type: req.context.query.type } } }, { queueName: "users" }, (err) => {
 *                 context.reply(err);
 *             });
 *         });
 *         callback();
...
{
    return this.runningJobs.reduce((m, x) => (Math.max(m, x.maxRuntime || 0)), this.maxRuntime);
}
...
jobs.checkTimes = function()
{
    ...
    if (this.runningJobs.length) {
        ...
        if (maxRuntime > 0) {
            const badJobs = this.runningJobs.filter((x) => (now - x.jobTime > maxRuntime));
...
jobs.submitJob = function(jobspec, options, callback)
{
    ...
    logger.debug("submitJob:", jobs.name, jobspec, "OPTS:", options, "Q:", q.name);
...
/**
 * Async version of {@link module:jobs.submitJob}
 * @param {object} jobspec - an object with jobs to run
 * @param {object} [options]
 * @memberOf module:jobs
 * @method asubmitJob
 ...
 */
jobs.asubmitJob = function(jobspec, options)
{
    return new Promise((resolve, _reject) => {
        jobs.submitJob(jobspec, options, (err, data) => {
            resolve({ err, data });
        });
    });
}
...
jobs.runJob = function(jobspec, options, callback)
{
    ...
    logger.debug("runJob:", jobs.name, q, "started:", jobspec);
    ...
    lib.series([
            ...
            cache.lock("JOB:" + key, { ttl, cacheName: jobs.uniqueCache }, (err, locked) => {
                ...
                if (!locked) {
                    if (!err && jobspec.uniqueDrop) {
                        logger.info("runJob:", jobs.name, q, "dropped:", jobspec);
                ...
                logger.debug("runJob:", jobs.name, q, cache.getCache(jobs.uniqueCache).name, "locked:", locked, ttl, "key:", key, "JOB:", jobspec, err)
                next(err);
            });
        },
        ...
        function(next) {
            ...
            lib.forEvery(Object.keys(jobspec.job), (task, next2) => {
                ...
                const idx = jobs.runningJobs.indexOf(jobspec);
