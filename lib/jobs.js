        ...
        ipc.listen({ queueName: name }, function(msg, next) {
            ...
            core.runMethods("configureJob", opts, function(err) {
                ...
                jobs.runJob(opts.message, { queueName: name }, function(err) {
                    ...
                    if (err) logger.logger(err.status == 600 ? err.logger || "warn" : err.status != 200 ? "error" : "info", "endJob:", name, lib.traceError(err), jobs.descrJob(opts.message), opts.elapsed, "ms");
