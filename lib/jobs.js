 ...
 *
 * Once configured, all calls to {@link module:jobs.submit} will push jobs to be executed to the provided or default queue.
 *
 ...
 * ## Security note
 *
 * While this convention is flexible and simple it relies that submitting jobs is secure to avoid remote code execution by someone unaauthorized
 * to do it. This requires to know whihc modules and methods are available of course which means access to the source code.
 *
 ...
 *     {
 *         api.app.post("/process/users", (context) => {
 *
 *             jobs.submit({ job: { "mymod.processUsers": { type: req.context.query.type } } }, { queueName: "users" }, (err) => {
 *                 context.reply(err);
 *             });
 *         });
 *         callback();
...
module.exports = {
     ...
     */
    args: [
        { name: "cap-(.+)", type: "int", strip: "cap-", same_type: 1, descr: "Capability parameters" },
        ...
        { name: "worker-queue", type: "list", onupdate() { if (ipc.role==="worker"&&app.role==="worker") this.subscribeWorker()}, descr: "Queue(s) to subscribe for workers, multiple queues can be processed at the same time, i.e. more than one job can run from different queues" },
        ...
        { name: "max-runtime", type: "int", min: 0, descr: "Max number of milliseconds a job can run before being killed" },
        { name: "max-lifetime", type: "int", min: 0, descr: "Max number of milliseconds a worker can live, after that amount of time it will exit once all the jobs are finished, 0 means indefinitely" },
        ...
        { name: "cron", type: "json", onupdate(v) { if (app.role === "server") this.scheduleCronjobs("config", v) }, logger: "error", descr: "Cron jobs to be scheduled, the JSON must be in the same format as crontab file, cron format by https://croner.56k.guru" },
        ...
        { name: "unique-ttl-([0-9]+)", type: "regexp", obj: "uniqueTtl", make: "$1", descr: "Override unique TTL to a new value if matches the unique key", example: "-jobs-unique-ttl-100 KEY" },
        { name: "unique-logger", descr: "Log level for unique error conditions" },
        { name: "retry-visibility-timeout", type: "map", map_type: "int", descr: "Visibility timeout by error code >= 500 for queues that support it" },
    ...
    workerQueue: [],
    workerCpuFactor: 2,
    workerEnv: Object.create(null),
    workerOptions: Object.create(null),
    globalIgnore: ["local", "worker"],
    properties: [
        "noWait", "noVisibilityTimeout", "noRetryVisibilityTimeout",
        "visibilityTimeout", "retryVisibilityTimeout",
        "stopOnError",
        "startTime", "endTime",
        "uniqueTtl", "uniqueKey", "uniqueKeep",
        "uniqueDrop", "uniqueOnce",
        "maxRuntime"
    ],
    metrics: {
...
jobs.shutdown = function(_options, callback)
{
...
jobs.shutdownWorker = function(options, callback)
{
    ...
    setTimeout(callback, options?.shutdownTimeout || this.shutdownTimeout).unref();
}
...
jobs.initServer = function(options, callback)
{
    ...
    // Launch the workers
    const workers = this.workers || Math.round(app.maxCPUs * (this.workerCpuFactor || 1));
...
jobs.initWorker = function(options, callback)
{
    ...
    this._checkTimer = setInterval(this.checkTimes.bind(this), 30000).unref();
...
jobs.subscribeWorker = function()
{
    ...
    if (!jobs.workerQueue.length) jobs.workerQueue.push("queue");

    for (const name of jobs.workerQueue) {
        // Unsubscribed if started with -
        if (/^[!-]/.test(name)) {
            jobs.unsubscribe(name.substr(1));
            continue;
        }
        jobs.subscribe(name, jobs.workerOptions[name]);
    }
}

/**
 * Subscribe to a queue to process jobs
 * @param {string} name - a queue name
 * @param {object} [options]
 * @returns {string|undefined} - a subscription on success
 * @memberof module:jobs
 * @method subscribe
 *
 */
jobs.subscribe = function(name, options)
{
    const q = queue.getQueue(name);
    const sub = q.subscription(name);
    if (jobs.subscribed.has(sub)) return;
    const qopts = Object.assign({ queueName: name }, options);
    queue.listen(qopts, jobs.processJobMessage.bind(jobs, name));
    jobs.subscribed.add(sub);
    logger.info("subscribeWorker:", jobs.name, q.name, sub);
    return sub;
}

/**
 * Unsibscribe from a queue, no more jobs will be processed
 * @param {object} options
 ...
 *
 */
jobs.unsubscribe = function(name)
{
    const q = queue.getClient(name);
    const sub = q.subscription(name);
    if (!jobs.subscribed.delete(sub)) return;
    queue.unlisten({ queueName: name });
    logger.info("unsubscribe:", jobs.name, q.name, sub);
    return sub;
}

jobs.processJobMessage = function(name, msg, next)
{
    var opts = { queue: name, message: msg, stime: Date.now() };
    if (typeof next !== "function") next = lib.noop;

    app.runMethods("configureJob", opts, { stopOnError: true, direct: true }, (err) => {
        if (err) return next(err);

        const _timer = queue.getQueue(name).metrics.start();
        jobs.run(opts.message, { queueName: name }, (err) => {
            _timer.end();
            opts.error = err;
            opts.etime = Date.now();
            opts.elapsed = opts.etime - opts.stime;
            if (err) {
                logger.logger(err.status >= 600 ? err.logger || "warn" : !err.status || err.status < 200 || err.status > 299 ? "error" : "info", "endJob:", jobs.name, name, lib.traceError(err), opts.message, opts.elapsed, "ms");
            } else {
                logger.logger(opts.message.logger || "debug", "endJob:", jobs.name, name, opts.message, opts.elapsed, "ms");
            }

            app.runMethods("finishJob", opts, { stopOnError: true, parallel: true, direct: true, }, () => {
                next(err);
            });
...
jobs.markCancelled = function(msg)
{
    ...
    for (const job of this.running) {
        job.cancelKey = lib.arrayUpdate("add", job.cancelKey, msg.key);
    }
    logger.info("markCancelled:", this.running.length, msg);
}
...
jobs.isCancelled = function(key)
{
    ...
    for (const job of this.running) {
        if (lib.includes(job?.cancelKey, key)) return 1;
    }
    return this.exiting;
}

// Find the max runtime allowed in milliseconds
jobs.getMaxRuntime = function()
{
    return this.running.reduce((m, x) => (Math.max(m, x.maxRuntime || 0)), this.maxRuntime);
}

// Return a list of unique job names currently running
jobs.getRunning = function()
{
    var jobs = {};
    for (const job of this.running) {
...
jobs.checkTimes = function()
{
    ...
    if (this.running.length) {
        const maxRuntime = this.getMaxRuntime();
        if (maxRuntime > 0) {
            const badJobs = this.running.filter((x) => (now - x.jobTime > maxRuntime));
            // Stuck jobs can run much longer if we are still processing other small jobs
            if (badJobs.length) {
                if (this.running.length === badJobs.length) {
                    ...
                    return this.exitWorker({ jobs: badJobs, shutdownReason: "maxRuntime", maxRuntime });
                } else
                if (now - this.checkRuntime > maxRuntime) {
                    logger.warn('checkLifetime:', jobs.name, 'exceeded max run time but other jobs still running', maxRuntime, badJobs);
                }
        ...
        if (this.maxLifetime > 0 && now - app.ctime + lib.randomShort() > this.maxLifetime) {
            logger.log('checkLifetime:', jobs.name, 'exceeded max life time', this.maxLifetime);
            return this.exitWorker({ shutdownReason: "maxLifetime", maxLifetime: this.maxLifetime });
        }
    }
}
...
{
    return lib.newError('Invalid job: ' + lib.inspect(jobspec), 400, "InvalidJob");
}
 ...
 * The queue and the way how it will be executed depends on the configured queue. See {@link module:jobs.isJob} for
 ...
 * creates a global lock to serialize jobs by this key only if `options.uniqueTtl` or `options.visibilityTimeout` found.
 *
 ...
 *
 * @param {int} [options.maxRuntime] - defines max number of milliseconds this job can run, if not specified then the queue default is used
 *
 * @param {int} [options.uniqueOnce] - if true than the visibility timeout is not kept alive while the job is running
 *
 * @param {boolean|int} [options.noWait] - delete the job it from the queue immediately or after delay, not at the end, for one-off jobs:
 * - 1 or true - delete immediately
 * - > 1 - number of milliseconds before deleting the job giving it time to check for something
 *
 * @param {int} [options.visibilityTimeout] - custom timeout in milliseconds for how long to keep this job invisible, overrides the default timeout
 *
 * @param {boolean} [options.noVisibilityTimeout] disables auto extending visibilityTimeout while the job is running, allows default
 * queue visibility timeout to be used instead
 *
 * @param {boolean} [options.noRetryVisibilityTimeout] - will always delete messages after processing, ignore 600 errors as well
 *
 ...
 *
 * @param {int} [options.delay] - is only supported by SQS currently, it delays the job execution for the specified amount of milliseconds
 *
 * @param {int} [options.dedupTtl] - if set it defines number of milliseconds to keep track of duplicate messages, it tries to preserver only-once behaviour. To make
 *  some queue to automatically use dedup mode it can be set in the queue options: `-queue[-NAME]-options-dedup-ttl 86400000`.
 *  Note: `options.uniqueTtl` takes precedence and if present dedup is ignored.
 * @param {function} callback
 * @memberOf module:jobs
 * @method submit
 * @example <caption>Common job object to be used in below examples</caption>
 * const accountId = "..."
 * const job = { job: { "mymod.processOrder": { id: 1, name: "Book" } } }
 *
 * @example <caption>Simple on-off job using default queue</caption>
 * jobs.submit(job)
 *
 * @example <caption>Use specific queue</caption>
 * jobs.submit(job, { queueName: "orders" })
 *
 * @example <caption>Custom visibilityTimeout of 5 mins</caption>
 * jobs.submit(job, { queueName: "orders", visibilityTimeout: 300000 })
 *
 * @example <caption>Place an order but delay processing for an hour</caption>
 * jobs.submit(job, { queueName: "orders", startTime: Date.now() + 3600000 })
 *
 * @example <caption>Serialize exact orders at least every 5 mins</caption>
 * jobs.submit(job, { queueName: "orders", uniqueTtl: 300000 })
 *
 * @example <caption>Serialize all orders for the same account</caption>
 * jobs.submit(job, { queueName: "orders", uniqueKey: `ORDER:${accountId}`, uniqueTtl: 300000 })
 *
 * @example <caption>Drop duplicate orders for the same account if an order processing is still runing</caption>
 * jobs.submit(job, { queueName: "orders", uniqueKey: `ORDER:${accountId}`, uniqueDrop: true })
 *
 */
jobs.submit = function(jobspec, options, callback)
{
     ...
     */
    if (this.globalQueue && !lib.includes(this.globalIgnore, qname)) {
        qname = this.globalQueue;
    }
    const q = queue.getQueue(qname);

    // Ignore duplicate messages
    const ttl = lib.toNumber(q.options.dedupTtl || options?.dedupTtl);
    if (ttl > 0) {
    ...
    logger.debug("submit:", jobs.name, jobspec, "OPTS:", options, "Q:", q.name);
    ...
    // Queue global properties
    for (const p of ["uniqueTtl", "visibilityTimeout"]) {
        if (lib.isEmpty(jobspec[p]) && q.options[p] > 0) {
            jobspec[p] ??= q.options[p];
        }
    }
...
/**
 * Async version of {@link module:jobs.submit}
 * @param {object} jobspec - an object with jobs to run
 * @param {object} [options]
 * @memberOf module:jobs
 * @method asubmit
 * @async
 * @example
 * const { err, data } = await jobs.asubmit({ job: { "mymod.processOrder": { id: ... } } }, { queueName: "orders" })
 */
jobs.asubmit = function(jobspec, options)
{
    return new Promise((resolve, _reject) => {
        jobs.submit(jobspec, options, (err, data) => {
            resolve({ err, data });
        });
    });
}
...
jobs.run = function(jobspec, options, callback)
{
    var q = options?.queueName;

    logger.debug("run:", jobs.name, q, "started:", jobspec);
    ...
    lib.series([
        function(next) {
            if (lib.testRegexp(key, jobs.uniqueIgnore)) return next();

            // Make sure we do not have this job in the queue
            ttl = lib.toNumber(jobspec.uniqueTtl, { min: 0 });
            key = jobspec.uniqueKey;

            // Managing throughput by changing ttl
            if (key && jobs.uniqueTtl) {
                for (const p in jobs.uniqueTtl) {
                    if (lib.testRegexp(key, jobs.uniqueTtl[p])) {
                        ttl = jobspec.uniqueTtl = lib.toNumber(p);
                        break;
                    }
                }
            }

            // Use dedup in form ttl:uuid if present, simulate unique properties
            if (!ttl && jobspec.dedup) {
            ...
            if (!ttl && key) {
                ttl = lib.toNumber(jobspec.visibilityTimeout, { min: 0 });
            }

            // Neither ttl nor key, skip
            if (!ttl) return next();

            key = jobspec.uniqueKey = key || lib.hash(lib.stringify(jobspec.job), "sha256");

            cache.lock("JOB:" + key, { ttl, cacheName: jobs.uniqueCache }, (err, locked) => {
                // If the queue service is down keep all messages in the queue until it is up again
                if (!locked) {
                    if (!err && jobspec.uniqueDrop) {
                        logger.info("run:", jobs.name, q, "dropped:", jobspec);
                    ...
                    ipc.emitMsg("jobs:nolock", { job: jobspec, queueName: q, err });
                } else

                if (!err && !jobspec.uniqueOnce) {
                    // Keep the lock active while the job is running
                    timer = setInterval(() => {
                        logger.debug("runJob:", jobs.name, q, "relocked:", ttl, jobspec);
                        cache.lock("JOB:" + key, { ttl, cacheName: jobs.uniqueCache, set: 1 });
                    }, Math.max(ttl * 0.7, 1000));
                }
                logger.debug("run:", jobs.name, q, cache.getCache(jobs.uniqueCache).name, "locked:", locked, ttl, "key:", key, "JOB:", jobspec, err)
                next(err);
            });
        },
        ...
        function(next) {
            ...
            jobs.running.push(jobspec);
            if (cluster.isWorker) process.title = `${app.id}: worker ${jobs.getRunning()}`;
            ...
            lib.forEvery(Object.keys(jobspec.job), (task, next2) => {
                ...
                const idx = jobs.running.indexOf(jobspec);
                if (idx > -1) jobs.running.splice(idx, 1);
                if (cluster.isWorker) process.title = `${app.id}: worker ${jobs.getRunning()}`;
 ...
 * @method cancel
 */
jobs.cancel = function(key, callback)
{
    ipc.broadcast(":worker", ipc.newMsg("jobs:cancel", { key }), callback);
}
...
function _runTask(name, jobspec, options, callback)
{
    ...
    if (jobs.taskIgnore?.test(name)) {
        logger.error("runTask:", jobs.name, options?.queueName, name, "task ignored", job, "RX:", jobs.taskIgnore);
        return callback(lib.newError("Task ignored: " + name, 499, "TaskIgnored"));
    }

    const [context, method] = app.getMethodContext(name);
    if (!context) {
        logger.error("runTask:", jobs.name, options?.queueName, name, "unknown method", job);
    ...
    d.run(() => {
        logger.debug('runTask:', jobs.name, 'started', name, job);
        jobs.runTime = Date.now();
...
function _finishTask(err, name, jobspec, options, callback)
{
    ...
    if (err && !(err.status >= 200 && err.status < 300)) {
        ...
        logger.debug('endTask:', jobs.name, options.queueName, name, err, job);
    }
