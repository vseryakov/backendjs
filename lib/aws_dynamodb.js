...
aws.ddbCreateTable = function(name, attrs, options, callback)
{
    ...
    if (!options) options = {};
    var params = { TableName: name, AttributeDefinitions: [], KeySchema: [] };
    ...
    if (options.add) {
        ...
        for (var name in options.add) {
            ...
            if (!index.ProvisionedThroughput && !options.onDemand) {
                index.ProvisionedThroughput = { ReadCapacityUnits: options.readCapacity || aws.ddbReadCapacity || 1, WriteCapacityUnits: options.writeCapacity || aws.ddbWriteCapacity || 1 };
            }
