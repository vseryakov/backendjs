...
aws.buildProjectionExpression = function(params, names, index)
{
    ...
    lib.strSplit(names).forEach(function(name) {
        if (!/^[a-zA-Z][a-zA-Z0-9]+$/.test(name) || aws.ddbReserved[name.toUpperCase()]) {
            if (!params.ExpressionAttributeNames) params.ExpressionAttributeNames = {};
...
aws.ddbUpdateTable = function(options, callback)
{
    ...
    if (!options.onDemand && options.BillingMode != "PAY_PER_REQUEST" && !params.StreamSpecification && r && w) {
        params.ProvisionedThroughput = { ReadCapacityUnits: r, WriteCapacityUnits: w };
    }
    if (options.add) {
        ...
        for (var name in options.add) {
            ...
            if (!index.ProvisionedThroughput && !options.onDemand && r && w) {
