...
aws.queryDDB = function (action, obj, options, callback)
{
    ...
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (params.status != 200) {
            ...
            logger[opts.quiet || err.code == "ConditionalCheckFailedException" ? "debug" : "error"]('errorDDB:', action, obj, params.toJSON(), "token:", aws.key, aws.tokenExpiration, params.headers.Authorization);
        } else {
...
{
    return this.status == 500 || this.status == 503 || this.data.match(/(ProvisionedThroughputExceededException|ThrottlingException|SerializationException)/)
}
...
aws.queryExpression = function(obj, options, join, index)
{
    ...
    for (var name in obj) {
        ...
        if (opsMap[op]) op = opsMap[op];
        if (val == null) op = "null";
        if (name.match(/^[0-9]/) || this.ddbReserved[name.toUpperCase()]) {
            names["#n" + n] = name;
            name = "#n" + n++;
        }
