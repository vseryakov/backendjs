...
aws.queryExpression = function(obj, options, join, index)
{
    ...
    for (var name in obj) {
        ...
        if (/^[0-9]/.test(name) || this.ddbReserved[name.toUpperCase()]) {
...
aws.ddbQueryTable = function(name, condition, options, callback)
{
    ...
    if (typeof options == "function") callback = options, options = null;
    if (!options) options = lib.empty;
    var params = { TableName: name }, expr;
    if (options.names) {
        params.ExpressionAttributeNames = self.toDynamoDB(options.names);
    }
    if (options.values) {
    ...
    if (options.select) {
        params.ProjectionExpression = lib.strSplit(options.select).join(",");
    }
    if (options.count > 0) {
    ...
    if (Array.isArray(options.keys)) {
        ...
        this.buildExpression(params, "KeyConditionExpression", keys, options);
        this.buildExpression(params, "FilterExpression", filter, options, "", 10);
    } else
    if (lib.isObject(options.keys)) {
        this.buildExpression(params, "KeyConditionExpression", options.keys, options);
    } else {
        this.buildExpression(params, "KeyConditionExpression", condition, options);
    }
