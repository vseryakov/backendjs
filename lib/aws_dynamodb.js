...
aws._queryDDB = function(target, service, action, obj, options, callback)
{
    ...
    var uri = lib.rxUrl.test(options.endpoint) ? options.endpoint : ((options.endpoint_protocol || 'https') + '://' + (service || 'dynamodb') + "." + region + '.amazonaws.com/');
    ...
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (params.status != 200) {
            ...
            logger.logger(options.logger_db || "debug", 'queryDDB:', action, 'finished:', Date.now() - start, 'ms', params.obj.Item ? 1 : (params.obj.Count || 0), 'rows', params.obj.ConsumedCapacity || "");
        }
