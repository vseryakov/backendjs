...
aws.queryDDB = function (action, obj, options, callback)
{
    ...
    var uri = lib.rxUrl.test(options.endpoint) ? options.endpoint : ((options.endpoint_protocol || 'http://') + 'dynamodb.' + region + '.amazonaws.com/');
        ...
        if (level) {
            for (var p in value) {
                ...
                case 'SS':
                    return value[p];
                case 'B':
                    return Buffer.from(value[i]['B'], "base64");
                case 'BS':
                    var res = [];
...
aws.ddbTransactWriteItems = function(items, options, callback)
{
    ...
    lib.isArray(items, []).forEach(function(x) {
        ...
        for (var p in opts) if (p[0] >= 'A' && p[0] <= 'Z') obj[p] = options[p];
        if (obj.ReturnValues) {
