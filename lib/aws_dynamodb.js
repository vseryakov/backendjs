...
aws.ddbRetryRx = /(ProvisionedThroughputExceededException|ThrottlingException|SerializationException|UnrecognizedClientException|LimitExceededException)/;
...
aws._queryDDB = function(target, service, action, obj, options, callback)
{
    ...
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (params.status != 200) {
            ...
            logger.logger(options.logger_db || "debug", 'queryDDB:', action, 'finished:', Date.now() - start, 'ms', params.obj.Item ? '1 row' : params.obj.Count ? params.obj.Count + ' rows' : "", params.obj.ConsumedCapacity || "");
        }
