...
aws.ddbRetryRx = /(ProvisionedThroughputExceededException|ThrottlingException|SerializationException|UnrecognizedClientException)/;
...
aws._queryDDB = function(target, service, action, obj, options, callback)
{
    ...
    var uri = lib.rxUrl.test(options.endpoint) ? options.endpoint : ((options.endpoint_protocol || 'http://') + (service || 'dynamodb') + "." + region + '.amazonaws.com/');
    ...
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (params.status != 200) {
            ...
            logger.errorWithOptions(err, options, 'errorDDB:', err, action, obj, params.toJSON(), "token:", aws.key, aws.tokenExpiration, params.headers.Authorization);
        } else {
    ...
    for (var name in obj) {
        ...
        case 'IN':
            if (Array.isArray(val)) {
                ...
                val.forEach(function(x) { if (x) cond.AttributeValueList.push(aws.toDynamoDB(x));});
            } else
            if (val) {
...
aws.queryExpression = function(obj, options, join, index)
{
    ...
    for (var name in obj) {
        ...
        if (d) {
            ...
            if (e) {
                expr.push("(" + e.expr + ")");
                for (const p in e.names) names[p] = e.names[p];
                for (const p in e.values) values[p] = e.values[p];
            }
            continue;
        }
        ...
        case 'not contains':
            ...
            if (Array.isArray(val)) {
                ...
                for (let i = 0; i < val.length; i++) {
                    if (!val[i]) continue;
                    vals.push("not contains(" + name + ",:v" + v + ")");
                    values[":v" + v++] = val[i];
                }
                if (!vals.length) break;
                expr.push("(" + vals.join(" and ") + ")");
            } else {
                _checkName();
...
aws.ddbUpdateTable = function(options, callback)
{
    ...
    if (typeof options == "function") callback = options, options = null;
    if (!options) options = {};
    var r = options.readCapacity || aws.ddbReadCapacity || 5, w = options.writeCapacity || aws.ddbWriteCapacity || 1;
    var params = {
    ...
    if (!options.onDemand && options.BillingMode != "PAY_PER_REQUEST" && !params.StreamSpecification) {
        params.ProvisionedThroughput = { ReadCapacityUnits: r, WriteCapacityUnits: w };
    }
    if (options.add) {
        ...
        for (var name in options.add) {
            ...
            if (!index.ProvisionedThroughput && !options.onDemand) {
                index.ProvisionedThroughput = { ReadCapacityUnits: r, WriteCapacityUnits: w };
            }
...
aws.ddbUpdateItem = function(name, keys, item, options, callback)
{
    ...
    if (typeof item == "object") {
        ...
        for (let p in item) {
            ...
            if (/^[0-9]|/.test(p) || this.ddbReserved[p.toUpperCase()]) {
                names["#c" + c] = p;
                p = "#c" + c++;
            }
...
aws.ddbGetItem = function(name, keys, options, callback)
{
    ...
    this.queryDDB('GetItem', params, options, function(err, rc) {
        rc.Item = rc.Item ? aws.fromDynamoDB(rc.Item) : null;
        if (typeof callback == "function") callback(err, rc);
    });
}
...
aws.ddbQueryTable = function(name, condition, options, callback)
{
    ...
    this.queryDDB('Query', params, options, function(err, rc) {
        rc.Items = rc.Items ? aws.fromDynamoDB(rc.Items) : [];
        if (typeof callback == "function") callback(err, rc);
    });
}
...
aws.ddbTransactWriteItems = function(items, options, callback)
{
    ...
    lib.isArray(items, []).forEach(function(x) {
        ...
        case "check":
            ...
            if (x.opts.expected) {
                aws.buildExpression(obj, "ConditionExpression", x.opts.expected, x.opts, x.opts.expectedJoin);
            }
            if (x.opts.expr) {
                obj.ConditionExpression = x.opts.expr;
            }
            if (x.opts.names) {
                obj.ExpressionAttributeNames = aws.toDynamoDB(x.opts.names);
            }
            if (x.opts.values) {
                obj.ExpressionAttributeValues = aws.toDynamoDB(x.opts.values);
            }
            break;
        case "update":
            obj = aws.ddbUpdateItem(x.table, x.obj, x.options);
            break;
        case "put":
            obj = aws.ddbPutItem(x.table, x.obj, x.options);
            break;
        case "del":
            obj = aws.ddbDeleteItem(x.table, x.obj, x.options);
            break;
        default:
            return;
        }
        for (const p in opts) if (p[0] >= 'A' && p[0] <= 'Z') obj[p] = options[p];
        if (obj.ReturnValues) {
            obj.ReturnValuesOnConditionCheckFailure = "ALL_OLD";
            delete obj.ReturnValues;
        }
        params.TransactWriteItems.push(obj);
    });
