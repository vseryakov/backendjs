...
aws.queryExpression = function(params, obj, options, join)
{
    ...
    function _checkName() {
        if (!aws.ddbNameRx.test(name) || aws.ddbReserved[name.toUpperCase()]) {
            for (var p in params.ExpressionAttributeNames) {
                if (params.ExpressionAttributeNames[p] == name) name = params.ExpressionAttributeNames[p];
            }
            if (name[0] != "#") {
    ...
    for (var name in obj) {
        ...
        case 'begins_with':
            ...
            _checkName();
            expr.push("begins_with(" + name + ", :v" + _addValue(val) + ")");
            break;
        }
    }
