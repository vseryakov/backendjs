    ...
    lib.everySeries([
        ...
        function(next, err, shards) {
            ...
            _processShards(shards, stream, options, processcallback, (err) => {
                logger.logger(err ? "error" : shards.list.length && processcallback("running", stream, options) ? "warn" : "debug", "ddbProcessStreamShards:", err, stream, shards);
                next();
            });
        },
        function(next, err) {
