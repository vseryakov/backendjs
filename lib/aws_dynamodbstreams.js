...
aws.ddbShardRetryCount = 3;
aws.ddbShardRetryTimeout = 500;
aws.ddbShardRetryMaxTimeout = 3000;
aws.ddbShardRetryClosedCount = 100;
aws.ddbShardConcurrency = 5;
...
aws.ddbProcessShardRecords = function(stream, shard, options, processcallback, callback)
{
    lib.everySeries([
        ...
        function(next, err) {
            ...
            aws.ddbGetShardIterator(q, options, next);
        },
        function(next, err, it) {
            if (err || !it || !it.ShardIterator) return next(err);
            var retries = lib.toNumber(options.shardRetryCount) || aws.ddbShardRetryCount;
            var retriesClosed = lib.toNumber(options.shardRetryClosedCount) || aws.ddbShardRetryClosedCount;
            var timeout = lib.toNumber(options.shardRetryTimeout) || aws.ddbShardRetryTimeout;
            var maxTimeout = lib.toNumber(options.shardRetryMaxTimeout) || aws.ddbShardRetryMaxTimeout;
            shard.retries = 0;
...
function _prepareShards(info, stream, options, processcallback)
{
    ...
    info.Shards.forEach((x) => {
        if (x.ParentShardId && !shards.map[x.ParentShardId]) {
            var p = { ShardId: x.ParentShardId, SequenceNumberRange: {} };
            if (processcallback("shard-check", p, options)) shards.map[x.ParentShardId] = p;
        }
        if (processcallback("shard-check", x, options)) shards.map[x.ShardId] = x;
    });
...
{
    shards.options = {
        max: options.shardConcurrency || aws.ddbShardConcurrency,
        interval: options.shardRetryTimeout || aws.ddbShardRetryTimeout,
        timeout: (options.shardRetryMaxTimeout || aws.ddbShardRetryMaxTimeout) * shards.list.length,
    };
    lib.forEachItem(shards.options,
        function(next) {
...
aws.ddbProcessStreamShards = function(stream, options, processcallback, callback)
{
    lib.everySeries([
        ...
        function(next, err, shards) {
            ...
            logger.debug("ddbProcessStreamShards:", "start:", stream, shards);
            _processShards(shards, stream, options, processcallback, (err) => {
                logger.logger(err ? "error" : shards.list.length && processcallback("running", stream, options) ? "warn" : "debug", "ddbProcessStreamShards:", "done:", err, stream, shards);
                next();
            });
        },
        function(next, err) {
...
aws.ddbProcessStream = function(stream, options, processcallback, callback)
{
    lib.series([
        ...
        function(next) {
            ...
            aws.ddbDescribeTable(options.table, options, (err, res) => {
                if (!err) {
                    stream.StreamArn = res.Table.LatestStreamArn;
                }
                next(err);
            });
        },
