...
aws.ddbShardRetryMaxTimeout = 5000;
                ...
                function(next2) {
                    aws.ddbGetShardRecords(it, options, function(err, res) {
                        ...
                        if (!res.Records.length && it.ShardIterator) {
                            ...
                            if (!shard.SequenceNumberRange.EndingSequenceNumber && retries > 0 && c++ > retries) return next();
                            return setTimeout(next2, (t *= 2) % maxtimeout);
                        }
                        c = 0;
                        t = timeout;
