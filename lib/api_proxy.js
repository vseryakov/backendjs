...
api.getProxyTarget = function(req)
{
    ...
    if (host) {
        for (const p in this.proxyHost) {
            if (lib.testRegexpObj(host, this.proxyHost[p])) return { target: p, xfwd: true };
        }
    }
    // Proxy by url patterns
    var url = req.url;
    for (const p in this.proxyUrl) {
        if (lib.testRegexpObj(url, this.proxyUrl[p])) return { target: p, xfwd: true };
    }
