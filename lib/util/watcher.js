...
mod.start = function(options, callback)
{
    ...
    for (let dir of app.path.modules) {
        ...
        if (!lib.includes(this.dir, dir)) this.dir.push(dir);
    }
...
mod.startProcess = function()
{
    ...
    logger.log('startProcess:', app.role, 'version:', app.version, 'home:', app.home, 'port:', api.port, 'uid:', lib.getuid(), 'pid:', process.pid, 'app:', app.env);
}
...
function _rebuild(file)
{
    ...
    let mode = path.basename(file.name);
    if (/^dir/.test(mod.buildMode)) {
        const dirs = path.dirname(file.name).split("/");
    ...
    mod._wtimer = setTimeout(() => {
        ...
        lib.execProcess(mod.build + " " + file.name, (err, stdout, stderr) => {
            mod._web[mode] = undefined;
            mod._wtimer = undefined;
