 ...
 *
 * Watches local log files (and optionally AWS CloudWatch Logs) for matching patterns,
...
const logwatcher = {
    ...
    ignore: {},
    once: {},
    local: { newline: 1, size: 1024*1024*100 },
    subject: "logwatcher: @counter@ @type@s: @hostname@/@ipaddr@/@instance.id@/@instance.tag@/@instance.roles@/@instance.region@",
     ...
     * @property {string} [file] Absolute path to the log file.
     * @property {string} [name] Name of the property in a module that contains the file path (resolved via {@link lib.objGet}).
     * @property {string} [mod] Module name inside `modules` to use when resolving `name` (defaults to `"app"`).
     * @property {RegExp} [ignore] Local ignore regexp for this file.
     * @property {RegExp} [match] Local match regexp for this file (if present it forces matching for this entry).
     * @property {string} [type="all"] Channel to report to when `match` matches (e.g. `"error"`, `"warning"`, `"all"`).
     * @var files
     */
    files: [
        { name: "logFile" },
...
logwatcher.run = function(options, callback)
{
    ...
    this.prepare(options, (err, opts) => {
        this[this.cwRun ? "runCw" : "runLocal"](opts, (err) => {
            this.running = 0;
            this.notify(opts, callback);
        });
    });
}

logwatcher.runLocal = function(options, callback)
{
    lib.forEach(this.files, (log, next) => {
        var file = log.file;
        if (!file && log.name) {
            file = lib.objGet(modules[log.mod || "app"], [log.name]);
        }
        if (!file) return next();
        ...
        fs.stat(file, (err, st) => {
            ...
            fs.open(file, "r", (err, fd) => {
                ...
                fs.read(fd, buf, 0, buf.length, Math.max(0, pos), (err, nread, buffer) => {
                    ...
                    this.match(options, buffer.slice(0, nread).toString().split("\n"), log);

                    if (options?.dryrun) return next();

                    // Save current size to start from next time
                    this.save(file, st.size, () => (next()));
                });
            });
        });
    }, callback, true)
...
logwatcher.runCw = function(options, callback)
{
    logger.debug('cwRun:', this.name, this.cwGroups, this.cwFilter, options);

    lib.forEach(Object.keys(this.cwGroups), (name, next) => {
        var q = {
            name: name,
            filter: typeof this.cwFilters[name] != "undefined" ? this.cwFilters[name] : this.cwFilter,
            stime: lib.toNumber(options.last_pos[name]) || (Date.now() - 3600000),
            etime: Date.now(),
            timeout: options.interval,
        };
        aws.cwlFilterLogEvents(q, (err, rc) => {
            logger.debug('cwRun:', this.name, err, q, "matches:", rc.events.length);
            if (err) return next();

            var log = this.cwMatches[name] ? { match: this.cwMatches[name], type: this.cwGroups[name] } : null;
            var lines = rc.events.map((x) => (x.message));
            this.match(options, lines, log);

            if (options?.dryrun) return next();

            this.save(name, q.etime, () => {
                if (!this.local?.file) return next();

                lib.writeLines(this.local?.file, lines, this.local, (err) => {
                    if (err) logger.error("cwRun:", this.name, q, "save:", err);
                    next();
                });
            });
        });
...
logwatcher.prepare = function(options, callback)
{
    var opts = {
    ...
    if (!this.table) return callback(null, opts);
...
logwatcher.save = function(file, pos, callback)
{
    if (!this.table) return lib.tryCall(callback);

    db.put(this.table, { name: 'logwatcher:' + file, value: pos }, { pool: this.pool }, (err) => {
        if (err) logger.error('save:', this.name, file, err);
        lib.tryCall(callback, err);
    });
}

logwatcher.match = function(options, lines, log)
{
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (log && lib.testRegexp(lines[i], log.ignore) || matchObj(options.ignore, lines[i])) {
            ...
            logger.debug("match:", this.name, "ignore", log, "LINE:", lines[i]);
            continue;
        }
        ...
        if (chan) {
            ...
            chan = chan == "any" && i - options.last_line <= logwatcher.anyRange ? (options.last_chan || "any") : chan;
...
logwatcher.notify = function(options, callback)
{
    var errors = options.errors || {};
    var once = options.once || {};
    var seen = options.seen || {};
    // From address, use current hostname
    if (!this.from) this.from = "logwatcher@" + app.domain;

    var keys = lib.objKeys(errors);
    if (!keys.length) return lib.tryCall(callback, null, options);

    lib.forEvery(keys, (type, next) => {
        if (lib.isEmpty(errors[type])) {
            return next(null, options);
        }

        logger.log('notify:', this.name, type, options.counter[type], 'matches found, sending to', this.send[type]);
        var channel = this.send[type];
        if (!channel) {
            return next(null, options);
        }
        ...
        var subject = lib.toTemplate(this.subject, [{ type: type, hostname: os.hostname(), counter: options.counter[type] }, app]);
        var d = channel.split("://");

        switch (d[0]) {
        case "console":
            d[1] = d[1] || "log";
            if (typeof console[d[1]] == "function") console[d[1]](subject + "\n" + text);
            next(null, options);
            break;

        case "logger":
            logger.logger(d[1], subject, text);
            next(null, options);
            break;
        ...
        case "https":
            lib.fetch(channel, {
                  headers: {
                     ...
                     "bk-type": type,
                     "bk-ipaddr": app.ipaddr,
                     "bk-host": os.hostname(),
                     "bk-instance-id": app.instance.id,
                     "bk-instance-tag": app.instance.tag,
                     "bk-instance-region": app.instance.region,
                     "bk-instance-roles": app.instance.roles,
                  },
                  method: "POST",
                  retryCount: 3,
                  retryOnError: 1,
