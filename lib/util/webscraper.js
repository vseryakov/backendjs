...
async function getScreenshot(options, page)
{
    ...
    try {
        ...
        files.store(image1, `${root}/page.png`);
    } catch (e) {
    ...
    try {
        ...
        files.store(image2, `${root}/scroll.png`);

        const image3 = await image.stitch([image1, image2]);
        files.store(await image3.png().toBuffer(), `${root}/full.png`);
...
async function getDetails(options, page)
{
    try {
        ...
        files.store(Buffer.from(html), `${options.root}/page.html`);

        const text = await page.$eval("body", (elements => elements.innerText));
        files.store(Buffer.from(text), `${options.root}/page.txt`);
    ...
    try {
        ...
        for (const link of links) {
            ...
            if (link[0] == "text/calendar") {
                ...
                if (options.ical) {
                    files.store(Buffer.from(data), `${options.root}/ld.ical`);
                }
            } else
            ...
            if (link[0] == "application/ld+json") {
                ...
                if (data) {
                    ...
                    files.store(Buffer.from(lib.stringify(data, null, 2)), `${options.root}/ld.json`);
                }
            }
...
async function getLogos(options, page)
{
    ...
    if (options.logo) {
        ...
        if (!err) files.store(data, `${options.root}/logo.png`);
    }
}
