...
image.convert = async function(input, item)
{
    ...
    if (item.blur_sigma) {
        if (item.blur_sigma == "auto") {
            const stats = await input.stats();
            item.blur_sigma = stats.entropy > 7 ? 5 : stats.entropy > 6 ? 3 : 1;
        }
        ...
        <svg width="${width}" height="${height}">
            <defs>
                <radialGradient id="grad" cx="50%" cy="50%" r="100%" fx="50%" fy="50%">
                    ...
                    <stop offset="80%" stop-color="${gcolor}" stop-opacity="0.3"/>
                    <stop offset="90%" stop-color="${gcolor}" stop-opacity="0.1"/>
                    <stop offset="100%" stop-color="${gcolor}" stop-opacity="0"/>
                </radialGradient>
            </defs>
            <rect cx="${Math.round(width/2)}" cy="${Math.round(height/2)}" rx="${Math.round(width/2.5)}" ry="${Math.round(height/2.5)}" fill="url(#grad)" />
        </svg>`);
...
image.createOutline = function(item)
{
    ...
    const text = lib.isString(item.text || item.outline).replaceAll("\\n", "\n").trim();
...
image.composite = async function(items, defaults)
{
    ...
    items = lib.isArray(items, []).
                ...
                filter(item => (item.file || item.data || item.text || item.outline || item.background));
