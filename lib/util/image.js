 ...
 *
 * Computes the effective placement of `item` on top of `bgitem` (using `_top`/`top` and `_left`/`left`,
 * normalized via `this.toNumber()`), then crops the item's buffer if any part would overflow the
 * background width/height. If the item is completely outside the background, it marks it as skipped.
 *
 * Side effects:
 ...
 * @async
 * @memberof module:image
 * @this {*} Expects `this.toNumber(value, max)` to be available.
 ...
 * @param {(number|string)} [item._top] Top position (relative to background).
 * @param {(number|string)} [item.top] Top position (relative to background).
 * @param {(number|string)} [item._left] Left position (relative to background).
...
image.clip = async function(item, bgitem)
{
    ...
    let top = this.toNumber([item._top, item.top], height);
    let left = this.toNumber([item._left, item.left], width);
    ...
    const region = image.region(item, width, height);
    top = top || region.top;
    left = left || region.left;

    let w, h;
    if (top + item._meta.height > height) h = height - top;
    if (left + item._meta.width > width) w = width - left;
    if (!w && !h) return;

    const clip = { top: 0, left: 0, width: w || item._meta.width, height: h || item._meta.height };

    logger.debug("clip:", image.name, item.id, item._meta.width, item._meta.height, "c:", top, left, w, h, "region:", region);

    const img = sharp(await sharp(item._buffer).extract(clip).toBuffer());
...
image.stitch = async function(images)
{
    ...
    images = await Promise.allSettled(images.map(async (img) => {
        img = await image.metadata(img);
        buffers.push({ input: img.buffer, left: 0, top });
        top += img.meta.height;
        return img;
    }));
    logger.debug("stitch:", image.name, images);

    const bg = sharp({
            create: {
                width: images.reduce((x, y) => (Math.max(x, y.value?.meta.width)), 0),
                height: images.reduce((x, y) => (x + y.value?.meta.height), 0),
                channels: 4,
...
image.createGradient = async function(item, bgitem)
{
    ...
    if (!gcolor) {
        var ncolor = color.negate(color.hex2rgb(item.color || item._color || "#fff"));
...
image.createOutline = async function(item, bgitem)
{
    ...
    const dcolor = this.toColor(item.dilate_color || fcolor, item.dilate_alpha || 40);
    ...
    const filter = item.dilate_radius || item.dilate_alpha || item.dilate_color ?
       `<filter id="filter">
            <feMorphology in="SourceAlpha" result="DILATED" operator="dilate" radius="${item.dilate_radius || 5}"></feMorphology>
            <feFlood flood-color="${dcolor}" result="FLOOD"></feFlood>
    ...
    const lines = lib.split(lib.isString(item.text).replaceAll("\\n", "\n"), "\n");
    ...
    const results = await Promise.allSettled(lines.map(async (line) => {
        ...
        line = line.replace(/<([^>]+)>/gi, (tag, name) => {
            ...
            switch (name) {
            case "b": return `<tspan font-weight="bold">`
        ...
        return sharp(Buffer.from(svg)).trim({ lineArt: true }).toBuffer();
    }));
...
image.createText = function(item, bgitem)
{
    ...
    for (const p of ["_color", "color", "bgcolor", "size", "weight", "style", "stretch",
        ...
        let val = item[p];
        if (val) {
            if (p.endsWith("alpha") && val.at(-1) != "%") val += "%"; else
            if (p == "size" && lib.isNumeric(val)) {
 ...
 * @param {number} [item.outline] - trigger outlined text with given stroke width
 * @param {string} [item.outline_color] - outline fill color
 * @param {number} [item.outline_alpha] - opacity for the outline fill color
