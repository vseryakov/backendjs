...
image.detect = async function(options, bgimage)
{
    ...
    const stats = await input.stats(bgimage, region);
    ...
    case "yiq":
        stats.color = color.yiq(c).l > 128 ? options.black || image.black : options.white || image.white;
        break;
...
image.composite = async function(items, defaults)
{
    ...
    const buffers = items.slice(1).map(x => ({
        input: x.buffer,
        gravity: x.gravity || undefined,
        blend: x.blend || undefined,
        top: x.top || undefined,
        left: x.left || undefined
    }));
