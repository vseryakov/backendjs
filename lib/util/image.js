...
image.stats = async function(input, region)
{
    ...
    logger.debug("stats:", input.name, region, "stats:", stats);
    return stats;
}
...
image.detect = async function(options, bgimage)
{
    ...
    case "complement":
        stats.color = color.hex(color.rotate(color.negate(c)));
        break;

    case "yiq":
        stats.color = color.yiq(c).y > 128 ? options.black || image.black : options.white || image.white;
        break;

    case "wcag":
        stats.color = color.rluminance(c) > 0.179 ? options.black || image.black : options.white || image.white;
        break;
    ...
    default:
        stats.color = color.hex(color.negate(c));
    }
...
image.gradient = function(item)
{
    ...
    const ncolor = color.negate(tcolor);
    let gcolor = color.hex(ncolor);
    const contrast = color.contrast(tcolor, ncolor);

    if (contrast < 1.5) gcolor = color.hex(color.lighter(ncolor, 1));

    logger.debug("gradient:", image.name, item.id, width, height, "c:", item.color, tcolor, "n:", ncolor, gcolor, "l:", color.yiq(tcolor).y, color.yiq(ncolor).y, contrast)

    return Buffer.from(`
    <svg width="${width}" height="${height}">
      <defs>
        <radialGradient id="grad" cx="50%" cy="50%" r="85%" fx="50%" fy="50%">
      ...
      <ellipse cx="${Math.round(width/2)}" cy="${Math.round(height/2)}" rx="45%" ry="45%" fill="url(#grad)"  />
    </svg>`);
}
...
image.composite = async function(items, defaults)
{
    ...
    await Promise.all(other.map(async (item) => {
        ...
        if (item.text && !item.color) {
            const stats = await image.detect(item, bgimage);
        ...
        if (item.text || item.gradient) {
            const gradient = await sharp(image.gradient(item)).
