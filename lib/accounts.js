...
const mod = {
    ...
    deps: "-",
    args: [
        { name: "table", descr: "Table to use for user users" },
...
mod.configureWeb = function(options, callback)
{
    ...
    this.configureWebsocketAPI(options);
...
mod.configureAccountAPI = function(options)
{
    ...
    api.app.post(/^\/account\/update$/, function(req, res, next) {
        ...
        mod.update(req, options, (err, data) => {
            api.sendJSON(req, err, data);
        });
    });

    api.app.all(/^\/account\/get$/, function(req, res, next) {
        var options = api.getOptions(req);
        options.cleanup = mod.table;
        options.cleanup_strict = 1;

        mod.get(req, options, (err, data, info) => {
            api.sendJSON(req, err, data);
        });
    });
}

mod.configureWebsocketAPI = function(options)
{
    if (this.nowebsocket) return;

    api.app.all(/^\/account\/(ws|ws\/query|ws\/account)$/, function(req, res, next) {
        var options = api.getOptions(req);

        core.runMethods("bkWebsocketRequest", { wsid: req.wsid, account: req.account, query: req.query, options }, () => {
...
mod.add = function(req, options, callback)
{
    if (typeof options == "function") callback = options, options = null;
    ...
    lib.series([
        ...
        function(next) {
            mod.addUser(req.query, options, (err, row) => {
                if (!err) {
...
mod.update = function(req, options, callback)
{
    if (typeof options == "function") callback = options, options = null;
    ...
    lib.series([
        ...
        function(next) {
            mod.updateUser(req.query, options, next);
        },
        function(next) {
...
mod.del = function(req, callback)
{
    var started = Date.now();

    lib.series([
        function(next) {
            mod.getUser(req.account, req.options, next);
        },
        function(next, user) {
        ...
        function(next) {
            ...
            mod.updateUser({ id: req.account.id, login: req.account.login, type: req.account.type }, req.options, next);
        },
        function(next) {
            delete req.stopOnError;
        ...
        function(next) {
            if (lib.isFlag(req.options.keep, ["all", "account", mod.table])) return next();
            mod.delUser(req.account, req.options, next);
        },
     ], (err) => {
...
require(__dirname + "/accounts/users")
require(__dirname + "/accounts/utils")
require(__dirname + "/accounts/passkey")
