 ...
 * @param {function} [create] - method to be called to return a new resource item, takes 1 argument, a callback as `function(err, item)`
 * @param {function} destroy - method to be called to destroy a resource item
 * @param {function} reset - method to be called just before releasing an item back to the resource pool, this is a chance to reset the item to the initial state
 * @param {function} validate - method to verify active resource item, return false if it needs to be destroyed
 * @param {function} init - method to cal on pool.init, it may be called multiple times
 * @param {function} shutdown - method to call on pool shutdown to clear other resources
...
module.exports = class Pool {
    ...
    init(options)
    {
        ...
        if (this._create && this._pool.idle > 0 && (idle != this._pool.idle || !this._pool.interval)) {
            clearInterval(this._pool.interval);
            this._pool.interval = setInterval(this._timer.bind(this), this._pool.idle/2);
    ...
    */
    ...
    release(item)
    {
        ...
        for (const id in this._pool.queue) {
            this._pool.queue_count--;
    ...
    {
        return {
            avail: this._pool.avail.length,
            busy: this._pool.busy.length,
            queue: this._pool.queue_count,
            min: this._pool.min,
            max: this._pool.max,
            max_queue: this._pool.max_queue,
            idle: this._pool.idle,
        };
    }
    ...
    */
    shutdown(options, callback)
    {
        ...
        const timer = setInterval(() => {
            if (this._pool.busy.length && (!options?.timeout || Date.now() - this._pool.time < options?.timeout)) return;
            clearInterval(timer);
            callback();
        }, 50);
    }
