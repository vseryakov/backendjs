...
db.cassandraInitPool = function(options)
{
    ...
    pool.connect = function(options, callback) {
        var self = this;
        var hosts = corelib.strSplit(options.db).map(function(x) { return url.parse(x); });
    ...
    pool.nextToken = function(client, req, rows, opts) {
        if (opts.count > 0 && rows.length == opts.count) {
            ...
            return keys.map(function(x) { return corelib.newObj(x, rows[rows.length-1][x]) });
        }
        return null;
    }
...
db.cassandraQuery = function(text, values, options, callback)
{
    ...
    try {
        this.cql(text, corelib.cloneObj(values), function(err, results) {
