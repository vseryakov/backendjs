...
Pool.prototype.open = function(callback)
{
    ...
    client.query = function() { self.clientQuery.apply(client, arguments) }
...
Pool.prototype.clientQuery = function(text, values, options, callback)
{
    this.execute(text, values ? lib.cloneObj(values) : null, options, function(err, result) {
        if (err || !result || !result.rows) return callback(err, []);
        var rows = [];
