...
aws.ec2RunInstances = function(options, callback)
{
    ...
    this.queryEC2("RunInstances", req, options, function(err, rc) {
        ...
        if (!info.instances.length) return lib.tryCall(callback, err, rc);
        info.instanceId = info.instances[0].instanceId;

        // Dont wait for instance if no additional tasks requested
        if (options.noWait || !(options.waitRunning || info.name || info.tags || info.elasticIp || info.elbName || info.alarms)) {
...
aws.ec2AfterRunInstances = function(options, callback)
{
    lib.series([
        function(next) {
            if (!options.instanceId) return next();
            aws.ec2WaitForInstance(options.instanceId, "running", options, next);
        },
        function(next) {
        ...
        function(next) {
            ...
            lib.forEachSeries(options.instances, function(item, next2) {
                lib.forEachSeries(options.alarms, function(alarm, next3) {
                    ...
                    aws.cwPutMetricAlarm(lib.objExtend(alarm, options), next3);
                }, next2);
            }, next);
        },
    ], callback);
...
aws.ssmWaitForCommand = function(cmdId, instanceId, options, callback)
{
    ...
    var expires = Date.now() + lib.toNumber(options.waitTimeout, { dflt: 30000, min: 100 });
