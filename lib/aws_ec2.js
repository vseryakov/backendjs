...
aws.ec2RunInstances = function(options, callback)
{
    ...
    var tags, iname = options.name && options.name.indexOf("%i") > -1 ? options.name : null;
    if (options.name && !iname) {
    ...
    logger.debug('runInstances:', this.name, req, options, tags);
    this.queryEC2("RunInstances", req, options, function(err, obj) {
        ...
        if (!items.length) return lib.tryCall(callback, err, obj);
        ...
        if (!iname &&
            ...
            return lib.tryCall(callback, err, obj);
        }
        var instanceId = items[0].instanceId;
        var retryOpts = {
            retryCount: lib.toNumber(options.retryCount, { min: 5 }),
            retryTimeout: lib.toNumber(options.retryTimeout, { min: 500 }),
            retryOnError: 1,
        };

        lib.series([
           function(next) {
               aws.ec2WaitForInstance(instanceId, "running", options, next);
           },
           function(next) {
               // Set tag name for all instances
               if (!iname && !tags) return next();
               lib.forEachSeries(items, function(item, next2) {
                   if (iname) tags.Name = iname.replace("%i", lib.toNumber(item.amiLaunchIndex) + 1)
                   aws.ec2CreateTags(item.instanceId, tags, retryOpts, next2);
               }, next);
           },
           function(next) {
               // Add to the ELB
               if (!options.elbName) return next();
               aws.elbRegisterInstances(options.elbName, items.map(function(x) { return x.instanceId }), retryOpts, next);
           },
           function(next) {
               // Elastic IP
               if (!options.elasticIp) return next();
               var opts = lib.objExtend({ subnetId: req.SubnetId || req["NetworkInterface.0.SubnetId"] }, retryOpts);
               aws.ec2AssociateAddress(instanceId, options.elasticIp, opts, next);
           },
           function(next) {
               // CloudWatch alarms
               ...
               lib.forEachSeries(items, function(item, next2) {
                   lib.forEachSeries(options.alarms, function(alarm, next3) {
                       ...
                       lib.objExtend(alarm, retryOpts);
                       aws.cwPutMetricAlarm(alarm, next3);
                   }, next2);
               }, next);
           },
        ], function() {
            lib.tryCall(callback, err, obj);
        });
    });
}
...
aws.ec2WaitForInstance = function(instanceId, status, options, callback)
{
    ...
    var waitDelay = lib.toNumber(options.waitDelay, { dflt: 10000, min: 5000 });
    var opts = {
        retryCount: lib.toNumber(options.retryCount, { min: 3 }),
        retryTimeout: lib.toNumber(options.retryTimoeut, { min: 1000 }),
        retryOnError: 1
    };
    lib.doWhilst(
      function(next) {
          aws.queryEC2("DescribeInstances", { 'Filter.1.Name': 'instance-id', 'Filter.1.Value.1': instanceId }, opts, function(err, rc) {
              if (err) return next(err);
              state = lib.objGet(rc, "DescribeInstancesResponse.reservationSet.item.instancesSet.item.instanceState.name");
              setTimeout(next, num++ ? waitDelay : 0);
          });
      },
      function() {
...
aws.ec2DescribeInstances = function(options, callback)
{
    ...
    this.queryEC2("DescribeInstances", req, function(err, rc) {
...
// The name is as tring or an object with tags. The options also may contain tags property which is an object with tag key and value
//
// Example
//
...
aws.ec2CreateTags = function(id, name, options, callback)
{
    ...
    if (options && options.tags) {
        for (const p in options.tags) {
            tags["ResourceId." + i] = id;
            tags["Tag." + i + ".Key"] = p;
            tags["Tag." + i + ".Value"] = String(options[p]);
            i++;
        }
    }
