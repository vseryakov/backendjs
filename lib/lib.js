...
lib.toTemplate = function(text, obj, options)
{
    ...
    while (str) {
        ...
        if (/^if/.test(tag)) {
            ...
            var d = tag.match(/^(if|ifeq|ifgt|ifge|iflt|ifle|ifnot|ifall|ifstr) ([a-zA-Z0-9]+) +(.+)$/)
            if (!d) continue;
            var ok, val = null;
...
lib.isTimeRange = function(time1, time2, tz)
{
    ...
    if (typeof time1 == "string") {
        var d = time1.match(/^([0-9]+):([0-9]+) *([ampAMP]+)?/);
        if (d) {
            var h1 = lib.toNumber(d[1]);
            var m1 = lib.toNumber(d[2]);
            switch (d[3]) {
            case "am":
            case "AM":
                if (h1 > 12) h1 -= 12;
    ...
    if (typeof time2 == "string") {
        var d = time2.match(/^([0-9]+):([0-9]+) *([ampAMP]+)?/);
        if (d) {
            var h1 = lib.toNumber(d[1]);
            var m1 = lib.toNumber(d[2]);
            switch (d[3]) {
...
lib.readFile = function(file, options, callback)
{
    ...
    function onError(err) {
        var data = "";
        if (options) {
            if (options.logger && !(options.missingok && e.code == "ENOENT")) logger.logger(options.logger, 'readFile:', file, e.stack);
