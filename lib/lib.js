...
lib.toParams = function(query, schema, options)
{
    ...
    for (var name in schema) {
        ...
        if (typeof opts.novalue == "object") {
            if (rc[n] === rc[opts.novalue.name] || rc[n] === opts.novalue.value) delete rc[n];
        } else
        if (rc[n] === opts.novalue) delete rc[n];
    }
    return rc;
}
...
{
    return list && Array.isArray(list) && list.indexOf(name) > -1;
}
...
{
    return type && /^(int|smallint|bigint|now|counter|real|float|double|numeric|number|decimal|long)/i.test(String(type).trim());
}
...
// All properties in an object `obj` must match all properties in the object `condition`, each value in `condition` is treated as RegExp,
