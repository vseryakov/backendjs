...
lib.suuid = function(prefix, options)
{
    ...
    var key = (options.salt || "") + (options.min || 0) + (options.alphabet || this.base36);
    if (!this.hashids[key]) {
        this.hashids[key] = new Hashids(options.salt || this.uuid(), lib.toNumber(options.min), options.alphabet || this.base36);
        this.hashids[key]._counter = this.randomUShort();
    }
    var tm = bkutils.getTimeOfDay(2);
    if (++this.hashids[key]._counter > 65535) this.hashids[key]._counter = 1;
    var s = this.hashids[key].encode(tm.tv_sec, tm.tv_usec, this.hashids[key]._counter);
