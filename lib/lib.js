...
lib.isTimeRange = function(time1, time2, tz)
{
...
lib.objDescr = function(obj, options)
{
    ...
    if (typeof obj == "string" || typeof obj == "number" || typeof obj == "boolean") {
        if (!this.isEmpty(v) || options.keepempty) rc = String(obj);
        return rc;
    }
    for (var p in obj) {
        if (options.ignore && typeof options.ignore.test == "function" && options.ignore.test(p)) continue;
        if (options.allow && typeof options.allow.test == "function" && !options.allow.test(p)) continue;
        var v = obj[p];
        if (typeof v == "undefined" || typeof v == "function") continue;
        if (typeof v == "function" && options.nofunc) continue;
        if (Buffer.isBuffer(v)) {
            if (v.length || options.keepempty) {
                rc += (rc ? ", " : "") + p + ":[" + v.length + "] " + v.slice(0, options.length || 32).toString("hex");
                n++;
            }
        } else
        if (Array.isArray(v)) {
            if (v.length || options.keepempty) {
                if (options._depth >= (options.depth || 4)) {
                    ...
                    options._depth++;
                    rc += (rc ? ", " : "") + p + ":[" + v.length + "] " + v.slice(0, options.count || 5).map(function(x) { return lib.objDescr(x, options) });
                    n++;
                    options._depth--;
                }
            }
        ...
        if (typeof v == "string") {
            if (v || options.keepempty) {
                rc += (rc ? ", " : "") + p + ":" + v.slice(0, options.length || 256);
                n++;
            }
        } else {
            if (!this.isEmpty(v) || options.keepempty) {
                rc += (rc ? ", " : "") + p + ":" + (this.isEmpty(v) ? "" : v);
                n++;
            }
        }
