...
lib.encodeURIComponent = function(str)
{
    try {
        ...
        logger.error("encodeURIComponent:", str);
    }
}
...
lib.toFormat = function(format, data, options)
{
    ...
    default:
        var json = "";
        for (var i = 0; i < rows.length; i++) {
            json += JSON.stringify(allow ? allow.reduce(function(x,y) { x[map[y] || y] = rows[i][y] || ""; return x }, {}) : rows[i]) + "\n";
        }
        return json;
    }
}
...
lib.isTrue = function(val1, val2, op, type)
{
    ...
    case "ilike":
    case "not ilike":
        if (String(val1).toLowerCase() != String(val2).toLowerCase()) return no;
        break;
...
// Split a string into phrases separated by `separators` characters and surrounded by characters in `quotes`.
lib.phraseSplit = function(str, separators, quotes)
{
    return bkutils.strSplit(str, separators, quotes).filter(function(x) { return x.length });
}
