...
{
    return String(name || "").replace(/_/g, " ").split(/[ ]+/).reduce(function(x,y) {
...
{
    return String(str).replace(/([A-Z])/g, function(_, c, index) { return (index ? sep || '-' : '') + c.toLowerCase(); });
}
...
lib.toDate = function(val, dflt, invalid)
{
    ...
    return this.isDate(d) ? d : invalid ? null : new Date(dflt || 0);
}
...
lib.toValue = function(val, type, options)
{
    ...
    case "phone":
        if (typeof val == "number") {
            ...
            if (val < 19999999999 && val > 10000000000) val -= 10000000000;
            return String(val);
        }
        ...
        // Keep US phones without 1
        if (val.length == 11 && val[0] == "1") val = val.substr(1);
        return val;
...
lib.toParams = function(query, schema, options)
{
    ...
    for (var name in schema) {
        ...
        case "obj":
            if (!v && !opts.empty) break;
            o = this.toParams(v || lib.empty, opts.params);
            if (Object.keys(o).length || opts.empty) rc[n] = o;
            break;
        case "array":
            if (!v && !opts.empty) break;
            o = lib.isArray(v, []).map(function(x) {
        ...
        case "email":
        case "phone":
            if (v) v = this.toValue(v, opts.type);
            if (v || opts.empty) rc[n] = v || "";
            break;
        case "string":
        case "text":
...
lib.toTemplate = function(text, obj, options)
{
    ...
    while (str) {
        ...
        if (/^if/.test(tag)) {
            ...
            const d = tag.match(/^(if|ifeq|ifgt|ifge|iflt|ifle|ifnot|ifall|ifstr) ([a-zA-Z0-9]+) +(.+)$/)
            if (!d) continue;
            var ok, val = null;
            for (let i = 0; i < rc.length && !val; i++) val = rc[i][d[2]];
            switch (d[1]) {
            case "if":
        ...
        if (/^[a-zA-Z0-9_]+$/.test(tag)) {
            for (let i = 0; i < rc.length && !v; i++) v = rc[i][tag];
        } else {
            tmpl += "@" + tag + "@";
        }
        if (v) {
...
{
    return type && /^(int|smallint|bigint|now|clock|mtime|counter|real|float|double|numeric|number|decimal|long)/i.test(String(type).trim());
}

// Retyurns true if the given type belongs to the text family of data types
lib.isTextType = function(type)
{
    return !type || /^(text|str|string)/i.test(type)
}
...
// Returns 0 if the current time is not within specified valid time range, invalid range is ignored, no timezone means current timezone.
// `date` must be a list of dates in the format: YYY-MM-DD
lib.isTimeRange = function(time1, time2, tz, date)
{
    ...
    if (date) {
        const d = lib.strftime(now, "%Y-%m-%d");
        if (lib.strSplit(date).indexOf(d) == -1) return 0;
    }
    var h0 = now.getUTCHours();
    var m0 = now.getUTCMinutes();
    if (typeof time1 == "string") {
        const d = time1.match(/^([0-9]+):([0-9]+) *([ampAMP]+)?/);
        if (d) {
            let h1 = lib.toNumber(d[1]);
            const m1 = lib.toNumber(d[2]);
            switch (d[3]) {
            case "am":
            case "AM":
                if (h1 > 12) h1 -= 12;
            ...
            logger.debug("isTimeRange:", "start:", h0, m0, " - ", h1, m1, d[3], "tz:", tz, "now:", now);
            if (h0*100+m0 < h1*100+m1) return 0;
        }
    }
    if (typeof time2 == "string") {
        const d = time2.match(/^([0-9]+):([0-9]+) *([ampAMP]+)?/);
        if (d) {
            let h1 = lib.toNumber(d[1]);
            const m1 = lib.toNumber(d[2]);
            switch (d[3]) {
            ...
            logger.debug("isTimeRange:", "end:", h0, m0, " - ", h1, m1, d[3], "tz:", tz, "now:", now);
            if (h0*100+m0 > h1*100+m1) return 0;
        }
    }
    return 1;
...
lib.forEachLine = function(file, options, lineCallback, endCallback)
{
    ...
    var data = '';
    options.nlines = options.ncalls = 0;
    if (options.split) {
        ...
        fs.read(fd, buffer, 0, buffer.length, pos, function(err, nread) {
            ...
            lib.forEachSeries(lines, function(line, next) {
                ...
                if (options.nlines == 1 && options.header) {
                    if (typeof options.header != "function") return next();
                    if (options.header(line)) return next();
                }
...
lib.whilst = function(test, iterator, callback)
{
    ...
    iterator(function itWhilst(err) {
        ...
        setImmediate(lib.whilst.bind(lib, test, iterator, callback));
    });
};
...
lib.doWhilst = function(iterator, test, callback)
{
    ...
    iterator(function itDoWhilst(err) {
        if (err) return callback(err);
        if (!test()) return callback();
        setImmediate(lib.doWhilst.bind(lib, iterator, test, callback));
    });
}
...
lib.tuuid = function(prefix)
{
    if (!this._hostHash) {
        var b = new Buffer(crypto.createHash('sha512').update(os.hostname(), 'ascii').digest('binary'));
        this._hostHash = new Buffer([b[1], b[3], b[5], (process.pid) & 0xFF, (process.pid >> 8) & 0xFF ]);
        this._hostCounter = 0;
    }
...
lib.strftime = function(date, fmt, options)
{
    ...
    var handlers = {
        ...
        A: function(t) { return (lang ? [lib.__({ phrase: "Sunday", locale: lang }),
                                        ...
                                        'Thursday', 'Friday', 'Saturday' ])[utc ? t.getUTCDay() : t.getDay()]
                                   },
        b: function(t) { return (lang ? [lib.__({ phrase: "Jan", locale: lang }),
                                        ...
                                        'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec' ])[utc ? t.getUTCMonth() : t.getMonth()]
                                   },
        B: function(t) { return (lang ? [lib.__({ phrase: "January", locale: lang }),
                                        ...
                                        'June', 'July', 'August', 'September', 'October', 'November', 'December' ])[utc ? t.getUTCMonth() : t.getMonth()]
                                   },
        ...
        W: function(t) {
            var d = new Date(utc ? t.getUTCFullYear() : t.getFullYear(), 0, 1);
            return zeropad(Math.round(Math.ceil((((t - d) / 86400000) + (utc ? d.getUTCDay() : d.getDay()) + 1) / 7))); },
...
{
    if (typeof str != "string" || !util.isRegExp(rx)) return null;
    var d = str.match(rx);
    return d ? d[index || 1] : null;
}
...
// Remove all whitespace from the begining and end of the given string
lib.trimWhite = function(str)
{
    if (typeof str != "string" || !str) return "";
    if (!this._whitespace) this._whitespace = new RegExp("(^[" + this.whitespace.join("") + "]+)|([" + this.whitespace.join("") + "]+$)", "gi");
    return str.replace(this._whitespace, "");
}
...
{
    return this.objDescr(err || "", { ignore: /^domain$/ }) + " " + (util.isError(err) && err.stack ? err.stack : "");
}
...
//  Example
//
//       var o = lib.objMerge({ a:1, b:2, c:3 }, { c:5, d:1 })
//       o = { a:1, b:2, c:5, d:1 }
...
//
// Options properties:
//  - allow - a regexp which properties are allowed to be mrged
...
lib.objExtend = function(obj, val, options)
{
    ...
    if (this.typeName(arguments[1]) == "object") {
        for (var p in arguments[1]) {
            if (options && options.ignore && typeof options.ignore.test == "function" && options.ignore.test(p)) continue;
            if (options && options.allow && typeof options.allow.test == "function" && !options.allow.test(p)) continue;
            var v = arguments[1][p];
            if (options && options.remove && typeof options.remove.test == "function") p = p.replace(options.remove, "");
            rc[p] = v;
        }
    } else {
        for (var i = 1; i < arguments.length - 1; i += 2) rc[arguments[i]] = arguments[i + 1];
...
// the object depth is limited by `options.depth` or 3 levels deep, the number of properties are limited by options.count or 10,
...
// match it will be output.
lib.objDescr = function(obj, options)
{
    if (typeof obj != "object") {
        if (typeof obj == "string" || typeof obj == "number" || typeof obj == "boolean") {
            if (!this.isEmpty(obj) || (options && options.keepempty)) return String(obj);
        }
        return "";
    }
    ...
    for (var p in obj) {
        ...
        if (typeof v == "function" && !options.func) continue;
        h = hide && hide.test(p);

        if (Buffer.isBuffer(v)) {
            if (v.length || options.keepempty) {
                rc += (rc ? ", " : "") + p + ":[" + v.length + "] ";
                if (!h) rc += v.slice(0, options.length || 32).toString("hex");
                n++;
            }
        } else
        if (Array.isArray(v)) {
            if (v.length || options.keepempty) {
                if (options.__depth >= (options.depth || 4)) {
                    rc += (rc ? ", " : "") + p + ": {...}";
                    n++;
                } else {
                    ...
                    if (options.__seen.indexOf(v) > -1) {
                        ...
                        options.__depth++;
                        rc += (rc ? ", " : "") + p + ":[" + v.length + "] ";
                        if (!h) rc += v.slice(0, options.count || 5).map(function(x) { return lib.objDescr(x, options) });
                        n++;
                        options.__seen.pop();
                        options.__depth--;
                    }
        ...
        if (this.isObject(v)) {
            if (options.__depth >= (options.depth || 4)) {
                ...
                if (options.__seen.indexOf(v) > -1) {
                    ...
                    options.__seen.push(v);
                    options.__depth++;
                    rc += (rc ? ", " : "") + p + ":{ " + (h ? Object.keys(v).length : this.objDescr(v, options)) + " }";
                    n++;
                    options.__seen.pop();
                    options.__depth--;
                }
        ...
        if (typeof v == "string") {
            if (v || options.keepempty) {
                rc += (rc ? ", " : "") + p + ":" + (h ? "..." : v.slice(0, options.length || 256));
                n++;
            }
        } else {
            e = this.isEmpty(v);
...
lib.execProcess = function(cmd, callback)
{
    return child.exec(cmd, function (err, stdout, stderr) {
        if (err) logger.error('execProcess:', cmd, err, stderr);
...
lib.readFile = function(file, options, callback)
{
    ...
    if (offset || (offset === 0 && length > 0)) {
        fs.open(file, 'r', function(err, handle) {
            ...
            fd = handle;
            var data = binary ? Buffer.from("") : "";
            var buffer = new Buffer(Math.min(length, 4096));
