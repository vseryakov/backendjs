...
const lib = {
    ...
    locales: {},
    locale: "",
    base62: "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".split(""),
    base64: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
    uriSafe: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-$",
    baseShort: "0123456789abcdefghijklmnopqrstuvwxyz",
    base64Dict: {},
    uriSafeDict: {},
    whitespace: " \r\n\t\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200A\u202F\u205F\u3000\u008D\u009F\u0080\u0090\u009B\u0010\u0009\u0000\u0003\u0004\u0017\u0019\u0011\u0012\u0013\u0014\u2028\u2029\u2060\u202C",
}
...
{
    var s = '';
    if (Array.isArray(num) && typeof num[0] == "number") num = new Buffer(num);
...
// Returns as short unique id
lib.suuid = function(prefix, options)
{
    if (!this._hashids) {
        this._hashidsCounter = process.pid;
        this._hashids = new Hashids(options && options.salt || this.uuid(), options && options.min || 5, this.baseShort);
    }
    var tm = bkutils.getTimeOfDay(2);
    if (++this._hashidsCounter > 65535) this._hashidsCounter = 1;
    var s = this._hashids.encode(tm.tv_sec, tm.tv_usec, this._hashidsCounter);
...
lib.newError = function(msg, status, code)
{
    ...
    var err = new Error(msg && msg.message || this.__("Internal error occured, please try again later"));
...
lib.objDescr = function(obj, options)
{
    ...
    var rc = "", n = 0, v, h, e;
    if (options.sort && !options.__depth && this.typeName(obj) == "object") {
        obj = Object.keys(obj).sort().reduce((a, b) => { a[b] = obj[b]; return a }, {});
    }
    for (var p in obj) {
        if (ignore && ignore.test(p)) continue;
        if (allow && !allow.test(p)) continue;
        ...
        if (typeof v == "string") {
            if (v || options.keepempty) {
                rc += (rc ? ", " : "") + p + ":";
                if (v.length > options.length) rc += "[" + v.length + "] ";
                rc += h ? "..." : v.slice(0, options.length || 256);
                n++;
            }
        } else {
            e = this.isEmpty(v);
