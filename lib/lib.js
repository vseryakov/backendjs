...
lib.tryCall = function(callback)
{
    ...
    if (callback) logger.warn("tryCall:", arguments);
}
...
//                                                email: { type: "list", datatype: "email" } },
//                                                internal: { ignore: 1 },
//                                                status: { value: "ready" },
...
lib.toParams = function(query, schema, options)
{
    ...
    for (var name in schema) {
        ...
        case "list":
            if (!v) break;
            rc[n] = this[opts.unique ? "strSplitUnique" : "strSplit"](v, opts.separator, opts.datatype);
            if (Array.isArray(opts.values)) rc[n] = rc[n].filter(function(x) { return opts.values.indexOf(x) > -1 });
            if (typeof opts.min == "number" && rc[n].length < opts.min) delete rc[n];
            break;
        case "map":
            if (!v) break;
            ...
            if (!rc[n]) rc[n] = {};
            for (var i = 0; i < list.length -1; i += 2) rc[n][list[i]] = list[i+1];
            break;
        case "obj":
            if (!v) break;
        ...
        case "token":
            ...
            break;
        case "mtime":
            if (v) rc[n] = this.toDate(v).getTime();
            break;
        case "date":
        case "time":
            if (v) rc[n] = this.toDate(v, opts.dflt);
            break;
        case "timestamp":
            if (!v || this.toBool(v)) v = Date.now();
            rc[n] = this.strftime(this.toDate(v), opts.format || "%Y-%m-%d-%H:%M:%S.%L");
            break;
        case "json":
            if (!v) break;
        ...
        default:
            if (!v) break;
            v = String(v);
            if (opts.max && v.length > opts.max) return options && options.null ? null : opts.errmsg || this.__("%s is too long, the max is %s", name, opts.max);
...
lib.toFormat = function(format, data, options)
{
    ...
    var map = options && options.mapping || {};
    ...
    case "xml":
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            xml += (allow || Object.keys(rows[i])).map(function(y) {
                var t = map[y] || y;
                return "<" + t + ">" + String(rows[i][y]).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/'/g, '&apos;').replace(/"/g, '&quot;') + "</" + t + ">\n";
            });
            xml += "</" + tag + ">\n";
        }
        return xml;
    ...
    default:
        var json = "";
        for (var i = 0; i < rows.length; i++) {
            json += lib.stringify(allow ? allow.reduce(function(x,y) { x[map[y] || y] = rows[i][y] || ""; return x }, {}) : rows[i]) + "\n";
        }
        return json;
    }
}
...
lib.toTemplate = function(text, obj, options)
{
    ...
    var d, v, rx = /(@[^@]+@)/g, tags = {}, tag, rc = [];
    if (!Array.isArray(obj)) obj = [obj];
    for (var i = 0; i < obj.length; i++) {
        if (typeof obj[i] == "object" && obj[i]) rc.push(obj[i]);
    }
    while (d = rx.exec(text)) {
        v = null;
        tag = d[1].substr(1, d[1].length - 2);
        for (var i = 0; i < rc.length && this.isEmpty(v); i++) v = rc[i][tag];
        if (v) {
            switch (options && options.encoding) {
            case "url":
    ...
    for (var p in tags) {
        tag = tags[p];
        if (options && Array.isArray(options.allow) && options.allow.indexOf(p.slice(1, -1)) == -1) tag = "";
        if (options && Array.isArray(options.skip) && options.skip.indexOf(p.slice(1, -1)) > -1) continue;
        if (options && Array.isArray(options.only) && options.only.indexOf(p.slice(1, -1)) == -1) continue;
        text = text.replace(new RegExp(p, "g"), tag);
    }
...
lib.isUuid = function(val)
{
    return this.rxUuid.test(val);
}
...
// Returns true if `name` exists in the array `list`, search is case sensitive.
...
lib.isTrue = function(val1, val2, op, type)
{
    ...
    case "not in":
        var list = Array.isArray(val2) ? val2 : this.strSplit(val2);
        if (list.indexOf(this.toValue(val1)) == -1) return no;
        break;
...
lib.isMatched = function(obj, condition)
{
    for (var p in condition) {
        ...
        var rx = condition[p], not = 0;
        if (typeof rx == "string" && rx[0] == "!") not = 1, rx = rx.substr(1);
        if (!util.isRegExp(rx)) rx = new RegExp(rx);
...
// - split - splitevery line before calling the callback, it uses phraseSplit
...
{
    this.forEachSeries(tasks, function itSeries(task, next, data) {
        task(function itNext(err, data) {
            setImmediate(next.bind(null, err, data));
        }, data);
    }, function(err, data) {
...
lib.geoHash = function(latitude, longitude, options)
{
    ...
    var range = this.geoHashRange(minDistance);
...
// Returns the first geohash range greater than given distance
lib.geoHashRange = function(distance)
{
    return this.geoHashRanges.filter(function(x) { return x[1] > distance })[0];
}
...
// Return a bounding box geohash for a given point around distance in km
lib.geoBoundingBox = function(latitude, longitude, distance)
{
    return bkutils.geoBoundingBox(latitude, longitude, distance);
}

// Decode a geohash into lattiude and longitude, returns an array with the coordinates
lib.geoHashDecode = function(geohash)
{
    return bkutils.geoHashDecode(geohash);
}
...
{
    return (prefix || "") + uuid.v4().replace(/[-]/g, '').toLowerCase();
}
...
lib.strSplit = function(str, sep, type)
{
    ...
    return (Array.isArray(str) ? str : String(str).split(sep || /[,\|]/)).
            map(function(x) { return typed ? lib.toValue(x, type) : typeof x == "string" ? x.trim() : x }).
...
// Merge an object with the options, all properties from the `options` override existing properties in the `obj`, returns a new object, shallow copy,
...
// Example
//
//          lib.searchObj({id:{index:1},name:{index:3},type:{index:2},descr:{}}, { exists: 'index', sort: 1 });
//          { id: { index: 1 }, type: { index: 2 }, name: { index: 3 } }
//
//          lib.searchObj({id:1,name:"test",type:"test",descr:"descr"}, { hasvalue: 'test', count: 1});
//          2
//
...
lib.objDescr = function(obj, options)
{
    ...
    for (var p in obj) {
        ...
        if (Buffer.isBuffer(v)) {
            rc += p + ":[" + v.length + "] " + v.slice(0, options.length || 32).toString("hex");
        } else
        if (Array.isArray(v)) {
            rc += p + ":[" + v.length + "] " + v.slice(0, options.count || 5).map(function(x) { return lib.objDescr(x) });
        } else
        if (this.isObject(v)) {
            if (options._depth >= (options.depth || 3)) {
                ...
                options._depth++;
                rc += p + ":{ " + this.objDescr(v, options) + " }";
                options._depth--;
            }
        } else
        if (typeof v == "string") {
            rc += p + ":" + v.slice(0, options.length || 64);
        } else {
            rc += p + ":" + (this.isEmpty(v) ? "" : v);
        }
