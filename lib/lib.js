...
var lib = {
    ...
    rxUrl: /^https?:\/\/.+/,
    rxEmail: /^[A-Z0-9\._%+-]+@[A-Z0-9\.-]+\.[A-Z]{2,5}$/i,
    rxEmail1: /[^@\<\> ]+@[^@\<\> ]+/,
    rxEmail2: /\<?([^@\<\> ]+@[^@\<\> ]+)\>?/,
    rxPhone: /^[0-9 \.\+\(\)-]+$/,
    rxPhone2: /[^0-9]/g,
...
lib.toValue = function(val, type, options)
{
    ...
    case "phone":
        ...
        val = val.replace(this.rxPhone2, "").trim();
        // Keep US phones without 1
...
lib.isTimeRange = function(time1, time2, tz)
{
    ...
    now = new Date(now.getTime() + (tz || (now.getTimezoneOffset() * -60000)));
    var h0 = now.getUTCHours();
    var m0 = now.getUTCMinutes();
    if (typeof time1 == "string") {
        var d = time1.match(/([0-9]+):([0-9]+) *(amAMpmPM)?/);
        if (d) {
            ...
            logger.debug("isTimeRange:", "start:", h0, m0, " - ", h1, m1, "tz:", tz, "now:", now);
            if (h0*100+m0 < h1*100+m1) return 0;
        }
    }
    if (typeof time2 == "string") {
        var d = time2.match(/([0-9]+):([0-9]+) *(amAMpmPM)?/);
        if (d) {
            var h1 = lib.toNumber(d[1]);
            var m1 = lib.toNumber(d[2]);
            switch (d[3]) {
            ...
            logger.debug("isTimeRange:", "end:", h0, m0, " - ", h1, m1, "tz:", tz, "now:", now);
            if (h0*100+m0 > h1*100+m1) return 0;
        }
    }
    return 1;
...
{
    if (!this._hostHash) {
        var b = new Buffer(crypto.createHash('md5').update(os.hostname(), 'ascii').digest('binary'));
        this._hostHash = new Buffer([b[1], b[3], b[5], (process.pid) & 0xFF, (process.pid >> 8) & 0xFF ]);
        this._hostCounter = 0;
    }
    this._hostCounter = ~~this._hostCounter + 1;
...
{
    if (typeof str != "string" || !rx) return null;
    var d = str.match(rx);
