...
/**
 * Called by logger methods to perform input formatting, calls default or installed inspector
 * @param {any[]} - items to output
 * @param {object} [inspectArgs] - default for inspector
 * @memberof module:logger
 * @method format
 */
logger.format = function(args, inspectArgs)
{
    var str = "", line;
    for (const p in args) {
        if (args[p] === undefined) continue;
        line = logger._inspect(args[p], inspectArgs || logger._inspectArgs);
        if (logger.oneline) line = line.replace(/[\r\n\t]+| {2,}/g, " ");
        str += line + this.separator;
    }
    return str;
}
