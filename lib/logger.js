...
const logger = {
    ...
    date: process.BKJS_LOG_DATE || "datetime",
    pid: process.env.BKJS_LOG_PID || process.pid,
    tag: process.env.BKJS_LOG_TAG || "",
...
logger.setOptions = function(options)
{
    for (const p in options) {
        switch (p) {
        case "filter":
            this.setDebugFilter(options[p]);
            break;

        case "level":
            this.setLevel(options[p]);
            break;
        ...
        default:
            if (p.startsWith("syslog.")) {
                ...
                this._syslog[p.substr(7)] = options[p];
            }
        }
    }
    if (this.syslog) {
