...
logger.setOptions = function(options)
{
    for (const p in options) {
        if (p === "__proto___" || p === "constructor" || p === "prototype") continue;
        const v = options[p];
        switch (p) {
        case "filter":
...
logger.setDebugFilter = function(label, handler)
{
    String(label).split(",").forEach((x) => {
        ...
        if (x[0] === "!" || x[0] === "-") {
            logger.filters.delete(x.substr(1));
        } else {
            if (x[0] === '+') x = x.substr(1);
            logger.filters.add(x);
        }
    });
}
...
{
    return logger.filters.has(args[0]) || logger.filters.has(args[1]);
}
