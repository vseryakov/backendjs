...
logger.setSyslog = function(facility, tag)
{
    if (facility === 1 || facility === true || (typeof facility == "string" && facility.indexOf(":") > 0)) {
        var opts = { tag: tag };
        for (const p in this._syslog) opts[p] = this._syslog[p];
...
logger.setSyslogOptions = function(val)
{
    this._syslog = {};
    if (!val || typeof val != "string") return;
    val.split(",").forEach((x) => {
        x = x.trim().split(":").map((y) => (y.trim()));
        if (x[0]) this._syslog[x[0]] = x[1];
    });
    if (this.syslog) {
        for (const p in this._syslog) this.syslog[p] = this._syslog[p];
    }
}
...
// The `func` can be a function to be instead of regular logging, this is for rerouting some output to a custom console or for
