...
db.sqlWhere = function(pool, req, query, keys, join)
{
    ...
    for (var k in query) {
        ...
        if (k == "$or" || k == "$and") {
            v = this.sqlWhere(pool, req, v, keys, k.substr(1));
            if (v) where.push("(" + v + ")");
            continue;
        }
...
db.sqlUpdate = function(pool, req)
{
    ...
    if (req.options && lib.isObject(req.options.expected) && !pool.poolOptions.ifExpected) {
        var expected = this.sqlWhere(pool, req, req.options.expected, Object.keys(req.options.expected), req.options.expected_join);
