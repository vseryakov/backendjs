...
db.sqlCreate = function(pool, req)
{
    ...
    function keys(name) {
        ...
        if (name == "index" && pool.configOptions.noCompositeIndex) return cols.pop();
        return cols.join(',');
    }
    if (req.op == "create") {
                  ...
                  Object.keys(req.obj).
                      map(function(x) {
                          return x + " " +
                              ...
                              (!pool.configOptions.noNulls && req.obj[x].notnull ? " NOT NULL " : " ") +
                      ...
                      (function(x) { return x ? ",PRIMARY KEY(" + x + ")" : "" })(keys('primary')) + " " + (pool.configOptions.tableOptions || "") + ")" ];
...
db.sqlInsert = function(pool, req)
{
    ...
    req.text = (req.op == "put" && !pool.configOptions.noReplace ? "REPLACE" : "INSERT") + " INTO " + req.table + "(" + names.join(",") + ") values(" + pnums.join(",") + ")";
    if (req.options) {
...
db.sqlUpdate = function(pool, req)
{
    ...
    for (p in req.obj) {
        ...
        if (op == "not_exists" && !pool.configOptions.noCoalesce) {
            sets.push(p + "=COALESCE(" + p + "," + placeholder + ")");
        } else
        // Concat to a string
        if (op == "concat" && !pool.configOptions.noConcat) {
            sets.push(p + "=CONCAT(" + p + "," + placeholder + ")");
        } else
        // Append to a list
        if (op == "append" && !pool.configOptions.noAppend) {
