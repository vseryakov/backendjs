...
db.sqlCacheColumns = function(pool, options, callback)
{
    ...
    pool.acquire(function(err, client) {
        ...
        if (!pool.poolOptions.schema.length) pool.poolOptions.schema.push(client.name);
        client.query("SELECT c.table_name,c.column_name,LOWER(c.data_type) AS data_type,c.column_default,c.ordinal_position,c.is_nullable " +
                     ...
                     "WHERE c.table_schema IN (" + self.sqlValueIn(pool.poolOptions.schema) + ") AND c.table_name=t.table_name " +
...
db.sqlExpr = function(pool, name, value, options)
{
    ...
    case "in":
        ...
        sql += name + " " + op + " (" + self.sqlValueIn(list, type) + ")";
        break;
    ...
    case "not between":
        ...
        if (list.length > 1) {
            if (pool.poolOptions.noBetween) {
                sql += name + ">=" + this.sqlValue(list[0], options) + " AND " + name + "<=" + this.sqlValue(list[1], options);
            } else {
                sql += name + " " + op + " " + this.sqlValue(list[0], type) + " AND " + this.sqlValue(list[1], options);
            }
        } else {
...
db.sqlLimit = function(options)
{
    ...
    ["", "1", "2"].forEach(function(x) {
        var sort = options && options['sort' + x];
        if (!sort) return;
        var desc = lib.toBool(options && options['desc' + x]);
        orderby += (orderby ? "," : "") + sort + (desc ? " DESC" : "");
    });
    if (orderby) rc += " ORDER BY " + orderby;

    // Limit clause
    var page = lib.toNumber(options && options.page, { float: false, dflt: 0, min: 0 });
    var count = lib.toNumber(options && options.count, { float: false, dflt: 50, min: 0 });
    var start = lib.toNumber(options && options.start, { float: false, dflt: 0, min: 0 });
    if (count) {
        rc += " LIMIT " + count;
    }
    if (start) {
...
db.sqlWhere = function(pool, req, query, keys)
{
    var self = this;
    ...
    if (Array.isArray(query)) {
        ...
        return query.map(function(x) { return "(" + keys.map(function(y) { return y + "=" + self.sqlQuote(self.getBindValue(req.table, req.options, x[y], cols[y])) }).join(" AND ") + ")" }).join(" OR ");
    }
    // Regular object with conditions
    var where = [], opts = {};
    var ops = req.options && req.options.ops || lib.empty;
    var opsMap = pool.poolOptions.opsMap || lib.empty;
    var typesMap = pool.poolOptions.typesMap || lib.empty;
    (keys || lib.emptylist).forEach(function(k) {
        if (k[0] == "_") return;
        var v = query[k];
        if (!v && v != null) return;
        ...
        var sql = self.sqlExpr(pool, k, v, opts);
        if (sql) where.push(sql);
    });
    return where.join(" " + (req.options && req.options.join || "AND") + " ");
}
...
db.sqlCreate = function(pool, req)
{
    ...
    function keys(name) {
        ...
        if (name == "index" && pool.poolOptions.noCompositeIndex) return cols.pop();
        return cols.join(',');
    }
    if (req.op == "create") {
        var rc = ["CREATE TABLE " + (!pool.poolOptions.noIfExists ? "IF NOT EXISTS " : " ") + req.table + "(" +
                  Object.keys(req.obj).
                      map(function(x) {
                          return x + " " +
                              (function(t) { return (pool.poolOptions.typesMap || {})[t] || t })(req.obj[x].type || pool.poolOptions.defaultType || "text") + (!pool.poolOptions.noLengths && req.obj[x].len ? " (" + req.obj[x].len + ") " : " ") +
                              (!pool.poolOptions.noNulls && req.obj[x].notnull ? " NOT NULL " : " ") +
                              (!pool.poolOptions.noAuto && req.obj[x].auto ? " AUTO_INCREMENT " : " ") +
                              (!pool.poolOptions.noDefaults && typeof req.obj[x].value != "undefined" ? "DEFAULT " + self.sqlValue(req.obj[x].value, req.obj[x].type) : "") }).join(",") + " " +
                      (function(x) { return x ? ",PRIMARY KEY(" + x + ")" : "" })(keys('primary')) + " " + (pool.poolOptions.tableOptions || "") + ")" ];

    } else {
        var dbcols = pool.dbcolumns[req.table] || lib.empty;
        rc = Object.keys(req.obj).filter(function(x) { return !(x in dbcols) }).
                map(function(x) {
                    return "ALTER TABLE " + req.table + " ADD " + x + " " +
                        (function(t) { return (pool.poolOptions.typesMap || {})[t] || t })(req.obj[x].type || pool.poolOptions.defaultType || "text") + (!pool.poolOptions.noLengths && req.obj[x].len ? " (" + req.obj[x].len + ") " : " ") +
                        (!pool.poolOptions.noDefaults && typeof req.obj[x].value != "undefined" ? "DEFAULT " + self.sqlValue(req.obj[x].value, req.obj[x].type) : "") }).
    ...
    ["","1","2","3","4"].forEach(function(y) {
        ...
        if (!cols) return;
        var idxname = req.table + "_" + cols.replace(",", "_") + "_idx";
        if (pool.dbindexes[idxname]) return;
        rc.push("CREATE INDEX " + (!pool.poolOptions.noIfExists ? "IF NOT EXISTS " : " ") + idxname + " ON " + req.table + "(" + cols + ")");
    });

    req.text = pool.poolOptions.noMultiSQL && rc.length ? rc : rc.join(";");
}
...
db.sqlSelect = function(pool, req)
{
    ...
    req.text = "SELECT " + select + " FROM " + req.table + where + this.sqlLimit(req.options);
}
...
db.sqlInsert = function(pool, req)
{
    ...
    for (var p in req.obj) {
        ...
        names.push(p);
        pnums.push(pool.poolOptions.sqlPlaceholder || ("$" + i));
    ...
    req.text = (req.op == "put" && !pool.poolOptions.noReplace ? "REPLACE" : "INSERT") + " INTO " + req.table + "(" + names.join(",") + ") values(" + pnums.join(",") + ")";
    if (req.options) {
...
db.sqlUpdate = function(pool, req)
{
    ...
    for (p in req.obj) {
        ...
        if (v && lib.isNumericType(data_type) && typeof v != "number") v = lib.toNumber(v);
        var placeholder = pool.poolOptions.sqlPlaceholder || ("$" + i);
        var op = req.options && req.options.updateOps && req.options.updateOps[p];
        // Update only if the value is null, otherwise skip
        if (op == "not_exists" && !pool.poolOptions.noCoalesce) {
            sets.push(p + "=COALESCE(" + p + "," + placeholder + ")");
        } else
        // Concat to a string
        if (op == "concat" && !pool.poolOptions.noConcat) {
            sets.push(p + "=CONCAT(" + p + "," + placeholder + ")");
        } else
        // Append to a list
        if (op == "append" && !pool.poolOptions.noAppend) {
            sets.push(p + "=" + p + "+" + placeholder);
        } else
        // Increment a number
        if (op == "incr") {
            sets.push(p + "=" + (pool.poolOptions.noCoalesce ? p : "COALESCE(" + p + ",0)") + "+" + placeholder);
        } else {
    ...
    if (req.options && lib.isObject(req.options.expected) && !pool.poolOptions.ifExpected) {
        var expected = this.sqlWhere(pool, req, req.options.expected, Object.keys(req.options.expected));
    ...
    if (req.options && lib.isObject(req.options.expected) && pool.poolOptions.ifExpected) {
        var expected = Object.keys(req.options.expected).
                              ...
                              map(function(x) { return x + "=" + self.sqlValue(req.options.expected[x]) }).
                              join(" AND ");
