...
db.sqlCreate = function(pool, req)
{
    ...
    if (req.op == "create") {
                  ...
                  Object.keys(req.obj).
                      map(function(x) {
                          return self.sqlColumn(x, pool) + " " +
                              (typesMap[req.obj[x].type] || req.obj[x].type || "text") +
                              ...
                              (!pool.configOptions.noDefaults && typeof req.obj[x].value != "undefined" ? "DEFAULT " + self.sqlValue(req.obj[x].value, req.obj[x]) : "") }).join(",") + " " +
        ...
        rc = Object.keys(req.obj).filter(function(x) { return !(x in dbcols) }).
                map((x) => ("ALTER TABLE " + req.table + " ADD " + self.sqlColumn(x, pool) + " " +
                             (typesMap[req.obj[x].type] || req.obj[x].type || "text") +
                             (!pool.configOptions.noLengths && req.obj[x].len ? " (" + req.obj[x].len + ") " : " ") +
...
db.sqlInsert = function(pool, req)
{
    ...
    var op = req.op == "put" && !pool.configOptions.noReplace ? "REPLACE" :
             pool.configOptions.upsert ? "UPSERT" : "INSERT";
    req.text = op + " INTO " + req.table + "(" + names.map(function(x) { return self.sqlColumn(x, pool) }).join(",") + ") values(" + pnums.join(",") + ")";
    if (req.options) {
