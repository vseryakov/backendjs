...
const mod = {
    name: "watch",
    args: [
        { name: "dir", type: "list", array: 1, descr: "Watch sources directories for file changes to restart the server, for development only, the backend module files will be added to the watch list automatically, so only app specific directores should be added. In the production -monitor must be used." },
    ...
    web: [],
    match: /\.(js|css|html)$/,
    build: "bkjs web-bundle -dev -file",
    delay: 200,
};
...
mod.start = function()
{
    const server = core.modules.server;
