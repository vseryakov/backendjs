...
api.listAccount = function(rows, options, callback)
{
    ...
    db.list("bk_account", Object.keys(map).map(function(x) { return { id: x } }), { select: options.select }, function(err, list, info) {
        ...
        self.checkResultColumns("bk_account", list, options);
...
// check secrets but only existence of a user by login. If the user with login exists, this works as `api.getAccount`
...
api.addAccount = function(req, options, callback)
{
    ...
    corelib.series([
       function(next) {
           ...
           self.prepareAccountSecret(query, options);
           ...
           if (!req.account || !self.checkAccountType(req, "admin")) self.clearQuery(query, options, "bk_auth", "admin");
           self.clearQuery(query, options, "bk_auth", "private");
           db.add("bk_auth", query, options, next);
       },
       function(next) {
           var query = corelib.cloneObj(req.query);
           // Only admin can add accounts with admin properties
           if (!req.account || !self.checkAccountType(req, "admin")) self.clearQuery(query, options, "bk_account", "admin");
           self.clearQuery(query, options, "bk_account", "private");
...
api.updateAccount = function(req, options, callback)
{
    ...
    corelib.series([
       function(next) {
           ...
           if (!self.checkAccountType(req, "admin")) self.clearQuery(query, options, "bk_auth", "admin");
           self.clearQuery(query, options, "bk_auth", "private");
       ...
       function(next) {
           self.clearQuery(req.query, options, "bk_account", "private");
           self.clearQuery(req.query, options, "bk_account", "location");
           // Skip admin properties if any
           if (!self.checkAccountType(req, "admin")) self.clearQuery(req.query, options, "bk_account", "admin");
...
api.setAccountSecret = function(req, options, callback)
{
    ...
    db.update("bk_auth", { login: req.query.login, secret: req.query.secret, token_secret: req.query.token_secret }, options, function(err, data, info) {
       if (!err) req.query._updated = true;
