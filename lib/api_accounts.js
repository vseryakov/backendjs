...
//  - scramble can be used to sign the secret and login so the db never stores cleartext credentials:
//    1 - scramble secret, 2 - scramble login and secret
api.prepareAccountSecret = function(query, options)
{
    if (options.scramble && query.login && query.secret) {
        ...
        if (options.scramble > 1) query.login = corelib.sign(query.login, query.login);
    }
    // Ignore the supplied vale, always set with new uuid
    if (query.token_secret) query.token_secret = corelib.uuid();
}
