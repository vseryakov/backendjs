...
//  - bcrypt - use bcrypt module to generate password hash
api.prepareAccountSecret = function(query, options, callback)
{
    ...
    if (query.login && query.secret) {
        ...
        if (options && options.bcrypt) {
            return bcrypt.hash(query.secret, 10, function(err, hash) {
                query.secret = hash;
                callback();
            });
        }
