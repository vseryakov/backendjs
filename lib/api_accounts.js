...
api.configureDefaultAPI = function()
{
    ...
    this.app.all("/auth", function(req, res) {
        api.handleSessionSignature(req, api.getOptions(req));
        if (!req.account || !req.account.id) return api.sendReply(res, 417, "No login provided");
...
//  - scramble (default) is used to encrypt the secret with login as HMAC_SHA256 so the db never stores cleartext credentials, this is an alternative to the
...
api.prepareAccountSecret = function(query, options)
{
    ...
    // From a "cleatext" password produce a hash and random secret, the password field is expected to be a hash of sha256(login:password), never
    // use the actual password
    if (query.password) {
