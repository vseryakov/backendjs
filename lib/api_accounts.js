...
api.prepareAccountSecret = function(account, options, callback)
{
    if (!options) options = {};

    if (!account.secret) delete account.secret;

    // Ignore the supplied value, always set with new uuid
    if (account.token_secret) {
        account.token_secret = lib.random();
    }

    if (account.login && account.secret && options.scramble) {
        account.secret = lib.sign(account.secret, account.login, "sha256");
    }

    lib.series([
        function(next) {
            if (!account.secret) return next();
            if (!options.bcrypt) return next();
            if (!api.bcrypt) api.bcrypt = require('bcrypt');
            api.bcrypt.hash(account.secret, lib.toNumber(options.bcrypt), function(err, hash) {
                account.secret = hash;
                next();
            });
        },
        function(next) {
            var hooks = api.findHook('secret', '', account.login);
            if (!hooks.length) return next();
            lib.forEachSeries(hooks, function(hook, next2) {
                hook.callback.call(api, account, options, next2);
            }, next);
        },
    ], callback);
}
...
api.addAccount = function(query, options, callback)
{
    ...
    this.prepareAccountSecret(query, options, () => {
        if (!(options.admin || this.checkAccountType(options.account, "admin"))) {
            this.clearQuery(this.authTable, query, "admin");
        }
        options.result_obj = options.first = 1;
        db.add(this.authTable, query, options, function(err, row) {
            if (err) return callback(err);
