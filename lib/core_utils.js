...
{
    if (lib.toBool(set)) {
        if (!logger._inspect) logger._inspect = logger.inspect;
        logger.inspect = core.inspect;
    } else {
        if (logger._inspect) logger.inspect = logger._inspect;
    }
}

core.inspect = function(obj)
{
    return lib.objDescr(obj, core.logInspect);
}
