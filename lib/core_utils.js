...
core.killBackend = function(name, signal, callback)
{
    ...
    var rx = new RegExp(lib.strSplit(name).map((x) => (`${core.name}: ${x}`)).join("|"));
    lib.execProcess("/bin/ps agx", (err, stdout, stderr) => {
        var pids = stdout.split("\n").
                   filter((x) => (rx.test(x))).
                   map((x) => (lib.toNumber(x))).
                   filter((x) => (x != process.pid));
        lib.forEachSeries(pids, (pid, next) => {
            try { process.kill(pid) } catch(e) { logger.dev("killBackend:", name, pid, e) }
            setTimeout(() => {
                try { process.kill(pid, "SIGKILL") } catch(e) { logger.dev("killBackend:", name, pid, e) }
                next();
            }, 1000);
        }, callback);
    });
