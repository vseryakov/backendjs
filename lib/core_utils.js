...
core.watchLogs = function(options, callback)
{
    ...
    logger.debug('watchLogs:', this.logwatcherSend, this.logwatcherFiles);
    ...
    db.select("bk_property", { name: 'logwatcher:' }, { ops: { name: 'begins_with' }, pool: db.local }, function(err, rows) {
        ...
        lib.forEachSeries(core.logwatcherFile, function(log, next) {
            ...
            lib.forEach(Object.keys(errors), function(type, next) {
                ...
                logger.log('logwatcher:', type, 'found matches, sending to', core.logwatcherSend[type]);
                ...
                case "https://":
                    core.sendRequest({ url: uri,
                                         ...
                                         retryOnError: 1,
                                         retryTimeout: 3000,
                                         postdata: text }, function() { next() });
                    break;

                case "sns://":
                    core.modules.aws.snsPublish(d[2], text, { subject: subject }, function() { next() });
                    break;

                case "ses://":
                    core.modules.aws.sesSendEmail(d[2], subject, text, { from: core.logwatcherFrom }, function() { next() });
                    break;

                default:
                    core.sendmail({ from: core.logwatcherFrom, to: d[2], subject: subject, text: text }, function() { next() });
                }
            }, function(err) {
