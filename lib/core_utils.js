...
core.killBackend = function(name, signal, callback)
{
    ...
    this.getProcesses(`${core.name}: ` + (name ? `(${name})`: ""), (pids) => {
        logger.debug("killBackend:", name, pids);
        lib.forEach(pids, (pid, next) => {
...
core.getProcesses = function(filter, callback)
{
    lib.execProcess("/bin/ps agx", (err, stdout, stderr) => {
        callback(stdout.split("\n").
                 filter((x) => (x.match(filter))).
                 map((x) => (lib.toNumber(x))).
                 filter((x) => (x != process.pid)));
    });
}
...
core.watchLogs = function(options, callback)
{
    ...
    var match = {};
    for (const p in this.logwatcherMatch) match[p] = lib.toRegexp(this.logwatcherMatch[p].map(function(x) { return "(" + x + ")"}).join("|"));
    var ignore = {};
    for (const p in this.logwatcherIgnore) ignore[p] = lib.toRegexp(this.logwatcherIgnore[p].map(function(x) { return "(" + x + ")"}).join("|"));
    var once = {};
    for (const p in this.logwatcherOnce) once[p] = lib.toRegexp(this.logwatcherOnce[p].map(function(x) { return "(" + x + ")"}).join("|"));
    ...
    db.select("bk_property", { name: 'logwatcher:' }, { ops: { name: 'begins_with' }, pool: db.local }, function(err, rows) {
        ...
        lib.forEachSeries(core.logwatcherFile, function(log, next) {
                ...
                fs.open(file, "r", function(err, fd) {
                    ...
                    fs.read(fd, buf, 0, buf.length, Math.max(0, pos), function(err, nread, buffer) {
                        ...
                        for (var i = 0; i < lines.length; i++) {
                            ...
                            if ((log.ignore && log.ignore.test(lines[i])) || matchObj(ignore, lines[i])) {
                                if (!errors.ignore) errors.ignore = "";
                                errors.ignore += lines[i] + "\n";
                            ...
                            if (chan) {
                                ...
                                var tag = matchObj(once, lines[i]);
                                if (tag && lib.objIncr(seen, tag) > 1) chan = null;
                            }
                            if (chan) {
            ...
            lib.forEvery(Object.keys(errors), function(type, next) {
