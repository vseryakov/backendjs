...
// - file - a regexp to match files
// - ignore - a regexp to ignore files
core.watchFiles = function(options, fileCallback, endCallback)
{
    logger.debug('watchFiles:', options);

    var pattern = lib.toRegexp(options.file);
    if (!pattern) return lib.tryCall(endCallback, "invalid file pattern");
    var ignore = lib.toRegexp(options.ignore);
    ...
    fs.readdir(options.root, function(err, list) {
        ...
        list = list.filter(function(file) {
            return (!ignore || !ignore.test(file)) && pattern.test(file);
        }).map(function(file) {
