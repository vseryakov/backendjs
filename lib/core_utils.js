...
core.sendmail = function(options, callback)
{
    ...
    try {
        var h = url.parse(this.emailTransport || "", true);
        switch (h.protocol) {
        case "ses:":
...
core.startRepl = function(port, bind, options)
{
    ...
    try {
        this.repl.server = net.createServer(function(socket) {
            var repl = core.createRepl(lib.cloneObj(options, "prompt", '> ', "input", socket, "output", socket, "terminal", true, "useGlobal", false));
    ...
    db.select("bk_property", { name: 'logwatcher:' }, { ops: { name: 'begins_with' }, pool: db.local }, function(err, rows) {
        ...
        lib.forEachSeries(core.logwatcherFile, function(log, next) {
            ...
            lib.forEach(Object.keys(errors), function(type, next) {
                ...
                logger.log('logwatcher:', type, 'found matches, sending to', core.logwatcherEmail[type], core.logwatcherUrl[type], core.logwatcherTable[type]);

                if (core.logwatcherTable[type]) {
                    db.add(core.logwatcherTable[type], {
                               mtime: Date.now(),
                               type: type,
                               ipaddr: core.ipaddr,
                               host: os.hostname(),
                ...
                if (core.logwatcherUrl[type]) {
                    core.sendRequest({ url: core.logwatcherUrl[type],
                                         headers: {
                ...
                if (core.logwatcherEmail[type]) {
                    var subject = "logwatcher: " + type + ": " + os.hostname() + "/" + core.ipaddr + "/" + core.instance.id + "/" + core.instance.tag + "/" + core.runMode;
                    core.sendmail({ from: core.logwatcherFrom, to: core.logwatcherEmail[type], subject: subject, text: errors[type] }, function() { next() });
                    return;
                }
                next();
            }, function(err) {
