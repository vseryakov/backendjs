...
IpcRedisClient.prototype.get = function(key, options, callback)
{
    ...
    if (map) {
        if (key == "*") {
            ...
            this.client.hmget(map, key, function(err, val) {
                if (!Array.isArray(key)) val = val[0];
...
IpcRedisClient.prototype.put = function(key, val, options, callback)
{
    ...
    if (map) {
        ...
        if (ttl > 0) {
            this.client.setex([key, Math.ceil(ttl/1000), val], typeof callback == "function" && callback || lib.noop);
        } else {
            this.client.set([key, val], typeof callback == "function" && callback || lib.noop);
        }
    }
}
...
IpcRedisClient.prototype.del = function(key, options, callback)
{
    ...
    if (map) {
        if (key == "*") {
            this.client.del(map, typeof callback == "function" && callback || lib.noop)
        } else {
            this.client.hdel(map, key, typeof callback == "function" && callback || lib.noop)
        }
    } else {
        this.client.del(key, typeof callback == "function" && callback || lib.noop);
    }
}
