...
{
    if (!this.client.enable_offline_queue) this.client.enable_offline_queue = true;
    if (typeof msg == "object") msg = lib.stringify(msg);
...
IpcRedisClient.prototype.get = function(key, options, callback)
{
    ...
    if (options.listName) {
        ...
        if (!key) {
            this.client.eval(client.scripts.sget, 3, options.listName, options.del ? "spop" : "srandmember", options.del ? lib.toNumber(options.ttl) : 0, callback);
        } else {
...
IpcRedisClient.prototype.put = function(key, val, options, callback)
{
    ...
    if (options.listName) {
        ...
        if ((!Array.isArray(val) && val) || lib.isArray(val)) multi.sadd(options.listName, val);
...
IpcRedisClient.prototype.poller = function()
{
    ...
    this.client.eval(script, 2, this.options.queue, Date.now(), function(err, rc) {
        ...
        if (!self.emit("message", msg, function(err, next) {
            clearInterval(timer);
