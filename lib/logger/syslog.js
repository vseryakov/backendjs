...
mod.Syslog = class Syslog {
    ...
    label()
    {
        return [this.hostname, this.tag, this.role, this.udp && "udp", this.host, this.port, this.path, this._connecting].filter((x) => x).join(", ");
    }
    ...
    onError(err, msg, retry)
    {
        if (!err) return;
        retry = Number.parseInt(retry) || 0;
        ...
        try {
            ...
            if (this.udp) {
                ...
                this._sock.on('close', () => {
                    ...
                    setTimeout(() => { if (this._sock) this._sock.connect(this) }, 2 ** this._connecting++ * 1000);
                });
