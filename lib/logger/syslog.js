...
function bdate(d) { return `${months[d.getMonth()]} ${spacepad(d.getDate())} ${zeropad(d.getHours())}:${zeropad(d.getMinutes())}:${zeropad(d.getSeconds())}` }
...
Syslog.prototype.log = function(priority, msg)
{
    ...
    if (this.rfc3164) {
        msg = `<${priority}>${bdate(now)} ${this.hostname} ${this.tag}[${process.pid}]: ${msg}\n`;
    } else {
        const d = this.bsd ? bdate(now) : now.toISOString();
