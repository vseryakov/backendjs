...
// Supports sections in the form `[name:value]` where name is a property name in the options with exact value.
// This will work like a filter, only if a property matches it is used otherwise skipped completely.
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith("[") && line.endsWith("]")) {
            ...
            if (key == "global") {
                ...
                section = key.split(":").map((x) => (x.trim()));
            }
        }
        if (!/^([a-z0-9_-]+)/.test(line)) continue;
        if (section && options[section[0]] != section[1]) continue;
        line = line.split("=");
        if (options?.obj) {
