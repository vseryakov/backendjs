 ...
 * @param {number} [options.dflt] - default value
 * @param {boolean} [options.float] - treat as floating number if true, as integer if false
 * @param {number} [options.min] - minimal value, clip
 * @param {number} [options.max] - maximum value, clip
 * @param {number} [options.incr] - a number to add before checking for other conditions
 * @param {number} [options.mult] - a number to multiply before checking for other conditions
 * @param {number} [options.no_value] - replace this number with default
 * @param {number} [options.zero] - replace with this number if result is 0
 * @param {number} [options.digits] - how many digits to keep after the floating point, between 0 and 100
 * @param {boolean} [options.bigint] - return BigInt if not a safe integer
 * @param {number} [options.base=10] - base of the input, 2, 10, 16, ...
 * @return {number}
 *
 * @example
 * lib.toNumber("123")
 * 123
 *
 * lib.toNumber("1.23", { float: true, dflt: 0, min: 0, max: 2 })
 * 1.23
 *
...
lib.toNumber = function(val, options)
{
    ...
    if (typeof val === "boolean") {
        ...
        if (typeof val !== "string") {
            n = typeof options?.dflt === "number" ? options.dflt : 0;
        } else {
            // Autodetect floating number
            const float = options?.float === undefined || options?.float === null ? this.rxFloat.test(val) : options?.float;
            n = val[0] === 't' ? 1 : val[0] === 'f' ? 0 :
                val === "infinity" ? Number.POSITIVE_INFINITY :
                (float ? Number.parseFloat(val) : Number.parseInt(val, options?.base || 10));
        }
    }
    if (Number.isNaN(n)) {
        n = typeof options?.dflt === "number" ? options.dflt : 0;
    ...
    for (const p in options) {
        ...
        switch (p) {
        case "no_value":
            if (n === v) n = typeof options.dflt === "number" ? options.dflt : 0;
            break;
        case "incr":
        ...
        case "mult":
            ...
            break;
        case "min":
            if (typeof v === "number" && n < v) n = v;
            break;
        case "max":
            if (typeof v === "number" && n > v) n = v;
            break;
        case "float":
            if (v === false || v === 0) n = Math.round(n);
            break;
        case "zero":
            if (typeof v === "number" && !n) n = v;
            break;
        case "digits":
            if (typeof v === "number" && v >= 0 && v <= 100) n = Number.parseFloat(n.toFixed(v)) || n;
            break;
        case "bigint":
            if (v && typeof n === "number" && !Number.isSafeInteger(n)) n = BigInt(val);
            break;
        }
    }
    return n;
...
/**
 * Return Date object for given text or numeric date representation, for invalid date returns epoch start.
 * @param {string|Date|number} val
 * @param {Date|number} [dflt]
 * @return {Date}
 * @memberof module:lib
 * @method toDate
  * @example
...
lib.toDate = function(val, dflt)
{
    ...
    try { val = new Date(val) } catch (_e) {}
    return this.isDate(val) ? val :
           this.isDate(dflt) ? dflt :
           new Date(typeof dflt === "number" ? dflt : 0);
}
...
{
    return this.toDate(val).getTime() || (typeof dflt === "number" ? dflt : 0);
}
 ...
 * @return {string} valid email in lower case
 * @memberof module:lib
 * @method toEmail
  * @example
 ...
 * @param {string} val
 * @param {object} [options]
 * @param {boolean} [options.delimiter=,] - pairs separators
 * @param {boolean} [options.separator=:;] - key and value separators
 * @param {boolean} [options.keep_empty] - keep empty values
 * @param {string} [options.map_type] - convert values to this type
 * @param {number} [options.max_list] - max number of items in the map, return null if exceeded
 * @param {boolean} [options.trunc] - truncate to the max_list if exceeded
 * @param {int} [options.max] - will skip key if value over the specificed size if no `trunc`
 * @param {boolean} [options.camel] - camelize keys
 * @param {boolean} [options.cap] - capitalize value
 * @param {boolean} [options.lower] - lowercase value
 * @param {boolean} [options.upper] - uppercase value
 * @param {string|regexp} [options.strip] - remove occurences in values
...
{
    let list = lib.split(val, options?.delimiter || ",");
    if (options?.max_list > 0 && list.length > options.max_list) {
        if (!options.trunc) {
            return null;
        }
        list = list.slice(0, options.max_list);
    }

    return list.map((y) => (lib.split(y, options?.separator || /[:;]/))).
        reduce((obj, item) => {
            if (!item.length) return obj;
            let value = item.length > 2 ? item.slice(1) : item[1];
            if (options?.map_type) {
                value = lib.toValue(value, options.map_type, options);
            }
                ...
                for (const p in options) {
                    ...
                    case "max":
                        if (value.length > options.max) {
                            if (!options.trunc) return obj;
                            value = value.substr(0, options.max);
                        }
                        break;
                    case "strip":
                        value = value.replace(options.strip, "");
                        break;
                    case "replace":
                        for (const p in options.replace) {
                            value = value.replaceAll(p, options.replace[p]);
                        }
                        break;
                    case "trim":
                        value = value.trim();
                    ...
                    case "upper":
                        ...
                        break;
                    case "cap":
                        value = lib.toTitle(value);
                        break;
                    }
                }
            }
            if (!options?.keep_empty && lib.isEmpty(value)) {
                return obj;
            }
            if (options?.camel) {
                item[0] = lib.toCamel(item[0]);
            }
            obj[item[0]] = value;
            return obj;
        }, Object.create(null));
 ...
 * - if null or empty return the value as is without any conversion
 ...
 * - uuid - empty string if not UUIDv4 according to {@link module:lib.isUuid()}
...
lib.toValue = function(val, type, options)
{
    ...
    case "mtime":
        return val ? this.toMtime(val, options?.dflt) : 0;
...
{
    var d = lib.rxMilliseconds.exec(duration);
    if (!d) return;
 ...
 * @param {any} data
 * @param {string} [secret]
 * @param {object} [options] - passed to {@link module:lib.encrypt} if secret is given
 * @param {string} [options.encode='base64'] - Output encoding ("base64", "base64url", "hex" or "binary")
 * @return {string}
 * @memberof module:lib
 * @method jsonToBase64
 * @example
...
lib.jsonToBase64 = function(data, secret, options)
{
    ...
    return Buffer.from(data).toString(options?.encode || "base64");
}
 ...
 * @param {any} data
 * @param {string} [secret]
 * @param {object} [options] - passed to {@link module:lib.decrypt} if secret is given
 * @param {string} [options.encode='base64'] - Input encoding ("base64", "base64url", "hex" or "binary")
 * @return {object}
 * @memberof module:lib
 * @method base64ToJson
  * @example
...
lib.base64ToJson = function(data, secret, options)
{
    ...
    try {
        if (typeof data === "number" || (typeof data === "string" && lib.rxNumber.test(data))) {
            ...
            if (!secret) data = Buffer.from(data, options?.encode || "base64").toString();
...
lib._jsonFormatPresets = new Map([
    [
        "default",
        {
            indent: "",
...
{
    if (!name) return;
    let preset = lib._jsonFormatPresets.get(name);
    if (!preset) {
        preset = Object.create(null);
        lib._jsonFormatPresets.set(name, preset);
    }
...
lib.jsonFormat = function(obj, options)
{
    ...
    if (!options.__preset) {
        ...
        const preset = Object.assign(Object.create(null), lib._jsonFormatPresets.get("default"), ...presets.map((x) => (lib._jsonFormatPresets.get(x))));
 ...
 * @param {boolean} [options.keep_empty] - show empty values
...
lib.inspect = function(obj, options)
{
    ...
    for (const i in keys) {
        ...
        switch (t) {
        case "buffer":
            if (v.length || options.keep_empty) {
                if (p || v.length) {
                    if (rc) rc += ", ";
                    if (p) rc += p + ": ";
                    if (!h) {
...
lib._unicodeCache = Object.create(null);
...
lib.unicode2Ascii = function(str, types)
{
    ...
    if (!map) {
        map = this._unicodeCache[types] = Object.create(null);
