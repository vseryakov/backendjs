 ...
 * @param {boolean} [options.no_empty] - ignore empty values
 * @param {boolean} [options.map_camel] - camelize keys
 * @param {string} [options.map_type] - convert values to this type
 * @param {boolean} [options.no_proto] - create an object using null prototype
 * @return {object}
 * @memberof module:lib
 * @method toMap
 * @example
    ...
    return lib.split(val, options?.delimiter || ",").
        ...
        reduce((a, b) => {
            ...
            if (b.length < 2) {
                ...
                if (options?.map_type) v = lib.toValue(v, options.map_type, options);
            }
            if (options?.no_empty && lib.isEmpty(v)) return a;
            if (options?.map_camel) b[0] = lib.toCamel(b[0]);
            a[b[0]] = v;
            return a;
        }, options?.no_proto ? Object.create(null) : {});
}
