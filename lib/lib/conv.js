...
lib.toParams = function(query, schema, options)
{
    ...
    for (const name in schema) {
        ...
        case "obj":
            if (!v && !opts.empty) break;
            v = this.toParams(v || lib.empty, opts.params);
        ...
        case "array":
            if (!v && !opts.empty) break;
            v = lib.isArray(v, []).map((x) => (lib.toParams(x, opts.params, { null: 1 }))).filter((x) => (x !== null));
            if (opts.minlist > 0 && v.length < opts.minlist) {
        ...
        default:
            ...
            if (opts.regexp) {
                ...
                if (!rx.some((r) => (lib.testRegexp(v, r)))) {
                    if (!opts.required && opts.errmsg) return options.null ? null : typeof opts.errmsg == "string" ? opts.errmsg : this.__("invalid data for %s", name);
                    break;
                }
            }
