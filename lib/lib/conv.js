...
lib.toNumber = function(val, options)
{
    ...
    if (typeof val == "boolean") {
        ...
        if (typeof val != "string") {
            ...
            var f = options?.float === undefined || options?.float === null ? this.rxFloat.test(val) : options?.float;
 ...
 * @param {boolean} [options.error] - always return Error on any error
...
lib.toParams = function(query, schema, options)
{
    const onError = (opts, name, value, errmsg) => {
        ...
        return options?.error ? lib.newError(errmsg, { name, value }) : errmsg;
    };
    ...
    for (const name in schema) {
        ...
        case "obj":
            if (!v && !opts.empty) break;
            e = this.toParams(v || lib.empty, opts.params, options);
        ...
        case "object":
            ...
            if (opts.params) {
                e = this.toParams(v, opts.params, options);
                if (typeof e == "string") return onError(opts, name, v, e);
                v = e;
            }
            if (opts.max > 0 && lib.objSize(v) > opts.max) {
        ...
        case "array":
            ...
            if (opts.params) {
                const list = [];
                for (let a of v) {
                    a = lib.toParams(a, opts.params, options)
        ...
        case "json":
            ...
            if (opts.params) {
                const e = this.toParams(v, opts.params, options);
                if (typeof e == "string") return onError(opts, name, v, e);
                v = e;
            }
...
{
    try {
        return lib.escapeUnicode(replacer || space ? JSON.stringify(obj, replacer, space) : JSON.stringify(obj));
    } catch (e) {
 ...
 * @param {object} [options.replace] - replace occurences of a property with value: { "aaa": "1" }
            ...
            if (options.__depth >= depth) {
                ...
                if (options.__seen.has(v)) {
                    ...
                    if (h) {
                        ...
                        const vv = [];
                        for (const k of v) {
                            vv.push(lib.inspect(k[0]) + ": " + lib.inspect(k[1]));
                            if (vv.length >= count) {
        ...
        case "object":
            if (options.__depth >= depth) {
                ...
                if (options.__seen.has(v)) {
                    ...
                    options.__seen.add(v);
                    options.__depth++;
                    v = h ? Object.keys(v).length : this.inspect(typeof v.toJSON == "function" ? v.toJSON() : v, options);
