...
{
    var str = "";
    mtime = typeof mtime == "number" ? mtime : util.isDate(mtime) ? mtime.getTime() : this.toNumber(mtime);
    if (mtime > 0) {
        var lang = options && options.lang;
...
//  - setnull - if the value is equal this then set property to null, useful to reset lists, maps...
...
lib.toParams = function(query, schema, options)
{
    ...
    for (const name in schema) {
        ...
        dflt = options.defaults && (options.defaults[name] || options.defaults['*']);
        for (const p in dflt) if (typeof opts[p] == "undefined") opts[p] = dflt[p];
        if (opts.ignore) continue;
        n = opts.name || name;
        p = (options.prefix || "") + name;
        if (options.existing && !(p in query)) continue;
        v = query[p];
        if (options.setnull && options.setnull === v) {
            rc[n] = null;
            continue;
        }
        ...
        case "map":
            if (!v && !opts.empty) break;
            var map = lib.strSplit(v, opts.delimiter || ",").
                      map((x) => (lib.strSplit(x, opts.separator || /[:;]/, opts))).
                      reduce((a, b) => {
                          a[b[0]] = b.length == 2 ? b[1] : b.slice(1);
                          if (opts.maptype) a[b[0]] = lib.toValue(a[b[0]], options.maptype);
                          return a;
                      }, {});
            if (this.isEmpty(map) && !opts.empty) break;
            if (!rc[n]) rc[n] = {};
            for (const p in map) rc[n][p] = map[p];
            break;
        case "obj":
            if (!v && !opts.empty) break;
            o = this.toParams(v || lib.empty, opts.params);
            if (typeof o == "string") return options.null ? null : o;
            if (!this.isEmpty(o) || opts.empty) rc[n] = o;
            break;
        case "array":
            if (!v && !opts.empty) break;
        ...
        default:
            ...
            if (opts.maxlength && v.length > opts.maxlength) v = v.substr(0, opts.maxlength);
            if (opts.max && v.length > opts.max) {
                if (!opts.trunc) {
                    return options.null ? null : opts.errmsg || this.__("%s is too long, the max length is %s", name, opts.max);
                }
                v = v.substr(0, opts.max);
            }
            if (opts.min && v.length < opts.min) {
                return options.null ? null : opts.errmsg || this.__("%s is too short, the min length is %s", name, opts.min);
            }
            if (opts.regexp) {
                var rx = lib.isArray(opts.regexp, [opts.regexp]);
