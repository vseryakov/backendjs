    ...
    case "map":
        return lib.strSplit(val, options?.delimiter || ",").
               ...
               reduce((a, b) => {
                  ...
                  if (options?.maptype) a[b[0]] = lib.toValue(a[b[0]], options.maptype, options);
                  return a;
        }, {});
...
{
    if (util.isRegExp(str)) return str;
    try {
...
lib.toParams = function(query, schema, options)
{
        ...
        case "mtime":
            ...
            if (v) {
                if (opts.mindate && v < opts.mindate) {
                    return options?.null ? null : this.__(opts.errmsg || "%s is too soon, the earliest date is %s", name, lib.toDate(opts.mindate));
                }
                if (opts.maxdate && v > opts.maxdate) {
                    return options?.null ? null : this.__(opts.errmsg || "%s is too late, the latest date is %s", name, lib.toDate(opts.maxdate));
                }
                rc[n] = v.getTime();
            }
            break;
        ...
        if (!this.isEmpty(v) && opts.type != "list") {
            if (typeof v == "number") {
                if (opts.maxnum && v > opts.maxnum) {
                    return options?.null ? null : this.__(opts.errmsg || "%s is too large, the max value is %s", name, opts.maxnum);
                }
                if (opts.minnum > 0 && v < opts.minnum) {
                    return options?.null ? null : this.__(opts.errmsg || "%s is too small, the min value is %s", name, opts.minnum);
                }
            }
    ...
    for (const req of required) {
        if (this.isMatched(rc, req.required)) {
            return options?.null ? null : opts.errmsg || this.__("%s is required", req.name);
        }
    }
    return rc;
}
...
lib._toTemplate = function(text, obj, options, encoder)
{
    ...
    while (str) {
        ...
        if (tag.startsWith("if")) {
            ...
            d = tag.match(/^(if|ifnull|ifnotnull|ifempty|ifnotempty|ifne|ifeq|ifgt|ifge|iflt|ifle|ifnot|ifall|ifstr|ifnotstr) ([a-zA-Z0-9._-]+) *(.*)$/)
            if (!d) continue;
...
// Options:
// - preset - predefined set of options, currently only `compact` which prints text version
...
lib.jsonFormat = function(obj, options)
{
    ...
    for (let name in obj) {
        ...
        if (type != "array") {
            ...
            if (!prefix) indent += options.space.repeat(options.nspace);
        ...
        case "string":
            ...
            if (options.wrap > 0 && val.length > options.wrap && options.nl1) {
                text += options.squote1;
                text += lib.strWrap(val, { wrap: options.wrap, nl: options.nl1, over: options.over, delim: options.delim, indent });
                text += options.squote2;
            } else {
