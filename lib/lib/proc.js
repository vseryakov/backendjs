...
lib.findProcess = function(options, callback)
{
    if (os.platform() === "linux") {
        ...
        lib.execProcess("/bin/ps agx -o pid,args", (err, stdout, _stderr) => {
...
lib.afindProcess = function(options)
{
...
lib.aexecProcess = function(cmd, options)
{
    return new Promise((resolve, _reject) => {
...
lib.spawnProcess = function(cmd, args, options, callback)
{
    ...
    let exited = 0
    const proc = child.spawn(cmd, args || [], options || {});
    proc.on("error", err => {
        if (exited++) return;
    ...
    proc.on('close', (code, signal) => {
        if (exited++) return;
        const err = code || signal ? lib.newError(`Failed ${cmd}`, { status: 500, signal, code, killed: proc.killed, args }) : undefined;
...
lib.aspawnProcess = function(cmd, args, options)
{
    return new Promise((resolve, _reject) => {
...
lib.aspawnSeries = function(cmds, options)
{
    return new Promise((resolve, _reject) => {
