...
// - concurrency - how many lines to process at the samer time
//
...
{
    fs.read(ctx.fd, ctx.buffer, 0, ctx.buffer.length, options.nlines == 0 ? options.start : null, (err, nread) => {
        ctx.data += ctx.buffer.slice(0, nread).toString(options.encoding || 'utf8');
        var lines = ctx.data.split(options.rxLine || this.rxLine), n = 0;
        // Only if not the last part
        if (nread == ctx.buffer.length) ctx.data = lines.pop();
        lib.forEachLimit(lines, options.concurrency, (line, next) => {
            ...
            if (options.json) {
                ...
                if (ctx.batch) ctx.batch.push(obj); else return lineCallback(obj, doNext);
            } else
            if (options.split) {
                ...
                options.ncalls++;
                options.nbytes += line.length;
                if (ctx.batch) ctx.batch.push(line); else return lineCallback(line, doNext);
            }
            if (!ctx.batch || ctx.batch.length < options.count) return doNext();
            ...
            if (options.abort || err ||
                ...
                nread < ctx.buffer.length) {
                if (err || !ctx.batch || !ctx.batch.length) return endCallback(err);
                return lineCallback(ctx.batch, endCallback);
            }
...
lib.forEachLineSync = function(file, options, lineCallback)
{
    ...
    while (!options.abort) {
        const nread = fs.readSync(fd, buffer, 0, buffer.length, options.nlines == 0 ? options.start : null);
        data += buffer.slice(0, nread).toString(options.encoding || 'utf8');
        const lines = data.split(options.rxLine || lib.rxLine);
    ...
    if (batch?.length) lineCallback(batch);
...
lib.readFileSync = function(file, options)
{
    ...
    try {
        ...
        if (options) {
            ...
            if (options.list) data = data.split(options.list); else
...
lib.findFileSync = function(file, options)
{
    ...
    try {
        ...
        var name = options?.base ? path.basename(file) : file;
        if (options?.details) stat.file = name;
    ...
    fs.readdir(dir, function(err, files) {
        ...
        lib.forEachSeries(files, function(file, next) {
            ...
            fs.stat(full, function(err, stat) {
                ...
                if (options.details) stat.file = file;
...
{
    var rc = 0;
    var list = path.normalize(dir).split(path.sep);
