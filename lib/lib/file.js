...
/**
 * Removes invalid characters `% : \ \x0 - \x19` making path safe but non-existent, does not support `..`
 * @param {...any} args - concat all parts using path.join after sanotizing each part
 * @memberof module:lib
 * @method sanitizePath
 * @example
...
{
    try {
        return path.normalize(path.join(...args.map((x) => lib.isString(x).replace(/([%:\\\x00-\x19])|(\.{2,})/g, ""))))
    } catch (e) {
        logger.error("normalize:", e, args);
    }
    return "";
}
