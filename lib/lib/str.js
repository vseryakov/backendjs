 ...
 * @memberof module:lib
 * @method testRegexpObj
 */
lib.testRegexpObj = function(str, rx)
{
    if (!util.types.isRegExp(rx?.rx)) return false;
    var rc = rx.rx.test(str);
    return rx.not ? !rc : rc;
}
 ...
 * - over - length factor, > 1 how long to go for delimiter forward, < 1 how long to go backwards
 ...
 * 'very long text\nline to wrap\nbackwards'
 */
lib.wrap = function(str, options)
{
    if (typeof str !== "string" || !str) return "";
    var wrap = options?.wrap || 80;
    if (str.length <= wrap) return str;
    ...
    const parse = (str, indent = "") => {
        let pos = 0
        for (; pos < str.length && pos < wrap && str[pos] !== nl; pos++);
        if (over >= 1) {
            for (; pos < str.length && pos < wrap*over && !delim.includes(str[pos]); pos++);
            for (; pos < str.length && delim.includes(str[pos]); pos++);
        } else {
            const save = pos;
            for (; pos > 0 && pos > wrap*over && !delim.includes(str[pos]); pos--);
            if (!delim.includes(str[pos])) pos = save;
        }
        text += indent + quotes[0] + str.substr(0, pos).trim() + quotes[1];
        return str.substr(pos).trim();
    }

    str = parse(str);
    if (str) text += nl;
    while (str) {
        str = parse(str, indent);
        if (!str) break;
        text += nl;
    }
    return text;
 ...
 * @param {object} [options]
 * @param {string} [options.data_type] will be used with `lib.toValue` to convert the value for each item
 ...
 * @method split
 */
lib.split = function(str, sep, options)
{
    if (!str) return [];
    ...
    for (let i = 0; i < len; ++i) {
        ...
        if (options?.data_type) {
            v = lib.toValue(v, options.data_type, options);
        }
        if (typeof v !== "string") {
            rc.push(v);
            continue;
        ...
        for (let k = 0; k < keys?.length; ++k) {
            switch (keys[k]) {
            case "range":
                const dash = v.indexOf("-", 1);
                if (dash === -1) break;
                let s = lib.toNumber(v.substr(0, dash));
                const e = lib.toNumber(v.substr(dash + 1));
            ...
            case "regexp":
                if (typeof options.regexp.test !== "function") break;
                if (!options.regexp.test(v)) v = "";
                break;

            case "no_regexp":
                if (typeof options.no_regexp.test !== "function") break;
 ...
 * @param {string} str
 * @param {object} [options]
 * @param {boolean} [options.keep_empty] is given all empty parts will be kept in the list.
 * @return {string[]}
 * @memberof module:lib
 * @method phraseSplit
 */
lib.phraseSplit = function(str, options)
{
    ...
    while (i < len) {
        ...
        if (quotes.indexOf(str[i]) > -1) {
            ...
            if (q < len) {
                if (keep_empty || q - i > 0) rc.push(str.substr(i, q - i));
        ...
        if (q >= len) {
            if (keep_empty || len - i > 0) rc.push(str.substr(i, len - i));
            break;
        } else {
            if (keep_empty || q - i > 0) rc.push(str.substr(i, q - i));
        }
...
{
    var pad = "";
    while (pad.length < width - 1 && n < 10 ** (width - pad.length - 1)) pad += "0";
    return pad + String(n);
}
...
lib.sprintf = function(fmt, ...args)
{
    ...
    return fmt.replace(regex, (sym, p0, p1, p2, p3, p4) => {
        ...
        var arg = args[i];
        var exp = p2 ? Number.parseInt(p2.substr(1)) : undefined;
        var base = p3 ? Number.parseInt(p3.substr(1)) : undefined;
        var val;
        switch (p4) {
        case 's':
            val = arg;
        ...
        case 'c':
            ...
            break;
        case 'f':
            val = Number.parseFloat(arg).toFixed(exp);
        ...
        case 'g':
            ...
            break;
        case 'p':
            val = Number.parseFloat(arg).toPrecision(exp);
            if (Number.isNaN(val)) val = 0;
            break;
        case 'e':
            val = Number.parseFloat(arg).toExponential(exp);
            if (Number.isNaN(val)) val = 0;
            break;
        case 'x':
            val = Number.parseInt(arg).toString(base ? base : 16);
            if (Number.isNaN(val)) val = 0;
            break;
        case 'd':
            val = Number.parseFloat(Number.parseInt(arg, base ? base : 10).toPrecision(exp)).toFixed(0);
            if (Number.isNaN(val)) val = 0;
            break;
        }
        val = typeof val === "object" ? JSON.stringify(val) : val.toString(base);
        var sz = Number.parseInt(p1); /* padding size */
 ...
 * @param {string} data
 * @param {string} [encoding] - base64, utf16
 * @memberof module:lib
 * @method lzCompress
 */
lib.lzCompress = function(data, encoding)
{
    switch (encoding) {
    case "base64":
        const rc = this._lzCompress(data, 6, function(a) { return lib.base64.charAt(a) });
        switch (rc.length % 4) {
        case 1:
            return rc + "===";
        case 2:
        ...
        return rc;
    case "utf16":
        return this._lzCompress(data, 15, function(a) { return String.fromCharCode(a + 32) }) + " ";
    default:
        return this._lzCompress(data, 16, String.fromCharCode);
    }
}
 ...
 * @method lzDecompress
 */
lib.lzDecompress = function(data, encoding)
{
    if (data === null || data === "") return "";
    switch (encoding) {
    case "base64":
        if (!this.base64Dict.A) for (let i = 0; i < this.base64.length; i++) this.base64Dict[this.base64.charAt(i)] = i;
        return this._lzDecompress(data.length, 32, function(index) { return lib.base64Dict[data.charAt(index)] });
    case "utf16":
        return this._lzDecompress(data.length, 16384, function(index) { return data.charCodeAt(index) - 32; });
    default:
        return this._lzDecompress(data.length, 32768, function(index) { return data.charCodeAt(index); });
    }
}

lib._lzCompress = function(data, bitsPerChar, getCharFromInt)
{
    if (data === null || data === "") return "";
    ...
    for (ii = 0; ii < data.length; ii += 1) {
        cc = data.charAt(ii);
        if (!Object.hasOwn(dict,cc)) {
            dict[cc] = dictSize++;
            _dict[cc] = true;
        }
        cwc = cw + cc;
        if (Object.hasOwn(dict,cwc)) {
            cw = cwc;
        } else {
            if (Object.hasOwn(_dict,cw)) {
