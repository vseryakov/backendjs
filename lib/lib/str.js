...
lib.wrap = function(str, options)
{
    ...
    var text = "" + quotes[0], t;
    for (t = 0; t < str.length && t < wrap && str[t] != nl; t++);
    for (; t < str.length && t < wrap*over && !delim.includes(str[t]); t++);
    for (;t < str.length && delim.includes(str[t]); t++);
    text += str.substr(0, t).trim() + quotes[1] + nl;
    str = str.substr(t).trim();
    while (str) {
        for (t = 0; t < str.length && t < wrap && str[t] != nl; t++);
        for (; t < str.length && t < wrap*over && !delim.includes(str[t]); t++);
        for (;t < str.length && delim.includes(str[t]); t++);
        text += indent + quotes[0] + str.substr(0, t).trim() + quotes[1];
        str = str.substr(t).trim();
        if (!str) break;
        text += nl;
    }
