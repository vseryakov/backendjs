...
// where the index points to the last character of the found keywords
// Example:
...
//
// If `delimiters` string is provided then return words only if surrounded by characters in the delimiters, this is
// to return true words and not substrings
//
lib.AhoCorasick.prototype.search = function(text, delimiters)
{
    if (typeof text != "string") text = "";
    if (typeof delimiters != "string") delimiters = null;
        ...
        if (this.output[state]) {
            if (delimiters) {
                if (!text[i + 1] || delimiters.includes(text[i + 1])) {
                    const b = text[i - this.output[state][0].length];
                    if (!b || delimiters.includes(b)) {
                        rc.push([i, this.output[state]]);
                    }
                }
            } else {
                rc.push([i, this.output[state]]);
