...
lib.strWrap = function(val, options)
{
    ...
    var text = "", t;
    for (t = wrap; t < val.length && t < wrap*over && !delim.includes(val[t]); t++);
    for (;t < val.length && delim.includes(val[t]); t++);
    text += val.substr(0, t).trimStart() + nl;
    val = val.substr(t);
    while (val) {
        for (t = wrap; t < val.length && t < wrap*over && !delim.includes(val[t]); t++);
        for (;t < val.length && delim.includes(val[t]); t++);
        text += indent + val.substr(0, t).trimStart();
        val = val.substr(t);
        if (!val) break;
        text += nl;
    }
