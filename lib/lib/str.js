 ...
 * @param {object} [options]
 * @param {string} [options.datatype] will be used with `lib.toValue` to convert the value for each item
...
lib.split = function(str, sep, options)
{
    ...
    for (let i = 0; i < len; ++i) {
        ...
        if (options?.datatype) {
            v = lib.toValue(v, options.datatype, options);
        }
        if (typeof v !== "string") {
            rc.push(v);
            continue;
 ...
 * @param {string} str
 * @param {object} [options]
 * @param {boolean} [options.keepempty] is given all empty parts will be kept in the list.
 * @return {string[]}
 * @memberof module:lib
 * @method phraseSplit
 */
