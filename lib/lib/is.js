...
/**
 Returns the string value or empty string if `val` is not a string
 * @param {string} val
 * @return {string}
 */
...
{
    return Array.isArray(list) && (Array.isArray(name) ? name.some((x) => (list.includes(x))) : name && list.includes(name));
}
