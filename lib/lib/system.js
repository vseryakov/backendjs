...
lib.networkStats = function(callback)
{
    ...
    if (os.type() == "Linux") {
        ...
        function _parse(data) {
            ...
            stats.net_rx_bytes = lib.toNumber(eth0[1]);
            stats.net_rx_packets = lib.toNumber(eth0[2]);
            stats.net_rx_errors = lib.toNumber(eth0[3]);
            stats.net_rx_dropped = lib.toNumber(eth0[4]);
            stats.net_tx_bytes = lib.toNumber(eth0[9]);
            stats.net_tx_packets = lib.toNumber(eth0[10]);
            stats.net_tx_errors = lib.toNumber(eth0[11]);
            stats.net_tx_dropped = lib.toNumber(eth0[12]);
            if (lib._eth0) {
                var t = (now - lib._eth0.now)/1000;
                stats.net_rx_rate = lib.toNumber((stats.net_rx_bytes - lib.toNumber(lib._eth0[1])) / t, { digits: 2 });
                stats.net_tx_rate = lib.toNumber((stats.next_tx_bytes - lib.toNumber(lib._eth0[9])) / t, { digits: 2 });
            }
            lib._eth0 = eth0;
        }
