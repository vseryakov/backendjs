...
{
    for (const w of this.getWorkers(filter)) {
        try { process.kill(w.process.pid) } catch (_e) {}
    }
}
 ...
 * @method domainName
 */
lib.domain = function(host, toplevel)
{
    if (typeof host !== "string" || !host) return "";
    var name = this.split(host, '.');
    var colon = name.at(-1).indexOf(":");
    if (colon > 0) {
        name.push(name.pop().slice(0, colon));
    }
    return (toplevel ? name.slice(-2).join(".") : name.length > 2 ? name.slice(1).join('.') : name.join(".")).toLowerCase();
}
