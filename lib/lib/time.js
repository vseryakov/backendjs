...
lib.parseTime = function(time)
{
    ...
    const d = time.trim().match(/^(([0-9]+)|([0-9]+):([0-9]+)) *(am|pm)?$/i);
    if (!d) return;
...
lib.isTimeRange = function(time1, time2, options)
{
    ...
    let now = new Date(), tz = options?.tz;
    if (tz === "GMT" || tz === "UTC") {
        tz = 0;
    } else {
        tz = typeof tz === "string" && tz.match(/GMT(-|\+)?([0-9]{2}):?([0-9]{2})/);
        if (tz) tz = (Number.parseInt(tz[2], 10) * 3600000 + Number.parseInt(tz[3], 10) * 60000) * (tz[1] === "+" ? 1 : -1);
    ...
    const h0 = now.getUTCHours();
    const m0 = now.getUTCMinutes();
    if (time1) {
...
const _strftime = {
    a(t, utc, locale, _tz) {
        return lib.__({ phrase: weekDays[utc ? t.getUTCDay() : t.getDay()], locale })
    },
    A(t, utc, locale, _tz) {
        return lib.__({ phrase: weekDaysFull[utc ? t.getUTCDay() : t.getDay()], locale })
    },
    b(t, utc, locale, _tz) {
        return lib.__({ phrase: months[utc ? t.getUTCMonth() : t.getMonth()], locale })
    },
    B(t, utc, locale, _tz) {
        return lib.__({ phrase: monthsFull[utc ? t.getUTCMonth() : t.getMonth()], locale })
    },
    c(t, utc, _locale, _tz) {
    ...
    Q(t, utc, locale, _tz) {
        ...
        return h < 12 ? lib.__({ phrase: "Morning", locale }) :
               h < 17 ? lib.__({ phrase: "Afternoon", locale }) :
                        lib.__({ phrase: "Evening", locale }) },
    '%'() { return '%' },
};
...
lib.strftime = function(date, fmt, options)
{
    ...
    const locale = options?.lang || options?.locale;
    ...
    for (const p in _strftime) {
        fmt = fmt.replace('%' + p, _strftime[p](date, utc, locale, tz));
    }
    return fmt;
}
