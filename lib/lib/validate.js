 ...
 *   - list types: list, set
 *   - object types: map, obj, object, array, json,
 *   - date/time types: mtime, date, time, timestamp, datetime
 *   - special types: email, symbol, url, phone, e164, regexp
 *
 * @property {boolean} [dflt] - use this value if property does not exists or undefined
 * @property {boolean} [dflt_empty] - also use the dflt value for empty properties
 ...
 * @property {boolean} [strict] - perform strict validation
 *   - for numeric types return an error if not a number, uses {@link module:lib.isNumeric} before converting
 ...
 *  processed by {@link module:lib.toTemplate}. Any properties listed here can be used like @label@, @max@, @max_date||date@
 ...
 * @property {regexp|string} [regexp] - validate input against this regexp and return an error if not matched, for list type skip items not matched
 * @property {regexp|string} [no_regexp] - validate the input against this regexp and return an error if matched, for list type skip items matched
 * @property {boolean} [map_type] - for maps convert each value to this type
 * @property {boolean} [no_value] - if the target value equals then ignore the parameter,
 ...
 * @property {boolean} [set_empty] - to be used with `empty`, instead of skipping set with this value at the end
 * @property {boolean} [not_empty] - if true and the target value is empty after type conversion return an error
 * @property {boolean} [keep_empty] - for list type keep empty items in the list, default is skip empty items
 * @property {boolean} [min_list] - min allowed length of the target array for list/map types, returns error if less
 * @property {boolean} [max_list] - max allowed length of the target array for list/map types, returns error if longer
 * @property {boolean} [min_num] - min allowed number after convertion by toNumber, for numbers and mtime
 * @property {boolean} [max_num] - max allowed number after convertion by toNumber, for numbers and mtime
 * @property {boolean} [min_date] - min allowed date after convertion by toDate, can be a Date or number
 * @property {boolean} [max_date] - max allowed date after convertion by toDate, can be a Date or number
 ...
 * @property {boolean} [data_type] - convert each value or item into this type, used by string/list types
 * @property {boolean} [strip] - a regexp with characters to strip from the final value
 * @property {boolean} [upper] - transform to upper case
 * @property {boolean} [lower] - transform to lower case
 ...
 * - convert incoming user data into desired types, means the schema is not strict (no `strict` flag) in the sense that all
 *   incoming data must be of desired type, all transformations like strip/replace.... are run in the order as defined in
 *   the schema object to control order of changes
 *
 ...
 *        age: { type: "int", min_num: 10, max_num: 99 },
 ...
 *        data: { type: "json", data_type: "obj" },
 *        mtime: { type: "mtime", name: "timestamp" },
 *        date: { type: "date", min_date: new Date(2000,1,1) },
 ...
 *        phone: { type: "list", data_type: "number" },
 *        }, {
 *        defaults: {
...
{
    const onError = (opts, name, value, errmsg, extra) => {
        const message = lib.toTemplate(opts.errmsg || errmsg, [{ label: opts.label || name, name, value }, opts]);
        const err = { status: 400, message, name, code: "validate" };
        if (extra) Object.assign(err, extra);
        return { err };
    };
    const data = Object.create(null), dflts = options?.defaults, required = [];
    let item, opts, dopts, p, n, v, e, old

    for (const name in schema) {
        if (lib.badProperties.has(name)) continue;
        item = schema[name];
        switch (this.typeName(item)) {
        case "undefined":
            continue;
        case "object":
        ...
        opts = Object.assign(Object.create(null), item)
        dopts = dflts ? (options?.dprefix ? dflts?.[options.dprefix + name] : null) ||
                dflts[name] ||
        ...
        if (lib.badProperties.has(p)) continue;
        ...
        if (v === undefined || (opts.dflt_empty && this.isEmpty(v))) {
            v = opts.dflt;
        }
        ...
        old = v;
        logger.dev("validate:", name, n, typeof v, "VAL:", v, "OPTS:", opts, "DOPTS:", dopts);
        ...
        case "decimal":
            ...
            if (opts.strict && !lib.isNumeric(v)) {
                return onError(opts, name, v, "@label@ must be a number");
            }
            opts.float = 1;
            data[n] = this.toNumber(v, opts);
            break;
        ...
        case "list":
            ...
            v = opts.keep_empty ? (Array.isArray(v) ? v : this.phraseSplit(v, opts)) : this.split(v, opts.separator, opts);
            if (Array.isArray(opts.values)) v = v.filter((x) => (opts.values.indexOf(x) > -1));
            if (Array.isArray(opts.no_value)) v = v.filter((x) => (opts.no_value.indexOf(x) === -1));
            if (opts.min_list > 0 && v.length < opts.min_list) {
                return onError(opts, name, v, "@label@ is too short, the min size is @min_list@");
            }
            if (opts.max_list > 0 && v.length > opts.max_list) {
                if (!opts.trunc) {
                    return onError(opts, name, v, "@label@ is too long, the max size is @max_list@")
                }
                v = v.slice(0, opts.max_list);
            }
        ...
        case "map":
            ...
            if (!lib.isObject(data[n])) {
                data[n] = Object.create(null);
            }
            Object.assign(data[n], v);
            break;
        ...
        case "array":
            ...
            if (opts.min_list > 0 && v.length < opts.min_list) {
                return onError(opts, name, v, "@label@ is too short, the min length is @min_list@")
            }
            if (opts.max_list > 0 && v.length > opts.max_list) {
                if (!opts.trunc) {
                    return onError(opts, name, v, "@label@ is too long, the max length is @max_list@")
                }
                v = v.slice(0, opts.max_list);
            }
        ...
        case "mtime":
            if (!v) break;
            v = this.toDate(v, opts.dflt);
            if (v.getTime()) {
                if (opts.min_date && v < opts.min_date) {
                    return onError(opts, name, v, "@label@ is too soon, the earliest date is @min_date||date@");
                }
                if (opts.max_date && v > opts.max_date) {
                    return onError(opts, name, v, "@label@ is too late, the latest date is @max_date||date@");
                }
                data[n] = v.getTime();
            }
            break;
        ...
        default:
            ...
            if (opts.no_regexp) {
                if (typeof opts.no_regexp === "string") {
                    opts.no_regexp = item.no_regexp = lib.toRegexp(opts.no_regexp);
                }
                if (lib.testRegexp(v, opts.no_regexp)) {
                    if (!opts.required && opts.errmsg) {
                        return onError(opts, name, v, "invalid characters in @label@", { pattern: opts.no_regexp });
                    }
                    break;
                }
            } else
            ...
            if (opts.regexp) {
                if (typeof opts.regexp === "string") {
                    opts.regexp = item.regexp = lib.toRegexp(opts.regexp);
                }
                if (!lib.testRegexp(v, opts.regexp)) {
                    if (!opts.required && opts.errmsg) {
                        return onError(opts, name, v, "invalid characters in @label@", { pattern: opts.regexp });
                    }
                    break;
                }
            }
            ...
            for (const p in opts) {
                ...
                case "cap":
                    ...
                    break;
                case "data_type":
                    v = lib.toValue(v, opts.data_type, opts);
                    break;
                }
            }
            if (!v && !opts.empty) break;
        ...
        if (lib.isEmpty(v)) {
            if (opts.set_empty !== undefined) {
                v = data[n] = opts.set_empty;
            }
            if (opts.not_empty && !lib.isEmpty(old)) {
                return onError(opts, name, v, "@label@ is invalid")
            }
        } else {
            switch (opts.type) {
            case "list":
                if (typeof opts.no_value === "number" && v.length <= opts.no_value) {
                    delete data[n];
                }
                break;
            ...
            default:
                if (typeof v === "number") {
                    if (opts.max_num && v > opts.max_num) {
                        return onError(opts, name, v, "@label@ is too large, the max value is @max_num@");
                    }
                    if (opts.min_num > 0 && v < opts.min_num) {
                        return onError(opts, name, v, "@label@ is too small, the min value is @min_num@");
                    }
                }
                ...
                if (v === opts.no_value || Array.isArray(opts.no_value) && opts.no_value.includes(v)) {
                    delete data[n];
                } else

                if (typeof opts.no_value === "object") {
                    if (v === data[opts.no_value.name] || v === opts.no_value.value) delete data[n];
                } else
