...
lib.flatten = function(obj, options)
{
    ...
    for (let p in obj) {
        ...
        const idx2 = Array.isArray(v) && typeof options?.index === "number" ? options.index : 0;
        for (const x in v2) {
            const p2 = idx2 ? lib.toNumber(x) + idx2 : x;
            rc[p + sep + p2] = v2[x];
        }
...
lib.objSize = function(obj, options, priv)
{
    ...
    let rc = 0;
    const depth = options?.depth || 10, pad = lib.toNumber(options?.pad) || 5;
