...
lib.shuffle = function(list)
{
    ...
    for (let i = 0; i < list.length; i++) {
        const j = Math.round((list.length - 1) * this.randomFloat());
        if (i === j) {
            continue;
        }
        const item = list[j];
...
lib.arrayUnique = function(list, key)
{
    ...
    const rc = [], keys = Object.create(null);
    list.forEach((x) => {
        if (key) {
 ...
 * // => ["admin", "verified"]
 */
lib.arrayUpdate = function(cmd, list, name)
{
    switch (cmd) {
    case "concat":
    ...
    case "update":
        ...
        name.forEach((x) => {
            if (typeof x === "string" && x[0] === "-") {
                const i = list.indexOf(x.substr(1));
...
{
    var rc = Array.isArray(obj) ? [] : Object.create(null), o1, o2;
    for (const p in obj) {
        if (lib.badProperties.has(p)) continue;
        if (!Object.hasOwn(obj, p)) continue;
        o1 = obj[p];
 ...
 *  - depth - only go this deep into objects
 * @return {object}
 * @example
...
lib.flatten = function(obj, options)
{
    ...
    const idx = Array.isArray(obj) && typeof options?.index === "number" ? options.index : 0;
    ...
    for (let p in obj) {
        ...
        const v = obj[p];
        p = idx ? lib.toNumber(p) + idx : p;

        if (typeof v !== 'object' || v === null) {
            if (v !== undefined) rc[p] = v;
            continue;
        }

        if (options.depth && options.__depth >= options.depth) {
            continue;
        }
        ...
        const v2 = this.flatten(v, options);
        for (const x in v2) {
            rc[p + sep + x] = v2[x];
        }
...
{
    obj = Array.isArray(obj) || lib.isObject(obj) || typeof obj === "function" ? obj : Object.create(null);
 ...
 * the name uses . to separate objects. This is a convenient method to extract properties from nested objects easily and dynamically when optional chanining
 * cannot be used.
 * @param {object} obj
 * @param {string} name
 * @param {object} [options]
...
lib.objGet = function(obj, name, options)
{
    if (!obj) {
        ...
        return options.list ? [] : options.obj ? Object.create(null) : options.str ? "" : options.num ? options.dflt || 0 : null;
    }
    const path = Array.isArray(name) ? name: String(name).split(".");
    let owner = obj;
    for (let i = 0; i < path.length; i++) {
        if (lib.badProperties.has(path[i])) return obj;
        if (i && owner) {
            owner = owner[path[i - 1]];
        }
    ...
    if (options) {
        ...
        if (obj) {
            ...
            if (options.obj && typeof obj !== "object") return Object.assign(Object.create(null), { name, value: obj });
...
{
    if (this.typeName(obj) !== "object") obj = Object.create(null);
    if (!Array.isArray(name)) name = String(name).split(options?.separator || ".");
    if (!name?.length) return obj;

    const p = name[name.length - 1];
    // Skip prototype properties
    if (lib.badProperties.has(p)) return obj;

    const v = obj;

    for (let i = 0; i < name.length - 1; i++) {
        // Skip prototype properties in the path
        if (lib.badProperties.has(name[i])) continue;
        if (typeof obj[name[i]] === "undefined") {
            obj[name[i]] = Object.create(null);
        }
        obj = obj[name[i]];
    }
    let old = obj[p];
 ...
 * @param {object} [options]
 * @param {int} [options.depth] - limits how deep it goes, on limit returns a number greater than MAX_SAFE_INTEGER
...
lib.objSize = function(obj, options, priv)
{
    ...
    let rc = 0;
    const depth = options?.depth || 10, pad = lib.toNumber(options?.pad, { dflt: 5 });
    if (this.typeName(obj) !== "object") obj = { "": obj };

    if (typeof priv?.depth !== "number") {
        priv = { depth: 0, seen: new WeakSet() };
    }
    ...
    for (const p in obj) {
        ...
        case "array":
            if (priv.depth > depth || priv.seen.has(v)) return options?.nan ? Number.NaN : Number.MAX_SAFE_INTEGER;
            priv.seen.add(v);
            priv.depth++;
