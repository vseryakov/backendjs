 ...
 *
 * @param {string|object} url - can be full URL or an object with property `url:`
 * @param {object|FetchRequest} [options] - customize request with options, see {@link module:lib.FetchRequest}
...
{
    logger.dev("fetch:", url, options, callback);
    if (typeof options === "function") callback = options, options = null;
    if (!options && url?.url) options = url, url = url.url;

    if (!(options instanceof FetchRequest)) {
        logger.dev("httpNew:", url, options, callback);
        options = new FetchRequest(options);
    }

    var opts = options.init(url, callback);
    if (!opts) return;
    logger.dev("httpStart:", url, opts);
 ...
 * @property {function} retryPrepare - a function to be called before retrying, it can update any parameter, most related: _url, retryTimeout, retryMultiplier
...
class FetchRequest {
    ...
    init(url, callback)
    {
        ...
        if (query) {
            url += (url.includes("?") ? "" : "?") + (query[0] === "&" ? "" : "&") + query;
        }
        ...
        this.date = null;
        this.obj = null;
        this.headers = Object.assign(Object.create(null), this.headers);
        this.resheaders = Object.create(null);
        this.stime = Date.now();
        ...
        logger.dev("httpOpen:", this.method, this.url, "HDR:", this.headers, "POST:", this.postsize, this.postdata);
            ...
            if (this.chunked) {
                ...
                if (typeof this.postsize !== "number") {
                    fs.stat(this.postfile, (err, stats) => {
                        ...
                        lib.fetch(this.url, this, callback);
                    });
                    return;
                }
    ...
    onError(err, callback)
    {
        ...
        if (this.errorCount-- > 0) {
            ...
            setTimeout(lib.fetch.bind(null, this._url, this, callback), lib.objMult(this, "retryTimeout", this.retryMultiplier, "old"));
    ...
    onEnd(res, callback)
    {
        ...
        if (this.cookies) {
            this.rescookies = this.parseCookies(headers["set-cookie"]);
        }
        ...
        if ((this.status < 200 || this.status >= 400) &&
            ...
            if (!this.quiet) {
                logger.debug("httpRetry:", this.toJSON({ query: 1, postdata: 256, data: 256 }));
            }
            if (typeof this.retryPrepare === "function") {
                this.retryPrepare.call(this);
            }
            setTimeout(lib.fetch.bind(null, this._url, this, callback), lib.objMult(this, "retryTimeout", this.retryMultiplier, "old"));
            if (this.warnTimeout && this.retryTimeout >= this.warnTimeout) {
                logger.warn("httpRetry:", this.toJSON({ query: 1, postdata: 256, data: 256 }));
            }
        ...
        if (this.data && !this.noparse) {
            const opts = { datatype: this.datatype, logger: this.datalogger, url: this.url };
            switch (this.type) {
            case "text/json":
    ...
    close()
    {
        ...
        if (this.stream) {
            try { this.stream.end(this.onFinish); } catch (_e) {}
            delete this.stream;
        }
    }
    ...
    toJSON(options)
    {
        ...
        if (this.lastError) rc.lastError = this.lastError;
    ...
    checkRedirect(callback)
    {
        ...
        case 308:
            ...
            if (url.indexOf("://") === -1) url = this.url.split("/").slice(0, 3).join("/") + url;
            const cols = ['method','query','postdata','postfile','poststream','signer','checksum'];
            for (const c of cols) delete this[c];
            const headers = Object.create(null);
            for (const i in this.passheaders) {
                headers[this.passheaders[i]] = this.headers[this.passheaders[i]];
            }
            this.headers = headers;
            lib.fetch(url, this, callback);
            return true;
        }
    }
