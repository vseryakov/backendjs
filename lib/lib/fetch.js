...
class FetchRequest {
    ...
    parseSetCookies(header) {
        ...
        const str = Array.isArray(header) ? header.filter(x => lib.isString(x)).join("\n") : lib.isString(header);
        if (str.length > 65536) return cookies;

        for (const item of str.split(/[:](?=\s*[a-zA-Z0-9_-]+\s*[=])/g)) {
            const parts = item.split(";");
            const pair = parts[0].match(/([^=]+)=([\s\S]*)/);
            if (!pair) continue;
