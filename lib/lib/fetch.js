...
lib.fetch = function(url, options, callback)
{
    ...
    const opts = options.init(url, callback);
    if (!opts) return;

    options.logger("dev", "new:");
    ...
    try {
        req = mod.request(opts, (res) => {
            ...
            res.on("close", () => {
                ...
                if (!req.complete && options.streaming) req.destroy();
            });
        });
        req.on('error', (err) => {
 ...
 * @property {function} retryPrepare - a function to be called before retrying, it can update any parameter,
...
class FetchRequest {
    ...
    constructor(options)
    {
        ...
        this.retryTotal = options.retryTotal || options.retryCount;
        ...
        this.warnTimeout = lib.toNumber(options.warnTimeout, { min: 0, dflt: 30000 });
        ...
        this.postdata = options.postdata || options.body;
    ...
    prepare(callback)
    {
        ...
        if (this.checksum && this.postdata) {
            this.checksum = lib.hash(this.postdata, "sha1");
        }
    ...
    preparePost(callback)
    {
        ...
        if (this.postfile) {
            ...
            this.poststream.on("error", (err) => {
                this.logger("error", 'poststream:', err.stack)
            });
        }
        return true;
    }
        ...
        try {
            ...
            if (this.fd) {
                ...
                fs.write(this.fd, chunk, 0, chunk.length, null, (err) => {
                    ...
                    this.logger("error", "writeFile:", err);
                    this.err = err;
                    res.req.abort();
                });
            }
        } catch (e) {
            this.logger("error", "writeFile:", e);
            this.err = e;
            res.req.abort();
        }
    }
    ...
    onError(err, callback)
    {
        ...
        if (this.errorCount-- > 0) {
            ...
            setTimeout(lib.fetch(this.origUrl, this, callback), lib.objMult(this, "retryTimeout", this.retryMultiplier, "old"));

            if (this.warnTimeout && this.retryTimeout >= this.warnTimeout) {
                this.logger("warn", "onError:", err, "retry:", this.errorCount, this.retryTimeout);
            }
        } else {
    ...
    onEnd(res, callback)
    {
        ...
        if ((this.status < 200 || this.status >= 400) &&
            ...
            if (!this.quiet) {
                this.logger("debug", "onEnd:", "retry:", this.retryCount, this.retryTimeout);
            }
            ...
            if (this.warnTimeout && this.retryTimeout >= this.warnTimeout) {
                this.logger("warn", "onEnd:", "retry:", this.retryCount, this.retryTimeout);
            }
