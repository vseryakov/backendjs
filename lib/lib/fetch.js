 ...
 * @property {string} password - for Basic authorization
...
class FetchRequest {
    ...
    onEnd(res, callback)
    {
        ...
        if ((this.status < 200 || this.status >= 400) &&
            ...
            this.retryCount-- > 0) {
            this.lastError = `${this.status}: ${this.data.slice(0, 256)}`;
        ...
        if (this.data && !this.binary && !this.noparse) {
        ...
        if ((this.status < 200 || this.status > 299) && !this.err && this.raise) {
            ...
            if (!lib.isEmpty(this.obj)) {
                ...
                this.err = { message: "Error " + this.status + (this.data ? ": " + this.data.slice(0, 256) : ""), status: this.status };
            }
    ...
    parseSetCookies(header) {
        ...
        const headers = Array.isArray(header) ? header.filter(x => lib.isString(x)) : [lib.isString(header)];
        ...
        for (const item of headers) {
            ...
            const pair = parts[0].match(RX);
            if (!pair) continue;

            const name = pair[1].trim();
            const cookie = { value: (pair[2] || "").trim() };
            if (cookie.value.includes("%")) {
                cookie.value = lib.decodeURIComponent(cookie.value);
            }

            for (let i = 1; i < parts.length; i++) {
                const p = parts[i].match(RX);
                if (!p) continue;
                const key = p[1].trim().toLowerCase();
                let value = (p[2] || "").trim();
                if (value.includes("%")) {
        ...
        this.logger("dev", "parseSetCookies:", cookies);
        return cookies;
    }
}

lib.FetchRequest = FetchRequest;
