...
lib.fetch = function(url, options, callback)
{
    ...
    if (options.poststream) {
        stream.pipeline(options.poststream, req, (err) => {
            options.logger("error", 'poststream:', err.stack);
        });
        return req;
    }
...
class FetchRequest {
    ...
    onData(res, chunk)
    {
        ...
        this.size += Buffer.isBuffer(chunk) ? chunk.length: Buffer.byteLength(chunk);
        this.logger("dev", "onData:");

        if (this.maxSize > 0 && this.size > this.maxSize) {
            res.req.destroy(lib.newError("too large", 413, "toolarge"));
        }
    }
