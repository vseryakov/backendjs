...
lib.Pool = class Pool {
    ...
    */
    use(callback)
    {
        ...
        if (this.#busy.length >= this.#max) {
            if (this.#queue_count >= this.#max_queue) {
                return callback(lib.newError("no more resources", 503));
            }

            this.#queue_count++;
            return lib.deferCallback({}, function(m) {
                callback(m.item ? null : lib.newError("timeout waiting for the resource", 504), m.item);
            }, this.#timeout);
        }
        // New item
        this.#call("_create", (err, item) => {
            if (err) {
                ...
                if (!item) item = {};
                this.#busy.push(item);
                logger.dev('use:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length);
            }
            callback(err, item);
        });
    }
    ...
    destroy(item, callback)
    {
        ...
        logger.dev('destroy:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length);
    ...
    release(item)
    {
        ...
        logger.dev('release:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length, 'max:', this.#max);

        // Pass it to the next waiting item
        for (const id in this.#queue) {
            this.#queue_count--;
            this.#queue[id].item = item;
            return lib.runCallback(this.#queue[id]);
        }
    ...
    */
    ...
    {
        logger.debug('shutdown:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length);
        this.#max = -1;
        this.destroyAll();
        this.#queue = {};
        clearInterval(this.#interval);
    ...
    #call(name, callback)
    {
        ...
        try {
            ...
            logger.error('call:', 'pool', this.name, name, e);
        ...
        if (this.#idle > 0) {
            for (let i = 0; i < this.#avail.length; i++) {
                if (now - this.#mtime[i] > this.#idle && this.#avail.length + this.#busy.length > this.#min) {
                    logger.dev('timer:', 'pool', this.name, 'idle', i, 'avail:', this.#avail.length, 'busy:', this.#busy.length);
