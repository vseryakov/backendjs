...
lib.Pool = class Pool {
    ...
    */
    use(callback)
    {
        ...
        this.#call("_create", this, (err, item) => {
            if (err) {
                logger.error("use:", 'pool', this.name, lib.traceError(err));
            } else {
                if (!item) item = {};
                this.#busy.push(item);
                logger.dev('use:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length, 'queue:', this.#queue.length);
            }
            callback(err, item);
        });
    }
    ...
    release(item)
    {
        ...
        const idx = this.#busy.indexOf(item);
        if (idx === -1) {
            logger.trace('release:', 'pool', this.name, 'not known', item);
            return;
        }
        logger.dev('release:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length, 'queue:', this.#queue.length, 'max:', this.#max);
    ...
    destroy(item, callback)
    {
        ...
        logger.dev('destroy:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length, 'queue:', this.#queue.length);

        let idx = this.#busy.indexOf(item);
        if (idx > -1) {
            this.#busy.splice(idx, 1);
            return this.#call("_destroy", item, callback);
        }
    ...
    */
    ...
    {
        logger.debug('shutdown:', 'pool', this.name, 'avail:', this.#avail.length, 'busy:', this.#busy.length, 'queue:', this.#queue.length);
        this.#max = -1;
        clearInterval(this.#interval);
        this.#interval = null;
        this.destroyAll();
    ...
    #call(name, item, callback)
    {
        ...
        try {
            ...
            logger.error('call:', 'pool', this.name, name, item, e);
        ...
        if (this.#idle > 0) {
            for (let i = 0; i < this.#avail.length; i++) {
                if (now - this.#mtime[i] > this.#idle && this.#avail.length + this.#busy.length > this.#min) {
                    logger.dev('timer:', 'pool', this.name, 'idle', i, 'avail:', this.#avail.length, 'busy:', this.#busy.length, 'queue:', this.#queue.length);
