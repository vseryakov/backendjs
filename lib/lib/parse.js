 ...
 *   formatted as @name@ where name can refer to any property inside the `options` or inside the `modules` as
 *   `@module.name@`
 *
 * Supports sections:
 ...
 * @param {object} [options] - options with context
 * @param {object} [options.modules] - an object with modules to use for section conditions, in most cases it is {$link module:modules}
...
lib.configParse = function(data, options)
{
    ...
    var argv = [], lines = lib.strSplit(data, "\n"), section;
 ...
 * @params {string} [options.datatype] - make sure the result is returned as type: obj, list, str
 * @params {string} [options.dflt] - return this in case of error
 * @params {string} [options.empty] - if true silent about empty input, no logging
 * @params {string} [options.logger] - report in the log with the specified level, log, debug, ...
