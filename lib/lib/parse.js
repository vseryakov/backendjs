...
const fxp = require(__dirname + '/../../dist/fxp.cjs');
 ...
 * @param {object} [options.context] - all properties from the context can be used in the sections conditions and file names
...
lib.configParse = function(data, options)
{
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith("include ")) {
            const file = lib.toTemplate(line.substr(8), [options?.context]).trim();
            const incl = lib.readFileSync(file, { list: "\n" });
            lines.splice(i--, 1, ...incl);
            continue;
        } else

        if (line.startsWith("[") && line.endsWith("]")) {
            let key = line.slice(1, -1).trim();
            if (key === "global") {
                section = null;
            } else {
                key = lib.split(key, /[=,]/);
        ...
        if (section) {
            const obj = lib.objGet(options?.context, section.name);
        ...
        line = line.split("=");
        line[0] = line[0].trim();
        if (lib.badProperties.has(line[0])) continue;

        if (options?.obj) {
            if (line[0]) argv[line[0]] = line.slice(1).join('=').trim();
        } else
        if (options?.list) {
            if (line[0]) argv.push([line[0], line.slice(1).join('=').trim()]);
        } else {
 ...
 * @param {string} [options.data_type] - make sure the result is returned as type: obj, list, str
 ...
 * @param {object} [options.parser] - custom options for xmlParser from "fast-xml-parser" packagge
 * @memberof module:lib
 * @method xmlParse
 */
...
function _parse(type, data, options)
{
    if (!data && !options?.empty) {
        return _checkResult(type, lib.newError("empty " + type), data, options);
    }
    try {
        return _parseResult(type, data, options);
    } catch (err) {
        return _checkResult(type, err, data, options);
    }
}
...
function _parseResult(type, obj, options)
{
    if (typeof obj === "string" && obj.length) {
        ...
        case "xml":
            const parser = new fxp.XMLParser(Object.assign({
                attributeNamePrefix: "",
                htmlEntities: true,
...
function _checkResult(type, err, obj, options)
{
    ...
    if (options.data_type === "object" || options.data_type === "obj") return {};
    if (options.data_type === "list") return [];
    if (options.data_type === "str") return "";
    return null;
}
