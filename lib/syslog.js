...
function Syslog(options)
{
    ...
    this.facility = this.facility || mod.LOG_LOCAL0;
...
Syslog.prototype.log = function(priority, msg)
{
    ...
    const now = new Date();
    let pri = this.facility * 8 + priority;
    if (isNaN(pri)) pri = mod.LOG_LOCAL0 * 8 + mod.LOG_NOTICE;

    if (this.rfc5424) {
        msg = `<${pri}>1 ${now.toISOString().replace("Z", "+00:00")} ${this.hostname} ${this.tag} ${process.pid} - - ${msg}\n`;
    } else
    if (this.rfc3164) {
        const d = `${months[now.getMonth()]} ${spacepad(now.getDate())} ${zeropad(now.getHours())}:${zeropad(now.getMinutes())}:${zeropad(now.getSeconds())}`
        msg = `<${pri}>${d} ${this.hostname} ${this.tag}[${process.pid}]: ${msg}\n`;
    } else {
        msg = `<${pri}>${now.toISOString().replace("Z", "+00:00")} ${this.tag}[${process.pid}]: ${msg}\n`;
    }
    this.write(msg);
}
