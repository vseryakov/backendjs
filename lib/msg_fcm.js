...
client.send = function(dev, options, callback)
{
    ...
    if (options.badge) msg.notification.badge = lib.toBool(options.badge);
    if (options.sound) msg.notification.sound = typeof options.sound == "string" ? options.sound : "default";
    if (options.vibrate) msg.notification.vibrate = lib.toBool(options.vibrate);
    ...
    core.httpGet('https://fcm.googleapis.com/fcm/send', opts, function(err, params) {
        ...
        if (err) logger.error("send:", "fcm", agent._app, dev, err, msg);
        agent._queue--;
        agent._sent++;
