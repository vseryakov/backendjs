...
Pool.prototype.open = function(callback)
{
    try {
        ...
        var port = h[1] || this.connect.port || 6379;
        var host = h[0];
        var client = redis.createClient(port, host, this.connect);
...
Pool.prototype.getKey = function(table, obj, options, search)
{
    ...
    for (var p in keys) key += options.sep + keys[p];
    if (search) key += key == table ? options.sep + "*" : "*";
    return key;
}
...
Pool.prototype.getList = function(client, table, obj, options, callback)
{
    ...
    if (typeof obj[0] == "string") {
        obj = obj.map(function(x) {
            return x.split(options.sep).slice(1).map(function(x,i) { return [x, keys[i]] }).reduce(function(x,y) { x[y[1]] = y[0]; return x }, {});
        });
    }
    ...
    var rows = [];
    lib.forEachLimit(obj, options.concurrency || this.settings.concurrency, function(item, next) {
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    switch (req.op) {
    case "drop":
        client.keys(table + options.sep + "*", function(err, list) {
            if (err || !list.length) return callback(err, []);
            lib.forEachLimit(list, options.concurrency || self.settings.concurrency, function(key, next) {
                client.del(key, next);
            }, function(err) {
                callback(err, []);
            });
    ...
    case "select":
        ...
        var filter = function(items) {
            if (other.length > 0) items = db.filterRows(obj, items, { keys: other, cols: cols, ops: options.ops, typesMap: options.typesMap });
            return items;
        }
        var rows = [];
