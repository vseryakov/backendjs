    ...
    pool.query = function(client, req, opts, callback) {
        ...
        case "select":
            ...
            var filter = function(items) {
                if (other.length > 0) items = self.filterRows(obj, items, { keys: other, cols: cols, ops: opts.ops, typesMap: opts.typesMap });
                return items;
            }
            var rows = [];
        ...
        case "incr":
            var key = this.getKey(table, obj, opts);
            var nums = lib.searchObj(opts.updateOps, { value: "incr", names: 1 }).map(function(x) { return { name: x, value: obj[x] } });
