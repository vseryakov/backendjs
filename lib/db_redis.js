...
function Pool(options)
{
    ...
    this.poolOptions = lib.mergeObj(this.poolOptions, this.poolOptions);
}
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "select":
        ...
        var filter = function(items) {
            if (other.length > 0) items = db.filterRows(obj, items, { keys: other, cols: cols, ops: options.ops, typesMap: options.typesMap || this.poolOptions.typesMap });
            return items;
        }
        var rows = [];
