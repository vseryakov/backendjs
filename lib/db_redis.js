...
Pool.prototype.getKey = function(table, obj, options, search)
{
    ...
    for (var p in keys) key += this.configOptions.sep + keys[p];
    if (search) key += key == table ? this.configOptions.sep + "*" : "*";
    return key;
}
...
Pool.prototype.getList = function(client, table, obj, options, callback)
{
    ...
    if (typeof obj[0] == "string") {
        obj = obj.map(function(x) {
            return x.split(self.configOptions.sep).slice(1).map(function(x,i) { return [x, keys[i]] }).reduce(function(x,y) { x[y[1]] = y[0]; return x }, {});
        });
    }
    ...
    var rows = [];
    lib.forEachLimit(obj, options.concurrency || this.configOptions.concurrency, function(item, next) {
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    switch (req.op) {
    case "drop":
        client.keys(table + this.configOptions.sep + "*", function(err, list) {
            if (err || !list.length) return callback(err, []);
            lib.forEachLimit(list, options.concurrency || self.configOptions.concurrency, function(key, next) {
                client.del(key, next);
            }, function(err) {
                callback(err, []);
            });
    ...
    case "select":
        ...
        var filter = function(items) {
            if (other.length > 0) items = db.filterRows(obj, items, { keys: other, cols: cols, ops: options.ops, typesMap: options.typesMap || self.configOptions.typesMap });
            return items;
        }
        var rows = [];
    ...
    case "incr":
        var key = this.getKey(table, obj, options);
        var nums = lib.searchObj(options.updateOps, { hasvalue: "incr", names: 1 }).map(function(x) { return { name: x, value: obj[x] } });
