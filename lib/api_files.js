...
api.getFile = function(req, file, options)
{
    ...
    if (options.filesS3 || this.filesS3) {
        if (options.attachment) options.headers = { "Content-Disposition": "attachment; filename=" + path.basename(file) };
...
api.putFile = function(req, name, options, callback)
{
    ...
    if (btype == "object" && req.body[name]) {
        var data = Buffer.isBuffer(req.body[name]) ? req.body[name] : Buffer.from(req.body[name], options.encoding || "base64");
    ...
    if (req.query[name]) {
        var data = Buffer.isBuffer(req.query[name]) ? req.query[name] : Buffer.from(req.query[name], options.encoding || "base64");
        this.detectStoreFile(data, outfile, options, callback);
    } else {
