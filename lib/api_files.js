...
api.listFile = function(options, callback)
{
    var files = [];
    if (options.filesS3 || this.filesS3) {
        core.modules.aws.s3List(`${options.filesS3 || this.filesS3}/${options.path}/`, { retryCount: 3, retryTimeout: 500, retryOnError: 1 }, (err, rows) => {
            for (const i in rows) {
        ...
        lib.findFile(`${options.filesPath || core.path.files}/${options.path}`, { types: "f", depth: 0, include: options.filter }, (err, rows) => {
            for (const i in rows) {
