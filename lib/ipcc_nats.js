...
IpcNatsClient.prototype.subscribe = function(channel, options, callback)
{
    ...
    var opts = {
        ...
        callback: (err, msg) => {
            ...
            if (msg.reply) {
                this.emit(channel, mod.sc.decode(msg.data), (err, data) => {
                    if (!err) msg.respond(mod.sc.encode(data));
                });
            } else {
                this.emit(channel, mod.sc.decode(msg.data));
            }
        }
    };
...
IpcNatsClient.prototype.subscribeQueue = function(options, callback)
{
    ...
    for (const n of ["deliverAall", "deliverLastPerSubject", "deliverLast", "deliverNew",
    ...
    for (const n of ["queue", "durable", "deliverTo", "deliverGroup", "idleHeartbeat", "limit", "filterSubject",
        ...
        if (typeof options[n] != "undefined") opts[n](options[n]);
    }
    ...
    p.then((sub) => {
        ...
        logger.error("listen:", mod.name, queue, err);
    });
}
...
{
    if (!this.client) return this._pending.push(["publishQueue", channel, options, callback]);
    var queue = this.options.queue || this.queueName;
    if (!this.js) this.js = this.client.jetstream();
    var opts = options;
    if (options.unique) {
...
IpcNatsClient.prototype._callback = function(err, jsmsg)
{
    ...
    var now = this._mtime = Date.now(), running = this._running, ack = this.options.ackNext ? "next": "ack";

    var msg = lib.jsonParse(mod.sc.decode(jsmsg.data), { logger: "info" });
    logger.debug("nats.poller:", queue, running.size, "MSG:", msg, "ITEM:", jsmsg);

    if (!msg || (msg.endTime > 0 && msg.endTime < now)) {
        mtimer.end();
        return jsmsg[ack]();
    }
    if (msg.startTime > 0 && msg.startTime > now) {
        mtimer.end();
        return jsmsg.working();
    }
    if (msg.noWait) {
        jsmsg[ack]();
    } else
    ...
    try {
        ...
        try { jsmsg.working() } catch (e2) { e._errmsg = e2.message }
