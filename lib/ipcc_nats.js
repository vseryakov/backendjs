...
IpcNatsClient.prototype.subscribeQueue = function(options, callback)
{
    ...
    if (this._subs[chan]) return logger.warn("subscribeQueue:", mod.name, this.queueName, "already subscribed to:", chan, options);
...
IpcNatsClient.prototype.unsubscribeQueue = function(options, callback)
{
    ...
    this._unsubscribe(this.channel(options));
}
...
{
    var chan = this.channel(options);
    if (err && err.code != 404) return logger.error("pollQueue:", this.name, chan, this._running.size, err, jsmsg);
    if (!jsmsg) return;
