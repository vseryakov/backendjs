...
module.exports = {
    ...
    args: [
        ...
        { name: "user-([a-z,*]+)-(/.+)", type: "map", no_camel: 1, ephemeral: 1, onupdate, descr: "Endpoints/methods to limit by path and authenticated user", example: "middleware-limiter-user-get,post,put-/admin/* = rate:10,interval:5000" },
...
mod.handle = function(context, next)
{
    ...
    const rates = [];
    if (config.ip?.rate) {
        rates.push([[config.method, context.ip], config.ip]);
    }
    if (config.path?.rate) {
        rates.push([[config.method, context.path], config.path]);
    }
    if (config.user?.rate && context.userId) {
        rates.push([[config.method, context.userId, context.path], config.user]);
    }
