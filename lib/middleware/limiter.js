...
const mod = {
    ...
    args: [
        ...
        { name: "(rate|max|interval|ttl|delay|multiplier|queue)", autotype: 1, descr: "Default rate limiter parameters for Token Bucket algorithm. `queue` to use specific queue, ttl` is to expire cache entries", example: "middleware-limiter-queue = redis\nmiddleware-limiter-rate = 1\nmiddleware-limiter-ttl = 60000" },
        { name: "map-(.+)", type: "map", obj: "map.$1", merge: 1, descr: "Rate limiter parameters for Token Bucket algorithm", example: "middleware-limiter-map-/url=rate:1,interval:2000\nmiddleware-limiter-map-GET/url=rate:10" },
    ],
 ...
 *
 *         api-limiter-rate = 100
 *         api-limiter-queue = redis
 *         api-limiter-map-GET/api/path = rate:10,queue:local
 *         api-limiter-map-/api/path/* = rate:1
 *         api-limiter-map-/api/path/127.0.0.1 = rate:100
 *         api-limiter-map-/api/* = rate:100,interval:1000
 *
 ...
 * @example
 *
 *  middleware.limiter.limiter({ path: context.path, ip: context.ip, type: "ip", rate: 100, interval: 60000 }, (err, info) => {
 *     if (err) return context.send(err.status, err);
 *     ...
 *  });
 * @example <caption>More endpoint config examples</caption>
 * api-limiter-map-/pub/settings = rate:10,interval:1000,delay:250
 * api-limiter-map-GET/passkey/login = rate:3,interval:1000,delay:250
 * api-limiter-map-/login = rate:3,interval:30000,delay:1000,multiplier:1.5,queue:unique
 * api-limiter-map-/checkin* = rate:5,interval:30000
 * @memberof module:middleware/limiter
 * @method limiter
 */
