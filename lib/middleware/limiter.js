 ...
 * ## Users and sessions
 *
 * The difference between user and session "value":
 *  - session - id is just parsed out from cookies or headers, no verification is done.
 *  - user - requires valid user to be verified and set in the context.
 *
...
mod.handle = function(context, next)
{
    ...
    for (const [config, route] of configs) {
        ...
        for (const p in config) {
            ...
            case "user":
                rates.push([[method, path, context[p]?.id ||context[p]?.login], config[p]]);
                break;
            }
        }
    }
