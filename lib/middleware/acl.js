...
mod.reset = function()
{
    ...
    this.public = ["public"];
    this.allow = this.deny = this.authenticated = this.anonymous = null;
}
mod.reset();
...
{
    if (!Array.isArray(acls) || !path) return;
 ...
 * @method isAllowed
 */
mod.isAllowed = function(context)
{
    if (this.allow) {
        const roles = context.user?.roles;
        for (const i in roles) {
            var p = roles[i];
            if (this.isMatched(context.path, this.allow[p])) {
                logger.debug("isAllowed:", this.name, 403, context, p, this.allow[p]);
                return true;
            }
        }
    }
    logger.debug("isAllowed:", this.name, 403, context, "nomatch", context.user?.roles);
}
 ...
 * @method isDenied
 */
mod.isDenied = function(context)
{
    if (this.deny) {
        const roles = context.user?.roles;
        for (const i in roles) {
            var p = roles[i];
            if (this.isMatched(context.path, this.deny[p])) {
                logger.debug("isDenied:", this.name, 403, context, p, this.deny[p]);
                return true;
            }
        }
    }
 ...
 * @method isPublic
 */
mod.isPublic = function(context)
{
    if (this.isMatched(context.path, this.public)) return true;

    logger.debug("isPublic:", this.name, 403, context, this.public);
}
 ...
 */
mod.isAuthenticated = function(context)
{
    if (context.user?.id && this.isMatched(context.path, this.authenticated)) return true;

    logger.debug("isAuthenticated:", this.name, 403, context, this.authenticated);
}
 ...
 * @method isAnonymous
 */
mod.isAnonymous = function(context)
{
    if (this.isMatched(context.path, this.anonymous)) return true;

    logger.debug("isAnonymous:", this.name, 403, context, this.anonymous);
}
