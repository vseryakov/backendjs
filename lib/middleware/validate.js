 ...
 *
 *
 * # Rate limit all /admin/XXX requests by session id and XXX part to allow only 10 per 30s
 * middleware-validate-params-*-0-/admin/* = rate:10,rate_interval:30000,rate_session:true
 *
 * # Rate limit all /admin/XXX requests by user and XXX part to allow only 10 per 30s
...
mod.handle = function(context, next)
{
    ...
    for (const [config, route, params] of configs) {
        ...
        for (const name in config) {
            ...
            logger.dev("handle:", mod.name, context.path, "schema:", schema, name + ":", body, "data:", data, "err:", err);
