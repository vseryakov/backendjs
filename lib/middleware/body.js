 ...
 *
 * ```
 * Additional content types can be collected by binding the parser with custom object containing `contentType` property.
 *
 * ```js
 * api.app.post("/data", middleware.body.handle.bind({ contentType: ["mime/type1", "mime/type2"], maxSize: 100000 }))
 * ```
 *
 */
module.exports = mod;
...
mod.handle = function(context, next)
{
    ...
    default:
        if (!lib.isFlag(this.contentType, contentType)) {
            return next();
        }
