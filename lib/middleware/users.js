 ...
 * - All authenticated users can access /app
 *
 * ```
 ...
 *
 * - Only users with `api` role can use token based access to the `/api/*` endpoints.
 *
 * ```
...
module.exports = {
     ...
     */
    args: [
        { name: "enable", type: "list", descr: "Enable user session middlware globally for the given list of endpoints" },
        { name: "enable-token", type: "list", descr: "Enable user API token middlware globally for the given list of endpoints" },
...
mod.configureMiddleware = function(_options, callback)
{
    ...
    if (mod.enable?.length) {
        if (mod.loginPath) {
            api.app.use("POST" + method, mod.loginPath, mod.login);
        }
        if (mod.profilePath) {
            api.app.use("GET" + method, mod.profilePath, mod.profile);
        }
        if (mod.logoutPath) {
            api.app.use("POST" + method, mod.logoutPath, mod.logout);
        }
 ...
 */
mod.login = function(context, _next)
{
 ...
 *
 */
mod.profile = function(context, _next)
{
    api.users.verifySession(context, (err) => {
 ...
 */
mod.logout = function(context, _next)
{
    api.users.verifySession(context, (err) => {
        if (err) return context.reply(err);
