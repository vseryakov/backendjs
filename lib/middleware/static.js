 ...
 *     API routes, static files with configuration only,
...
const mod = {
    ...
    args: [
        ...
        { name: "options", type: "map", obj: "options", merge: 1, descr: "Options to pass to serve-static module: maxAge, noCache, lastModified" },
...
mod.checkRouting = function(req)
{
    ...
    for (const p in mod.compressed) {
        if (lib.testRegexp(req.context.path, mod.compressed[p])) {
            ...
            req.res.setHeader("Content-Type", lib.getMimeType(req.context.opath));
