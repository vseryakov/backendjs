 ...
 * middleware-static-priority = 9999
 *
...
mod.configureMiddleware = function(_options, callback)
{
    if (this.enable) {
        ...
        const method = `GET#${this.priority ?? ""}`;

        for (const root of app.path.web) {
            api.app.use(method, "*", Object.assign({}, opts, { root, handle: mod.handle }));
        }
        api.app.use(method, "*", Object.assign({}, opts, { root: path.resolve(__dirname + "/../../web"), handle: mod.handle }));
    }
