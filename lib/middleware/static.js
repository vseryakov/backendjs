...
const mod = {
    ...
    args: [
        ...
        { name: "views-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: 'views', descr: "Paths to be rendered as views, use ! in front of regexp to remove particular redirect from the list, variables can be used for substitution: @HOST@, @PATH@, @URL@, @BASE@, @DIR@, @QUERY@,", example: "api-static-views-^/user/get = show-user" },
...
mod.configureStaticWeb = function(options, callback)
{
    ...
    for (let i = 0; i < app.path.web.length; i++) {
        api.app.use(api.Static(app.path.web[i], mod.options));
    }
    api.app.use(api.Static(__dirname + "/../../web", mod.options));
...
mod.checkViews = function(req, res)
{
    ...
    for (const p in mod.views) {
        if (lib.testRegexpObj(req.context.path, mod.views[p])) {
            logger.debug("checkViews:", mod.name, req.context.location, "render by:", p);
            res.render(api.checkPlaceholders(req.context, p));
            return true;
        }
    }
}
...
{
    if (mod.vhost && !lib.testRegexpObj(req.context.path, mod.noVhost)) {
        for (const p in mod.vhost) {
            if (lib.testRegexp(req.context.host, mod.vhost[p])) {
                req.context.setUrl("/" + p + req.context.path);
                logger.debug("vhost:", mod.name, req.context.host, "rerouting to", req.url);
                break;
            }
        }
    }
    ...
    for (const p in mod.compressed) {
        if (lib.testRegexp(req.context.path, mod.compressed[p])) {
            req.context.setUrl(req.context.path + "." + p);
            req.res.setHeader("Content-Encoding", p == "br" ? "brotli" : "gzip");
            req.res.setHeader("Content-Type", app.getMimeType(req.context.opath));
            logger.debug("compressed:", mod.name, req.context.opath, "rerouting to", req.url);
            break;
        }
    }
}
