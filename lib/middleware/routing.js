...
const mod = {
    ...
    args: [
        ...
        { name: "/.+", nocamel: 1, ephemeral: 1, descr: "Paths to be re-routed/redirected", onupdate, example: "middleware-routing-/user/get = /user/details\nmiddleware-routing-/old/path = 302 /new/path?@SEARCH@" },
    ],
    router: new Router(),
};
 ...
 *
 * @example
 * // Enable dynamic routing, every request will be chercked against current config
...
mod.configureMiddleware = function(options, callback)
{
    if (mod.enable == "fixed") {
        mod.router.walk(node => {
            api.app.use("#0", node.methods[0].path, { route: node.methods[0].handler, handle: mod.handle });
        });
    } else
 ...
 *
 * api.app.post("/acct/*", { route: "/account/@PATH2@", handle: routing.handle })
 *
 */
mod.handle = function(context, next)
{
    ...
    if (!route) {
        ...
        route = routes[0].route.handler;
    }

    logger.debug("handle:", mod.name, context, "switch to:", route);

    if (route[0] == "3") {
        const loc = route.split(" ");
        context.redirect(loc[0], loc[1]);
    } else {
        context.setUrl(context.format(route));
        next("restart");
    }
}
