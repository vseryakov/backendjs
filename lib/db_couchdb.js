...
db.couchdbInitPool = function(options)
{
    ...
    pool.query = function(client, req, opts, callback) {
        ...
        case "upgrade":
            var views = {}, changed = 0;
            var cols = Object.keys(corelib.searchObj(req.obj, { name: 'primary', sort: 1, flag: 1 }));
            if (cols.length) views.primary = cols;

            ["", "1", "2", "3", "4", "5"].forEach(function(n) {
                var cols = Object.keys(corelib.searchObj(req.obj, { name: "unique" + n, sort: 1, flag: 1 }));
                if (cols.length) views[cols.join("_")] = cols;
                var cols = Object.keys(corelib.searchObj(req.obj, { name: "index" + n, sort: 1, flag: 1 }));
                if (cols.length) views[cols.join("_")] = cols;
            });
        ...
        case "list":
            ...
            var rows = [];
            corelib.forEachLimit(ids, opts.concurrency || core.concurrency, function(key, next) {
                key = key.replace(/[\/]/g, "%2F");
                query("get", "GET", key, "", opts, function(err, res) {
                ...
                query("get", "GET", key, "", opts, function(err, res) {
                    ...
                    for (var p in res) {
                        if (opts.counter && opts.counter.indexOf(p) > -1) {
                            req.obj[p] = corelib.toNumber(res[p]) + corelib.toNumber(req.obj[p]);
                        } else
                        if (!req.obj[p]) {
                            req.obj[p] = res[p];
                        }
