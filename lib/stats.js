...
const mod = {
    ...
    columns: {
        ...
        jobs_que_size: { type: "number" },                  // Jobs running
        jobs_err_count: { type: "number" },                 // Jobs errors
        jobs_task_count: { type: "number" },                // Jobs tasks executed
        jobs_run_time: { type: "number" },                  // Jobs running time
        jobs_run_time_p50: { type: "number" },              // Jobs running time 50% percentile
        jobs_run_time_p95: { type: "number" },              // Jobs running time 95% percentile
        db_dynamodb_req_count: { type: "number" },          // DB pool queries executed
        db_dynamodb_req_rate_m5: { type: "number" },        // DB pool query rate for the last 5 mins
        db_dynamodb_req_rate_m15: { type: "number" },       // DB pool query rate for the last 15 mins
        db_dynamodb_res_time: { type: "number" },           // DB pool query response time
        db_dynamodb_res_time_p50: { type: "number" },       // DB pool query response time 50% percentile
        db_dynamodb_res_time_p95: { type: "number" },       // DB pool query response time 95% percentile
        db_dynamodb_que_size: { type: "number" },           // DB pool query queue size
        db_dynamodb_que_size_p50: { type: "number" },       // DB pool query queue size time 50% percentile
        db_dynamodb_que_size_p95: { type: "number" },       // DB pool query queue size time 50% percentile
    },
    roles: [],
};
module.exports = mod;
...
mod.collect = function(options, callback)
{
    ...
    // IPC clients
    for (let q in ipc.clients) {
        const m = ipc.clients[q].metrics.toJSON();
        if (!m.meter?.count) continue;
        q = ipc.clients[q].queueName;
        stats["ipc_" + q + "_req_count"] = m.meter.count;
        stats["ipc_" + q + "_req_rate"] = m.meter.rate;
        stats["ipc_" + q + "_req_rate_m5"] = m.meter.m5;
        stats["ipc_" + q + "_req_rate_m15"] = m.meter.m15;
        stats["ipc_" + q + "_res_time"] = m.histogram.mean;
        stats["ipc_" + q + "_res_time_p50"] = m.histogram.med;
        stats["ipc_" + q + "_res_time_p95"] = m.histogram.p95;
    }
...
mod.queryElasticsearch = function(options, callback)
{
    ...
    var groups = lib.strSplit(options.groups || "tag");
    var nterms = options.nterms || 50;
    var age = lib.toNumber(options.age || 300) * 1000;
    var interval = lib.toNumber(options.interval || 180) * 1000;
    var fields = lib.strSplit(options.fields || "host_cpu_util");

    var q = {
        size: 0,
        query: {
            query_string: {
                 query: `timestamp:>${Date.now() - age}`
            }
        },
        aggs: {
            stats: {
        ...
        if (!err) {
            ...
            if (!options.raw) {
                for (const t of info.aggregations.stats.buckets) {
                    groups[t.key] = {};
                    for (const f of fields) {
                        groups[t.key][f] = t.fields.buckets.sort((a, b) => (b.key - a.key)).map((x) => (x[f] && Math.round(x[f].value) || 0)).join(", ");

                        if (options.hdr && !header) {
                            var now = Date.now();
                            header = "# tag, fields: " + t.fields.buckets.map((x) => (Math.floor((now - x.key)/1000/60)+"m")).join(", ");
                        }
                    }
                }
            }
        }
        callback(err, { groups, header, info, rows });
    });
