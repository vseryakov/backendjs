        ...
        if (!err) {
            ...
            if (!options.raw) {
                for (const t of info.aggregations.stats.buckets) {
                    data[t.key] = {};
                    for (const f of fields) {
                        const col = columns[f], blen = t.fields.buckets.length - 1;
                        let sum = 0, ndocs = 0;
                        ...
                        if (sum || options.zeros) data[t.key][f] = d;
