...
const mod = {
    ...
    args: [
        ...
        { name: "target", type: "json", descr: "Target options, type: must be provided, other options type specific, file, url, table, log..." },
        { name: "roles", type: "list", descr: "Process roles that report stats only" },
    ],
    columns: {
        id: { keyword: 1, primary: 1 },
...
mod.configure = function(options, callback)
{
    ...
    if (this.target?.type == "db" && this.target.table) {
        this.tables = {
            [this.target.table]: this.columns
        }
    }
...
mod.run = function()
{
    ...
    this.collect((err, stats) => {
        this.send({ target: this.target, stats }, (err) => {
            this._running = 0;
            this._stats = stats;
            this._error = err;
...
mod.send = function(options, callback)
{
    if (typeof callback != "function") callback = lib.noop;
    if (!options.target?.type) return callback();
    ...
    switch (target.type) {
    case "log":
        logger.logger(target.log, "send:", mod.name, stats);
        callback();
        break;

    case "file":
        fs.appendFile(target.file, lib.stringify(stats) + "\n", callback);
        break;

    case "db":
        db.add(target.table, stats, { pool: target.pool, no_columns: 1 }, callback);
        break;

    case "http":
        core.httpGet(target.url, { method: "POST", retryCount: 3, retryOnError: 1, postdata: stats, headers: target.headers }, callback);
        break;
