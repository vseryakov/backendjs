                ...
                for (const t of info.aggregations.stats.buckets) {
                    ...
                    for (const f of fields) {
                        ...
                        const v = t.fields.buckets.map((x) => {
                            x = x[f] && x[f].value || 0;
                            if (col) {
                                if (col?.mult) x *= col.mult; else
                                if (col?.div) x /= col.div; else
                                if (col?.incr) x += col.incr;
                                if (col?.abs) x = Math.abs(x);
                                if (typeof col?.min == "number" && x < col.min) x = col.min; else
                                if (typeof col?.max == "number" && x > col.max) x = col.max;
                            }
                            sum += x;
                            return x;
                        });
                        if (sum || options.zeros) data[t.key][f] = v;
