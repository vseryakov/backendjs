...
module.exports = {
    ...
    args: [
        ...
        { name: "interval", type: "int", descr: "Interval for process stats collection in ms" },
        ...
        { name: "filter", obj: "filter", type: "map", map_type: "regexp", merge: 1, not_empty: 1, descr: "For each metric prefix provide regexp to keep only matched stats", example: "-stats-filter db:dynamodb" },
    ],
    columns: {
...
stats.configureModule = function(_options, callback)
{
...
stats.shutdown = function(_options, callback)
{
...
stats.collect = function(options, callback)
{
    ...
    // GC stats if enabled
    var gc = lib.gcStats(lib.includes(this.flags, "gc"));
    ...
    stats.role = app.role;
    stats.host = app.host;
    stats.hostname = app.hostName;
    stats.arch = app.arch;
    stats.ip = app.ipaddr;
    stats.id = app.env.id;
    stats.tag = app.env.tag;
    stats.zone = app.env.zone;

    lib.parallel([
        function(next) {
            if (!lib.includes(stats.flags, "net")) return next();
            lib.networkStats(app.env.netdev, (_err, net) => {
                Object.assign(stats, net);
                next(null, stats);
            });
        },
        function(next) {
            app.runMethods("configureCollectStats", options, { sync: 1 }, () => {
                logger.debug("collect:", stats.name, options);
                next(null, stats);
            });
        }
    ], callback);
...
stats.send = function(target, stats, callback)
{
    ...
    if (target.url) {
        const url = lib.toTemplate(target.url, stats);
        const opts = Object.assign({ method: "POST", retryCount: 2, retryOnError: 1 }, target.options);
        if (opts.posdata) {
            opts.postdata = lib.toTemplate(opts.postdata, { stats, target });
        } else {
            opts.postdata = stats;
        }
        return lib.fetch(url, opts, callback);
    }
...
stats.queryElasticsearch = function(options, callback)
{
    var tsize = options.tsize || 50;
    var ssize = options.ssize || undefined;
    var tags = lib.split(options.tags);
    var groups = lib.split(options.groups);
    ...
    var fields = lib.split(options.fields);
    ...
    logger.debug("queryElasticsearch:", stats.name, options, "Q:", q);
