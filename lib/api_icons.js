...
api.downloadIcon = function(uri, options, callback)
{
    ...
    if (options.verify) {
        ...
        fs.stat(imgfile, function(err, stats) {
            ...
            if (err) return self.downloadIcon(uri, id, corelib.delObj(options, 'verify'), callback);
            ...
            core.httpGet(uri, { method: 'HEAD' }, function(err2, params) {
                ...
                if (params.size !== stats.size) return self.downloadIcon(uri, id, corelib.delObj(options, 'verify'), callback);
    ...
    var tmpfile = path.join(core.path.tmp, corelib.random().replace(/[\/=]/g,'') + path.extname(opts.pathname));
...
api.putIcon = function(req, id, options, callback)
{
    ...
    if (nfiles) {
        ...
        corelib.forEachSeries(Object.keys(req.files), function(f, next) {
            var opts = corelib.extendObj(options, 'type', req.body[f + '_type'] || (type && nfiles == 1 ? type : ""));
