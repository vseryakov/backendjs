...
api.putIcon = function(req, id, options, callback)
{
    ...
    if (nfiles) {
        ...
        lib.forEachSeries(Object.keys(req.files), function(f, next) {
            var opts = lib.objExtend(options,
                                     'type', req.body[f + '_type'] || (type && nfiles == 1 ? type : ""),
                                     "filesize", req.files[f].size,
                                     "filename", path.basename(req.files[f].name));
            self.saveIcon(req.files[f].path, id, opts, function(err, ofile) {
                outfile = ofile;
                next(err);
            });
        }, function(err) {
            lib.tryCall(callback, err, outfile);
        });
    } else
