...
client.init = function(options)
{
    ...
    for (var i in config) {
        ...
        if (config[i].feeback) {
            ...
            agent.feedback.on("error", logger.error.bind(logger, "apn:"));
            agent.feedback.on("feedbackError", logger.error.bind(logger, "apn:"));
            agent.feedback.start();
        }
        this.agents[config[i]._app] = agent;
        logger.info("init:", "apn", lib.objDescr(config[i]), lib.objDescr(agent.options));
    }
}
...
client.close = function(callback)
{
    lib.forEach(Object.keys(this.agents), function(key, next) {
        ...
        logger.info('closeAPN:', key, lib.objDescr(agent.options), 'sent:', agent._sent);
        agent.shutdown();
...
{
    logger[err >= 512 ? "error" : "debug"]('apn:', err, message, dev);
