...
{
    return dev.service == "apn" || (!dev.service && dev.urn.match(/^[0-9a-f]+$/i));
}
...
client.init = function(options)
{
    ...
    for (var p in this.config) {
        ...
        if (!d || !msg.config[p] || typeof msg.config[p] != "string") continue;
        var file = msg.config[p], app = d[1] || "default";
        if (this.agents[app]) continue;
        ...
        try {
            ...
            logger.error("init:", "apn", app, file, e.stack);
            continue;
        }
        ...
        this.agents[app] = agent;
        logger.info("init:", "apn", app, agent.settings);
    }
}
