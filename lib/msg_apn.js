...
client.init = function(options)
{
    ...
    for (var i in config) {
        ...
        opts.pfx = config[i]._pfx ? config[i]._pfx.match(/\.p12$/) ? config[i]._pfx : Buffer.from(config[i]._pfx, "base64") : "";
        ...
        try {
            ...
            logger.info("init:", client.name, config[i], agent.options);
        } catch(e) {
            logger.error("init:", client.name, config[i], agent.options, e);
        }
    }
}
