...
mod.scheduleCronjob = function(jobspec)
{
    ...
    if (!this.croner || lib.toBool(jobspec.disabled)) {
        logger.debug("scheduleCronjob:", "skip", jobspec, "croner:", !!this.croner);
        return false;
    }
    logger.debug('scheduleCronjob:', jobspec);
    try {
        const cj = new this.croner.Cron(jobspec.cron, jobspec.croner || {}, (job) => {
            mod.submit(job.jobspec, { queueName: job.jobspec.queueName || mod.cronQueue }, (err) => {
...
mod.scheduleCronjobs = function(type, list)
{
    ...
    if (!this.croner) this.croner = lib.tryRequire(__dirname + '/../../dist/croner.cjs');
...
mod.parseCronjobs = function(type, data)
{
    ...
    var n = this.scheduleCronjobs(type, lib.jsonParse(data, { data_type: "list", logger: "error" }));
