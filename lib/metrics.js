...
//   - rcount: The total of all values added to the meter.
...
Timer.prototype.toJSON = function()
{
    ...
    for (const key in json) result[key] = json[key];
    return result;
}
...
function Metrics()
{
    for (let i = 0; i < arguments.length - 1; i+= 2) this[arguments[i]] = arguments[i + 1];
}

Metrics.prototype.toString = function(name)
{
    return this.toJSON(name);
}

Metrics.prototype.toJSON = function(name)
{
    if (name) return this[name] && this[name].toJSON();
    var json = {};
    for (const p in this) {
        if (typeof this[p] == "undefined" || typeof this[p] == "function" || this[p] === null) continue;
        json[p] = typeof this[p].toJSON == "function" ? this[p].toJSON() : this[p];
    }
    return json;
}
...
{
    if (!this[name]) this[name] = new Meter(properties);
    return this[name];
}
