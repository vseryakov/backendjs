...
db.createTables = function(options, callback)
{
    ...
    var pools = db.none ? [] : lib.isArray(options?.pools, Object.keys(db.pools)).filter(x => !lib.isFlag(db.skip?.pools, x));
    ...
    lib.forEveryLimit(pools, options?.concurrency || db.concurrency, (name, next) => {
        ...
        lib.series([
            ...
            function(next2) {
                ...
                tables = Object.keys(db.tables).filter((x) => (!lib.isFlag(db.skip?.tables, x) && !lib.isFlag(pool.config.skipTables, x)));

                // Skip tables not supposed to be in this pool
                if (lib.isArray(pool.config.tables)) {
                    tables = tables.filter((x) => (lib.isFlag(pool.config.tables, x)));
                }
