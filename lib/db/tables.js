...
db.createTables = function(options, callback)
{
    ...
    var pools = db.none ? [] : lib.isArray(options?.pools, Object.keys(db.pools)).filter(x => !lib.isFlag(db.skipPools, x));
    ...
    lib.forEveryLimit(pools, options?.concurrency || db.concurrency, (name, next) => {
        ...
        lib.series([
            ...
            function(next2) {
                ...
                tables = Object.keys(db.tables).filter((x) => (!lib.isFlag(db.skipTables, x) && !lib.isFlag(pool.config.skipTables, x)));
