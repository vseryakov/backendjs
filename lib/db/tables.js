...
/**
 * Return a normalized table name, take first part before spaces/punctuations
 * @param {string} table
 * @return {string}
 * @memberof module:db
 * @method table
...
{
    return typeof table === "string" ? table.match(db.rxTable)?.[1]?.toLowerCase() : "";
}
 ...
 * @param {number} [options.concurrency] - how many pools to run in parallel
 * @param {function(err, {created:object, upgraded:object})} [callback]
 * @memberof module:db
 * @method createTables
 */
db.createTables = function(options, callback)
{
    ...
    const pools = db.none ? [] : lib.isArray(options?.pools, Object.keys(db.pools)).filter(x => !lib.includes(db.skip?.pools, x));

    const created = [], upgraded = [];

    lib.forEveryLimit(pools, options?.concurrency || db.concurrency, (name, next) => {
        const pool = db.getPool(name);
        const opts = { pool: pool.name };

        logger.debug("createTables:", "db", app.role, pool.name, pool.config);

        lib.series([
            function(next2) {
                db.cacheColumns(opts, next2);
            },

            function(next2) {
                // All allowed tables for the pool
                let tables = Object.keys(db.tables).filter((x) => (!lib.includes(db.skip?.tables, x) && !lib.includes(pool.config.skipTables, x)));
                ...
                lib.forEachSeries(tables, (table, next3) => {
                    ...
                    if (!exists) {
                        db.create(table, cols, opts, (err, _rows) => {
                            if (!err) {
                                created.push(table);
                                changes++;
                            }
                        ...
                        db.upgrade(table, cols, opts, (err, _rows) => {
                            if (!err) {
                                upgraded.push(table);
                                changes++;
                            }
                    ...
                    logger.logger(changes ? "info": "debug", "createTables:", "db", app.role, pool.name, 'created:', created, "upgraded:", upgraded);
                    next2();
                }, true);
            },
        ], next, true);
    }, (err) => {
        lib.tryCall(callback, err, { created, upgraded });
    });
}

/**
 * Async version of {@link module:db.createTables}
 * @param {objects|string} options
 * @param {string[]} [options.pools] - only create in these pools
 * @return {{err:Error, created:object, upgraded:object}}
 * @memberof module:db
 * @method acreateTables
 * @async
 */
db.acreateTables = function(options)
{
    return new Promise((resolve, _reject) => {
        db.createTables(options, (err, data) => {
            resolve({ err, ...data });
        });
    });
}
...
db.describeTables = function(tables, callback)
{
    if (typeof tables === "string") {
        tables = lib.jsonParse(tables, { data_type: "obj", logger: "error" });
    }
 ...
 * stores tables in `pool.dbcolumns`, primary keys in `pool.dbkeys`, indexes in `pool.dbindexes`.
...
db.cacheColumns = function(options, callback)
{
    ...
    pool.use((err, client) => {
        ...
        pool.cacheColumns(client, options, (err) => {
            if (err) logger.error('cacheColumns:', "db", pool.name, lib.traceError(err));

            pool.cacheIndexes(client, options, (err) => {
                if (err) logger.error('cacheIndexes:', "db", pool.name, err);
...
/**
 * Async version of {@link module:db.cacheColumns}
 * @param {string|object} options - a pool name or an object
 * @param {string} [options.pool] - pool name
 * @return {{err:object}}
 * @memberof module:db
 * @method acacheColumns
 * @async
 */
db.acacheColumns = function(options)
{
    return new Promise((resolve, _reject) => {
        db.cacheColumns(options, (err) => {
            resolve({ err });
        });
    });
}
