...
Pool.prototype._getTable = function(name)
{
    if (typeof this.configOptions.tableMap == "string") {
        this.configOptions.tableMap = lib.toValue(this.configOptions.tableMap, "map");
    }
...
Pool.prototype.getGeoQuery = function(table, obj, options)
{
...
Pool.prototype._buildCondition = function(req, obj, expected, join)
{
    ...
    for (const p in expected) {
        ...
        if (val === null) op = "null";
        var not = op.startsWith("not_") ? "!" : "";

        switch (op) {
        case "null":
            expr.push(`!ctx._source.containsKey('${n}')`);
            break;
        ...
        case "not_in":
            ...
            expr.push(`${not}(ctx._source.containsKey('${n}') && params._${p}.contains(ctx._source.${n}))`);
        ...
        case "not_between":
            ...
            expr.push(`${not}(ctx._source.containsKey('${n}') && ctx._source.${n} >= _${p} && ctx._source.${n} <= __${p})`);
        ...
        case "begins_with":
        case "not_begins_with":
            expr.push(`${not}(ctx._source.containsKey('${n}') && ctx._source.${n}.startsWith(params._${p}))`);
            obj.script.params["_" + p] = val;
            break;

        case "contains":
        case "not_contains":
            expr.push(`${not}(ctx._source.containsKey('${n}') && ctx._source.${n}.contains(params._${p}))`);
            obj.script.params["_" + p] = val;
            break;
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    var cols = db.getColumns(req.table, req.options);
    var path, query, obj, dynamic_templates = [];
    var config = this.configOptions;

    switch (req.op) {
    case "create":
    case "upgrade":
        var ecols = this.dbcolumns[table];
        ...
        for (var p in req.obj) {
            ...
            if (o.elasticsearch) {
                for (const f in o.elasticsearch) {
                    if (f == "dynamic_templates") {
                        dynamic_templates.push(...o.elasticsearch[f]);
                    } else {
                        if (!properties[p]) properties[p] = {};
                        properties[p][f] = o.elasticsearch[f];
                    }
                }
            } else
    ...
    case "drop":
        ...
        this.doQuery("DELETE", table, "", "", config, callback);
        break;
    ...
    case "search":
        ...
        path = table + "/" + (req.options.op || "_search");
    ...
    case "get":
        path = table + "/" + config.docType + "/" + lib.escape(this._getKey(keys, req.obj));
    ...
    case "update":
        path = table + "/_update/" + lib.escape(this._getKey(keys, req.obj));
        query = this._getQuery("index", req.options);
    ...
    case "updateall":
        path = table + "/_update_by_query";
        query = this._getQuery("updateall", req.options);
        if (lib.isObject(req.obj)) {
