...
class ElasticsearchPool extends DbPool {
    ...
    _getNodes(callback)
    {
        ...
        this.doQuery("GET", "/_nodes", "", "", { quiet: 1, logger_db: "none" }, (err, data, params) => {
            if (err) logger.error("getNodes:", this.name, err, this._nodes, params?.data);
            ...
            var interval = !err && this._nodes.length ? this.config.discoveryInterval || 300000 : this._nodesInterval;
    ...
    cacheColumns(_client, _options, callback)
    {
        ...
        this.doQuery("GET", "/_mappings", "", "", this.config, (err, data, _params) => {
    ...
    query(client, req, callback)
    {
        ...
        case "drop":
            ...
            this.doQuery("DELETE", this._getTable(req.table), "", "", this.config, callback);
            break;
    ...
    queryCreate(_client, req, callback)
    {
        ...
        if (missing) {
            return this.doQuery("PUT", table + "/_mappings", { properties }, "", config, callback);
        }
        callback();
    }
    ...
    queryList(_client, req, callback)
    {
        ...
        for (const i in req.query) ids.push(this._getKey(Object.keys(req.query[i]), req.query[i]));
            this.doQuery("GET", path, { ids }, query, req.options, (err, res) => {
                if (err?.status === 404) err = null;
                if (err || !res) return callback(err, []);
