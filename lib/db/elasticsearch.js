...
Pool.prototype._getUpdate = function(req)
{
    ...
    if (req.options && (!lib.isEmpty(req.options.updateOps) || !lib.isEmpty(req.options.expected))) {
        ...
        for (const p in req.obj) {
            ...
            // Handle dotted columns as nested maps, 2 levels only
            if (p.indexOf(".") > 0) {
                param = p.split(".");
                obj.script.source += `if(!ctx._source.containsKey('${param[0]}') || ctx._source.${param[0]}==null) ctx._source.${param[0]} = new HashMap();`;
                src += "." + param[0];
                prop = param[1];
                param = param.join("");
            }
