 ...
 */
class ElasticsearchPool extends db.Pool {
    ...
    getQueryCondition(req, obj, join)
    {
        ...
        return Object.keys(obj).map((name) => {
            ...
            const col = db.prepareColumn(pool, req, name, val);
    ...
    getGeoQuery(table, obj, options)
    {
        options.sort_names = ["distance"];
        var q = typeof obj == "string" ? obj : this.getQueryString({ table, obj, options });
    ...
    _buildCondition(req, obj, expected, join)
    {
        var expr = [];
        for (const p in expected) {
            ...
            if (d) {
                const e = this._buildCondition(req, obj, val, d[1] == "or" ? "||" : "");
                if (e) expr.push("(" + e + ")");
                continue;
            }
            const col = db.prepareColumn(this, req, p, val);
            var not = col.op.startsWith("not_") ? "!" : "";
            ...
            case "not_in":
                ...
                obj.script.params["_" + p] = col.value;
                break;
            ...
            case "not_between":
                ...
                obj.script.params["_" + p] = col.value[0];
                obj.script.params["__" + p] = col.value[1];
                break;
    ...
    _getUpdate(req)
    {
        ...
        if (!lib.isEmpty(req.options.updateOps) || !lib.isEmpty(req.options.expected)) {
            ...
            const expr = this._buildCondition(req, obj, req.options.expected, req.options.expectedJoin || "and");
            ...
            for (const p in req.obj) {
                ...
                case "not_exists":
                    obj.script.source += `if(!ctx.${src}.containsKey('${prop}') || ctx.${src}.${prop}==null) ctx.${src}.${prop}=params.${param};`;
                    obj.script.params[param] = val;
    ...
    query(client, req, options, callback)
    {
        ...
        case "sql":
            this.querySql(client, req, options, callback);
            break;

        case "select":
        case "search":
            this.querySelect(client, req, options, callback);
            break;
        ...
        case "updateall":
            this.queryUpdateAll(client, req, options, callback);
            break;
        ...
        case "delall":
            this.queryDelAll(client, req, options, callback);
            break;

        case "bulk":
            this.queryBulk(client, req, options, callback);
            break;
    ...
    queryCreate(client, req, options, callback)
    {
    ...
    queryGet(client, req, options, callback)
    {
        var table = this._getTable(req.table);
        var keys = db.getKeys(req.table, req.options);
        var config = this.configOptions;
        var path = table + "/" + config.docType + "/" + lib.escape(this._getKey(keys, req.obj));
    ...
    querySelect(client, req, options, callback)
    {
        var table = this._getTable(req.table);
        var config = this.configOptions;
        var cols = db.getColumns(req.table, req.options);
        ...
        if (lib.isObject(req.obj)) {
            if (req.obj.aggs || req.obj.aggregations || req.obj.query) {
                ...
                if (req.obj.q && typeof req.obj.q == "string") {
                    req.obj.q = lib.phraseSplit(req.obj.q, req.options.splitOptions);
                }
        ...
        if (req.options.scroll_timeout > 0) {
            query.scroll = lib.toNumber(req.options.scroll_timeout) + "ms";
        } else
        if (req.options.fullscan) query.scroll = config.scroll_timeout;
    ...
    queryList(client, req, options, callback)
    {
        var table = this._getTable(req.table);
    ...
    queryAdd(client, req, options, callback)
    {
        var table = this._getTable(req.table);
        var keys = db.getKeys(req.table, req.options);
        var config = this.configOptions;
    ...
    queryUpdate(client, req, options, callback)
    {
        var table = this._getTable(req.table);
        var keys = db.getKeys(req.table, req.options);
        var config = this.configOptions;
        var path = table + "/_update/" + lib.escape(this._getKey(keys, req.obj));
        var query = this._getQuery("index", req.options);
    ...
    queryUpdateAll(client, req, options, callback)
    {
        var table = this._getTable(req.table);
    ...
    queryDel(client, req, options, callback)
    {
        var table = this._getTable(req.table);
        var keys = db.getKeys(req.table, req.options);
        var config = this.configOptions;
    ...
    queryBulk(client, req, options, callback)
    {
        var keys = db.getKeys(req.table, req.options);
        var config = this.configOptions;

        var info = { retry_count: 0, _size: req.obj.length, _retries: 0, _timeout: config.bulkRetryTimeout };
        var bulk = req.obj, bulkSize = req.options.bulkSize || config.bulkSize;
        var map = {}, errors = [];
        lib.doWhilst(
            (next) => {
                var item, meta, data = "";
                ...
                this.doQuery("POST", "/_bulk", data, "", options, (err, res) => {
                    if (!err && res) {
                        var retry, item;
    ...
    queryDelAll(client, req, options, callback)
    {
        var table = this._getTable(req.table);
    ...
    querySql(client, req, options, callback)
    {
        var path = "/_sql";
