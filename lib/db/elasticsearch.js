...
exports.defaults = {
    ...
    config: {
        ...
        typesMap: {
            ...
            ttl: "long", timeout: "long", counter: "long", random: "long",
            long: "long", bigint: "long", int: "integer",
            now: { type: "date", format: "epoch_millis" },
            mtime: { type: "date", format: "epoch_millis" },
            datetime: { type: "text" },
            bool: "boolean", keyword: "keyword", symbol: "keyword", "list": "keyword", set: "keyword",
            text: "text", str: "text", string: "text", json: "text", "varchar": "text",
            object: "object",
...
const _mappings = ["index","store","boost","format","doc_values","null_value","normalizer","analyzer","search_analyzer","norms","fields","fielddata","similarity"]
const escapeRx = /([:+=&|><!(){}[\]^"~*?/\\-])/g
const quoteRx = /[ @/]/
const subfieldRx = /^([a-z0-9_]+:)(.+)/i
 ...
 *
 * Configuration:
 *  - `db-elasticseach-pool=default` - connect to local ElasticSearch server
 *
 *  - `db-elasticseach-pool=https://eshost:9200/` - connect to remote ElasticSearch server
 *
...
class ElasticsearchPool extends DbPool {
    ...
    constructor(options)
    {
        ...
        this._nodes = [];
        this._nodesTimer = setTimeout(this._getNodes.bind(this), this.config.discoveryDelay || 500);
    }
    ...
    _getNodes(callback)
    {
        if (!this.url || this._nodesTimer === true) return;
        clearTimeout(this._nodesTimer);
        ...
        if (this.config.noDiscovery) return;
        this._nodesTimer = true;
        this.queryES("GET", "/_nodes", "", "", { quiet: 1, logger_db: "none" }, (err, data, params) => {
            if (!err && typeof data?.nodes === "object") {
                ...
                for (const p in data.nodes) {
                    ...
                    if (lib.isEmpty(node.roles) || (!this.config.clientNodes && lib.includes(node.roles, "data"))) {
            ...
            this._nodesInterval = ((err ? this._nodesInterval : 0) || this.config.refreshInterval) * (err ? this.config.refreshBackoff : 1);
            const interval = !err && this._nodes.length ? this.config.discoveryInterval || 300000 : this._nodesInterval;
            this._nodesTimer = setTimeout(this._getNodes.bind(this), interval);
            logger.debug("getNodes:", this.name, this.url, this._node, this._version, "nodes:", this._nodes, "interval:", this._nodesInterval, interval);
    ...
    _retryPrepare(params)
    {
        ...
        if (node) {
            ...
            const servers = lib.split(this.config.servers);
            if (!this._servers?.every((x) => (servers.indexOf(x) > -1))) this._servers = servers;
        ...
        params.origUrl = params.nodeHost + params.nodePath;
    ...
    queryES(method, path, postdata, query, options, callback)
    {
        var self = this;
        var opts = {
            ...
            retryTimeout: options.retryTimeout || this.config.retryTimeout,
            retryOnError() { return this.status === 429 || this.status >= 500 },
            retryPrepare() { self._retryPrepare(this) },
            retryNodes: 0,
        };
        lib.fetch(opts.nodeHost + opts.nodePath, opts, (err, params) => {
            logger.logger(options.logger_db || "debug", "queryES:", this.name, params.elapsed, 'ms', opts, "hits:", params.obj?.hits?.hits?.length, err);
            ...
            if (err || !params.status || params.status >= 400) {
                if (!err) {
                    const error = params.obj?.error;
                    err = lib.newError({
                        message: (error ? lib.inspect(error) : "") || (method + " Error " + params.status + " " + params.data),
                        code: error?.type,
                        status: params.status || 500
                    });
                    logger[options.quiet || params.status === 404 ? "debug": "error"]("queryES:", this.name, opts, "ERR:", params.status, params.data);
                } else {
                    params.obj = null;
                }
            }
    ...
    convertError(_req, err)
    {
        if (err.status === 429) err.code = "OverCapacity";
        return err;
    }

    nextToken(_client, req, rows)
    {
    ...
    _getTable(name)
    {
        return this.config.tableMap[name] || name;
    }
    ...
    _escape(val, options, name)
    {
        ...
        case "string":
            ...
            opts.noescape === true ||
            opts.noescape === name ||
            lib.includes(opts.noescape, name);

            if (!noescape) {
                if (opts.subfields) {
                    const d = val.match(subfieldRx);
                    if (d) {
                        prefix = d[1];
                        val = d[2];
                    }
                }
                val = val.replace(escapeRx, '\\$1');
            }
            if (val === "OR" || val === "AND" || val === "NOT" || quoteRx.test(val)) val = '"' + val + '"';
            break;
        default:
            val = null;
        }
        ...
        if (name) {
            if (opts._fuzziness?.[name]) val += "~" + opts._fuzziness[name];
            if (opts._boost?.[name]) val += "^" + opts._boost[name];
            if (lib.includes(opts._ends_with, name) && val.endsWith("\\*")) val = val.slice(0, -2) + "*";
            if (lib.includes(opts._begins_with, name) && val.startsWith("\\*")) val = "*" + val.substr(2);
        }
        return val;
    }
    ...
    getQueryString(req, query, join)
    {
        ...
        return Object.keys(query).map((name) => {
            ...
            const d = name.match(/^\$+(OR|AND|NOT)$/i);
            if (d) {
                switch (d[1]) {
                case "NOT":
                case "not":
                    val = this.getQueryString(req, val);
                    if (val) val = "(NOT (" + val + "))";
                    break;
                default:
            ...
            if (typeof val === "string") {
                val = col.value.trim();
                if (!val && col.op !== "null" && col.op !== "not null") return 0;
            } else
            if (lib.isArray(val)) {
            ...
            switch (col.op) {
            case "null":
                return lib.isArray(val) ? '(' + val.map((_y) => (`(NOT _exists_:${col.name})`)).join(cjoin) + ')' : `(NOT _exists_:${col.name})`;

            case "not null":
                return lib.isArray(val) ? '(' + val.map((_y) => (`_exists_:${col.name}`)).join(cjoin) + ')' : "_exists_:" + col.name;
            ...
            case "not all in":
                if (lib.isEmpty(val)) break;
                return lib.isArray(val) ? '(' + val.map((y) => ("NOT " + name + this._escape(y, options, col.name))).join(` ${col.join || "AND"} `) + ')' : "NOT " + name + this._escape(val, options, col.name);

            case "in":
            case "contains":
                if (lib.isEmpty(val)) break;
                return lib.isArray(val) ? '(' + val.map((y) => (name + this._escape(y, options, col.name))).join(` ${col.join || "OR"} `) + ')' : name + this._escape(val, options, col.name);

            case "not in":
            case "not contains":
                if (lib.isEmpty(val)) break;
                return lib.isArray(val) ? name + '(' + val.map((y) => ("NOT " + name + this._escape(y, options, col.name))).join(cjoin) + ')' : "NOT " + name + this._escape(val, options, col.name);
            ...
            case "like%":
                if (lib.isEmpty(val)) break;
                return lib.isArray(val) ? val.map((y, _i) => (name + this._escape(y, options, col.name) + '*')).join(cjoin) : name + this._escape(val, options, col.name) + '*';

            case "not begins with":
            case "not like%":
                if (lib.isEmpty(val)) break;
                return lib.isArray(val) ? val.map((y, _i) => (`NOT ${name}${this._escape(y, options, col.name)}*`)).join(cjoin) : `NOT ${name}${this._escape(val, options, col.name)}*`;
    ...
    getGeoQuery(table, query, options)
    {
        ...
        var d = options.distance || this.config.distance;
    ...
    {
        return _params[name].reduce((x, y) => {
            if (options[y]) x[y] = options[y]; else
            if (this.config.defaultParams[y]) x[y] = this.config.defaultParams[y];
            return x;
        }, {});
    }
    ...
    _buildCondition(req, query, expected, join)
    {
        ...
        for (const p in expected) {
            let val = expected[p];
            const op = p.match(/^\$+(OR|AND|NOT)$/i)?.[1];
            if (op) {
                switch (op) {
                case "NOT":
                case "not":
                    val = this._buildCondition(req, query, val);
                    if (val) expr.push("!(" + val + ")");
                    break;

                default:
                    val = this._buildCondition(req, query, val, op);
                    if (val) expr.push("(" + val + ")");
                }
                continue;
            }
            ...
            const not = col.op.startsWith("not ") ? "!" : "";
    ...
    _buildUpdate(req, obj, query)
    {
        ...
        if (!lib.isEmpty(req.options.ops) || !lib.isEmpty(query)) {
            ...
            for (const p in obj) {
                let val = obj[p], op = req.options.ops?.[p], prop = p, param = p, src = "_source", col;
    ...
    cacheColumns(_client, _options, callback)
    {
        var docType = this.config.docType;
        this.queryES("GET", "/_mappings", "", "", this.config, (err, data, _params) => {
            if (err || !data) return callback(err);
            const dbcolumns = Object.create(null), dbindexes = Object.create(null), dbkeys = Object.create(null);

            for (const table in data) {
                if (!data[table].mappings) continue;
                if (!dbcolumns[table]) dbcolumns[table] = Object.create(null);
                if (!dbkeys[table]) dbkeys[table] = 1;
                const properties = data[table].mappings.properties || data[table].mappings[docType]?.properties;
                const index_cols = db.getIndexColumns(table);
                for (const c in properties) {
                    if (!dbcolumns[table][c]) dbcolumns[table][c] = Object.create(null);
                    const col = properties[c];
                    for (const p in col) {
                        if (p === "type" && col[p] === "text") continue;
                        dbcolumns[table][c][p] = col[p];
                    }
                    for (const i in index_cols) {
                        if (index_cols[i].includes(c)) {
                            dbindexes[table + "_" + index_cols[i].join("_")] = index_cols[i];
                        }
                    }
                }
            }
        ...
        setTimeout(this._getNodes.bind(this), this.config.discoveryDelay || 500);
    }
    ...
    query(client, req, callback)
    {
        ...
        case "drop":
            ...
            this.queryES("DELETE", this._getTable(req.table), "", "", this.config, callback);
            break;
    ...
    queryCreate(_client, req, callback)
    {
        var missing = 0
        const table = this._getTable(req.table);
        const ecols = this.dbcolumns[table];
        const dynamic_templates = [];
        const properties = {};
        const config = req.config;
        const tconfig = req.query._$elasticsearch || req.query._$db;

        if (tconfig?.dynamic_templates?.length) {
            dynamic_templates.push(...tconfig.dynamic_templates);
        }

        for (const name in req.query) {
            const col = req.query[name];
            if (req.op === "upgrade" && ecols?.[name]) continue;

            // All specific properties go as is
            const mapping = col._$elasticsearch?.mapping;
            for (const f in mapping) {
                if (f === "dynamic_templates" && mapping[f]?.length) {
                    dynamic_templates.push(...mapping[f]);
                } else {
                    if (!properties[name]) properties[name] = {};
                    properties[name][f] = mapping[f];
                }
            }
            ...
            if (col.params && col.type === "object") {
                ...
                for (const param in col.params) {
                    ...
                    for (const m in _mappings) {
                       if (prop[m]) properties[name].properties[param][m] = prop[m];
                    }
               }
            } else {
                // Just the type mapping
                const type = col.join ? "text" : db.getColumnType(req, col.type);
            ...
            if (!properties[name].type && !properties[name].properties) {
                properties[name].type = col.keyword ? "keyword": db.getColumnType(req, col.type);
            }
            if (!ecols?.[name]) missing++;
        }

        if (!this.dbkeys[table]) {
            this.dbcolumns[table] = properties;
            this.dbkeys[table] = 1;
            const query = {
                settings: {
                    number_of_shards: tconfig?.shards || config[lib.toCamel("shards-" + table, "-")] || config.shards,
                    number_of_replicas: tconfig?.replicas || config[lib.toCamel("replicas-" + table, "-")] || config.replicas,
                    analysis: {
                        filter: {
                            email: {
                ...
                mappings: {
                    date_detection: tconfig?.date_detection || config.date_detection,
                    numeric_detection: tconfig?.numeric_detection || config.numeric_detection,
                    dynamic_templates,
                },
            };
            ...
            lib.series([
                ...
                (next) => {
                    this.queryES("PUT", table, query, "", config, next);
                },
            ], callback, true);
            return;
        }
        if (missing) {
            return this.queryES("PUT", table + "/_mappings", { properties }, "", config, callback);
        }
        callback();
    }
    ...
    queryGet(_client, req, callback)
    {
        ...
        this.queryES("GET", path, null, query, req.options, (err, res) => {
    ...
    querySelect(_client, req, callback)
    {
        var table = this._getTable(req.table);
        var config = req.config;
        ...
        if (lib.isObject(req.query)) {
            if (req.query.aggs || req.query.aggregations || req.query.query) {
                ...
                if (lib.isObject(req.options.filter)) {
                    ...
                    if (!obj.query.bool.must.query_string.query) obj.query.bool.must.query_string = undefined;
                } else {
                    obj = {
                        query: {
                    ...
                    if (!obj.query.query_string.query) obj.query = undefined;
                    if (req.options.noscan && lib.isEmpty(obj)) {
                        logger.info('querySelect:', this.name, req, "NO EMPTY SCANS ENABLED");
                        return callback();
                    }
                }
                opts = obj;
        ...
        if (req.options.sort && !query.scroll_id) {
            const sort = lib.split(req.options.sort).map((x) => {
                if (!x) return null;
                ...
                const col = cols[x];
                if (col) {
                    const raw = col._$elasticsearch?.raw;
                    if (desc) {
                        return raw ? { [x + ".raw"]: { order: "desc" } } : { [x]: { order: "desc" } };
                    }
                    return raw ? x + ".raw" : x;
                }
                // Nested object
        ...
        this.queryES(method, path, obj, query, req.options, (err, res) => {
            if (err?.status === 404) {
            ...
            if (res.hits) {
                ...
                rows = res.hits.hits.map((x) => {
                    if (Array.isArray(req.options.sort_names) && Array.isArray(x.sort)) {
                        for (let i = 0; i < req.options.sort_names.length; i++) {
            ...
            if (res._scroll_id) {
                if (!rows.length || (req.options.limit > 0 && req.options.limit_count + rows.length >= req.options.limit)) {
                    this.queryES("DELETE", "/_search/scroll", { scroll_id: res._scroll_id }, "", config, lib.noop);
                    res.next_token = null;
                }
            }
    ...
    queryList(_client, req, callback)
    {
        ...
        for (const i in req.query) {
            ids.push(this._getKey(Object.keys(req.query[i]), req.query[i]));
        }
        this.queryES("GET", path, { ids }, query, req.options, (err, res) => {
            if (err?.status === 404) err = null;
            if (err || !res) return callback(err, []);
            const rows = res.docs ? res.docs.filter(x => x.found).map((x) => (x._source || x.fields || {})) : [];
            if (!req.options.debug) res.docs = undefined;
            callback(null, rows, res);
        });
    }
    ...
    queryAdd(_client, req, callback)
    {
        ...
        if (req.options.returning === "*" || req.options.returning === "new") req.options.result_query = 1;
        this.queryES("PUT", path, req.query, query, req.options, (err, res) => {
    ...
    queryUpdate(_client, req, callback)
    {
        ...
        this.queryES("POST", path, obj, query, req.options, (err, res) => {
            if (!err && res?.result === "updated") res.affected_rows = 1;
            if (err?.status === 404) err = null;
            if (err?.status === 409) err.code = "AlreadyExists";
            const rows = obj._source && res.get?._source ? [res.get?._source] : [];
            callback(err, rows, res);
        });
    }

    queryUpdateAll(_client, req, callback)
    {
        var table = this._getTable(req.table);
        ...
        const u = this._buildUpdate(req, req.query);
        if (u?.script) {
            obj.script = u.script;
        }
        this.queryES("POST", path, obj, query, req.options, (err, res) => {
    ...
    queryDel(_client, req, callback)
    {
        ...
        this.queryES("DELETE", path, null, query, req.options, (err, res) => {
            if (!err && res) res.affected_rows = 1;
            if (err?.status === 404) err = null;
            callback(err, [], res);
        });
    ...
    queryBulk(_client, req, callback)
    {
        ...
        var bulk = req.query, bulkSize = req.options.bulkSize || config.features?.bulk;
        var map = {}, errors = [];
        lib.doWhilst(
            (next) => {
                var item, meta, data = "";
                ...
                for (const i in batch) {
                    ...
                    meta = { _id: this._getKey(keys, item.query), _index: this._getTable(item.table) };
                    switch (item.op) {
                    case "add":
                        data += lib.stringify({ create: meta }) + "\n";
                        data += lib.stringify(item.query) + "\n";
                        break;
                    case "put":
                ...
                this.queryES("POST", "/_bulk", data, "", req.options, (err, res) => {
                    if (!err && res) {
                        let retry, item;
                        for (const i in res.items) {
                            ...
                            if (item?.status >= 400 && map[item._id]) {
                                if (item.status === 429) {
                                    ...
                                    errors.push(Object.assign(map[item._id], { _id: item._id, errstatus: item.status, errmsg: item.error?.reason }));
                                }
                            }
                        }
    ...
    queryDelAll(_client, req, callback)
    {
        var table = this._getTable(req.table);
    ...
    querySql(_client, req, callback)
    {
        ...
        this.queryES("POST", path, obj, query, req.options, (err, res) => {
            if (err?.status === 404) err = null;
            if (err || !res) return callback(err, []);
            const rows = lib.isArray(res.rows, []).map((x) => (x.reduce((r, v, i) => { r[res.columns[i].name] = v; return r }, {})));
            if (!req.options.no_next_token) res.next_token = res.cursor;
            if (!req.options.debug) res.rows = undefined;
            callback(err, rows, res);
        });
    }
...
exports.Pool = ElasticsearchPool;
