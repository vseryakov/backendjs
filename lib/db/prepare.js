...
{
    var req = db.prepareRequest({ op, table, query, options });
...
db.prepareRequest = function(options)
{
    ...
    Object.defineProperty(req, "now", { value: Date.now() });
    return req;
}
...
db.prepareColumn = function(req, name, value)
{
    ...
    var opsMap = req?.config?.opsMap || lib.empty;
    var typesMap = req?.config?.typesMap || lib.empty;
    var orig = name;
    ...
    op = (opsMap[op] || op || "").toLowerCase().replaceAll("_", " ");

    var type = (req?.columns || lib.empty)[name]?.type;
...
function prepareForUpdate(req)
{
    ...
    for (const p in req.query) {
        ...
        if (col) {
            ...
            if (req.config.noJoinColumns && Array.isArray(col.join) && !col.join.includes(p)) continue;
            ...
            if (v !== null) {
                ...
                default:
                    if (col.primary || col.index || col.type || (typeof v != "undefined" && req.config.defaultType)) {
            ...
            if (typeof v == "string") {
                max = col.max || req.config.maxSize;
            ...
            if (col.not_empty && lib.isEmpty(v)) {
                ...
                if (typeof req.config.emptyValue != "undefined") {
        ...
        if (lib.isEmpty(v)) {
            ...
            if (req.config.skipEmpty) {
                if (lib.testRegexp(col && col.type, req.config.skipEmpty[req.op])) continue;
            }
        }
        // Skip NULLs by op and type
        if ((v === null || v === "") && !col?.notempty && req.config.skipNull) {
    ...
    for (const p in req.columns) {
        ...
        max = col.maxlist || req.config.maxList;
