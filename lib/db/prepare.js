...
db.prepare = function(op, table, obj, options)
{
    ...
    const req = { op: op, table: this.alias(table), text: "", obj: obj, options: lib.objClone(options) };
    ...
    for (const p in req.obj) {
        ...
        if (col) {
            ...
            if (v !== null) {
                ...
                case "set":
                case "list":
                    if (pool.configOptions.noObjects) {
                        if (typeof v != "string") v = lib.stringify(v);
                        if (v === "[]" || v === "{}") v = null;
                        break;
                    }
    ...
    for (const p in cols) {
        ...
        if (!opts.noconvertrows_json) {
            if (col.type == "json" ||
                ...
                const list = col.type == "list" || col.type == "set";
