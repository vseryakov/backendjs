 ...
 * @method prepare
 */
db.prepare = function(op, table, query, options)
{
    var req = db.prepareReq(op, table, query, options);
    const pool = db.getPool(req.pool);
    if (!req.options.nopreparequery) {
        db.prepareQuery(pool, req);
    }
    pool.prepare(req);
 ...
 * @method prepareReq
 */
db.prepareReq = function(op, table, query, options)
{
    const req = {
        op,
        pool: db.getPool(options).name,
        table: db.alias(table),
        text: "",
        query,
        options: Object.assign({}, typeof options == "object" && options),
        keys: db.getKeys(table),
        custom: {},
        column: function(name) { return this.columns[name] || this.custom[name] },
        now: Date.now(),
    };
    Object.defineProperty(req, "columns", { enumerable: false, value: db.getColumns(table) });
    return req;
}
 ...
 * @method prepareQuery
 */
db.prepareQuery = function(pool, req)
{
    if (!pool) pool = db.getPool(req.options);
    ...
    case "object":
        ...
        for (const p in req.query) {
            ...
            if (!pool.configOptions.noCustomColumns && !req.columns[p]) {
    ...
    pool.prepareQuery(req);

    switch (req.op) {
    case "incr":
        prepareForIncr(pool, req);

    case "add":
    case "put":
    case "update":
        prepareForUpdate(pool, req);
        break;

    case "del":
    case "delall":
    case "updateall":
        prepareForSelect(pool, req);
        break;
...
function prepareForIncr(pool, req)
{
 ...
 */
function prepareForUpdate(pool, req)
{
    ...
    for (const p in req.query) {
        ...
        if (!col && !req.options?.no_columns) {
            ...
            if (pool.configOptions.noObjectTypes) continue;
            var dot = p.indexOf(".");
            if (dot == -1) continue;
            col = req.column(p.substr(0, dot));
            if (!col || !lib.rxObjectType.test(col.type)) continue;
        }
        if (Array.isArray(req.options?.skip_columns) && req.options.skip_columns.includes(p)) continue;
        if (Array.isArray(col?.allow_pools) && !col.allow_pools.includes(req.pool)) continue;

        if (col) {
            // Skip artificial join columns
            if (pool.configOptions.noJoinColumns && Array.isArray(col.join) && !col.join.includes(p)) continue;
            ...
            if (v !== null) {
                ...
                case "object":
                    ...
                    if (pool.configOptions.noObjectTypes && typeof v != "string") {
                        v = lib.stringify(v);
                        if (v === "{}" || v === "[]") v = null;
                    }
                    break;
                ...
                case "set":
                case "list":
                    if (pool.configOptions.noListTypes) {
            ...
            if (Array.isArray(col.values) && !col.values.includes(v)) {
                failed.push([p, "values", col.values.length]);
                continue;
            }
            // Skip non-matching values, mostly for string types
            if (col.noregexp) {
                const rx = Array.isArray(col.noregexp) && col.noregexp || [col.noregexp];
                if (rx.some((r) => (lib.testRegexp(v, r)))) {
                    failed.push([p, "noregexp", col.noregexp]);
                    continue;
                }
            } else
            if (col.regexp) {
                const rx = Array.isArray(col.regexp) && col.regexp || [col.regexp];
                if (!rx.some((r) => (lib.testRegexp(v, r)))) {
                    failed.push([p, "regexp", col.regexp]);
                    continue;
                }
            }
            ...
            if (typeof v == "string") {
                max = col.max || pool.configOptions.maxSize;
            ...
            if (col.not_empty && lib.isEmpty(v)) {
                if (!insert) continue;
        ...
        if ((v === null || v === "") && !col?.notempty && pool.configOptions.skipNull) {
            if (lib.testRegexp(col?.type, pool.configOptions.skipNull[req.op])) continue;
        }
        // auto update ops
    ...
    for (const p in req.columns) {
        col = req.columns[p];
        // Restrictions
        if ((col.readonly && (req.op == "incr" || req.op == "update")) ||
        ...
        if (insert) {
            ...
            if (req.query[p] === undefined) {
                if (col.type == "counter" && pool.configOptions.initCounters) req.query[p] = 0;
            }
        }
        ...
        case "now":
            if (insert && !req.query[p]) {
                req.query[p] = col.epoch ? lib.now() : col.clock ? lib.clock()/1000 : req.now;
            }
            break;
        case "clock":
            if (insert && !req.query[p]) {
        ...
        case "ttl":
            ...
            if (insert && !req.query[p] && (col.days > 0 || col.hours > 0 || col.minutes > 0)) {
                ...
                if (!pool.configOptions.epochTtl) req.query[p] *= 1000;
            }
            break;
        }
        if (typeof req.query[p] == "number") {
            if (col.not_zero && req.query[p] === 0) delete req.query[p];
            if (col.multiplier) req.query[p] *= col.multiplier;
            if (col.increment) req.query[p] += col.increment;
            if (col.decimal > 0) req.query[p] = lib.toNumber(req.query[p].toFixed(col.decimal));
        }
        if (typeof req.query[p] == "string") {
            if (col.strip) req.query[p] = req.query[p].replace(col.strip, "");
            for (const r in col.replace) req.query[p] = req.query[p].replaceAll(r, col.replace[r]);
            if (col.trim) req.query[p] = req.query[p].trim();
            if (col.lower) req.query[p] = req.query[p].toLowerCase();
            if (col.upper) req.query[p] = req.query[p].toUpperCase();
            if (col.cap) req.query[p] = lib.toTitle(req.query[p], col.cap);
            if (col.word > 0) req.query[p] = lib.strSplit(req.query[p], col.separator || " ")[col.word - 1];
        }
        if (req.query[p] !== undefined && col.type == "counter") {
            req.query[p] = lib.toNumber(req.query[p]);
        }
        if (req.query[p] !== undefined && typeof col.format == "function") {
            req.query[p] = col.format(req.query[p], req);
        }
        // Max length limits for arrays and objects, approximate
        max = col.maxlist || pool.configOptions.maxList;
        ...
        joinColumn(req, req.query, p, col);
        if (insert && col.fail_ifempty && lib.isEmpty(req.query[p])) {
...
function prepareForSelect(pool, req)
{
    var col, v, type, ops, op;
    ...
    for (const p in req.columns) {
        ...
        op = ops[p] || (col.ops && col.ops[req.op]) || (Array.isArray(v) && "in");
        if (op && !ops[p]) {
        ...
        default:
            ...
            if (Array.isArray(v) && ops[p] && !lib.isFlag(db.arrayOps, ops[p])) {
        ...
        // Case conversion
        if (type == "string") {
            if (col.trim) req.query[p] = v.trim();
            if (col.lower) req.query[p] = v.toLowerCase();
            if (col.upper) req.query[p] = v.toUpperCase();
        }
        ...
        if (lib.isFlag(db.arrayOps, ops[p])) {
            if (!Array.isArray(v) || v.length) {
                if (v) {
                    req.query[p] = lib.strSplit(v, null, { unique: 1, datatype: col.datatype || !lib.rxListType.test(col.type) && col.type });
                } else {
                    delete req.query[p];
                }
            } else
...
function prepareForList(pool, req)
{
...
/**
 * Prepare a column for an operation, convert aliases, types and ops according to the request and pool mapping
 *
 ...
 *
 * @return {object} - an object with final column details: { name, type, op, value, join }
 * @memberOf module:db
 * @method prepareColumn
 */
db.prepareColumn = function(pool, req, name, value)
{
    ...
    var jops = req.options.joinOps || lib.empty;
    var orig = name;
    ...
    if (!op && value === null) op = "null";
    if (!op && Array.isArray(value)) op = "in";
    op = opsMap[op] || op;
    ...
    return { name, type, op, value, join: jops[name], orig };
}
...
function joinColumn(req, query, name, col)
{
    ...
    if (col.join_ifempty && query[name]) return;
    if (req.options.noJoinColumns) return;
    if (Array.isArray(req.options.join_skip) && req.options.join_skip.indexOf(name) > -1) return;
    if (Array.isArray(col.join_ops) && col.join_ops.indexOf(req.op) == -1) return;
    if (Array.isArray(col.join_pools) && col.join_pools.indexOf(req.pool) == -1) return;
    if (Array.isArray(col.join_nopools) && col.join_nopools.indexOf(req.pool) > -1) return;

    var separator = col.separator || db.separator;
    if (!col.join_force && typeof query[name] == "string" && query[name].indexOf(separator) > -1) return;
    var c, d, v = "", n = 0;
    ...
    if (col.join_cap) v = lib.toTitle(v, col.col_join_cap);
    if (col.join_hash) v = lib.hash(v);
    if (typeof col.join_process == "function") v = col.join_process(req, v, query, col);
    query[name] = v;
}
...
function unjoinColumns(rows, name, col, options)
{
    if (Array.isArray(col.unjoin) || (lib.toBool(col.unjoin) && Array.isArray(col.join))) {
        ...
        var row, separator = col.separator || db.separator;
        for (var i = 0; i < rows.length; i++) {
            ...
            if (typeof row[name] == "string" && row[name].indexOf(separator) > -1) {
                ...
                if (v.length >= unjoin.length) {
                    ...
                    if (!col.join_keep && unjoin.indexOf(name) == -1) delete row[name];
                }
            }
        }
    }
...
db.convertRows = function(pool, req, rows, options)
{
    ...
    if (!pool) pool = db.getPool(req.options || options);
    ...
    for (const p in req.columns) {
        ...
        if (!opts.noconvertrows_json) {
            if (col.type == "json" || (pool.configOptions.noObjectTypes && lib.rxObjectType.test(col.type))) {
            ...
            if (pool.configOptions.noListTypes && lib.rxListType.test(col.type)) {
                for (i = 0; i < rows.length; i++) {
