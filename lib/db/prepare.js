...
{
    var req = db.prepareReq(op, table, obj, options);
    const pool = db.getPool(req.pool);
    if (!req.options.nopreparerow) {
 ...
 * @method prepareReq
 */
db.prepareReq = function(op, table, obj, options)
{
    const req = {
        op: op,
...
db.prepareRow = function(pool, req)
{
    ...
    if (!req.options.noprocessrows) {
        ...
        default:
            ...
            db.runProcessRows("pre", req.table, req, req.obj);
        }
        // Always run the global hook, keep the original object
        db.runProcessRows("pre", "*", req, req.obj);
    }
    ...
    case "bulk":
        ...
        for (const i in req.obj) {
            ...
            obj = db.prepare(obj.op, obj.table, obj.obj, obj.options);
            if (obj.error) {
...
function prepareForIncr(pool, req)
{
    if (!lib.objSearch(req.options.updateOps, { hasValue: "incr", count: 1 })) {
        ...
        for (const p in req.custom) {
            if (req.custom[p].type == "counter" && typeof req.obj[p] != "undefined") req.options.updateOps[p] = "incr";
        }
    }
}
...
function prepareForUpdate(pool, req)
{
    ...
    for (const p in req.columns) {
        ...
        if (insert) {
            if (col.value !== undefined && req.obj[p] === undefined) req.obj[p] = col.value;
            if (req.obj[p] === undefined) {
        ...
        case "suuid":
            ...
            break;
        case "sfuuid":
            if (insert && !req.obj[p]) req.obj[p] = lib.sfuuid(col);
            break;
        case "random":
            if (insert && !req.obj[p]) {
        ...
        case "now":
            if (insert && !req.obj[p]) {
                req.obj[p] = col.epoch ? lib.now() : col.clock ? lib.clock()/1000 : req.now;
            }
            break;
        case "clock":
            if (insert && !req.obj[p]) {
        ...
        if (typeof req.obj[p] == "number") {
            if (col.not_zero && req.obj[p] === 0) delete req.obj[p];
        ...
        if (typeof req.obj[p] == "string") {
            ...
            for (const r in col.replace) req.obj[p] = req.obj[p].replaceAll(r, col.replace[r]);
            ...
            if (col.cap) req.obj[p] = lib.toTitle(req.obj[p], col.cap);
            if (col.word > 0) req.obj[p] = lib.strSplit(req.obj[p], col.separator || " ")[col.word - 1];
        }
        if (req.obj[p] !== undefined && col.type == "counter") {
            req.obj[p] = lib.toNumber(req.obj[p]);
        }
        if (req.obj[p] !== undefined && typeof col.format == "function") {
        ...
        if (max > 0 && Array.isArray(req.obj[p])) {
            if (req.obj[p].length > max) {
                if (col.trunc) {
                    req.obj[p] = req.obj[p].slice(0, max);
                } else {
                    failed.push([p, "maxlist", req.obj[p].length, max]);
                    delete req.obj[p];
                }
            }
        }
        max = col.max || pool.configOptions.maxSize;
        if (max > 0 && lib.isObject(req.obj[p])) {
            v = lib.objSize(req.obj[p]);
            if (v > max) {
...
function prepareForDelete(pool, req)
{
    var v, col;
    for (const p in req.obj) {
        v = req.obj[p];
    ...
    for (const p in req.columns) {
        joinColumn(req, req.obj, p, req.columns[p]);
    }
}
    ...
    for (const p in req.columns) {
        ...
        if (lib.isFlag(db.arrayOps, ops[p])) {
            if (!Array.isArray(v) || v.length) {
                if (v) {
                    req.obj[p] = lib.strSplit(v, null, { unique: 1, datatype: col.datatype || !lib.rxListType.test(col.type) && col.type });
                } else {
                    delete req.obj[p];
                }
            } else
...
function joinColumn(req, obj, name, col)
{
    if (!col) return;
    switch (col.type) {
    case "geohash":
    case "geopoint":
        ...
        var lat = lib.toNumber(obj[col.lat || "latitude"] || req.orig[col.lat || "latitude"]);
        var lon = lib.toNumber(obj[col.lon || "longitude"] || req.orig[col.lon || "longitude"]);
        if (lat && lon) {
            obj[name] = lat + "," + lon;
        } else {
            delete obj[name];
    ...
    if (typeof col.join_process == "function") v = col.join_process(req, v, obj, col);
    obj[name] = v;
}
