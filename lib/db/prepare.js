...
function prepareForUpdate(req)
{
    ...
    for (let name in req.query) {
        ...
        const $d = name.lastIndexOf("_$");
        if ($d > 0) {
            let j = $d + 2;
            while (name[j] === "$") j++;
            const op = name.substr(j);
            name = name.substr(0, $d);
            if (op) {
                if (!req.options.ops) req.options.ops = {};
                req.options.ops[name] = op;
            }
        }
    ...
    for (const name in req.columns) {
        ...
        switch (col.type) {
        case "uuid":
            if (insert && !lib.isUuid(req.query[name], col.prefix)) req.query[name] = lib.uuid(col.prefix);
            break;
        case "suuid":
            if (insert && !req.query[name]) req.query[name] = lib.suuid(col.prefix, col);
            break;
        case "sfuuid":
            if (insert && !req.query[name]) req.query[name] = lib.sfuuid(col);
            break;
        case "random":
        ...
        case "now":
            if (insert && !req.query[name]) {
                req.query[name] = col.convert?.epoch ? lib.now() : req.now;
            }
            break;
        case "clock":
            if (insert && !req.query[name]) {
                req.query[name] = lib.clock();
            }
            break;
        case "ttl":
