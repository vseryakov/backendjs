...
function prepareForUpdate(pool, req)
{
    ...
    for (const p in req.obj) {
        v = req.obj[p];
        if (db.skipColumn(req, p, v)) continue;
        col = req.column(p);
        if (col) {
            ...
            if (v !== null) {
                ...
                default:
                    if (pool.configOptions.noTypes) break;
    ...
    req.obj = o;
    var allkeys = db.getKeys(req.table, { allkeys: 1 });
    for (const p in req.columns) {
        col = req.columns[p];
        // Restrictions
        if (col.hidden ||
            db.skipColumn(req, p, true) ||
        ...
        case "random":
            ...
            break;
        case "now":
            if (insert || !req.obj[p] || !allkeys.includes(p)) {
        ...
        if (typeof req.obj[p] == "number") {
            if (col.nozero && req.obj[p] === 0) delete req.obj[p];
        ...
        if (typeof req.obj[p] != "undefined" && typeof col.format == "function") {
...
function prepareForDelete(pool, req)
{
    ...
    for (const p in req.obj) {
        v = req.obj[p];
        col = req.column(p);
        if (col?.hidden || db.skipColumn(req, p, v)) continue;
        // Convert into native data type
        if (!pool.configOptions.noTypes && typeof v != "undefined") {
...
function prepareForSelect(pool, req)
{
    ...
    var o = {}, col, v, type, ops, op;
    for (const p in req.obj) {
        col = req.column(p);
        if (col?.hidden) continue;
        if (p.includes("$") || !db.skipColumn(req, p, req.obj[p])) {
            o[p] = req.obj[p];
        }
    }
    req.obj = o;
 ...
 *
 * Op in the name after _$ takes precedence
 * @example
...
db.prepareColumn = function(pool, req, name, value)
{
    ...
    return { name, type, op, value, join: jops[name] };
}
