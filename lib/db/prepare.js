...
{
    var req = this.prepareReq(op, table, obj, options);
    const pool = this.getPool(req.pool);
...
db.prepareReq = function(op, table, obj, options)
{
    const req = {
        op: op,
        pool: this.getPool(options).name,
    ...
    Object.defineProperty(req, "columns", { enumerable: false, value: this.getColumns(table) });
    return req;
}
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.columns) {
        ...
        if (req.options.syncMode) {
            this.joinColumn(req, req.obj, p, col);
            continue;
        }
...
db.prepareForDelete = function(pool, req)
{
    ...
    for (const p in req.obj) {
        v = req.obj[p];
        col = req.column(p);
        if (col?.hidden || this.skipColumn(req, p, v)) continue;
        // Convert into native data type
    ...
    req.obj = o;
    for (const p in req.columns) {
        this.joinColumn(req, req.obj, p, req.columns[p]);
    }
}
...
db.prepareForSelect = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (p.includes("$") || !this.skipColumn(req, p, req.obj[p])) {
            o[p] = req.obj[p];
        }
    }
    req.obj = o;
 ...
 * - `join_process`` - a function(req, value, obj, col) - must return a value to be used, the value is joined already
 */
db.joinColumn = function(req, obj, name, col)
{
    if (!col) return;
    switch (col.type) {
    case "geohash":
    ...
    if (Array.isArray(col.nojoin_pools) && col.nojoin_pools.indexOf(req.pool) > -1) return;
