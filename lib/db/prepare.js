...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (col) {
            ...
            if (typeof v == "string") {
                let max = col.max || pool.configOptions.maxSize;
                if (max > 0 && v.length > max) continue;

                max = col.maxlength || pool.configOptions.maxLength;
                if (max > 0 && v.length > max && lib.rxTextType.test(col.type || pool.configOptions.defaultType || "text")) {
                    v = v.substr(0, max);
                }
            }
