...
class DbRequest {
    ...
    constructor(options) {
        ...
        /**
         * Per-process unique request id
         * @member {number}
         */
        this.drID = ++_id >= _id_max ? (_id = 1) : _id;
         ...
         * @member {string}
         */
        this.text = lib.isString(options.text)

        /**
         * values to be used for SQL binding
         * @member {any[]}
         */
        this.values = lib.isArray(options.values, undefined)

        /**
         * a query object
         * @member {object}
         */
        this.query = lib.isObject(options.query) ||
                     lib.isArray(options.query) ||
                     lib.isString(options.query) ||
                     Object.create(null)
         ...
         */
        this.options = Object.assign(Object.create(null), lib.isObject(options.options), { pool: this.#pool.name })

        this.options.ops = lib.isObject(this.options.ops, Object.create(null));
    ...
    /**
     * link pool.config for quick convenient access
     * @member {object}
     */
    get config() {
...
db.prepare = function(options)
{
    ...
    logger.logger(req.options.logger_db || "debug", "prepare:", "db", req);
    return req;
}
...
db.prepareQuery = function(req)
{
    ...
    case "object":
        for (const p in req.query) {
            if (req.config.features?.custom && !req.columns[p]) {
    ...
    case "drop":
        if (lib.testRegexp(req.table, db.skip?.drop)) {
            req.error = lib.newError("Unable to drop", 400, "SkipDrop");
        }
        break;
    }
}
...
db.prepareColumn = function(req, name, value)
{
    ...
    op = op.toLowerCase().replaceAll("_", " ");
    op = req?.config?.opsMap?.[op] || op;
 ...
 * @param {DbRequest} [req] - request object or null, it is not required to be strict DbRequest
 * @param {object} options
 * @param {any} [options.type] - converted native type
 ...
 * @method prepareValue
 */
db.prepareValue = function(_req, options)
{
    ...
    const type = col?.type || options.type;
    ...
    default:
        if (lib.rxDateType.test(type)) {
            if (value) options.value = lib.toValue(value, type);
        } else

        if (lib.rxNumericType.test(type)) {
            if (vtype === "string" && value) options.value = lib.toNumber(value);
        } else

        if (vtype === "number") {
            options.value = lib.toValue(value, type);
        } else
    ...
    // Case conversion, only subset in queries
    if (vtype === "string" && value) {
    ...
    logger.dev("prepareValue:", "db", options);
    return options;
}
...
function prepareForIncr(req)
{
    for (const p in req.columns) {
        if (req.columns[p].type === "counter" && req.query[p] !== undefined) {
            req.options.ops[p] = "incr";
        }
    }
    for (const p in req.custom) {
        if (req.custom[p].type === "counter" && req.query[p] !== undefined) {
            req.options.ops[p] = "incr";
        }
    }
}
...
function prepareForUpdate(req)
{
    ...
    for (let name in req.query) {
        ...
        // Skip unsupported columns, still can be used for $or/$and/$join
        if (!name || name[0] === '_') continue;

        // Keep all ops for skip logic later
        if (aop) {
            req.options.ops[name] = aop;
        }

        if (value === undefined || aop === "none") continue;

        col = req.column(name);

        // Allow nested fields if objects supported and the parent exists
        if (!col && !req.options?.no_columns) {
            if (!req.config.features?.json) continue;
        ...
        if (col) {
            // Restrictions
            if (col.read_only && !insert) continue;
            ...
            if (value !== null) {
                ...
                case "object":
                    ...
                    if (!req.config.features?.json && typeof value !== "string") {
                        value = lib.stringify(value);
                        if (value === "{}" || value === "[]") value = null;
                    }
                    break;
                ...
                case "set":
                case "list":
                    if (!req.config.features?.list) {
                        max = col.validate?.max_list || req.config.maxList;
                        if (max > 0 && Array.isArray(value)) {
                            if (value.length > max) {
                                if (col.trunc) {
                                    value = value.slice(0, max);
                                } else {
                                    failed.push(`Column ${name} is too large, ${value.length} > ${max}`);
                                }
                            }
                        }
                ...
                default:
                    ...
                    if (col.type || (col.primary && insert)) {
                        value = lib.toValue(value, col.type, col);
                    }
                }
            }
            ...
            if (typeof value === "string") {
                max = col.validate?.max || req.config.maxSize;
                if (max > 0 && value.length > max) {
                    if (col.validate?.trunc) {
        ...
        if (lib.isEmpty(value) && col?.validate?.skip_empty) continue;
    ...
    for (const name in req.columns) {
        ...
        if (insert) {
            if (col.value !== undefined && value === undefined) {
                value = req.query[name] = col.value;
            }
        }
        ...
        case "now":
            if (!value) {
                value = col.convert?.epoch ? lib.now() : col.convert?.clock ? lib.clock(): req.now;
            }
            break;

        case "uuid":
            if (insert && !lib.isUuid(value, col.prefix)) {
                value = lib.uuid(col.prefix);
            }
            break;

        case "suuid":
            if (insert && !value) {
                value = lib.suuid(col.prefix, col);
            }
            break;

        case "sfuuid":
            if (insert && !value) {
                value = lib.sfuuid(col);
            }
            break;

        case "random":
            if (insert && !value) {
                value = col.max || col.min ? lib.randomInt(col.min, col.max) : lib.randomUInt();
            }
            break;
        }

        if (typeof value === "number") {
            for (const p in col.convert) {
                const v = col.convert[p];
                switch (p) {
                case "multiplier":
                    if (lib.isNumber(v)) value *= v;
                    break;
                case "increment":
                    if (typeof v === "number") value += v; else
                    if (v === "now") value += Date.now();
                    break;
                case "decimal":
                    if (v > 0) value = lib.toNumber(value.toFixed(v));
                    break;
                }
            }
        }

        if (typeof value === "string" && col.convert) {
            for (const p in col.convert) {
                const v = col.convert[p];
                switch (p) {
                case "strip":
                    ...
                    break;
                case "replace":
                    for (const r in v) value = value.replaceAll(r, v[r]);
                    break;
                case "trim":
                    value = value.trim();
                    break;
                ...
                case "upper":
                    ...
                    break;
                case "cap":
                    value = lib.toTitle(value, col.cap);
                    break;
                }
            }
        }

        if (value !== undefined && col.type === "counter") {
            value = lib.toNumber(value);
        }

        if (value !== undefined && typeof col.convert?.format === "function") {
            value = col.convert?.format(value, req);
        }
        ...
        max = col.validate?.max || req.config.maxSize;
        if (max > 0 && lib.isObject(value)) {
            const size = lib.objSize(value);
            if (size > max) {
                failed.push(`Column ${name} is too large, ${size} > ${max}`);
            }
        }

        if (insert && col.validate?.not_empty && lib.isEmpty(value)) {
            failed.push(`Column ${name} must not be empty`);
        }

        if (value === undefined) delete req.query[name]; else req.query[name] = value;
    }

    if (failed.length) {
        req.error = { status: 400, message: failed.join("\n"), code: "ValidationFailed" };
    }
...
function prepareForSelect(req)
{
    ...
    for (const name in req.query) {
        ...
        const col = db.prepareColumn(req, name, req.query[name]);
        if (!col.col && !req.options?.no_columns) continue;
        if (col.op === "none") {
            req.options.ops[name] = col.op;
            continue;
        }
        row[name] = col.value;
    }
...
function prepareForList(req)
{
    ...
    for (const row of req.query) {
        ...
        for (const name in row) {
            ...
            const col = db.prepareColumn(req, name, row[name]);
            if (!col.col && !req.options?.no_columns) continue;
            if (col.op === "none") {
                req.options.ops[name] = col.op;
                continue;
...
{
    const joins = db.joins[req.table];
    if (!joins?.length) return;
    for (const name of joins) {
        const col = req.column(name);
        if (!col?.join?.length) continue;
        if (req.options.ops[name] === "none") continue;

        const value = col.join.map(x => query[x]);
        if (value.indexOf(undefined) > -1) continue;
 ...
 * @method convertRows
 */
db.convertRows = function(req, rows, _options)
{
    var i, col;
    ...
    for (const name in req.columns) {
        ...
        if (col.type === "json" || (!req.config.features?.json && lib.rxObjectType.test(col.type))) {
        ...
        if (col.split) {
            for (i = 0; i < rows.length; i++) {
                rows[i][name] = lib.toValue(rows[i][name], "list", col.split);
            }
        } else

        if (!req.config.features?.list && lib.rxListType.test(col.type)) {
            for (i = 0; i < rows.length; i++) {
                rows[i][name] = lib.toValue(rows[i][name], col.type, col.split);
            }
        }

        // Default value on return
        if (req.columns[name].dflt) {
            for (i = 0; i < rows.length; i++) {
                if (rows[i][name] === undefined || rows[i][name] === null) {
                    switch (typeof req.columns[name].dflt) {
                    case "object":
                        rows[i][name] = lib.clone(req.columns[name].dflt);
                        break;
                    default:
 ...
 */
db.getProcessRows = function(type, table, _options)
{
    if (!type || !table || !db.processRows[type]) return null;
    return lib.isArray(db.processRows[type][table], null);
}
...
{
    if (!req) return rows;
    const hooks = db.getProcessRows(type, table, req.options);
 ...
 */
db.setProcessRow = function(type, table, callback)
{
    if (!type || !table || typeof callback !== "function") return;
    if (!db.processRows[type]) db.processRows[type] = Object.create(null);
