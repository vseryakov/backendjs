...
function prepareForUpdate(req)
{
    ...
    for (let name in req.query) {
        ...
        if (col) {
            ...
            if (col.check?.not_empty && lib.isEmpty(value)) {
                if (!insert) continue;
                value = req.config.emptyValue ?? null;
            }
        }
    ...
    for (const name in req.columns) {
        ...
        if (typeof value === "number") {
            if (col.check?.not_zero && value === 0) value = undefined;
            if (col.convert) {
                if (lib.isNumber(col.convert.multiplier)) value *= col.convert.multiplier;
                if (typeof col.convert.increment === "number") value += col.convert.increment; else
                if (col.convert.increment === "now") value += Date.now();
                if (col.convert.decimal > 0) value = lib.toNumber(value.toFixed(col.convert.decimal));
            }
        }

        if (typeof value === "string" && col.convert) {
            if (col.convert.strip) value = value.replace(col.convert.strip, "");
            for (const r in col.convert.replace) value = value.replaceAll(r, col.convert.replace[r]);
            ...
            if (col.convert.cap) value = lib.toTitle(value, col.cap);
        }
        ...
        if (max > 0 && Array.isArray(value)) {
            if (value.length > max) {
                if (col.trunc) {
                    ...
                    failed.push([name, "maxlist", value.length, max]);
                    value = undefined;
                }
            }
        }
        ...
        if (insert && col.check?.fail_ifempty && lib.isEmpty(value)) {
...
function prepareForSelect(req)
{
    ...
    for (const name in req.query) {
        ...
        row[name] = db.prepareColumn(req, name, req.query[name]).value;
    }
    req.query = row;
    prepareJoinColumns(req, req.query);
}
