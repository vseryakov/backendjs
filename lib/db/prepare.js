...
{
    const pool = this.getPool(table, options);

    // Check for table name, it can be determined in the real time
    const atable = pool.resolveTable(op, table, obj, options);

    // Prepare row properties
    const req = { op: op, table: atable, text: "", obj: obj, options: lib.objClone(options) };

    // Prevent from using non-app tables, because of aliases have to allow it for DDL
    switch (req.op) {
    case "create":
    case "upgrade":
        break;
    default:
        if (!this.tables[table] && !(this.unknownTables || req.options.unknowntables)) {
            req.error = lib.newError("unknown table");
        }
    }
    ...
    logger.logger(req.options.logger_db || "debug", "prepare:", atable, req);
    return req;
}
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (col) {
            ...
            if (col.maxlength && typeof v == "string" && lib.rxTextType.test(col.type || pool.configOptions.defaultType || "text") && v.length > col.maxlength) {
