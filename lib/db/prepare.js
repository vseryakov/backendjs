...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (col) {
            ...
            if (typeof v == "string") {
                const max = col.max || pool.configOptions.maxSize;
                const lmax = col.maxlength || pool.configOptions.maxLength;
                if (max > 0 && v.length > max && !lmax) continue;

                if (lmax > 0 && v.length > lmax && lib.rxTextType.test(col.type || pool.configOptions.defaultType || "text")) {
                    v = v.substr(0, lmax);
                }
            }
