...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.columns) {
        ...
        case "sfuuid":
            ...
            break;
        case "uid":
            if (req.options.user) req.obj[p] = req.options.user.id;
            break;
        case "uname":
            if (req.options.user) req.obj[p] = req.options.user.name;
            break;
        case "random":
            if (insert && !req.obj[p]) {
...
db.prepareForSelect = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (p[0] == "$" || !this.skipColumn(req, p, req.obj[p])) {
            o[p] = req.obj[p];
        }
    }
    req.obj = o;
    ...
    for (const p in req.columns) {
        ...
        if (lib.isFlag(db.arrayOps, ops[p])) {
            if (!Array.isArray(v) || v.length) {
                if (v) {
                    req.obj[p] = lib.strSplitUnique(v, null, { datatype: col.datatype || !lib.rxListType.test(col.type) && col.type });
                } else {
                    delete req.obj[p];
                }
            } else
