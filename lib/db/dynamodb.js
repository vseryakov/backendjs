...
class DynamoDBPool extends DbPool {
    ...
    queryCreate(client, req, callback)
    {
        ...
        var cap = lib.split(this.config["capacity" + ctable], ",", { datatype: "int" });
    ...
    queryUpgrade(client, req, callback)
    {
        ...
        for (const type in indexes) {
            ...
            if (this.dbindexes[table] && this.dbindexes[table][name]) continue;

            const add = {}, cap = this.dbcapacity[table] && this.dbcapacity[table][table] || lib.empty;
