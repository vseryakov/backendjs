...
class DynamoDBPool extends DbPool {
    ...
    {
        req.options.query = db.getKeys(req.table).map((x) => (x)).reduce((x,y) => { x[y] = null; return x }, {});
        ...
        lib.doWhilst(
            function(next) {
                ...
                list.forEach((x) => {
                    switch (x.op) {
                    case "add":
                        x.options.query = db.getKeys(x.table).map((x) => (x)).reduce((x,y) => { x[y] = null; return x }, {});
                        break;
                    case "incr":
                    case "update":
                        ...
                        if (x.options.noupsert || (x.op == "update" && !x.options.upsert)) {
                            if (x.options.query) {
                                for (const p in x.keys) if (!x.options.query[p]) x.options.query[p] = x.keys[p];
                            } else
                        if (!x.options.Expected && !x.options.expr && !x.options.ConditionExpression) x.options.query = x.keys;
                    }
                    break;
                case "del":
