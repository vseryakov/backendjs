...
const pool = {
    ...
    createPool: function(options) {
        return new DynamoDBPool(options);
    }
}
module.exports = pool;
...
class DynamoDBPool extends DbPool {
    ...
    cacheColumns(options, callback)
    {
        ...
        lib.series([
            (next) => {
                if (lib.isArray(tables)) return next();
    ...
    prepareCreate(table)
    {
        ...
        var keys = Object.keys(table).filter((x, i) => (table[x].primary)).
        ...
        for (const type of ["index", "unique"]) {
            (new Array(this.configOptions.maxIndexes)).fill(0, 0).forEach((_, n, t) => {
                n = n || "";
                t = type + n;
                var idx = Object.keys(table).filter((x) => (table[x][t])).sort((a,b) => (table[a][t] - table[b][t])).filter((x, i) => (i < 2));
                if (!idx.length) return;
                var name = idx.join("_");
                if (name.length < 3) name = "i_" + name;
                // Index starts with the same hash, local unless explicitly defined as global
                if (idx.length == 2 && idx[0] == hash && !table[idx[1]].global) {
                ...
                idx.forEach((y) => { attrs[y] = 1 });
                projections[name] = table[idx[0]].projections;

                var p = Object.keys(table).filter((x, i) => {
                    if (idx.includes(x) || keys.includes(x)) return 0;
                    return table[x]["projection" + n] ||
                        (table[x].projections && !Array.isArray(table[x].projections)) ||
                        (Array.isArray(table[x].projections) && table[x].projections.includes(lib.toNumber(n)));
                });
    ...
    queryCreate(client, req, callback)
    {
        ...
        Object.keys(attrs).forEach((x) => {
            attrs[x] = !req.query[x].join && lib.isNumericType(req.query[x].type) ? "N" : "S";
            for (const p in req.query[x].custom?.dynamodb) {
                req.options[p] = req.query[x].custom.dynamodb[p];
            }
        });
    ...
    queryUpgrade(client, req, callback)
    {
        ...
        (new Array(this.configOptions.maxIndexes)).fill(0, 0).forEach((u, n) => {
            n = n || "";
            var idx = Object.keys(req.query).filter((x, i) => (req.query[x]["index" + n])).
            ...
            idx.forEach((x) => {
                if (req.query[x].readCapacity) add.readCapacity = req.query[x].readCapacity;
                if (req.query[x].writeCapacity) add.writeCapacity = req.query[x].writeCapacity;
                if (!add.readCapacity && cap.read) add.readCapacity = cap.read;
                if (!add.writeCapacity && cap.write) add.writeCapacity = cap.write;
                add[x] = !req.query[x].join && lib.isNumericType(req.query[x].type) ? "N" : "S";
            });
            add.projection = Object.keys(req.query).filter((x, i) => {
                if (x == idx[0] || x == idx[1]) return 0;
                return (req.query[x].projections && !Array.isArray(req.query[x].projections)) ||
                (Array.isArray(req.query[x].projections) && req.query[x].projections.indexOf(lib.toNumber(n)) > -1);
            });
            global[name] = add;
            return;
        });
    ...
    {
        var keys = db.getSearchQuery(req.table, req.query, { noempty: 1 });
    ...
    queryPrepareSelect(client, req, callback)
    {
        ...
        if (!req.query[dbkeys[0]] && !req.options.fullscan) {
            for (const p in dbindexes) {
                ...
                const max = idx[1] && req.query[idx[1]] ? 2 : 1;
        ...
        req.options.keys = !req.options.sort ? Object.keys(req.query) : Object.keys(req.query).
            filter((x) => ((columns[x] && columns[x].primary) || dbkeys.includes(x) || (dbattrs && dbattrs.includes(x)))).
            filter((x) => (x != dbkeys[0] && Array.isArray(join) && join.indexOf(x) > -1 ? 0 : 1));

        var query = db.getSearchQuery(req.table, req.query, req.options);
        ...
        var select = lib.strSplit(req.options.select);
        if (select.length && dbattrs) {
            var lsi = req.options.sort && dbkeys.every((x) => (dbcolumns[x] && dbcolumns[x].lsi));
            select = lsi ? select.filter((x) => (columns[x])) :
                           select.filter((x) => ((columns[x] && columns[x].primary) || dbkeys.includes(x) || dbattrs.includes(x)));
        }
        req.options.select = select;
        req.options.keys = dbkeys;
        req.options.attrs = dbattrs;
        this.queryRunSelect(op, client, req, query, callback);
    }

    queryRunSelect(op, client, req, query, callback)
    {
        ...
        if (op == 'ddbScanTable' && req.options.noscan) {
            logger.info('select:', this.name, req.table, op, req.options.sort, 'query:', query, 'keys:', req.options.keys, 'attrs:', req.options.attrs, "NO EMPTY SCANS ENABLED");
        ...
        if (!req.options.count) req.options.count = this.configOptions.selectCount;

        logger.debug('select:', this.name, op, req.table, req.options.sort, 'query:', query, 'keys:', req.options.keys, 'attrs:', req.options.attrs, 'count:', req.options.count, 'cap:', cap.rateCapacity, cap.useCapacity);
        ...
        for (var p in req.options.ops) {
            ...
            if (req.options.ops[p] == "in" && lib.isArray(query[p]) && (p == req.options.keys[0] || p == dbkeys[0])) {
                if (query[p].length == 1) {
                    query[p] = query[p][0];
                    delete req.options.ops[p];
                } else {
            ...
            // Noop for a hash key
            if (req.options.ops[p] && op == "ddbQueryTable" && (p == req.options.keys[0] || (!req.options.sort && p == dbkeys[0]))) {
                req.options.ops[p] = '';
            }
            ...
            function(next) {
                aws[op](req.table, query, req.options, (err, item) => {
                    if (req.options.total) {
                        ...
                        if (inFilter) item.Items = item.Items.filter((x) => (inFilter.includes(x[inKey])));
    ...
    queryList(client, req, callback)
    {
        ...
        lib.forEachLimit(chunks, req.options.concurrency, (chunk, next) => {
            const batch = { [req.table]: { keys: chunk, select: db.getSelectedColumns(req), consistent: req.options.consistent } };
    ...
    {
        req.options.expected = db.getKeys(req.table).map((x) => (x)).reduce((x,y) => { x[y] = null; return x }, {});
    ...
    {
        var keys = db.getSearchQuery(req.table, req.query);
    ...
    {
        var info = { consumed_capacity: 0, retry_count: 0, _size: req.query.length, _retries: 0 }, rows = [];
        ...
        lib.doWhilst(
            function(next) {
                ...
                aws.ddbBatchWriteItem(breq, req.options, (err, item) => {
        ...
        lib.doWhilst(
            function(next) {
                ...
                list.forEach((x) => {
                    switch (x.op) {
                    case "add":
                        x.options.expected = db.getKeys(x.table).map((x) => (x)).reduce((x,y) => { x[y] = null; return x }, {});
                        break;
                    case "incr":
                    case "update":
