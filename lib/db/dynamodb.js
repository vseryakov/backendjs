...
Pool.prototype.cacheColumns = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            // Replace all at once
            for (const p in schema) self[p] = schema[p];
            next();
        },
    ], callback, true);
}
...
Pool.prototype.queryCreate = function(client, req, callback)
{
    ...
    var keys = Object.keys(req.obj).filter((x, i) => (req.obj[x].primary)).
                      sort((a,b) => (req.obj[a].primary - req.obj[b].primary)).
                      filter((x, i) => (i < 2)).
    ...
    for (const type of ["index", "unique"]) {
        (new Array(this.configOptions.maxIndexes)).fill(0, 0).forEach((_, n, t) => {
            n = n || "";
            t = type + n;
            var idx = Object.keys(req.obj).filter((x) => (req.obj[x][t])).sort((a,b) => (req.obj[a][t] - req.obj[b][t])).filter((x, i) => (i < 2));
            ...
            var p = Object.keys(req.obj).filter((x, i) => {
