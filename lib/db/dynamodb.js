...
class DynamoDBPool extends db.Pool {
    ...
    {
        const { keys, local, global, attrs, projections } = this.prepareCreate(req.obj);
        ...
        Object.keys(attrs).forEach((x) => {
            ...
            for (const p in req.obj[x].custom?.dynamodb) {
                req.options[p] = req.obj[x].custom.dynamodb[p];
            }
        });
    ...
    queryUpgrade(client, req, callback)
    {
        ...
        (new Array(this.configOptions.maxIndexes)).fill(0, 0).forEach((u, n) => {
            n = n || "";
            var idx = Object.keys(req.obj).filter((x, i) => (req.obj[x]["index" + n])).
                sort((a, b) => (req.obj[a]["index" + n] - req.obj[b]["index" + n])).
                filter((x, i) => (i < 2));
            if (!idx.length) return;
            var name = idx.join("_");
            if (name.length < 3) name = "i_" + name;
            ...
            add.projection = Object.keys(req.obj).filter((x, i) => {
    ...
    queryPrepareSelect(client, req, callback)
    {
        ...
        req.options.keys = !req.options.sort ? Object.keys(req.obj) : Object.keys(req.obj).
    ...
    queryList(client, req, callback)
    {
        ...
        var dbkeys = db.getKeys(req.table);
        var chunks = [];
        for (let i = 0; i < req.obj.length; i+= this.configOptions.batchSize) {
            chunks.push(req.obj.slice(i, i + this.configOptions.batchSize).filter((x) => (dbkeys.every((y) => (x[y] !== "")))));
        }
    ...
    queryAdd(client, req, callback)
    {
        ...
        aws.ddbPutItem(req.table, req.obj, req.options, (err, rc) => {
            if (!rc) rc = {};
    ...
    queryUpdate(client, req, callback)
    {
        ...
        if (req.options.useCapacity || req.options.capacity) req.options.ReturnConsumedCapacity = "TOTAL";
        aws.ddbUpdateItem(req.table, keys, req.obj, req.options, (err, rc) => {
            if (!rc) rc = {};
            if (!err) rc.affected_rows = 1;
            if (!err && rc.ConsumedCapacity) rc.consumed_capacity = rc.ConsumedCapacity.CapacityUnits;
