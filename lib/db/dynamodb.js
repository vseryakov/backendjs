 ...
 *
 * Configuration:
 *  - `db-dynamodb-pool=http://localhost:8181` - connect to local DynamoDB server
 *
 *  - `db-dynamodb-pool=default` - connect to AWS DynamoDB in the same region
 *
 */
...
class DynamoDBPool extends DbPool {
    ...
    constructor(options)
    {
        ...
        if (this.url === "default") this.url = "";

        this.dbcapacity = Object.create(null);

        for (const table in db.tables) {
            const { keys, local, global } = this.prepareCreate(db.tables[table]);
            this.dbkeys[table] = keys;
            for (const i in local) {
                if (!this.dbcolumns[table]) this.dbcolumns[table] = Object.create(null);
            ...
            if (!lib.isEmpty(local)) {
                this.dbindexes[table] = Object.keys(local).reduce((a, b) => { a[b] = Object.keys(local[b]); return a }, Object.create(null));
            }
        }
    }
    ...
    _parseTable(table, rc, schema)
    {
        ...
        (rc.Table.AttributeDefinitions || []).forEach(function(x) {
            if (!schema.dbcolumns[table]) schema.dbcolumns[table] = Object.create(null);
        ...
        (rc.Table.KeySchema || []).forEach(function(x) {
            ...
            lib.objSet(schema.dbcapacity, [table, table], {
                read: rc.Table.ProvisionedThroughput?.ReadCapacityUnits || 0,
    ...
    cacheColumns(_client, options, callback)
    {
        ...
        const schema = {
            dbcolumns: Object.create(null),
            dbkeys: Object.create(null),
            dbindexes: Object.create(null),
            dbcapacity: Object.create(null),
            dbprojections: Object.create(null)
        };
    ...
    prepareCreate(table)
    {
        const local = {}, global = {}, attrs = {};

        const indexes = db.getIndexColumns(table);

        const keys = indexes.primary?.slice(0, 2) || [];
        ...
        for (const type in indexes) {
            ...
            const [hash, range] = index;

            const hash_type = !table[hash].join && lib.rxNumericType.test(table[hash].type) ? "N" : "S";
            const range_type = !table[range].join && lib.rxNumericType.test(table[range].type) ? "N" : "S";
            const projection = table[hash]._$dynamodb?.["projection" + type.replace(/[^0-9]/g, "")];

            // Index starts with the same hash, local unless explicitly defined as global
            if (hash === keys[0] && range && !table[hash]._$dynamodb?.global) {
                local[name] = {
                    [hash]: hash_type,
                    [range]: range_type,
                    projection,
                ...
                if (range) {
                    global[range] = range_type;
                }
            }
        }
        return { keys, local, global, attrs };
    }

    queryCreate(_client, req, callback)
    {
        const { keys, local, global } = this.prepareCreate(req.query);

        // All native properties for options from the key columns
        keys.forEach(key => {
            for (const p in req.query[key]._$dynamodb) {
                req.options[p] = req.query[key]._$dynamodb[p];
            }
        });
        ...
        const ctable = req.table.substr(0,1).toUpperCase() + req.table.substr(1);
        const cap = lib.split(this.config["capacity" + ctable], ",", { data_type: "int" });
        ...
        lib.series([
            (next) => {
                aws.ddbCreateTable(req.table, req.options, (err, item) => {
                    rc = item;
    ...
    queryUpgrade(_client, req, callback)
    {
        var global = {}, rc;
        ...
        for (const type in indexes) {
            ...
            add.projection = req.query[index[0]]._$dynamodb?.["projection" + type.replace(/[^0-9]/, "")];
    ...
    queryPrepareSelect(client, req, callback)
    {
        ...
        const op = req.query[dbkeys[0]] !== undefined && !req.options.fullscan ? 'ddbQueryTable' : 'ddbScanTable';

        this.queryRunSelect(op, client, req, dbkeys, req.query, callback);
    }
    ...
    queryRunSelect(op, client, req, keys, query, callback)
    {
        ...
        const cap = req.options.capacity || db.getCapacity(req.table, { useCapacity: req.options.useCapacity || "read", factorCapacity: req.options.factorCapacity || 0.9 });
        ...
        const rows = [], info = { consumed_capacity: 0, total: 0, retry_count: 0 };
    ...
    queryList(_client, req, callback)
    {
        ...
        // Keep retrieving items until we reach the end or our limit
        const dbkeys = req.keys, chunks = [];
        const batchSize = req.config?.features?.batch || 100;
        ...
        lib.forEachLimit(chunks, req.options.concurrency, (chunk, next) => {
            ...
            aws.ddbBatchGetItem(batch, req.options, (err, item) => {
                ...
                let moreKeys = item.UnprocessedKeys || null;
                lib.whilst(
        ...
        lib.doWhilst(
            function(next) {
                ...
                for (const i in list) {
                    ...
                    switch (list[i].op) {
                    case "del":
                        list[i].query = db.getQueryForKeys(req.keys, list[i].query);
                    case "put":
                    case "add":
                ...
                if (lib.isEmpty(breq)) return next();

                aws.ddbBatchWriteItem(breq, req.options, (err, rc) => {
                    if (err) return callback(err, []);

                    const consumed = lib.toNumber(rc.ConsumedCapacity?.CapacityUnits);
                    info.consumed_capacity += consumed;
                    info._retries++;
                    if (rc.retry_count) info.retry_count += rc.retry_count;
                    if (!lib.isEmpty(rc.UnprocessedKeys)) {
                        items.push(...rc.UnprocessedKeys);
                    }
            ...
            function() {
                return info._retries < info._size && items.length > 0;
            },
            function(err) {
