...
class DynamoDBPool extends db.Pool {
    ...
    queryGet(client, req, callback)
    {
        ...
        req.options.select = db.getSelectedColumns(req);
    ...
    queryPrepareSelect(client, req, callback)
    {
        ...
        var select = db.getSelectedColumns(req);
        if (select && dbattrs) {
            var lsi = req.options.sort && dbkeys.every((x) => (dbcolumns[x] && dbcolumns[x].lsi));
            select = lsi ? req.options.select.filter((x) => (columns[x])) :
            req.options.select.filter((x) => ((columns[x] && columns[x].primary) || dbkeys.includes(x) || dbattrs.includes(x)));
        }
