...
db.init = function(options, callback)
{
    ...
    lib.forEachLimit(pools, options.concurrency || core.concurrency, (name, next) => {
        ...
        if (!pool.url || options.noCacheColumns || db.noCacheColumns) return next();

        // Trigger create or cache columns only for pools with url set
        if (cluster.isMaster &&
        ...
        if (!options.noCacheColumns && pool.configOptions.cacheColumns && !pool._cacheColumnsTime) {
...
db.getConfig = function(options, callback)
{
    ...
    var roles = lib.toFlags("add", [options?.role || core.role], core.configRoles);
    ...
    this.select(table, query, opts, (err, rows, info) => {
        ...
        logger.debug("getConfig:", query, rows?.length, "rows", info.elapsed, "ms");
