...
class PgClient {
    ...
    query(req, callback) {
        ...
        logger.dev("query:", "pg", req.text, req.values);
 ...
 * uses default SqlPool config. Can be used directly with databases like CockroachDB, RDS, DSQL, ....
 *
 * Configuration:
...
class PgPool extends sqlPool.Pool {
    ...
    constructor(options, defaults)
    {
        ...
        _pg?.types?.setTypeParser(_pg.types.builtins.INT8, (val) => {
            try {
                return val?.length > 16 ? BigInt(val) : Number.parseInt(val, 10);
            } catch (_e) {
                return val;
            }
        });
