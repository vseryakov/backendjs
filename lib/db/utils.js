...
db.initTables = function()
{
    for (const p in core.modules) {
        if (p != this.name && lib.isObject(core.modules[p].tables)) {
...
db.createTables = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            copts.tables = [];
            const tables = Object.keys(db.tables).filter((x) => (!lib.isFlag(db.skipTables, x)));
...
// The filter can be:
//  - a string, means return all columns that (not)contain the given property
//  - a regexp, means return all columns with names matching the regexp
//  - an array of strings or regexps
//  - an object, match column properties by value, null means not exists, `name`` will match against the column name not its properties
...
//      db.getFilteredColumns("bk_user", /^email/)
...
db.getFilteredColumns = function(table, filter, options)
{
    ...
    for (let name in filter) {
        ...
        if (name === "name" && util.isRegExp(v)) name = v;
...
db.getQueryForKeys = function(keys, obj, options)
{
    ...
    return (keys || lib.emptylist).
            ...
            map((x) => ([ options.keysMap ? (options.keysMap[x] || x) : x, obj[x] ])).
