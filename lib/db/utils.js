...
//  - an object, match column properties by value, `null`` means does not exist, `name`` will match against the column name not a property
//
...
db.getFilteredColumns = function(table, filter, options)
{
    ...
    if (typeof filter == "string") filter = { [filter]: true };
    for (const p in cols) {
        i = m = 0;
        col = cols[p];
