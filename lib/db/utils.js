 ...
 */
db.getSearchQuery = function(table, query, options)
{
    return this.getQueryForKeys(this.getKeys(table, options), query, options);
}
 ...
 */
db.getQueryForKeys = function(keys, query, options)
{
    keys = lib.isArray(keys, lib.emptylist).filter((x) => (x && x[0] != '_' && typeof query[x] != "undefined")).map((x) => ([x, query[x]]));
