...
db.createTables = function(options, callback)
{
    ...
    var copts = lib.objClone(options, "pool", pool.name, "tables", [], "unknowntables", 1);
    ...
    lib.series([
        ...
        function(next) {
            lib.forEachSeries(Object.keys(db.tables), (table, next2) => {
                ...
                const atable = db.getTable(table);
                const exists = pool.dbcolumns[atable];
                logger.debug("createTables:", core.role, pool.name, exists ? "upgrade" : "create", atable, cols);
                if (!exists) {
                    db.create(table, cols, copts, (err, rows, info) => {
                        if (!err && info.affected_rows) copts.tables.push(atable);
                        next2();
                    });
                } else {
    ...
    pool.cacheColumns.call(pool, options, (err) => {
        ...
        pool.cacheIndexes.call(pool, options, (err) => {
            ...
            if (db.noUnknownTables || pool.configOptions.noUnknownTables) {
                for (let p in pool.dbcolumns) {
                    p = p.toLowerCase();
                    if (!db.tables[db.getTable(p)] && !Object.keys(db.aliases).some((x) => (db.aliases[x] == p))) {
