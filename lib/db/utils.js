    ...
    for (const table of tables) {
        ...
        for (const p in dbcols) {
            ...
            if (r && !options?.isInternal) {
                if (col.api.noroles && lib.includes(roles, col.api.noroles)) r = cols[p] = 0; else
                if (col.api.roles && !lib.includes(roles, col.api.roles)) r = cols[p] = 0;
    ...
    for (let i = 0; i < rows.length; ++i) {
        ...
        for (const p in row) {
            ...
            if (r) {
                // Lazy copy on modify
                if (!nrow) {
                    nrow = Object.create(null);
                    for (const k in row) nrow[k] = row[k];
                    row = nrow;
                }
                if (Array.isArray(col) && row[p]) {
                    const crows = Array.isArray(row[p]) ? row[p] : [row[p]];
