...
/**
 * Returns true if the operation used the cache, this is just a wrapper around {@link module:db.getCached} with
 * consistent check when to use caching logic
 * @memberof module:db
 * @method checkCached
 */
db.checkCached = function(_op, table, query, options, callback)
{
    if (!options?.__cached &&
        !options?.nocache &&
        (options?.cached || this.cache.tables.includes(table))) {
...
db.getCached = function(op, table, query, options, callback)
{
    ...
    if (!options?.cacheKey && options?.cacheKeyName) {
        const key = this.getCacheKeys(table, query, options.cacheKeyName)[0];
        if (!key) return false;
        options.cacheKey = key;
    }
    options = lib.clone(options, { __cached: true });
    // Get the full record if not a specific cache
    if (!options.cacheKeyName) options.select = undefined;
    const req = this.prepare({ op, table, query, options });
    this.getCache(table, req.query, options, (data, cached) => {
        ...
        if (data) {
            ...
            return callback(null, data, { cached });
        }
        pool.metrics.miss_count++;
        // Retrieve user from the database, use the parameters like in Select function
...
db.getCache = function(table, query, options, callback)
{
    ...
    const ttl2 = this.getCache2Ttl(table, options);
    if (ttl2 > 0) {
        const val = this.lru.get(key);
        if (val) {
            logger.debug("getCache:", "db", "lru:", key, options, 'ttl2:', ttl2);
            return callback(val, 2);
        }
    }
    const opts = this.getCacheOptions(table, options);
    cache.get(key, opts, (_err, val) => {
        if (!val) return callback();
        if (ttl2 > 0) {
            this.lru.put(key, val, Date.now() + ttl2);
        }
        logger.debug("getCache:", "db", "ipc:", key, opts, 'ttl2:', ttl2);
        callback(val, 1);
    });
}
...
db.putCache = function(table, query, options)
{
    ...
    if (!key) return;
    const val = lib.stringify(query);
    const ttl2 = this.getCache2Ttl(table, options);
    if (ttl2) {
        this.lru.put(key, val, Date.now() + ttl2);
    }
    const opts = this.getCacheOptions(table, options, 1);
    cache.put(key, val, opts);
    logger.debug("putCache:", "db", key, opts, 'ttl2:', ttl2);
}
...
db.delCache = function(table, query, options)
{
    ...
    const opts = this.getCacheOptions(table, options, 1);
    cache.del(key, opts);
    logger.debug("delCache:", "db", key, opts, 'ttl2:', ttl2);
}
...
db.getCacheKey = function(table, query, options)
{
    ...
    let keys = this.getKeys(table, options).filter((x) => (query[x])).map((x) => (query[x])).join(this.separator);
...
{
    table = this.alias(table);
    const ttl = this.cache.ttl?.[table] || this.cache.ttl?.["*"];
    const cacheName = options?.cacheName ||
                    (update ? options?.pool && this.cache.update[options.pool + "." + table] || this.cache.update[table] || this.cache.update["*"]: "") ||
                    options?.pool && this.cache.name[options.pool + "." + table] || this.cache.name[table] || this.cache.name["*"];
    if (ttl || cacheName) {
        return { cacheName, ttl };
    }
    return null;
}
...
{
    table = this.alias(table);
    const keys = table && query ? this.cache.keys[table] : null, rc = [];
    for (const p in keys) {
        const key = !name || p === name ? keys[p].map((x) => (query[x])).join(":") : null;
...
{
    var cached = req.table && req.query && (options?.cached || this.cache.tables.includes(req.table));
    var keys = [];

    switch (req.op) {
    case "add":
        keys = this.getCacheKeys(req.table, req.query);
        break;
    case "put":
    case "update":
    case "incr":
        keys = this.getCacheKeys(req.table, req.query);
        if (cached) keys.push(this.getCacheKey(req.table, req.query, options));
    ...
    if (!keys.length) return lib.tryCall(callback, Number.NaN);

    const ttl2 = this.getCache2Ttl(req.table, options);
    if (ttl2 > 0) {
        for (const i in keys) this.lru.del(keys[i]);
    }
    const opts = this.getCacheOptions(req.table, options, 1);
    logger.debug("delCacheKeys:", "db", keys, opts, options);
