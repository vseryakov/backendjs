...
class SqliteClient {
    ...
    query(req, callback) {
        ...
        const params = {}, placeholder = req.config?.placeholder || "$";

        for (let i = 1; i <= req.values?.length; i++) {
            params[placeholder + i] = req.values[i - 1];
        }

        logger.dev("query:", "sqlite", req.text, params);
 ...
 *
 * Configuration:
 *  - `db-sqlite-pool=var/db` - path to db file
 */
...
class SqlitePool extends sqlPool.Pool {
    ...
    {
        var schema = { dbcolumns: Object.create(null), dbkeys: Object.create(null), dbindexes: Object.create(null) };
        ...
        client.query("SELECT name FROM sqlite_master WHERE type='table'", (err, tables) => {
            ...
            lib.forEachSeries(tables, (table, next) => {
                    ...
                    client.query("PRAGMA index_list(" + table.name + ")", (_err4, indexes) => {
                        lib.forEachSeries(indexes, function(idx, next2) {
                            client.query("PRAGMA index_info(" + idx.name + ")", (_err5, cols) => {
                                cols.forEach((x) => {
                                    if (!schema.dbcolumns[table.name]) schema.dbcolumns[table.name] = {};
                                    if (!schema.dbcolumns[table.name][x.name]) schema.dbcolumns[table.name][x.name] = Object.create(null);
                                    const col = schema.dbcolumns[table.name][x.name];
                ...
                // Replace all at once, it runs with pool instance context
