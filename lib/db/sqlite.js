...
class SqliteClient {
    ...
    query(req, callback) {
        ...
        for (let i = 1; i <= req.values?.length; i++) {
            params["$" + i] = req.values[i - 1];
        }

        if (/^ *(INSERT|UPDATE|DELETE|CREATE)/i.test(req.text) && !req.options?.returning) {
...
class SqlitePool extends SqlPool {
    ...
    open(callback)
    {
        ...
        var db = new pool.sqlite.DatabaseSync(file, { readOnly: !!config.readonly, allowExtension: !!config.extentions });
        ...
        for (const stmt of sql) {
            try {
                ...
                logger.error('open:', this.name, file, err);
            }
        }
