...
sql.where = function(req, query, join)
{
    ...
    if (Array.isArray(query)) {
        ...
        return query.map((x) => `(${keys.map((y) => (sql.column(req.pool, y) + "=" + sql.quote(req.pool.bindValue(req, y, x[y])))).join(" AND ")})`).
                     join(" OR ");
    }
...
{
    const { columns, placeholders, values } = sql.prepareInsert(req);
    ...
    req.text = `${op} INTO ${req.table}(${columns}) VALUES(${placeholders})`;

    if (req.op == "put" && !req.config?.upsert && req.config?.onConflictUpdate) {
        req.text += ` ON CONFLICT (${req.keys.map((x) => (sql.column(req, x)))}) DO UPDATE SET (${columns}) = (${placeholders})`;
    }
    ...
    if (req.options.ifnotexists) {
        req.text += " IF NOT EXISTS ";
    }
    if (req.options.using_ttl) {
        req.text += " USING TTL " + req.options.using_ttl;
    }
    if (req.options.using_timestamp) {
        req.text += " USING TIMESTAMP " + req.options.using_timestamp;
    }
}

/**
 * Prepare SQL insert statement, produce column names, positional and actual values
 * @param {DBRequest} req - request object
 * @return {object} as { columns, placeholders, values }
 * @memberof module:sql
 * @method prepareInsert
 */
sql.prepareInsert = function(req)
{
    const placeholder = req.config?.placeholder || "$";
    var i = 1, columns = [], values = [], placeholders = [];

    for (const p in req.query) {
        columns.push(sql.column(req, p));
        placeholders.push(placeholder + i);
        values.push(req.pool.bindValue(req, p, req.query[p]));
        i++;
    }
...
sql.update = function(req)
{
    ...
    const { placeholders, values, query } = sql.prepareUpdate(req);

    req.values = values;

    var where = sql.where(req, query);

    req.text += " SET " + placeholders + " WHERE " + where;
...
/**
 * Prepare for SQL update statement with values, SET clause and query
 * @param {DBRequest} req - request object
 * @return {object} in format { placeholders, values, query }
 * @memberof module:sql
 * @method prepareUpdate
 */
sql.prepareUpdate = function(req)
{
    const placeholder = req.config?.placeholder || "$";
    const updateOps = req.options?.updateOps || lib.empty;
    var i = 1, placeholders = [], values = [], query = {};
    ...
    for (let p in req.query) {
        ...
        p = sql.column(req, p);
        placeholders.push(req.pool.updateOps(req, updateOps[p], p, v, placeholder + i));
        values.push(req.pool.bindValue(req, p, v, updateOps[p]));
        i++;
    }
    if (req.options.expected) {
        Object.assign(query, req.options.expected);
    }
    return { placeholders, values, query };
}
