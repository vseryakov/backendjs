...
sql.createIndexes = function(req)
{
    ...
    for (const name in indexes) {
        ...
        const key = indexes[name][0];

        const copts = req.query[key]?.["_$" + req.pool.type]?.[name] || req.query[key]?._$db?.[name];
        ...
        if (_sql) {
            ...
            const type = lib.isString(copts?.[name]);
...
{
    if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(req.table)) {
        logger.error("get:", "sql", "invalid table", req.table);
        return null;
    }
