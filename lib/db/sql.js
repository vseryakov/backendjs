...
sql.where = function(req, query, join)
{
    ...
    if (Array.isArray(query)) {
        ...
        if (props.length == 1 && keys.includes(props[0])) {
            return sql.column(req, props[0]) + " IN (" + sql.valueIn(query.map((x) => (x[props[0]]))) + ")";
        }
        return query.map((x) => `(${keys.map((y) => (sql.column(req, y) + "=" + sql.quote(sql.bindValue(req, y, x[y])))).join(" AND ")})`).
                     join(" OR ");
    }
...
sql.create = function(req)
{
    ...
    if (req.op == "create") {
        ...
        rc = [`CREATE TABLE ${!req.config?.noIfExists ? "IF NOT EXISTS" : ""} ${req.table} (` +
                ...
                map((x) => {
                    var col = req.query[x], fk = col.foreign, custom = col.custom || lib.empty;
...
{
    var i = 1, columns = [], placeholders = [];

    req.values = [];

    for (const p in req.query) {
        columns.push(sql.column(req, p));
        placeholders.push(req.config.placeholder + i++);
        req.values.push(req.query[p]);
    }
...
sql.update = function(req)
{
    ...
    for (let name in req.query) {
        ...
        let placeholder = req.config.placeholder + (req.values.length + 1);
        let op = updateOps[name];

        name = sql.column(req, name);

        // Pool specific syntax
        const custom = req.pool.updateOps(req, name, value, op, placeholder);
        if (custom) {
        ...
        // Default syntax for placehoders
        switch (op) {
        case "unset":
        case "remove":
            placeholder = "NULL";
        ...
        case "not exists":
            ...
            placeholder = `COALESCE(${name},${placeholder})`;
            break;

        case "incr":
            // Increment a number
            placeholder = `COALESCE(${name},0)+${placeholder}`;
            break;
    ...
    req.text = "UPDATE " + req.table + " SET " + placeholders + " WHERE " + where;
