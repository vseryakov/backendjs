    ...
    if (req.op == "create") {
        ...
        rc = [`CREATE TABLE ${!pool.configOptions.noIfExists ? "IF NOT EXISTS" : ""} ${req.table} (` +
                ...
                map((x) => {
                    ...
                    (!pool.configOptions.noDefaults && typeof col.value != "undefined" ? "DEFAULT " + db.sqlValue(col.value, col) : "") +
                    (col.foreign_key?.table ? `REFERENCES ${col.foreign_key.table}(${col.foreign_key.name || x}) ${col.foreign_key.ondelete ? "ON DELETE " + col.foreign_key.ondelete : ""}`: "")
                }).join(",") + " " +
                (pk ? `,PRIMARY KEY(${pk})` : "") + " " +
...
{
    var select = lib.strSpit(req.options.select).map((x) => (db.sqlColumn(x, pool)));
