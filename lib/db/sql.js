...
{
    if (d) {
       try { d = (new Date(d)).toISOString() } catch (_e) { d = '' }
    } else {
...
sql.limit = function(req)
{
    ...
    if (orderby.length) {
        ...
        if (req.options.desc && !orderby.includes(" DESC")) {
            expr += " DESC";
        }
    }

    // Limit clause
    const count = lib.toNumber(req.options.count, { float: false, dflt: req.config?.features?.limit, min: 0 });
    if (count) {
        expr += " LIMIT " + count;
    }

    const page = lib.toNumber(req.options.page, { float: false, dflt: 0, min: 0 });
    const start = lib.toNumber(req.options.start, { float: false, dflt: 0, min: 0 });
    if (start) {
        expr += " OFFSET " + start;
    } else
    if (page && count) {
...
sql.expr = function(req, name, value, column)
{
    ...
    let text = req.pool.prepareExpr(req, name, value, column);
    ...
    case "not all in":
        ...
        if (raw) {
            ...
            list = list.map(val => {
                return req.pool.placeholder(req, "%" + val + "%");
            });
        }
        const not = op[0] == "n" ? "NOT " : "";
        text = "(" + list.map(x => (`${not} ${name} LIKE ${x}`)).join(" AND ") + ")";
        break;
    ...
    case "not between":
        ...
        if (list.length > 1) {
            if (raw) {
                ...
                text = `${name} ${op} ${req.pool.placeholder(req, list[0])} AND ${req.pool.placeholder(req, list[1])}`;
            }
        } else {
            if (raw) {
                text = `${name} = ${sql.value(value, column)}`;
            } else {
                text = `${name} = ${req.pool.placeholder(req, value)}`;
            }
        }
        break;
    ...
    case '@@':
        if (raw) {
            ...
            text = `${name} ${op} to_tsquery('${column.lang || "english"}',${req.pool.placeholder(req, value)})`;
        }
        break;
    ...
    case '!~* any':
        if (raw) {
            ...
            text = req.pool.placeholder(req, value) + " " + op + "(" + name + ")";
        }
        break;
    ...
    case 'not includes':
        ...
        if (raw) {
            ...
            text = name + " " + op + " " + req.pool.placeholder(req, value);
        }
        break;
    ...
    case "expr":
        if (!raw && Array.isArray(value) && typeof value[0] === "string") {
            ...
            text = value[0].replace(/\?/g, () => {
                ...
                return req.pool.placeholder(req, value[i]);
            });
        } else {
            text = value;
        }
        ...
        case "expr":
            ...
            if (Array.isArray(expr.value) && typeof expr.value[0] === "string") {
                ...
                expr.text = expr.value[0].replace(/\?/g, () => {
                    ...
                    return req.pool.placeholder(req, expr.value[i]);
                });
            } else {
...
sql.list = function(req, list)
{
    ...
    if (keys.length === 1) {
        ...
        for (const row of list) {
            placeholders.push(req.pool.placeholder(req, row[keys]));
        }
    ...
    for (const row of list) {
        const cols = [];
        for (const p in row) {
            cols.push(sql.column(req, p) + "=" + req.pool.placeholder(req, row[p]));
        }
...
sql.create = function(req)
{
    ...
    const ifNotExists = config?.features?.ifexists ? "IF NOT EXISTS" : "";
...
sql.createColumn = function(req, name, col)
{
    ...
    return name + " " + type + " " +
           ...
           (col.auto && config?.features?.auto ? " AUTO_INCREMENT " : "") +
           (col.value !== undefined ? ` DEFAULT ${sql.value(col.value, col)} `: "") +
           (opts?.sql_extra || "") +
           (fk?.table ? ` REFERENCES ${fk.table}(${fk.name || name}) ${fk.on_delete ? "ON DELETE " + fk.on_delete : ""} ${fk.custom || ""}`: "")
}
...
sql.createIndexes = function(req)
{
    ...
    for (const name in indexes) {
        ...
        const key = req.query[indexes[name][0]];

        const copts = key?.["_$" + req.pool.type]?.[name] || key?._$db?.[name];
        ...
        if (_sql) {
            ...
            const type = lib.isString(copts?.[name]) || key?.unique && "UNIQUE" || "";
...
sql.select = function(req)
{
    ...
    const select = lib.split(req.options.select).map((x) => (sql.column(req, x))).join(",") || "*";
    const where = w.where.length ? " WHERE " + w.where.join(" AND ") : "";
    const join = w.join?.length ? w.join.join(" ") : "";
...
sql.insert = function(req)
{
    ...
    if (upsert) {
        sets = sql.updateExpr(req, req.query);
        ...
        for (const name of req.keys) {
            ...
            placeholders.push(req.pool.placeholder(req, req.query[name]));
        }
    } else {
    ...
    if (upsert) {
        ...
        if (req.options.query) {
            const w = sql.where(req, req.options.query);
            if (w.where.length) {
...
sql.update = function(req)
{
    ...
    if (req.options.query) {
        const w2 = sql.where(req, req.options.query);
        if (w2.where.length) w.where.push(...w2.where);
    }
