...
sql.select = function(req)
{
    ...
    var joins = ""
    for (const table in req.query.$join) {
        const join = req.query.$join[table];

    }

    var select = lib.split(req.options.select).map((x) => (sql.column(req, x))).join(",") || "*";
    req.text = `SELECT ${select} FROM ${req.table} ${joins} ${where} ${sql.limit(req)}`;
}
