...
sql.create = function(req)
{
    ...
    const config = req.config || lib.empty;
    const typesMap = config.typesMap || lib.empty;
    var rc;

    if (req.op == "create") {
        const pk = keys('primary');
        rc = [`CREATE TABLE ${!config.noIfExists ? "IF NOT EXISTS" : ""} ${req.table} (` +
                ...
                map((x) => {
                    ...
                    return sql.column(req, x) + " " +
                        ...
                        (!config.noLengths ? ` (${col.length || config.maxLength}) ` : " ") +
                        (!config.noNulls && (col.not_null || col.check?.not_empty) ? " NOT NULL " : " ") +
                        (!config.noAuto && col.auto ? " AUTO_INCREMENT " : " ") +
                        (!config.noDefaults && col.value !== undefined ? "DEFAULT " + sql.value(col.value, col) : "") +
        ...
        rc = Object.keys(req.query).
             ...
             map((x) => {
                ...
                return `ALTER TABLE ${req.table} ADD ${sql.column(req, x)} ` +
                    (typesMap[col.type] || config.defaultType || "text") +
                    (!config.noLengths ? ` (${col.length || config.maxLength}) ` : " ") +
                    (!config.noDefaults && col.value !== undefined ? "DEFAULT " + sql.value(col.value, col) : "") +
                    `${col[req.pool.type] || ""} ${col.sql || ""} ` +
                    (fk?.table ? `REFERENCES ${fk.table}(${fk.name || x}) ${fk.ondelete ? "ON DELETE " + fk.ondelete : ""} ${fk.custom || ""}`: "")
    ...
    for (const type of ["index", "unique"]) {
        Array(config.maxIndexes).fill(0, 0).forEach((_, n, t) => {
            n = n || "";
            t = type + n;
            var cols = keys(t);
            if (!cols) return;
            ...
            rc.push(`CREATE ${type[0] == "u" && "UNIQUE" || ""} INDEX ${!config?.noIfExists ? "IF NOT EXISTS" : ""} ${idx} ON ${req.table}(${cols})`);
        });
    }
    req.text = config.noMultiSQL && rc.length ? rc : rc.join(";");
}
