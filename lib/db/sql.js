...
sql.expr = function(req, name, value, column)
{
    ...
    logger.debug("expr:", "sql", name, value, "C:", column, "R:", req)
...
sql.create = function(req)
{
    ...
    const ifNotExists = !config?.noIfExists ? "IF NOT EXISTS" : "";
    ...
    req.text = config?.noMultiSQL && text.length ? text : text.join(";");
}
...
sql.createColumn = function(req, name, col)
{
    ...
    return name + " " + type + " " +
           (col.length && !lib.isFlag(config?.noLength, type) ? ` (${col.length}) ` : " ") +
           (col.not_null || col.check?.not_empty ? " NOT NULL " : "") +
           (col.auto && !config?.noAuto ? " AUTO_INCREMENT " : "") +
