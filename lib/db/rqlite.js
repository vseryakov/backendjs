...
class RqliteClient {
    ...
    query(req, callback) {
        ...
        var opts = {
            ...
            method: "POST",
            query: { associative: 1 },
            postdata: [Array.isArray(req.text) ? req.text : req.values ? [req.text, req.values] : req.text],
        };

        lib.fetch(opts, (err, rc) => {
            var result = rc.obj?.results?.[0];
            if (!err && rc.status >= 400) {
                err = { status: rc.status, message: rc.data };
            }
            callback(err, result?.rows, { affected_rows: result?.rows_affected, last_rowid: result?.last_insert_id });
        });
    }
}
