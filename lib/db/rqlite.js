...
class RqliteClient {
    ...
    query(req, callback) {
        ...
        logger.dev("query:", "rqlite", req.text, req.values);

        let postdata, cmd = "request";
 ...
 *
 * Configuration:
 *  - `db-rqlite-pool=default` - connect to local Rqlite server
 *
 *  - `db-rqlite-pool=http://rhost:4001` - connect to remote Rqlite server
 */
