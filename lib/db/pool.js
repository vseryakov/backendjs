 ...
 * // Or async version
 *
 * const { item } = await pool.ause();
 * const { err, rows, info } = await item.aquery("SELECT * from users");
 * pool.release(item);
 *
 */
...
class DbPool extends lib.Pool {
    ...
    constructor(options, defaults)
    {
        ...
        if (lib.isPositive(options?.max)) {
            methods.create = (pool, callback) => {
                pool.openDb(callback);
            }
        ...
        /** @var {object} - pool type */
        this.type = options?.type || defaults?.type || "none";

        /** @var {object} - pool name */
        this.name = options?.pool || this.type;

        /** @var {object} - connect url */
        this.url = this.url || options?.url || defaults?.url;

        /** @var {object} - config merged with defaults */
        this.config = lib.extend(Object.create(null), defaults?.config);

        /** @var {object} - connect options */
        this.connect = lib.extend(Object.create(null), defaults?.connect);

        /** @var {object} - cached tables */
        this.dbcolumns = Object.create(null);

        /** @var {object} - cached primary keys */
        this.dbkeys = Object.create(null);

        /** @var {object} - cached indexes */
        this.dbindexes = Object.create(null);

        /** @var {object} - runtime metrics */
        this.metrics = {
            running: 0,
            err_count: 0,
            retry_count: 0,
    ...
    configure(options)
    {
        ...
        lib.extend(this.config, options?.config);
        lib.extend(this.connect, options?.connect);
    ...
    shutdown(options, callback)
    {
        ...
        super.shutdown(options, () => {
            ...
            this.dbcolumns = this.dbkeys = this.dbindexes = Object.create(null);
            this.config = this.connect = Object.create(null);
     ...
     * @param {function} [callback]
     */
    cacheIndexes(_client, _options, callback)
    {
        if (typeof callback === "function") callback();
    }
