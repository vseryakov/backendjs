...
exports.defaults = {
    config: {
        ...
        typesMap: {
            real: "real", float: "float", number: "numeric", numeric: "numeric", decimal: "numeric",
            bigint: "bigint", smallint: "smallint", int: "bigint", int32: "bigint", int64: "bigint",
            now: "bigint", mtime: "bigint", clock: "bigint",
            ttl: "bigint", timeout: "bigint", random: "bigint", counter: "bigint",
            list: "varchar", set: "varchar", obj: "json", array: "json", object: "json",
            bool: "boolean",
            text: "varchar", string: "varchar", str: "varchar", keyword: "varchar",
            uuid: "varchar", suuid: "varchar", sfuuid: "varchar", email: "varchar",
            url: "varchar", phone: "varchar", e164: "varchar",
            "": "varchar",
        },
        opsMap: {
        ...
        keywords: new Set([
            ...
            'CASCADE','CASE','CAST','CHECK','COLLATE','COLUMN','COMMIT','CONFLICT','CONSTRAINT','COUNT','CREATE','CROSS','CURRENT_DATE','CURRENT_TIME','CURRENT_TIMESTAMP',
 ...
 * @param {int} options.max - max number of clients to be allocated in the pool, 25 default for SQL but can be set to 0 for drivers like Rqlite or RDS Data.
...
class SqlPool extends DbPool {
    ...
    constructor(options, defaults)
    {
        ...
        if (options.max !== 0 && !lib.isPositive(options.max)) options.max = 25;
    ...
    cacheColumns(client, options, callback)
    {
        ...
        client.query("SELECT c.table_name,c.column_name,LOWER(c.data_type) AS data_type,c.column_default,c.ordinal_position,c.is_nullable " +
            ...
            for (const i in rows) {
                ...
                let isserial = false, val = rows[i].column_default ? String(rows[i].column_default).replace(/'/g,"").split("::")[0] : null;
                if (val && val.startsWith("nextval")) {
                    val = null;
                    isserial = true;
                }
                if (val && val.startsWith("ARRAY")) {
    ...
    prepare(req)
    {
        ...
        case "update":
            if (req.options.upsert && req.config.features.upsert) {
                sql.insert(req);
                break;
            }
        case "updateall":
    ...
    {
        logger.debug("query:", "sqlpool", req);
    ...
    {
        return req.options?.count && rows.length === req.options.count ?
               lib.toNumber(req.options.start) + lib.toNumber(req.options.count) :
               null;
    }
     ...
     * @param {DbRequest} req
     * @param {any} value - value to added to req.values
     * @return {string}
     */
    placeholder(req, value)
    {
        req.values.push(value);
        return "$" + req.values.length;
    }
     ...
     * @return {string}
     */
    prepareExpr(_req, _name, _value, _column)
    {
    }
    ...
    * @param {string} [expr.placeholder] - set placeholder if needed
    */
    prepareUpdateExpr(req, expr)
    {
        ...
        case "not_exists":
            ...
            expr.placeholder = req.pool.placeholder(req, expr.value);
            expr.text = `${expr.column}=COALESCE(${req.table}.${expr.column},${expr.placeholder})`;
            break;

        case "incr":
            // Increment a number
            expr.placeholder = req.pool.placeholder(req, expr.value);
            expr.text = `${expr.column}=COALESCE(${req.table}.${expr.column},0)+${expr.placeholder}`;
            break;

        case "add":
            // Add to a list
            expr.placeholder = req.pool.placeholder(req, expr.value);
            expr.text = `${expr.column}=TRIM(COALESCE(${req.table}.${expr.column},'')||','||${expr.placeholder},',')`;
            break;

        case "del":
            // Delete from a list
            expr.placeholder = req.pool.placeholder(req, expr.value);
            expr.text = `${expr.column}=TRIM(REPLACE(${req.table}.${expr.column},${expr.placeholder},''),',')`;
            break;
        ...
        case "prepend":
            // Append to a value
            expr.placeholder = req.pool.placeholder(req, expr.value);
            expr.text = `${expr.column}=${expr.placeholder}||COALESCE(${req.table}.${expr.column},'')`;
            break;
    ...
    queryTransaction(client, req, callback)
    {
        ...
        this.queryBulk(client, req, (err, errors, info) => {

            const text = `${err || errors?.length ? "ROLLBACK" : "COMMIT"} TRANSACTION`;
            client.query(text, (e) => {
                if (e) {
                    errors.push({ text, error: e });
                }
    ...
    queryBulk(client, req, callback)
    {
        ...
        lib.forEachLimit(req.query, req.options.concurrency, (item, next) => {
            client.query(item, (err, _rc, meta) => {
                if (err) {
                    item.error = err;
                    errors.push(item);
                } else
