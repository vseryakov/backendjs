...
//
//      -queue=redis://host1
//      -queue-options-interval=1000
//      -queue=redis://host1?bk-visibilityTimeout=30000&bk-count=2
//
...
{
    if (/^rediss?:/.test(options?.url)) return new RedisClient(options);
}
...
util.inherits(RedisClient, Client);
...
RedisClient.prototype.applyOptions = function(options)
{
...
RedisClient.prototype.initClient = function(host, port)
{
...
RedisClient.prototype.onMessage = function(channel, msg)
{
    logger.dev("onMessage:", channel, msg);
    this.emit(channel, msg);
}

RedisClient.prototype.limiter = function(options, callback)
{
...
RedisClient.prototype.subscribe = function(channel, options, callback)
{
    if (!this.subclient) {
...
RedisClient.prototype.unsubscribe = function(channel, options, callback)
{
...
RedisClient.prototype.publish = function(channel, msg, options, callback)
{
    if (!this.client.enable_offline_queue) this.client.enable_offline_queue = true;
    this.client.publish(channel, msg, callback);
}

RedisClient.prototype.stats = function(options, callback)
{
    var rc = {};
    lib.parallel([
        (next) => {
...
RedisClient.prototype.clear = function(pattern, callback)
{
    if (pattern) {
...
RedisClient.prototype.get = function(key, options, callback)
{
    if (options.listName) {
        if (key == "*") {
            if (options.del) {
...
RedisClient.prototype.put = function(key, val, options, callback)
{
    var ttl = lib.toNumber(options.ttl) || lib.toNumber(this.options.ttl);
    switch (typeof val) {
    case "boolean":
...
RedisClient.prototype.incr = function(key, val, options, callback)
{
    var ttl = lib.toNumber(options.ttl) || lib.toNumber(this.options.ttl);
...
RedisClient.prototype.del = function(key, options, callback)
{
    if (options.listName) {
...
RedisClient.prototype.lock = function(name, options, callback)
{
...
RedisClient.prototype.unlock = function(name, options, callback)
{
    this.client.del(name, callback || lib.noop);
}

RedisClient.prototype.publishQueue = function(job, options, callback)
{
    var chan = this.channel(options);
    this.client.zadd(chan, Date.now(), job, callback);
}

RedisClient.prototype._monitorQueue = function(options)
{
...
RedisClient.prototype.monitorQueue = function(options)
{
...
RedisClient.prototype.pollQueue = function(options)
{
    var visibilityTimeout = lib.validPositive(options.visibilityTimeout, this.options.visibilityTimeout);
...
RedisClient.prototype.unpublishQueue = function(msg, options, callback)
{
