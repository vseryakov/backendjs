 ...
 *
 * Protocol rediss: will use TLS to connect to Redis servers, this is required for RedisCache Serverless
 *
 * @example
...
class RedisClient extends CacheClient {
    ...
    connect(host, port) {
        ...
        if (!redisClient.redis) redisClient.redis = require("redis");
        var client = new redisClient.redis.createClient(host[1] || port || this.options.port || 6379, host[0] || "127.0.0.1", this.options);
    ...
    get(key, options, callback) {
        if (options.listName) {
            ...
            if (!key) {
                this.client.eval(redisClient.scripts.sget, 3, options.listName, options.del ? "spop" : "srandmember", lib.toNumber(options.ttl), callback);
            } else {
        ...
        if (options.set) {
            ...
            this.client.eval(redisClient.scripts.getset, 3, key, ttl, options.set, callback);
        } else {
    ...
    put(key, val, options, callback) {
        ...
        if (options.mapName) {
            if (options.setmax) {
                this.client.eval(redisClient.scripts.hmsetmax, 4, options.mapName, key, val, ttl, callback);
            } else {
                const multi = this.client.multi();
                if (!key || key == "*") {
            ...
            if (options.setmax) {
                this.client.eval(redisClient.scripts.setmax, 3, key, val, ttl, callback);
            } else
            if (ttl > 0) {
    ...
    lock(name, options, callback) {
        ...
        this.client.eval(redisClient.scripts.lock, 4, name, ttl, process.pid, set ? 1 : 0, callback);
    }
    ...
    limiter(options, callback) {
        this.client.eval(redisClient.scripts.limiter, 8,
            options.name,
            options.rate,
            options.max,
            options.interval,
