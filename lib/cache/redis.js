...
//      -cache-redis-options-interval=1000
...
class RedisClient extends Client {
    ...
    connect(host, port) {
        ...
        if (!mod.redis) mod.redis = require("redis");
        var client = new mod.redis.createClient(host[1] || port || this.options.port || 6379, host[0] || "127.0.0.1", this.options);
    ...
    get(key, options, callback) {
        if (options.listName) {
            ...
            if (!key) {
                this.client.eval(mod.scripts.sget, 3, options.listName, options.del ? "spop" : "srandmember", lib.toNumber(options.ttl), callback);
            } else {
        ...
        if (options.set) {
            ...
            this.client.eval(mod.scripts.getset, 3, key, ttl, options.set, callback);
        } else {
    ...
    put(key, val, options, callback) {
        ...
        if (options.mapName) {
            if (options.setmax) {
                this.client.eval(mod.scripts.hmsetmax, 4, options.mapName, key, val, ttl, callback);
            } else {
                const multi = this.client.multi();
                if (!key || key == "*") {
            ...
            if (options.setmax) {
                this.client.eval(mod.scripts.setmax, 3, key, val, ttl, callback);
            } else
            if (ttl > 0) {
    ...
    lock(name, options, callback) {
        ...
        this.client.eval(mod.scripts.lock, 4, name, ttl, process.pid, set ? 1 : 0, callback);
    }
    ...
    limiter(options, callback) {
        this.client.eval(mod.scripts.limiter, 8,
            options.name,
            options.rate,
            options.max,
            options.interval,
