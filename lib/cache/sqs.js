...
// Examples:
//
//      -queue=sqs://messages?bk-interval=60000
//      -queue=https://sqs.us-east-1.amazonaws.com/123456/messages?bk-visibilityTimeout=300&bk-count=2
//
...
{
    if (/^sqs:\/\/|^https:\/\/sqs/.test(options?.url)) return new SQSClient(options);
}

function SQSClient(options)
{
    Client.call(this, options);
    this.applyOptions();

    if (/^sqs:/.test(this.url)) {
        this.url = "";
...
util.inherits(SQSClient, Client);

SQSClient.prototype.applyOptions = function(options)
{
...
SQSClient.prototype.stats = function(options, callback)
{
...
SQSClient.prototype.pollQueue = function(options)
{
    var self = this;
    if (!this.url) return;
    ...
    aws.sqsReceiveMessage(url, this.options, (err, items) => {
        ...
        var mtimer = this.metrics.start(items.length);
...
SQSClient.prototype.publishQueue = function(job, options, callback)
{
...
SQSClient.prototype.unpublishQueue = function(msg, options, callback)
{
