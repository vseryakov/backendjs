...
api.parseSignature = function(req)
{
    ...
    if (!rc.signature && !this.noAccessToken) {
...
api.checkRequestSignature = function(req, callback)
{
    ...
    if (sig.expires < now - this.signatureAge) {
        var path = sig.path || "";
        if (path) path = "[" + path.substr(0, 6).replace(/[\/]/g,"") + "..." + path.slice(-3) + "]";
        var msg = req.__("Expired request %s, check your clock, the server time is %s, your clock is %s",
                         path,
...
api.checkAccountSignature = function(req, sig, account, callback)
{
    ...
    if (account.acl_deny && sig.url.match(account.acl_deny)) {
        return callback({ status: 403, message: "Access denied", code: "ACL" });
    }
    if (account.acl_allow && !sig.url.match(account.acl_allow)) {
        ...
        return callback({ status: 403, message: "Not permitted", code: "ACL" });
    }
