...
api.checkAuthorization = function(req, status, callback)
{
    ...
    if (!this.checkAccountType(req.account, "admin")) {
        ...
        if (req.account.id &&
            (this.denyAuthenticated.rx && this.denyAuthenticated.rx.test(req.options.path) ||
             this.denyIncludeAuthenticated.some((x) => (api.denyAccount[x].rx && api.denyAccount[x].rx.test(req.options.path))))) {
            logger.debug("checkAuthorization:", "denyAuthenticated:", req.account.id, req.account.name, req.account.type, req.options.path, this.denyAuthenticated.list, this.denyIncludeAuthenticated);
        ...
        for (const i in req.account.type) {
            ...
            if ((this.denyAccount[p] && this.denyAccount[p].rx && this.denyAccount[p].rx.test(req.options.path)) ||
                (this.denyInclude[p] && this.denyInclude[p].some((x) => (api.denyAccount[x].rx && api.denyAccount[x].rx.test(req.options.path))))) {
                logger.debug("checkAuthorization:", "denyAccount:", req.account.id, req.account.name, req.account.type, req.options.path, this.denyAccount[p].rx && this.denyAccount[p].list, this.denyInclude[p]);
                return this.checkAuthHooks(req, { status: 401, message: "Access denied", code: "DENY" }, callback);
            }
        }
        // Authenticated only below
        if (req.account.id &&
            (this.allowAuthenticated.rx && this.allowAuthenticated.rx.test(req.options.path) ||
             this.allowIncludeAuthenticated.some((x) => (api.allowAccount[x].rx && api.allowAccount[x].rx.test(req.options.path))))) {
            logger.debug("checkAuthorization:", "allowAuthenticated:", req.account.id, req.account.name, req.account.type, req.options.path, this.allowAuthenticated.list, this.allowIncludeAuthenticated);
            ...
            for (const p in this.allowAccount) {
                ...
                matched.push(p);
                if (lib.isFlag(req.account.type, p) || lib.isFlag(req.account.type, this.allowInclude[p])) {
                    allow.push(p);
                    break;
                }
            }
