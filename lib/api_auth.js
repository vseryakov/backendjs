    ...
    if (!this.checkAccountType(req.account, "admin")) {
        ...
        if (req.account.id &&
            (this.denyAuthenticated.rx && this.denyAuthenticated.rx.test(req.options.path) ||
             this.denyAclAuthenticated.some((x) => (api.acl[x].rx && api.acl[x].rx.test(req.options.path))))) {
        ...
        for (const i in req.account.type) {
            ...
            if ((this.denyAccount[p] && this.denyAccount[p].rx && this.denyAccount[p].rx.test(req.options.path)) ||
                (this.denyAcl[p] && this.denyAcl[p].some((x) => (api.acl[x].rx && api.acl[x].rx.test(req.options.path))))) {
                logger.debug("checkAuthorization:", "denyAccount:", req.account.id, req.account.name, req.account.type, req.options.path, this.denyAccount[p].rx && this.denyAccount[p].list, this.denyAcl[p]);
                return this.checkAuthHooks(req, { status: 401, message: "Access denied", code: "DENY" }, callback);
            }
        }
        ...
        if (req.account.id &&
            (this.allowAuthenticated.rx && this.allowAuthenticated.rx.test(req.options.path) ||
             this.allowAclAuthenticated.some((x) => (api.acl[x].rx && api.acl[x].rx.test(req.options.path))))) {
