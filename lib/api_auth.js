...
api.checkAccess = function(req, callback)
{
    ...
    if (this.checkAcl(this.deny, this.denyAcl, req.options)) return callback({ status: 403, message: "Access denied" });
...
api.checkAuthorization = function(req, status, callback)
{
    ...
    if (req.signature.source == "s" && this.checkAcl(this.disableSession, this.disableSessionAcl, req.options)) {
