...
api.checkAclOnly = function(req)
{
    for (const p in this.onlyAccount) {
        ...
        logger.debug("checkAuthorization:", "onlyAccount:", req.account.id, req.account.name, req.account.type, req.options.path, this.onlyAccount[p].list);
    ...
    for (const p in this.onlyAcl) {
        ...
        logger.debug("checkAuthorization:", "onlyAccount:", req.account.id, req.account.name, req.account.type, req.options.path, this.onlyAcl[p]);
        return { status: 401, message: "Access denied", code: "ONLY" };
    }
}
