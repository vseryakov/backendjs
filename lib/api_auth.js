    ...
    if (!this.checkAccountType(req.account, "admin")) {
        ...
        if (!this.allowAuthenticated.rx || !req.account.id || !req.options.path.match(this.allowAuthenticated.rx)) {
            ...
            for (var p in this.allowAccount) {
                if (this.allowAccount[p].rx && req.options.path.match(this.allowAccount[p].rx)) {
                    if (this.checkAccountType(req.account, p)) allow++;
                    match++;
                }
            }
            if (match && !allow) {
                logger.debug("checkAuthorization:", "allowAccount:", req.account.id, req.account.alias, req.account.type, req.options.path, "rc:", match, allow);
