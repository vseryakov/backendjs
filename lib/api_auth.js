...
api.handleSignature = function(req, res, next)
{
    ...
    self.checkAccess(req, function(status) {
        ...
        if (status) {
            ...
            if (status.status) self.sendReply(res, status);
            return;
        }
        ...
        self.checkSignature(req, function(status) {
            ...
            self.checkAuthorization(req, status, function(status) {
                if (status && status.status != 200) return self.sendReply(res, status);
                next();
            });
        });
    });
...
api.checkSignature = function(req, callback)
{
    ...
    db.get("bk_auth", { login: sig.login }, function(err, account) {
        ...
        if (account.acl_allow && !sig.url.match(account.acl_allow)) {
            logger.debug("checkSignature:", req.account.id, req.account.alias, sig.url, account.acl_allow);
...
api.checkAuthorization = function(req, status, callback)
{
    ...
    if (self.disableSession.rx) {
        if (req.signature.source == "s" && req.options.path.match(self.disableSession.rx)) {
            logger.debug("checkAuthorization:", "disableSession:", req.account.id, req.account.alias, req.account.type, req.options.path, self.disableSession.rx);
    ...
    if (!self.checkAccountType(req.account, "admin")) {
        ...
        if (self.allowAdmin.rx) {
            if (req.options.path.match(self.allowAdmin.rx)) {
                logger.debug("checkAuthorization:", "allowAdmin:", req.account.id, req.account.alias, req.account.type, req.options.path, self.allowAdmin.rx);
                return callback({ status: 401, message: "Restricted access" });
            }
        }
        // Authenticated only
        if (!self.allowAuthenticated.rx || !req.account.id || !req.options.path.match(self.allowAuthenticated.rx)) {
            // Specific account type only
            for (var p in self.allowAccount) {
                if (self.allowAccount[p].rx) {
                    if (req.options.path.match(self.allowAccount[p].rx)) {
                        if (self.checkAccountType(req.account, p)) break;
                        logger.debug("checkAuthorization:", "allowAccount:", p, req.account.id, req.account.alias, req.account.type, req.options.path, self.allowAccount[p].rx);
