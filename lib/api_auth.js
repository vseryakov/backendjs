...
api.checkAuthorization = function(req, status, callback)
{
    ...
    if (self.disableSession.rx) {
        if (req.signature.source == "s" && req.options.path.match(self.disableSession.rx)) {
            logger.debug("checkAuthorization:", "disableSession:", req.account.id, req.account.alias, req.account.type, req.options.path, self.disableSession[p].rx);
    ...
    if (!self.checkAccountType(req.account, "admin")) {
        ...
        if (self.allowAdmin.rx) {
            if (req.options.path.match(self.allowAdmin.rx)) {
                logger.debug("checkAuthorization:", "allowAdmin:", req.account.id, req.account.alias, req.account.type, req.options.path, self.allowAdmin[p].rx);
