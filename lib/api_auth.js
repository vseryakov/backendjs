...
api.handleSignature = function(req, res, next)
{
    this.checkAccess(req, function afterAccess(status) {
        ...
        api.checkRequestSignature(req, function afterSig(status) {
            if (status && status.status != 200) logger.debug('handleSignature:', status, lib.objDescr(req.signature), 'hdrs:', req.headers, 'session:', lib.stringify(req.session));
...
{
    if (!sig) return;
    var d = String(sig.signature).match(this.rxSignature);
    if (!d) return;
