...
api.checkRequestSignature = function(req, callback)
{
    ...
    if (!callback) callback = lib.noop;

    // Extract all signature components from the request
    var sig = this.parseSignature(req);

    if (logger.level >= logger.DEBUG) logger.debug('checkRequestSignature:', sig, 'hdrs:', req.headers, 'session:', JSON.stringify(req.session));
    ...
    if (sig.expires < now - this.signatureAge) {
        var msg = "Expired request, check your clock, the server time is " + lib.strftime(now, "%m/%d/%Y %H:%M:%S GMT", 1) + ", your clock is " + lib.strftime(sig.expires, "%m/%d/%Y %H:%M:%S GMT", 1);
...
api.checkAccountSignature = function(req, sig, account, callback)
{
    ...
    if (account.acl_allow && !sig.url.match(account.acl_allow)) {
        logger.debug("checkAccountSignature:", account.id, account.alias, sig.url, account.acl_allow);
    ...
    if (!this.verifySignature(sig, account)) {
...
// Assign an account record to be the current for the given request
api.setCurrentAccount = function(req, account)
{
    if (!account || !account.id) return;
    req.account = account;
    if (!req.options) req.options = {};
    req.options.account = lib.newObj("id", account.id, "login", account.login, "alias", account.alias, "type", account.type);
}
...
api.checkAccess = function(req, callback)
{
    ...
    if (this.allowIp.rx && req.options.ip.match(this.allowIp.rx)) rc = { status: 200, message: "" };
    if (this.allow.rx && req.options.path.match(this.allow.rx)) rc = { status: 200, message: "" };
...
api.checkAuthorization = function(req, status, callback)
{
    ...
    if (this.disableSession.rx) {
        if (req.signature.source == "s" && req.options.path.match(this.disableSession.rx)) {
            logger.debug("checkAuthorization:", "disableSession:", req.account.id, req.account.alias, req.account.type, req.options.path, this.disableSession.rx);
            return callback({ status: 401, message: "Not authorized" });
        }
    }
    ...
    if (!this.checkAccountType(req.account, "admin")) {
        ...
        if (this.allowAdmin.rx) {
            if (req.options.path.match(this.allowAdmin.rx)) {
                logger.debug("checkAuthorization:", "allowAdmin:", req.account.id, req.account.alias, req.account.type, req.options.path, this.allowAdmin.rx);
                return callback({ status: 401, message: "Restricted access" });
            }
        }
        // Authenticated only
        if (!this.allowAuthenticated.rx || !req.account.id || !req.options.path.match(this.allowAuthenticated.rx)) {
            // Specific account type only
            for (var p in this.allowAccount) {
                if (this.allowAccount[p].rx) {
                    if (req.options.path.match(this.allowAccount[p].rx)) {
                        if (this.checkAccountType(req.account, p)) break;
                        logger.debug("checkAuthorization:", "allowAccount:", p, req.account.id, req.account.alias, req.account.type, req.options.path, this.allowAccount[p].rx);
