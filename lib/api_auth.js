...
{
    return lib.testRegexpObj(options.path, rx) || (Array.isArray(acl) && acl.some((x) => (lib.testRegexpObj(options.path, api.acl[x]))));
}
...
{
    // Make sure we will not crash on wrong object
    if (!req || !req.query) req = { query: {}, headers: {}, options: {} };

    // Required values must be present and not empty
    if (!req.query.login || !req.query.secret) {
        return callback({ status: 417, message: this.checkErrmsg(req, null, "No username or password provided"), code: "NOLOGIN" });
    }
