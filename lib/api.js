...
//
// The `pub_enc` property makes the column to be returned in the encrypted form using the api query token secret
//
...
api.checkResultColumns = function(table, data, options)
{
    ...
    for (let i = 0; i < tables.length; i++) {
        ...
        for (const p in dbcols) {
            ...
            cols[p] = typeof rules[p] != "undefined" ? rules[p] :
                      c ? (c.pub ? 1 : c.pub_max > 1 ? c.pub_max : c.pub_enc ? 999 : c.secure ? -1 : c.admin || c.admins ? admin : 0) : 0;
        }
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        for (const p in row) {
            ...
            if (!owner && cols[p] <= 0) delete row[p];
            if (cols[p] == 999) {
                row[p] = "$" + lib.encrypt(this.queryTokenSecret, String(row[p]));
            } else
            if (cols[p] > 1) {
                c = String(row[p]);
                row[p] = c.substr(0, cols[p]) + "X".repeat(c.length - cols[p]);
            }
        }
    }
    return data;
