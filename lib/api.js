...
const api = {
    ...
    args: [
        ...
        { name: "rlimits-map-(.+)", type: "map", obj: "rlimitsMap.$1", make: "$1", maptype: "auto", merge: 1, descr: "Rate limiter parameters for Token Bucket algorithm. set all at once, ex. -api-rlimits-map-/url=rate:1,interval:2000 -api-rlimits-map-GET/url=rate:10" },
...
api.startServerRequest = function(req, res, next)
{
    ...
    if (api.maxRequestQueue && api.metrics.Counter("api_nreq").toJSON() >= api.maxRequestQueue) {
        api.metrics.Counter('api_full_count').inc();
    ...
    api.checkRateLimits(req, { type: ["ip","path","opath"] }, (err) => {
        if (err) {
            api.metrics.Counter(err.type + '_count').inc();
...
api.handleMetrics = function(req, elapsed)
{
    ...
    this.metrics.Counter("api_req_count").inc();
    this.metrics.Counter("api_" + req.res.statusCode + "_count").inc();

    if (req.res.statusCode >= 400 && req.res.statusCode < 500) {
        this.metrics.Counter("api_bad_count").inc();
    }
    if (req.res.statusCode >= 500) {
        this.metrics.Counter("api_err_count").inc();
    }
}
...
api.handleBody = function(req, res, next)
{
    ...
    if (req.options.clength > 0 && req.options.clength >= this.queryLimit) {
        api.metrics.Counter('api_large_count').inc();
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        var map = this.rlimitsMap[req.method + key] || this.rlimitsMap[key];
