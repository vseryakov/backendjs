...
const api = {
    ...
    args: [
        ...
        { name: "rlimits-map-(.+)", type: "map", obj: "rlimitsMap", make: "$1", maptype: "int", descr: "Rate limiter parameters for Token Bucket algorithm. set all at once, ex. -api-rlimits-map-/url=rate:1,interval:2000" },
...
//
// NOTE: SSL redirection rules use the original URL but the general redirect rules use the final url which may not be the original due to routing
//
api.checkRedirect = function(req, options)
{
    var url = req.signatureUrl || req.url, location = options.host + url;
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        ipc.limiter(limit, (delay, info) => {
            ...
            callback({ status: 429, message: lib.__(options.message || api.rlimits.message, lib.toDuration(delay)) }, info);
        });
    }, callback, true);
}
