...
var api = {
    ...
    tables: {
        ...
        bk_auth: { login: { primary: 1 },                              // Account login
                   id: { type: "text" },                               // Auto generated UUID to be linked with other records
                   alias: { type: "text" },                            // Account alias
    ...
    if (util.isError(status)) {
        ...
        if (status.message && !String(status.message).match(this.errlogLimiterIgnore)) {
            if (!this.errlogLimiterToken || this.errlogLimiterToken.consume(1)) {
                logger.error("sendReply:", res.req.url, status.message, res.req.headers, lib.traceError(status));
            }
        }
