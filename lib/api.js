...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        default:
            rate = options.rate || this.rlimitsRate[options.type] || 0;
            if (!rate) return next();
            max = options.max || this.rlimitsMax[options.type] || rate;
            interval = options.interval || this.rlimitsInterval[options.type] || this.rlimits.interval || 1000;
            key = 'TB' + type;
        }
