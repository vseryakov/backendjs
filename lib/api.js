...
const api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "restart", descr: "On address in use error condition restart the specified server" },
    ...
    tables: {
        ...
        bk_auth: {
            ...
            type: { type: "text", list: 1, lower: 1, admin: 1 },    // Account roles: admin, ....
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        case "id":
            ...
            rate = options.rate || req.account.rlimits_rate || this.rlimitsRate.id || 0;
            if (!rate) return next();
            max = options.max || req.account.rlimits_max || this.rlimitsMax.id || rate;
            interval = options.interval || req.account.rlimits_interval || this.rlimitsInterval.id || 1000;
            key = 'TBid:' + req.account.id;
            break;
...
api.registerRateLimits = function(name, rate, max, interval, queue)
{
    ...
    if (queue) this.rlimitsQueue[name] = queue;
    return true;
}
...
api.checkResultColumns = function(table, data, options)
{
    ...
    var staff = options.staff || this.checkAccountType(options.account, "staff") ? 1 : 0;
    ...
    for (let i = 0; i < tables.length; i++) {
        ...
        for (const p in dbcols) {
            ...
            cols[p] = typeof rules[p] != "undefined" ? rules[p] :
                      c ? (c.pub ? 1 :
                           c.secure ? -1 :
                           c.admin || c.admins ? admin :
                           c.staff ? staff :
                           c.pub_max > 1 ? c.pub_max :
                           c.pub_enc ? 9999 : 0) : 0;
        }
    }
...
api.addHook = function(type, method, path, callback)
{
    ...
    if (hooks.some(function(x) { return x.method == method && String(x.path) === String(path) && x.callback == callback })) return false;
