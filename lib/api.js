...
module.exports = {
    ...
    args: [
        ...
        { name: "context-options", type: "map", obj: "context-options", merge: 1, descr: "Options for every request context", example: "trustProxy:false, noHashSecret:true" },
        { name: "defaults-([a-z0-9_]+)-(.+)", obj: "defaults.$2", make: "$1", auto_type: 1, descr: "Global body limits for `api.validate`, format is: api-defaults-LIMIT-NAME, where LIMIT is an property that performs limiting like max, maxlist, min, required.., NAME is a schema property, it can be path specific", example: "# Limit all names length up to 128 chars\napi-defaults-max-name = 128\n# Limit groups list size for /endpoint to 255\napi-defaults-maxlist-/endpoint-groups = 255" },
        { name: "restart-hours", type: "list", data_type: "int", descr: "List of hours when to restart api workers, only done once for each hour" },
    ...
    /** @var {int} port - HTTP port to listen to for web server,
      * @default 8000
      */
    ...
    /** @var {object} - options to apply for every new request context
     * @default
     */
    contextOptions: {
        trustProxy: true,
    ...
    /**
     * @var {object} - used by {@link module:api.validate} global validation defaults, hardcoded values are set below:
     * @property {string} ".*" - matches all fields of all types, maxlist: 512
     * @property {string} "*.json" - matches all json fields, max: 4000
     * @property {string} "*.string" - matches all string fields, max: 512
     * @property {string} "*.text" - matches all text fields, max: 512
     * @property {string} "*.object" - matches all object fields, max: 32000
     * @property {string} "*.array" - matches all array objects, max: 32000
     */
    defaults: {
        "*": {
            maxlist: 512,
...
api.init = function(options, callback)
{
    ...
    if (app.isWorker) {
        ...
        api._restartInterval = setInterval(() => {
            if (!lib.includes(api.restartHours, new Date().getHours())) return;
...
api.shutdown = function(options, callback)
{
    ...
    _exiting = true;
    logger.debug("shutdown:", app.name, app.role, options);

    clearInterval(api._restartInterval);
    api._restartInterval = undefined;
    api.metrics.req.end();
    ...
    app.runMethods("shutdownServer", options, { direct: 1, parallel: 1 }, () => {
        api.app = undefined;
        _exiting = false;
...
api.shutdownServer = function(options, callback)
{
    lib.forEvery([ "server", "sslServer" ], (name, next) => {
        ...
        logger.log("shutdownServer:", api.name, app.role, name, "closed", options, err);
        next();
    }, callback, true);
}
...
{
    if (!api.app) return;
    const m = metrics.toJSON(api.metrics, { reset: 1, take: /_count$/ });
