...
var api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "proxy-url-(.+)", type: "regexpobj", reverse: 1, obj: 'proxy-url', lower: /.+/, descr: "URL regexp to be passed to other web server running behind, each parameter defines an url regexp and the destination in the value in the form http://host[:port], example: -server-proxy-url-^/api http://127.0.0.1:8080" },
           { name: "proxy-host-(.+)", type: "regexpobj", reverse: 1, obj: 'proxy-host', lower: /.+/, descr: "Virtual host mapping, to match any Host: header, each parameter defines a host name and the destination in the value in the form http://host[:port], example: -server-proxy-host-www.myhost.com http://127.0.0.1:8080" },
    ],
...
api.init = function(options, callback)
{
    ...
    this.app.use(function apiLimits(req, res, next) {
        // Latency watcher
        if (self.maxLatency && lib.busyTimer("busy")) {
    ...
    var corsHeaders = ['content-type', self.signatureHeaderName, self.appHeaderName, self.versionHeaderName, self.langHeaderName, self.tzHeaderName].join(", ");
    var corsMethods = self.corsMethods.join(", ");
    this.app.use(function(req, res, next) {
        ...
        res.header('Access-Control-Allow-Credentials', self.corsCredentials);
        if (logger.level >= logger.DEBUG) logger.debug('handleServerRequest:', core.port, req.options.ip, req.connection.remoteAddress, req.method, req.options.path, req.get('content-type'), req.get(self.appHeaderName), req.get(self.signatureHeaderName), req.get(self.tzHeaderName), req.get("x-forwarded-for"));
    ...
    if (!this.noAccessLog) {
        ...
        this.app.use(function apiLogger(req, res, next) {
            ...
            res.end = function(chunk, encoding) {
                ...
                for (var i in self.accessLogFields) {
                    var v = self.accessLogFields[i];
                    switch (v[1] == ":" ? v[0] : "") {
                    case "q":
    ...
    core.runMethods("configureMiddleware", options, function() {
        ...
        core.runMethods("configureWeb", options, function(err) {
            ...
            if (!self.noStatic) {
                for (var p in self.vhostPath) {
                    self.app.use(function(rx, handler) {
                        return function vhost(req, res, next) {
                            if (!rx.test(req.options.host)) return next()
                            handler(req, res, next);
                          }
                    }(self.vhostPath[p], serveStatic(path.join(core.path.web, p), self.staticOptions)));
                }
            ...
            if (core.ws.port) {
                ...
                if (server) {
                    var ws = require("ws");
                    opts = { server: server, verifyClient: function(data, callback) { self.checkWebSocketRequest(data, callback); } };
...
api.checkBody = function(req, res, next)
{
    ...
    form.on('progress', function(bytesReceived, bytesExpected) {
        if (bytesExpected > self.uploadLimit) {
...
api.getOptions = function(req, controls)
{
    ...
    var params = lib.toParams(req.query, controls ? lib.mergeObj(this.controls, controls) : this.controls, opts);
    ...
    if (util.isError(status)) {
        ...
        if (status.message && !String(status.message).match(this.errlogLimiterIgnore)) {
            if (!this.errlogLimiterToken || this.errlogLimiterToken.consume(1)) {
                logger.error("sendReply:", res.req.url, status.message, res.req.headers, lib.objDescr(res.req.query), lib.traceError(status));
            }
        }
        text = status._msg ? status._msg : this.errorMessage || res.__("Internal error occured, please try again later");
...
api.sendFile = function(req, res, file, redirect)
{
    fs.exists(file, function(yes) {
        if (req.method == 'HEAD') return res.sendStatus(yes ? 200 : 404);
