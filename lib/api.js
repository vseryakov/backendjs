...
const api = {
    ...
    args: [
        ...
        { name: "rlimits-(rate|max|interval|ttl|ip|delay|multiplier|queue)-(.+)", autotype: 1, obj: "rlimitsMap.$2", make: "$1", descr: "Rate limiter parameters for Token Bucket algorithm. `queue` to use specific queue, ttl` is to expire cache entries, `ip` is to limit by IP address as well, ex. -api-rlimits-ip-ip=10, -api-rlimits-rate-/path=1" },
        { name: "rlimits-map-(.+)", type: "map", obj: "rlimitsMap.$1", make: "$1", maptype: "auto", merge: 1, descr: "Rate limiter parameters for Token Bucket algorithm. set all at once, ex. -api-rlimits-map-/url=rate:1,interval:2000" },
        ...
        { name: "reset-acl", type: "callback", callback: function(v) { if (v) this.resetAcl() }, descr: "Reset all ACL, routing and login related rules and permissions" },
