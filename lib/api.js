...
//
// `defaults` is an object with runtime data or different defaults for the parameters.
//
//         api.getOptions(req, null, { count: { min: 5, max: 100 } })
//
//
api.getOptions = function(req, controls, defaults)
{
    var opts = { prefix: "_", data: { token: { secret: this.getTokenSecret(req) } } };
    for (var p in defaults) opts.data[p] = defaults[p];
    ...
    if (controls) {
        ...
        for (var p in params) {
            if (!this.controls[p] || !this.controls[p].ignore) req.options[p] = params[p];
        }
    }
    return req.options;
}
...
api.sendReply = function(res, status, text)
{
    if (util.isError(status)) {
        ...
        text = status._msg ? status._msg : this.errorMessage || res.__("Internal error occured, please try later");
