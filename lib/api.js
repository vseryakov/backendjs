...
api.init = function(options, callback)
{
    ...
    this.app.use(function apiLoginLimits(req, res, next) {
...
api.handleResponseHeaders = function(req, res, next)
{
    ...
    for (const i in api.responseHeaders) {
        ...
        if (lib.isArray(rule.value) && lib.testRegexpObj(req.options.path, rule) || lib.testRegexpObj(location, rule)) {
            for (let j = 0; j < rule.value.length - 1; j += 2) res.setHeader(rule.value[j], rule.value[j + 1]);
        }
    }
...
//
// When used for accounts, it is possible to override rate limits for each account in the `bk_user` table by setting `rlimit_max` and `rlimit_rate`
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        var rate, max, interval, key;
        switch (type) {
        case "ip":
            rate = options.rate || this.rlimitsRate.ip || 0;
            if (!rate) return next();
            max = options.max || this.rlimitsMax.ip || rate;
            interval = options.interval || this.rlimitsInterval.ip || this.rlimits.interval || 1000;
        ...
        case 'opath':
            ...
            rate = options.rate || this.rlimitsRate[path] || 0;
            if (!rate) return next();
            max = options.max || this.rlimitsMax[path] || rate;
            interval = options.interval || this.rlimitsInterval[path] || 1000;
            key = 'TBreq:' + (options.ip || req.options.ip) + path;
            break;

        case "login":
            rate = options.rate || this.rlimitsRate.login || 0;
            if (!rate) return next();
            var sig = this.getSignature(req);
            if (!sig || !sig.login) return next();
            max = options.max || this.rlimitsMax.login || rate;
            interval = options.interval || this.rlimitsInterval.login || this.rlimits.interval || 1000;
            key = 'TBlogin:' + sig.login;
            break;

        default:
            rate = options.rate || this.rlimitsRate[type] || 0;
            if (!rate) return next();
            max = options.max || this.rlimitsMax[type] || rate;
            interval = options.interval || this.rlimitsInterval[type] || this.rlimits.interval || 1000;
            key = 'TB' + type;
        }
        ...
        var limit = { name: key, rate: rate, max: max, interval: interval, queueName: options.queue || this.rlimitsQueue[type] || this.limiterQueue };
        ipc.limiter(limit, function(delay, info) {
            if (!delay) return next();
            logger.debug("checkRateLimits:", options, limit, info);
            callback({ status: 429, message: options.message || lib.__(api.rlimitsMessage) });
        });
    }, callback);
}
...
api.sendJSON = function(req, err, rows)
{
    ...
    lib.forEachSeries(hooks, function(hook, next) {
        ...
        if (req.options.cleanup) {
            api.checkResultColumns(req.options.cleanup, rows.count && rows.data ? rows.data : rows, req.options);
        }
...
api.handleWebSocketUpgrade = function(req, socket, head)
{
    ...
    // Prepare request/response for signas\ture verification, have to similate Express middleware flow
    Object.setPrototypeOf(req, this.app.request);
    req.body = req.query = qs.parse(url.parse(req.url).query);
...
api.handleWebSocketConnect = function(ws, req)
{
    ...
    ws.on("message", api.handleWebSocketRequest.bind(api, ws));

    api.wsServer.connections[ws.wsid] = ws;
...
api.handleWebSocketRequest = function(ws, data)
{
    ...
    if (data[0] == "/") {
        req.url = data;
        req.body = req.query = qs.parse(url.parse(data).query);
    } else
    if (data[0] == "{" || data[0] == "[") {
...
{
    if (!api.wsServer) return;
    var ws = api.wsServer.connections[wsid];
    if (!ws) return;
    if (typeof msg != "string") msg = lib.stringify(msg);
    try { ws.send(msg) } catch (e) { logger.error("wsSend:", ws.wsid, ws.account.id, e, msg) }
}
...
// - query - an object to be used for condition, the original websocket query during the upgrade will be compared to this condition
api.wsBroadcast = function(options, msg)
{
    ...
    if (typeof msg != "string") msg = lib.stringify(msg);
    api.wsServer.clients.forEach((ws) => {
        if (options.account_id == ws.account.id ||
            lib.isFlag(options.account_id, ws.account.id) ||
            options.wsid == ws.wsid ||
            lib.isFlag(options.wsid, ws.wsid) ||
            (!lib.isEmpty(options.query) && lib.isMatched(ws.query, options.query))) {
            logger.debug("wsBroadcast:", "send:", ws.wsid, ws.account.id, msg);
            try { ws.send(msg) } catch (e) { logger.error("wsBroadcast:", ws.wsid, ws.path, ws.account.id, e) }
        }
    });
}
