...
api.handleServerRequest = function(req, res)
{
    ...
    var d = domain.create();
    d.on('error', (err) => {
        logger.error('handleServerRequest:', api.port, req.url, lib.traceError(err));
...
api.handleBody = function(req, res, next)
{
    ...
    if (req.options.clength > 0 && req.options.clength >= this.queryLimit) {
        ...
        logger.debug("handleBody:", "too large:", req.url, req.headers);
    ...
    req.on('data', (chunk) => {
        ...
        if (size > api.queryLimit) {
            this.metrics.large_count++;
            logger.debug("handleBody:", "too large:", req.url, req.headers, buf);
...
api.handleMultipart = function(req, res, next)
{
    ...
    form.parse(req, (err) => {
        logger.debug("handleMultipart:", err, req.url, req.headers, data, Object.keys(files));
        if (err) {
