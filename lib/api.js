...
const api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "query-defaults-([a-z0-9]+)-(.+)", obj: "queryDefaults.$2", make: "$1", autotype: 1, descr: "Global query defaults for getQuery" },
           ...
           { name: "csrf-skip-methods", type: "list", descr: "Skip CSRf checks for specified methods" },
    ...
    csrfSetPath: {},
    csrfCheckPath: {},
    csrfSkipMethods: ["GET", "HEAD", "OPTIONS", "TRACE" ],
    ...
    controls: {
        ...
        apiVersion: { ignore: 1 },
        noscan: { ignore: 1 },
        // Supported params from query
        total: { type: "bool" },
        session: { type: "bool" },
        format: { type: "string", max: 32 },
        separator: { type: "string", max: 32 },
        encoding: { type: "string", max: 32 },
        ...
        tm: { type: "timestamp" },
        ext: { type: "string", max: 32 },
        ops: { type: "map", max: 128, maxlist: 32 },
        start: { type: "token", max: 1024 },
        token: { type: "token", max: 1024 },
        select: { type: "list", max: 64, maxlist: 32 },
        desc: { type: "bool" },
        sort: { type: "string", max: 128 },
        join: { type: "string", max: 32 },
        joinOps: { type: "map", max: 128, maxlist: 32 },
    },
