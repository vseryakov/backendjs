...
const api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "access-log-level", descr: "Syslog level in the format: priority.facility" },
           ...
           { name: "rlimits-rate-(.+)", type: "int", obj: "rlimits-rate", descr: "Set fill/normal rate limit by the given property, it is used by the request rate limiter using Token Bucket algorithm. Predefined types: ip, path, opath, gpath, gopath" },
           { name: "rlimits-ttl-(.+)", type: "int", obj: "rlimits-ttl", descr: "Set expiration TTL by the given property, 0 to keep in the cache forever, it is used by the request rate limiter using Token Bucket algorithm. Predefined types: ip, path, opath, gpath, gopath" },
           { name: "rlimits-interval-(.+)", type: "int", obj: "rlimits-interval", descr: "Set rate interval in ms by the given property, it is used by the request rate limiter using Token Bucket algorithm. Predefined types: ip, path, opath, gpath, gopath" },
...
api.configureAccessLog = function()
{
    if (logger.syslog) {
        ...
        this.accessLog.write = function(data) { logger.printSyslog(api.accessLogLevel, data); return true; };
    } else
...
api.startServerRequest = function(req, res, next)
{
    ...
    api.checkRateLimits(req, { type: ["ip","path","opath","gpath","gopath"] }, (err) => {
        if (!err) return next();
...
api.checkQuery = function(req, res, next)
{
    ...
    req.on('data', function(chunk) {
        ...
        if (size > api.uploadLimit) {
            if (size > api.uploadLimit * 2) {
                return next(lib.newError({ message: "too large", _msg: "Unable to process the request, it is too large", status: 413, maxsize: api.uploadLimit, length: clen }));
            }
            buf = null;
        } else {
            buf += chunk;
    ...
    req.on('end', function() {
        try {
            if (size > api.uploadLimit) {
                return next(lib.newError({ message: "too large", _msg: "Unable process the request, it is too large", status: 413, maxsize: api.uploadLimit, length: size }));
            }
...
api.checkBody = function(req, res, next)
{
    ...
    var form = formidable({ uploadDir: core.path.tmp, keepExtensions: true, maxFileSize: api.uploadLimit });
    var data = {}, files = {}, done;
...
//     - path - limit number of requests per configured interval for an API path and IP address, must be configured like: `-api-rlimits-rate-/api/path=2`
//     - gpath - limit globaly by request path, * can be used at the end to match only the beginning, `-api-rlimits-rate-/api/path/*=2`
//
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        case 'gopath':
            type = type.slice(1);
            key = name = options[type] || req.options && req.options[type];
            if (!key || this.rlimitsRate[key]) break;
            for (const p in this.rlimitsRate) {
