...
api.shutdown = function(callback)
{
    ...
    this.exiting = true;
    logger.log("api.shutdown:", app.role, "started");
...
api.shutdownServer = function(options, callback)
{
    lib.forEach([ api.server, api.sslServer ], (server, next) => {
        ...
        logger.log("shutdownServer:", server?.serverName, app.role, "closed", err);
        next();
    }, callback, true);
}
...
api.configureServers = function(options, callback)
{
    ...
    if (this.ssl.port && (this.ssl.key || this.ssl.pfx)) {
        api.sslServer = this.createServer({
            name: "https",
            ssl: this.ssl,
            port: this.ssl.port,
            bind: this.ssl.bind,
    ...
    app.runMethods("configureServer", options, { direct: 1 }, callback);
}
...
api.handleResponseHeaders = function(req, res, next)
{
    ...
    logger.debug('handleResponseHeaders:', api.port, req.method, req.connection.remoteAddress, req.options, req.headers);
 ...
 *
 * - `options.defaults` - merged with global `queryDefaults`
 * - `options.query` - use only `req.query`
 *
...
api.cleanupHeaders = function(headers)
{
    ...
    if (headers.cookie) {
        headers.cookie = cookie.parse(headers.cookie);
    }
    if (headers["set-cookie"]) {
        headers["set-cookie"] = cookie.parse(headers["set-cookie"]);
    }
    return headers;
}
...
api.createServer = function(options, callback)
{
    ...
    server.on('error', function(err) {
        ...
        if (err.code == 'EADDRINUSE' && options.restart) {
            app.killBackend(options.restart, "SIGKILL", function() { process.exit(0) });
        }
    });

    try { server.listen(options.port, options.bind, options.backlog); } catch (e) { logger.error('server: listen:', options, e); server = null; }
