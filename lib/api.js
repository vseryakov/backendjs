...
{
    if (!req || !api.wsServer) return;
    var ws = api.wsServer.connections[req.wsid];
    if (!ws) return;
    switch (type) {
    case "query":
    case "account":
        if (lib.isObject(value) && !lib.isEmpty(value)) ws.query = value;
        break;
    }
}
...
api.wsBroadcast = function(options, msg)
{
    ...
    if (typeof msg != "string") msg = lib.stringify(msg);
    api.wsServer.clients.forEach((ws) => {
        if (options.wsid == ws.wsid ||
            options.account_id == ws.account.id ||
            lib.testRegexp(ws.path, options.path) ||
            lib.isFlag(options.account_id, ws.account.id) ||
            lib.isMatched(ws.account, options.account) ||
            lib.isFlag(options.wsid, ws.wsid) ||
            lib.isMatched(ws.query, options.query)) {
            logger.debug("wsBroadcast:", "send:", ws.wsid, ws.path, ws.account.id, msg);
