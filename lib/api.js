...
const api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "subscribe-queue", descr: "Default queue to use for client subscriptions" },
    ...
    core.runMethods("configureMiddleware", options, function() {
        ...
        core.runMethods("configureWeb", options, function(err) {
            ...
            if (core.ws.port) {
                ...
                var opts = { ssl: core.ws.ssl ? core.ssl : null, port: core.ws.port, bind: core.ws.bind, restart: api.restart };
                if (!server) server = core.createServer(opts, function(req, res) { res.status(200).send("OK"); });
                if (server) {
                    var ws = require("ws");
                    opts = { server: server, verifyClient: api.checkWebSocketRequest.bind(api) };
...
api.subscribe = function(req, options)
{
    ...
    req.msgQueue = options.queueName || this.subscribeQueue;

    ipc.subscribe(req.msgKey, { queueName: req.msgQueue }, function(msg, cb) {
        api.sendEvent(req, msg, cb);
    });
...
api.unsubscribe = function(req)
{
    if (req && req.msgKey) {
        logger.debug('unsubscribe:', req.msgKey);
        ipc.unsubscribe(req.msgKey, { queueName: req.msgQueue });
    }
}
...
api.sendEvent = function(req, data, next)
{
    logger.debug('sendEvent:', req.msgQueue, 'key:', req.msgKey, 'data:', data, 'sent:', req.res.headersSent, 'match:', req.msgMatch, 'timeout:', req.msgTimeout);
    // If for any reason the response has been sent we just bail out
    if (req.res.headersSent) {
        api.unsubscribe(req);
        return lib.tryCall(next);
    }
    if (!req.msgData) req.msgData = [];
    req.msgData.push(data);
    if (req.msgTimeout) clearTimeout(req.msgTimeout);
    if (!req.msgInterval) {
        req.res.json(req.msgData);
        if (!req.httpProtocol) api.unsubscribe(req);
    } else {
        req.msgTimeout = setTimeout(function() {
            if (!req.res.headersSent) req.res.json(req.msgData);
            if (!req.httpProtocol) api.unsubscribe(req);
        }, req.msgInterval);
    }
    lib.tryCall(next);
