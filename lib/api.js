...
api.init = function(options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            core.runMethods("configureMiddleware", options, { allow: api.allowConfigureMiddleware }, next);
        },
        ...
        function(next) {
            ...
            core.runMethods("configureWeb", options, { allow: api.allowConfigureWeb }, next);
        },
        ...
        function(next) {
            ...
            api.app.use(function apiEmpty(req, res, next) {
                ...
                res.set("Content-Type", core.mime.lookup(req.options.path)).status(200).send();
            });
...
api.configureStatic = function(options)
{
    ...
    core.runMethods("configureStaticWeb", options, { allow: api.allowConfigureMiddleware }, () => {
...
api.sendJSON = function(req, err, data)
{
    ...
    lib.forEachSeries(hooks, (hook, next) => {
        ...
        if (req.options.cleanup) {
            api.cleanupResult(req.options.cleanup, typeof data.count == "number" && Array.isArray(data.data) ? data.data : data, req.options);
        }
...
api.sendFormatted = function(req, err, data, options)
{
    ...
    case "xml":
        if (req.options.cleanup) this.cleanupResult(req.options.cleanup, typeof data.count == "number" && Array.isArray(data.data) ? data.data : data, req.options);
        var xml = "<data>\n";
    ...
    if (util.types.isNativeError(status)) {
        ...
        if (status.message && !this.errlogLimiterIgnore?.rx.test(status.message)) {
            if (!this.errlogLimiterToken || this.errlogLimiterToken.consume(1)) {
                logger.error("sendReply:", res.req.url, status.message, api.cleanupHeaders(res.req.headers), res.req.query, res.req.options, lib.traceError(status));
            }
        }
