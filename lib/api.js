        ...
        api.app.use(function apiStatic(req, res, next) {
            ...
            for (const p in api.compressed) {
                if (lib.testRegexp(req.options.path, api.compressed[p])) {
                    req.url = req.options.path + "." + p + req.url.substr(req.options.path.length);
                    res.setHeader("Content-Encoding", p == "br" ? "brotli" : "gzip");
                    res.setHeader("Content-Type", core.mime.getType(req.options.path));
                    logger.debug("compressed:", req.options.path, "rerouting to", req.url);
                    break;
                }
            }
            next();
...
api.checkRouting = function(req, name, ignore)
{
    ...
    for (var p in rules) {
        if (lib.testRegexpObj(req.options.path, rules[p]) || lib.testRegexpObj(location, rules[p])) {
            ...
            req.options.path = this.checkRedirectPlaceholders(req, p);
