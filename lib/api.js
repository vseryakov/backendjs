...
api.checkRouting = function(req)
{
    ...
    for (var p in this.routing) {
        if (this.routing[p].rx && (this.routing[p].rx.test(req.options.path) || this.routing[p].rx.test(location))) {
            ...
            logger.debug("checkRouting:", location, "switch to", p, req.options.path);
            break;
        }
    }
}
