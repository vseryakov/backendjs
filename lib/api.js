...
var api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "routing-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: 'routing', lower: /.+/, descr: "URL path to be re-routed to other path, this is done inside the server, only the path is replaced, same format and placeholders as in redirect-url, example: -api-routing-^/account/get /acount/read" },
...
{
    var pathname = req.path, location = req.hostname + pathname;
    // Change the url into a new one according to the reroute rule
    for (var p in this.routing) {
        if (this.routing[p].rx && (this.routing[p].rx.test(pathname) || this.routing[p].rx.test(location))) {
            logger.debug("checkRouting:", location, "switch to", p);
            var u = url.parse(req.url);
            u.pathname = this.checkRedirectPlaceholders(req, p);
            u.path = null;
