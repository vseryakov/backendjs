...
const api = {
    ...
    args: [
        ...
        { name: "images-raw", type: "bool", descr: "Return raw urls for the images, requires images-url to be configured. The path will reflect the actual 2 level structure and user id in the image name" },
        ...
        { name: "files-raw", type: "bool", descr: "Return raw urls for the files, requires files-url to be configured. The path will reflect the actual 2 level structure and user id in the file name" },
        ...
        { name: "csrf-set-path", type: "regexpobj", descr: "Regexp for URLs to set CSRF token for all methods, token type(user|pub) is based on the current session" },
        ...
        { name: "trace-options", type: "map", maptype: "auto", descr: "Options for tracing, host where to send if not local, path:regexp for URLs to be traced, interval:Interval in ms how often to trace requests, must be > 0 to enable tracing" },
...
//  - type - predefined: `ip,  path, opath`, determines by which property to perform rate limiting, when using user properties
