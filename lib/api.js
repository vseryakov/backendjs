...
api.wsSet = function(type, req, value)
{
    ...
    case "query":
    case "account":
        if (lib.isObject(value) && !lib.isEmpty(value)) ws[type] = value;
        break;
    }
}
...
api.wsBroadcast = function(options, msg)
{
    ...
    for (const ws of this.wsServer.clients) {
        if (options.wsid == ws.wsid ||
            ...
            if (options.cleanup) {
                const opts = { cleanup_copy: 1, account: ws.account, isInternal: ws.account.isInternal, isAdmin: ws.account.isAdmin, isStaff: ws.account.isStaff };
