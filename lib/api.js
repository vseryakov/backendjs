...
const api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "access-log-level", type: "int", descr: "Syslog level priority, a number, emerg=0, alert=1, err=3, warn=4, notice=5, info=6, debug=7" },
...
api.configureAccessLog = function()
{
    if (logger.syslog) {
        ...
        this.accessLog.write = (data) => { logger.printSyslog(api.accessLogLevel, data); return true; };
    } else
...
//     - path - limit number of requests for an API path, * can be used at the end to match only the beginning
//
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        case 'opath':
            ...
            if (!key || this.rlimitsMap[key]) break;
        ...
        var map = this.rlimitsMap[key] || lib.empty;
        ...
        ipc.limiter(limit, (delay, info) => {
            ...
            callback({ status: 429, message: options.message || lib.__(api.rlimits.message), type: type }, info);
        });
    }, callback, true);
}
