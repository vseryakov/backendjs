...
//      - protocolVersion - app specific version provided in the header
...
var api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "allow-account-([a-z]+)", type: "regexpobj", obj: "allow-account", descr: "Add URLs which can be accessed by specific account type only, can be partial urls or Regexp" },
           ...
           { name: "allow-empty", type: "regexpobj", descr: "Regexp for URLs that should return empty responses if not found, for example return nothing for non-existent javascript files" },
           ...
           { name: "collect-pool", descr: "Database pool where to save collected statistics, make sure to call -api-collect-create before using this" },
           { name: "collect-create-tables", type: "callback", callback(){api.createStatisticsTables()}, descr: "Create statistics tables, it is not created by default with -db-create-tables", pass: 1 },
           ...
           { name: "url-metrics-ignore", type: "regexpobj", descr: "Ignore matched urls from the metrics collection" },
           { name: "url-metrics-max", type: "int", descr: "Max number of path related metrics objects to keep, the most recent or the most popular urls will be kept only" },
           { name: "url-metrics-path-(.+)", type: "int", obj: "url-metrics-path", camel: "-", descr: "Defines the length of an API request path to be stored in the statistics by matching the beginning of the request url, the smallest path wins, example: -api-url-metrics-/img 2 -api-url-metrics-/account/get 3" },
           ...
           { name: "proxy-url-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: 'proxy-url', lower: /.+/, descr: "URL regexp to be passed to other web server running behind, each parameter defines an url regexp and the destination in the value in the form http://host[:port], example: -api-proxy-url-^/api http://127.0.0.1:8080" },
           ...
           { name: "routing-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: 'routing', lower: /.+/, descr: "URL path to be re-routed to other path, this is done inside the server, only the path is checked and replaced, example: -api-routing-^/account/get /acount/read" },
           { name: "login-redirect-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: "login-redirect", lower: /.+/, descr: "Define a location where to redirect if no login is provided and it matches host/path regexp, same placeholders as in redirect-url, example: api-login-redirect-^/admin/=/login.html" },
    ],
    ...
    // This object tells which urls we collect metrics about based on the leading path
    urlMetricsPath: {},
    urlMetricsMax: 25,
    urlMetricsList: [],
    urlMetricsIgnore: {},
        ...
        core.runMethods("configureWeb", options, function(err) {
            ...
            if (!api.noStatic) {
                api.app.use(function apiVHost(req, res, next) {
                    ...
                    if (api.noVhostPath.rx && api.noVhostPath.rx.test(req.options.path)) return next();
            ...
            api.app.use(function apiEmpty(req, res, next) {
                ...
                logger.info("allowEmpty:", req.options.path);
...
api.prepareRequest = function(req)
{
    ...
    var v = req.headers[this.appHeaderName] || uagent;
    ...
    req.options.protocolBuild = req.headers[this.versionHeaderName] || "";
    // Timezone offset from UTC passed by the client, we just keep it, how to use it is up to the application
    req.options.appTimezone = lib.toNumber(req.headers[this.tzHeaderName], { dflt: 0, min: -720, max: 720 }) * 60000;
    // Localization from the supplied language
    req.options.appLocale = req.headers[this.langHeaderName] || (req.headers['accept-language'] || "").toLowerCase().split(/[,;-]/)[0] || "";
...
api.handleMetrics = function(req, elapsed)
{
    ...
    var path;
    if (!this.urlMetricsIgnore.rx || !this.urlMetricsIgnore.rx.test(req.options.path)) {
        for (var p in this.urlMetricsPath) {
            if (req.options.path.substr(0, p.length) == p) {
                var p = "url_" + req.options.apath.slice(0, this.urlMetricsPath[p]).map(function(x) { return x.replace("_", "__")}).join("_");
    ...
    if (req.res.statusCode >= 200 && req.res.statusCode < 300 && path) {
        var m = this.metrics.Counter(path +'_0');
        m.inc();
        if (!m._mtime) this.urlMetricsList.push(path);
        m._mtime = Date.now();
    ...
    if (req.res.statusCode >= 400 && req.res.statusCode < 500) {
        ...
        if (req.res.statusCode != 404 && path) this.metrics.Counter(path +'_bad_0').inc();
    }
    if (req.res.statusCode >= 500) {
        this.metrics.Counter("api_err_0").inc();
        if (path) this.metrics.Counter(path + "_err_0").inc();
    }
}

api.cleanupMetrics = function()
{
    if (this.urlMetricsList.length <= this.urlMetricsMax) return;
    this.urlMetricsList.sort((a,b) => (a[1] - b[1]));
    while (this.urlMetricsList.length > this.urlMetricsMax) {
        const path = this.urlMetricsList.pop()[0];
        logger.debug("cleanupMetrics:", path, this.metrics.toJSON(path));
        this.metrics.destroy(path);
        this.metrics.destroy(path + "_0");
        this.metrics.destroy(path + "_bad_0");
    }
}
...
api.checkQuery = function(req, res, next)
{
    ...
    if (this.mimeIgnore.rx && this.mimeIgnore.rx.test(type)) return next();
...
api.checkRedirectRules = function(req, options, rules)
{
    var location = options.host + req.url;
    rules = this[rules];
    for (var i in rules) {
        let rx = util.isRegExp(rules[i].rx) ? rules[i].rx : util.isRegExp(rules[i]) ? rules[i] : null;
        if (rx && rx.test(location)) {
            let url = typeof i == "string" ? i : rules[i].value || "";
            if (!url) continue;
            url = url.replace(/@(HOST|PATH|URL|BASE|DIR|QUERY)@/g, function(_, m) {
                return m[0] == "H" ? options.host :
                       m[0] == "P" ? options.path :
                       m[0] == "U" ? req.url :
                       m[0] == "Q" ? qs.stringify(req.query) :
                       m[0] == "D" ? path.dirname(options.path) :
                       m[0] == "B" ? path.basename(options.path).split(".").shift() : "";
            });
            logger.debug("checkRedirectRules:", rules, location, options.path, "=>", url, "rule:", i, rules[i]);
...
api.checkAccountType = function(account, type)
{
    ...
    return lib.isFlag(account._types, type);
}
...
//   - `path, apath, ip, host, mtime, cleanup, secure, noscan, appName, appVersion, appPlatform, appLocale, appTimezone, protocolVersion`
//
...
api.getPublicColumns = function(table, options)
{
    ...
    return Object.keys(cols).filter(function(x) {
        ...
        for (var i in disallow) if (cols[x][disallow[i]]) return false;
    ...
    for (var i = 0; i < tables.length; i++) {
        ...
        for (var p in dbcols) {
            ...
            cols[p] = typeof rules[p] != "undefined" ? rules[p] :
                      c ? (c.pub ? 1 : c.secure ? -1 : c.admin || c.admins ? admin : 0) : 0;
        }
    }
...
api.clearQuery = function(table, query, options)
{
    ...
    var filter = options && lib.isArray(options.filter);
    var keep = options && options.keep && options.keep.test ? options.keep : null;
    var clear = options && options.clear && options.clear.test ? options.clear : null;
    ...
    if (clear || force) {
        for (var p in query) {
            ...
            if (force && !cols[p]) delete query[p];
        }
    }
    return query;
}
...
// still valid and can be channged.
//
// Example, do custom logging of all requests
//
    ...
    if (util.isError(status)) {
        ...
        if (status.message && !String(status.message).match(this.errlogLimiterIgnore)) {
            if (!this.errlogLimiterToken || this.errlogLimiterToken.consume(1)) {
                logger.error("sendReply:", res.req.url, status.message, res.req.headers,
                             lib.objDescr(res.req.query),
                             lib.objDescr(res.req.options, { ignore: /account|apath/ }),
                             lib.objDescr(res.req.account, { allow: /id|name|type|status|flags/ }),
                             lib.traceError(status));
            }
        }
