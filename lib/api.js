...
const api = {
    ...
    args: [
        ...
        { name: "images-mod", descr: "Images scaling module, sharp or bkjs-wand" },
        ...
        { name: "rlimits-(rate|max|interval|ttl|ip|delay|queue)-(.+)", autotype: 1, obj: "rlimitsMap.$2", make: "$1", descr: "Rate limiter parameters for Token Bucket algorithm. `queue` to use specific queue, ttl` is to expire cache entries, `ip` is to limit by IP address as well, ex. -api-rlimits-ip-ip=10, -api-rlimits-rate-/path=1" },
        { name: "rlimits-map-(.+)", type: "map", obj: "rlimitsMap", make: "$1", maptype: "auto", descr: "Rate limiter parameters for Token Bucket algorithm. set all at once, ex. -api-rlimits-map-/url=rate:1,interval:2000" },
        ...
        { name: "auth-message:", descr: "Default authenticated message to be returned with default auth status" },
        ...
        { name: "csrf-set-path", type: "regexpobj", descr: "Regexp for URLs to set CSRF token for all methods" },
        ...
        { name: "delays-([0-9]+)", type: "int", obj: "delays", nocamel: 1, descr: "Delays in ms by status code, useful for delaying error responses to slow down brute force attacks, ex. -api-delays-401 1000" },
    ...
    core.runMethods("configureMiddleware", options, () => {
        ...
        core.runMethods("configureWeb", options, (err) => {
            ...
            api.app.use(function apiEmpty(req, res, next) {
                ...
                return res.set("Content-Type", core.mime.getType(req.options.path)).status(200).send();
            });
...
{
    // Parse application version, extract first product and version only, expected format is Name/Version
    if (!req.options.appName || req.query[this.appHeaderName]) {
        let v = req.query[this.appHeaderName] || req.headers[this.appHeaderName] || req.headers['user-agent'];
        if (v) {
            v = v.substr(0, 64).split(/[ ,;/]/);
            if (v[0]) req.options.appName = v[0];
            if (v[1] && "0123456789".includes(v[1][0])) req.options.appVersion = v[1];
        }
    }
    ...
    // Localization from the supplied language or headers
    if (!req.options.appLocale || req.query[this.langHeaderName]) {
        let v = req.query[this.langHeaderName] || req.headers[this.langHeaderName] || req.headers['accept-language'];
        if (v) {
            v = v.substr(0, 10).toLowerCase().split(/[ ,;-]/);
            req.locale = req.options.appLocale = v[0];
...
api.checkQuery = function(req, res, next)
{
    ...
    var type = req.get("content-type") || "", sc = type.indexOf(";");
    if (sc > 0) type = type.substr(0, sc).trim();
    ...
    if (clen > 0 && clen >= this.queryLimit*1.5) {
    ...
    req.on('data', function(chunk) {
        ...
        if (size > api.queryLimit) {
            logger.debug("checkQuery:", "too large:", req.path, req.headers, buf);
...
api.checkBody = function(req, res, next)
{
    ...
    form.once('error', (err) => {
        ...
        logger.debug("checkBody:", "too large:", err, req.path, req.headers, data, Object.keys(files));
    ...
    form.on('progress', (bytesReceived, bytesExpected) => {
        ...
        if (bytesExpected > api.uploadLimit) {
            logger.debug("checkBody:", "too large:", req.path, req.headers, data, Object.keys(files));
            done = true;
...
api.checkStaticRouting = function(req)
{
    ...
    for (const p in api.compressed) {
        if (lib.testRegexp(req.options.path, api.compressed[p])) {
            ...
            req.res.setHeader("Content-Type", core.mime.getType(req.options.opath));
...
// This is used by API servers and proxy servers for early redirections. It returns null
...
api.checkRateLimits = function(req, options, callback)
{
    ...
    lib.forEachSeries(types, (type, next) => {
        ...
        ipc.limiter(limit, (delay, info) => {
            ...
            var err = { status: 429, message: lib.__(options.message || api.rlimits.message, lib.toDuration(delay)) };
...
api.sendStatus = function(res, options)
{
    ...
    var delay = req.options?.sendDelay || api.delays[status];
    try {
        switch (status) {
        case 301:
        case 302:
...
api.sendFile = function(req, file, redirect)
{
    ...
    fs.stat(file, function(err, st) {
        ...
        if (req.method == 'HEAD') return req.res.set("Content-Length", err ? 0 : st.size).set("Content-Type", core.mime.getType(file)).status(!err ? 200 : 404).send();
