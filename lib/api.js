...
var api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "redirect-url", type: "regexpmap", descr: "Add to the list a JSON object with property name defining a host/path regexp to be matched early against in order to redirect using the value of the property, if the regexp starts with !, that means negative match, 2 variables can be used for substitution: @HOST@, @PATH@, @URL@, @BASE@, example: { '^[^/]+/path/$': '/path2/index.html', '.+/$': '@PATH@/index.html' } " },
...
api.checkRedirectRules = function(req, options, rules)
{
    ...
    for (var i in rules) {
        ...
        if (rx && rx.test(location)) {
            ...
            if (!url) continue;
            url = url.replace(/@(HOST|PATH|URL|BASE)@/g, function(_, m) {
