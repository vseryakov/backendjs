...
api.checkBody = function(req, res, next)
{
    ...
    var form = formidable.formidable(opts);
    ...
    form.on('progress', (bytesReceived, bytesExpected) => {
        ...
        form.emit("error", lib.newError({ message: "too large", _msg: "Cannot process the request, it is too large", status: 413, maxsize: api.uploadLimit, length: bytesExpected }));
    });

    form.parse(req, (err) => {
        if (err) {
            logger.debug("checkBody:", err, req.path, req.headers, data, Object.keys(files));
            if (err && /maxFile|maxField|maxTotal/.test(err.message)) err._msg = "Cannot process the request, it is too large";
            return next(err);
        }
        try {
