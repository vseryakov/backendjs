...
var api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "static-path", type: "path", array: 1, descr: "List of additional paths to be used when serving static content" },
    ...
    core.runMethods("configureMiddleware", options, function() {
        ...
        core.runMethods("configureWeb", options, function(err) {
            ...
            if (!api.noTemplating) {
                ...
                api.app.set('views', core.path.views.concat([core.home + "/views", core.path.web + "/../views", __dirname + '/../views' ]));
            ...
            if (!api.noStatic) {
                ...
                for (var i = 0; i < api.staticPath.length; i++) {
                    api.app.use(serveStatic(api.staticPath[i], api.staticOptions));
                }
                api.app.use(serveStatic(core.path.web, api.staticOptions));
                api.app.use(serveStatic(__dirname + "/../web", api.staticOptions));
                logger.debug("static:", core.path.web, __dirname + "/../web", api.staticPath);
            }
...
//
// If no names are specified the query will only keep existing columns for the given table.
//
    ...
    if (util.isError(status)) {
        ...
        if (status.message && !String(status.message).match(this.errlogLimiterIgnore)) {
            if (!this.errlogLimiterToken || this.errlogLimiterToken.consume(1)) {
                logger.error("sendReply:", res.req.url, status.message, res.req.headers, lib.objDescr(res.req.query), lib.objDescr(res.req.account), lib.traceError(status));
            }
        }
