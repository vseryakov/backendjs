 ...
 *      - id - unique random id generated for each request for tracking
...
module.exports = {
    ...
    args: [
        ...
        { name: "cleanup-fields", obj: "cleanup", type: "list", array: 1, descr: "List of fields inside requests to explicitely cleanup on end" },
    ...
    cleanup: {
        ...
        /** @var {string[]} cleanup.fields - Properties to be cleaned up on finish */
        fields: ["options", "user", "signature", "body", "trace"],
...
api.handleCleanup = function(req)
{
    ...
    lib.forEverySeries(hooks, (hook, next) => {
        ...
        for (const p in req) {
            if (p.startsWith("__") || api.cleanup.fields.includes(p)) {
