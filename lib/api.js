...
var api = {
    ...
    args: [{ name: "images-url", descr: "URL where images are stored, for cases of central image server(s), must be full URL with optional path" },
           ...
           { name: "ignore-ssl", type: "regexpobj", descr: "Allow plain HTTP from matched IP addresss" },
           ...
           { name: "routing-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: 'routing', lower: /.+/, descr: "URL path to be re-routed to other path, this is done inside the server at the beginning, only the path is replaced, same format and placeholders as in redirect-url, example: -api-routing-^/account/get /acount/read" },
           { name: "routing-auth-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: 'routing-auth', lower: /.+/, descr: "URL path to be re-routed to other path after the authentication is successful, this is done inside the server, only the path is replaced, same format and placeholders as in redirect-url, example: -api-routing-auth-^/account/get /acount/read" },
           { name: "login-redirect-(.+)", type: "regexpobj", reverse: 1, nocamel: 1, obj: "login-redirect", lower: /.+/, descr: "Define a location where to redirect if no login is provided, same format and placeholders as in redirect-url, example: api-login-redirect-^/admin/=/login.html" },
    ],
...
api.checkRouting = function(req, name)
{
    ...
    for (var p in rules) {
        if (rules[p].rx && (rules[p].rx.test(req.options.path) || rules[p].rx.test(location))) {
...
{
    return pathname.replace(/@(HOST|PATH|URL|BASE|DIR|QUERY)@/g, function(_, m) {
        return m[0] == "H" ? req.options.host :
               m[0] == "P" ? req.options.path :
...
api.checkRedirect = function(req, options)
{
    ...
    if (this.allowSsl.rx) {
        if (!options.secure && this.allowSsl.rx.test(options.path)) {
            if (!this.ignoreSsl.rx || !this.ignoreSsl.rx.test(options.ip)) {
...
api.checkRedirectRules = function(req, options, name)
{
    ...
    for (var i in rules) {
        ...
        if (rx && (rx.test(url) || rx.test(location))) {
            let url = !lib.isNumeric(i) ? i : rules[i].value || "";
            if (!url) continue;
            url = this.checkRedirectPlaceholders(req, url);
            logger.debug("checkRedirectRules:", name, location, options.path, "=>", url, "rule:", i, rules[i]);
...
api.sendReply = function(res, status, text)
{
    if (util.isError(status)) {
        ...
        text = this.allowErrorCode.rx && this.allowErrorCode.rx.test(status.code) ? res.__(status.message) :
               status._msg ? res.__(status._msg) :
               this.errorMessage || res.__("Internal error occured, please try again later");
