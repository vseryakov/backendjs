...
function Pool(options)
{
    ...
    this._errors = {};
    this._nodes = [];
    this._version = this.configOptions.minversion || 6;
...
Pool.prototype.doQuery = function(method, path, postdata, query, options, callback)
{
    ...
    var opts = {
        ...
        node: this._getNodeUrl(this._node),
    };
    var uri = opts.node + (path[0] == "/" ? "" : "/") + path;
    logger.logger(options.logger_db || "debug", "elasticsearchQuery:", path, lib.objDescr(opts, { depth: 10 }));
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (err || !params.status || params.status >= 400) {
            if (!err) {
                ...
                logger[options.quiet || params.status == 404 ? "debug": "error"]("elasticsearchQuery:", this._version, method, uri, lib.objDescr(params.query, { depth: 10 }), "OBJ:", lib.objDescr(postdata), "ERR:", params.status, params.data);
            } else {
                params.obj = null;
            }
        }
...
{
    return keys.filter(function(x) { return obj[x] }).map(function(x) { return obj[x] }).join("|");
}

Pool.prototype._getDocType = function(table)
{
    return this._version < 7 ? (table || "") + "/" + this.configOptions.docType : table || "";
}
...
Pool.prototype._escape = function(val, options, name)
{
    ...
    if (name && opts.boosts && opts.boosts[name]) val += "^" + opts.boosts[name];
    return val;
}
...
{
    var self = this;
    return lib.phraseSplit(q).map(function(y) { return self._escape(y, options) }).join(" AND ");
}
...
Pool.prototype.getQueryCondition = function(req, obj, join)
{
    ...
    return Object.keys(obj).map(function(x) {
        ...
        switch (op) {
        case "null":
            return "(NOT _exists_:" + x + ")";
        case "not_null":
            return "_exists_:" + x;
        case "!=":
        case "ne":
            return '-' + n + self._escape(val, options, x);
        case ">":
        case "gt":
            return n + '>' + self._escape(val, options, x);
        case "<":
        ...
        case "in":
        case "all_in":
            return !lib.isEmpty(val) ? (lib.isArray(val) ? '(' + val.map(function(y) { return n + self._escape(y, options, x) }).join(op == "in" ? " OR " : " AND ") + ')' : n + self._escape(val, options, x)) : "";
        case "not_in":
            return !lib.isEmpty(val) ? (lib.isArray(val) ? n + '(' + val.map(function(y) { return "NOT " + self._escape(y, options, x) }).join(" AND ") + ')' : "NOT " + n + self._escape(val, options, x)) : "";
        case "between":
            return !lib.isEmpty(val) ? n + (val.length == 2 ? '[' + self._escape(val[0], options, x) + ' TO ' + self._escape(val[1], options, x) + ']' : self._escape(val, options, x)) : "";
        case "begins_with":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return n + self._escape(y, options, x) + '*' }).join(" AND ") : n + self._escape(val, options, x) + '*' : "";
        case "like":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return n + self._escape(y, options, x) + '~1' }).join(" AND ") : n + self._escape(val, options, x) + '~1' : "";
        case "like%":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return n + self._escape(y, options, x) + '~' }).join(" AND ") : n + self._escape(val, options, x) + '~' : "";
        case "contains":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y) { return n + '*' + self._escape(y, options, x) + '*' }).join(" AND ") : n + '*' + self._escape(val, options, x) + '*' : "";
        case "not_contains":
            return !lib.isEmpty(val) ? (lib.isArray(val) ? n + '(' + val.map(function(y) { return "NOT *" + self._escape(y, options, x) + '*' }).join(" AND ") + ')' : "NOT " + n + '*' + self._escape(val, options, x)) + '*': "";
        case "=":
        case "eq":
        default:
            if (val === "") break;
            return Array.isArray(val) ? val.map(function(y) { return n + self._escape(y, options, x) }).join(" AND ") : n + self._escape(val, options, x);
        }
    }).filter(function(x) {
        return x;
    ...
    if (req.options && (!lib.isEmpty(req.options.updateOps) || !lib.isEmpty(req.options.expected))) {
        ...
        for (var p in req.obj) {
            ...
            default:
                ...
                if (p.indexOf(".") > 0) {
                    v = p.split(".");
                    obj.script.source += "if(!ctx._source.containsKey('" + v[0] + "') || ctx._source." + v[0] + "==null) ctx._source." + v[0] + " = new HashMap(); ctx._source." + p + " = params." + v[1] + ";";
                    obj.script.params[v[1]] = val;
                    break;
                }
                if (keys.indexOf(p) == -1) {
...
{
    var self = this, docType = this.configOptions.docType;
    this.doQuery("GET", "/_mappings", "", "", this.configOptions, function(err, data, params) {
        ...
        for (var table in data) {
            ...
            var properties = data[table].mappings.properties || data[table].mappings && data[table].mappings[docType] && data[table].mappings[docType].properties;
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "create":
        ...
        if (!this.dbindexes[req.table]) {
            ...
            if (self._version < 7) {
                query.mappings[this.configOptions.docType] = { properties: properties };
            } else {
        ...
        if (missing) {
            return this.doQuery("PUT", req.table + "/_mappings" + this._getDocType(), { properties: properties }, "", this.configOptions, callback);
        }
        callback();
        break;
    ...
    case "search":
        ...
        if (req.options.start) {
            ...
            if (lib.isObject(req.options.start) && req.options.start.scroll_id && req.options.start.scroll) {
                ...
                path = this._getDocType(req.table) + "/_search/scroll";
                method = "GET";
            }
        }
        ...
        if (req.options.total) {
            path = this._getDocType(req.table) + "/_count";
            delete opts.sort;
        }
        ...
        this.doQuery(method, path, obj, query, req.options, function(err, res) {
            ...
            if (res.hits) {
                ...
                if (!res.count) res.count = res.hits.total && res.hits.total.value || res.hits.total;
    ...
    case "list":
        path = this._getDocType(req.table) + "/_mget";
    ...
    case "get":
        path = this._getDocType(req.table) + "/" + lib.escape(this._getKey(keys, req.obj));
    ...
    case "incr":
    case "update":
        path = this._version < 7 ?
               req.table + "/" + this.configOptions.docType + "/" + lib.escape(this._getKey(keys, req.obj)) + "/_update" :
               req.table + "/_update/" + lib.escape(this._getKey(keys, req.obj));
        query = this._getQuery("index", req.options);
    ...
    case "bulk":
        ...
        for (const i in req.obj) {
            ...
            meta = { _id: this._getKey(keys, item.obj), _index: item.table };
            if (this._version < 7) meta._type = this.configOptions.docType;
            switch (item.op) {
            case "add":
            ...
            case "update":
                if (!item.options || typeof req.options.retry_on_conflict != "number") {
                    meta[(this._versioin < 7 ? "_" : "") + "retry_on_conflict"] = this.configOptions.retryOnConflict;
                }
        ...
        if (!data) return callback(null, info);
        ...
        this.doQuery("POST", "/_bulk", data, "", options, function(err, res) {
            if (!err && res) {
                for (var i in res.items) {
                    ...
                    if (item && item.status >= 400 && map[item._id]) {
                        info.push(lib.objExtend(map[item._id], "errstatus", item.status, "errmsg", item.error && item.error.reason, "columns", undefined));
                    }
                }
                delete res.items;
                info.affected_rows = req.obj.length - info.length;
            }
