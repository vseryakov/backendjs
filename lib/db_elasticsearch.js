...
Pool.prototype.doQuery = function(op, method, path, obj, options, callback)
{
    ...
    var opts = {
        ...
        query: {},
        datatype: "obj",
        retryCount: this.configOptions.retryCount,
        retryTimeout: this.configOptions.retryTimeout,
    ...
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (err || !params.status || params.status >= 400) {
            if (!err) err = lib.newError({ message: (rc.error && rc.error.reason) || (method + " Error-" + params.status), code: rc.error && rc.error.type, status: params.status || 500 });
            logger.debug("elasticsearch:", method, uri, params.query, "OBJ:", obj, "ERR:", err, rc);
        }
        callback(err, rc);
    });
}
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "get":
        ...
        this.doQuery("get", "GET", path, "", oreq.ptions, function(err, res) {
            if (err) return callback(err, []);
