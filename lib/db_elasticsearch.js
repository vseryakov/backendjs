...
function Pool(options)
{
    ...
    this.configOptions = lib.mergeObj(pool.configOptions, this.configOptions);
    this._errors = {};
    this._nodes = [];
...
Pool.prototype.doQuery = function(table, method, path, postdata, query, options, callback)
{
    ...
    logger.dev("elasticsearch:", table, uri, opts);
...
Pool.prototype.queryParams = function(name, options)
{
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "search":
        ...
        query = this.queryParams("query", req.options);
        ...
        if (typeof req.obj == "string") {
            query = this.queryParams("q", req.options);
            query.q = req.obj;
            opts = query;
        } else {
            return callback();
        ...
        var sort = db.getSortingColumn(req.table, req.options);
        if (!sort && req.options._sort) sort = req.options._sort;
        if (sort) {
            if (obj) {
                opts.sort = lib.strSplit(sort).map(function(x) { return req.options.desc ? lib.newObj(x, { order: "desc" }) : x });
            } else {
        ...
        this.doQuery(req.table, method, path, obj, query, req.options, function(err, res) {
            ...
            if (res.hits) {
                ...
                if (!res._scroll_id && Array.isArray(opts.sort) && rows.length == opts.size) {
    ...
    case "list":
        ...
        query = this.queryParams("list", req.options);
    ...
    case "get":
        ...
        query = this.queryParams("get", req.options);
    ...
    case "put":
        path = req.table +  "/" + this._getKey(keys, req.obj);
        query = this.queryParams("index", req.options);
    ...
    case "update":
        ...
        if (!req.options.noscript && lib.isObject(req.options.updateOps) && Object.keys(req.options.updateOps).length) {
            obj = { script: { lang: "painless", inline: "", params: {} }, scripted_upsert: true, upsert: {} };
            for (var p in req.obj) {
        ...
        if (typeof req.options.retry_on_conflict == "undefined") query.retry_on_conflict = 3;
    ...
    case "del":
        path = req.table +  "/" + this._getKey(keys, req.obj);
        query = this.queryParams("del", req.options);
