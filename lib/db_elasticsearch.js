...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    switch (req.op) {
    case "create":
        var properties = {}, missing = 0, ecols = this.dbcolumns[req.table];
        ...
        if (!Object.keys(this.dbcolumns).length) {
            this.dbcolumns[req.table] = properties;
            var query = { settings: { number_of_shards: this.configOptions.nshards, number_of_replicas: this.configOptions.nreplicas }, mappings: {} };
            query.mappings[req.table] = { properties: properties };
            return this.doQuery("", "POST", "", query, this.configOptions, callback);
        }
        if (missing) {
            return this.doQuery("", "PUT", "_mappings/" + req.table, { properties: properties }, this.configOptions, callback);
        }
        callback();
        break;
        ...
        if (lib.isObject(req.obj)) {
            if (lib.isObject(req.obj.query)) {
                ...
                req.options.q = Object.keys(req.obj).map(function(x) {
                    ...
                    case "!=":
                    case "ne": return n + '-' + self._escape(val, req.options);
                    case ">":
                    case "gt": return n + '>' + self._escape(val, req.options);
                    case "<":
                    case "lt": return n + '<' + self._escape(val, req.options);
                    case ">=":
                    case "ge": return n + '>=' + self._escape(val, req.options);
                    case "<=":
                    case "le": return n + '<=' + self._escape(val, req.options);
                    case "in": return (Array.isArray(val) ? '(' + val.map(function(y) { return n + self._escape(y, req.options) }).join(" OR ") + ')' : n + self._escape(val, req.options));
                    case "between": return n + (val.length == 2 ? '[' + self._escape(val[0], req.options) + ' TO ' + self._escape(val[1], req.options) + ']' : self._escape(val, req.options));
                    case "begins_with": return Array.isArray(val) ? val.map(function(y) { return n + self._escape(y, req.options) + '*' }).join(" AND ") : n + self._escape(val, req.options) + '*';
                    case "contains": return Array.isArray(val) ? val.map(function(y) { return n + '*' + self._escape(y, req.options) + '*' }).join(" AND ") : n + '*' + self._escape(val, req.options) + '*';
                    case "not_contains": return Array.isArray(val) ? val.map(function(y) { return n + '>' + self._escape(y, req.options) }).join(" AND ") : n + '>' + self._escape(val, req.options);
                    case "=":
                    case "eq": return Array.isArray(val) ? val.map(function(y) { return n + '"' + self._escape(y, req.options) + '"' }).join(" AND ") : n + '"' + self._escape(val, req.options) + '"';
                    default: return Array.isArray(val) ? val.map(function(y) { return n + '"' + self._escape(y, req.options) + '"' }).join(" AND ") : n + self._escape(val, req.options);
                    }
                }).filter(function(x) {
                    return x;
