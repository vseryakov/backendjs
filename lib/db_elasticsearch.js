...
Pool.prototype._getNodes = function(callback)
{
    ...
    this.doQuery("GET", "/_nodes", "", "", { quiet: 1 }, (err, data, params) => {
        ...
        if (!err && data && typeof data.nodes == "object") {
            ...
            for (const p in data.nodes) {
                ...
                if (!self._version) self._version = lib.toVersion(node.version);
                if (lib.isEmpty(node.roles) || (!self.configOptions.clientNodes && lib.isFlag(node.roles, "data"))) {
...
Pool.prototype._retryPrepare = function(params)
{
    ...
    logger.debug("elasticsearchRetry:", params.nodePath, "trying node:", this._node, "count:", params.retryCount, params.retryTotal, "nodes:", this._nodes, this._servers);
}
...
Pool.prototype.doQuery = function(method, path, postdata, query, options, callback)
{
    ...
    core.httpGet(opts.nodeHost + opts.nodePath, opts, function(err, params) {
        logger.logger(options.logger_db || "debug", "elasticsearchQuery:", opts, "hits:", params.obj && params.obj.hits && params.obj.hits.hits.length, err);
...
Pool.prototype._getUpdate = function(req)
{
    ...
    if (req.options && (!lib.isEmpty(req.options.updateOps) || !lib.isEmpty(req.options.expected))) {
        obj = { script: { source: "", params: {} }, scripted_upsert: true, upsert: {} };
        const updateOps = req.options.updateOps || lib.empty;
        const expr = this._buildCondition(req, obj, req.options.expected, req.options.expectedJoin);
        if (expr) obj.script.source = "if(" + expr + ")ctx.op=\"none\";";
        for (const p in req.obj) {
            var val = req.obj[p], v, op = updateOps[p];
            if (v === null || v === "") op = "remove";
            switch (op) {
            case "incr":
            case "append":
        ...
        if (lib.isObject(req.obj)) {
            if (req.obj.aggs || req.obj.aggregations || req.obj.query) {
                ...
                if (lib.isObject(req.options.filter)) {
                    ...
                    if (req.options.noscan && !req.options.sort && !req.options.count && lib.isEmpty(obj)) {
                ...
                for (const i in sort) {
                    if (req.options.desc || req.options.sort_missing) {
                        ...
                        for (const p in o) {
                            ...
                            if (req.options.sort_missing) o[p].missing = req.options.sort_missing;
                        }
                        opts.sort.push(o);
                    } else {
        ...
        this.doQuery(method, path, obj, query, req.options, function(err, res) {
            ...
            if (res._scroll_id) {
                if (!rows.length || (req.options.limit > 0 && req.options.limit_count + rows.length >= req.options.limit)) {
                    self.doQuery("DELETE", "/_search/scroll", { scroll_id: res._scroll_id }, "", self.configOptions, lib.noop);
                    res.next_token = null;
                }
            }
