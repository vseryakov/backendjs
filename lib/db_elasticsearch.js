...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    var dbcols = req.columns || db.getColumns(req.table, options);
    ...
    case "search":
        ...
        if (lib.isObject(req.obj)) {
            if (lib.isObject(req.obj.query)) {
                ...
                options.q = Object.keys(req.obj).map(function(x) {
                    ...
                    var op = (options.ops && options.ops[x]) || (val === null ? "ne" : "");
                    var n = x && x != "q" ? x + ":" : "";
                    switch (op) {
                    case "!=":
        ...
        if (options.sort_timeout) options.scroll = lib.toNumber(options.sort_timeout) + "ms"; else
        if (options.fullscan) options.scroll = this.poolOptions.sort_timeout;
        if (options.start) {
