...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "search":
        ...
        if (req.options.sort) {
            ...
            if (sort.length) {
                if (obj) {
                    opts.sort = sort.map(function(x) { return req.options.desc ? lib.newObj(x, { order: "desc" }) : x });
                } else {
    ...
    case "update":
        ...
        if (!req.options.noscript && lib.isObject(req.options.updateOps) && Object.keys(req.options.updateOps).length) {
            obj = { script: { lang: self._version >= 5 ? "painless" : "groovy", inline: "", params: {} }, scripted_upsert: true, upsert: {} };
            for (var p in req.obj) {
