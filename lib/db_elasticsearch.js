...
//
// To place different tables into different indexes define the index mapping: `-db-elasticsearch-pool-options-index-table-INDEX_NAME=TABLES_REGEXP`,
...
//
// A simple way to create an index per table: `-db-elasticsearch-pool-options-index-tables=bk_account,bk_auth,bk_message`,
// this will create a separate index for each table from the list.
//
...
Pool.prototype._getNodeUrl = function(node, table)
{
    ...
    return [ h.protocol + "//" + h.host, this._getIndex(table) || h.pathname ];
}

Pool.prototype._getIndex = function(table)
{
    if (!table) return;
    if (this.configOptions.indexTables) {
        this.configOptions.indexTables = lib.strSplit(this.configOptions.indexTables);
        if (this.configOptions.indexTables.indexOf(table) > -1) return table;
    }
    ...
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (err || !params.status || params.status >= 400) {
            if (!err) {
                ...
                err = lib.newError({ message: (error ? lib.objDescr(error) : "") || (method + " Error " + params.status),
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "drop":
        ...
        if (!name || name != req.table || !name.match(/^[a-zA-Z0-9_]+$/)) return callback();
