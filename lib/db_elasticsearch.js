...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "bulk":
        var info = { _size: req.obj.length, _retries: 0, _timeout: self.configOptions.bulkRetryTimeout };
        ...
        lib.doWhilst(
            function(next) {
                ...
                for (const i in batch) {
                    ...
                    if (this._version < 7) meta._type = self.configOptions.docType;
                    switch (item.op) {
                    case "add":
                    ...
                    case "update":
                        if (!item.options || typeof req.options.retry_on_conflict != "number") {
                            meta[(self._versioin < 7 ? "_" : "") + "retry_on_conflict"] = self.configOptions.retryOnConflict;
                        }
