...
Pool.prototype.doQuery = function(table, method, path, postdata, query, options, callback)
{
    ...
    logger.dev("doQuery:", "elasticsearch:", this._version, table, uri, opts);
...
Pool.prototype._escape = function(val, options)
{
    ...
    case "string":
        ...
        if (val == "OR" || val == "AND" || pool.quoteRx.test(val)) val = '"' + val + '"';
        break;
    default:
        val = null;
    }
...
{
    var opsMap = { 'like%': 'begins_with', "=" : "==", 'eq': '==', 'ne': '!=', 'le': '<=', 'lt': '>', 'ge': '>=', 'gt': '>' };
    ...
    if (req.options && (!lib.isEmpty(req.options.updateOps) || !lib.isEmpty(req.options.expected))) {
        ...
        for (var p in req.options.expected) {
            var op = ops[p] || "=";
            ...
            case "not_null":
                if (this._version < 5) {
                    obj.script.inline += "if(" + not + "ctx._source." + p + ") ctx.op=\"none\";";
                } else {
                    obj.script.inline += "if(" + not + "ctx._source.containsKey('" + p + "')) ctx.op='none';";
                }
                break;
            case "in":
            case "not_in":
                if (this._version < 5) {
                    obj.script.inline += "if(" + not + "ctx._source." + p + " || " + not + "ctx._source." + p + ".contains(_" + p + ")) ctx.op=\"none\";";
                } else {
                    obj.script.inline += "if(" + not + "ctx._source.containsKey('" + p + "') || " + not + "ctx._source." + p + ".contains(params._" + p + ")) ctx.op='none';";
                }
                obj.script.params["_" + p] = req.options.expected[p];
                break;
            case "between":
                if (!Array.isArray(req.options.expected[p]) || req.options.expected[p].length != 2) break;
                if (this._version < 5) {
                    obj.script.inline += "if(!ctx._source." + p + " || ctx._source." + p + " < " + " _" + p + " || ctx._source." + p + " > __" + p + ") ctx.op=\"none\";";
                } else {
                    obj.script.inline += "if(!ctx._source.containsKey('" + p + "') || ctx._source." + p + " < " + " _" + p + " || ctx._source." + p + " > __" + p + ")) ctx.op='none';";
                }
                obj.script.params["_" + p] = req.options.expected[p][0];
                obj.script.params["__" + p] = req.options.expected[p][1];
                break;
            case "begins_with":
            case "not_begins_with":
                if (this._version < 5) {
                    obj.script.inline += "if(" + not + "ctx._source." + p + " || " + not + "ctx._source." + p + ".startsWith(_" + p + ")) ctx.op=\"none\";";
                } else {
                    obj.script.inline += "if(" + not + "ctx._source.containsKey('" + p + "') || " + not + "ctx._source." + p + ".startsWith(params._" + p + ")) ctx.op='none';";
                }
                obj.script.params["_" + p] = req.options.expected[p];
                break;
            case "contains":
            ...
            case "==":
                if (this._version < 5) {
                    obj.script.inline += "if(!ctx._source." + p + " || !(ctx._source." + p + " " + op + " _" + p + ")) ctx.op=\"none\";";
                } else {
                    obj.script.inline += "if(!ctx._source.containsKey('" + p + "') || !(ctx._source." + p + " " + op + " params._" + p + ")) ctx.op='none';";
                }
                obj.script.params["_" + p] = req.options.expected[p];
                break;
            }
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "search":
        ...
        if (req.options.sort) {
            ...
            if (sort.length) {
                if (obj) {
                    opts.sort = sort.map(function(x) { return req.options.desc ? lib.objNew(x, { order: "desc" }) : x });
                } else {
                    opts.sort = sort + (req.options.desc ? ":desc" : "");
                }
            }
        }
        this.doQuery(req.table, method, path, obj, query, req.options, function(err, res) {
            ...
            if (res.hits) {
                ...
                if (!res._scroll_id && Array.isArray(opts.sort) && rows.length == opts.size && self._version >= 5) {
                    res.next_token = opts.sort.map(function(x) {
                        if (typeof x != "string") x = Object.keys(x)[0];
    ...
    case "update":
        ...
        if (typeof req.options.retry_on_conflict == "undefined") query.retry_on_conflict = this.retryOnConflict;
...
Pool.prototype.bulk = function(list, options, callback)
{
    var data = "", req, keys, meta;
    for (var i in list) {
        req = list[i];
        keys = db.getKeys(req.table, req.options);
        meta = { _id: this._getKey(keys, req.obj), _index: this._getIndex(req.table), _type: req.table };
        switch (req.op) {
        case "add":
            data += lib.stringify({ create: meta }) + "\n";
            data += lib.stringify(req.obj) + "\n";
            break;
        case "put":
        ...
        case "incr":
        case "update":
            if (!req.options || typeof req.options.retry_on_conflict != "number") meta._retry_on_conflict = this.retryOnConflict;
            data += lib.stringify({ update: meta }) + "\n";
            data += lib.stringify(this._getUpdate(req)) + "\n";
            break;
        case "del":
