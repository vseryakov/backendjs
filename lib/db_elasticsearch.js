...
Pool.prototype.getQueryCondition = function(req, obj, join)
{
    ...
    return Object.keys(obj).map(function(x) {
        ...
        case "!=":
        case "ne":
            return '-' + n + self._escape(val, options);
        case ">":
        case "gt":
            return n + '>' + self._escape(val, options);
        case "<":
        ...
        case "in":
        case "all_in":
            return !lib.isEmpty(val) ? (lib.isArray(val) ? '(' + val.map(function(y) { return n + self._escape(y, options) }).join(op == "in" ? " OR " : " AND ") + ')' : n + self._escape(val, options)) : "";
        case "not_in":
            return !lib.isEmpty(val) ? (lib.isArray(val) ? n + '(' + val.map(function(y) { return "NOT " + self._escape(y, options) }).join(" AND ") + ')' : "NOT " + n + self._escape(val, options)) : "";
        case "between":
            return !lib.isEmpty(val) ? n + (val.length == 2 ? '[' + self._escape(val[0], options) + ' TO ' + self._escape(val[1], options) + ']' : self._escape(val, options)) : "";
        case "begins_with":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return n + self._escape(y, options) + '*' }).join(" AND ") : n + self._escape(val, options) + '*' : "";
        case "like":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return n + self._escape(y, options) + '~1' }).join(" AND ") : n + self._escape(val, options) + '~1' : "";
        case "like%":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return n + self._escape(y, options) + '~' }).join(" AND ") : n + self._escape(val, options) + '~' : "";
        case "contains":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y) { return n + '*' + self._escape(y, options) + '*' }).join(" AND ") : n + '*' + self._escape(val, options) + '*' : "";
        case "not_contains":
            return !lib.isEmpty(val) ? (lib.isArray(val) ? n + '(' + val.map(function(y) { return "NOT *" + self._escape(y, options) + '*' }).join(" AND ") + ')' : "NOT " + n + '*' + self._escape(val, options)) + '*': "";
        case "=":
        case "eq":
        default:
            if (val === "") break;
            return Array.isArray(val) ? val.map(function(y) { return n + self._escape(y, options) }).join(" AND ") : n + self._escape(val, options);
        }
    }).filter(function(x) {
        return x;
