...
Pool.prototype.doQuery = function(op, method, path, obj, options, callback)
{
    ...
    core.httpGet(uri, params, function(err, params) {
        ...
        err = null;
        var rc = lib.jsonParse(params.data, { obj: 1 });
...
Pool.prototype.query = function(client, req, options, callback)
{
        ...
        if (lib.isObject(req.obj)) {
            if (lib.isObject(req.obj.query)) {
                ...
                options.q = Object.keys(req.obj).map(function(x) {
                    ...
                    var n = x && x != "q" ? x + ":" : "";
                    switch (op) {
                    case "in": return n + (Array.isArray(val) ? '(' + val.map(function(y) { return '"' + y + '"' }).join(" OR ") + ')' : val);
                    ...
                    case "begins_with": return n + '"' + val + '*"';
                    case "contains": return n + '"*' + val + '*"';
                    case "not_contains": return n + '>' + val;
    ...
    case "update":
        ...
        if (!options.noscript && lib.isObject(options.updateOps) && Object.keys(options.updateOps).length) {
            var query = { script: { inline: "", params: {} }, scripted_upsert: true, upsert: {}, retry_on_conflict: 3 };
            ...
            var query = { doc: req.obj, retry_on_conflict: 3 };
