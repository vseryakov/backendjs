...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "bulk":
        var data = "", item, meta, map = {}, info = {}, rows = [];
        ...
        if (!data) return callback(null, rows, info);
            ...
            if (!err && res) {
                for (var i in res.items) {
                    ...
                    if (item && item.status >= 400 && map[item._id]) {
                        rows.push(lib.objExtend(map[item._id], { _id: item._id, errstatus: item.status, errmsg: item.error && item.error.reason }, { del: /^(columns|orig|options)/ }));
                    }
                }
                delete res.items;
                res.affected_rows = req.obj.length - rows.length;
                info = res;
            }
