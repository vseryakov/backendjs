...
Pool.prototype.cacheColumns = function(options, callback)
{
    ...
    this.doQuery("GET", "/_mappings", "", "", this.configOptions, function(err, data, params) {
        ...
        for (var table in data) {
            ...
            var properties = data[table].mappings.properties || data[table].mappings[docType] && data[table].mappings.properties[docType].properties;
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "search":
        ...
        if (req.options.start) {
            ...
            if (lib.isObject(req.options.start) && req.options.start.scroll_id && req.options.start.scroll) {
                ...
                path = this.getDocType(req.table) + "/_search/scroll";
                method = "GET";
            }
        }
        ...
        if (req.options.total) {
            path = this.getDocType(req.table) + "/_count";
            delete opts.sort;
        }
    ...
    case "list":
        path = this.getDocType(req.table) + "/_mget";
    ...
    case "get":
        path = this.getDocType(req.table) + "/" + lib.escape(this._getKey(keys, req.obj));
    ...
    case "bulk":
        var data = "", item, meta, map = {}, info = [], _type = this.getDocType();
