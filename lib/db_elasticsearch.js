...
Pool.prototype._escape = function(val)
{
    return typeof val == "string" ? val.replace(this.escapeRx, '\\$1') : val;
}
        ...
        if (lib.isObject(req.obj)) {
            if (lib.isObject(req.obj.query)) {
                ...
                req.options.q = Object.keys(req.obj).map(function(x) {
                    ...
                    switch (op) {
                    case "!=":
                    case "ne": return n + '-' + self._escape(val);
                    case ">":
                    case "gt": return n + '>' + self._escape(val);
                    case "<":
                    case "lt": return n + '<' + self._escape(val);
                    case ">=":
                    case "ge": return n + '>=' + self._escape(val);
                    case "<=":
                    case "le": return n + '<=' + self._escape(val);
                    case "in": return (Array.isArray(val) ? '(' + val.map(function(y) { return n + self._escape(y) }).join(" OR ") + ')' : n + self._escape(val));
                    case "between": return n + (val.length == 2 ? '[' + self._escape(val[0]) + ' TO ' + self._escape(val[1]) + ']' : self._escape(val));
                    case "begins_with": return Array.isArray(val) ? val.map(function(y) { return n + self._escape(y) + '*' }).join(" AND ") : n + self._escape(val) + '*';
                    case "contains": return Array.isArray(val) ? val.map(function(y) { return n + '*' + self._escape(y) + '*' }).join(" AND ") : n + '*' + self._escape(val) + '*';
                    case "not_contains": return Array.isArray(val) ? val.map(function(y) { return n + '>' + self._escape(y) }).join(" AND ") : n + '>' + self._escape(val);
                    case "=":
                    case "eq": return Array.isArray(val) ? val.map(function(y) { return n + '"' + self._escape(y) + '"' }).join(" AND ") : n + '"' + self._escape(val) + '"';
                    default: return Array.isArray(val) ? val.map(function(y) { return n + '"' + self._escape(y) + '"' }).join(" AND ") : n + self._escape(val);
                    }
                }).filter(function(x) {
                    return x;
