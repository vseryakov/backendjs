...
var pool = {
    ...
    configOptions: {
        ...
        replicas: 1,
        typesMap: {
            float: "float", real: "float", ttl: "bigint",
...
Pool.prototype.doQuery = function(method, path, postdata, query, options, callback)
{
    ...
    logger.logger(options.logger_db || "debug", "elasticsearchQuery:", path, opts);
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (err || !params.status || params.status >= 400) {
            if (!err) {
                ...
                logger[options.quiet || params.status == 404 ? "debug": "error"]("elasticsearchQuery:", this._version, method, uri, params.query, "OBJ:", postdata, "ERR:", params.status, params.data);
            } else {
                params.obj = null;
            }
        }
    ...
    case "create":
        ...
        for (var p in req.obj) {
            ...
            if (o.params) {
                ...
                var t = o.join ? "text" : this.configOptions.typesMap[o.type];
                if (t) properties[p] = typeof t == "object" ? t : { type: t };
            }
            if (!properties[p]) continue;
            if (!properties[p].type) properties[p].type = this.configOptions.typesMap[o.type] || this.configOptions.defaultType;
    ...
    case "search":
        ...
        this.doQuery(method, path, obj, query, req.options, function(err, res) {
            ...
            if (res.hits) {
                ...
                if (!res.total) res.total = res.hits.total && res.hits.total.value || res.hits.total;
