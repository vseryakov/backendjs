...
Pool.prototype.getQueryCondition = function(req, obj, join)
{
    ...
    return Object.keys(obj).map(function(x) {
        ...
        case "not_between":
            return !lib.isEmpty(val) ? "NOT " + n + (val.length == 2 ? '[' + self._escape(val[0], options, x) + ' TO ' + self._escape(val[1], options, x) + ']' : self._escape(val, options, x)) : "";
        case "begins_with":
        ...
        case "not_begins_with":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return "NOT " + n + self._escape(y, options, x) + '*' }).join(" AND ") : "NOT " + n + self._escape(val, options, x) + '*' : "";
        case "like":
            return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y, i) { return n + self._escape(y, options, x) + '~' }).join(" AND ") : n + self._escape(val, options, x) + '~' : "";
        case "contains":
...
Pool.prototype.query = function(client, req, options, callback)
{
        ...
        if (req.options.sort) {
            ...
            if (sort.length) {
                ...
                if (obj) {
                    for (var i in sort) {
                        if (req.options.desc) {
                            var o = typeof sort[i] == "string" ? { [sort[i]]: {} } : sort[i];
    ...
    case "bulk":
        var info = { retry_count: 0, _size: req.obj.length, _retries: 0, _timeout: self.configOptions.bulkRetryTimeout };
        var bulk = req.obj, bulkSize = req.options.bulkSize || self.configOptions.bulkSize;
        var map = {}, errors = [];
        lib.doWhilst(
            function(next) {
                var item, meta, data = "";
                ...
                for (const i in batch) {
                    ...
                    if (self._version < 7) meta._type = self.configOptions.docType;
                    switch (item.op) {
                    case "add":
                    ...
                    case "update":
                        if (!item.options || typeof req.options.retry_on_conflict != "number") {
                            meta[(self._version < 7 ? "_" : "") + "retry_on_conflict"] = self.configOptions.retryOnConflict;
                        }
                        data += lib.stringify({ update: meta }) + "\n";
                        data += lib.stringify(self._getUpdate(item)) + "\n";
                        break;
                    case "del":
                ...
                self.doQuery("POST", "/_bulk", data, "", options, function(err, res) {
                    if (!err && res) {
                        var retry, item;
