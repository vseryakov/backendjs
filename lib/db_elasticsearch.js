...
Pool.prototype.doQuery = function(method, path, postdata, query, options, callback)
{
    ...
    var uri = opts.nodeHost + opts.nodePath;
    core.httpGet(uri, opts, function(err, params) {
        logger.logger(options.logger_db || "debug", "elasticsearchQuery:", uri, opts, "hits:", params.obj && params.obj.hits && params.obj.hits.hits.length, err);
        ...
        if (err || !params.status || params.status >= 400) {
            if (!err) {
                ...
                logger[options.quiet || params.status == 404 ? "debug": "error"]("elasticsearchQuery:", self._version, method, uri, params.query, "OBJ:", postdata, "ERR:", params.status, params.data);
            } else {
                params.obj = null;
            }
        }
