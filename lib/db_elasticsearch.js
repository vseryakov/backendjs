...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "create":
        ...
        if (missing) {
            return this.doQuery("PUT", req.table + "/_mappings/_doc", { properties: properties }, "", this.configOptions, callback);
        }
        callback();
        break;
    ...
    case "search":
        ...
        if (req.options.start) {
            ...
            if (lib.isObject(req.options.start) && req.options.start.scroll_id && req.options.start.scroll) {
                ...
                path = req.table + "/_doc/_search/scroll";
                method = "GET";
            }
        }
    ...
    case "get":
        path = req.table + "/_doc/" + this._getKey(keys, req.obj);
    ...
    case "incr":
    case "update":
        path = req.table + "/_doc/" + this._getKey(keys, req.obj) + "/_update";
        query = this._getQuery("index", req.options);
