...
Pool.prototype.doQuery = function(method, path, postdata, query, options, callback)
{
    ...
    logger.logger(options.logger_db || "debug", "doQuery:", path, lib.objDescr(opts));
    core.httpGet(uri, opts, function(err, params) {
        ...
        if (err || !params.status || params.status >= 400) {
            if (!err) {
                ...
                logger[options.quiet || params.status == 404 ? "debug": "error"]("doQuery:", this._version, method, uri, lib.objDescr(params.query), "OBJ:", lib.objDescr(postdata), "ERR:", params.status, params.data);
            } else {
                params.obj = null;
            }
        }
