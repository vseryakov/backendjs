...
var pool = {
    ...
    configOptions: {
        ...
        typesMap: {
            ...
            ttl: "long", counter: "long", random: "bigint",
...
Pool.prototype.doQuery = function(method, path, postdata, query, options, callback)
{
    ...
    core.httpGet(uri, opts, function(err, params) {
        logger.logger(options.logger_db || "debug", "elasticsearchQuery:", path, opts, "hits:", params.obj && params.obj.hits && params.obj.hits.hits.length);
...
{
    var self = this;
    return lib.phraseSplit(q).map((y) => (self._escape(y, options))).join(" AND ");
}
...
Pool.prototype.getQueryCondition = function(req, obj, join)
{
    ...
    return Object.keys(obj).map((x) => {
        ...
        case "!=":
        case "ne":
            return lib.isArray(val) ? '(' + val.map((y) => ("-" + n + self._escape(y, options, x))).join(` ${jop || "AND"} `) + ')' : '-' + n + self._escape(val, options, x);
        ...
        case "all_in":
            ...
            return lib.isArray(val) ? '(' + val.map((y) => (n + self._escape(y, options, x))).join(` ${jop || op == "in" ? "OR" : "AND"} `) + ')' : n + self._escape(val, options, x);

        case "not_in":
            if (lib.isEmpty(val)) break;
            return lib.isArray(val) ? n + '(' + val.map((y) => ("NOT " + self._escape(y, options, x))).join(` ${jop || "AND"} `) + ')' : "NOT " + n + self._escape(val, options, x);

        case "between":
            if (lib.isEmpty(val)) break;
            return n + (val.length == 2 ? `[${self._escape(val[0], options, x)} TO ${self._escape(val[1], options, x)}]` : self._escape(val, options, x));

        case "not_between":
            if (lib.isEmpty(val)) break;
            return "NOT " + n + (val.length == 2 ? `[${self._escape(val[0], options, x)} TO ${self._escape(val[1], options, x)}]` : self._escape(val, options, x));

        case "begins_with":
            if (lib.isEmpty(val)) break;
            return lib.isArray(val) ? val.map((y, i) => (n + self._escape(y, options, x) + '*')).join(` ${jop || "AND"} `) : n + self._escape(val, options, x) + '*';

        case "not_begins_with":
            if (lib.isEmpty(val)) break;
            return lib.isArray(val) ? val.map((y, i) => (`NOT ${n}${self._escape(y, options, x)}*`)).join(` ${jop || "AND"} `) : `NOT ${n}${self._escape(val, options, x)}*`;
        ...
        default:
            if (val === "") break;
            return Array.isArray(val) ? val.map((y) => (n + self._escape(y, options, x))).join(`${jop || "AND"} `) : n + self._escape(val, options, x);
        }
        return null;
