    ...
    this.doQuery("GET", "/_nodes", "", "", { quiet: 1 }, (err, data, params) => {
        ...
        if (!err && data && typeof data.nodes == "object") {
            ...
            for (const p in data.nodes) {
                ...
                if ((self.configOptions.clientNodes && lib.isEmpty(node.roles)) ||
                    (!self.configOptions.clientNodes && lib.isFlag(node.roles, "data"))) {
...
Pool.prototype._nextNode = function(err, params)
{
    ...
    if (err || (params.retryTotal > 0 && params.retryCount <= 0)) {
        ...
        if (this.configOptions.rotateNodes && this._nodes.length > 1) {
            this._node = this._nodes.shift();
