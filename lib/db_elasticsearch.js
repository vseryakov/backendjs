...
Pool.prototype.getQueryCondition = function(req, obj, join)
{
    ...
    return Object.keys(obj).map(function(x) {
        ...
        case "not_contains": return !lib.isEmpty(val) ? lib.isArray(val) ? val.map(function(y) { return n + '>' + self._escape(y, options) }).join(" AND ") : n + '>' + self._escape(val, options) : "";
        case "=":
        case "eq":
        ...
        if (!this.dbindexes[eindex]) {
            ...
            if (self._version < 5) {
                for (var p in properties) {
                    if (properties[p].type == "text") properties[p].type = "string";
                }
            }
