...
{
    if (this._nodesTimerRunning) return;
    var self = this;
    clearTimeout(this._nodesTimer);
    this._nodesTimerRunning = 1;
...
Pool.prototype._getNodeUrl = function(node, table)
{
    ...
    return [ h.protocol + "//" + h.host, table ];
}
...
Pool.prototype._getUpdate = function(req)
{
    ...
    if (req.options && (!lib.isEmpty(req.options.updateOps) || !lib.isEmpty(req.options.expected))) {
        ...
        var obj = { script: { inline: "", params: {} }, scripted_upsert: true, upsert: {} };
        ...
        for (var p in req.obj) {
            ...
            case "incr":
            case "append":
                obj.script.inline += "if(!ctx._source.containsKey('" + p + "') || ctx._source." + p + "==null) ctx._source." + p + " = params." + p + "; else ctx._source." + p + " += params." + p + ";";
            ...
            case "add":
                ...
                for (var i in v) {
                    obj.script.inline += "if(!ctx._source.containsKey('" + p + "') || ctx._source." + p + "==null) ctx._source." + p + " = [params." + p + i + "]; else if(!ctx._source." + p + ".contains(params." + p + i + ")) ctx._source." + p + ".add(params." + p + i + ");";
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    case "create":
        ...
        for (var p in req.obj) {
            ...
            if (!properties[p].type) properties[p].type = "text";
        ...
        if (!this.dbindexes[eindex]) {
            ...
            lib.series([
              ...
              function(next) {
                  self.doQuery(req.table, "PUT", "", query, "", self.configOptions, next);
              },
            ], callback);
            return;
        }
    ...
    case "bulk":
        ...
        for (var i in req.obj) {
            ...
            meta = { _id: this._getKey(keys, item.obj), _index: item.table, _type: item.table };
            switch (item.op) {
            case "add":
