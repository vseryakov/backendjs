...
class Push extends EventEmitter {
    ...
    configureModule(options, callback)
    {
        for (const type in this._config) {
            try {
                ...
                logger.error("configureModule:", this.name, options, e.stack);
            }
        }
        callback();
    }
    ...
    shutdown(_options, callback)
    {
        ...
        lib.forEach(Object.keys(this.clients), (type, next) => {
            ...
            if (!lib.isFunc(client.close)) return next();
            client.close(next);
        }, callback);
    }
    ...
    * @param {object} options - see each driver for specific properties,
    * {@link module:push.WebPushClient}, {@link module:push.APNPushClient}, {@link module:push.FCMPushClient},
    * {@link module:push.SNSPushClient},
    * @param {string} options.msg - message body
    ...
    */
    send(device, options, callback)
    {
        ...
        var devs = lib.split(device, null, "string").map((x) => {
        ...
        logger.debug("send:", this.name, device, options, "DEV:", devs);
    ...
    /**
     * Async version of {@link module:push.send}
    * @param {string} device - where to send the message to, can be multiple ids separated by , or |.
    * @param {object} options
    * @returns {Promise} as object { err }
    * @async
    * @memberof module:push
    * @method asend
     *
     */
    asend(device, options)
    {
        return new Promise((resolve, _reject) => {
            this.send(device, options, (err) => {
                resolve({ err });
            })
        });
    }
    ...
    */
    parseDevice(device)
    {
        ...
        const d = device.match(/^([a-z]+:\/\/)([^@]+)?@?([a-z0-9._-]+)?/i);
        if (d) {
