...
Pool.prototype.queryList = function(client, req, callback)
{
    ...
    lib.forEachLimit(chunks, req.options.concurrency, (chunk, next) => {
        const batch = lib.objNew(req.table, { keys: chunk, select: db.getSelectedColumns(req.table, req.options), consistent: req.options.consistent });
        ...
        function(next) {
            ...
            if (moreItems) {
                ...
                for (var i in list) {
                    ...
                    switch (list[i].op) {
                    case "del":
                        list[i].obj = db.getKeys(list[i].table).map((x) => (list[i].obj[x]));
                    case "put":
                        if (!breq[list[i].table]) breq[list[i].table] = [];
                        breq[list[i].table].push({ [list[i].op]: list[i].obj });
                        break;
                    default:
                        rows.push(lib.objExtend(list[i], "errstatus", "NotSupported", "columns", undefined));
                    }
                }
            }
