...
db.dynamodbInitPool = function(options)
{
    ...
    options.settings = { noJson: 1, strictTypes: 1, skipNull: { add: 1, put: 1 }, retries: 3, timeout: 100 };
    ...
    pool.query = function(client, req, opts, callback) {
        ...
        case "upgrade":
            ...
            ["","1","2","3","4","5"].forEach(function(n) {
                ...
                idx.forEach(function(x) {
                    if (obj[x]["projection" + n]) add.projection = add.projection.concat(obj[x]["projection" + n]);
        ...
        case "search":
            ...
            logger.debug('select:', 'dynamodb', op, keys, dbkeys, opts.sort, opts.count, opts.noscan, op == 'ddbScanTable' && opts.noscan ? "NO EMPTY SCANS ENABLED" : "");
        ...
        default:
            ...
            callback(new Error("invalid op: " + req.op), []);
        }
    };
    return pool;
}
