...
db.dynamodbInitPool = function(options)
{
    ...
    options.settings = { noJson: 1, strictTypes: 1, noConcat: 1, skipNull: { add: 1, put: 1 }, retryCount: 7, retryTimeout: 25, httpTimeout: 250 };
    var pool = this.createPool(options);

    function describeTable(table, rc) {
    ...
    pool.cacheColumns = function(opts, callback) {
        ...
        aws.ddbListTables(opts, function(err, rc) {
            ...
            lib.forEachSeries(rc.TableNames, function(table, next) {
                aws.ddbDescribeTable(table, opts, function(err, rc) {
                    if (err || rc.Table.TableStatus == "DELETING") return next();
                    describeTable(table, rc);
                    next();
                });
            }, callback);
        });
    ...
    pool.query = function(client, req, opts, callback) {
        ...
        case "create":
            ...
            Object.keys(attrs).forEach(function(x) {
                attrs[x] = lib.isNumericType(obj[x].type) ? "N" : "S";
            ...
            aws.ddbCreateTable(table, attrs, opts, function(err, item) {
                ...
                if (item && item.TableDescription && !pool.dbindexes[table]) {
                    describeTable(table, { Table: item.TableDescription });
                }
                callback(err, [], item);
            });
            break;
        ...
        case "upgrade":
            ...
            ["","1","2","3","4","5"].forEach(function(n) {
                ...
                idx.forEach(function(x) {
                    ...
                    add[x] = lib.isNumericType(obj[x].type) ? "N" : "S";
                });
                global[name] = add;
            });
        ...
        case "get":
            ...
            aws.ddbGetItem(table, keys, opts, function(err, rc) {
        ...
        case "search":
            ...
            if (opts.useCapacity || opts.capacity) {
                var cap = opts.capacity || db.getCapacity(table, { useCapacity: opts.useCapacity, factorCapacity: opts.factorCapacity });
            ...
            for (var p in opts.ops) {
                ...
                // No op for a hash key
                if (opts.ops[p] && p == dbkeys[0] && op == "ddbQueryTable") opts.ops[p] = '';
            }
            opts.select = self.getSelectedColumns(table, opts);
               ...
               function(next) {
                   aws[op](table, keys, opts, function(err, item) {
                       ...
                       if (!err && item.ConsumedCapacity) {
                           ...
                           if (opts.useCapacity) return db.checkCapacity(cap, item.ConsumedCapacity.CapacityUnits, next);
                       }
                       next(err);
                   });
               },
        ...
        case "list":
            var req = {}, info = { consumed_capacity: 0 }, rows = [];
            // Capacity rate limiter
            if (opts.useCapacity > 0 || opts.capacity) {
                var cap = opts.capacity || db.getCapacity(table, { useCapacity: opts.useCapacity, factorCapacity: opts.factorCapacity });
                opts.ReturnConsumedCapacity = "TOTAL";
            }
            // Keep retrieving items until we reach the end or our limit
                       ...
                       lib.whilst(
                           ...
                           function(next2) {
                               ...
                               aws.ddbBatchGetItem({}, opts, function(err, item) {
                                   ...
                                   if (!err && item.ConsumedCapacity) {
                                       info.consumed_capacity += item.ConsumedCapacity.CapacityUnits;
                                       if (opts.useCapacity) return db.checkCapacity(cap, item.ConsumedCapacity.CapacityUnits, next2);
                                   }
                                   next2(err);
                               });
                       }, function(err) {
        ...
        case "add":
            ...
            if (opts.useCapacity || opts.capacity) opts.ReturnConsumedCapacity = "TOTAL";
