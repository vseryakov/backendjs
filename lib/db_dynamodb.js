...
{
    var self = this;
    options = lib.cloneObj(options, "endpoint", this.url);
    ...
    lib.series([
      ...
      function(next) {
          lib.forEachLimit(tables, options.concurrency || self.poolOptions.concurrency || core.concurrency, function(table, next) {
...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    req.options.endpoint = this.url;
    req.options.retryCount = this.poolOptions.retryCount;
    req.options.retryTimeout = this.poolOptions.retryTimeout;
    req.options.httpTimeout = this.poolOptions.httpTimeout;
    switch(req.op) {
    case "create":
...
Pool.prototype.queryCreate = function(client, req, callback)
{
    ...
    req.options.projection = projection;
    req.options.readCapacity = req.options.readCapacity || this.poolOptions.readCapacity;
    req.options.writeCapacity = req.options.writeCapacity || this.poolOptions.writeCapacity;
...
Pool.prototype.queryPrepareSelect = function(client, req, callback)
{
    ...
    req.options.keys = !dbattrs ? Object.keys(req.obj) : Object.keys(req.obj).filter(function(x) { return dbkeys.indexOf(x) > -1 || dbattrs.indexOf(x) > -1 });
    ...
    logger.debug('select:', 'dynamodb', req.table, op, query, dbkeys, dbattrs, req.options.sort, req.options.count, req.options.noscan, op == 'ddbScanTable' && req.options.noscan ? "NO EMPTY SCANS ENABLED" : "");

    // Scans explicitely disabled
    if (op == 'ddbScanTable' && req.options.noscan) return callback(null, []);
...
Pool.prototype.queryRunSelect = function(op, client, req, query, callback)
{
    ...
    var cap;
    if (req.options.useCapacity || req.options.factorCapacity || req.options.capacity) {
        cap = req.options.capacity || db.getCapacity(req.table, { useCapacity: req.options.useCapacity, factorCapacity: req.options.factorCapacity });
    ...
    for (var p in req.options.ops) {
        ...
        if (req.options.ops[p] == "in" && p == req.options.keys[1]) req.options.keys = [ req.options.keys[0] ];
        if (req.options.ops[p] == "in" && p == req.options.keys[0]) op = 'ddbScanTable';
        // Noop for a hash key
        if (req.options.ops[p] && p == req.options.keys[0] && op == "ddbQueryTable") req.options.ops[p] = '';
    }
    req.options.select = db.getSelectedColumns(req.table, req.options);
...
Pool.prototype.queryList = function(client, req, callback)
{
    ...
    // Capacity rate limiter
    if (req.options.useCapacity > 0 || req.options.capacity) {
        var cap = req.options.capacity || db.getCapacity(req.table, { useCapacity: req.options.useCapacity, factorCapacity: req.options.factorCapacity });
        req.options.ReturnConsumedCapacity = "TOTAL";
    }
    // Keep retrieving items until we reach the end or our limit
    lib.doWhilst(
...
Pool.prototype.queryAdd = function(client, req, callback)
{
    ...
    if (req.options.useCapacity || req.options.capacity) options.ReturnConsumedCapacity = "TOTAL";
