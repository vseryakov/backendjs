        ...
        idx.forEach(function(x) {
            ...
            add.projection = Object.keys(req.obj).filter(function(y, i) {
                return req.obj[y]["projection" + n] ||
                        ...
                        (Array.isArray(req.obj[y].projections) && req.obj[y].projections.indexOf(lib.toNumber(n)) > -1);
            });
...
Pool.prototype.queryRunSelect = function(op, client, req, query, callback)
{
    ...
    for (var p in req.options.ops) {
        ...
        if (req.options.ops[p] == "in" && p == req.options.keys[1]) {
       ...
       function(next) {
           aws[op](req.table, query, req.options, function(err, item) {
               if (req.options.total) {
                   ...
                   if (inFilter) item.Items = item.Items.filter(function(x) { return inFilter.indexOf(x[req.options.keys[1]]) > -1 });
