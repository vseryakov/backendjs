...
Pool.prototype.queryCreate = function(client, req, callback)
{
    ...
    ["","1","2","3","4","5"].forEach(function(n) {
        ...
        var p = Object.keys(req.obj).filter(function(x, i) {
            return req.obj[x]["projection" + n] ||
                    ...
                    (Array.isArray(req.obj[x].projections) && req.obj[x].projections.indexOf(n || 0) > -1);
        });
    ...
    req.options.readCapacity = req.options.readCapacity || this.configOptions.readCapacity;
    req.options.writeCapacity = req.options.writeCapacity || this.configOptions.writeCapacity;
...
Pool.prototype.queryUpgrade = function(client, req, callback)
{
    ...
    ["","1","2","3","4","5"].forEach(function(n) {
        ...
        idx.forEach(function(x) {
            ...
            add.projection = Object.keys(req.obj).filter(function(y, i) {
                return req.obj[y]["projection" + n] ||
                        (req.obj[y].projections && !Array.isArray(req.obj[y].projections)) ||
                        (Array.isArray(req.obj[y].projections) && req.obj[y].projections.indexOf(n || 0) > -1);
            });
...
Pool.prototype.queryPrepareSelect = function(client, req, callback)
{
    ...
    req.options.keys = !dbattrs ? Object.keys(req.obj) : Object.keys(req.obj).filter(function(x) {
        return (!req.options.sort && req.columns[x] && req.columns[x].primary) || dbkeys.indexOf(x) > -1 || dbattrs.indexOf(x) > -1;
    });
    ...
    logger.debug('select:', 'dynamodb', req.table, op, req.options.sort, 'query:', query, 'keys:', dbkeys, 'attrs:', dbattrs, req.options.count, req.options.noscan, op == 'ddbScanTable' && req.options.noscan ? "NO EMPTY SCANS ENABLED" : "");
...
Pool.prototype.queryRunSelect = function(op, client, req, query, callback)
{
    ...
    if (req.options.useCapacity || req.options.factorCapacity || req.options.capacity) {
        cap = req.options.capacity || db.getCapacity(req.table, { useCapacity: req.options.useCapacity || "read", factorCapacity: req.options.factorCapacity });
