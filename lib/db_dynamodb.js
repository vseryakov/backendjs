...
Pool.prototype.describeTable = function(table, rc)
{
    ...
    (rc.Table.KeySchema || []).forEach(function(x) {
        ...
        lib.objSet(self.dbkeys, [table], x.AttributeName, { push: 1, unique: 1 });
        lib.objSet(self.dbcapacity, [table, table], { read: rc.Table.ProvisionedThroughput.ReadCapacityUnits || 0, write: rc.Table.ProvisionedThroughput.WriteCapacityUnits || 0 });
    });
    (rc.Table.LocalSecondaryIndexes || []).forEach(function(x) {
        ...
        x.KeySchema.forEach(function(y) {
            lib.objSet(self.dbindexes, [table, x.IndexName], y.AttributeName, { push: 1, unique: 1 });
            self.dbcolumns[table][y.AttributeName].lsi = 1;
        });
    });
    (rc.Table.GlobalSecondaryIndexes || []).forEach(function(x) {
        ...
        x.KeySchema.forEach(function(y) {
            lib.objSet(self.dbindexes, [table, x.IndexName], y.AttributeName, { push: 1, unique: 1 });
            self.dbcolumns[table][y.AttributeName].gsi = 1;
        });
        lib.objSet(self.dbcapacity, [table, x.IndexName], { read: rc.Table.ProvisionedThroughput.ReadCapacityUnits || 0, write: rc.Table.ProvisionedThroughput.WriteCapacityUnits || 0 });
    });
}
...
Pool.prototype.cacheColumns = function(options, callback)
{
    ...
    aws.ddbListTables(options, function(err, rc) {
        ...
        lib.forEachLimit(rc.TableNames, options.concurrency || core.concurrency, function(table, next) {
...
Pool.prototype.queryRunSelect = function(op, client, req, query, options, callback)
{
    ...
    var cap;
    if (options.useCapacity || options.factorCapacity || options.capacity) {
        cap = options.capacity || db.getCapacity(req.table, { useCapacity: options.useCapacity, factorCapacity: options.factorCapacity });
...
Pool.prototype.queryUpdate = function(client, req, options, callback)
{
    ...
    var old = this.saveOptions(options, 'expected');
    var keys = db.getSearchQuery(req.table, req.obj);
    if (req.op == "update" && !options.upsert) {
        if (options.expected) {
