...
db.dynamodbInitPool = function(options)
{
    ...
    pool.cacheColumns = function(opts, callback) {
        ...
        aws.ddbListTables(options, function(err, rc) {
            ...
            pool.dbcolumns = {};
            pool.dbindexes = {};
            corelib.forEachLimit(rc.TableNames, 3, function(table, next) {
                aws.ddbDescribeTable(table, options, function(err, rc) {
                    ...
                    (rc.Table.LocalSecondaryIndexes || []).forEach(function(x) {
                        x.KeySchema.forEach(function(y) {
                            corelib.objSet(pool.dbindexes, [table, x.IndexName], y.AttributeName, { push: 1 });
    ...
    pool.convertError = function(table, op, err, opts) {
        ...
        case "add":
        case "put":
            if (err.code == "ConditionalCheckFailedException") return corelib.newError({ message: "Record already exists", code: "ExpectedCondition", status: 409 });
            break;
        case "incr":
        case "update":
            if (err.code == "ConditionalCheckFailedException") return corelib.newError({ message: "Record not found", code: "ExpectedCondition", status: 406 });
            break;
        }
        return err;
    }
        ...
        case "create":
            ...
            ["","1","2","3","4","5"].forEach(function(n) {
                ...
                if (idx.length == 2 && idx[0] == hash) {
                    local[name] = corelib.newObj(idx[0], 'HASH', idx[1], 'RANGE');
                } else
                // Global if does not start with the primary hash
                if (idx.length == 2) {
                    global[name] = corelib.newObj(idx[0], 'HASH', idx[1], 'RANGE');
                } else {
                    global[name] = corelib.newObj(idx[0], 'HASH');
                }
