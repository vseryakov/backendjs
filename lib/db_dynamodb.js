...
Pool.prototype.query = function(client, req, options, callback)
{
    ...
    req.options.region = req.options.endpoint || this.configOptions.region;
...
Pool.prototype.queryPrepareSelect = function(client, req, callback)
{
    ...
    // Remove not projected columns
    var select = db.getSelectedColumns(req.table, req.options);
    if (select && dbattrs) {
        select = req.options.select.filter((x) => ((req.columns[x] && req.columns[x].primary) || dbkeys.indexOf(x) > -1 || dbattrs.indexOf(x) > -1));
    }
...
{
    var info = { consumed_capacity: 0 }, rows = [], breq, moreItems;
