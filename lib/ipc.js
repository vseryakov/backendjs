...
class Ipc extends EventEmitter {
    ...
    handleWorkerMessages(msg) {
        if (!msg) return;
        logger.dev('handleWorkerMessages:', this.name, msg.__op, app.role, msg)
        lib.runCallback(msg);
    ...
    newMsg(op, msg, _options) {
        ...
        return Object.assign(msg, { __op: String(op) });
    }
    ...
    * @param {object|string} msg
    * @param {object} [options]
    * @param {int} [options.timeout=30000] can be used to specify a timeout for how long to wait the reply, default is 5s to avoid hanging
    * if a worker exists
    ...
    */
    sendMsg(op, msg, options, callback) {
        ...
        logger.dev("sendMsg:", this.name, msg.__op, app.role, msg);
        ...
        if (typeof callback === "function") {
            msg.__res = true;
            lib.deferCallback(msg, callback, options?.timeout || this.deferTimeout);
        }
