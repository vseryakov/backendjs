...
Ipc.prototype.applyOptions = function(val, options)
{
    ...
    var d = options.obj.match(/^configParams\.([^.]+)/);
    var client = d && this.getClient(d[1]);
    if (!client || client.queueName != d[1]) return;
    logger.debug("applyOptions:", options.obj, options.name, "OLD:", client.options);
...
//
// The callback will receive the same arguments as `ipc.limiter`` with `info.retries`` set to how many times it tried.
Ipc.prototype.checkLimiter = function(options, callback)
{
    ...
    this.limiter(options, (delay, info) => {
        ...
        if (!delay || (options.retry && info.retries >= options.retry)) {
