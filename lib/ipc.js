...
// If it is an array then a queue will be picked sequentially by maintaining internal sequence number.
//
// Default local cache and queue created automatically, can be used as `queueName: 'local'`.
//
...
//
// Drivers MUST NOT modify the `options` passed, it can be a shared empty object, for customizations make a copy inside a driver method
//
function Ipc()
{
    ...
    // Config params
    this.configParams = { local: { url: "local://" } };
    this.args = [
        ...
        { name: "broadcast-queue", descr: "System queue name to subscribe for broadcast control messages, if not defined the 'system' queue will be used, this is a PUB/SUB queue to process system messages like restart, re-init config,..." },
...
Ipc.prototype.applyOptions = function(val, options)
{
    ...
    var client = this.getClient({ queueName: name });
    if (!client) return;
...
Ipc.prototype.initServer = function()
{
    ...
    this.subscribe(core.name + ":" + core.role, { queueName: this.broadcastQueue || this.systemQueue }, (msg) => {
...
Ipc.prototype.getClient = function(options)
{
    var client, name = typeof options == "string" ? options : options && (options.queueName || options.cacheName);
    if (name) {
        if (Array.isArray(name) && name.length > 1) {
            name = name[this._nameIndex++ % name.length];
            if (this._nameIndex >= Number.MAX_SAFE_NUMBER) this._nameIndex = 0;
        }
...
Ipc.prototype.getQueue = Ipc.prototype.getCache = function(name)
{
    return this.getClient({ queueName: name });
}
    ...
    for (const name in this.configParams) {
        if (!this.clients[name]) {
            ...
            if (client) {
                client.queueName = name || "default";
                this.clients[name] = client;
                logger.debug("checkClients:", name, "added");
            }
        }
    }
}
...
Ipc.prototype.stats = function(options, callback)
{
    ...
    try {
        this.getClient(options).stats(options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
...
Ipc.prototype.get = function(key, options, callback)
{
    ...
    try {
        this.getClient(options).get(key, options || lib.empty, (err, val) => {
...
Ipc.prototype.del = function(key, options, callback)
{
    ...
    try {
        this.getClient(options).del(key, options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
...
Ipc.prototype.put = function(key, val, options, callback)
{
    ...
    try {
        this.getClient(options).put(key, val, options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
...
Ipc.prototype.incr = function(key, val, options, callback)
{
    ...
    try {
        this.getClient(options).incr(key, lib.toNumber(val), options || lib.empty, (err, val) => {
...
Ipc.prototype.subscribe = function(channel, options, callback)
{
    ...
    try {
        this.getClient(options).subscribe(channel, options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
...
Ipc.prototype.unsubscribe = function(channel, options, callback)
{
    ...
    try {
        this.getClient(options).unsubscribe(channel, options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
...
Ipc.prototype.publish = function(channel, msg, options, callback)
{
    ...
    try {
        ...
        this.getClient(options).publish(channel, msg, options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
...
Ipc.prototype.publishQueue = function(msg, options, callback)
{
    ...
    try {
        if (typeof msg != "string") msg = lib.stringify(msg);
        this.getClient(options).publishQueue(msg, options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
...
Ipc.prototype.monitorQueue = function(options)
{
    ...
    try {
        this.getClient(options).monitorQueue(options || lib.empty);
    } catch (e) {
...
Ipc.prototype.unpublishQueue = function(msg, options, callback)
{
    ...
    try {
        this.getClient(options).unpublishQueue(msg, options || lib.empty, callback);
    } catch (e) {
...
Ipc.prototype.unlock = function(name, options, callback)
{
    ...
    try {
        this.getClient(options).unlock(name, options || lib.empty, typeof callback == "function" ? callback : undefined);
    } catch (e) {
