...
function Ipc()
{
    ...
    this.args = [
             ...
             { name: "cache(-([a-z0-1]+)?-?options)-(.+)$", obj: "configParams.cache$1", make: "$3", camel: '-', autotype: 1, descr: "Additional parameters for cache clients, specific to each implementation, Example: `-ipc-cache-options-ttl 3000`" },
             { name: "queue(-([a-z0-1]+)?-?options)-(.+)$", obj: "configParams.queue$1", make: "$3", camel: '-', autotype: 1, descr: "Additional parameters for queue clients, specific to each implementation, Example: `-ipc-queue-redis-options-max_attempts 3`" },
         ];
}
...
//    ipc.get("my:counter", { set: 10 }, function(data) { console.log(data) });
//
