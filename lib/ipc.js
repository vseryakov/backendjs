    ...
    try {
        ...
        case 'cache:get':
            ...
            if (msg.list) {
                ...
                if (!msg.name) {
                    msg.value = bkcache.lruKeys(msg.list + ":")[0];
                    if (msg.value) {
                        if (msg.del) bkcache.lruDel(msg.value);
                        msg.value = msg.value.substr(msg.list.length);
                    }
                } else {
...
//  if no key is provided it will return a random key, to get the whole list specify * as the key. Specifying
...
Ipc.prototype.checkLimiter = function(options, callback)
{
    ...
    this.limiter(options, function(delay, info) {
