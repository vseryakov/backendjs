...
Ipc.prototype.applyOptions = function(val, options)
{
    ...
    if (!client) return;
    logger.debug("applyOptions:", obj, "NEW:", options.ctx, "OLD:", client.options);
    lib.objExtend(client.options, options.ctx);
    client.applyOptions();
}
...
Ipc.prototype.handleServerMessages = function(worker, msg)
{
    ...
    try {
        ...
        case "api:restart":
            ...
            for (const p in cluster.workers) this.restarting.push(cluster.workers[p].pid);
        ...
        setInterval(function() {
            ...
            for (var i in cluster.workers) {
                ...
                if (t >= 0 && now - t > self.pingInterval) {
                    logger.error("initServer:", core.role, "dead worker detected", w.id, w.process.pid, w.taskName, "interval:", self.pingInterval, "last ping:", now - t, "started:", now - w.startTime);
                    try { process.kill(w.process.pid, now -t > self.pingInterval*2 ? "SIGKILL" : "SIGTERM"); } catch(e) {}
                }
            }
        }, this.pingInterval/2);
    }
...
Ipc.prototype.initWorker = function()
{
    ...
    if (this.pingInterval > 0) {
        setInterval(this.sendMsg.bind(this, "worker:ping"), this.pingInterval/3);
...
Ipc.prototype.lock = function(name, options, callback)
{
    ...
    var started = Date.now(), ttl = lib.toNumber(options && options.ttl);
    lib.doWhilst(
      function(next) {
          try {
