...
class Ipc extends EventEmitter {
    ...
    initServer(options) {
        ...
        cluster.on("disconnect", (worker, _code, _signal) => {
        ...
        for (const chan of [app.role, app.role + ":" + app.env.tag]) {
            queue.subscribe(app.id + ":" + chan, { queueName: this.systemQueue }, (msg) => {
                this.handleServerMessages({ send: lib.noop }, this.newMsg(msg));
            });
        }

        this._pingTimer = setInterval(this.handleIdleWorkers.bind(this), this.ping.interval/2 || 60000).unref();
    ...
    initWorker(options) {
        ...
        if (this.ping.interval > 0) {
            this._pingTimer = setInterval(this.sendMsg.bind(this, "worker:ping"), Math.max(1000, this.ping.interval/5)).unref();
    ...
    shutdown(_options, callback) {
        logger.debug("shutdown:", this.name);
        clearInterval(this._pingTimer);
        this._pingTimer = undefined;
        for (const p in this.#defer) {
            clearTimeout(this.#defer[p].timer);
            delete this.#defer[p];
        }
    ...
    handleWorkerMessages(msg) {
        if (!msg) return;
        logger.dev('handleWorkerMessages:', this.name, app.role, msg)

        const defer = this.#defer[msg.__msgid];
        if (defer) {
            delete this.#defer[msg.__msgid];
        ...
        try {
            ...
            logger.error('handleWorkerMessages:', this.name, e.stack, msg);
        }
    }
    ...
    handleServerMessages(worker, msg) {
        if (!msg) return false;
        logger.dev('handleServerMessages:', this.name, msg.__op, app.role, worker.id, msg);
        ...
        try {
            ...
            case "api:ready":
                ...
                if (this.restarting.length) {
                    while (this.restarting.length > 0) {
                        ...
                        try { w.send({ __op: "api:restart" }) } catch (_e) { w = 0 }
                        if (w) break;
                    }
                }
            ...
            case "ipc:lock":
                msg.locked = cache.localLock(msg.name, msg.options);
                worker.send(msg);
                break;

            case "ipc:unlock":
                cache.localUnlock(msg.name, msg.options);
                worker.send(msg);
                break;
            ...
            logger.error('handleServerMessages:', this.name, e.stack, msg);
        }
    }
    ...
    handleIdleWorkers() {
        ...
        for (const w of lib.getWorkers()) {
            const pt = w.pingTime || 0, lt = now - pt;
            if (pt >= 0 && lt > this.ping.interval) {
                logger.error("handleIdleWorkers:", this.name, app.role, "dead worker detected", w.id, w.process.pid, "ping:", this.ping, "last ping:", lt, "started:", now - w.startTime);
                if (this.ping.kill > 0 && lt > this.ping.kill) {
                    try { process.kill(w.process.pid, w.killTime ? "SIGKILL" : "SIGTERM"); } catch (_e) {}
                    w.killTime = Date.now();
                }
            }
        }
    ...
    sendReplPort(role, worker) {
        ...
        let port = app.repl[role + "Port"];
        if (!port) return;
        const ports = Object.keys(this.ports).sort();
        ...
        logger.debug("sendReplPort:", this.name, port, role, worker.id, this.ports);
        worker.send({ __op: "repl:init", port });
    }

    /**
     * Returns an IPC message object
     * @param {object|string} op -if an object it is returned as is
     * @param {object|string} [msg] - 2 new internal properties are set `__op` and `__msgid`
     * @memberof module:ipc
     * @method newMsg
     */
    newMsg(op, msg) {
        ...
        if (typeof op === "string" && op[0] === "{" && op[op.length-1] === "}") {
            return lib.jsonParse(op, { data_type: "obj" });
        }
        if (typeof msg === "string" && msg) {
            msg = lib.jsonParse(msg, { logger: "info" });
        }
        return Object.assign(msg, { __op: String(op), __msgid: this.#id++ });
    }
     ...
     */
    emitMsg(op, msg) {
        if (op) this.emit(op, this.newMsg(op, msg));
    }
    ...
    * @param {object|string} msg
    * @param {object} [options]
    * @param {int} [options.timeout=5000] can be used to specify a timeout for how long to wait the reply, default is 5s to avoid hanging
    * if a worker exists
    ...
    */
    sendMsg(op, msg, options, callback) {
        ...
        logger.dev("sendMsg:", this.name, app.role, msg);

        if (!cluster.isWorker) {
            if (this.role === "server") {
                this.handleServerMessages({ send: lib.noop }, msg);
            }
        ...
        if (typeof callback === "function") {
            ...
            const timer = setTimeout(msgId => {
                const defer = this.#defer[msgId];
                if (defer) {
                    delete this.#defer[msgId];
                    defer.callback();
                }
            }, lib.toNumber(options?.timeout) || this.deferTimeout, msg.__msgid);

            this.#defer[msg.__msgid] = { callback, timer };
        }
    ...
    broadcast(channel, msg, options, callback) {
        ...
        if (!options?.queueName) {
            options = Object.assign({}, options, { queueName: this.systemQueue });
        }
        if (typeof channel !== "string") channel = String(channel);
        if (channel[0] === ":") channel = app.id + channel;
        logger.debug("broadcast:", this.name, channel, msg, options, queue.getClient(options?.queueName).name);
 ...
 * ## Redis system bus
 *
 * If configured all processes on all connected hosts subscribe to it and listen for system messages.
 ...
 * ## Worker system bus
 *
 * For a single host worker queue can be used for system messages, all worker processes will be listening and process events
 * the same was as in Redis case.
 * ```
