...
class Ipc extends EventEmitter {
    ...
    initServer() {
        ...
        queue.subscribe(core.name + ":" + core.role, { queueName: this.systemQueue }, (msg) => {
        ...
        logger.debug("initServer:", this.name, "started", core.role, core.workerId || process.pid, "ping:", this.pingInterval, this.systemQueue, queue.getClient(this.systemQueue).name);
    }
    ...
    initWorker() {
        ...
        logger.debug("initWorker:", this.name, "started", core.role, core.workerId || process.pid, this.systemQueue, queue.getClient(this.systemQueue).name);
    }
    ...
    sendMsg(op, msg, options, callback) {
        ...
        if (typeof callback == "function") {
            msg.__res = true;
            lib.deferCallback(this.msgs, msg, callback, options?.timeout);
        }
    ...
    // If the channel starts with : the `core.name` will be prepended automatically.
    broadcast(channel, msg, options, callback) {
        ...
        if (channel[0] == ":") channel = core.name + channel;
