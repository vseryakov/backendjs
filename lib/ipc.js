...
//
// A special system queue can be configured and it will be used by all servers to listen for messages on the channel `system`, the
// same messages that are processed by the server message handler like api:restart, config:init,.... All instances will be listening
// and processing these messages at once, the most usefull use case is refreshing the DB config on demand.
//
function Ipc()
{
...
Ipc.prototype.initServer = function()
{
    ...
    if (this.systemQueue) {
        this.subscribe("system", { queueName: this.systemQueue }, function(msg) {
    ...
    if (this.pingInterval) {
        setInterval(function() {
            ...
            for (var i in cluster.workers) {
                ...
                var t = w.pingTime || core.ctime;
                if (now - t > self.pingInterval) {
                    logger.error("initServer:", core.role, "dead worker detected", w.id, w.process.pid, w.taskName, "last ping:", t);
                    w.kill();
                }
            }
        }, this.pingInterval/2);
...
Ipc.prototype.initWorker = function()
{
    ...
    if (this.pingInterval) {
        setInterval(function() { self.sendMsg("worker:ping") }, this.pingInterval/2);
    }
}
