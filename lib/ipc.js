...
Ipc.prototype.handleWorkerMessages = function(msg)
{
    ...
    try {
        switch (msg.__op || "") {
        case "api:restart":
            core.modules.api.shutdown(() => { process.exit(0) });
            break;

        case "worker:restart":
            if (cluster.isWorker) core.modules.jobs.exitWorker({ shutdownReason: "restart" });
            break;

        case "worker:job":
            core.modules.jobs.processJobMessage("#worker", msg);
            break;
        ...
        case "ipc:lru:del":
            msg.data = this.lru.del(msg.name);
...
{
    if (!msg) return false;
    logger.dev('handleServerMessages:', core.role, worker.id, msg);
    try {
        switch (msg.__op) {
        case "api:restart":
...
{
    if (core.noIpc || this.__init) return;
    this.__init = 1;
    this.role = "server";
    this.initClients();
...
Ipc.prototype.checkClients = function()
{
    for (const name in this.configParams) {
...
Ipc.prototype.closeClients = function()
{
...
Ipc.prototype.incr = function(key, val, options, callback)
{
    ...
    try {
        this.getClient(options).incr(key, val, options || {}, (err, val) => {
