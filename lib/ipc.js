...
{
    if (this.none || this.__sinit) return;
    this.__sinit = 1;
    this.role = "server";
    this.initClients("cache");
    this.initClients("queue");
    this.initServerSystemQueue();
...
Ipc.prototype.initServerSystemQueue = function()
{
    if (!this.systemQueue || this.__sqinit) return;
    this.__sqinit = 1;
...
{
    if (this.none || this.__winit) return;
    this.__winit = 1;
    this.role = "worker";
    this.initClients("cache");
    this.initClients("queue");
    this.initWorkerSystemQueue();

    // Handle messages from the master
    process.on("message", (msg) => {
        this.handleWorkerMessages(msg);
    });
...
// Subscribe to the system bus
Ipc.prototype.initWorkerSystemQueue = function()
{
    if (!this.systemQueue || this.__wqinit) return;
    this.__wqinit = 1;
    this.subscribe(core.name + ":" + core.role, { queueName: this.systemQueue }, (msg) => {
