...
const server = {
    ...
    args: [
        ...
        { name: "api-restart-hours", type: "list", datatype: "int", descr: "List of hours when to restart api workers, only done once for each hour" },
    ],
...
server.startWebServer = function(options)
{
    ...
    if (cluster.isMaster) {
        ...
        d.run(function() {
            ...
            server.restartInterval = setInterval(server.restartWebWorkers, 3600000);
...
server.restartWebWorkers = function()
{
    var now = new Date();
    if (lib.isFlag(server.apiRestartHours, now.getHours())) {
