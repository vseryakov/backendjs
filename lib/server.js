...
server.startWatcher = function()
{
    ...
    core.watchdirs.forEach(function(dir) {
        core.watchFiles({ root: dir, file: /\.js$/, ignore: core.watchIgnore.rx }, _restart);
    });
    ...
    if (lib.isArray(core.watchWeb) && core.buildWeb) {
        ...
        webdirs.forEach(function(dir) {
            core.watchFiles({ root: dir, file: /\.(js|css)$/, ignore: core.watchIgnore.rx }, function(file) {
                if (server._watchWeb || !file.stat.size) return;
                logger.log('watcher:', 'running', core.buildWeb, file.name);
                server._watchWeb = lib.execProcess(core.buildWeb + " " + file.name, function(err, stdout, stderr) {
