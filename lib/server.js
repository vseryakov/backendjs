...
server.start = function()
{
    ...
    process.on('warning', function(err) { logger.warn(core.role, err.type, err.message, err.emiter, err.stack) });
    ...
    if (lib.isArg("-master") && !lib.isArg("-no-master")) {
        const opts = { role: cluster.isMaster ? "master" : "worker", noLocales: cluster.isMaster };
        return core.init(opts, function(err, opts) {
    ...
    if (lib.isArg("-web") && !lib.isArg("-no-web")) {
        const opts = { role: cluster.isMaster ? "server" : "web", noLocales: cluster.isMaster };
        return core.init(opts, function(err, opts) {
...
server.startMaster = function(options)
{
    if (cluster.isMaster) {
        ...
        core.runMethods("configureWorker", options, function() {
            ...
            logger.log('startWorker:', 'id:', cluster.worker.id, 'version:', core.version, 'home:', core.home, 'uid:', process.getuid(), 'gid:', process.getgid(), core.instance);
        });
    }
}
...
server.startWeb = function(options)
{
    ...
    if (cluster.isMaster) {
        ...
        logger.log('startWeb:', core.role, 'id:', cluster.worker.id, 'version:', core.version, 'home:', core.home, 'port:', core.port, core.bind, 'repl:', core.repl.portWeb, 'uid:', process.getuid(), 'gid:', process.getgid(), core.instance);
    }
}
...
server.handleChildProcess = function(child, type, method)
{
    ...
    child.on('exit', function (code, signal) {
        ...
        core.killBackend(type, "SIGKILL", function() {
            lib.checkRespawn(function() { server[method](); });
        });
    });
    child.unref();
}
...
{
    this.exiting = true;
    core.runMethods("shutdown" + lib.toTitle(core.role), () => {
        process.exit(0);
    });
}
...
{
    fs.writeFile(path.join(core.path.spool, core.role + ".pid"), process.pid, (err) => { if (err) logger.error("writePidfile:", err) });
}
