...
server.start = function()
{
    ...
    if (lib.isArg("-watch") && !lib.isArg("-no-watch")) {
        const opts = { role: "watcher", noDb: 1, noDns: 1, noConfigure: 1, noLocales: 1, noModules: 1, noWatch: 1 };
        return core.init(opts, function(err, opts) {
    ...
    if (lib.isArg("-master") && !lib.isArg("-no-master")) {
        const opts = { role: cluster.isMaster ? "master" : "worker", localTables: cluster.isMaster && !lib.isArg("-db-create-tables"), noLocales: cluster.isMaster };
        return core.init(opts, function(err, opts) {
    ...
    if (lib.isArg("-web") && !lib.isArg("-no-web")) {
        const opts = { role: cluster.isMaster ? "server" : "web", localTables: cluster.isMaster && !lib.isArg("-db-create-tables"), noLocales: cluster.isMaster };
        return core.init(opts, function(err, opts) {
...
server.startWatcher = function()
{
    ...
    core.watchdirs.forEach(function(dir) {
        core.watchFiles({ root: dir, files: /\.js$/, ignore: core.watchIgnore.rx }, _restart);
    });
    ...
    if (lib.isArray(core.watchWeb) && core.buildWeb) {
        ...
        logger.info('startWatcher:', core.buildWeb, webdirs);
        webdirs.forEach(function(dir) {
            core.watchFiles({ root: dir, match: core.watchMatch.rx, ignore: core.watchIgnore.rx }, (file) => {
                if (server._watchWeb || !file.stat.size) return;
                logger.log('watcher:', 'running', core.buildWeb, file.name);
                server._watchWeb = lib.execProcess(core.buildWeb + " " + file.name, (err, stdout, stderr) => {
                    if (stdout) console.log(stdout);
                    if (stderr) console.error(stderr);
                    if (err) console.error(err);
                    delete server._watchWeb;
                });
            }, (err) => {
                if (err) logger.error("startWatcher:", dir, err);
            });
        });
    }
    this.startProcess();
