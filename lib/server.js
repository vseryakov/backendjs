    ...
    if (cluster.isMaster) {
        ...
        d.run(function() {
            ...
            core.runMethods("configureMaster", options, { direct: 1 }, () => {
                ...
                if (core.webMaster) {
                    return server.startServer(options);
                }
        ...
        if (process.env.BKJS_WORKER_TYPE == "web") {
            this.startWeb(options);
        } else {
    ...
    if (cluster.isMaster) {
        ...
        d.run(function() {
            ...
            core.runMethods("configureServer", options, { direct: 1 }, () => {
                ...
                for (let i = 0; i < server.workers; i++) cluster.fork({ "BKJS_WORKER_TYPE": "web" });
