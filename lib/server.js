    ...
    if (cluster.isMaster) {
        ...
        d.run(function() {
            ...
            core.runMethods("configureMaster", options, { direct: 1 }, () => {
                ...
                logger.log('startMaster:', 'version:', core.version, 'home:', core.home, 'port:', core.port, 'uid:', lib.getuid(), core.instance);
            });
        });
    } else {
    ...
    if (cluster.isMaster) {
        ...
        d.run(function() {
            ...
            core.runMethods("configureServer", options, { direct: 1 }, () => {
                ...
                logger.log('startWeb:', core.role, 'version:', core.version, 'home:', core.home, 'port:', core.port, 'uid:', lib.getuid(), core.instance);
            });
        });
    } else {
...
server.startWebProcess = function(options)
{
    ...
    logger.log('startWeb:', core.role, 'id:', cluster.isMaster ? process.pid : cluster.worker.id, 'version:', core.version, 'home:', core.home, 'port:', core.port, core.bind, 'repl:', core.repl.portWeb, 'uid:', lib.getuid(), core.instance);
}
...
server.startWorker = function(options)
{
    ...
    core.runMethods("configureWorker", options, { direct: 1 }, () => {
        ...
        logger.log('startWorker:', 'id:', cluster.isMaster ? process.pid : cluster.worker.id, 'version:', core.version, 'home:', core.home, 'uid:', lib.getuid(), core.instance);
    });
}
...
server.startProcess = function()
{
    ...
    logger.log('startProcess:', core.role, 'version:', core.version, 'home:', core.home, 'port:', core.port, 'uid:', lib.getuid(), 'pid:', process.pid);
}
...
server.startWatcher = function()
{
    ...
    if (!lib.isFlag(core.watchdirs,__dirname)) {
        core.watchdirs.push(__dirname, __dirname + "/../modules");
        core.watchdirs.push(...lib.findFileSync(__dirname, { types: "d" }));
    }
    for (const dir of core.path.modules) {
        if (!lib.isFlag(core.watchdirs, dir)) core.watchdirs.push(dir);
    }
    logger.info('startWatcher:', core.watchdirs);
    for (const dir of core.watchdirs) {
        lib.watchFiles({ root: dir, files: /\.js$/, ignore: core.watchIgnore.rx, depth: 3 }, _restart);
    }
    ...
    if (lib.isArray(core.watchWeb) && core.buildWeb) {
        ...
        for (const dir of core.watchWeb) {
        ...
        for (const dir of webdirs) {
            lib.watchFiles({ root: dir, match: core.watchMatch.rx, ignore: core.watchIgnore.rx, recursive: true }, (file) => {
                if (!file.stat.size) return;
                var mode = path.extname(file.name);
                if (/^dir/.test(core.buildMode)) {
                    var dirs = path.dirname(file.name).split("/");
                    mode = dirs.slice(0, -lib.toNumber(core.buildMode.substr(3)) || dirs.length).join("/");
                }
                if (server._watchWeb[mode]) return;
                server._watchWeb[mode] = file.name;
                logger.log('watcher:', 'running', core.buildWebMode, mode, core.buildWeb, file.name);
                lib.execProcess(core.buildWeb + " " + file.name, (err, stdout, stderr) => {
                    delete server._watchWeb[mode];
