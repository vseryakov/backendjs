...
server.startWatcher = function()
{
    ...
    if (core.watchWeb) {
        ...
        for (const i in core.path.web) {
            if (!lib.isFlag(webdirs, core.path.web[i])) webdirs.push(core.path.web[i] + "/js", core.path.web[i] + "/css");
        }
        logger.info('startWatcher:', core.watchWeb, webdirs);
        webdirs.forEach(function(dir) {
            core.watchFiles(dir, /\.(js|css)$/, function(file) {
                if (server._watchWeb || !file.stat.size) return;
                logger.log('watcher:', 'running', core.watchWeb, file.name);
                server._watchWeb = lib.execProcess(core.watchWeb + " " + file.name, function(err, stdout, stderr) {
