...
const server = {
    ...
    args: [
        ...
        { name: "process-name", descr: "Path to the command to spawn by the monitor instead of node, for external processes guarded by this master" },
...
server.configureMaster = function(options, callback)
{
    ipc.initServer();
...
server.configureServer = function(options, callback)
{
    ipc.initServer();
...
server.start = function()
{
    ...
    if (lib.isArg("-watch") && !lib.isArg("-no-watch")) {
        const opts = { role: "watcher", noDb: 1, noIpc: 1, noJobs: 1, noDns: 1, noConfigure: 1, noLocales: 1, noModules: 1 };
    ...
    if (lib.isArg("-web") && !lib.isArg("-no-web")) {
        const opts = { role: cluster.isMaster ? "server" : "web", noJobs: 1, noLocales: cluster.isMaster, noModules: cluster.isMaster };
        return core.init(opts, (err, opts) => {
    ...
    if (cluster.isMaster) {
        ...
        d.run(function() {
            ...
            core.runMethods("configureMaster", options, { direct: 1 }, () => {
                ...
                // Master web server, run it inside this process
                if (core.webMaster) {
                    return server.startWebServer(options);
                }
    ...
    if (cluster.isMaster) {
        ...
        d.run(function() {
            ...
            core.runMethods("configureServer", options, { direct: 1 }, () => {
                ...
                for (let i = 0; i < server.workers; i++) cluster.fork();
...
server.startWorker = function(options)
{
    ...
    core.runMethods("configureWorker", options, { direct: 1 }, () => {
        ipc.sendMsg("worker:ready", { id: cluster.isMaster ? process.pid : cluster.worker.id });
