...
db.lmdbInitPool = function(options)
{
    ...
    pool.getLevelDB = function(callback) {
        ...
        try {
            if (!corelib.exists(this.create_if_missing)) options.create_if_missing = true;
    ...
    pool.query = function(client, req, opts, callback) {
        ...
        case "drop":
            client.select(table, table, opts, function(err, rows) {
                ...
                corelib.forEachLimit(rows, opts.concurrency || pool.concurrency, function(row, next) {
        ...
        case "search":
            ...
            client.select(opts.start || key, key, { begins_with: 1, count: opts.count }, function(err, items) {
                ...
                items.forEach(function(row) {
                    row = corelib.jsonParse(row.value);
                    if (!row) return;
                    if (selected) row = selected.map(function(x) { return [x, row[x] ]}).reduce(function(x,y) { x[y[0]] = y[1]; return x }, {});
                    rows.push(row);
                });
        ...
        case "list":
            var rc = [];
            var selected = self.getSelectedColumns(table, opts);
            corelib.forEachSeries(obj, function(o, next) {
        ...
        case "incr":
            ...
            client.get(key, function(err, item) {
                if (err) return callback(err);
                item = corelib.jsonParse(item);
                if (!item) item = obj; else nums.forEach(function(x) { item[x] = corelib.toNumber(item[x]) + obj[x]; });
...
db.nndbInitPool = function(options)
{
    ...
    pool.get = function(callback) {
        ...
        if (this.sock.type == utils.NN_REQ) {
            this.socknum = 1;
            this.callbacks = {};
            this.sock.setCallback(function(err, msg) { corelib.runCallback(pool.callbacks, msg); });
        }
    ...
    pool.query = function(client, req, opts, callback) {
        ...
        case "select":
            ...
            obj.id = this.socknum++;
            corelib.deferCallback(this.callbacks, obj, function(msg) { callback(null, msg.value) });
