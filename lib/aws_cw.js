...
aws.watchLogs = function(options, callback)
{
    ...
    logger.debug('watchLogs:', this.logwatcherGroup);

    this.watchLogsInit(options, (err, opts) => {

        lib.forEachSeries(aws.logwatcherGroups, (name, next) => {
            var q = {
                name: name,
