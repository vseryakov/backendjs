...
IpcSQSClient.prototype.pollQueue = function(options)
{
    var self = this;
    if (!this.url) return;
    if (!this.ready) return setTimeout(this.pollQueue.bind(this, options), this.options.retryInterval);
    ...
    aws.sqsReceiveMessage(url, this.options, (err, items) => {
        if (err) return setTimeout(self.pollQueue.bind(self, options), self.options.retyInterval);
