...
IpcSQSClient.prototype.pollQueue = function()
{
    var self = this;
    if (!this.url) return;
    if (!this.ready) return setTimeout(this.pollQueue.bind(this), this.options.retryInterval);

    var url = this.url;
    var name = url.split("/").pop();
    var metrics = this.metrics, mtimer;
    aws.sqsReceiveMessage(url, this.options, (err, items) => {
        if (err) return setTimeout(self.pollQueue.bind(self), self.options.retyInterval);
        ...
        lib.forEach(items, (item, next) => {
            ...
            if (!self.emit("message", msg, (err) => {
                ...
                if (!msg.noVisibility && (err && err.status >= 500 || msg.noWait)) {
                    if (err && msg.retryVisibilityTimeout > 0 && (!msg.retryVisibilityStatus || lib.isTrue(msg.retryVisibilityStatus, err.status))) {
                        const req = { QueueUrl: url, ReceiptHandle: item.ReceiptHandle, VisibilityTimeout: Math.round(msg.retryVisibilityTimeout/1000) };
            ...
            mtimer.end();
            self.schedulePollQueue(processed ? self.options.interval : self.options.retryInterval);
        });
    });
}
