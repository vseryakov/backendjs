...
function runMethod(mod, name, params, options, callback)
{
    ...
    ctx[name](params, (err) => {
        if (err) {
            const o = lib.isArray(options.logger_allow) ? options.logger_allow.reduce((a, b) => { a[b] = params[b]; return a }, {}) : params;
            logger.logger(options.logger_error || "error", "runMethods:", app.role, name, mod, err, o);
            if (options.stopOnError || params.stopOnError) {
                return callback(err);
            }
        }
        const stop = options.stopFilter || params.stopFilter;
        callback(typeof stop == "function" && stop(params) ? { code: "stop" } : null);
    });
}
