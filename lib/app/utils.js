...
app.runMethods = function(name, data, options, callback)
{
    ...
    if (options?.sync) {
        ...
        for (const p of mods) {
            ...
            if (stop && stop(data, options)) break;
        }
...
app.arunMethods = async function(name, data, options)
{
...
function runMethod(mod, name, data, options, callback)
{
    ...
    ctx[name](data, (err) => {
        ...
        callback(stopFilter && stopFilter(data, options) ? { code: "stop" } : null);
    });
}
 ...
 * @param {regexp} [exclude] - regexp what files or paths to exlude, default is ^_
