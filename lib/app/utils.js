 ...
 *
 * @param {string|boolean} [val] - json for JSON output, true for lib.inspect or reset to default
 * @memberof module:app
 * @method setLogInspect
 */
app.setLogInspect = function(val)
{
    const json = val === "json";
    logger.setOptions({ json });
    logger.setInspect(json || lib.toBool(val) ? lib.inspect : null, app.logInspect);
}
 ...
 *
 */
app.checkConfig = function(_callback)
{
...
app.origin = function()
{
    return `${app.role}:${process.pid}:${app.ipaddr}:${app.env.tag || ""}:${lib.clock()}`;
}
...
app.killBackend = function(name, signal, callback)
{
    ...
    lib.findProcess({ filter: `${app.process}: ` + (name ? `(${name})`: "") }, (_err, list) => {
        logger.debug("killBackend:", name, list);
        lib.forEach(list.map((x) => (x.pid)), (pid, next) => {
            try {
                process.kill(pid, signal)
            } catch (e) {
                logger.debug("killBackend:", name, pid, e)
            }
            setTimeout(() => {
                try {
...
app.createRepl = function(options)
{
    ...
    // Support history
    if (options?.file) {
        r.history = lib.readFileSync(options?.file, { list: '\n', offset: -options.size }).reverse();
        r.addListener('line', (code) => {
            if (code) {
                fs.appendFile(options?.file, code + '\n', lib.noop);
            } else {
                r.historyIndex++;
                r.history.pop();
            }
...
app.startRepl = function(port, bind, options)
{
    ...
    try {
        this.repl.server = net.createServer((socket) => {
            var repl = app.createRepl(lib.clone(options, {
                prompt: '> ',
                input: socket,
                output: socket,
                terminal: true,
...
/**
 * Parse a module and method from the given string, the format is `module.method`.
 * @param {string} name
 * @returns {array} empty if invalid or not found or `[context, method]`
 * @memberof module:app
 * @method getMethodContext
 * @example
 * # app.getMethodContext("dfrefr")
 * []
 *
 * # app.getMethodContext("jobs.submit")
 * { context: { name: "job", ....}, method: "submit" }
 */
app.getMethodContext = function(name)
{
    const parts = lib.isString(name).split('.');
    const method = parts.at(-1);
    const context = modules[parts.slice(0, -1).join(".")];
    return typeof context?.[method] === "function" ? [context, method] : [];
}
 ...
 *
 * The modules's **dependencies** property defines the position in the modules list and thus determines the order of calling methods.
 * The property contains other module name this module depend on, i.e. it must be placed after it.
 * if **dependencies** starts with **-** it means place this module before the other module.
 * A single __-__ means place it at the beginningh of the list.
 * A single __*__ means place it at the end of the list.
 *
 * @param {string} name - method name
 * @param {any} data - specific data passed to methods
 * @param {object} [options] - additional options to control method execution
 * @param {regexp} [options.allow] - regexp with allowed modules, in options only
 * @param {regexp} - [options.allowModules] - a regexp of the modules names to be called only
 * @param {boolean} - [options.stopOnError] - on first error stop and return, otherwise all errors are ignored and all modules are processed
 * @param {function} [options.stopFilter] - a function(data, options) that must return true in order to stop execution of other methods
 * @param {string} - [options.logger_error] - logger level, if not specified an error with status 200 will be reported with log level 'info' and other errors with level 'error'
 * @param {object} - [options.logger_inspect] - an object with inspect options to override current inspect parameters
 * @param {array} - [options.logger_allow] - a list of properties allowed in the log on error, this is to prevent logging too much or sensitive data
 * @param {boolean} - [options.parallel] - if true run methods for all modules in parallel using lib.forEach
 * @param {number} - [options.concurrency] - if a number greater than 1 run that many methods in parallel using lib.forEachLimit
 * @param {boolean} - [options.sync] - if true treat methods as simple functions without callbacks, methods MUST NOT call the second callback argument but simply return
 * @param {boolean} - [options.direct] - if true call all methods directly otherwise via setImmediate
 * @param [callback] - function to be called at the end
 * @memberof module:app
 * @method runMethods
 */
app.runMethods = function(name, data, options, callback)
{
    if (typeof options === "function") callback = options, options = null;

    if (!this._modules) app.sortModules();

    if (!this._methods) this._methods = Object.create(null);
    ...
    var allow = options?.allow || options?.allowModules || this.modules.methods[name];
    if (util.types.isRegExp(allow)) mods = mods.filter((x) => (allow.test(x)));

    logger.debug("runMethods:", name, this._modules, mods);

    if (options?.sync) {
        const stop = lib.isFunc(options.stopFilter);
        for (const p of mods) {
            logger.debug("runMethod:", app.role, name, p);
            modules[p][name](data, options);
            if (stop?.(data, options)) break;
        }
    ...
    if (options?.parallel) {
        lib.forEach(mods, (mod, next) => {
            runMethod(mod, name, data, options, next);
        }, callback, options.direct);
    } else

    if (options?.concurrency > 1) {
        lib.forEachLimit(mods, options.concurrency, (mod, next) => {
            runMethod(mod, name, data, options, next);
        }, callback, options.direct);
    } else {
...
/**
 * Async version of {@link module:app.runMethods}
 * @returns {undefined|Error|object} - an error
 * @memberof module:app
 * @method arunMethods
 * @async
 */

app.arunMethods = function(name, data, options)
{
    return new Promise((resolve, _reject) => {
        app.runMethods(name, data, options, (err) => {
            resolve(err);
        });
    });
}
 ...
 * @param {string} name - method name
 * @param {any|object} data - data for the method
 * @param {object} options - additional options to control method execution
 * @param {array} [options.logger_allow] - list of properties to be logged only on error instead of data
 ...
 *
 */
function runMethod(mod, name, data, options, callback)
{
    logger.debug("runMethod:", app.role, name, mod);
    const ctx = modules[mod];
    ctx[name](data, (err) => {
        if (err) {
            const o = lib.isArray(options?.logger_allow) ? options.logger_allow.reduce((a, b) => { a[b] = data?.[b]; return a }, {}) : data;
            logger.logger(options?.logger_error || "error", "runMethods:", app.role, name, mod, err, o);
            if (options.stopOnError) {
                return callback(err);
            }
        }
        const stopFilter = lib.isFunc(options?.stopFilter);
        callback(stopFilter?.(data, options) ? { code: "stop" } : null);
    });
}
 ...
 * **init** method it is called immediately with options passed as an argument.
 *
 * This is a synchronous function so it is supposed to be
 ...
 *
 * Only files from top level are loaded by default unless the depth is provided. {@link module:app.app.addModule addModule} is called automatically,
 ...
 *
 * Each module is put in the top level **modules** registry by name, the name must be defined as property **name**,
 * modules with empty name will be ignored.
 *
 ...
 * @param {regexp} [exclude] - regexp what files or paths to exclude, default is ^_
...
app.loadModules = function(dir, options)
{
    ...
    lib.findFileSync(path.resolve(dir), opts).sort().forEach((file) => {
        try {
            ...
            logger.debug("loadModules:", app.role, file, mod.name, mod.name ? "loaded" : "ignored");
        } catch (e) {
...
/**
 *  Sort modules according to the **dependencies** property.
 * @memberof module:app
 * @method sortModules
 */
app.sortModules = function()
{
    ...
    for (const m in modules) {
        ...
        let d = modules[m].dependencies;
        if (typeof d === "string" && d) {
            if (d[0] === "-" && d.length > 1) d = d.substr(1);
            if (!dependencies[d]) dependencies[d] = [];
            dependencies[d].push(m);
        }
    }

    // Sort groups
    const groups = Object.keys(dependencies).map((key, pos) => {
        Object.keys(dependencies).forEach((x, j) => {
            if (dependencies[x].includes(key)) pos += j;
        });
        return { key, pos, dependencies: dependencies[key] };
    }).sort((a, b) => (a.pos - b.pos));

    // Sort modules by group
    for (const g of groups) {
        for (const m of g.dependencies) {
            const d = modules[m].dependencies;
            let j;
            if (d[0] === "-") {
...
app.loadPackages = function(list, options)
{
    ...
    for (const pkg of lib.split(list)) {
        try {
            const mod = require.resolve(pkg).replace(/(node_modules\/[^/]+)\/(.+)/,"$1/");
            this.packages[pkg] = { path: mod };
            const cfg = lib.readFileSync(mod + app.config);
            if (cfg) {
                config = config + "\n" + cfg;
            ...
            const json = lib.readFileSync(mod + "package.json", { json: 1, logger: "error", missingok: 1 });
            if (json.version) {
