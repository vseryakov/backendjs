...
app.showHelp = function(options)
{
    ...
    Object.keys(modules).forEach((n) => {
        if (modules[n].args) args.push([n, modules[n].args]);
    });
    args.forEach((x) => {
        x[1].forEach((y) => {
            ...
            var dflt = lib.objGet(modules[x[0]], `${y.obj ? y.obj + "." : ""}${name}`);
            var line = (x[0] ? x[0] + '-' : '') + y.name + " - " + y.descr + (dflt ? ". Default: " + JSON.stringify(dflt) : "");
            console.log(" -" + line);
        });
    });
    process.exit(0);
...
/**
 * Make a HTTP request, see {$link module:httpGet} module for more details.
 * @memberof module:app
 */
app.httpGet = function(uri, params, callback)
{
    return modules.httpGet(uri, params, callback);
}

/**
 * Async version of httpGet method, returns a promise
 */
app.ahttpGet = util.promisify(app.httpGet.bind(app));
 ...
 * - login - login to use for access credentials
 * - secret - secret to use for access
...
app.sendRequest = function(options, callback)
{
    ...
    // Relative urls resolve against local host
    if (typeof url == "string" && !url.includes("://")) {
        url = `http://localhost:${this.port}${options.url}`;
    }

    return this.httpGet(url, options, (err, params) => {
...
/**
 * Async version of sendRequest, returns a promise
 */
app.asendRequest = util.promisify(app.sendRequest.bind(app));
 ...
 *
 * Only .js files from top level are loaded by default unless the depth is provided. {@link module:app.addModule} is called automatically,
 * it uses {@link module:lib.findFileSync} to locate the modules, options `depth`, `include or `exclude` can be provided
 *
