...
{
    if (val == "json") {
        logger.setInspect(val => (lib.stringify(val, (key, value) => {
            if (util.types.isNativeError(value)) {
                return Object.getOwnPropertyNames(value).reduce((a, b) => { a[b] = value[b]; return a }, {});
            }
            if (app.logInspect?.ignore?.test(key)) return undefined;
            return value;
        })))
    } else {
        logger.setInspect(lib.toBool(val) ? lib.inspect : null, app.logInspect);
    }
}
