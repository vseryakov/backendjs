...
{
    var args = this.args.filter((x) => (x.env && process.env[x.env] !== undefined)).
                         map((x) => ([this, x, process.env[x.env]]));

    for (const p in modules) {
        args.push(...lib.isArray(modules[p].args, []).
                     filter((x) => (x.env && process.env[x.env] !== undefined)).
                     map((x) => ([modules[p], x, process.env[x.env]])));
    }
...
app.processArg = function(options)
{
    ...
    var o = Object.create(null, { errnull: { value: true, enumerable: true, writable: true } });
    for (const p in arg) {
    ...
    try {
        ...
        if (o.obj) {
            ...
            if (o.obj.includes(".")) {
                ...
                if (!context) {
                    lib.objSet(mod, obj, context = Object.create(null));
                }
                o.obj = obj.join(".");
            } else {
                // Substitutions from the matched key
                ...
                // Skip bad prototype properties
                if (lib.badProperties.has(o.obj)) return false;

                if (!mod[o.obj]) {
                    mod[o.obj] = Object.create(null);
                }
                context = mod[o.obj];
        ...
        // Final bad prototype properties check
        if (lib.badProperties.has(o.name)) return false;
        ...
        if (arg.once) {
            if (!arg._once) arg._once = Object.create(null);
...
function _findArg(mod, key, val, pass, file)
{
    ...
    for (const i in mod.args) {
        ...
        if (matches) {
            return { mod, arg, key, val, pass, file, matches };
        }
    }
}
...
function _processArg(obj, key, val, arg, reverse, type)
{
    ...
    if (lib.badProperties.has(key)) return val;
    ...
    case "rlist":
        ...
        const k = key;
        arg.unique = 1;
        key = lib.split(val, arg.separator, arg).filter(x => !lib.badProperties.has(x));
        val = lib.split(k, arg.separator, arg);
        arg.array = 1;
        for (const p of key) {
            _processArg(obj, p, val, arg);
        }
        break;
    ...
    if (arg.merge) {
        ...
        case "json":
            ...
            for (const p in val) {
                if (lib.badProperties.has(p) || typeof obj[p] === "function") continue;
        ...
        case "map":
            ...
            for (const p in val) {
                const o = obj[p], v = val[p];
                if (lib.badProperties.has(p) || typeof o === "function") continue;
                if (v === arg.no_value || Array.isArray(arg.no_value) && arg.no_value.includes(v)) continue;
                if (arg.not_empty && lib.isEmpty(v)) continue;
                if (Array.isArray(o)) {
                    if (Array.isArray(v)) {
                        lib.arrayUpdate("update", o, v);
                    } else {
