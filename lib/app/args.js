 ...
 * @examples
 *  tag=T, instance.tag=T, runMode=M, appName=N, role=R, roles=dev, aws.region=R, aws.tags=T, env.NAME=V
 * @memberOf module:app
 */
 ...
 * - array - if true prepend a value to the list, to remove an item prepend it with `!!`
 * - push - for array: mode append a value
 ...
 * - maptype - usually `auto` to parse map values or can be any value type, for type `map`
 ...
 * - sort - sort array params
 * - unique - only keep unique items in lists
 * @memberOf module:app
 */
...
function _processArg(obj, key, val, arg, reverse, type)
{
    ...
    case "regexp":
        ...
        if (!val) return;
        if (val.test("") && !arg.empty) return logger.warn("processArg:", "wide open regexp", app.role, arg._conf, key, val);
        break;

    case "regexpobj":
        val = lib.toRegexpObj(obj[key], val, arg);
        if (!val) return;
        if (val.rx?.test("") && !arg.empty) return logger.warn("processArg:", "wide open regexp", app.role, arg._conf, key, val);
        if (!val.rx) val = null;
        break;
    ...
    case "regexpmap":
        ...
        if (!val) return;
        for (const i in val) {
            if (val[i]?.rx?.test("") && !arg.empty) return logger.warn("processArg:", "wide open regexp", app.role, arg._conf, key, val);
        }
        break;
 ...
 * @params {object[]} args - a list of objects in the format: { name: N, type: T, descr: D, min: M, max: M, array: B... }, all except name are optional.
