 ...
 * @param {boolean} [options.shell] - run the shell process
 * @param {boolean} [options.watch] - run the watcher process
 * @param {boolean} [options.nowatch] - ignore watcher mode
 * @param {boolean} [options.server] - run the main server process with api workers
 * @param {boolean} [options.noserver] - ignore server mode
 * @param {boolean} [options.api] - run the standalone api process
 * @param {boolean} [options.worker] - run the standalone worker process
 * @param {function} [callback] - run after initialization to ensure that modules are ready,
 * will NOT be called in shell or daemon mode
 * @memberof module:app
 * @method start
 */
app.start = function(options, callback)
{
    ...
    if (options?.shell || lib.isArg("-shell")) {
        ...
        return app.init(opts, (_err, opts) => {
    ...
    process.once('SIGINT', app.exit);
    process.once('SIGTERM', app.exit);
    ...
    if ((options?.server && !options?.noserver) || (lib.isArg("-server") && !lib.isArg("-no-server"))) {
        ...
        return app.init(opts, (_err, opts) => {
            if (cluster.isPrimary) {
                _startServer(opts, callback);
            } else {
                if (wtype === "web") {
                    _startWeb(opts, callback);
                } else {
                    _startWorker(opts, callback);
                }
            }
        });
    }
...
/**
 * Async version of {@link module:app.start}
 * @memberof module:app
 * @method astart
 * @async
 */
app.astart = function(options, callback)
{
    return new Promise((resolve, _reject) => {
        app.start(options, resolve);
    });
}
 ...
 *
 */
app.stop = function(options, callback)
{
    if (typeof options === "function") callback = options, options = null;

    app.runMethods("shutdown", options, { parallel: 1, direct: 1 }, () => {
        app.runMethods("shutdown" + lib.toTitle(app.role || "node"), options, { parallel: 1, direct: 1 }, () => {
            lib.tryCall(callback);
        });
    });
}
...
/**
 * Async version of {@link module:app.stop}
 * @memberof module:app
 * @method astop
 * @async
 */
app.astop = function(options)
{
    return new Promise((resolve, _reject) => {
        app.stop(options, resolve);
    });
}

/**
 * Terminates the server process, it is called on SIGTERM signal but can be called manually for graceful shutdown,
 * it runs `shutdown[Role]` methods before exiting. if app.exitTimeout defined it is used by default to always delay process exit.
 * @param {object} [options]
 * @param {int} [options.exitTimeout] - how long to wait before exiting, not immediately
 * @method exit
 * @memberof module:app
 */
app.exit = function(options)
{
    app.exiting = true;
    const timeout = options?.exitTimeout || app.exitTimeout;
    if (timeout) {
        setTimeout(() => { process.exit(0) }, timeout).unref();
    }
    app.stop(() => {
        if (!timeout) process.exit(0);
    });
}

// Setup worker environment
function _startServer(options, callback)
{
    if (app.pidFile) {
    ...
    d.run(() => {
        ...
        app.runMethods("configureServer", options, { direct: 1 }, () => {
            ...
            if (app.isOk("web")) {
                ...
                const workers = app.workers || Math.round(app.maxCPUs * (app.workerCpuFactor || 1));
                for (let i = 0; i < workers; i++) {
                    const child = cluster.fork({ "BKJS_WORKER_TYPE": "web" });
...
function _startWeb(options, callback)
{
...
function _startWorker(options, callback)
{
    // Setup IPC communication
    modules.ipc.initWorker();
