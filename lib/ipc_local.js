...
function IpcLocalClient(host, options)
{
    ...
    if (!this.options.interval) this.options.interval = 5;
    this.emit("ready");
}
...
{
    core.modules.ipc.sendMsg("cache:keys", { name: options && options.name }, function(msg) {
        callback(msg.value);
    });
}
...
IpcLocalClient.prototype.startListening = function(channel)
{
    var self = this;
    core.modules.ipc.sendMsg("queue:pop", {}, this.options, function(msg) {
        var rc = 0;
        if (msg.value) {
            rc = self.emit(channel, msg.value, function(err) {
                setTimeout(self.startListening.bind(self, channel), self.options.interval * 1000);
            });
        }
        if (!rc) setTimeout(self.startListening.bind(self, channel), self.options.interval * 1000);
    });
}

IpcLocalClient.prototype.publish = function(channel, msg, options, callback)
{
    core.modules.ipc.sendMsg("queue:push", { name: channel, value: msg }, options, function(msg) {
        if (typeof callback == "function") callback(msg.value);
    });
}
