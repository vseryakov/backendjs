...
{
    ipc.sendMsg("cache:clear", { name: pattern }, typeof callback == "function" ? function(m) { callback(null, m.value) } : null);
}
...
IpcLocalClient.prototype.get = function(key, options, callback)
{
    ...
    ipc.sendMsg("cache:get", opts, options, typeof callback == "function" ? function(m) { callback(null, m.value) } : null);
}

IpcLocalClient.prototype.put = function(key, val, options, callback)
{
    var ttl = (options && lib.toNumber(options.ttl)) || lib.toNumber(this.options.ttl);
    var opts = { name: key, value: val, expire: ttl ? Date.now() + ttl : 0, map: options && options.mapName  };
    ipc.sendMsg("cache:put", opts, options, typeof callback == "function" ? function(m) { callback(null, m.value) } : null);
}
...
IpcLocalClient.prototype.incr = function(key, val, options, callback)
{
    var ttl = (options && lib.toNumber(options.ttl)) || lib.toNumber(this.options.ttl);
    var opts = { name: key, value: val, expire: ttl ? Date.now() + ttl : 0, map: options && options.mapName };
    ipc.sendMsg("cache:incr", opts, options, typeof callback == "function" ? function(m) { callback(null, m.value) } : null);
}

IpcLocalClient.prototype.del = function(key, options, callback)
{
    var opts = { name: key, map: options && options.mapName };
    ipc.sendMsg("cache:del", opts, options, typeof callback == "function" ? function(m) { callback(null, m.value) } : null);
}
