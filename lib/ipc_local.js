...
function IpcLocalClient(host, options)
{
    ...
    this.options.interval = lib.toNumber(this.options.interval, { dflt: 5, min: 1 });
    this.emit("ready");
}
...
IpcLocalClient.prototype.startListening = function()
{
    var self = this;
    core.modules.ipc.sendMsg("queue:pop", {}, this.options, function(msg) {
        if (!msg.value || !self.emit(msg.channel || "message", msg.data, function(err) {
            setTimeout(self.startListening.bind(self), self.options.interval * 1000);
        })) {
            setTimeout(self.startListening.bind(self), self.options.interval * 1000);
        }
    });
