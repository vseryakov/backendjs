...
function IpcRedisClient(url, options)
{
    ...
    if (this.options.sentinel || this.options['sentinel-servers']) this.initSentinel();
}
...
IpcRedisClient.prototype.onSentinelConnect = function()
{
    ...
    lib.series([
      ...
      function(next) {
        ...
        if (self.connected) setTimeout(self.onSentinelConnect.bind(self), self.options.sentinel.retry_timeout || 500);
    });
}
