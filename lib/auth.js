...
mod.configure = function(options, callback)
{
    this.tables = {
        [this.table]: {
            login: { primary: 1, maxlength: mod.maxLength },            // Account login/username
            id: { type: "uuid", prefix: "u_", unique: 1 },              // Autogenerated ID
            name: { type: "text", maxlength: mod.maxLength },           // Account name
            status: { type: "text", maxlength: mod.maxLength },         // Status of the account
            secret: { priv: 1, maxlength: mod.maxLength },              // Signature secret or scrambled password
            api_secret: { priv: 1, maxlength: mod.maxLength },          // Secret for API access token
...
mod.prepareSecret = function(query, options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            mod.bcryptMod.hash(query.secret, mod.bcrypt, (err, hash) => {
        ...
        function(next) {
            ...
            mod.argon2Mod.hash(query.secret, mod.argon2).then((hash) => {
                query.secret = hash;
                next();
            }).catch(next);
        },
