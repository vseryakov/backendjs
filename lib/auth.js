...
mod.srp.toBuffer = function(n, len)
{
    return n instanceof this.BigInteger ? Buffer.from(n.toString(16).padStart(len*2, "0"), "hex") : Buffer.isBuffer(n) ? n : Buffer.from(n.padStart(len*2, "0"), "hex");
}
...
mod.srp.hash = function(...args)
{
    ...
    for (const i in args) {
        if (args[i] instanceof this.BigInteger) {
            h.update(this.toBuffer(args[i], 256));
        } else {
            h.update(args[i]);
        }
    }
...
mod.srp._hash = function(...args)
{
    const h = crypto.createHash('sha256');
    for (const i in args) h.update(args[i]);
    return h.digest();
}
...
mod.srp.verifier = function(user, secret, salt)
{
    ...
    const x = this.hash(this.toBuffer(s, 32), this._hash(user, ':', secret));
