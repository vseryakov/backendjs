...
mod.configureWeb = function(options, callback)
{
    ...
    api.app.all("/login", function(req, res) {
        if (!req.query.login || !req.query.secret) {
            return api.sendReply(res, { status: 417, message: api.checkErrmsg(req, null, "No username or password provided"), code: "NOLOGIN" });
        }
...
mod.checkSecret = function(user, secret, callback)
{
    ...
    api.bcrypt.compare(secret, user.secret, (err, rc) => {
        ...
        api.bcrypt.compare(hash, user.secret, (err, rc) => {
            callback(rc ? undefined : { status: 401, message: "Invalid user name or password" });
        });
    });
}
...
mod.get = function(query, options, callback)
{
    ...
    if (query.id) {
        db.select(this.table, { id: query.id }, { noscan: 1, cacheKeyName: "id", count: 1, first: 1 }, (err, row, info) => {
