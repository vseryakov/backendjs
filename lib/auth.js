...
mod.configure = function(options, callback)
{
    this.tables = {
        [this.table]: {
            ...
            expires: { type: "bigint", internal: 1, secure: 1 },        // Deny access to the account if this value is before current date, ms
...
mod.prepareSecret = function(query, options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            bcrypt.hash(query.secret, lib.toNumber(options.bcrypt, { min: 12, dflt: 12 }), (err, hash) => {
                query.secret = hash;
                next();
            });
        },
...
// Verify a plain text secret against existing user record, try all methods
mod.checkSecret = function(user, secret, callback)
{
    ...
    // Scrambled
    var hash1 = lib.sign(secret, user.login, "sha256");
    if (user.secret == hash1) return callback();

    // Salted
    var hash2 = this.salt ? lib.sign(this.salt, secret, "sha256") : NaN;
    if (user.secret === hash2) return callback();

    // Encrypted
    bcrypt.compare(secret, user.secret, (err, rc) => {
        if (rc) return callback();

        // Scrambled
        bcrypt.compare(hash1, user.secret, (err, rc) => {
            if (rc) return callback();

            if (!hash2) return callback({ status: 401, message: this.errInvalidSecret });

            // Salted
            bcrypt.compare(hash2, user.secret, (err, rc) => {
...
// Verify a plain text secret by user login or ID
mod.checkUserSecret = function(login, secret, callback)
{
    this.get(login, (err, row) => {
        if (err) return callback(err);
        this.checkSecret(row, secret, callback);
    });
}
