...
//
// The `options.cleanup_rules` can be an object with property names and the values 0, or 1 or a
// `function(name, column, options)`` that must return 0 or 1.
//
...
api.cleanupResult = function(table, data, options)
{
    ...
    var roles = lib.strSplit(options.account && options.account.type);
    var tables = lib.strSplit(table);
    var row, nrows, nrow;

    for (let i = 0; i < tables.length; i++) {
        const dbcols = db.getColumns(tables[i], options);
        const rules = options.cleanup_rules || this.cleanupRules[tables[i]] || lib.empty;
        for (const p in dbcols) {
            col = dbcols[p] || lib.empty;
            r = cols[p] = typeof rules[p] == "number" ? rules[p] ? 1 : 0 :
                          typeof rules[p] == "function" ? rules[p](tables[i], p, col, options) ? 1 : 0 :
                          col.priv ? 0 :
                          col.pub ? 1 :
                          col.pub_staff ? options.isStaff || options.isInternal ? 1 : 0 :
                          col.pub_admin ? options.isAdmin || options.isInternal ? 1 : 0 :
                          options.isInternal ? 1 : 0;
            if (r && !options.isInternal) {
                if (col.priv_types && lib.isFlag(roles, col.priv_types)) cols[p] = 0; else
                if (col.pub_types && !lib.isFlag(roles, col.pub_types)) cols[p] = 0;
            }
            all++;
            pos += r;
        }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        nrow = null;
        for (const p in row) {
            if (cols[p] === 0 || (typeof cols[p] == "undefined" && options.cleanup_strict)) {
                // Lazy copy on modify
    ...
    logger.debug("cleanupResult:", table, rows.length, all, pos, cols, options);
    return data;
}
...
// Return a copy of a request with only properties defined in the `allow` list if provided in addition to
// the `query`, `account`` and properties defined in the `api-request-cleanup`` list
api.copyRequest = function(req, allow)
{
    return ["query", "account", ...lib.isArray(allow, []), ...this.requestCleanup].reduce((a, b) => {
        a[b] = req[b];
        return a;
    }, {});
}
