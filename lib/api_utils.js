...
api.cleanupResult = function(table, data, options)
{
    ...
    for (let i = 0; i < tables.length; i++) {
        ...
        for (const p in dbcols) {
            ...
            cols[p] = typeof rules[p] == "number" ? rules[p] ? 1 : 0 :
            ...
            if (col.pub_types && cols[p] && !lib.isFlag(roles, col.pub_types)) cols[p] = 0;
            all++;
            pos += cols[p];
        }
    }
...
//        api.clearQuery("bk_user", req.query, { filter: ["admin","secure"] })
//        api.clearQuery("bk_user", req.query, { filter: ["admin","!secure"], keep: /^__/ })
