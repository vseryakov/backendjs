    ...
    for (let i = 0; i < tables.length; i++) {
        ...
        for (const p in dbcols) {
            ...
            r = cols[p] = typeof rules[p] == "number" ? rules[p] ? 1 : 0 :
                          ...
                          col.priv ? 0 :
                          col.pub ? 1 :
                          col.pub_staff ? options.isStaff ? 1 : 0 :
                          col.pub_admin ? options.isAdmin ? 1 : 0 :
                          options.isInternal ? 1 : 0;
            if (r) {
                if (col.pub_types && !lib.isFlag(roles, col.pub_types)) cols[p] = 0; else
                if (col.priv_types && lib.isFlag(roles, col.priv_types)) cols[p] = 0;
            }
            all++;
            pos += r;
        }
...
//        api.clearQuery("bk_user", req.query, "admin")
//        api.clearQuery("bk_user", req.query, { filter: "admin" })
//        api.clearQuery("bk_user", req.query, { filter: ["admin"] })
//        api.clearQuery("bk_user", req.query, { filter: ["!pub"] })
//        api.clearQuery("bk_user", req.query, { filter: ["admin","priv"] })
//        api.clearQuery("bk_user", req.query, { filter: ["admin","!priv"], keep: /^__/ })
