...
// Expose mime via core
core.mime = require("mime");
...
core.sendmail = function(options, callback)
{
    ...
    try {
        ...
        case "file:":
            fs.writeFile(`${core.path.tmp}/email-${options.to}.json`, lib.stringify(options), typeof callback == "function" && callback || lib.noop);
            break;
        ...
        case "sendgrid:":
            this.httpGet("https://api.sendgrid.com/v3/mail/send",
                         { headers: {
                             ...
                             if (err) logger.error("sendmail:", err, options.from, options.to, options.subject);
                             lib.tryCall(callback, err);
                         });
            break;

        default:
            var mailer = require("nodemailer").createTransport(core.smtp || { host: "localhost", port: 25 });
