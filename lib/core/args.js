...
// - array - if true prepend a value to the list
...
core.processArg = function(options)
{
    ...
    try {
        ...
        logger.debug("processArg:", core.role, options.file, mod.name, type || "str", o.obj, o.name, "(" + key + ")", "=", val === null ? "null" : val);
        ...
        case "file":
            ...
            try {
                ...
                logger.error('processArg:', core.role, options.file, mod.name, o.name, val, e);
            }
            break;
        ...
        logger.error("processArg:", core.role, options.file, mod.name, o.name, val, e.stack);
    }
...
{
    var prefix = mod === core ? "-" : "-" + mod?.name?.replaceAll(".", "-") + "-";
...
function _processArg(obj, key, val, arg, reverse, type)
{
    ...
    case "regexp":
        ...
        if (!val) return;
        if (val.test("") && !arg.empty) return logger.warn("processArg:", "wide open regexp", core.role, arg._conf, key, val);
        break;

    case "regexpobj":
        val = lib.toRegexpObj(obj[key], val, arg);
        if (!val) return;
        if (val.rx?.test("") && !arg.empty) return logger.warn("processArg:", "wide open regexp", core.role, arg._conf, key, val);
        if (!val.rx) val = null;
        break;
