...
// Parse config lines for the file or other place, supports the following sections:
// - tag:T, runMode:M, appName:N, hostName:N, domain:D
core.parseConfig = function(data, pass, file)
{
    // Prepare for possible sections in the config
    var opts = {
    ...
    var argv = lib.configParse(data, opts);
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        mod.args.forEach(function(x) {
            ...
            try {
                ...
                case "url":
                case "json":
                    val = put(context, o.name, val, o, x.reverse, type);
                    break;
                ...
                default:
                    val = put(context, o.name, val, o, x.reverse);
                }
