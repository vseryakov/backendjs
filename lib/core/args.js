...
core.processArgs = function(mod, argv, pass, file)
{
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        mod.args.forEach(function(x) {
            ...
            if (util.types.isRegExp(x.regexp) && !x.regexp.test(val)) return;
            ...
            if (typeof x.onparse == "function") {
                val = x.onparse.call(mod, val, o);
            }
            ...
            try {
                ...
                logger.debug("processArgs:", core.role, file, type || "str", o.obj, o.name, "(" + key + ")", "=", val === null ? "null" : val);
                ...
                case "json":
                case "js":
                    val = _processArg(context, o.name, val, o, x.reverse, type);
                    break;
                ...
                case "path":
                    ...
                    _processArg(context, o.name, val, o, x.reverse, type);
                    break;

                case "file":
                    if (!val) break;
                    try {
                        _processArg(context, o.name, fs.readFileSync(path.resolve(val)), x);
                    } catch (e) {
                        logger.error('processArgs:', core.role, file, o.name, val, e);
                    }
                    break;
                ...
                default:
                    val = _processArg(context, o.name, val, o, x.reverse);
                }
...
function _processArg(obj, key, val, x, reverse, type)
{
    if (reverse) {
        var v = val;
        val = key;
    ...
    case "number":
        ...
        if (x.multiplier) val *= x.multiplier;
        if (x.ceil) val = Math.ceil(val);
        if (x.floor) val = Math.floor(val);
        break;
    case "regexp":
        if (!val) break;
        val = lib.toRegexp(val, x.regexp);
        if (!val) return;
        if (val && val.test("") && !x.empty) return logger.warn("processArgs:", "wide open regexp", core.role, x._conf, key, val);
        break;
    case "regexpobj":
        val = lib.toRegexpObj(obj[key], val, x);
        if (!val) return;
        if (val.rx && val.rx.test("") && !x.empty) return logger.warn("processArgs:", "wide open regexp", core.role, x._conf, key, val);
        break;
    case "regexpmap":
        val = lib.toRegexpMap(obj[key], val, x);
        if (!val) return;
        for (const i in val) {
            if (val[i].rx.test("") && !x.empty) return logger.warn("processArgs:", "wide open regexp", core.role, x._conf, key, val);
        }
        break;
    case "list":
    ...
    case "rlist":
        ...
        x.array = 1;
        for (const i in key) _processArg(obj, key[i], val, x);
        break;
    case "map":
        ...
        reduce((a, b) => {
          ...
          if (x.maptype) a[b[0]] = lib.toValue(a[b[0]], x.maptype, x);
          return a;
      }, {});
        break;
    case "url":
        ...
        try {
            ...
            return logger.warn("processArgs:", core.role, x._conf, key, val, e);
        }
        break;
    case "js":
    case "json":
    ...
    if (x.flatten && Array.isArray(val)) {
        for (const i in val) _processArg(obj, val[i], key, x);
    } else
    ...
    if (x.merge) {
        switch (type) {
        case "json":
            for (const p in val) if (typeof obj[p] != "function") obj[p] = val[p];
            break;
