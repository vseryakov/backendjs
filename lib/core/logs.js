...
{
    this.modules.db.put("bk_property", { name: 'logwatcher:' + file, value: pos }, { pool: this.logwatcherPool || this.modules.db.local }, (err) => {
...
core.watchLogsSend = function(options, callback)
{
    ...
    lib.forEvery(lib.objKeys(errors), (type, next) => {
        ...
        case "ses://":
            core.modules.aws.sesSendEmail(d[2], subject, text, { from: core.logwatcherFrom }, (err) => {
                if (err) logger.info("watchLogs:", err);
                next()
            });
            break;
        ...
        default:
            core.sendmail({ from: core.logwatcherFrom, to: d[2], subject: subject, text: text }, (err) => {
                if (err) logger.info("watchLogs:", err);
                next()
            });
        }
