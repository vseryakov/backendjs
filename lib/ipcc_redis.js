...
IpcRedisClient.prototype.initClient = function(host, port)
{
    ...
    var max_attempts = lib.toNumber(this.options.max_attempts, { min: 0 });
    this.options.retry_strategy = function(options) {
        logger.logger(options.attempt == 2 ? "error": "dev", "initClient:", self.url, options);
        if (max_attempts > 0 && options.attempt > max_attempts) undefined;
        return Math.min(options.attempt * 200, retry_max_delay);
    }
