...
//
//    ipc-client=redis://host1?bk-servers=host2,host3
...
{
    if (/^redisq?:/.test(url)) return new IpcRedisClient(url, options);
}
...
function IpcRedisClient(url, options)
{
    ...
    this.applyOptions();
    this.initClient(this.hostname, this.port);
}
...
IpcRedisClient.prototype.initClient = function(host, port)
{
    ...
    var connect_timeout = lib.toNumber(this.options.connect_timeout, { min: 60000, dflt: 900000 });
    var retry_max_delay = lib.toNumber(this.options.retry_max_delay, { min: 1000, dflt: 30000 });
    var max_attempts = lib.toNumber(this.options.max_attempts, { min: 1, dflt: 10 });
    var self = this;
    this.options.retry_strategy = function(options) {
        logger.logger(options.attempt == 2 ? "error": "dev", "initClient:", self.url, options);
        if (options.total_retry_time > connect_timeout || (lib.isArray(self.options.servers) && options.attempt > max_attempts)) {
            setTimeout(self.onError.bind(self, { code: 'CONNECTION_BROKEN', message: options.error && options.error.message }), 100);
            return undefined;
        }
    ...
    this.client = client;
    logger.debug("initClient:", this.url, "connecting:", host, port);
}

IpcRedisClient.prototype.onConnect = function()
{
    logger.debug("onConnect:", this.url, "connected", this.client.address);
    this.emit("connect");
}
...
IpcRedisClient.prototype.onError = function(err)
{
    logger.error("onError:", core.role, err);
    if (err.code == 'CONNECTION_BROKEN') {
        this.emit("disconnect");
        var host = this.options.servers.shift();
        if (!host) return;
        this.options.servers.push(host);
        logger.debug("disconnect:", core.role, this.url, "trying", host, "of", this.options.servers);
        setTimeout(this.initClient.call(this, host), this.options.reconnect_timeout || 50);
    }
}
...
IpcRedisClient.prototype.limiter = function(options, callback)
{
    this.client.eval(client.scripts.limiter, 7,
        ...
        callback(lib.toNumber(rc[0]), { queueName: this.queueName, delay: lib.toNumber(rc[0]), count: lib.toNumber(rc[1]), total: lib.toNumber(rc[2]), elapsed: lib.toNumber(rc[3]) });
    });
}
...
{
    this.client.zadd(this.options.queue, Date.now(), job, callback);
}

IpcRedisClient.prototype._monitorQueue = function()
{
    var self = this;
...
IpcRedisClient.prototype.monitorQueue = function()
{
    if (this.options.queue && !this._monitorTimer && this.options.visibilityTimeout) {
        this._monitorTimer = setInterval(this._monitorQueue.bind(this), this.options.monitorInterval || this.options.visibilityTimeout);
        this._monitorQueue();
    }
}

IpcRedisClient.prototype.pollQueue = function()
{
    var self = this;
    if (!this.options.queue) return;
    if (!this.client.ready) return self.schedulePollQueue(self.options.retryInterval);
    ...
    this.client.eval(script, 2, this.options.queue, Date.now(), (err, rc) => {
        ...
        if (!rc || !rc[0] || err) {
            self.schedulePollQueue(self.options.retryInterval);
            return;
        }
        var vtimer, data = rc[0];
        ...
        if (vtimeout && !msg.noWait) {
            ...
            if (msg.visibilityTimeout > 0) self.client.zadd(self.options.queue + "#", Date.now(), data);
            vtimer = setInterval(() => {
                if (msg.done) return;
