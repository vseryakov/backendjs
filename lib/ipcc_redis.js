...
IpcRedisClient.prototype.initClient = function(host, port)
{
    ...
    this.options.retry_strategy = function(options) {
        ...
        if (self.options.max_attempts > 0 && options.attempt > self.options.max_attempts) undefined;
        return Math.min(options.attempt * 200, self.options.retry_max_delay);
    }
...
IpcRedisClient.prototype.stats = function(options, callback)
{
    ...
    lib.parallel([
        function(next) {
            ...
            self.client.eval(client.scripts.stats, 1, chan, (err, count) => {
                if (!err) {
...
{
    if (!this.client.ready) return self.schedulePollQueue(options, this.options.retryInterval);
