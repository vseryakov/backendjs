...
{
    var self = this;
    var file = path.join(this.poolOptions.path || core.path.spool, (this.url || core.name)  + ".db");
    new bksqlite.Database(file, this.poolOptions.readonly ? bksqlite.OPEN_READONLY : 0, function(err) {
        if (err) {
            // Do not report errors about not existing databases
            if (err.code != "SQLITE_CANTOPEN" || !self.poolOptions.silent) logger.error('open:', file, err);
            return callback(err);
        }
        var db = this;
        ...
        var opts = [];
        if (typeof self.poolOptions.cache_size != "undefined") opts.push("PRAGMA cache_size=-" + self.poolOptions.cache_size);
        if (typeof self.poolOptions.temp_store != "undefined") opts.push("PRAGMA temp_store=" + self.poolOptions.temp_store);
        if (typeof self.poolOptions.journal_mode != "undefined") opts.push("PRAGMA journal_mode=" + self.poolOptions.journal_mode);
        if (typeof self.poolOptions.locking_mode != "undefined") opts.push("PRAGMA locking_mode=" + self.poolOptions.locking_mode);
        if (typeof self.poolOptions.synchronous != "undefined") opts.push("PRAGMA synchronous=" + self.poolOptions.synchronous);
        if (typeof self.poolOptions.read_uncommitted != "undefined") opts.push("PRAGMA read_uncommitted=" + self.poolOptions.read_uncommitted);
        if (typeof self.poolOptions.busy_timeout != "undefined") opts.push("SELECT busy_timeout(" + self.poolOptions.busy_timeout + ")");
