...
db.sqliteInitPool = function(options)
{
    ...
    options.type = "sqlite";
    options.file = path.join(options.path || core.path.spool, (options.url || core.name)  + ".db");
    ...
    pool.serialNum = 0;
    pool.validate = function(client) {
        return this.serialNum == client.poolSerial;
    }
...
db.sqliteSetup = function(client, callback)
{
    ...
    if (this.watchfile && !this.serialNum) {
        ...
        fs.watch(this.watchfile, function(event, filename) {
            logger.info('db.watch:', self.name, event, filename, self.watchfile, "#", self.serialNum);
            self.serialNum++;
            self.destroyAll();
        });
    }
