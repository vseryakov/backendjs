...
aws.s3List = function(path, options, callback)
{
    ...
    if (uri.key) options = lib.cloneObj(options, "key", uri.key, "secret", uri.secret);
    ...
    lib.doWhilst(
      function(next) {
          self.queryS3(uri.bucket, "", options, function(err, params) {
              ...
              options.query.marker = params.obj.ListBucketResult.NextMarker || rows[rows.length - 1].Key;
              next(err);
          });
      },
      function() {
