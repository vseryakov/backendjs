...
{
    return path.split('/').map(function(p) { return uriEscape(p) }).join('/');
}
...
aws.signS3 = function(method, bucket, path, options)
{
    ...
    if (region != 'us-east-1') {
        ...
        var q = Object.keys(options.query || []).sort().map(function(x) {
            return uriEscape(x) + (options.query[x] == null ? "" : "=" + uriEscape(options.query[x]));
        });
        if (q) path += "?" + q;
        if (!options.headers["x-amz-content-sha256"]) options.headers["x-amz-content-sha256"] = lib.hash(options.postdata || "", "sha256", "hex");
        var opts = {};
        this.querySign(region, "s3", host, method || "GET", path, options.postdata, options.headers, opts);
        if (options.url) {
