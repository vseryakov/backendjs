...
mod.startProcess = function()
{
    ...
    this.child.on('exit', (code, signal) => {
        ...
        app.killBackend("", "", () => {
            lib.checkRespawn(this.startProcess.bind(this));
        });
    });

    process.stdin.pipe(this.child.stdin);

    logger.log('startProcess:', app.role, 'version:', app.version, 'home:', app.home, 'port:', app.port, 'uid:', lib.getuid(), 'pid:', process.pid, 'app:', app.appName, app.runMode, app.instance);
}
