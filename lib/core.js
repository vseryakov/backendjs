...
var core = {
    ...
    args: [ { name: "help", type: "callback", callback: function() { this.showHelp() }, descr: "Print help and exit" },
            ...
            { name: "watch-ignore", type: "regexp", descr: "Files to be ignored by the watcher" },
            { name: "watch-web", descr: "Command to run on web files modifications in web/js and web/css directories, to be used with tools like minify/uglify, ready to use command is `bkjs web-bundle -file`" },
...
core.processArgs = function(ctx, argv, pass)
{
    ...
    function put(obj, key, val, x, reverse, type) {
        ...
        if (x.flatten && Array.isArray(val)) {
            for (var i in val) put(obj, val[i], key, x);
        } else
