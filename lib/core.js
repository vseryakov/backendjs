...
const core = {
    ...
    // for manual installations `bkjs init-server` will create a user with this id
    uid: 0,
    gid: 0,
    umask: '0002',
    ...
    args: [ { name: "help", type: "callback", callback: function() { this.showHelp() }, descr: "Print help and exit" },
            ...
            { name: "syslog", type: "callback", callback: function(v) { logger.setSyslog(lib.isNumeric(v) ? lib.toNumber(v) : v || true) }, descr: "Write all logging messages to syslog, connect to the local syslog server over Unix domain socket, pass 1 or syslog facility to enable, 0 to disable", pass: 1 },
...
core.runMethods = function(name, params, options, callback)
{
    ...
    if (!Array.isArray(mods)) {
        mods = this.methods[name] = Object.keys(this.modules).filter((mod) => (this.modules[mod] && typeof this.modules[mod][name] == "function"));
    }
