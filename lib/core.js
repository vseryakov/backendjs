...
core.init = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            if (options.noWatch || core.noWatch) return next();
            // Can only watch existing files
            var confs = [core.confFile, core.confFile + ".local"];
            for (const p in core.packages) {
                if (core.packages[p].config) confs.push(core.packages[p].path + "/etc/config");
            }
...
core.processArgs = function(ctx, argv, pass, file)
{
    ...
    function put(obj, key, val, x, reverse, type) {
        ...
        case "regexp":
            ...
            if (!val) return;
            if (val && val.test()) return logger.warn("processArgs:", "wide open regexp", core.role, file, key, val);
            break;
        case "regexpobj":
            val = lib.toRegexpObj(obj[key], val, x);
            if (!val) return;
            if (val.rx && val.rx.test()) return logger.warn("processArgs:", "wide open regexp", core.role, file, key, val);
            break;
        case "regexpmap":
            val = lib.toRegexpMap(obj[key], val, x);
            if (!val) return;
            for (var i in val) {
                if (val[i].rx.test()) return logger.warn("processArgs:", "wide open regexp", core.role, file, key, val);
            }
            break;
        case "list":
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        ctx.args.forEach(function(x) {
            ...
            try {
                ...
                logger.debug("processArgs:", core.role, file, type || "str", ctx.name + "." + x._name, "(" + key + ")", "=", val === null ? "null" : val);
                switch (type) {
                case "none":
                    break;
                case "bool":
                ...
                case "file":
                    if (!val) break;
                    try { put(o.ctx, o.name, fs.readFileSync(path.resolve(val)), x); } catch(e) { logger.error('processArgs:', core.role, file, o.name, val, e); }
                    break;
                case "callback":
                    if (!x.callback) break;
                    if (typeof x.callback == "string") {
                        ctx[x.callback](val, o, pass);
                    } else
                    if (typeof x.callback == "function") {
                        x.callback.call(ctx, val, o, pass);
                    }
                    break;
                default:
