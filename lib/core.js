...
const core = {
    ...
    watchdirs: [],
    watchIgnore: {},
    watchMatch: lib.toRegexpObj(null, "\\.(js|css)$"),
    ...
    args: [ { name: "help", type: "callback", callback: function() { this.showHelp() }, descr: "Print help and exit" },
            ...
            { name: "log-ignore", type: "callback", callback: function(v) { logger.ignore = lib.toRegexp(v) }, descr: "Regexp with property names which must not be exposed in the log" },
            ...
            { name: "force-uid", type: "callback", callback: function(){ lib.dropPrivileges() }, descr: "Drop privileges if running as root by all processes as early as possibly, this reqiures uid being set to non-root user. A convenient switch to start the backend without using any other tools like su or sudo.", pass: 1 },
...
core.runMethod = function(module, name, options, callback)
{
    ...
    ctx[name](options, function(err) {
        if (err) {
            ...
            if (logger.ignore && !log.hide) log = lib.objClone(log, "hide", logger.ignore);
