...
//
// Module name can dontain dots, meaning to place the module under hierarchy of namespaces.
// Intermediate objects must not be existing modules.
//
...
core.addModule = function(...args)
{
    for (const mod of args) {
        ...
        if (!mod?.name || typeof mod?.name != "string") {
            logger.trace("addModule:", "missing name", mod);
            continue
        }
        const name = mod.name.trim();
        if (name[0] == "_") continue;
        if (root[mod.name]) {
            logger.trace("addModule:", "already registered", name);
            continue
        }
        root[name] = mod;
        ...
        while (names.length) {
            ...
            if (root[part].name) {
                logger.trace("addModule:", "namespace exists", part, "in", name);
                break;
            }
            if (names.length) {
                root = root[part];
...
// closer to the top the module will be. The position of a module in the `modules` registry will define the order of `runMethod` call.
//
