...
core.processArgs = function(ctx, argv, pass)
{
    ...
    function put(obj, key, val, x, reverse, type) {
        ...
        case "regexp":
            ...
            if (!val) return;
            if (val && val.test()) return logger.warn("processArgs:", "wide open regexp", key, val);
            break;
        case "regexpobj":
            val = lib.toRegexpObj(obj[key], val, x);
            if (!val) return;
            if (val.rx && val.rx.test()) return logger.warn("processArgs:", "wide open regexp", key, val);
            break;
        case "regexpmap":
            val = lib.toRegexpMap(obj[key], val, x);
            if (!val) return;
            for (var i in val) {
                if (val[i].rx.test()) return logger.warn("processArgs:", "wide open regexp", key, val);
            }
            break;
        case "list":
        ...
        case "url":
            ...
            try {
                ...
                return logger.warn("processArgs:", key, val, e);
            }
            break;
        case "json":
            if (!val) break;
