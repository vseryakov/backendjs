...
const core = {
    ...
    // WebSockets config
    ws: { port: 0, bind: "0.0.0.0", ping: 30000 },
    ...
    modules: {},
    // By default allow some modules, must be allowed in the config otherwise
    allowModules: {
        "": lib.toRegexpObj(null, "^(bk_user)$"),
        shell: lib.toRegexpObj(null, "^(bk_shell)$"),
        server: lib.toRegexpObj(null, "^(?!x)x$"),
        master: lib.toRegexpObj(null, "^(?!x)x"),
    },
    denyModules: {},
    allowPackages: [],
    packages: {},
    ...
    args: [ { name: "help", type: "callback", callback: function() { this.showHelp() }, descr: "Print help and exit" },
            ...
            { name: "force-uid", type: "callback", callback: function() { lib.dropPrivileges(this.uid, this.gid) }, descr: "Drop privileges if running as root by all processes as early as possibly, this reqiures uid being set to non-root user. A convenient switch to start the backend without using any other tools like su or sudo.", pass: 1 },
            ...
            { name: "instance-region", obj: 'instance', descr: "Set instance region explicitely, skip all meta data checks for it", pass: 1 },
            ...
            { name: "allow-modules-?(.+)?", obj: "allow-modules", type: "regexpobj", strip: "allowModules", descr: "A regexp with modules name to be loaded on startup, only matched modules will be loaded, basename of the file is matched only, no path or extension, it is per role or global if no role is provided", pass: 1 },
            { name: "deny-modules-?(.+)?", obj: "deny-modules", type: "regexpobj", strip: "denyModules", descr: "A regexp with modules names that will never be loaded even if allowed, this is for blacklisted modules, can be per role or global for all processes", pass: 1 },
...
// - denyModules - which modules should not be loaded, regexp or regexpobj
// - allowModules - which modules to load, regexp or regexpobj
core.init = function(options, callback)
{
    ...
    lib.series([
        function(next) {
            ...
            [core.confFile, core.confFile + ".local"].forEach(function(file) {
        ...
        function(next) {
            ...
            lib.forEach(files, function(file, next2) {
                fs.stat(file, function(err) {
                    logger.debug("init:", "watch:", file);
                    if (!err) fs.watch(file, function (event, filename) {
                        core.setTimeout(file, function() { core.loadConfig(file) }, lib.randomInt(1000, 5000));
                    });
                    next2();
                });
            }, next);
...
core.loadConfig = function(file, callback)
{
    ...
    fs.readFile(file || "", (err, data) => {
        if (!err) core.parseConfig(data, 0, file);
...
core.runMethods = function(name, params, options, callback)
{
    ...
    if (options.parallel || params.parallel) {
        lib.forEach(Object.keys(this.modules), (mod, next) => {
            if (util.isRegExp(allow) && !allow.test(mod)) return next();
            runMethod(mod, name, params, options, next);
        }, callback);
    } else
    if (options.concurrency > 1 || params.concurrency > 1) {
        lib.forEachLimit(Object.keys(this.modules), options.concurrency || params.concurrency, (mod, next) => {
            if (util.isRegExp(allow) && !allow.test(mod)) return next();
            runMethod(mod, name, params, options, next);
        }, callback);
    } else {
        lib.forEachSeries(Object.keys(this.modules), (mod, next) => {
            if (util.isRegExp(allow) && !allow.test(mod)) return next();
            runMethod(mod, name, params, options, next);
        }, callback);
    }
...
function runMethod(mod, name, params, options, callback)
{
    ...
    if (!ctx || typeof ctx[name] != "function") return callback();
    logger.debug("runMethod:", core.role, name, mod);
    ctx[name](params, (err) => {
        if (err) {
            logger.errorWithOptions(err, options.logger_error ? options : params, "runMethods:", core.role, name, mod, err, params);
...
//  - denyModules - a regexp/regexpobj with modules name(s) to be excluded from loading, the basename of a file is checked only
//  - allowModules - a regexp/regexpobj with modules name(s) to be loaded only
//
...
core.loadModules = function(dir, options, callback)
{
    if (typeof options == "function") callback = options, options = null;
    if (!options) options = {};
    var denyModules = options.denyModules ? util.isRegExp(options.denyModules) ? options.denyModules:
                                            util.isRegExp(options.denyModules.rx) ? options.denyModules.rx : null : null;
    var allowModules = options.allowModules ? util.isRegExp(options.allowModules) ? options.allowModules:
                                              util.isRegExp(options.allowModules.rx) ? options.allowModules.rx: null : null;
    ...
    lib.findFileSync(path.resolve(dir), { depth: 1, types: "f", include: /\.js$/ }).sort().forEach(function(file) {
        try {
            ...
            logger.debug("loadModules:", core.role, file, mod.name, "loaded", "allow:", allowModules);
        } catch (e) {
    ...
    for (const i in names) {
        this.addModule(names[i], modules[names[i]]);
    }
    // Make sure all modules have names if not explicitly provided
    for (const p in core.modules) {
        if (!core.modules[p].name) core.modules[p].name = p;
    }
