...
const core = {
    ...
    logInspect: {
        ...
        length: 512,
        replace: { " ": /[\r\n\t]+/g },
        ignore: /secret|salt|password|passwd/i,
    },
    ...
    args: [ { name: "help", type: "callback", callback: function() { this.showHelp() }, descr: "Print help and exit" },
            ...
            { name: "allow-packages", type: "list", array: 1, descr: "NPM packages to load on startup, the modules, locales, viewes, web subfolders from the package will be added automatically to the system paths, modules will be loaded if present, the config file in etc subfolder will be parsed if present", pass: 1 },
...
core.init = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            ...
            core.parseArgs(process.argv, 0, 1);
            next();
        },
        ...
        function(next) {
            ...
            var confs = [core.confFile, core.confFile + ".local"];
            for (const p in core.packages) if (core.packages[p].config) confs.push(core.packages[p].path + "/etc/config");
            lib.forEach(confs, function(file, next2) {
...
core.parseConfig = function(data, pass)
{
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (line[1]) argv.push(line.slice(1).join('=').trim().replace(/%20|%0A|%0a/g, " "));
    }
    this.parseArgs(argv, pass);
}

// Parse command line arguments
core.parseArgs = function(argv, pass, cmdline)
{
...
core.processArgs = function(ctx, argv, pass)
{
    ...
    function put(obj, key, val, x, reverse, type) {
        ...
        case "real":
        case "number":
            val = lib.toNumber(val, x.decimals, x.value, x.min, x.max);
            break;
        case "regexp":
            val = val && lib.toRegexp(val, x.regexp) || null;
            if (val && val.test()) logger.warn("processArgs:", "wide open regexp", key, val);
            break;
        case "regexpobj":
            val = lib.toRegexpObj(obj[key], val, x);
            if (val.rx && val.rx.test()) logger.warn("processArgs:", "wide open regexp", key, val);
            break;
        case "regexpmap":
            val = lib.toRegexpMap(obj[key], val, x);
            for (var i in val) {
                if (val[i].rx.test()) logger.warn("processArgs:", "wide open regexp", key, val);
            }
            break;
        case "list":
            if (val === null && x.array) break;
            val = lib.strSplitUnique(val, x.separator, x);
            break;
        case "rlist":
            if (val === null && x.array) break;
            var k = key;
            ...
            break;
        case "map":
            val = lib.strSplit(val).map(function(x) { return lib.strSplit(x, x.separator || /[,:]/, x) }).reduce(function(x,y) { x[y[0]] = y[1]; return x }, {});
            break;
        case "url":
            val = val ? url.parse(val) : val;
            break;
        case "json":
            val = val ? lib.jsonParse(val, x) : val;
            break;
        case "path":
            ...
            if (val == null) {
                ...
                if (Array.isArray(val)) {
                    val.forEach(function(y) {
                        ...
                        if (obj[key].indexOf(y) == -1) obj[key].unshift(y);
                    });
                } else {
                    if (obj[key].indexOf(val) == -1) obj[key].unshift(val);
                }
            }
        } else {
            if (val == null) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        ctx.args.forEach(function(x) {
            ...
            try {
                ...
                logger.debug("processArgs:", type || "str", ctx.name + "." + x._name, "(" + key + ")", "=", val === null ? "null" : val);
                switch (type) {
                case "none":
                    break;
                case "bool":
                ...
                case "url":
                case "json":
                    put(o.ctx, o.name, val, o, x.reverse, type);
                    break;
                case "path":
                ...
                case "callback":
                    ...
                    break;
                default:
                    put(o.ctx, o.name, val, o, x.reverse);
                }
