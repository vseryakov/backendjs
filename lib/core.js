...
const core = {
    ...
    args: [ { name: "help", type: "callback", callback: function() { this.showHelp() }, descr: "Print help and exit" },
            ...
            { name: "log-ignore", type: "regexp", obj: "logInspect", strip: /log-/, descr: "Regexp with property names which must not be exposed in the log when using custom logger inspector" },
...
core.processArgs = function(ctx, argv, pass)
{
    ...
    function put(obj, key, val, x, reverse, type) {
        ...
        case "number":
            ...
            break;
        case "regexp":
            val = lib.toRegexp(val, x.regexp) || null;
            break;
        case "regexpobj":
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        ctx.args.forEach(function(x) {
            ...
            try {
                ...
                case "url":
                case "json":
                    put(o.ctx, o.name, val, x, x.reverse, type);
                    break;
                case "path":
...
//  - errorLogger - options for the logger on error, if not specified an error with status 200 will be reported with
//    log level 'info' and other errors with level 'error', the level is specified by the 'log' property, the rest will be passed to `lib.objDescr`
...
core.runMethod = function(module, name, options, callback)
{
    ...
    logger.debug("runMethod:", core.role, name, module);
    ctx[name](options, function(err) {
        if (err) {
            var log = options.errorLogger || lib.empty;
            if (core.logInspect.ignore && !log.hide) log = lib.objClone(log, "hide", core.logInspect.ignore);
            logger.logger(log.log || (err.status == 200 ? "info" : "error"), "runMethods:", core.role, name, module, err, lib.objDescr(options, log));
            if (options.stopOnError) return lib.tryCall(callback, err);
        }
        if (typeof options.stopFilter == "function" && options.stopFilter(options)) {
