    ...
    for (var i = 0; i < argv.length; i++) {
        ...
        ctx.args.forEach(function(x) {
            ...
            try {
                ...
                if (o.ucase) o.name = o.name.replace(o.ucase, function(v) { return v.toUpperCase(); });
                if (o.lcase) o.name = o.name.replace(o.lcase, function(v) { return v.toLowerCase(); });
                ...
                case "number":
                    ...
                    break;
                case "map":
                    put(o.ctx, o.name, lib.strSplit(val).map(function(x) { return x.split(":") }).reduce(function(x,y) { if (!x[y[0]]) x[y[0]] = {}; x[y[0]][y[1]] = 1; return x }, {}), x, x.reverse);
                    break;
                case "intmap":
                    put(o.ctx, o.name, lib.strSplit(val).map(function(x) { return x.split(":") }).reduce(function(x,y) { x[y[0]] = lib.toNumber(y[1]); return x }, {}), x, x.reverse);
                break;
                case "list":
                    put(o.ctx, o.name, lib.strSplitUnique(val, x.separator), x, x.reverse);
                    break;
                case "regexp":
                ...
                logger.error("processArgs:", self.role, o.name, val, e.stack);
            }
        });
    }
}
...
//  - stopOnProperty - this must be a name of a property to watch, as soon it becomes not empty, i.e. exists or set with
//    any value the processing will be stopped.
//
core.runMethods = function(name, options, callback)
{
    ...
    lib.forEachSeries(Object.keys(this.modules), function(mod, next) {
        ...
        ctx[name](options, function(err) {
            if (err) {
                logger.error('runMethods:', self.role, name, mod, err);
                if (options.stopOnError) return next(err);
            }
            if (options.stopOnProperty && options[options.stopOnProperty]) return lib.tryCall(callback);
            next();
        });
    }, callback);
}
