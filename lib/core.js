...
//  - stopOnError - on first error stop and return, otherwise all errors are ignored and all modules are processed, if
//    an error has status 200 then it will notbe reported in the error log
...
core.runMethod = function(module, name, options, callback)
{
    ...
    ctx[name](options, function(err) {
        if (err) {
            logger.logger(err.status == 200 ? "info" : "error", "runMethods:", core.role, name, module, err, lib.objDescr(options, { hide: logger.ignore }));
