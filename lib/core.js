...
core.init = function(options, callback)
{
    ...
    lib.series([
        ...
        function(next) {
            ...
            opts.exclude = options.excludeModules || core.excludeModules;
            for (const mod of modules) core.loadModules(mod, opts);
            next();
        },
...
//
// Only top level .js files are loaded, not subdirectories. `core.addModule` is called automatically.
//
...
//
// it uses lib.findFileSync to locate the modules, options.include or options.exclude can be provided
//
