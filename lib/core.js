...
const core = {
    ...
    appVersion: process.env.BKJS_APPVERSION || '',
    appDescr: process.env.BKJS_APPDESCR || "",
    ...
    configRoles: lib.strSplit(process.env.BKJS_CONFIG_ROLES),
    ...
    domain: lib.domainName(os.hostname()),
    ...
    // Config parameters
    args: [
        { name: "help", type: "callback", callback: function() { this.showHelp() }, descr: "Print help and exit" },
        ...
        { name: "no-log-filter", type: "bool", onupdate: function(v) { if (v) logger.filters={} }, descr: "Clear all log filters", pass: 1 },
        { name: "syslog", type: "callback", callback: function(v) { logger.setSyslog(v || 1, this.name) }, descr: "Log messages to syslog, pass 0 to disable, 1 or url (tcp|udp|unix):[//host:port][/path]?[facility=F][&tag=T][&retryCount=N][&bsd=1][&rfc5424=1][&rfc3164=1]...", pass: 1 },
        { name: "syslog-options", type: "callback", callback: function(v) { logger.setSyslogOptions(v) }, descr: "Update syslog options, the format is a map: name:val,...", pass: 1 },
        ...
        { name: "web", type: "callback", callback: function() { delete this.noWeb }, descr: "Start Web server processes, spawn workers that listen on the same port, for use without master process which starts Web servers automatically" },
        ...
        { name: "no", type: "callback", callback: function(v) { lib.strSplit(v).forEach((x) => { this[lib.toCamel("no-" + x)] = true }) }, descr: "List of subsystems to disable instead of using many inidividual -no-NNN parameters" },
        ...
        { name: "no-jobs", type: "bool", descr: "Do not initialize jobs processing", pass: 1 },
        { name: "no-ipc", type: "bool", descr: "Do not initialize IPC drivers", pass: 1 },
        { name: "no-db", type: "bool", descr: "Do not initialize DB drivers", pass: 1 },
        { name: "no-dbconf", type: "bool", descr: "Do not retrieve config from the DB", pass: 1 },
        ...
        { name: "exclude-modules", type: "regexp", descr: "Modules not to load, the whole path is checked", pass: 1 },
        ...
        { name: "host-name", type: "callback", callback: function(v) { if (v) this.hostName=v;this.domain = lib.domainName(v);this._name = "hostName" }, descr: "Hostname/domain to use for communications, default is current domain of the host machine" },
        { name: "config-domain", descr: "Domain to query for configuration TXT records, must be specified to enable DNS configuration" },
        { name: "config-roles", type: "list", array: 1, descr: "Roles to assume when pulling config parameters from the config, used in config files or config database" },
...
core.init = function(options, callback)
{
    ...
    if (intf.length) {
        core.ipaddr = intf[0].address;
        core.macaddr = intf[0].mac;
        core.ipaddrs = intf.map((x) => (x.address));
    }
    ...
    lib.series([
        ...
        function(next) {
            ...
            if (util.types.isRegExp(preload)) {
        ...
        function(next) {
            ...
            if (options.noDbconf || core.noDbconf) return next();
...
core.reloadConfig = function(callback)
{
    ...
    confs.push(this.configFile, this.confFile + ".local");
