...
var client = {
    ...
    scripts: {
        ...
        limiter: [
            ...
            "   count = max;",
            "   mtime = now;",
            "   redis.call('HMSET', name, 'count', max, 'mtime', now);",
            "end;",
            "if now < mtime then",
            "   mtime = now - mtime;",
            "end;",
...
IpcRedisClient.prototype.monitorQueue = function()
{
    ...
    this.client.eval(client.scripts.monitor, 2, this.queue + ":active", now, function(err) {
...
IpcRedisClient.prototype.monitor = function()
{
    if (this.options.queue && !this._monitorTimer && this.options.visibilityTimeout) {
        this._monitorTimer = setInterval(this.monitorQueue.bind(this), this.options.monitorInterval || (this.options.visibilityTimeout * 1.25));
        this.monitorQueue();
    }
}
...
IpcRedisClient.prototype.poller = function()
{
    ...
    this.client.eval(script, 2, this.options.queue, Date.now(), function(err, data) {
        if (err) logger.error("poller:", lib.traceError(err));
        if (!data) {
        ...
        if (self.options.visibilityTimeout) {
            timer = setInterval(function() {
                self.client.zadd(self.options.queue + "#", now, data, function(err) {
                    if (err) logger.error("ipc.poller:", lib.traceError(err));
                    if (err) clearInterval(timer);
                });
            }, self.options.visibilityTimeout * 0.75);
        }
...
{
    this.client.eval(client.scripts.limier, 5, options.name, options.rate, options.max, options.interval, Date.now(), function(err, delay) {
        if (err) logger.error("limiter:", lib.traceError(err));
