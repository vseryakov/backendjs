...
IpcRedisClient.prototype.poller = function()
{
    ...
    this.client.eval(script, 2, this.options.queue, Date.now(), function(err, data) {
        if (err) return logger.error("poller:", err.stack);
        ...
        if (data) {
            ...
            if (!self.emit(msg.channel || "message", msg.data, function(err, next) {
                ...
                if (err && err.status >= 500) {
                    ...
                    if (!self.options.visibilityTimeout) return;
            ...
            var interval = self.options.interval, elapsed = Date.now() - now;
            if (elapsed >= interval) interval = 0; else interval -= elapsed;
            if (self._polling) setTimeout(self.poller.bind(self), interval);
        }
    });
}
