 ...
 * @param {function(err:object, data:object)} [callback]
...
sendmail.send = function(options, callback)
{
    ...
    try {
        let transport, opts = { dryrun: options.dryrun };
        const h = URL.parse(options.transport || this.emailTransport || "");
        const proto = h?.protocol?.slice(0, -1);
        ...
        if (!mods.nodemailer) {
            mods.nodemailer = lib.tryRequire(__dirname + "/../dist/nodemailer");
            if (!mods.nodemailer) return lib.tryCall(callback, { status: 500, message: "service unavailable" });
        }
        const mailer = mods.nodemailer.createTransport(transport || sendmail.smtp || { host: "localhost", port: 25 });
...
/**
 * Async version of {@link module:sendmail.send}
 * @param {object} options
 ...
 */
sendmail.asend = (options)=> new Promise((resolve, _reject) => {
        sendmail.send(options, (err, data) => {
            resolve({ err, data });
        });
    })
