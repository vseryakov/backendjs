...
mod.send = function(options, callback)
{
    ...
    try {
        ...
        if (!mod.nodemailer) mod.nodemailer = require("nodemailer");
        var mailer = mod.nodemailer.createTransport(transport || mod.smtp || { host: "localhost", port: 25 });
...
SendGridTransport.prototype.send = function(mail, callback)
{
    mail.normalize((err, data) => {
        ...
        app.httpGet("https://api.sendgrid.com/v3/mail/send",
            { headers: {
                Authorization: `Bearer ${this.options.apikey || mod.sendgridKey}`,
