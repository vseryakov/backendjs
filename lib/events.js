...
const events = {
    ...
    args: [
        ...
        { name: "routing", obj: "routing", type: "map", merge: 1, maptype: "regexp", descr: 'Routing map by event subject or type, ex: `-events-routing redis:local.+,nats:.+' },
...
events.initWorker = function(options, callback)
{
    ...
    }, lib.toNumber(events.workerDelay) + lib.randomShort()/1000);
 ...
 * @param {string} subject - event stream or ID, can be hierarchical like path
 * @param {string} type - event type in reverse domain notation, what happened with the subject
 ...
 * @method putEvent
 */
events.putEvent = function(subject, type, data, options)
{
    var msg, n = 0;

    if (typeof subject != "string" || typeof type != "string" || typeof data != "object") {
        logger.error("putEvent:", "invalid", subject, type, data);
        return -1;
    }

    for (const q in this.routing) {
        if (this.routing[q].test(subject) || this.routing[q].test(type)) {
            if (!msg) {
                msg = Object.assign({}, {
                    subject,
                    type,
            ...
            queue.submit(msg, options, (err) => { if (err) logger.error("putEvent:", subject, type, err, msg) });
            n++;
        }
    }
    return n;
