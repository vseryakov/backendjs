 ...
 * or USER-EVENT: and event processor(s) for corresponding events</caption>
 *
 * queue-events = nats://
 *
...
module.exports = {
    ...
    args: [
        ...
        { name: "routing", obj: "routing", type: "map", merge: 1, map_type: "regexp", descr: "Routing map by event subject or type", example: "-events-routing redis:local.+, nats:.+, sqs:billing.+" },
    ...
    shutdownTimeout: 50,
    workerDelay: 50,
    workerQueue: Object.create(null),
    workerOptions: Object.create(null),
    routing: Object.create(null),
    routingOptions: Object.create(null),
};
...
events.initWorker = function(options, callback)
{
    ...
    events._checkTimer = setInterval(this.checkTimes.bind(this), 30000).unref();
...
{
    for (const name in events.workerQueue) {
        if (/^[!-]/.test(name)) {
            events.unsubscribe(name.substr(1));
            continue;
        }
        events.subscribe(name, events.workerQueue[name]);
    }
}

/**
 * Subscribe handlers to the given queue subscription.
 * Prevent subscription more than once to the same queue in case of invalid or nonexistent queues
 * @param {string} name - queue/subscription
 * @param {string|string[]|function|function[]} handlers - handlers to process events:
 * - strings in format `module.method` will be resolved
 * - functions just added to the list
 * @param {object|function} [context] - custom context to run inside the event handler, if not provided the event handler is run in the
 * method's module context
 * @returns {undefined|string} a subscription if subscribed or undefined
 * @memberof module:events
 * @method subscribe
 *
 */
events.subscribe = function(name, handlers, context)
{
    const q = queue.getQueue(name);
    const sub = q.subscription(name);
    if (events.subscribed.has(sub)) return;

    context = lib.isObject(context, lib.isFunc(context));

    const procs = [];
    for (const handler of lib.isArray(handlers, [handlers])) {
        if (typeof handler === "function") {
            procs.push(context ? handler.bind(context) : handler);
        } else {
            const [mod, method] = app.getMethodContext(handler);
            if (!mod) {
                logger.error("subscribe:", events.name, q.name, name, "invalid event proc:", handler);
                continue;
            }
            procs.push(mod[method].bind(context || mod));
        }
    }
    if (!procs.length) return;

    const qopts = Object.assign({ queueName: name }, handlers);
    queue.listen(qopts, events.processEvent.bind(events, sub, procs));
    events.subscribed.add(sub);
    logger.info("subscribe:", events.name, q.name, sub, handlers);
    return sub;
}
 ...
 * @param {string} name - queue/subscription
 * @returns {undefined|string} a subscription if unsubscribed or undefined
 * @memberof module:events
 * @method unsubscribe
 */
events.unsubscribe = function(name)
{
    const q = queue.getClient(name);
    const sub = q.subscription(name);
    if (!events.subscribed.delete(sub)) return;
    queue.unlisten({ queueName: name });
    logger.info("unsubscribe:", events.name, q.name, sub);
    return sub;
}
 ...
 * @param {string|object|Array} data - payload to be placed as the `data` property
...
events.putEvent = function(subject, data, options, callback)
{
    ...
    data = lib.isString(data) || lib.isArray(data) || lib.isObject(data);
    if (!data) {
 ...
 * @param {string} subject - event subject, topic, ID, ...
 * @param {object|string} data - payload to be placed as the `data` property
 * @param {object} [options] - queue specific properties
 ...
 * @async
 */
events.aputEvent = function(subject, data, options)
{
