...
mod.shutdownWorker = function(options, callback)
{
    ...
    for (const q in this.workerQueue) cache.unsubscribeQueue({ queueName: q });
...
mod.subscribeWorker = function()
{
    for (const name in this.workerQueue) {
        ...
        cache.subscribeQueue(qopts, this.processEvent.bind(this, name, procs));
...
mod.putEvent = function(topic, data, options)
{
    ...
    for (const q in this.routing) {
        if (this.routing[q].test(topic)) {
            ...
            cache.publishQueue(msg, { queueName: q }, (err) => { if (err) logger.error("putEvent:", topic, err, msg) });
            n++;
        }
    }
