...
const mod = {
    args: [
        ...
        { name: "max-runtime", type: "int", min: 0, descr: "Max number of seconds an event processing can run before being killed" },
    ...
    for (const name in this.workerQueue) {
        ...
        for (const i in this.workerQueue[name]) {
            ...
            if (!core.modules[proc[0]] || typeof core.modules[proc[0]][proc[1]] != "function") {
                logger.error("subscribeWorker:", mod.name, q.name, name, "invalid event proc:", proc);
                continue;
            }
            var qopts = lib.objExtend({ queueName: name }, this.workerOptions[name]);
            if (!qopts.groupName) qopts.groupName = this.workerQueue[name];
            ipc.subscribeQueue(qopts, this.processEvent.bind(this, name, proc));
            this.subscribed.add(q.canonical(name));
            logger.info("subscribeWorker:", mod.name, q.name, name, proc);
        }

        q.on("closed", () => {
            this.unsubscribeQueue(name);
            if (this._subscribeTimer) clearTimeout(this._subscribeTimer);
            this._subscribeTimer = setTimeout(this.subscribeWorker.bind(this), 1000);
        });
    }
}
...
mod.processEvent = function(name, proc, event, callback)
{
    var evt = { name, proc, event, time: Date.now() };
    this.running.add(evt);
    this.runTime = Date.now();
    logger.debug("processEvent:", mod.name, evt);

    try {
        core.modules[proc[0]][proc[1]](event, (err) => {
...
mod.putEvent = function(topic, data, options)
{
    ...
    for (const q in this.routing) {
        if (this.routing[q].test(topic)) {
            ipc.publishQueue(msg, { queueName: q }, (err) => { if (err) logger.error("putEvent:", topic, err, data) });
            n++;
        }
    }
    logger.logger(n ? "debug" : "warn", "putEvent:", topic, "ignored", data);
    return n;
}
