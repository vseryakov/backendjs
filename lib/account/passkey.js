...
mod.passkey.read = function(user, passkeyId, callback)
{
    mod.get(user, (err, user) => {
        var passkey = lib.jsonParse(user?.passkey, { datatype: "list" }).filter((x) => (x.id == passkeyId)).pop();
        callback(err, user, passkey);
    });
}
...
{
    var allkeys = lib.jsonParse(account?.passkey, { datatype: "list" });
