...
//  - account_id - an account id associated with this token, for debugging and invalid token management(feedback)
...
Msg.prototype.send = function(device, options, callback)
{
    ...
    logger.debug("send:", device, options);

    var apps = [], dflts = [], devs = [], services = lib.strSplit(options.service_id);
