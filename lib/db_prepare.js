...
db.prepare = function(op, table, obj, options)
{
    ...
    table = pool.resolveTable(op, String(table), obj, options);
...
db.prepareForUpdate = function(pool, req)
{
    ...
    for (const p in req.obj) {
        ...
        if (col) {
            ...
            if (lib.isArray(col.values) && col.values.indexOf(String(v)) == -1) continue;
...
db.prepareForList = function(pool, req)
{
    ...
    for (var i = 0; i < req.obj.length; i++) {
        ...
        for (const p in columns) {
            ...
            // Case conversion
            if (col.lower && typeof row[p] == "string") row[p] = row[p].toLowerCase();
            if (col.upper && typeof row[p] == "string") row[p] = row[p].toUpperCase();
        ...
        if (!opts.noconvertrows_json) {
            if ((pool.configOptions.noJson && col.type == "json") ||
                ...
                for (i = 0; i < rows.length; i++) {
                    if (typeof rows[i][p] == "string" && rows[i][p]) rows[i][p] = lib.jsonParse(rows[i][p], { logger: "error", dflt: col.dflt });
                }
            }
        }
