...
class APNPushClient {
     ...
     * @param {int} [options.badge] - badge number
     * @param {boolean|string} [options.sound] - true or a sound file to play
     ...
     * @param {string} [options.payload] - an object with additional fields to send in the message payload
    ...
    */
    send(device, options, callback)
    {
        ...
        var hex = null;
        try { hex = Buffer.from(device.token, "hex"); } catch (_e) {}
        if (!hex) return callback(lib.newError("invalid device token", { device }));

        const agent = this.agents[device.app] || this.agents.default;
