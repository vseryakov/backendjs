 ...
 *
 * Config must have this entry for default included notfications worker to work:
 *
 * ```
 * api.app.get("/js/webpush.js", (context, next) {
 *     context.setHeader("Service-Worker-Allowed", "/");
 *     next();
 * });
 * ```
 *
...
class WebPushClient {
    ...
    constructor(options) {
        ...
        for (const agent of options) {
            ...
            logger.debug("webpush:", agent.app, options);
        }
    }
