...
db.mysqlInitPool = function(options)
{
    ...
    options.type = "mysql";
    options.settings = { typesMap: { json: "text", bigint: "bigint" }, sqlPlaceholder: "?", defaultType: "VARCHAR(128)", noIfExists: 1, noJson: 1, noMultiSQL: 1 };
    var pool = this.sqlInitPool(options);
    pool.open = function(opts, callback) {
        if (opts.url == "default") opts.url = "mysql:///" + self.dbName;
        new utils.MysqlDatabase(opts.url, function(err) {
            callback(err, this);
        });
    }
